...
- **동작**:
  ...
  - **일반 PR**: 두 TC repo에 `tc/pr-<N>` 생성 (이미 존재하면 skip)
  - **Revert PR**: 원본 PR의 TC merge commit을 `git revert -m 1`으로 취소 후 `tc/pr-<N>`에 push
    - 브랜치 존재 시 force-push, 없으면 신규 생성
    - 원본 PR에 TC 변경 없으면 silently skip
    - conflict 시 PR에 수동 처리 요청 코멘트 게시
  - 실패 시 PR에 에러 코멘트 게시
- **concurrency**: 동일 PR 번호 중복 실행 시 이전 것 취소

### `tc-branch-finalize.yml`
- **트리거**: `pull_request_target` (closed)
- **merged == true**: `tc/pr-<N>` → develop `--no-ff` 머지 후 브랜치 삭제. conflict 시 PR에 코멘트
- **merged == false**: `tc/pr-<N>` 브랜치만 삭제 (GitHub REST API `DELETE /repos/.../git/refs/heads/...` 사용)
- 브랜치 없으면 skip
