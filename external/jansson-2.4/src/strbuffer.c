...
void
strbuffer_close (strbuffer_t * strbuff)
{
...
void
strbuffer_clear (strbuffer_t * strbuff)
{
  strbuff->length = 0;
  strbuff->value[0] = '\0';
}
...
int
strbuffer_append (strbuffer_t * strbuff, const char *string)
{
  return strbuffer_append_bytes (strbuff, string, strlen (string));
}

int
strbuffer_append_byte (strbuffer_t * strbuff, char byte)
{
  ...
  if (size >= strbuff->size - strbuff->length)
    {
	  ...
	  || size > STRBUFFER_SIZE_MAX - 1 || strbuff->length > STRBUFFER_SIZE_MAX - 1 - size)
	return -1;
