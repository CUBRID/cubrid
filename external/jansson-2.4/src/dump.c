...
static int
dump_to_strbuffer (const char *buffer, size_t size, void *data)
{
  return strbuffer_append_bytes ((strbuffer_t *) data, buffer, size);
}

static int
dump_to_file (const char *buffer, size_t size, void *data)
{
  FILE *dest = (FILE *) data;
...
static int
dump_indent (size_t flags, int depth, int space, json_dump_callback_t dump, void *data)
{
...
static int
dump_string (const char *str, json_dump_callback_t dump, void *data, size_t flags)
{
  const char *pos, *end;
  int32_t codepoint;
...
static int
object_key_compare_keys (const void *key1, const void *key2)
{
  return strcmp (((const struct object_key *) key1)->key, ((const struct object_key *) key2)->key);
}

static int
object_key_compare_serials (const void *key1, const void *key2)
{
...
static int
do_dump (const json_t * json, size_t flags, int depth, json_dump_callback_t dump, void *data)
{
	...
	if (flags & JSON_SORT_KEYS || flags & JSON_PRESERVE_ORDER)
	  {
		...
		if (dump (separator, separator_length, data) || do_dump (value, flags, depth + 1, dump, data))
		  {
		    jsonp_free (keys);
		    goto object_error;
		  }
	...
	else
	  {
		...
		if (dump (separator, separator_length, data) ||
		    do_dump (json_object_iter_value (iter), flags, depth + 1, dump, data))
		  goto object_error;
...
int
json_dump_callback (const json_t * json, json_dump_callback_t callback, void *data, size_t flags)
{
