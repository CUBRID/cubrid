...
camellia_setkey (void *c, const byte * key, unsigned keylen)
{
  ...
  _gcry_burn_stack ((19 + 34 + 34) * sizeof (u32) + 2 * sizeof (void *)	/* camellia_setup256 */
...
camellia_encrypt (void *c, byte * outbuf, const byte * inbuf)
{
  ...
  _gcry_burn_stack (sizeof (int) + 2 * sizeof (unsigned char *) + sizeof (KEY_TABLE_TYPE) + 4 * sizeof (u32) + 2 * sizeof (u32 *) + 4 * sizeof (u32) + 2 * 2 * sizeof (void *)	/* Function calls.  */
    );
}
...
gcry_cipher_spec_t _gcry_cipher_spec_camellia128 = {
  ...
  sizeof (CAMELLIA_context), camellia_setkey, camellia_encrypt,
  camellia_decrypt
};
