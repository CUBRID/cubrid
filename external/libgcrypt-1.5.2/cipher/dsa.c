...
gen_k (gcry_mpi_t q)
{
  ...
  for (;;)
      ...
      if (!rndbuf || nbits < 32)
	{
	  gcry_free (rndbuf);
	  rndbuf =
	    gcry_random_bytes_secure ((nbits + 7) / 8, GCRY_STRONG_RANDOM);
	}
      else
		  ...
		  gcry_sexp_t * r_extrainfo)
{
  ...
  if (deriveparms || use_fips186 || use_fips186_2 || fips_mode ())
    {
      ...
      ec = generate_fips186 (&sk, nbits, qbits, deriveparms, use_fips186_2,
			     &domain, &counter, &seed, &seedlen, &h_value);
      gcry_sexp_release (deriveparms);
      if (!ec && h_value)
	{
  ...
  if (!ec)
      ...
      else
	{
	  ...
	  for (nfactors = 0; *retfactors && (*retfactors)[nfactors];
	       nfactors++)
	    ;
