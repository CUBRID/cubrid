...
#ifdef USE_BARRETT
static void barrett_mulm (gcry_mpi_t w, gcry_mpi_t u, gcry_mpi_t v,
			  gcry_mpi_t m, gcry_mpi_t y, int k, gcry_mpi_t r1,
			  gcry_mpi_t r2);
static gcry_mpi_t init_barrett (gcry_mpi_t m, int *k, gcry_mpi_t * r1,
				gcry_mpi_t * r2);
static int calc_barrett (gcry_mpi_t r, gcry_mpi_t x, gcry_mpi_t m,
 ...
 */
void
_gcry_mpi_mulpowm (gcry_mpi_t res, gcry_mpi_t * basearray,
	      ...
	      for (j = 0; j < k; j++)
		{
		  if ((idx & (1 << j)))
			...
			barrett_mulm (G[idx], G[idx], basearray[j],
				      m, barrett_y, barrett_k, barrett_r1,
				      barrett_r2);
		    }
		}
	      if (!G[idx])
...
#ifdef USE_BARRETT
static void
barrett_mulm (gcry_mpi_t w, gcry_mpi_t u, gcry_mpi_t v, gcry_mpi_t m,
	      gcry_mpi_t y, int k, gcry_mpi_t r1, gcry_mpi_t r2)
{
  mpi_mul (w, u, v);
 ...
 */
static int
calc_barrett (gcry_mpi_t r, gcry_mpi_t x, gcry_mpi_t m, gcry_mpi_t y, int k,
	      gcry_mpi_t r1, gcry_mpi_t r2)
{
  int xx = k > 3 ? k - 3 : 0;
