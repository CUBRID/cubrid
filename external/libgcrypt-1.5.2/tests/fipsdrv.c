  ...
  if (decode)
    {
      ...
      for (s = buffer, p = buffer, nread = 0; nread + 1 < buflen;
	   s += 2, nread += 2)
	{
	  if (!hexdigitp (s) || !hexdigitp (s + 1))
	    {
...
read_private_key_file (const char *fname, int show)
{
  ...
  for (idx = 0; idx < n_keyparms; idx++)
      ...
      if (show)
	{
      ...
      err =
	gcry_mpi_scan (keyparms + idx, GCRYMPI_FMT_USG, der, ti.length, NULL);
      if (err)
...
read_sig_file (const char *fname)
{
  ...
  if (!err)
    err = gcry_sexp_build (&s_sig, NULL, "(sig-val(rsa(s %m)))", tmpmpi);
  if (err)
    die ("error building S-expression: %s\n", gpg_strerror (err));
			...
			const void *dt, size_t dtlen)
{
  return gcry_control (PRIV_CTL_INIT_EXTRNG_TEST,
		       r_context, flags,
		       key, keylen, seed, seedlen, dt, dtlen);
}
...
dsa_gen (int keysize)
{
  ...
  err = gcry_sexp_build (&keyspec, NULL,
			 "(genkey (dsa (nbits %d)(use-fips186-2)))", keysize);
  if (err)
...
dsa_gen_with_seed (int keysize, const void *seed, size_t seedlen)
{
  ...
  err = gcry_sexp_build (&keyspec, NULL,
			 ...
			 "    (use-fips186-2)"
			 "    (derive-parms"
			 "      (seed %b))))", keysize, (int) seedlen, seed);
  if (err)
    die ("gcry_sexp_build failed for DSA key generation: %s\n",
	 gpg_strerror (err));
...
main (int argc, char **argv)
{
  ...
  if (!chunksize
      ...
      && strcmp (mode_string, "rsa-gen") && strcmp (mode_string, "dsa-gen"))
    {
