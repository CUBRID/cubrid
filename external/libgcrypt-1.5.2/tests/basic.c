...
check_aes128_cbc_cts_cipher (void)
{
  ...
  for (i = 0; i < sizeof (tv) / sizeof (tv[0]); i++)
	...
	fprintf (stderr, "    checking encryption for length %i\n",
		 tv[i].inlen);
      err =
	gcry_cipher_encrypt (hd, out, MAX_DATA_LEN, plaintext, tv[i].inlen);
      if (err)
	{
...
check_ctr_cipher (void)
{
  ...
  } tv[] =
  {
	...
	"\x96\xe9\x3d\x7e\x11\x73\x93\x17\x2a",
	    9, "\x26\x1b\xef\x68\x64\x99\x0d\xb6\xce"},
	{
	"\xae\x2d\x8a\x57\x1e\x03\xac\x9c\x9e\xb7\x6f\xac\x45\xaf\x8e",
	    15,
	    "\x98\x06\xf6\x6b\x79\x70\xfd\xff\x86\x17\x18\x7b\xb9\xff\xfd"},
	...
	"\x51\x30\xc8\x1c\x46\xa3\x5c\xe4\x11",
	    9, "\xff\x5a\xe4\xdf\x3e\xdb\xd5\xd3\x5e"},
	{
	"", 0, ""}
      }
    },
	...
	"\xae\x2d\x8a\x57\x1e\x03\xac\x9c",
	    8, "\x16\xe8\x72\x77\xb0\x98\x29\x68"},
	{
	"\x9e\xb7\x6f\xac\x45\xaf\x8e\x51",
	    8, "\x9a\xb3\xa8\x03\x3b\xb4\x14\xba"},
	{
	"\xae\x2d\x8a\x57\x1e\x03\xac\x9c\xa1\x00",
	    10, "\x31\x5e\xd3\xfb\x1b\x8d\xd1\xf9\xb0\x83"},
	{
	"", 0, ""}
      }
    },
  ...
  for (i = 0; i < sizeof (tv) / sizeof (tv[0]); i++)
      ...
      for (j = 0; tv[i].data[j].inlen; j++)
	{
	  err = gcry_cipher_encrypt (hde, out, MAX_DATA_LEN,
				     ...
				     tv[i].data[j].inlen == -1 ?
				     strlen ((char *) tv[i].data[j].
					     plaintext) : tv[i].data[j].
				     inlen);
	  if (err)
	    {
...
check_bulk_cipher_modes (void)
{
  ...
  } tv[] =
  {
	...
	"abcdefghijklmnop", 16, "1234567890123456", 16,
/*[0]*/
      {
	...
	"abcdefghijklmnopABCDEFG", 24, "1234567890123456", 16,
/*[1]*/
      {
	...
	"abcdefghijklmnopABCDEFGHIJKLMNOP", 32, "1234567890123456", 16,
/*[2]*/
      {
  ...
  for (i = 0; i < DIM (tv); i++)
    {
      if (verbose)
	fprintf (stderr,
		 "    checking bulk encryption for %s [%i], mode %d\n",
...
check_one_cipher (int algo, int mode, int flags)
{
  ...
  for (bufshift = 0; bufshift < 4; bufshift++)
	...
	return;
      ...
      if (check_one_cipher_core
	  (algo, mode, flags, key + 1, 32, plain + 1, 16, bufshift,
	   2 + 10 * bufshift))
	return;
...
check_hmac (void)
{
  ...
  for (i = 0; algos[i].md; i++)
	...
	fprintf (stderr,
		 ...
		 gcry_md_algo_name (algos[i].md),
		 algos[i].md, strlen (algos[i].key), strlen (algos[i].data));
...
check_pubkey_crypt (int n, gcry_sexp_t skey, gcry_sexp_t pkey, int algo)
{
  ...
  } datas[] =
  {
    ...
    0, "(data\n (flags raw)\n"
	" (value #11223344556677889900AA#))\n",
	"(flags oaep)", 1, 0, GPG_ERR_ENCODING_PROBLEM},
    {
    GCRY_PK_RSA,
	"(data\n (flags oaep)\n"
	" (value #11223344556677889900AA#))\n",
	"(flags pkcs1)", 1, 0, GPG_ERR_ENCODING_PROBLEM},
    {
    0, "(data\n (flags pss)\n"
...
get_keys_new (gcry_sexp_t * pkey, gcry_sexp_t * skey)
{
  ...
  rc = gcry_sexp_new (&key_spec,
		      ...
		      : "(genkey (rsa (nbits 4:1024)(transient-key)))", 0, 1);
  if (rc)
      ...
      else
	{
	  ...
	  if (err)
	  ...
	  else
	    {
	      ...
	      if (gcry_control (GCRYCTL_OPERATIONAL_P, 0))
		fail
		  ("expected error state but still in operational state\n");
	      else
		{
