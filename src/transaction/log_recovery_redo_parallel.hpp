	...
	bool force_each_log_page_fetch)
{
  ...
  if (log_recovery_redo_perf_stat_values != nullptr)
    {
      log_recovery_redo_perf_stat_definition->time_and_increment (
	      *log_recovery_redo_perf_stat_values, PERF_STAT_ID_REDO_OR_PUSH_PREP);
    }
  ...
  if (parallel_recovery_redo == nullptr || need_sync_redo)
	...
	{
	  log_recovery_redo_perf_stat_definition->time_and_increment (
		  *log_recovery_redo_perf_stat_values, PERF_STAT_ID_REDO_OR_PUSH_DO_SYNC);
	}
    }
  else
    {
	...
	{
	  log_recovery_redo_perf_stat_definition->time_and_increment (
		  *log_recovery_redo_perf_stat_values, PERF_STAT_ID_REDO_OR_PUSH_DO_ASYNC);
	}
    }
