	...
	bool force_each_log_page_fetch)
{
  ...
  log_recovery_redo_perf_stat_ptr->time_and_increment (PERF_STAT_ID_REDO_OR_PUSH_PREP);
  ...
  if (parallel_recovery_redo == nullptr || need_sync_redo)
    {
      ...
      log_recovery_redo_perf_stat_ptr->time_and_increment (PERF_STAT_ID_REDO_OR_PUSH_DO_SYNC);
    }
  else
    {
      // dispatch async
	...
	new redo_job_impl_t (rcv_vpid, rcv_lsa, end_redo_lsa, log_rtype, force_each_log_page_fetch)
      ...
      log_recovery_redo_perf_stat_ptr->time_and_increment (PERF_STAT_ID_REDO_OR_PUSH_DO_ASYNC);
    }
