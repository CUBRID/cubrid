...
struct boot_dbparm
{
  ...
  VOLID temp_nvols;		/* Number of temporary volumes that have been
				 * created */
  VOLID last_volid;		/* Next volume identifer */
  VOLID temp_last_volid;	/* Next temporary volume identifer. This goes
...
static VOLID boot_add_volume (THREAD_ENTRY * thread_p,
			      ...
			      const char *vol_comments, DKNPAGES npages,
			      DISK_VOLPURPOSE vol_mpurpose,
			      bool do_overwrite);
...
static int boot_xremove_temp_volume (THREAD_ENTRY * thread_p, VOLID volid,
				     ...
				     void *ignore_arg);
static void boot_remove_unknown_temp_volumes (void);
...
static bool boot_remove_useless_path_separator (char *new_path,
						const char *path);
 ...
 */
static int
boot_server (const int up_or_down)
{
  boot_Server_up = up_or_down;
...
{
  if (BO_ISSERVER_RESTARTED () && boot_Server_process_id == getpid ())
    {
		  ...
		  OID * dbparm_oid)
{
  ...
  if (heap_first
      (thread_p, &boot_Db_parm->hfid, NULL, dbparm_oid, &recdes, NULL,
       COPY) != S_SUCCESS)
    {
      return ER_FAILED;
    }
 ...
 *
 *   vol_fullname(in): Full name of volume to be created
 *   vol_comments(in): Optional comments that are included in the header of the
 *                     volume.
 *   npages(in): Size on the volume in pages
 *   vol_mpurpose(in): The main purpose of the volume
 *   do_overwrite(in): If the volume exist, can it be overwritten?
 *
 *
 ...
 */
static VOLID
boot_add_volume (THREAD_ENTRY * thread_p, const char *vol_fullname,
		 const char *vol_comments, DKNPAGES npages,
		 DISK_VOLPURPOSE vol_mpurpose, bool do_overwrite)
{
  VOLID volid;
  bool ignore_old;
  ...
  if (vol_mpurpose != DISK_TEMPVOL_TEMP_PURPOSE)
    {
      volid = boot_Db_parm->last_volid + 1;
  ...
  if (do_overwrite == false && fileio_is_volume_exist (vol_fullname))
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_BO_VOLUME_EXISTS, 1, vol_fullname);
  ...
  /* Format the volume */
  if (disk_format
      (thread_p, boot_Db_full_name, volid, vol_fullname, vol_comments, npages,
       vol_mpurpose) == NULL_VOLID)
    {
      (void) log_end_system_op (thread_p, LOG_RESULT_TOPOP_ABORT);
      goto error;
    }

  if (logpb_add_volume (NULL, volid, vol_fullname, vol_mpurpose) != volid)
    {
      (void) log_end_system_op (thread_p, LOG_RESULT_TOPOP_ABORT);
      goto error;
    }
  ...
  if (heap_update (thread_p, &boot_Db_parm->hfid, boot_Db_parm_oid,
		   &recdes, &ignore_old, NULL) == NULL)
    {
  ...
  (void) fileio_synchronize (fileio_get_volume_descriptor
			     (boot_Db_parm->hfid.vfid.volid),
			     !PRM_SUPPRESS_FSYNC);
...
boot_remove_volume (THREAD_ENTRY * thread_p, VOLID volid)
{
  ...
  if (heap_update (thread_p, &boot_Db_parm->hfid, boot_Db_parm_oid, &recdes,
 ...
 * return : volid or NULL_VOLID (in case of failure)
 *
 *   ext_path(in): Directory where the database extension is created.
 *                 If NULL, is given, it defaults to the system parameter.
 *   ext_name(in): Name of the volume extension
 *   ext_comments(in): Comments which are included in the volume extension
 *                     header.
 *   ext_npages(in): Number of pages
 *   ext_overwrite(in): If the volume exist, can it be overwritten?
 *
 ...
 */
VOLID
xboot_add_volume_extension (THREAD_ENTRY * thread_p, const char *ext_path,
			    const char *ext_name, const char *ext_comments,
			    DKNPAGES ext_npages, DISK_VOLPURPOSE ext_purpose,
			    bool ext_overwrite)
{
  char real_pathbuf[PATH_MAX];

  if (ext_path != NULL && realpath ((char *) ext_path, real_pathbuf) != NULL)
    {
      ext_path = real_pathbuf;
    }

  return boot_xadd_volume_extension (thread_p, ext_path, ext_name,
				     ext_comments, ext_npages, ext_purpose,
				     ext_overwrite);
}

/*
 * boot_remove_useless_path_separator () - Remove useless PATHSLASH in path string
 *
 ...
 *
 * Note: This function removes uselses PATHSLASH in path string.
 *       For example,
 ...
 */
static bool
boot_remove_useless_path_separator (char *new_path, const char *path)
{
  ...
  while (*path == PATHSLASH)
    {
      slash_num++;
      path++;
    }

  /* If there is preceding consumed slash, append PATHSLASH */
  if (slash_num)
    {
      *new_path++ = PATHSLASH;
#if defined(WINDOWS)
      /*
       * In Windows/NT,
       * If first duplicated PATHSLASHs are appeared, they are survived.
       * For example,
       * \\pooh\user\ -> \\pooh\user(don't touch the first duplicated PATHSLASHs)
       */
      if (slash_num > 1)
	{
	  *new_path++ = PATHSLASH;
  ...
  /*
   * If current character is PATHSLASH,
  ...
  while (*path)
	...
	{
	  /*
	   * If there is consumed slash, append PATHSLASH.
 ...
 */
static VOLID
boot_xadd_volume_extension (THREAD_ENTRY * thread_p, const char *ext_path,
			    const char *ext_name, const char *ext_comments,
			    DKNPAGES ext_npages, DISK_VOLPURPOSE ext_purpose,
			    bool ext_overwrite)
{
  ...
  if (ext_name == NULL)
    {
      ...
      fileio_make_volume_ext_name (vol_fullname, ext_path, ext_name,
				   boot_Db_parm->last_volid + 1);
    }
  else
    {
      fileio_make_volume_ext_given_name (vol_fullname, ext_path, ext_name);
    }
  ...
  if (stat (vol_fullname, &stat_buf) == 0	/* file exist */
    ...
    {				/* is the raw device */
      ...
      ext_path = fileio_get_directory_path (link_path, boot_Db_full_name);
      if (ext_path == NULL)
	{
	  link_path[0] = '\0';
	  ext_path = link_path;
      ...
      ext_name = fileio_get_base_file_name (boot_Db_full_name);
      fileio_make_volume_ext_name (link_fullname, ext_path, ext_name,
				   boot_Db_parm->last_volid + 1);
				...
				DISK_SETPAGE_TYPE setpage_type)
{
  ...
  if (ext_npages < BOOT_VOLUME_MINPAGES)
    {
  ...
  volid =
    boot_xadd_volume_extension (thread_p, NULL, NULL,
				"Automatic Volume Extension", ext_npages,
				DISK_PERMVOL_GENERIC_PURPOSE, false);

  if (volid != NULL_VOLID)
    {
      er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE,
	      ER_BO_NOTIFY_AUTO_VOLEXT, 2, fileio_get_volume_label (volid),
	      ext_npages);
    }
				  ...
				  const char *filename_addmore_vols)
{
  ...
  while ((line = fgets (input_buffer, BOOTSR_MAX_LINE, fp)) != NULL)
      ...
      if (ext_npages <= 0)
	{
      ...
      if (xboot_add_volume_extension
	  (thread_p, ext_path, ext_name, ext_comments, ext_npages,
	   ext_purpose, false) == NULL_VOLID)
	{
	  error_code = ER_FAILED;
	}
    }
...
boot_add_temp_volume (THREAD_ENTRY * thread_p, DKNPAGES min_npages)
{
  ...
  if (boot_Temp_volumes_max_pages == -2)
    {
      /*
       * Get the maximum number of temporary pages that can be allocted for
  ...
  if (boot_Temp_volumes_sys_pages == 0)
    {
      ...
      if (disk_set_alloctables (DISK_TEMPVOL_TEMP_PURPOSE, 0, 0,
				&sect_alloctb_npages,
				&page_alloctb_npages,
...
boot_remove_all_temp_volumes (THREAD_ENTRY * thread_p)
{
  ...
  if (boot_Db_parm->temp_nvols != 0
      || boot_Db_parm->temp_last_volid != NULL_VOLID)
    {
      ...
      recdes.data = (char *) boot_Db_parm;
      if (heap_update
	  (thread_p, &boot_Db_parm->hfid, boot_Db_parm_oid, &recdes,
...
/*
 * bo_rem_unknown_tempvols () -
 *
 * return: none
 */
static void
boot_remove_unknown_temp_volumes (void)
{
  VOLID temp_volid;
  char temp_vol_fullname[PATH_MAX];
  ...
  for (i = 0; i < 2; i++, temp_volid--)
	...
	{
	  fileio_unformat (temp_vol_fullname);
	}
    }
...
/*
 * boot_shrink_temp_volume () - shrink temporarily volumes to the database
 *
 * return : NO_ERROR if all OK, ER_ status otherwise
 *
 * Note:
 *    Remove or truncate volumes if the followings are all true.
 *    - free page ratio is more than 20% of the total pages
 *    - last allocated pageid is deallocated
 */
int
boot_shrink_temp_volume (THREAD_ENTRY * thread_p)
{
  VOLID volid, nvols;
  DKNPAGES total_pgs, free_pgs, npages;
  VPID last_vpid;
  bool vpid_decreased;
  bool cont = true;
  int error_code = NO_ERROR;
  ...
  if (disk_get_all_total_free_numpages
      (thread_p, DISK_TEMPVOL_TEMP_PURPOSE, &nvols, &total_pgs,
       &free_pgs) < 0)
    {
      error_code = ER_FAILED;
      goto end;
  ...
  if (nvols == 0 || free_pgs < (total_pgs * 0.2))
    {
      goto end;
    }

  error_code = disk_get_temporarytmp_shrink_info (&last_vpid,
						  &vpid_decreased);
  if (error_code != NO_ERROR)
    {
      goto end;
  ...
  for (volid = boot_Db_parm->temp_last_volid;
       cont == true && volid <= LOG_MAX_DBVOLID; volid++)
    {
      npages = disk_shrink_tmp (thread_p, volid, &cont);
      if (npages < NULL_PAGEID)
	{
	  csect_exit (CSECT_BOOT_SR_DBPARM);
	  error_code = ER_FAILED;
	  goto end;
				  ...
				  void *args)
{
  ...
  if (newvolpath || !use_volinfo
      || (num_vols =
	  logpb_scan_volume_info (thread_p, NULL, volid, LOG_DBFIRST_VOLID,
				  fun, args)) == -1)
    {
      /*
      ...
      do
	{
	  ...
	  if (next_volid != volid)
	    {
	      error_code =
		(*fun) (thread_p, next_volid, next_vol_fullname, args);
	      if (error_code != NO_ERROR)
		{
		  goto end;
		}
	...
	{
	  error_code =
	    boot_find_rest_permanent_volumes (thread_p, newvolpath, false,
					      volid, fun, args);
	  if (error_code != NO_ERROR)
	    {
  ...
  if (num_vols != boot_Db_parm->nvols)
    {
      er_set (ER_SYNTAX_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_BO_INCONSISTENT_NPERM_VOLUMES, 2, num_vols,
	      boot_Db_parm->nvols);
      error_code = ER_BO_INCONSISTENT_NPERM_VOLUMES;
      goto end;
    }
	    ...
	    void *ignore_arg)
{
  ...
  if (fileio_mount (boot_Db_full_name, vlabel, volid,
			 ...
			 TRAN_ISOLATION client_isolation)
{
  ...
  if (log_path == NULL)
    {
      /*
       * nop, do not set as ""
       * if set as "", program flow may be changed.
       * Thus, side effect will be appeared.
       */
      ;
    }
  else
   ...
   * with event handling.
   */
  er_save_dbname (boot_Db_full_name);
...
/* Temporary fix for NT file locking problem */
      || (dbtxt_vdes = fileio_mount (dbtxt_label, dbtxt_label,
  ...
  if (dbtxt_vdes != NULL_VOLDES)
    {
      fileio_dismount (dbtxt_vdes);	/* unlock the directory file */
  ...
  if ((int) strlen (boot_Db_full_name) > DB_MAX_PATH_LENGTH)
    {
...
exit_on:
  ...
  if (new_db_path)
    {
      free_and_init (new_db_path);
    }

  if (new_log_path)
    {
      free_and_init (new_log_path);
    }
...
error:
  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_BO_UNABLE_TO_RESTART_SERVER,
	  0);
  return error_code;
}
 ...
 * return : true
 *
 *   iserfinal(in): Terminate the error module..
 *
 ...
 */
bool
xboot_shutdown_server (THREAD_ENTRY * thread_p, bool iserfinal)
{
  if (BO_ISSERVER_RESTARTED ())
    {
#if defined(CUBRID_DEBUG)
      ...
      (void) boot_remove_all_temp_volumes (thread_p);
      log_final (thread_p);
      boot_server_all_finalize (thread_p, iserfinal);
    }
  return true;
}
 ...
 *                         TRAN_COMMIT_CLASS_COMMIT_INSTANCE
 *                         TRAN_COMMIT_CLASS_UNCOMMIT_INSTANCE
 *   transtate(in): State of transaction (Set as a side effect)
 *   current_pagesize(in): Size of the database (Set as a side effect)
 *
...
/*
 * boot_server_all_finalize () - terminate every single module except the log/recovery
 *                         manager
 *   iserfinal(in): Terminate the error module..
 *
 ...
 */
void
boot_server_all_finalize (THREAD_ENTRY * thread_p, bool iserfinal)
{
	      ...
	      int skip_activelog, PAGEID safe_pageid)
{
  ...
  error_code =
    logpb_backup (thread_p, boot_Db_parm->nvols, backup_path, backup_level,
	    ...
	    const char *fileof_vols_and_copypaths, bool newdb_overwrite)
{
  ...
  if (newdb_path == NULL || fileof_vols_and_copypaths != NULL)
    {
      /*
       * If a newdb path was given, it is ignored since only one option must
       * be specified
       */
      newdb_path = boot_find_new_db_path (newdb_pathbuf,
  ...
  else if (realpath ((char *) newdb_path, newdb_pathbuf2) != NULL)
    {
      newdb_path = newdb_pathbuf2;
    }

  if (newlog_path != NULL
      && realpath ((char *) newlog_path, newlog_pathbuf) != NULL)
    {
      newlog_path = newlog_pathbuf;
    }

  if (newlog_path == NULL)
    {
      /* Assign the data volume directory */
      strcpy (newlog_pathbuf, newdb_path);
      newlog_path = newlog_pathbuf;
    }
  ...
  if ((int) (strlen (newdb_name) + strlen (newdb_path) + 2)
      > DB_MAX_PATH_LENGTH)
    {
      /*
       * db_path + db_name is too long
       */
      er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_BO_FULL_DATABASE_NAME_IS_TOO_LONG, 3, newdb_path, newdb_name,
	      strlen (newdb_name) + strlen (newdb_path) + 2,
	      DB_MAX_PATH_LENGTH);
  ...
  /* Get the log prefix */
  newlog_prefix = fileio_get_base_file_name (newdb_name);
...
/* Temporary fix for NT file locking problem */
  ...
  if (dir != NULL && ((db = cfg_find_db_list (dir, newdb_name)) != NULL))
    {
      if (newdb_overwrite == false)
	{
	  /* There is a database with the same name and we cannot overwrite it */
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_BO_DATABASE_EXISTS, 1,
		  newdb_name);
      ...
      else
	{
	  ...
	  error_code = xboot_delete (thread_p, newdb_name, true);
	  if (error_code != NO_ERROR)
	    {
	      goto error;
	    }
	  ...
	  error_code = xboot_copy (thread_p, from_dbname, newdb_name,
  ...
  COMPOSE_FULL_NAME (newdb_fullname, newdb_path, newdb_name);
  ...
  error_code =
    logpb_copy_database (thread_p, boot_Db_parm->nvols, newdb_fullname,
			 newlog_path, newlog_prefix, new_volext_path,
			 fileof_vols_and_copypaths);
  if (error_code != NO_ERROR)
    {
      ...
      else
	{
      ...
      db = cfg_find_db_list (dir, newdb_name);

      if (db == NULL)
	{
	  db = cfg_add_db (&dir, newdb_name, newdb_path, newlog_path,
			   newdb_server_host);
	}
      else
	{
		   ...
		   bool force_delete)
{
  ...
  if (fileof_vols_and_renamepaths != NULL)
    {
       ...
       * be specified
       */
      newdb_path = boot_find_new_db_path (allocdb_path,
  ...
  if (newdb_path == NULL)
    {
       ...
       */
      newdb_path = fileio_get_directory_path (allocdb_path,
					      boot_Db_full_name);
      if (newdb_path == NULL)
	{
  ...
  if (newlog_path == NULL)
    {
       ...
       */
      newlog_path = fileio_get_directory_path (alloclog_path,
					       log_Name_active);
      if (newlog_path == NULL)
	{
  ...
  else if (realpath ((char *) newdb_path, newdb_pathbuf) != NULL)
    {
      newdb_path = newdb_pathbuf;
    }
  ...
  if (newlog_path == NULL)
    {
      /* Assign the data volume directory */
      strncpy (newlog_pathbuf, newdb_path, PATH_MAX);
      newlog_path = newlog_pathbuf;
    }
...
/* Temporary fix for NT file locking problem */
  ...
  error_code =
    logpb_rename_all_volumes_files (thread_p, boot_Db_parm->nvols,
				    newdb_fullname, newlog_path,
  ...
  if (extern_rename == true)
    {
      if (db == NULL)
	{
	  cfg_delete_db (&dir, olddb_name);
	  db = cfg_add_db (&dir, newdb_name, newdb_path, newlog_path,
			   newdb_server_host);
	}
      else
	{
	  cfg_update_db (db, newdb_path, newlog_path, newdb_server_host);
	}
      if (db == NULL || db->name == NULL || db->pathname == NULL ||
	  db->logpath == NULL || db->hosts == NULL)
	{
...
xboot_delete (THREAD_ENTRY * thread_p, const char *db_name, bool force_delete)
{
  ...
  dbtxt_vdes = fileio_mount (dbtxt_label, dbtxt_label, LOG_DBTXT_VOLID,
			     2, true);
  if (dbtxt_vdes == NULL_VOLDES)
    {
      return ER_FAILED;
  ...
  boot_server_all_finalize (thread_p,
			    (error_code == NO_ERROR) ? true : false);
#if defined(SA_MODE)
  boot_client_all_finalize ((error_code == NO_ERROR) ? true : false);
#endif /* SA_MODE */
			 ...
			 TRAN_ISOLATION client_isolation)
{
  ...
  if (xehash_create
      (thread_p, &boot_Db_parm->classname_table, DB_TYPE_STRING, -1,
       &boot_Db_parm->rootclass_oid, -1) == NULL)
    {
      error_code = ER_FAILED;
      goto error;
    }
  ...
  error_code =
    disk_set_boot_hfid (thread_p, LOG_DBFIRST_VOLID, &boot_Db_parm->hfid);
  if (error_code != NO_ERROR)
    {
      goto error;
    }
  ...
  if (heap_insert (thread_p, &boot_Db_parm->hfid, boot_Db_parm_oid, &recdes,
  ...
  if (xtran_server_commit (thread_p, false) != TRAN_UNACTIVE_COMMITTED)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE,
	      ER_BO_UNABLE_TO_RESTART_SERVER, 0);
      error_code = ER_BO_UNABLE_TO_RESTART_SERVER;
      goto error;
    }
			 ...
			 bool dirty_rem, bool force_delete)
{
  ...
  if (!BO_ISSERVER_RESTARTED ())
      ...
      if (!logpb_exist_log (thread_p, db_fullname, log_path, log_prefix))
	{
      ...
      error_code =
	boot_mount (thread_p, LOG_DBFIRST_VOLID, db_fullname, NULL);
      if (error_code != NO_ERROR)
	{
	  goto error_rem_allvols;
	}
      if (disk_get_boot_hfid
	  (thread_p, LOG_DBFIRST_VOLID, &boot_Db_parm->hfid) == NULL)
	{
	  error_code = ER_FAILED;
	  goto error_rem_allvols;
	}
      error_code =
	boot_get_db_parm (thread_p, boot_Db_parm, boot_Db_parm_oid);
      if (error_code != NO_ERROR)
	{
	  goto error_rem_allvols;
	}
  ...
  /* Now delete the database */
  error_code =
    logpb_delete (thread_p, boot_Db_parm->nvols, db_fullname, log_path,
		  log_prefix, force_delete);
  return error_code;
		       ...
		       bool recreate_log)
{
  ...
  COMPOSE_FULL_NAME (boot_Db_full_name, db->pathname, db_name);
  /*
   * Inform the error subsystem of location of the database name, for use
   * with event handling.
   */
  er_save_dbname (boot_Db_full_name);
  ...
  (void) sysprm_load_and_init (boot_Db_full_name, NULL);
  ...
  error_code =
    boot_find_rest_volumes (thread_p, NULL, LOG_DBFIRST_VOLID, boot_mount,
			    NULL);
  if (error_code != NO_ERROR)
    {
      fileio_dismount_all ();
  ...
  if (recreate_log == true)
      ...
      if (log_npages < 10)
	{
      ...
      log_recreate (thread_p, boot_Db_parm->nvols, boot_Db_full_name,
		    log_path, log_prefix, log_npages);
    }
  else
    {
  ...
  if (fileof_vols_and_wherepaths != NULL)
      ...
      if (where_paths_fp == NULL)
	{
      ...
      *from_volname = '\0';
      ...
      if (fscanf (where_paths_fp, "%d %s %s", &from_volid, from_volname,
      ...
      if (stat (db_pathbuf, &stat_buf) != -1 && S_ISCHR (stat_buf.st_mode))
	{
      ...
      name = strrchr (db_pathbuf, PATHSLASH);
#if defined(WINDOWS)
      {
...
boot_volume_info_log_path (char *log_path)
{
  ...
  while (true)
    {
      if (fscanf (volinfo_fp, "%d %s", &read_int_volid, log_path) != 2)
	{
  ...
  if (LOG_DBLOG_ACTIVE_VOLID == (VOLID) read_int_volid)
    {
      slash = strrchr (log_path, PATHSLASH);
#if defined(WINDOWS)
      {
