...
typedef enum
{
  ...
  RVHF_COMPENSATE_WITH_INSERT = 129,
  RVHF_COMPENSATE_WITH_UPDATE = 130,

  RV_NOP = 131,

  RVHF_SET_PREV_VERSION_LSA = 132,

  RV_LAST_LOGID = RVHF_SET_PREV_VERSION_LSA,
...
#define RCV_IS_HEAP_UNDO_WITH_VACUUM_CHECK(idx) \
  ((idx) == RVHF_DELETE) \
   || (idx) ==  RVHF_UPDATE \
   || (idx) ==  RVHF_MVCC_DELETE_MODIFY_HOME \
   || (idx) ==  RVHF_UPDATE_NOTIFY_VACUUM \
   || (idx) ==  RVHF_MVCC_UPDATE_OVERFLOW

#define RCV_IS_LOGICAL_COMPENSATE_MANUAL(idx) \
  (RCV_IS_BTREE_LOGICAL_LOG (idx) \
   || RCV_IS_HEAP_UNDO_WITH_VACUUM_CHECK (idx) \
   || (idx) == RVFL_ALLOC \
