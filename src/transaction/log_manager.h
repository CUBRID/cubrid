...
extern void log_append_compensate (THREAD_ENTRY * thread_p,
				   ...
				   int length, const void *data,
				   LOG_TDES * tdes);
extern void log_append_compensate_with_undo_nxlsa (THREAD_ENTRY * thread_p,
						   LOG_RCVINDEX rcvindex,
						   const VPID * vpid,
						   PGLENGTH offset,
...
extern int log_execute_run_postpone (THREAD_ENTRY * thread_p,
				     LOG_LSA * log_lsa, struct log_redo *redo,
				     char *redo_rcv_data);
...
extern void log_append_repl_info (THREAD_ENTRY * thread_p, LOG_TDES * tdes,
				  bool is_commit);
