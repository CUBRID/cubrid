...
extern MOP locator_add_class (MOBJ classobj, const OID * owner_oid, const char *classname);
...
extern LC_FIND_CLASSNAME locator_lockhint_classes (int num_classes, const char **many_classnames, LOCK * many_locks,
						   int *need_subclasses, LC_PREFETCH_FLAGS * flags, OID * owner_oids,
						   int quit_on_errors, LOCK lock_rr_tran, const char **resolved_names);
