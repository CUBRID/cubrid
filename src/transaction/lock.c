 ...
 *
 * lk.c - LOCK MANAGEMENT MODULE (AT THE SERVER)
 *
 */
 ...
 * invoked by:
 *     lk_internal_lock_object()
 *     lk_grant_blocked_waiter()
 *     lk_grant_blocked_waiter_partial()
 */
static void
 ...
 *
 * invoked by:
 *     lk_demote_shared_class_lock()
 *     lk_unlock_shared_class_lock()
 *     lock_unlock_object()
 */
static LK_ENTRY *
 ...
 *     lock_force_timeout_expired_wait_transactions()
 */
static void
 ...
 * invoked by:
 *     lk_internal_unlock_object()
 *     lk_internal_demote_shared_class_lock()
 *     lk_internal_demote_update_inst_lock()
 *
 * Note:
 */
static void
  ...
  if (release_flag == false)
    {
      if (entry_ptr->blocked_mode != NULL_LOCK)
	{
	  /* blocked waiter || blocked holder
	   * 1. blocked waiter
	   *    entry_ptr->count becomes 0(zero).
	   *    deallocate entry_ptr.
	   *    change total_holders_mode of waiter.
	   *    grant other blocked waiters.
	   *
	   * 2. blocked holder
	   *    entry_ptr->count might not be 0(zero).
	   *    reposition entry_ptr in holder list.
	   *    change total_holders_mode of holder list.
	   *    grant other blocked holders and waiters.
	   */
 ...
 * invoked by:
 *     lk_demote_shared_class_lock()
 *     lk_demote_all_shared_class_locks()
 *     lk_unlock_shared_class_lock()
 */
static int
 ...
 * invoked by:
 *     lock_unlock_object()
 *     lock_unlock_objects_lock_set()
 *     lock_unlock_scan()
 *     lock_unlock_classes_lock_hint()
 */
static void
 ...
 *
 * invoked by:
 *     lock_unlock_by_isolation_level()
 *     lock_unlock_all_shared_get_all_exclusive()
 */
static void
			    ...
			    const OID * class_oid, LOCK lock)
{
  ...
  /* When this function is called by lk_unlock_by_isolation() and
   * lock_unlock_all_shared_get_all_exclusive(), only one thread is executing for
   * one transaction. (transaction : thread = 1 : 1)
   * In those cases, there is no need to hold tran_lock->hold_mutex.
   *
   * In other cases, however, one or more threads can be executing
   * for one transaction. => It makes a problem..
   * Each thread must maintains its own lock hold list instead of
   * transaction lock hold list. Otherwise, this function can remove
   * instances locks acquired by other threads who are scanning.
   */
 ...
 * Note:
 * invoked by:
 *     lk_granted_blocked_holder()
 *     lk_granted_blocked_waiter()
 *     lk_granted_blocked_waiter_partial()
 *     lk_internal_unlock_object_uncond()
 */
static void
		   ...
		   int holder_flag, time_t edge_wait_stime)
{
  ...
  /* NOTE the checked_by_deadlock_detector of a transaction
   * When deadlock detection is performed, the checked_by_deadlock_detector
   * of each transaction is set to true by deadlock detector.
   * The checked_by_deadlock_detector of a transaction can be set to false
   * by the transaction in the execution of lock_unlock_all().
   * That is, when the transaction terminates,
   * the checked_by_deadlock_detector is set to false.
   * Therefore, if the checked_by_deadlock_detector of a transaction is false,
   * the deadlock detector can identify the fact that old transaction has
   * terminated and new transaction has started.
   */
 ...
 *
 * invoked by :
 *    tm/log_tb.c: logtb_define_trantable()
 */
int
lock_initialize (void)
{
  ...
  if (env_value != NULL)
    {
      lk_Gl.verbose_mode = atoi (env_value);
 ...
 *
 * Note:
 *     Lock the given "oid" object with the given lock according to
 *     the transaction isolation level. If the object is an instance,
 *     its class is also locked with an intention lock.
 *
 *     Depending on the isolation level and the kind of lock requested,
 *     the lock may not need to be acquired (i.e., the operation is ignored)
 *     or may be demoted to another lock mode.
 *
 *     The following rules are followed:
 *
 *     1) TRAN_SERIALIZABLE
 *        TRAN_REP_CLASS_REP_INSTANCE
 *        TRAN_REP_CLASS_COMMIT_INSTANCE (CURSOR STABILITY)
 *        TRAN_COMMIT_CLASS_COMMIT_INSTANCE
 *
 *        All requested locks must be acquired.
 *
 *     2) TRAN_REP_CLASS_UNCOMMIT_INSTANCE
 *        TRAN_COMMIT_CLASS_UNCOMMIT_INSTANCE
 *
 *        Requested exclusive type locks (X_LOCK, IX_LOCK,
 *        SIX_LOCK[converted to IX_LOCK]) are acquired.
 *        An S_LOCK on an instance is ignored, however, the IS_LOCK
 *        on the class is acquired.
 *        An S_LOCK (SIX_LOCK) on a class is demoted to IS_LOCK (IX_LOCK)
 *        and then it is acquired.
 *        The IS_LOCK on the Rootclass is also acquired.
 *
 * invoked by:
 *     qp/qp_xasl.c: xs_exec_update()              => class
 *     qp/qp_xasl.c: xs_exec_delete()              => class
 *     st/bt.c     : btree_delete()                   => instance
 *     st/bt.c     : btree_insert()                   => instance
 *     tm/lcsr.c   : xlocator_reserve_class_name()       => class
 *     tm/lcsr.c   : xlocator_delete_class_name()        => class
 *     tm/lcsr.c   : xlocator_find_class_oid()           => class
 *     tm/lcsr.c   : xlocator_fetch()                   => object (??)
 *     tm/lcsr.c   : xlocator_get_class()               => object (??)
 *     tm/lcsr.c   : xlocator_fetch_all()                => class
 *     tm/lcsr.c   : xlocator_find_lockhint_class_oids() => class or root class
 *     tm/lcsr.c   : xlocator_fetch_lockhint_classes()  => class or root class
 */
int
 ...
 *
 *  invoked by:
 *  st/bt.c     : btree_range_search()                => instance
 */
int
 ...
 *
 * Note:
 *     Acquire the locks lockset->reqobj_inst_lock,
 *     lockset->reqobj_class_lock for all requested objects
 *     lockset->objects that are instances and classes, respectively
 *     according to the transaction isolation level.
 *     The classes lockset->classes of the requested objects are
 *     locked with intension locks.
 *
 *     To avoid deadlock, the corresponding oids are sorted and then locked
 *     one by one. When the value of lockset->quit_on_errors is true,
 *     the transaction quits its execution as soon as any one lock acquisition
 *     request cannot be granted.
 *     However, if the value of lockset->quit_on_errors is false,
 *     the objects are locked individually, eliminating the ones that
 *     failed (Note: lockset is set as a side effect).
 *
 *     Depending on the isolation level and the kind of lock requested,
 *     the lock may not need to be acquired (i.e., the operation is ignored)
 *     or may be demoted to another lock mode.
 ...
 *
 * invoked by:
 *     tm/lcsr.c: xlocator_fetch_lockset()
 */
int
lock_objects_lock_set (THREAD_ENTRY * thread_p, LC_LOCKSET * lockset)
{
  ...
  if (ins_count > 0)
      ...
      for (i = 0; i < ins_count; i++)
	{
	  ...
	  intention_mode =
	    (ins_lockinfo[i].lock <= S_LOCK ? IS_LOCK : IX_LOCK);
	  class_entry =
	    lock_get_class_lock (&ins_lockinfo[i].class_oid, tran_index);
	  if (class_entry == NULL
 ...
 *
 * Note:
 *     The given class_oid is locked to see a consistent class definition
 *     during the evaluation of a scan no matter what transaction isolation
 *     level is in effect. If we do not do this, a situation like the
 *     following can happen:
 *     The above is possible since the first transaction did not
 *     acquired a lock.
 *
 *
 *     1) TRAN_SERIALIZABLE,
 *        TRAN_REP_CLASS_REP_INSTANCE
 *
 *        - IS_LOCK/S_LOCK is acquired.
 *        - IS_LOCK/S_LOCK is released at end of transaction.
 *
 *        It is likely that a lock in the correct mode has already been
 *        acquired to verify class properties (e.g., the heap). Thus,
 *        this operation is mainly used to find out the current lock.
 *
 *     2) TRAN_REP_CLASS_COMMIT_INSTANCE (CURSOR STABILITY)
 *
 *        - IS_LOCK/S_LOCK is acquired.
 *        - S_LOCK is demoted to IS_LOCK at the end of scan.
 *
 *        During the sequential scan, other transactions cannot
 *        insert/modify/delete instances of the scan class.
 *        During the index scan, however, other transactions can
 *        insert/modify/delete instances that are not scanned
 *        by current transaction.
 *        Other transactions cannot update the class
 *        until the transaction terminates.
 *        It is likely that a lock (IS_LOCK, IX_LOCK, X_LOCK) has
 *        already been acquired.
 *
 *     3) TRAN_COMMIT_CLASS_COMMIT_INSTANCE
 *
 *        - IS_LOCK/S_LOCK is acquired
 *        - IS_LOCK/S_LOCK is released at the end of scan.
 *
 *        During the sequential scan, other transactions cannot
 *        insert/modify/delete instances of the scan class.
 *        During the index scan, however, other transactions can
 *        insert/modify/delete instances that are not scanned
 *        by current transaction.
 *        Other transactions cannot update the class during the scan,
 *        but, they scan update the class immediately after the scan.
 *        It is possible that a lock (e.g., IX_LOCK, X_LOCK) has
 *        already been acquired.
 *
 *     4) TRAN_REP_CLASS_UNCOMMIT_INSTANCE
 *
 *         - IS_LOCK is acquired.
 *         - IS_LOCK is released at the end of the transaction.
 *
 *        Other transactions can insert/modify/delete instances of
 *        the class during and after the scan. Other transactions
 *        cannot modify the class until the transaction terminates.
 *        It is likely that a lock (IS_LOCK, IX_LOCK, X_LOCK) has
 *        already been acquired.
 *
 *     5) TRAN_COMMIT_CLASS_UNCOMMIT_INSTANCE
 *
 *        - IS_LOCK is acquired
 *        - IS_LOCK is released at the end of the scan.
 *
 *        Other transactions can insert/modify/delete instances of
 *        the class during and after the scan. Other transactions
 *        cannot modify the class during the scan.
 *        It is possible that a lock (e.g., IX_LOCK, X_LOCK) has
 *        already been acquired.
 *
 * invoked by:
 *     st/hf.c: hf_internal_start_scancache()
 */
int
	   ...
	   int lock_hint, LOCK * current_lock, int *scanid_bit)
{
  ...
  root_class_entry = lock_get_class_lock (oid_Root_class_oid, tran_index);
  granted =
    lock_internal_perform_lock_object (thread_p, tran_index, class_oid,
				       (OID *) NULL, class_lock, waitsecs,
				       &class_entry, root_class_entry);
  if (granted == LK_GRANTED)
    {
 ...
 *
 * Note:Acquire the locks requested for all classes in the hinted
 *     lock structure according to the transaction isolation level.
 *     To avoid deadlock, the class oids are sorted and then locked.
 *
 *     Depending on the isolation level and the kind of lock requested,
 *     the lock may not need to be acquired (i.e., the operation is ignored)
 *     or may be demoted to another lock mode.
 ...
 *        TRAN_COMMIT_CLASS_COMMIT_INSTANCE
 *
 *        All requested locks are acquired.
 *
 *     2) TRAN_REP_CLASS_UNCOMMIT_INSTANCE
 *        TRAN_COMMIT_CLASS_UNCOMMIT_INSTANCE
 *
 ...
 *
 * invoked by :
 *     tm/lcsr.c: xlocator_fetch_lockhint_classes()
 */
int
  ...
  for (i = 0; i < cls_count; i++)
	  ...
	  || root_class_entry->granted_mode < intention_mode)
	{
	  granted =
	    lock_internal_perform_lock_object (thread_p, tran_index,
					       oid_Root_class_oid,
					       (OID *) NULL, intention_mode,
					       waitsecs, &root_class_entry,
					       NULL);
	  if (granted != LK_GRANTED)
	    {
 ...
 *
 * Note:
 *     Depending upon the transaction isolation level, the lock
 *     on the "oid" object is released, demoted, or left untouched.
 *
 ...
 *
 *     2) TRAN_REP_CLASS_COMMIT_INSTANCE (CURSOR STABILITY)
 *        - X_LOCK, IX_LOCK, and IS_LOCK locks cannot be released until
 *          the end of the transaction.
 *        - S_LOCK on instances can be released in the middle of
 *          the transaction.
 *        - S_LOCK and SIX_LOCK on classes can be demoted to IS_LOCK and
 *          IX_LOCK, respectively in the middle of the transaction.
 *
 *     3) TRAN_COMMIT_CLASS_COMMIT_INSTANCE
 *        - X_LOCK and IX_LOCK locks cannot be released until the end of
 *          the transaction.
 *        - S_LOCK on instances can be released in the middle of
 *          the transaction.
 *        - S_LOCK and IS_LOCK on classes can be released in the middle of
 *          the transaction.
 *        - SIX_LOCK on classes can be demoted to IX_LOCK in the middle of
 *          the transaction.
 *
 *     4) TRAN_REP_CLASS_UNCOMMIT_INSTANCE
 *        - X_LOCK, IX_LOCK, and IS_LOCK locks cannot be released
 *          until the end of the transaction.
 *        - S_LOCK on instances were never acquired.
 *        - S_LOCK and SIX_LOCK on classes can be demoted to IS_LOCK and
 *          IX_LOCK, respectively in the middle of the transaction.
 *
 *     5) TRAN_COMMIT_CLASS_UNCOMMIT_INSTANCE
 ...
 *          the transaction.
 *
 *     The given lock is used as an optimization.
 *
 *     Note: force can be set to true, if the isolation level should not be
 *           checked. This should be used only when a dirty read is desired.
 *
 * invoked by:
 *     st/bt.c:   btree_delete()                    : force=true  => instance
 *     st/bt.c:   btree_insert()                    : force=true  => instance
 *     st/bt.c:   btree_range_search()                 : force=true  => instance
 *     st/hf.c:   hf_internal_insert_with_lock() : force=true  => object
 *     tm/lcsr.c: xlocator_reserve_class_name()        : force=true  => class
 *     tm/lcsr.c: xlocator_delete_class_name()         : force=true  => class
 *     tm/lcsr.c: xlocator_find_class_oid()            : force=true  => class
 *     tm/lcsr.c: xlocator_find_class_oid()            : force=false => class
 *     tm/lcsr.c: xlc_permoid_classname()        : force=true  => class
 *     tm/lcsr.c: xlocator_assign_oid()               : force=false
 *     tm/lcsr.c: xlocator_fetch()                    : force=false
 *     tm/lcsr.c: xlocator_get_class()                : force=false
 *     tm/lcsr.c: xlocator_fetch_all()                 : force=false
 *     tm/lcsr.c: xlocator_does_exist()                : force=false
 *     tm/lcsr.c: xlocator_find_lockhint_class_oids()  : force=true  => class
 */
void
 ...
 *
 * Note:
 *     Depending upon the transaction isolation level, the locks
 *     on the requested objects are released, demoted, or left untouched.
 *
 *     The following rules are observed:
 *
 *     1) TRAN_SERIALIZABLE
 ...
 * Note:
 *     Depending upon the transaction isolation level, the locks
 *     on the hinted classes are released, demoted, or left untouched.
 *
 *     The following rules are observed:
 *
 *     1) TRAN_SERIALIZABLE
 ...
 *        - X_LOCK, IX_LOCK, and IS_LOCK locks cannot be released
 *          until the end of the transaction.
 *        - S_LOCK and SIX_LOCK can be demoted to IS_LOCK and IX_LOCK,
 *          respectively in the middle of the transaction.
 *
 *     3) TRAN_COMMIT_CLASS_COMMIT_INSTANCE
 *        - X_LOCK and IX_LOCK locks cannot be released until the end of
 *          the transaction.
 *        - S_LOCK and IS_LOCK can be released in the middle of
 *          the transaction.
 *        - SIX_LOCK can be demoted to IX_LOCK in the middle of
 *          the transaction.
 *
 *     4) TRAN_REP_CLASS_UNCOMMIT_INSTANCE
 *        - X_LOCK, IX_LOCK, and IS_LOCK locks cannot be released until
 ...
 *
 * invoked by:
 *     tm/log.c: log_commit_local(), log_abort_local()
 */
void
 ...
 *
 * invoked by:
 *     tm/lcsr.c  : xlocator_notify_isolation_incons()
 *     tm/log_tb.c: xlogtb_reset_isolation()
 */
void
 ...
 *
 * invoked by:
 *     st/bt.c: btree_delete()
 *     st/bt.c: btree_insert()
 *     st/hf.c: hf_internal_start_scancache()
 *     st/hf.c: hf_reset_modify_scancache()
 */
LOCK
lock_get_object_lock (const OID * oid, const OID * class_oid, int tran_index)
{
  ...
  /*
   * case 3: instance lock
   *
   * get the granted lock mode acquired on the given instance oid
   * class_oid != NULL && !OID_EQ(class_oid, oid_Root_class_oid)
   * At first, find class lock in order to check if implicit lock
   * is held on the instance. If the implicit lock(S_LOCK or X_LOCK)
   * is already held, the implicit lock mode is returned.
   * otherwise, find real instance lock mode and return it.
   */
  MUTEX_LOCK (rv, tran_lock->hold_mutex);
  entry_ptr = tran_lock->class_hold_list;
  for (; entry_ptr != (LK_ENTRY *) NULL; entry_ptr = entry_ptr->tran_next)
 ...
 *
 * invoked by:
 *     tm/tmsr.c: xtran_server_has_updated()
 */
bool
 ...
 *
 * invoked by:
 *     css/sched.c: css_check_for_clients_down()
 *     css/srv_new.c: css_check_client_timeout()
 *     dirvers/sched.c: css_check_for_clients_down()
 */
bool
 ...
 *
 * invoked by:
 *     storage/btree.c: btree_initialize_BTS()
 *     storage/btree.c: btree_range_search()
 */
LK_ENTRY *
 ...
 *
 * invoked by:
 *     css/thrd.c: thread_stop_active_workers()
 */
void
 ...
 *
 * invoked by:
 *     tm/lcsr.c: xlocator_fetch()
 *     tm/lcsr.c: xlocator_fetch_all()
 *     tm/lcsr.c: xlocator_fetch_lockset()
 *     tm/lcsr.c: xlocator_notify_isolation_incons()
 *     tm/lcsr.c: xlocator_fetch_lockhint_classes()
 */
 ...
 *
 * invoked by:
 *     css/thrd.c: css_deadlock_detect_thread()
 */
bool
...
final:
  ...
  if (victim_count == 0)
    {
      /* In the environment that one or more threads are executing
       * for one transaction, following problem might be occurred...
       * For example, maximum number of transactions is same with
       * maximum number of threads (100 transactions and 100 threads).
       * Let's assume that one transaction T1 is holding a lock on object O1
       * and 100 threads are currently executing for transactions except T1.
       * If all the threads are blocked while requesting a lock on object O1,
       * the system will be blocked..
       * In that case, even if T1 tries to commit or abort, the termination
       * of T1 will not be executed since there is no free threads to execute
       * the termination of T1. Therefore, the system will be blocked forever.
       * In current CUBRID, async query and nested client methed invocation
       * require two or more threads for one transaction.
       * In later version, parellel query facility will be added.
       * Therefore, the above problem could be a problem.
       * To resolve the problem, the deadlock detector checks the system
       * blocking and resolves the problem if it occurs like following code.
       */
...
#if 0				/* NOT_USED */
/*
 * NOTE the following description on the global deadlock detection.
 *
 * Currently, the global deadlock detection mechanism is not implemented.
 * Because, the global deadlock detection mechanism is used only in the
 * multi-database environment.
 * We did not consider the multi-database environment, currently.
 *
 * If the multi-database environment must be considered, only the global
 * deadlock detection mechanism will be needed. Other facilities will not
 * be needed, I think.. The original global deadlock detection mechanism
 * uses the PCG(potential conflict graph) mechanism. The PCG is used
 * to determine if there is a possibility of a deadlock based on the access
 * patterns of clients to local databases. The access patterns of clients
 * to local databases must be maintained in the global coordinator(??).
 * That is, the global coordinator does not need the control information
 * of each local database. Therefore, the local deadlock detection can be
 * used without the consideration on the global deadlock detection.
 *
 * Currently, the footprint of original deadlock detection mechanism and
 * the code of original WFG(wait-for graph) module will be remained for
 * later versions. (ex, that are considering multi-database environment)
 */
  ...
  while (cycle_case == WFG_CYCLE_YES_PRUNE)
    {
      ...
      if (error == NO_ERROR &&
	  (cycle_case == WFG_CYCLE_YES_PRUNE || cycle_case == WFG_CYCLE_YES))
	{
		  ...
		  if (already_picked != true)
		      ...
		      else
			{
			  ...
			  if (isvictim_tg_waiting != NO_ERROR)
			    {
			      /* The currently selected victim is NOT waiting on a TG
			         resource.

			         Select the candidate as the new victim if either:
			         a) candidate is waiting on a TG resource
			         b) candidate timeout is smaller than selected victim
			         timeout
			         c) If timeouts are equal, then if the candidate transaction
			         is younger than selected victim transaction
			       */
			      if (iscandidate_tg_waiting == NO_ERROR
				  || (victims[num_victims].can_timeout ==
				      false && can_timeout == true)
				  || (victims[num_victims].can_timeout ==
				      can_timeout
				      && LK_ISYOUNGER (tranid,
						       victims[num_victims].
			    ...
			    {
			      /* The currently selected victim IS waiting on a TG
			         resource.

			         Select the candidate as the new victim if the candidate
			         transaction is also waiting on a TG resource and either of
			         the following holds:

			         a) candidate timeout is smaller than selected victim
			         timeout
			         b) If timeouts are equal, then if the candidate transaction
			         is younger than selected victim transaction
			       */
			      if (iscandidate_tg_waiting == NO_ERROR &&
				  ((victims[num_victims].can_timeout ==
 ...
 *
 * invoked by:
 *     tm/log_2pc.c: log_2pc_read_prepare()
 */
int
 ...
 *
 * invoked by:
 *     tm/log_2pc.c: log_2pc_commit() => (argumet: NULL)
 *     tm/log_2pc.c: log_2pc_prepare() => (argument: &acq_locks)
 */
void
 ...
 *
 * invoked by:
 *     ns/intcl.c: lock_dump()
 *     ns/intsr.c: slc_dump()
 *     tm/lk.c: lock_detect_local_deadlock()
 */
void
