  ...
  *ptr = NULL;
  ...
  for (i = 0; i < NUM_TOTAL_TRAN_INDICES; i++)
      ...
      if (vals == NULL)
	{
      ...
      /* State */
      db_make_string_copy (&vals[idx], log_state_short_string (tdes->state));
      idx++;

      /* isolation */
      db_make_string_copy (&vals[idx], log_isolation_string (tdes->isolation));
      idx++;
      ...
      else
	{
      ...
      db_make_string_copy (&vals[idx], str);
      idx++;
