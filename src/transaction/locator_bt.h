...
/* LC_OIDMAP
 *
 * Structure used to hold infomation about a single OID that needs
 * to be assigned.  It will be embedded inside an LC_CLASS_OIDSET structure.
 * The est_size field has the approxomate byte size of the object.
 * The perm_oid field will be filled in and returned with the permanent
 * oid assigned.  If the OID could not be assigned for some reason, the
 * field will be set to the "null" oid.
 *
 * The client_data field is meant for use on the client only, it will not
 * be sent to or returned by the server.  It is intended for use when
 * locator_unpack_oid_set is called with an existing client side LC_OIDSET structure.
 * This field allows us to attach additional information about this particular
 * OID so we can quickly get to the places on the client that need
 * to be changed when the permanent OID comes back.
 *
 * This "shared" structure approach is a little unconventional but it saves
 * having to maintain two similar but separate structures, one for the client
 * and the other to pass down to the server.
 */
