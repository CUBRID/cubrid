...
struct log_header
{
  ...
  /* Here exists 3 bytes */
  INT32 cdc_arv_num_to_keep;	/* Oldest archive volume CDC needs, biased by one. Use LOG_HDR_CDC_ARV_NUM_*. */
  ...
  LOG_LSA_ATOMIC append_lsa;	/* Current append location */
...
/* cdc_arv_num_to_keep outlives the server process so that archive removal keeps what CDC has yet to re-read.
 * It is biased by one: zero means "nothing to keep", which is what the padding field it took over already
 * held in every existing database. */
#define LOG_HDR_CDC_ARV_NUM_NONE        0
#define LOG_HDR_CDC_ARV_NUM_IS_SET(hdr) ((hdr)->cdc_arv_num_to_keep != LOG_HDR_CDC_ARV_NUM_NONE)
#define LOG_HDR_CDC_ARV_NUM_GET(hdr)    ((hdr)->cdc_arv_num_to_keep - 1)
#define LOG_HDR_CDC_ARV_NUM_SET(hdr, arv_num) ((hdr)->cdc_arv_num_to_keep = (arv_num) + 1)
#define LOG_HDR_CDC_ARV_NUM_RESET(hdr)  ((hdr)->cdc_arv_num_to_keep = LOG_HDR_CDC_ARV_NUM_NONE)
