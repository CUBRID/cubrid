...
#if defined(CUBRID_DEBUG)
    if (bufptr->pageid != LOGPB_HEADER_PAGE_ID && (bufptr->pageid < LOGPB_NEXT_ARCHIVE_PAGE_ID || bufptr->pageid > LOGPB_LAST_ACTIVE_PAGE_ID))
    {
 ...
 */
int logpb_initialize_header(THREAD_ENTRY *thread_p, LOG_HEADER *loghdr, const char *prefix_logname,
 ...
 */
int logpb_fetch_page(THREAD_ENTRY *thread_p, const LOG_LSA *req_lsa, LOG_CS_ACCESS_MODE access_mode,
                     LOG_PAGE *log_pgptr)
{
 ...
 */
int logpb_read_page_from_file(THREAD_ENTRY *thread_p, LOG_PAGEID pageid, LOG_CS_ACCESS_MODE access_mode,
                              LOG_PAGE *log_pgptr)
{
    bool log_csect_entered = false;
 ...
 *   log_pgptr(in/out):
 */
int logpb_read_page_from_active_log(THREAD_ENTRY *thread_p, LOG_PAGEID pageid, int num_pages, bool decrypt_needed,
                                    LOG_PAGE *log_pgptr)
{
...
int logpb_write_page_to_disk(THREAD_ENTRY *thread_p, LOG_PAGE *log_pgptr, LOG_PAGEID logical_pageid)
{
    ...
    assert(logical_pageid == LOGPB_HEADER_PAGE_ID || (!LOGPB_IS_ARCHIVE_PAGE(logical_pageid) && logical_pageid <= LOGPB_LAST_ACTIVE_PAGE_ID));
...
logpb_flush_all_append_pages(THREAD_ENTRY *thread_p)
{
...
#if defined(SERVER_MODE)
    if (thread_p && thread_p->type != TT_DAEMON && thread_p->type != TT_VACUUM_MASTER && thread_p->type != TT_VACUUM_WORKER)
    {
   ...
   */
    ...
    if (log_Pb.partial_append.status == LOGPB_APPENDREC_IN_PROGRESS)
    {
       ...
       */
        ...
        if (logpb_write_page_to_disk(thread_p, log_Pb.partial_append.log_page_record_header, first_append_pageid) != NO_ERROR)
        {
            error_code = ER_FAILED;
            goto error;
        }
    ...
    else if (log_Pb.partial_append.status == LOGPB_APPENDREC_PARTIAL_ENDED || log_Pb.partial_append.status == LOGPB_APPENDREC_SUCCESS)
    {
        /* Add an end of log marker to detect the end of the log.
   ...
   */
    ...
    {
        assert(log_Pb.partial_append.status == LOGPB_APPENDREC_SUCCESS || log_Pb.partial_append.status == LOGPB_APPENDREC_PARTIAL_FLUSHED_END_OF_LOG);
   ...
   */
    if (need_sync == true)
    {
        if (prm_get_integer_value(PRM_ID_SUPPRESS_FSYNC) == 0 || (log_Stat.total_sync_count % prm_get_integer_value(PRM_ID_SUPPRESS_FSYNC) == 0))
        {
            /* System volume. No need to sync DWB. */
...
#if defined(SERVER_MODE)
void logpb_send_flushed_lsa_to_ats()
{
...
logpb_start_append(THREAD_ENTRY *thread_p, LOG_RECORD_HEADER *header)
{
   ...
   */
    ...
    if (log_rec->type == LOG_END_OF_LOG)
    {
        ...
        assert(log_Pb.partial_append.status == LOGPB_APPENDREC_SUCCESS || log_Pb.partial_append.status == LOGPB_APPENDREC_PARTIAL_ENDED);
    ...
    if (fp != NULL)
    {
        while (fgets(line, LOG_MAX_LOGINFO_LINE, fp) != NULL)
        {
            if (strstr(line + TIME_SIZE_OF_DUMP_LOG_INFO,
                       msgcat_message(MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG, MSGCAT_LOG_LOGINFO_KEYWORD_ARCHIVE)) == line + TIME_SIZE_OF_DUMP_LOG_INFO)
            {
 ...
 */
int logpb_scan_volume_info(THREAD_ENTRY *thread_p, const char *db_fullname, VOLID ignore_volid, VOLID start_volid,
    ...
    while (true)
    {
        ...
        else
        {
		   ...
		   */
            ...
            fileio_make_log_archive_name(arv_name, log_Archive_path, log_Prefix, *ret_arv_num);
            retry = 3;
            while (retry != 0 && retry != 1 && (vdes = fileio_mount(thread_p, log_Db_fullname, arv_name, LOG_DBLOG_ARCHIVE_VOLID, false, false)) == NULL_VOLDES)
            {
...
logpb_archive_active_log(THREAD_ENTRY *thread_p)
{
   ...
   */
    ...
    if (prm_get_bool_value(PRM_ID_LOG_BACKGROUND_ARCHIVING) && bg_arv_info->vdes != NULL_VOLDES && arvhdr->fpageid == bg_arv_info->start_page_id)
    {
        pageid = bg_arv_info->current_page_id;
    ...
    logpb_log("In logpb_archive_active_log, new values from log_Gl.hdr.nxarv_pageid = %lld and log_Gl.hdr.nxarv_phy_pageid = %lld\n",
...
void logpb_remove_archive_logs(THREAD_ENTRY *thread_p, const char *info_reason)
{
    ...
    if ((!LSA_ISNULL(&newflush_upto_lsa) && LSA_LT(&newflush_upto_lsa, &flush_upto_lsa)) || (fileio_synchronize_all(thread_p, false) != NO_ERROR))
    {
...
logpb_get_archive_num_from_info_table(THREAD_ENTRY *thread_p, LOG_PAGEID page_id)
{
    ...
    {
        if (logpb_Arv_page_info_table.page_info[i].start_pageid <= page_id && page_id <= logpb_Arv_page_info_table.page_info[i].end_pageid)
        {
 ...
 */
int logpb_initialize_log_names(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath,
...
void logpb_checkpoint_trans(LOG_INFO_CHKPT_TRANS *chkpt_entries, log_tdes *tdes, int &ntrans, int &ntops,
...
int logpb_checkpoint_topops(THREAD_ENTRY *thread_p, LOG_INFO_CHKPT_SYSOP *&chkpt_topops,
                            LOG_INFO_CHKPT_TRANS *chkpt_trans, LOG_REC_CHKPT &tmp_chkpt, log_tdes *tdes, int &ntops,
                            size_t &length_all_tops)
{
    if (tdes != NULL && tdes->trid != NULL_TRANID && (!LSA_ISNULL(&tdes->rcv.sysop_start_postpone_lsa) || !LSA_ISNULL(&tdes->rcv.atomic_sysop_start_lsa)))
    {
   ...
   */
    ...
    if (log_Gl.hdr.last_arv_num_for_syscrashes != log_Gl.hdr.nxarv_num && log_Gl.hdr.last_arv_num_for_syscrashes != -1)
    {
        ...
        if (logpb_is_page_in_archive(smallest_pageid))
        {
            ...
            if ((logpb_fetch_from_archive(thread_p, smallest_pageid, NULL, &arv_num, NULL, true) == NULL) || (arv_num <= log_Gl.hdr.last_arv_num_for_syscrashes))
            {
                first_arv_num_not_needed = last_arv_num_not_needed = -1;
            }
            else
 ...
 *
 */
int logpb_backup(THREAD_ENTRY *thread_p, int num_perm_vols, const char *allbackup_path, FILEIO_BACKUP_LEVEL backup_level,
 ...
 */
int logpb_restore(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath, const char *prefix_logname,
	   ...
	   */
        ...
        while (success == NO_ERROR)
        {
            ...
            if (another_vol == 1)
            {
                ...
                if (to_volid == LOG_DBLOG_ACTIVE_VOLID || to_volid == LOG_DBLOG_INFO_VOLID || to_volid == LOG_DBLOG_ARCHIVE_VOLID)
                {
		   ...
		   */
                ...
                if (volume_name_p == tmp_logfiles_from_backup)
                {
                    ...
                    if (stat(to_volname, &stat_buf) != 0 && stat(tmp_logfiles_from_backup, &stat_buf) == 0)
                    {
                        ...
                        if (to_volid == LOG_DBLOG_ACTIVE_VOLID && !logpb_is_log_active_from_backup_useful(thread_p, tmp_logfiles_from_backup, db_fullname))
                        {
 ...
 */
int logpb_copy_database(THREAD_ENTRY *thread_p, VOLID num_perm_vols, const char *to_db_fullname, const char *to_logpath,
 ...
 */
int logpb_rename_all_volumes_files(THREAD_ENTRY *thread_p, VOLID num_perm_vols, const char *to_db_fullname,
 ...
 *              run when there are multiusers in the system.
 */
int logpb_delete(THREAD_ENTRY *thread_p, VOLID num_perm_vols, const char *db_fullname, const char *logpath,
 ...
 */
int logpb_check_exist_any_volumes(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath,
...
void logpb_fatal_error_exit_immediately_wo_flush(THREAD_ENTRY *thread_p, const char *file_name, const int lineno,
                                                 const char *fmt, ...)
{
    va_list ap;
...
logpb_remote_ask_user_before_delete_volumes(THREAD_ENTRY *thread_p, const char *volpath)
{
    ...
    if (asprintf(&ptr1, msgcat_message(MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG, MSGCAT_LOG_STARTS)) < 0 || asprintf(&ptr2, msgcat_message(MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG, MSGCAT_LOG_DELETE_BKVOLS), volpath) < 0 || asprintf(&ptr3, msgcat_message(MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG, MSGCAT_LOG_ENTER_Y2_CONFIRM)) < 0 || asprintf(&fullmsg, "%s%s%s%s", ptr1, ptr2, ptr3, ptr1) < 0)
    {
 ...
 *   return: NO_ERROR if all OK, ER status otherwise
 */
int logpb_remove_all_in_log_path(THREAD_ENTRY *thread_p, const char *db_fullname, const char *logpath,
                                 const char *prefix_logname)
{
...
{
    /* exclusive */
    assert(!((log_pgptr->hdr.flags & LOG_HDRPAGE_FLAG_ENCRYPTED_AES) && (log_pgptr->hdr.flags & LOG_HDRPAGE_FLAG_ENCRYPTED_ARIA)));
