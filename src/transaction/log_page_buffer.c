...
static void logpb_fatal_error_internal (THREAD_ENTRY * thread_p, bool log_exit, bool need_flush,
...
static int logpb_expand_pool (THREAD_ENTRY * thread_p, int num_new_buffers) 
{
	...
	
	{
		    ...
		    total_buffers * sizeof (*log_Pb.pool));
...
void logpb_finalize_pool (THREAD_ENTRY * thread_p) 
{
    ...
    {
      logpb_free_without_mutex (log_Gl.append.log_pgptr, thread_p);
...
static LOG_PAGE *logpb_fix_page (THREAD_ENTRY * thread_p, LOG_PAGEID pageid, PAGE_FETCH_MODE fetch_mode) 
{
  ...
  if (log_Pb.fst == 0 && pageid != LOGPB_HEADER_PAGE_ID)
    
    {
      log_Pb.cursor = pageid;
...
{
  logpb_free_without_mutex (log_pgptr, thread_p);
 ...
 */ 
void logpb_free_without_mutex (LOG_PAGE * log_pgptr, THREAD_ENTRY * thread_p) 
{
...
static void logpb_dump_pages (FILE * out_fp) 
{
	...
	{
	  fprintf (out_fp, "%3d %10lld %10d %3d %3d %4d  %p %p-%p %4s %5lld %5d\n", i, (long long) log_bufptr->pageid,
		    log_bufptr->phy_pageid, log_bufptr->dirty, log_bufptr->recently_freed, log_bufptr->fcnt,
 ...
 */ 
  LOG_PAGE * logpb_fetch_page (THREAD_ENTRY * thread_p, LOG_LSA * req_lsa, LOG_CS_ACCESS_MODE access_mode,
				LOG_PAGE * log_pgptr) 
{
 ...
 *              If not, read log page from log.
 */ 
static LOG_PAGE *logpb_copy_page (THREAD_ENTRY * thread_p, LOG_PAGEID pageid, LOG_CS_ACCESS_MODE access_mode,
				   LOG_PAGE * log_pgptr) 
{
...
static void logpb_write_toflush_pages_to_archive (THREAD_ENTRY * thread_p) 
{
	...
	
	{
      ...
      if (fileio_write (thread_p, bg_arv_info->vdes, log_pgptr, phy_pageid, LOG_PAGESIZE) == NULL)
 ...
 */ 
static int prior_lsa_gen_undoredo_record_from_crumbs (THREAD_ENTRY * thread_p, LOG_PRIOR_NODE * node,
 ...
 *   data(in):
 */ 
static int prior_lsa_gen_dbout_redo_record (THREAD_ENTRY * thread_p, LOG_PRIOR_NODE * node, LOG_RCVINDEX rcvindex,
					     int length, char *data) 
{
 ...
 *   topop_data(in):
 */ 
static int prior_lsa_gen_end_chkpt_record (THREAD_ENTRY * thread_p, LOG_PRIOR_NODE * node, int tran_length,
 ...
 */ 
  LOG_PRIOR_NODE * prior_lsa_alloc_and_copy_data (THREAD_ENTRY * thread_p, LOG_RECTYPE rec_type, LOG_RCVINDEX rcvindex,
						   LOG_DATA_ADDR * addr, int ulength, char *udata, int rlength,
						   char *rdata) 
{
 ...
 *   rcrumbs(in):
 */ 
  LOG_PRIOR_NODE * prior_lsa_alloc_and_copy_crumbs (THREAD_ENTRY * thread_p, LOG_RECTYPE rec_type,
						     LOG_RCVINDEX rcvindex, LOG_DATA_ADDR * addr,
						     ...
						     const int num_rcrumbs, const LOG_CRUMB * rcrumbs) 
{
	...
	prior_lsa_gen_undoredo_record_from_crumbs (thread_p, node, rcvindex, addr, num_ucrumbs, ucrumbs, num_rcrumbs,
						   rcrumbs);
...
int logpb_create_volume_info (const char *db_fullname) 
{
	...
	er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_BO_CANNOT_CREATE_VOL, 2, volinfo_fullname,
			     db_fullname);
 ...
 */ 
  VOLID 
  logpb_add_volume (const char *db_fullname, VOLID new_volid, const char *new_volfullname,
					...
					int *ret_arv_num, LOG_ARV_HEADER * ret_arv_hdr, bool is_fatal) 
{
	...
	
	{
		...
		
		{
			    ...
			    msgcat_message (MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG, MSGCAT_LOG_LOGARCHIVE_NEEDED),
		...
		
		{
		    ...
		    
		    {
			    ...
			    msgcat_message (MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG, MSGCAT_LOG_INPUT_RANGE_ERROR), 0,
			    3);
...
static void logpb_archive_active_log (THREAD_ENTRY * thread_p) 
{
  ...
  '\0'};			/* Archive name */
  ...
  if (fileio_write (thread_p, vdes, malloc_arv_hdr_pgptr, 0, LOG_PAGESIZE) == NULL)
	...
	
	{
      ...
      if (fileio_write_pages (thread_p, vdes, (char *) log_pgptr, ar_phy_pageid, num_pages, LOG_PAGESIZE) == NULL)
...
#endif	/*  */
    er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE, ER_LOG_ARCHIVE_CREATED, 3, arv_name, arvhdr->fpageid,
	     last_pageid);
 ...
 * NOTE:
 */ 
static int logpb_add_archive_page_info (THREAD_ENTRY * thread_p, int arv_num, LOG_PAGEID start_page,
					 LOG_PAGEID end_page) 
{
 ...
 *              volumes.
 */ 
static int logpb_backup_for_volume (THREAD_ENTRY * thread_p, VOLID volid, LOG_LSA * chkpt_lsa,
		   ...
		   FILEIO_ZIP_LEVEL zip_level, int skip_activelog, int sleep_msecs) 
{
  ...
  if (allbackup_path == NULL)
    ...
    session.type = FILEIO_BACKUP_WRITE;	/* access backup device for write */
       ...
       (log_Db_fullname, allbackup_path, &session, backup_level, backup_verbose_file_path, num_threads,
	sleep_msecs) == NULL)
	...
	(thread_p, log_Db_fullname, &log_Gl.hdr.db_creation, backup_level, &bkup_start_lsa, &chkpt_lsa, all_bkup_info,
  ...
  do
	...
	logpb_backup_needed_archive_logs (thread_p, &session, last_arv_needed + 1, log_Gl.hdr.nxarv_num - 1);
		    ...
		    BO_RESTART_ARG * r_args) 
{
	...
	(thread_p, db_fullname, logpath, prefix_logname, &db_iopagesize, &log_page_size, &db_creation,
	...
	
	{
	   ...
	   (thread_p, db_fullname, from_volbackup, db_creation, &bkdb_iopagesize, &bkdb_compatibility,
	    &session_storage, try_level, printtoc, bkup_match_time, r_args->verbose_file, r_args->newvolpath) == NULL)
 ...
 *              renaming volumes.
 */ 
static int logpb_start_where_path (const char *to_db_fullname, const char *toext_path, const char **toext_name,
				    char **ext_path, char **alloc_extpath, const char *fileof_vols_and_wherepaths,
				    FILE ** where_paths_fp) 
{
 ...
 */ 
static int logpb_next_where_path (const char *to_db_fullname, const char *toext_path, const char *ext_name,
				   char *ext_path, const char *fileof_vols_and_wherepaths, FILE * where_paths_fp,
				   int num_perm_vols, VOLID volid, char *from_volname, char *to_volname) 
{
 ...
 */ 
int logpb_copy_database (THREAD_ENTRY * thread_p, VOLID num_perm_vols, const char *to_db_fullname,
			  ...
			  const char *fileof_vols_and_copypaths) 
{
  ...
  if (fileio_write (thread_p, to_vdes, to_malloc_log_pgptr, phy_pageid, LOG_PAGESIZE) == NULL)
				     ...
				     const char *fileof_vols_and_renamepaths, int extern_rename, bool force_delete) 
{
	...
	
	{
	    ...
	    fileio_mount (thread_p, to_db_fullname, to_volname, LOG_DBLOG_ACTIVE_VOLID, true, false);
 ...
 *              database is exited.
 */ 
void logpb_fatal_error (THREAD_ENTRY * thread_p, bool log_exit, const char *file_name, const int lineno,
			 const char *fmt, ...) 
{
...
} void logpb_fatal_error_exit_immediately_wo_flush (THREAD_ENTRY * thread_p, const char *file_name,
							const int lineno, const char *fmt, ...) 
{
...
} static void logpb_fatal_error_internal (THREAD_ENTRY * thread_p, bool log_exit, bool need_flush,
					      const char *file_name, const int lineno, const char *fmt, va_list ap) 
{
 ...
 */ 
static int logpb_backup_needed_archive_logs (THREAD_ENTRY * thread_p, FILEIO_BACKUP_SESSION * session,
					      int first_arv_num, int last_arv_num) 
{
...
static bool  logpb_remote_ask_user_before_delete_volumes (THREAD_ENTRY * thread_p, const char *volpath) 
{
	...
	||asprintf (&ptr2, msgcat_message (MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG, MSGCAT_LOG_DELETE_BKVOLS),
		    volpath) < 0  ||asprintf (&ptr3, msgcat_message (MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG,
								      MSGCAT_LOG_ENTER_Y2_CONFIRM)) <
	...
	(thread_p, FILEIO_PROMPT_BOOLEAN_TYPE, fullmsg, user_response, NULL, -1, -1, NULL, -1) != NO_ERROR)
