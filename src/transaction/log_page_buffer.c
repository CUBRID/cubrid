...
#define SIZEOF_LOG_BUFFER \
  (offsetof(struct log_buffer, logpage) + SIZEOF_LOG_PAGE_PAGESIZE)
...
static void logpb_set_unavialble_archive (int arv_num);
static bool logpb_is_archive_available (int arv_num);
static LOG_PAGE *logpb_fetch_from_archive (THREAD_ENTRY * thread_p,
					   PAGEID pageid,
					   LOG_PAGE * log_pgptr,
					   int *arv_num);
...
static void logpb_remove_archive_to_page_id (THREAD_ENTRY * thread_p,
					     ...
					     PAGEID safe_pageid);
static int logpb_get_remove_archive_num (PAGEID safe_pageid, int archive_num);
static void logpb_remove_archive_logs_internal (int first,
						int last,
						const char *info_reason,
						bool check_backup);
...
logpb_initialize_log_buffer (LOG_BUFFER * log_buffer_p)
{
   ...
   */
  MEM_REGION_INIT (&log_buffer_p->logpage, IO_PAGESIZE);
...
logpb_initialize_pool (THREAD_ENTRY * thread_p)
{
  ...
  MUTEX_INIT (log_Gl.flush_run_mutex);

  COND_INIT (log_Gl.group_commit_info.gc_cond);
  MUTEX_INIT (log_Gl.group_commit_info.gc_mutex);
...
logpb_finalize_pool (void)
{
  ...
  MUTEX_DESTROY (log_Gl.flush_run_mutex);
...
logpb_fix_page (THREAD_ENTRY * thread_p, PAGEID pageid, int fetch_mode)
{
  ...
  if (log_bufptr == NULL)
	...
	{
	  if (logpb_read_page_from_file
	      (thread_p, pageid, &log_bufptr->logpage) == NULL)
	    {
	      goto error;
	    }
	}
...
logpb_dump_to_flush_page (FILE * out_fp)
{
  ...
  for (i = 0; i < log_Gl.flush_info.num_toflush; i++)
    {
      LOG_CAST_LOGPAGEPTR_TO_BFPTR (log_bufptr, log_Gl.flush_info.toflush[i]);
      if (i != 0)
	{
	  if ((i % 10) == 0)
	    (void) fprintf (out_fp, ",\n");
  ...
  if ((logpb_fetch_page (thread_p, LOGPB_HEADER_PAGE_ID, log_pgptr)) == NULL)
    {
      ...
      /* This statemant should not be reached */
...
/*   assert(log_lpb_mutex_not_own()); */
  ...
  if (log_bufptr != NULL)
    {
      memcpy (log_pgptr, &log_bufptr->logpage, SIZEOF_LOG_PAGE_PAGESIZE);
      ret_pgptr = log_pgptr;
    }
  LOG_MUTEX_UNLOCK (log_Pb.lpb_mutex);
			   ...
			   LOG_PAGE * log_pgptr)
{
  ...
  if (logpb_is_page_in_archive (pageid)
	  ...
	  || (pageid + LOGPB_ACTIVE_NPAGES) <= log_Gl.hdr.append_lsa.pageid))
    {
	  ...
	  (thread_p, pageid, log_pgptr, &ignore_arvnum) == NULL)
	{
	  goto error;
	}
    }
  else
    {
      ...
      if (fileio_read (log_Gl.append.vdes, log_pgptr, phy_pageid) == NULL)
	{
			  ...
			  PAGEID logical_pageid)
{
   ...
   */
  if (fileio_write (log_Gl.append.vdes, log_pgptr, phy_pageid) == NULL
      || fileio_synchronize (log_Gl.append.vdes,
			      ...
			      float *db_compatibility)
{
  ...
  if (log_Gl.trantable.area != NULL && log_Gl.append.log_pgptr != NULL)
    {
      *db_iopagesize = log_Gl.hdr.db_iopagesize;
      *db_creation = log_Gl.hdr.db_creation;
      *db_compatibility = log_Gl.hdr.db_compatibility;
      if (IO_PAGESIZE != *db_iopagesize)
	{
	  if (db_set_page_size (*db_iopagesize) != *db_iopagesize)
	    {
	      goto error;
	    }
	  logpb_finalize_pool ();
	  error_code =
	    logtb_define_trantable_log_latch (thread_p, log_Gl.trantable.
					      num_total_indices);
	  if (error_code != NO_ERROR)
	    {
	      goto error;
  ...
  *db_compatibility = hdr.db_compatibility;
   ...
   */
  if (rel_is_disk_compatible (*db_compatibility, NULL) == REL_NOT_COMPATIBLE)
    {
...
logpb_fetch_start_append_page (THREAD_ENTRY * thread_p)
{
  ...
  LOG_MUTEX_LOCK (rv, log_Gl.flush_info.flush_mutex);
  log_Gl.flush_info.toflush[log_Gl.flush_info.num_toflush] =
    log_Gl.append.log_pgptr;
  log_Gl.flush_info.num_toflush++;

  if (log_Gl.flush_info.num_toflush >= log_Gl.flush_info.max_toflush)
    {
      /*
  ...
  if (npages > 0)
    {
      ...
      if (fileio_writev
	  (log_Gl.append.vdes, (void **) to_flush, phy_pageid,
	   npages) == NULL)
	{
	  if (er_errid () == ER_IO_WRITE_OUT_OF_SPACE)
	    {
...
logpb_flush_all_append_pages_low (THREAD_ENTRY * thread_p)
{
  ...
  if (log_Gl.flush_info.num_toflush < 1)
    {
      need_flush = false;
    }
  ...
  if (log_Gl.flush_info.num_toflush == 1)
    {
      ...
      LOG_CAST_LOGPAGEPTR_TO_BFPTR (bufptr, log_Gl.flush_info.toflush[0]);
      assert (bufptr->fcnt > 0);

      if (!logpb_is_dirty (log_Gl.flush_info.toflush[0]))
	{
	  need_flush = false;
	}
    }
  ...
  if (log_Gl.append.nxio_lsa.pageid !=
      logpb_get_page_id (log_Gl.flush_info.toflush[0]))
    {
      er_log_debug (ARG_FILE_LINE,
  ...
  LOG_MUTEX_LOCK (rv, log_Gl.flush_run_mutex);
  ...
  for (i = 0; i < log_Gl.flush_info.num_toflush; i++)
    {
      LOG_CAST_LOGPAGEPTR_TO_BFPTR (bufptr, log_Gl.flush_info.toflush[i]);
      ...
      if (idxflush != -1 && prv_bufptr != NULL)
	{
	  ...
	  if ((bufptr->dirty == false)
	      ...
	      || (bufptr->phy_pageid != (prv_bufptr->phy_pageid + 1)))
	    {
	       ...
	       */
	      if (logpb_writev_append_pages
		  (thread_p, &log_Gl.flush_info.toflush[idxflush],
		   i - idxflush) == NULL)
		{
		  error_code = ER_FAILED;
		  goto error;
	      ...
	      else
		{
...
#if (LOG_DEBUG & LOG_DEBUG_MSG)
		  {
			...
			er_log_debug (ARG_FILE_LINE,
				      ...
				      "pageid(%d)\n",
				      (log_Gl.flush_info.toflush[idxflush])->
  ...
  if (idxflush != -1)
    {
      int pageToFlush = log_Gl.flush_info.num_toflush - idxflush;
      ...
      else
	{
#if (LOG_DEBUG & LOG_DEBUG_MSG)
	  {
		...
		er_log_debug (ARG_FILE_LINE,
			      ...
			      (log_Gl.flush_info.toflush[idxflush])->hdr.
			      logical_pageid + j);
	      }
	  }
#endif /* LOG_DEBUG & LOG_DEBUG_MSG */
  ...
  if (last_idxflush != -1)
    {
      ...
      if (logpb_writev_append_pages
	  (thread_p, &log_Gl.flush_info.toflush[last_idxflush], 1) == NULL
	  || fileio_synchronize (log_Gl.append.vdes,
				 !PRM_SUPPRESS_FSYNC) == NULL_VOLDES)
	{
	  error_code = ER_FAILED;
  ...
  for (i = 0; i < log_Gl.flush_info.num_toflush; i++)
      ...
      if (bufptr->dirty)
	{
      ...
      log_Gl.flush_info.toflush[i] = NULL;
#endif /* LOG_DEBUG */
      bufptr->dirty = false;
    }
#if defined(LOG_DEBUG)
...
#if defined(LOG_DEBUG)
  curr_flush_count = log_Gl.flush_info.num_toflush;
#endif /* LOG_DEBUG */
  ...
  if (log_Gl.append.delayed_free_log_pgptr != NULL)
    {
      ...
      log_Gl.flush_info.toflush[0] = log_Gl.append.delayed_free_log_pgptr;
      log_Gl.flush_info.num_toflush = 1;
  ...
  mutexState = 0;
  LOG_MUTEX_UNLOCK (log_Gl.flush_run_mutex);
...
direct_flush:
   ...
   */
  if (log_Gl.flush_info.flush_type == LOG_FLUSH_DIRECT
      || flush_type == LOG_FLUSH_DIRECT || !LOG_ISRESTARTED ())
    {
      ...
      er_log_debug (ARG_FILE_LINE,
		    ...
		    "[%d]flush direct\n", (int) THREAD_ID ());
#endif /* LOG_DEBUG & LOG_DEBUG_MSG */
      flush_type = log_Gl.flush_info.flush_type;
      log_Gl.flush_info.flush_type = LOG_FLUSH_DIRECT;
      (void) logpb_flush_all_append_pages_low (thread_p);
      log_Gl.flush_info.flush_type = flush_type;
  ...
  if (!PRM_LOG_ASYNC_COMMIT || (flush_type == LOG_FLUSH_FORCE))
    {
      ++log_Gl.group_commit_info.waiters;
#if (LOG_DEBUG & LOG_DEBUG_MSG)
      er_log_debug (ARG_FILE_LINE,
		    "logpb_flush_all_append_pages: "
		    "[%d]cond wait, flush_type(%d) waiters(%d)\n",
		    (int) THREAD_ID (),
		    flush_type, log_Gl.group_commit_info.waiters);
#endif /* LOG_DEBUG & LOG_DEBUG_MSG */
      if (LOG_IS_GROUP_COMMIT_ACTIVE ())
	{
      ...
      ret = COND_WAIT (log_Gl.group_commit_info.gc_cond,
		       log_Gl.group_commit_info.gc_mutex);
...
logpb_invalid_all_append_pages (THREAD_ENTRY * thread_p)
{
  ...
  LOG_MUTEX_LOCK (rv, log_Gl.flush_info.flush_mutex);
  log_Gl.flush_info.num_toflush = 0;
  log_Gl.flush_info.toflush[log_Gl.flush_info.num_toflush] = NULL;
  LOG_MUTEX_UNLOCK (log_Gl.flush_info.flush_mutex);
}
...
logpb_end_append (THREAD_ENTRY * thread_p, LOG_FLUSH flush, bool force_flush)
{
  ...
  if (log_rec->type != LOG_DUMMY_FILLPAGE_FORARCHIVE)
	...
	{
	  LOG_APPEND_ADVANCE_WHEN_DOESNOT_FIT (thread_p, IO_PAGESIZE);
	}
    }
  ...
  if (log_Gl.append.delayed_free_log_pgptr != NULL)
    {
       ...
       */
      for (i = log_Gl.flush_info.num_toflush - 1; i > 0; i--)
	{
	  if (log_Gl.flush_info.toflush[i] == log_Gl.append.log_pgptr)
	    break;
	  log_Gl.flush_info.num_toflush--;
	}
    }
...
/*
 * logpb_dump_log_info - Dump log information
 *
 * return: nothing
 *
 *   logname_info(in): Name of the log information file
 ...
 */
int
logpb_dump_log_info (const char *logname_info, int also_stdout,
		     const char *fmt, ...)
{
  FILE *fp;			/* Pointer to file                   */
...
logpb_get_guess_archive_num (PAGEID pageid)
{
  ...
  if (fp != NULL)
	...
	{
	  if (strstr (line, msgcat_message (MSGCAT_CATALOG_CUBRID,
					    MSGCAT_SET_LOG,
					    MSGCAT_LOG_LOGINFO_KEYWORD_ARCHIVE))
	      == line)
	    {
	      /* A candidate for a guess */
	      if (sscanf (line, "%*s %d %*s %d %d",
...
/*
 * logpb_set_unavialble_archive - Cache that given archive is unavailable
 *
 * return: nothing
 *
 ...
 */
static void
logpb_set_unavialble_archive (int arv_num)
{
  int *ptr;
  int size;
 ...
 */
void
logpb_decache_archive_info (void)
{
  assert (LOG_CS_OWN ());
  ...
  if (log_Gl.archive.unav_archives != NULL)
    {
      ...
      log_Gl.archive.unav_archives = NULL;
...
logpb_fetch_from_archive (THREAD_ENTRY * thread_p, PAGEID pageid,
			  LOG_PAGE * log_pgptr, int *arv_num)
{
  ...
  if (log_Gl.archive.vdes == NULL_VOLDES)
      ...
      if (log_Gl.hdr.nxarv_num <= 0)
	{
      ...
      *arv_num = logpb_get_guess_archive_num (pageid);
  ...
  while (true)
      ...
      else
	{
	  ...
	  if (has_guess_arvnum == false)
	    {
	      has_guess_arvnum = true;
	      if ((retry = logpb_get_guess_archive_num (pageid)) != *arv_num)
		{
		  *arv_num = retry;
		}
	    }
	      ...
	      if (arvhdr != NULL)
		{
		  if (direction == -1)
		  ...
		  else
		    {
		       ...
		       * current archive
		       */
		      if (pageid > arvhdr->fpageid + arvhdr->npages - 1)
			{
			  *arv_num += 1;
			}
		      else
	      ...
	      else
		{
		  ...
		  if (first_time != true)
			...
			{
			  *arv_num = *arv_num + direction;
			}
		    }
		}

	      first_time = false;
	      if (*arv_num < 0 || *arv_num == log_Gl.hdr.nxarv_num)
		{
	  ...
	  if (logpb_is_archive_available (*arv_num) == false)
	    {
	      arvhdr = NULL;
	      continue;
	    }
	  ...
	  while (retry != 0 && retry != 1
		...
		{
		case 0:	/* quit */
		  logpb_set_unavialble_archive (*arv_num);
		  er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE,
			  ER_LOG_NOTIN_ARCHIVE, 1, pageid);
		  logpb_fatal_error (thread_p, true, ARG_FILE_LINE,
...
logpb_archive_active_log (THREAD_ENTRY * thread_p, bool force_archive)
{
  ...
  old_flush_type = log_Gl.flush_info.flush_type;
  log_Gl.flush_info.flush_type = LOG_FLUSH_DIRECT;

  if ((log_Gl.hdr.nxarv_num - log_Gl.hdr.last_deleted_arv_num)
  ...
  if (log_Gl.hdr.nxarv_pageid >= log_Gl.hdr.append_lsa.pageid)
    {
      er_log_debug (ARG_FILE_LINE, "log_archive_active_log: WARNING "
		    ...
		    " which is incomplete\n");
      log_Gl.flush_info.flush_type = old_flush_type;
      return;
    }
	  ...
	  if (!BO_ISSERVER_RESTARTED ())
	    {
	      pgbuf_flush_check_point (thread_p, &smallest_lsa,
				       &newsmallest_lsa);
	      if (fileio_synchronize_all (!PRM_SUPPRESS_FSYNC, false) !=
		  NO_ERROR)
		{
		  LSA_SET_NULL (&newsmallest_lsa);
		}
  ...
  vdes =
    fileio_format (thread_p, log_Db_fullname, arv_name,
...
#if !defined(GOORM_BMT)
  if (PRM_SUPPRESS_FSYNC)
    {
      fileio_synchronize (log_Gl.append.vdes, true /* force */ );
    }
#endif
				 ...
				 const char *info_reason, PAGEID safe_pageid)
{
  ...
  if ((!LSA_ISNULL (&newflush_upto_lsa)
      ...
      || fileio_synchronize_all (!PRM_SUPPRESS_FSYNC, false) != NO_ERROR)
    {
  ...
  if (safe_pageid != NULL_PAGEID)
    {
      last_deleted_arv_num = MIN (last_deleted_arv_num,
				  logpb_get_remove_archive_num (safe_pageid,
								last_deleted_arv_num));
    }

  if (last_deleted_arv_num >= 0)
    {
      logpb_remove_archive_logs_internal (first_deleted_arv_num,
 ...
 */
static int
logpb_get_remove_archive_num (PAGEID safe_pageid, int archive_num)
{
 ...
 */
static void
logpb_remove_archive_logs_internal (int first, int last,
				    const char *info_reason,
				    bool check_backup)
{
  ...
  last_bkup_arv_inrange = MIN (log_Gl.hdr.highest_arv_num_for_backup, last);
...
logpb_append_archives_delete_pend_to_log_info (int first, int last)
{
  ...
  if (first == last)
    {
      error_code = logpb_dump_log_info (log_Name_info, true, catmsg, first,
		     ...
		     LOG_LSA * log_lsa, LOG_PAGE * log_page_p)
{
   ...
   * statemant, otherwise, copy it in parts
   */
...
#if (LOG_DEBUG & LOG_DEBUG_MSG)
  er_log_debug (ARG_FILE_LINE, "log_chkpt: call log flushall_append_pages\n");
#endif /* LOG_DEBUG & LOG_DEBUG_MSG */
  logpb_force (thread_p);
	  ...
	  if (chkpt_one->state == TRAN_UNACTIVE_ABORTED)
	    {
	       ...
	       * same than its tail. The recovery process will read the last
  ...
  *chkpt = tmp_chkpt;
  ...
  if (PRM_SUPPRESS_FSYNC)
    {				/* have to sync log vol, data vol */
      fileio_synchronize_all (true /* force */ , true /* include_log */ );
    }
#endif
 ...
 */
void
logpb_dump_checkpoint_trans (int length, void *data)
{
  int ntrans, i;
 ...
 */
void
logpb_dump_checkpoint_topops (int length, void *data)
{
  int ntops, i;
  struct log_chkpt_topops_commit_posp *chkpt_topops;	/* Checkpoint top system
							 * operations that are in
...
loop:
  ...
  if (session.bkup.dtype != FILEIO_BACKUP_VOL_DEVICE)
      ...
      while (bk_vol && !beenwarned)
	{
      ...
      if (beenwarned)
	{
	  fileio_remove_all_backup (backup_level);
	}
    }
  ...
  fileio_abort_backup (&session, false);
		...
		BO_RESTART_ARG * r_args)
{
  ...
  if (fileio_is_volume_exist (log_Name_active))
    {
      lgat_vdes = fileio_mount (db_fullname, log_Name_active,
				LOG_DBLOG_ACTIVE_VOLID, true, false);
  ...
  while (success == NO_ERROR && try_level >= FILEIO_BACKUP_FULL_LEVEL
	     ...
	     (backup_volinfo_fp = fopen (from_volbackup, "r")) == NULL)
	{
      ...
      if (backup_volinfo_fp != NULL)
	{
	  if (fileio_read_backup_info_entries
	      (backup_volinfo_fp, FILEIO_FIRST_BACKUP_VOL_INFO) == NO_ERROR)
	    {
	      volnameptr = fileio_get_backup_info_volume_name (try_level,
      ...
      if (first_time && db_iopagesize != bkdb_iopagesize)
	{
	  ...
	  if (log_Gl.append.vdes == NULL_VOLDES)
	    {
	       ...
	       * Reset the page size
	       */
	      if (db_set_page_size (bkdb_iopagesize) != bkdb_iopagesize)
		{
		  error_code = ER_FAILED;
		  log_Gl.flush_info.flush_type = LOG_FLUSH_NORMAL;
		  LOG_CS_EXIT ();
      ...
      if (db_compatibility != bkdb_compatibility)
	{
  ...
  if (restore_in_progress)
    {
      ...
      fprintf (stdout,
	       ...
	       session->bkup.name,
	       ((session->bkup.bkuphdr ==
		 NULL) ? FILEIO_INITIAL_BACKUP_UNITS : session->bkup.bkuphdr->
		unit_num), to_volname, session->dbfile.volid);
		   ...
		   LOG_LSA * to_volchkpt_lsa)
{
  ...
  if (fileio_synchronize (from_vdes, !PRM_SUPPRESS_FSYNC) != from_vdes)
    {
      return ER_FAILED;
    }
		     ...
		     const char *fileof_vols_and_copypaths)
{
  ...
  fileio_make_backup_volume_info_name (to_volname, to_logpath,
				       to_prefix_logname);
  if (logpb_add_volume
      (to_db_fullname, LOG_DBLOG_BKUPINFO_VOLID, to_volname,
       DISK_UNKNOWN_PURPOSE) != LOG_DBLOG_BKUPINFO_VOLID)
    {
      error_code = ER_FAILED;
      goto error;
  ...
  to_malloc_log_pgptr = (LOG_PAGE *) malloc (IO_PAGESIZE);
  ...
  if (fileio_write (to_vdes, to_malloc_log_pgptr, phy_pageid) == NULL)
    {
      error_code = ER_FAILED;
      fileio_dismount (to_vdes);
      goto error;
  ...
  if (to_volinfo_fp != NULL)
      ...
      while (true)
	{
	  ...
	  if (volid != NULL_VOLID)
	    {
	      error_code =
		disk_set_link (thread_p, LOG_DBCOPY_VOLID, to_volname, false,
			       DISK_FLUSH);
	      if (error_code != NO_ERROR)
		{
		  fileio_dismount (to_vdes);
	      ...
	      if (error_code != NO_ERROR)
		{
	      ...
	      if (fileio_synchronize (to_vdes,
				      !PRM_SUPPRESS_FSYNC) != to_vdes)
		{
		  fileio_dismount (to_vdes);
		  error_code = ER_FAILED;
		  goto error;
	  ...
	  to_vdes =
	    fileio_mount (log_Db_fullname, to_volname, LOG_DBCOPY_VOLID,
			  false, false);
	  if (to_vdes == NULL_VOLDES)
	    {
	      error_code = ER_FAILED;
				...
				int extern_rename, bool force_delete)
{
  ...
  if (force_delete)
      ...
      if (to_volinfo_fp != NULL)
	{
	  ...
	  if (fileio_read_backup_info_entries
	      (to_volinfo_fp, FILEIO_FIRST_BACKUP_VOL_INFO) == NO_ERROR)
	    {
	      /* Remove any backups that have been created up to now */
	      fileio_remove_all_backup (-1);
	      fileio_finalize_backup_info (FILEIO_FIRST_BACKUP_VOL_INFO);
	    }
	  ...
	  fileio_unformat (log_Name_bkupinfo);
	}
    }
  ...
  if (extern_rename == true)
    {
	  ...
	  != NULL)
	{
	  log_Gl.append.vdes = fileio_mount (to_db_fullname, to_volname,
					     LOG_DBLOG_ACTIVE_VOLID, true,
					     false);
	}
      else
	{
	  log_Gl.append.vdes = fileio_mount (log_Db_fullname, log_Name_active,
					     LOG_DBLOG_ACTIVE_VOLID, true,
					     false);
	  error_code = ER_FAILED;
	  goto error;
   ...
   * them back.
   */
  fileio_unformat (log_Name_volinfo);
  fileio_unformat (log_Name_info);
  ...
  for (volid = LOG_DBFIRST_VOLID; volid < num_perm_vols; volid++)
      ...
      if (error_code != NO_ERROR)
	{
      ...
      if (fileio_synchronize (fileio_get_volume_descriptor (volid),
			      !PRM_SUPPRESS_FSYNC) == NULL_VOLDES)
	{
	  error_code = ER_FAILED;
	  goto error;
      ...
      if (extern_rename == true)
	{
	  fileio_dismount (fileio_get_volume_descriptor (volid));
	  if (fileio_rename (volid, from_volname, to_volname) != NULL)
	    {
	      (void) fileio_mount (to_db_fullname, to_volname, volid, false,
				   false);
	    }
	  else
	    {
	      (void) fileio_mount (log_Db_fullname, from_volname, volid,
				   false, false);
	      error_code = ER_FAILED;
	      goto error;
	    }
	      ...
	      const char *prefix_logname, bool force_delete)
{
  ...
  if (force_delete)
      ...
      if (db_volinfo_fp != NULL)
	{
	  ...
	  if (fileio_read_backup_info_entries
	      (db_volinfo_fp, FILEIO_FIRST_BACKUP_VOL_INFO) == NO_ERROR)
	    {
	      /* Remove any backups that have been created up to now */
	      fileio_remove_all_backup (-1);
	      fileio_finalize_backup_info (FILEIO_FIRST_BACKUP_VOL_INFO);
  ...
  fileio_unformat (log_Name_active);
  fileio_unformat (log_Name_info);
...
{
  log_Gl.flush_info.max_toflush = log_Pb.num_buffers - 1;
  log_Gl.flush_info.num_toflush = 0;
  log_Gl.flush_info.toflush = (LOG_PAGE **) calloc (log_Pb.num_buffers,
						    sizeof (log_Gl.
							    flush_info.
							    toflush));

  MUTEX_INIT (log_Gl.flush_info.flush_mutex);
}
...
logpb_finalize_flush_info (void)
{
  ...
  if (log_Gl.flush_info.toflush != NULL)
    {
      free_and_init (log_Gl.flush_info.toflush);
    }

  log_Gl.flush_info.max_toflush = 0;
  log_Gl.flush_info.num_toflush = 0;
  log_Gl.flush_info.toflush = NULL;

  LOG_MUTEX_UNLOCK (log_Gl.flush_info.flush_mutex);
  MUTEX_DESTROY (log_Gl.flush_info.flush_mutex);
}
...
logpb_flush_pages_background (THREAD_ENTRY * thread_p)
{
  ...
  assert (log_Gl.flush_info.toflush != NULL);
  ...
  if (flush_expected == 0)
    {
      flush_expected = log_Gl.flush_info.max_toflush;
    }

  assert (log_Gl.flush_info.num_toflush > 0);

  LOG_MUTEX_LOCK (rv, log_Pb.lpb_mutex);

  find_first = false;
  for (loop = 0; loop != log_Gl.flush_info.num_toflush; ++loop)
    {
      LOG_CAST_LOGPAGEPTR_TO_BFPTR (bufptr, log_Gl.flush_info.toflush[loop]);
      ...
      if (flush_count >= flush_expected)
	{
      ...
      if (logpb_writev_append_pages
	  (thread_p, &log_Gl.flush_info.toflush[loop], 1) == NULL)
	{
	  logpb_fatal_error (thread_p, true, ARG_FILE_LINE,
			     "log_flush_pages_bg");
	  goto error;
      ...
      assert (log_Gl.flush_info.toflush[loop] != NULL);
    }
  if (flush_count > 0)
    {
      if (fileio_synchronize (log_Gl.append.vdes, !PRM_SUPPRESS_FSYNC) ==
	  NULL_VOLDES)
	{
	  goto error;
	}
  ...
  LOG_MUTEX_UNLOCK (log_Pb.lpb_mutex);
  assert (flush_count < log_Gl.flush_info.num_toflush - 1);
