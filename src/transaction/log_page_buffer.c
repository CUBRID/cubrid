...
#define SIZEOF_LOG_BUFFER (offsetof (LOG_BUFFER, logpage) + sizeof(LOG_PAGE*))
 ...
 */
  void logpb_free_without_mutex (LOG_PAGE * log_pgptr)
  {
    LOG_BUFFER *bufptr;		/* Log buffer associated with given page */
 ...
 */
  int logpb_print_hash_entry (FILE * outfp, const void *key, void *ent, void *ignore)
  {
 ...
 */
  LOG_PAGE *logpb_fetch_page (THREAD_ENTRY * thread_p, LOG_LSA * req_lsa, LOG_CS_ACCESS_MODE access_mode,
			      LOG_PAGE * log_pgptr)
  {
 ...
 *
 */
  LOG_PAGE *logpb_copy_page_from_log_buffer (THREAD_ENTRY * thread_p, LOG_PAGEID pageid, LOG_PAGE * log_pgptr)
  {
    LOG_PAGE *ret_pgptr = NULL;
 ...
 *
 */
  LOG_PAGE *logpb_copy_page_from_file (THREAD_ENTRY * thread_p, LOG_PAGEID pageid, LOG_PAGE * log_pgptr)
  {
    LOG_PAGE *ret_pgptr = NULL;
 ...
 */
  LOG_PAGE *logpb_read_page_from_file (THREAD_ENTRY * thread_p, LOG_PAGEID pageid, LOG_CS_ACCESS_MODE access_mode,
				       LOG_PAGE * log_pgptr)
  {
    bool log_csect_entered = false;
 ...
 *   log_pgptr(out):
 */
  int logpb_read_page_from_active_log (THREAD_ENTRY * thread_p, LOG_PAGEID pageid, int num_pages, LOG_PAGE * log_pgptr)
  {
 ...
 */
  LOG_PAGE *logpb_write_page_to_disk (THREAD_ENTRY * thread_p, LOG_PAGE * log_pgptr, LOG_PAGEID logical_pageid)
  {
    int nbytes;
    LOG_PHY_PAGEID phy_pageid;
 ...
 */
  LOG_PAGE *logpb_fetch_start_append_page (THREAD_ENTRY * thread_p)
  {
 ...
 */
  LOG_PRIOR_NODE *prior_lsa_alloc_and_copy_crumbs (THREAD_ENTRY * thread_p, LOG_RECTYPE rec_type, LOG_RCVINDEX rcvindex,
						   ...
						   const LOG_CRUMB * ucrumbs, const int num_rcrumbs,
						   const LOG_CRUMB * rcrumbs)
  {
    LOG_PRIOR_NODE *node;
    int error = NO_ERROR;
 ...
 *   node(in):
 */
  static int logpb_append_next_record (THREAD_ENTRY * thread_p, LOG_PRIOR_NODE * node)
  {
 ...
 */
  int logpb_recreate_volume_info (THREAD_ENTRY * thread_p)
  {
    ...
    if (logpb_add_volume (NULL, LOG_DBLOG_ACTIVE_VOLID, log_Name_active, DISK_UNKNOWN_PURPOSE) !=
 ...
 */
  LOG_PAGE *logpb_fetch_from_archive (THREAD_ENTRY * thread_p, LOG_PAGEID pageid, LOG_PAGE * log_pgptr,
	...
	else
	  {
		...
		switch (retry)
		  ...
		  default:	/* Something strange.  Get user to try again. */
		    fprintf (stdout,
			     msgcat_message (MSGCAT_CATALOG_CUBRID, MSGCAT_SET_LOG, MSGCAT_LOG_INPUT_RANGE_ERROR), 0,
			     3);
		    goto retry_prompt;
		  }
	      }
	...
	if (first_arv_num_not_needed != -1)
	  {
		...
		error_code = log_dump_log_info (log_Name_info, true, catmsg, logarv_name);
	      ...
	      {
		fileio_make_log_archive_name (logarv_name_first, log_Archive_path, log_Prefix,
 ...
 */
  void logpb_dump_checkpoint_topops (FILE * out_fp, int length, void *data)
  {
    int ntops, i;
    LOG_INFO_CHKPT_TOPOPS_COMMIT_POSP *chkpt_topops;	/* Checkpoint top system operations that are in commit postpone 
							 * mode */
 ...
 */
  ...
  loop:
	...
	if (fileio_is_volume_exist_and_file (log_Name_bkupinfo) != true)
	  {
	    er_set_with_oserror (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_LOG_CREATE_DBBACKUP_DIRINFO, 1,
				 log_Name_bkupinfo);
 ...
 *
 */
  void logpb_get_nxio_lsa (LOG_LSA * nxio_lsa_p)
  {
 ...
 */
  const char *logpb_perm_status_to_string (enum LOG_PSTATUS val)
  {
    switch (val)
      {
