...
lock_set_error_for_timeout (THREAD_ENTRY * thread_p, LK_ENTRY * entry_ptr)
{
  ...
  for (entry = res_ptr->holder; entry != NULL; entry = entry->next)
      ...
      if (entry == entry_ptr)
	{
      ...
      assert (entry->blocked_mode == NULL_LOCK || entry->mark_deleted == 0);
					...
					LOCK lock)
{
  ...
  if (entry_ptr == NULL || entry_ptr->mark_deleted != 0)
    {
...
start:
  ...
  if (entry_ptr == NULL)
	...
	{
	  assert (wait_entry_ptr->mark_deleted == 0);
...
lock_tran_lk_entry:
  ...
  for (i = res_ptr->holder; i != NULL; i = i->next)
    {
      if (i != entry_ptr && i->mark_deleted == 0)
	{
	  assert (i->granted_mode >= NULL_LOCK && group_mode >= NULL_LOCK);
	  group_mode = lock_Conv[i->granted_mode][group_mode];
	  assert (group_mode != NA_LOCK);
...
start:
  do
      ...
      if (LK_RES_HAS_HIGHEST_LOCK_INVALID_FLAG (old_cnt_highest_lock_mode_with_version_and_flags))
	{
      ...
      assert (res_ptr->non2pl == NULL);
      if (VOLATILE_ACCESS (res_ptr->holder, LK_ENTRY *) == entry_ptr
	  && (VOLATILE_ACCESS (entry_ptr->next, LK_ENTRY *) == NULL)
	  && VOLATILE_ACCESS (res_ptr->waiter, LK_ENTRY *) == NULL
	  && VOLATILE_ACCESS (res_ptr->non2pl, LK_ENTRY *) == NULL)
	{
	  /* The resource is not shared. Don't care about mark deletion here. This help us to remove
	   * the entries that are rarely accessed.
	   */
	  return false;
	}
      ...
      if (LK_RES_GET_HIGHEST_LOCK_COUNTER (new_cnt_highest_lock_mode_with_version_and_flags) == 0)
	{
	  ...
	  /* Needs cleanup and highest lock recomputation. It is safety to cleanup with mutex. */
	  return false;
	}
    }
  while (!ATOMIC_CAS_64
				     ...
				     bool release_flag, bool move_to_non2pl)
{
  ...
  if (release_flag == false)
    ...
    {
      if (lock_internal_set_mark_deleted (thread_p, entry_ptr))
	{
	  return;
	}
      else if (entry_ptr->mark_deleted == 2)
	{
...
lock_has_lock_on_object (const OID * oid, const OID * class_oid, int tran_index, LOCK lock)
{
  ...
  if (OID_EQ (oid, oid_Root_class_oid))
    {
      rv = pthread_mutex_lock (&tran_lock->hold_mutex);
      if (tran_lock->root_class_hold != NULL && tran_lock->root_class_hold->mark_deleted == 0)
	{
...
#define HOLDER_ENTRY_LENGTH (12)
  ...
  *out_buf = NULL;
  ...
  if (is_valid == false)
    {
      ...
      while (holder != NULL)
	{
	  if (holder->blocked_mode != NULL_LOCK && holder->tran_index == waiter_index && holder->mark_deleted == 0)
	    {
	      is_valid = true;
	      break;
	    }
  ...
  while (holder != NULL)
    {
      if (holder->tran_index != waiter_index && holder->mark_deleted == 0)
	{
	  holder_number++;
	}
      holder = holder->next;
...
lock_event_log_blocking_locks (THREAD_ENTRY * thread_p, FILE * log_fp, LK_ENTRY * wait_entry)
{
  ...
  for (entry = res_ptr->holder; entry != NULL; entry = entry->next)
    {
      if (entry == wait_entry || entry->mark_deleted == 0)
	{
	  continue;
	}
...
{
  UINT64 old_cnt_highest_lock_mode_with_version_and_flags, new_cnt_highest_lock_mode_with_version_and_flags;
  bool needs_highest_lock_recomputation = true;
  ...
  if ((new_total_holders_mode <= IX_LOCK)
      && (old_total_holders_mode == new_total_holders_mode || old_total_holders_mode == NULL_LOCK))
    {
      ...
      do
	{
	  needs_highest_lock_recomputation = false;
	  old_cnt_highest_lock_mode_with_version_and_flags = res_ptr->cnt_highest_lock_mode_with_version_and_flags;
	  ...
	  if (new_total_holders_mode != LK_RES_GET_HIGHEST_LOCK_MODE (old_cnt_highest_lock_mode_with_version_and_flags))
		...
		{
		  needs_highest_lock_recomputation = true;
		  break;
		}
	    }
  ...
  if (needs_highest_lock_recomputation == false)
    {
      /* Nothing to do. */
      return;
    }
