...
static bool log_verify_dbcreation (THREAD_ENTRY * thread_p, VOLID volid, const INT64 * log_db_creation);
 ...
 *   log_db_creation(in): Database creation time according to the log.
 *
 ...
 */
static bool
log_verify_dbcreation (THREAD_ENTRY * thread_p, VOLID volid, const INT64 * log_db_creation)
{
  INT64 db_creation;		/* Database creation time in volume */

  if (disk_get_creation_time (thread_p, volid, &db_creation) != NO_ERROR)
    {
      return false;
    }

  if (difftime ((time_t) db_creation, (time_t) (*log_db_creation)) == 0)
    {
      return true;
    }
  else
...
log_dump_header (FILE * out_fp, LOG_HEADER * log_header_p)
{
  ...
  time_t db_creation = (time_t) log_header_p->db_creation;
  time_t vol_creation = (time_t) log_header_p->vol_creation;
  ...
  fprintf (out_fp,
	   ...
	   "     DB_creation_time = %s, Volume_creation_time = %s\n"
	   ...
	   (long long) offsetof (LOG_PAGE, area), db_creation_val, vol_creation_val, log_header_p->db_release,
				  ...
				  const char *prefix_logname)
{
  ...
  INT64 dummy_ignore_vol_creation;
  ...
  if (logpb_find_header_parameters (thread_p, false, db_fullname, logpath, prefix_logname, &dummy_db_iopagesize,
				    &dummy_ignore_log_page_size, &dummy_ignore_db_creation,
				    &dummy_ignore_vol_creation, &dummy_ignore_dbcomp, &db_charset) == -1)
    {
      /*
       * For case where active log could not be found, user still needs
       * an error.
