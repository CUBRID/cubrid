...
struct db_page
{
    ...
    log_recovery_ns::ux_redo_lsa_log_entry generate_changes (const db_global_values &_db_global_values)
    {
      ...
      log_recovery_ns::ux_redo_lsa_log_entry entry_to_append
      {
        new log_recovery_ns::redo_log_rec_entry (lsa_log_entry_id, vpid, millis)
      };
      entries.push_front (std::move (entry_to_append));

      log_recovery_ns::ux_redo_lsa_log_entry entry_to_return
      {
        new log_recovery_ns::redo_log_rec_entry (lsa_log_entry_id, vpid, millis)
      };
    ...
    void apply_changes (log_recovery_ns::ux_redo_lsa_log_entry &&_lsa_log_entry)
    {
  ...
  private:
    std::forward_list<log_recovery_ns::ux_redo_lsa_log_entry> entries;
	  ...
	  if (update_or_add_page == 1 || pages.size() == 0)
	    {
	      ...
	      const auto lsa_log_entry_id = _db_global_values.increment_lsa_log_entry_id();
	      const int millis = _db_global_values.duration_in_millis();
	      log_recovery_ns::ux_redo_lsa_log_entry entry
	      {
		new log_recovery_ns::redo_log_rec_entry (lsa_log_entry_id, { /*static_cast<short> (pages.size())*/ NULL_PAGEID, volid }, millis)
	      };
	      add_new_page (pages);
	      return entry;
	    }
	  ...
	  if (update_or_add_volume == 1 || volumes.size() == 0)
	    {
	      ...
	      log_recovery_ns::ux_redo_lsa_log_entry entry
	      {
		new log_recovery_ns::redo_log_rec_entry (lsa_log_entry_id, { NULL_PAGEID, /*static_cast<short> (volumes.size())*/ NULL_VOLID }, millis)
	      };
