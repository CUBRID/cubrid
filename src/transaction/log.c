 ...
 *
 * log.c -
 *              LOG AND RECOVERY MANAGER (AT THE SERVER)
 *
 * Note:
 * The transaction concept encapsulates the ACID(Atomicity, Consistency,
 * Isolation, and Durability) properties[HAER83]. The log and recovery manager
 * is responsible for the atomicity and durability properties of the
 * transaction in the face of transaction, system, and media failures and
 * concurrent execution of multiple transactions. For this purpose,
 * transaction database updates are recorded in a log as log records. The log
 * is the source for ensuring either that the transaction's committed actions
 * are reflected in the database despite failures, or that its uncommitted
 * actions are removed from the database. Conceptually, the log is an infinite
 * sequential file in which log records are appended. The log actually
 * consists of a set of sequential files that are created by the system. The
 * log with the most recent changes is called the active log, and the rest
 * are called the archive logs. The log is used to refer to the entire set of
 * active and archive logs in the rest of the module. When the active log
 * becomes full, its pages are archived in an archive log to support
 * multimedia crashes and the effect of changes that have not been recorded in
 * the database disk. CUBRID declares archive logs that are no longer needed
 * for normal processing. However, the archive logs are still needed to
 * support media crashes as we will see later. The log contains a log header
 * which is composed of information such as:
 *
 * LOG_HEADER:
 *  db_creation_time: Database creation time, for safety reasons the value is
 *                    duplicated in every database volume.
 *  db_compatibility: Compatibility of the current database against the
 *                    current release.
 *  db_iopagesize:    Size of pages in the database.
 *  is_shutdown:      Was CUBRID shutdown?
 *  next_tranid:      Next transaction identifier
 *  npages:           Number of pages in the active portion of the log
 *  fpageid:          Logical log pageid at the first page (physical) of the
 *                    active log.
 *  append_lsa:       Current append location
 *  chkpt_lsa:        LSA of a checkpoint. Lowest LSA address to start the
 *                    recovery phase.
 *  nxarv_pageid:     Next logical log page to archive
 *  nxarv_phy_pageid: Physical location on the active log for the next logical
 *                    page to archive.
 *  lastarv_for_syscrashes: Last archive needed for system crashes. For media
 *                    crash all archives are needed since last database backup
 *  nxarv_num:        Next log archive number identifier
 *  prefix_name       Prefixes for the logs
 *
 * The first three fields are used to detect inconsistencies. For example the
 * database and the log do not belong to the same database. Another
 * incompatibility is when one database is restarted with the wrong CUBRID
 * version, etc. The is_shutdown field is used to find out if a system crash
 * recovery is needed during no-media crash restart. The rest of the fields
 * are used to simulate the infinite log. The active log is seen as a ring.
 * Pages that are no longer needed are archived to accommodate new appends.
 * Some pages that may still be needed may be archived to accommodate new
 * appends, for example, a transaction with a very huge number of updates.
 *
 * An archive log also contains a header to describe the log archive:
 * ARCHIVE_HEADER:
 *  next_tranid:      Next transaction identifier
 *  npages:           Number of pages in the log archive
 *  fpageid:          Logical log pageid at the first page (physical) of the
 *                    archive log.
 *  arv_num:          The archive number
 *
 * A Log record contains either or both data about how to undo and redo the
 * update. Depending upon the performed update, the undo/redo information may
 * be recorded physically (e.g., values of specific fields within an object)
 * or operationally (e.g., add 5 to a field of a page header). Operation
 * logging permits the use of high concurrency activities, which exploits the
 * semantics of the operations performed on the data. For example, with
 * certain operations, the same field of a record could have uncommitted
 * updates of many transactions. This is important to allow more concurrency
 * in some data structures such as B+tree, hashing, heap manager. Every log
 * record is assigned a unique log sequence address (LSA) when that record is
 * appended to the log. The LSAs are the logical addresses in the infinite
 * log.
 *
 * Log records are actually appended into log buffers. Log buffers are written
 * out to disk at a later times such as commit time or when data pages are
 * forced by the page buffer manager according to the widely accepted write
 * ahead logging (WAL) protocol[GRAY78]. This protocol asserts that log
 * records representing changes to some data must already be on disk before
 * the actual changed data is written out to disk. In other words, an updated
 * data page cannot be written out to disk until at least the undo portions of
 * the log records, which describes the updates to the page have been written
 * to disk. To enforce this protocol, the log and page buffer manager works
 * together. The most recent LSA of a log record that describes changes to a
 * page is copied in the header of the page. That is, when an update is to be
 * executed on an object of a page, after the object is locked, the page where
 * the object resides is fetched and locked in X mode, a log record describing
 * the old and new content of the object is logged, the LSA of the log record
 * is placed in the LSA of the page, the update is performed, the page is set
 * dirty, freed, and unlocked. The lock on the page is acquired to ensure
 * physical consistency on the page, for example, it will avoid other
 * transactions to be moving objects in the page. When the page is written
 * out, the page buffer manager notifies the recovery manager that log records
 * up to the LSA indicated in the page must be on disk.
 *
 * The log is also used to record status information of transactions. A
 * transaction cannot be considered complete until its committed status and
 * all its log records are written out to disk. This protocol allows the
 * restart recovery process to recover any transactions that completed
 * successfully but whose updated pages were not written out to disk before
 * system failures.
 *
 * The log and recovery manager maintains a set of log page buffers (the log
 * buffer pool) to hold the content of log disk pages, using a very simple
 * replacement technique. The replacement algorithm is based on the FIX/UNFIX
 * protocol[TRAI82] and the oldest log page scheme. All log disk pages I/Os
 * are directed to and form the log buffer; however, the actual disk reads
 * are writes are delegated to the I/O manager. When a new page need to be
 * defined to continue appending information to the log, the oldest page in
 * the log buffer pool that is not fixed is replaced. The rationale for this
 * is that if we need to go back to rollback other transactions, it is likely
 * that we need the earliest log pages and not the oldest one. A very old page
 * may be in the log buffer pool as a consequence of a previous rollback. The
 * log buffer pool is very small that we perform sequential scans to find out
 * if a page is resident. The log and recovery manager start with an area of
 * contiguous log buffers, if the number of log_buffers is very small, another
 * log_buffer area is defined. This could happen when a lot of aborts are done
 * at the same time.
 *
 * All log records written by a transaction are linked via their LSA in
 * chronological order. Thus, rollbacks are efficient. Partial rollbacks refer
 * to the the ability to place savepoints during the execution of the
 * transaction and later the transaction request the rolling back of the
 * changes performed by the transaction since the savepoint. Partial rollbacks
 * are crucial for handling, in a user-friendly fashion, integrity constraints
 * violations and application specific problems. After a partial rollback is
 * executed, the transaction can continue its execution going forward in the
 * log. At any point during the execution of a transaction, a savepoint can be
 * established. Any number of savepoints could be outstanding at a point in
 * time. A particular savepoint ceases if a partial rollback is performed to
 * that savepoint or to a preceding one or if a transaction rollback (total
 * rollback) is performed. When a savepoint is established, the LSA of the
 * latest log record written by the transaction is remembered as the
 * savepoint. When a partial rollback is desired, the LSA indicated by the
 * savepoint operation is supplied by the caller. Nested rollbacks may be
 * produced as a consequence of combinations of partial rollbacks and total
 * rollbacks or partial rollbacks whose point of termination is in an earlier
 * point of the first partial rollback. An Undo record is never executed twice
 * by producing a compensating log record (CLR) for the undo record. The CLR
 * is linked with the log records written during the normal execution of the
 * transaction. That is, when the undo of a log record causes a CLR to be
 * appended, the CLR beside containing a description of the compensating
 * action for redo purposes, is made to point to the the predecessor log
 * record of the just undone log record (i.e., next log record to be undone).
 * The LSA address of the predecessor log record is available in the undone
 * record since every log record, including the CLRs, of the transaction are
 * linked in chronological order throughout their LSAs. Thus, the most
 * recently CLR keeps track of the progress of the rollback. CLRs tells the
 * recovery manager from where to continue the rollback of a transaction, if a
 * system failure were to interrupt the completion of the rollback or if a
 * nested rollback were to be performed. Note that CLRs are never undone and
 * hence they are redo-only log records. A bounded fix amount of CLRs is
 * ensured during rollbacks, even in the presence of multiple failures since
 * undo records are never executed twice and CLRs are never undone. This
 * technique should not be confused with that of IBM`s IMS[PETE83] and DB2
 * [CRUS84] which may execute undo records several times and may undo CLR. In
 * the same way when a transaction abort is executed, the transaction is
 * rolled back by scanning the the list of log records backward and executing
 * undo actions. Redo and CPR are ignored and they are just used to find the
 * next (a preceding record in the log) log record to be considered for undo.
 *
 * We encourage that a single log record (i.e., an undo_redo log record) be
 * recorded for an update operation, however, we do not force this. If the
 * caller desires to record the undo and redo portions separately in two log
 * records, he must record the undo record before the redo record. There may
 * be some operations (e.g., allocation of data pages) that should never have
 * been undone. In this case a log record with only redo information can be
 * appended to the log. For example, new objects are inserted on the last page
 * of a heap file. The page is allocated by one transaction but it can be used
 * by another concurrent transaction. Thus, heap allocated pages cannot be
 * undone.
 *
 * Page-oriented undo (redo) is said to occur if the log record whose update
 * is being undone (redone) describes the page of the database where the
 * update occurs. The recovery (either undo or redo) of the content of the
 * page does not require accesses to any other data page of the database. This
 * makes the recovery process independent of updates. When other pages are
 * examined and possibly updated as part of the recovery process a logical
 * undo (redo) is said to happen. This makes the recovery process complex and
 * dependent on other data which may be in an uncertain state. The recovery
 * manager of CUBRID does not support logical redos, however, it supports
 * logical undoes to allow the system to provide higher levels of concurrency
 * that what would be possible in page-oriented undoes. This is because
 * logical undoes and CLR with appropriate concurrency control protocols,
 * would permit uncommitted updates of one transaction to be moved to a
 * different page by another transaction. For example, in a B+tree or
 * extendible hashing, one transaction must be able to split a page even if
 * that page currently has some uncommitted data inserted by another
 * transaction. Later, if the second transaction were to abort, then keys
 * inserted by the transaction are located by traversing the tree and then
 * deleting the keys from there (logical undo). CLRs are written to describe
 * the deleted keys in a page-oriented fashion for page oriented redos.
 * In short, the CUBRID log and recovery manager supports page-oriented redo
 * and undoes and logical undoes.
 *
 * The log records may describe operation logging or value logging so that
 * semantically rich concurrency could be supported. Operation logging may let
 * one transaction modify the same data that was modified earlier by another
 * transaction which has not yet committed, when the two transaction's
 * actions are semantically compatible (e.g., increment/decrement operations
 * in fields of data headers). Operation logging allows us to use computations
 * during the undo and redo of an action. Thus, everything that is changed on
 * a page does not need to be logged explicitly, thereby saving space. For
 * example, changes of control information in page headers such as the total
 * amount of free space may not need to be logged. The redo and undo
 * operations can find out the increments/decrements to the free space. The
 * difficulty in supporting operation logging is that the exact state of the
 * page with respect to the logged actions, must be known before a redo or
 * undo action is applied to the page. That is, a redo or undo of an update
 * should not be performed without being sure that the original update is not
 * present or is present, respectively. The state of a data page with respect
 * to updates is tracked down by using a LSA value on the header of the data
 * page. Whenever a page is updated and a log record is written for that
 * update, the LSA of the log record is copied into the LSA filed of the data
 * page.
 *
 * In several cases we would like some updates of a transaction to be
 * committed, irrespective of whether the transaction ultimately commits or
 * not. For example, When a transaction allocates a page for a file structure
 * such as the heap, extendible hashing, or B+tree, the allocation causes a
 * set of updates to the database such as updates to the volume allocation
 * table, file header, and so on. This allocated page is allowed to be used by
 * other transactions before the commit of the transaction that allocated the
 * page. If the transaction were to roll back, then it would not be acceptable
 * to undo the allocation of the page since such undo may lead to lost of
 * updates by other transactions, which may have been already committed. On
 * the other hand, if the set of updates made by the allocation failed, it
 * would be necessary to undo them. These kind of actions are handled by
 * opening a system_operation command which stack a transaction on top of the
 * current transaction. The current transaction will not be able to continue
 * until the stacked operation is finished. If the system_operation consists
 * of only a single update, then a redo_only log record would be enough.
 * Other applications for this technique are the split and merge operations in
 * the B+tree and extendible hashing which are independent of the transaction.
 *
 * The log manager takes checkpoints periodically during the normal
 * processing to reduce the amount of work to do during restart recovery. This
 * work relates to the extent of the log that needs to be scanned and the
 * amount of work that must be redone. For example, a dirty page should not
 * stay dirty for a long time in data buffers; otherwise, the recovery process
 * may take a substantial amount of time. The fuzzy checkpoint mechanism
 * flushes dirty data pages that have been dirty since the previous checkpoint
 * and also records the state of each active transaction (i.e., transaction
 * table) and the smallest LSA of the dirty pages that remain in the page
 * data buffer pool. The checkpoint is a fuzzy one since it is taken while
 * normal database activity i s going on without preventing access to any
 * data.
 * A checkpoint is taken when a fraction of the active log is used (i.e,
 * growth factor of the log). This factor can be specified using the system
 * parameter checkpoint_interval_factor (see DBA manual). The value of this
 * factor should be set with caution since if a very large factor is given,
 * the recovery process may take a long time. On the other hand, if the value
 * is very small, some performance problems will be experienced due to the
 * frequent flushing of dirty pages and gathering and recording information
 * since the previous checkpoint. A value between 15-30% is recommended.
 *
 * When a database is restarted after a normal shutdown or a machine failure,
 * the database is automatically recovered. The recovery process consists of
 * redoing any changes that were previously committed and currently missing
 * from the database disk, and undoing any changes that were not committed but
 * that are stored in the database disk. This restart recovery process ensures
 * the ACID properties of the transactions. The recovery is composed of three
 * phases: analysis, redo, and undo phase.
 *
 * In analysis phase, the log is scanned forward starting at the last
 * checkpoint log record, up to the end of the log. In this phase, we
 * determine the list of transactions that were in progress at the time of
 * the failure and which of these transactions need to be undone during the
 * undo phase and which of them need to be completed (e.g., it has after
 * commit lose-ends or client loose-ends). The current LSA addresses for undo
 * and loose-ends are found in the analysis phase. That is, the transaction
 * table is built in this phase. The starting address for the redo phase is
 * also found during the analysis phase by finding the smallest LSA of dirty
 * pages in the buffer pool at the time of the failure.
 *
 * In the redo phase, updates that are not reflected in the database are
 * repeated for not only the committed transaction but also for all aborted
 * transactions and the transactions that were in progress at the time of the
 * failure. This technique reestablishes the state of the database as of the
 * time of the failure. The redo phase starts by scanning the log records from
 * the redo LSA address determined in the analysis phase. When a redoable
 * record is found, a check is done to find out if the redo action is already
 * reflected in the page. If it is not, then the redo actions is executed. A
 * redo action can be skipped if the LSA of the affected page is greater or
 * equal than that of the LSA of the log record. No logging is done during the
 * redo phase. During this phase, any postpone actions (after commit actions)
 * of committed transactions that have not been executed are done. Loose_ends
 * of client actions that have not been done are postponed until the clients
 * are restarted. Locks are acquired to protect loose_end of client actions
 * (e.g., multimedia stuff).
 *
 * In the undo phase, all transactions that were in progress and not declared
 * as committed at the time of the failure are rolled back, in reverse
 * chronological order in a single sweep of the log. This is done by
 * continually finding the maximum of the LSAs of the next log records to be
 * undone for all of the transactions that remain to be undone. Unlike redos,
 * undoes do not have conditions (LSA comparasions) for their executions,
 * since the history was executed by the redo phase and undo records are
 * executed only once. In the process of rolling back an undo action, a CLR is
 * written. The next record to process for a transaction is determined by
 * looking at the previous LSA field in the log record. If a redo_only log
 * record or a compensating log record is found during the redo phase, they
 * are used just to determine the next log record to process. If a transaction
 * was rolling back at the time of the failure or it has done partial
 * rollbacks to return to a savepoint, the recovery manager will only rollback
 * those actions that had not already been undone.
 *
 * When a logical undo is executed, the system will allow undos and redos
 * since a logical undo may be composed (modify) of several parts of the DB,
 * and all of them must be undone. The undo records will help us to "undo"
 * whatever it was done during the logical undo, so we can restart the logical
 * undo from the beginning. That is, the logical undo, must be an atomic
 * recovery operation. At the end of the logical undo, a dump CLR is recorded.
 * This exception make sure that we do not have a situation in which a CLR
 * record of a partial "logical undo" whose UndoNxtLSA would be pointing to
 * the predecessor record to undo (i.e., next record to undo) when the full
 * logical undo operation has not been done. If the system would failed, the
 * crash recovery at restart time, would perform the remainder of the rollback
 * by starting the UndoNxtLSA of the transaction. Note that this field is
 * advanced when reading CLR during the anlysisis phase, therefore, we would
 * not finish the "logical undo". In addition, we must restart the "logical
 * undo" to the beginning since some of the remainding operations may not be
 * found if we start from the middle (e.g., case of deleting a key in a BTREE
 * and removing an empty page. The dummy CLR is the one that will point to the
 * predecessor undo record. A portion of this rule is described in [MOHA89].
 *
 * At the end of the restart recovery process all dirty data pages are written
 * out to disk and a checkpoint is taken.
 *
 * We should point out that the restart recovery phases of our log and
 * recovery manager differs from that of other schemes used by systems like
 * IBM System R, IBM's IMS, IBM's DB2[CRUS84], Ingres, Oracle. Our scheme is
 * very similar to the scheme proposed by ARIES[MOHA92]. Some of these
 * differences are:
 * - We do not perform selective redos on the fate of transactions.
 * - Redo phase precedes the undo phase.
 * - CLRs are logged during transaction rollbacks. Some systems do not log
 *   during transaction rollbacks.
 * - Undo log record are executed only once and CLR are never undone.
 * [MOHA92] describes why those schemes do not work in an environment where
 * the level of concurrency control is not the page.
 *
 * The log and recovery manager supports media crash recovery. Beside the
 * active and archive logs, database backups are used for media crash
 * recovery. A database backup is a fuzzy snapshot of a database. Backups are
 * fuzzy because they can be taken online when other transactions are updating
 * the database.Thus, the backup may contain some uncommitted updates. Even
 * worse, the backup is performed using the database disk directly. The more
 * recent version of some of the copied pages may be present in the page
 * buffer pool. We copy the database without going through to the page buffer
 * pool to avoid disrupting the page buffers. That is, avoid replacement of
 * useful pages and to avoid the page buffer manager overhead. When a fuzzy
 * backup is initiated, the location of the last checkpoint record is
 * remembered along with the backup. All updates with LSAs smaller than the
 * minimum of the LSA of the checkpoint record and the smallest LSA of the
 * dirty pages would have been written out to disk before the fuzzy checkpoint
 * has begun. Database backups should be taken at specific intervals. The
 * frequency with which you make these backups depends on the activity level
 * of updates to the database.
 *
 * Archive log and backup information is recorded in the information log file,
 * which is kept in the same directory as the rest of the logs and backups.
 * The information log is a text file that records the archive log and
 * database backups that have been created. The information log also provides
 * information about the archive logs that are no longer needed for normal
 * system restarts (i.e., other than media crash restarts/restores). These
 * archive logs can be sent to tape before they are destroyed, since they may
 * be needed if the database has a media failure. The information log also
 * provides information about which archive logs are no longer needed for
 * media recovery, assuming that the last database backup will no have media
 * failures. Before an archive log or database backup is destroyed by the DBA,
 * it is recommended that a snapshot of its contents be made and backed up to
 * tape in case the most recent database backup has a media failure due to
 * circumstances beyond our control.
 *
 * When a database is restarted after a media failure, some user intervention
 * is needed. The process consists of installing a backup of the database and
 * the active and archive logs since the database backup. The rest of the
 * process is similar to the normal restart recovery.
 *
 * We have seen that a database is composed of data volumes, database backup
 * files, and log files. The location and names of these files are specified
 * at database creation file (see createdb). We should emphasize that it is
 * strongly recommended that database files and log and database backup files
 * be placed in physically different disk devices. Otherwise, if the disk
 * where the database is located fails, the log and database backups may have
 * been damaged, and it may be impossible to recover the database. The name of
 * logs and backups are prefixed with a string given in the creation of the
 * database. If this prefix is not given, the name of the database is used.
 * For example, if the value of this prefix is "db",the names of the log
 * and backups are created as follow:
 *
 ...
 *
 * The log and recovery manager provides facilities to ignore media failures.
 * If media failure recovery is not desired, the log and recovery manager will
 * archive log pages of only active changes (i.e., those of currently running
 * transactions) and archive logs are automatically removed by the log manager
 * once the archive logs do not contain active changes. This behavior can be
 * specified by the user using the system parameter
 * media_failures_are_supported (See DBA manual for more explanations).
 *
 * The log manager and recovery manager also provide facilities to support
 * client recovery actions which are needed by the CUBRID multimedia manager
 * Client recovery actions are only of two types undo and postpine actions.
 * There is not a corresponding CLR for client undo actions. Client recovery
 * actions are always executed once. For this, there are few interface
 * functions that are exported to the client and in fact a small portion of
 * the recovery manager is placed in the client.
 *
 * This module also provide functions to copy and delete a database.
 *
 *
 * A transaction table is maintained by the log manager. Each entry in the
 * transaction table contains information about the transaction. An entry of
 * the transaction table is called the transaction descriptor. The transaction
 * descriptor is composed of the following fields:
 *
 * isloose_end:     Index to be assigned to a client user to finish client
 *                  loose ends.
 * tran_index:      Index onto transaction table.
 * tranid:          Transaction identifier.
 * state:           State of transaction such as active, committing, aborting.
 * waitsecs:        Wait until this number of seconds for locks.
 * head_lsa:        First LSA of transaction.
 * tail_lsa:        Last LSA of transaction.
 * undo_nxlsa:      Next LSA for undoes. Needed since CLR are logged during
 *                  UNDOs and partial rollbacks are supported.
 * posp_nxlsa:      Next LSA of a postpone action to execute. Most of the time
 *                  is the first address of a postpone record.
 * client_name:     Name of client user for whom the index was assigned
 * client_undo_lsa: First LSA of a client undo action
 * client_posp_lsa: First LSA of a client postpone action
 *
 *
 * MODULE INTERFACE
 *
 * The following modules are called by the log and recovery manager:
 *  input/output module:   to read and write log pages and fuzzy data pages
 *                            (e.g., during backups).
 *                         to mount and dismount log and data volumes.
 *                         to make fuzzy copies of data volumes (e.g., during
 *                            backups, restore, copydb).
 *  disk management module:to reset checkpoint values and creation times on
 *                            data volume headers. These values are replicated
 *                            in data volumes for verification and media crash
 *                            purposes.
 *  page buffer module:    to set/find LSA values on data pages.
 *                         to write pages that have been dirty for a long time
 *                         to find the smallest LSA of all dirty pages.
 *                         to flush all dirty pages during shutdown time.
 *                         to fetch data pages during restart recovery and
 *                            normal rollbacks.
 *  lock manager:          To release locks during commit/abort
 *  query evaluator:       To close cursors and release query resources during
 *                            commits and aborts.
 *
 * At least the following modules call the log manager:
 * Boot manager in the server: to restart and shutdown the system
 *                             to assign/free transaction indices to/from
 *                                clients
 *                             to backup, copy, delete databases
 *                             to restore a database
 * B+tree manager:             to record update actions
 * Catalog manager:            to record update actions
 * Disk Manager:               to record update actions
 * Extendible hashing manager: to record update actions
 * File manager:               to record update actions
 * Heap manager:               to record update actions
 * Long data manager:          to record update actions
 * Page buffer manager:        to follow WAL rule
 * Transaction manager:        to execute commit/aborts
 *
 *
 * REFERENCES:
 *
 * [CRUS84] Crus, R. "Data recovery in IBM Database 2," IBM System Journal,
 *          Vol. 23, No. 2, 1984.
 *
 ...
 *          1978, pp. 393-481.
 *
 * [HAER83] Haerder, T., and A. Reuter. "Principles of transaction oriented
 *          database recovery-A taxonomy," ACM Computer Survey, Vol. 15,
 *          No. 4, December 1983.
 *
 * [MOHA89] Mohan, C., F. Levine. "ARIES/IM: An Efficient and High Concurrency
 *          Index Management Method Using Write-Ahead Logging," IBM Research
 *          Report Number RJ 6846 (65380), August 29, 1989.
 *
 * [MOHA92] Mohan, C., et al. "ARIES: A Transaction Recovery Method Supporting
 *          Fine-Granularity Locking and Partial Rollbacks Using Write-Ahead
 *          logging", ACM Trans. on Database Systems, Vol. 17. No. 1,
 *          pp. 94-162, March 1992.
 *
 * [PETE83] Peterson, R. J., and J. P. Striickland. "Log write-ahead protocols
 *          and IMS/VS logging," In Proc. 2nd SIGACT-SIGMOD Symposium on
...
#include <time.h>
#if defined(SOLARIS)
#include <netdb.h>		/* umaks related */
#endif /* SOLARIS */
#include <sys/stat.h>		/* umaks related */
#include <assert.h>

#if defined(WINDOWS)
#include "porting.h"		/* for umask stubs */
#endif /* WINDOWS */
...
#include "wait_for_graph.h"	/* for wfg_get_tran_entries, wfg_dump */
 ...
 *
 * NOTE: Logging can be skipped for some specific database tasks. For
 *              example, to speed up the population of a newly created
 *              database. If there is a crash the user can always reexecute
 *              the creation and population of the database from scratch.
 *              There may be other cases, but the user is responsable of
 *              backing the the database before this operation is executed.
 *
 *              Logging can be skipped only in standalone mode when there is
 *              nothing that remains to recovery at this particular site. For
 *              logging cannot be skipped, if the current site is a
 *              participant of pending 2PC transaction.
 *
 * NOTE1: If logging is skipped, a transaction cannot be aborted and if there
 *        is a crash of somekind, your database may be corrupted. Therefore,
 *        the user should perform a backup of the database before the logging
 *        is turned off.
 *
 * NOTE2: After you are done with your job, you should backup the database,
 *        otherwise, you may not be able to redo the commits that were
 *        performed by your job after a media crash.
 *
 * NOTE3: During commit time all dirty page buffers are flushed to avoid
 *        corrupting the database in case of a failure.
 *
 * NOTE4: This function must be called after your database is restarted or
 *        your transaction is commited or aborted. See Note2.
 *
 * NOTE5: Logging off may not be good idea after all, if your backups are
 *        going to take a long time too.. Remeber you should perform two
 *        backups: one before you turn logging off and one after you are done
 *        with logging off.
 */
int
 ...
 *
 * NOTE:Log undo(before) and redo(after) data. A log record is
 *              constructed to recover data by undoing data during abort and
 *              during crash recovery, and by redoing data during crash
 *              recovery.
 *
 ...
 *              undo the data.
 *
 *              Similarly, for redo, the redo function described by rcvindex
 *              is called with a recovery structure which contains the page
 *              pointer and offset of the data to recover along with the redo
 *              data. It is up to this function to determine how to redo the
 *              data.
 *
 *     1)       The only type of logging accepted by this function is page
 *              operation level logging, thus, an address must be given.
 *              If logical logging (i.e., the one no associated with one
 *              page, but with a set of pages, for example in B+tree and
 *              extendible hashing), independent undo and redo functions must
 *              be used. REDO is always an operation page level logging. UNDO
 *              can be either operation page level logging or logical logging.
 *     2)       During the redo phase of crash recovery, any type of logging
 *              is not accepted.
 */
void
			   ...
			   const void *undo_data, const void *redo_data)
{
   ...
   * operation logging.
   *
   * The following is somewhat redundant to the above one. But we need to set
   * the LSA on the page anyhow.
   *
 ...
 *
 * NOTE:Log undo(before) and redo(after) crumbs of data. A log
 *              record is constructed to recover data by undoing data during
 *              abort and during crash recovery, and by redoing data during
 *              crash recovery. The log manager does not really record crumbs
 *              instead the log manager glues them togheter as a stream of
 *              data, and thus, it looses the knowledge that the data was
 *              from crumbs. This is done to avoid extra storage overhead. It
 *              is the responsibility of the recovery functions to build the
 *              crumbs when needed from the glued data. The main purpose of
 *              this function is to avoid memory copy of the crumbs into one
 *              memory area and then give it to the log manager.
 *
 *              In the case of a rollback, the undo function described by
 *              rcvindex is called with a recovery structure which contains
 *              the page pointer and offset of the data to recover along with
 *              the undo glued data. The undo function must construct the
 *              crumbs when needed. It is up to this function to determine how
 *              to undo the data, and build the crumbs (e.g., give the lengths
 *              as other crumbs).
 *
 *              Similarly, for redo, the redo function described by rcvindex
 *              is called with a recovery structure which contains the page
 *              pointer and offset of the data to recover along with the redo
 *              glued data. The redo function must construct the crumbs when
 *              needed, from the given glued data. It is up to this function
 *              to determine how to redo the data, and build the crumbs.
 *
 *     1)       Same notes as log_append_undoredo_data (see this function)
 *     2)       The only purpose of this function is to avoid extra data
 *              copying (the glue into one contiguous area) by the caller,
 *              otherwise, the same as log_append_undoredo_data.
 */
void
			    ...
			    const LOG_CRUMB * redo_crumbs)
{
  ...
  /*
   * When undo redo crumbs log compress
   * undo_crumbs[0] and redo_crumbs[0] must compress fail
   * and then make
   *
   *    undo_data :
   *                    undo_crumbs[0].data|undo_crumbs[1].data:...|undo_crumbs[num_undo_crumbs-1].data;
   *    redo_data :
   *                    redo_crumbs[0].data|redo_crumbs[1].data:...|redo_crumbs[num_redo_crumbs-1].data;
   *
   *    undo_data compress
   *    redo_data compress
   *
 ...
 *
 * NOTE:Log undo(before) crumbs of data. A log record is constructed
 *              to recover data by undoing data during abort and during crash
 *              recovery.
 ...
 *              as other crumbs).
 *
 *     1)       Same notes as log_append_undo_data (see this function)
 *     2)       The only purpose of this function is to avoid extra data
 *              copying (the glue into one contiguous area) by the caller,
 *              otherwise, the same as log_append_undo_data.
 *     3)       Very IMPORTANT: If an update is associated with two individual
 *              log records, the undo record must be logged before the redo
 *              record.
 */
void
 ...
 *
 * NOTE: Log undo(before) and redo(after) of record descriptors. A log
 *              record is constructed to recover data by undoing data during
 *              abort and during crash recovery, and by redoing data during
 *              crash recovery. The log manager does not really record
 *              descriptors, instead the log manager glues the type and the
 *              data of the record descriptors as data, and thus, it looses
 *              the knowledge that the data was coming from a record
 *              descriptor. This is done to avoid extra storage overhead. It
 *              is the responsibility of the recovery functions to build a
 *              record descriptor from the glued data (type+data of record
 *              descriptor).
 *
 *              In the case of a rollback, the undo function described by
 *              rcvindex is called with a recovery structure which contains
 *              the page pointer and offset of the data to recover along with
 *              the undo glued data (i.e., type+data of record descriptor).
 *              The undo function must construct the record descriptor from
 *              the given data. It is up to this function to determine how to
 *              undo the data.
 *
 *              Similarly, for redo, the redo function described by rcvindex
 *              is called with a recovery structure which contains the page
 *              pointer and offset of the data to recover along with the redo
 *              glued data (i.e., type+data of record descriptor). The redo
 *              function must construct the record descriptor, from the given
 *              data. It is up to this function to determine how to undo the
 *              data.
 *
 *     1)       Same notes as log_append_undoredo_data (see this function)
 *     2)       The only purpose of this function is to avoid extra data
 ...
 *
 * NOTE: Log undo(before) a record descriptor. A log record is
 *              constructed to recover data by undoing data during abort and
 *              during crash recovery.
 *              The log manager does not really store record descriptors,
 *              instead the log manager glues the type and the data of the
 *              record descriptors as data, and thus, it loses the knowledge
 *              that the data was coming from a record descriptor. This is
 *              done to avoid extra storage overhead. It is the
 *              responsibility of the recovery functions to build a record
 *              descriptor from the glued data (type+data of recdes).
 *
 *              In the case of a rollback, the undo function described by
 *              rcvindex is called with a recovery structure which contains
 *              the page pointer and offset of the data to recover along with
 ...
 *
 * NOTE: Log redo(after) of record descriptors. A log record is
 *              constructed to recover data by redoing data during recovery.
 *              The log manager does not really record descriptors, instead
 *              the log manager glues the type and the data of the record
 *              descriptors as data, and thus, it loses the knowledge that
 *              the data was coming from a record descriptor. This is done to
 *              avoid extra storage overhead. It is the responsibility of the
 *              recovery functions to build a record descriptor from the glued
 *              data (type+data of record descriptor).
 *
 *              During recovery(e.g., system crash recovery), the redo
 *              function described by rcvindex is called with a recovery
 *              structure which contains the page pointer and offset of the
 *              data to recover along with the redo glued data (i.e.,
 *              type+data of record descriptor). The redo function must
 *              construct the record descriptor, from the given data. It is up
 *              to this function, how to undo the data.
 *
 *     1)       Same notes as log_append_redo_data (see this function)
 *     2)       The only purpose of this function is to avoid extra data
 *              copying (the glue into one contiguous area) by the caller,
 ...
 *
 * NOTE: Prepare a system operation which does not depend on the
 *              destiny of the transaction, but the macro operation itself.
 *              Kind of a mini transaction for logging but no for concurrency
 *              control.
 *              A top nested operation will not conflict with any of its
 ...
 *              inconsistencies.
 *
 *     1)       A system operation can be started even during the undo of an
 *              operation.
 *     2)       Client recovery stuff should not be done with system
 *              operations.
 *     4)       Distributed transaction stuff should not be done with system
 *              operations.
 */
LOG_LSA *
			   ...
			   const LOG_TDES * tdes, LOG_DATA_ADDR * addr)
{
  ...
  /*
   * Undo records on new files are not needed unless a top system operation
   * is in effect (i.e., we may need to rollback the top system operation).
   * They are not needed since if the transaction is rolled back, the file
   * is deallocated.
   *
   * NOTE that temporary pages were already done on top. There is not an easy
   *      way to find out if we are dealing with file table pages from
   *      file manager to keep track of pages in the case of temporary
   *      pages. So, if the page does not have a temporary page lsa, it
   *      will be logged.
   */
 ...
 *   get_newtrid(in):
 *
 * NOTE: Declare the transaction as completely finish if possible or
 *              declare the transaction as a loose end transaction. A
 *              transaction is declared as finished or loose end according to
 *              the following criterios:
 *
 *              For coordinator site (either a root or not root coordinator)
 *              of a distributed transaction: Acknowledgements from all
 *              participants should have been received to declared the
 *              transaction as finished. If not all acknowledgements have been
 *              received and the system is in normal processing (i.e., not in
 *              restart recovery), this transaction is transfered to another
 *              transaction index which is declared as a distributed loose end
 *              index. The current transaction index (client) can continue its
 *              execution with a new transaction.
 *
 *              A transaction is finished by logging a commit or abort log
 *              record (depending upon the iscommitted flag).
 *
 *              If the transaction has been previously declared as a
 *              distributed loose end transaction, the transaction index is
 *              freed.
 */
TRAN_STATE
	...
	if (tdes->coord->ack_received[i] == false)
	  {
		...
		new_tran_index =
		  logtb_assign_tran_index (thread_p, NULL_TRANID,
					   TRAN_RECOVERY, NULL, NULL, NULL,
					   -1, NULL, PRM_LK_TIMEOUT_SECS,
					   TRAN_SERIALIZABLE);
		    ...
		    LOG_RESULT_TOPOP result)
{
  ...
  assert (tdes != NULL);
  /*
   * The following is done to avoid rollbacks and postpones of log records
   * of the current top system operation when other top system operations
   * are executed. For example, the current top system operation may be
   * nested in another top system operation.
   * If the top actions were aborted:
   *    - Postpone log records should not be executed by the parent since
   *      they were aborted
   *    - Undo log records should not be executed by the parent since they
   *      have already been undone by the rollback of the top actions.
   * If the top actions were committed:
   *    - Postpone log records should not be executed by the parent since
   *      they have already been executed by the committment of the top
   *      actions
   *    - Undo log records should not be executed by the parent since they
   *      cannot be aborted.. Top actions were committed.
   */
  LOG_CS_ENTER (thread_p);
  ...
  while (!LSA_ISNULL (&lsa) && dump_npages-- > 0)
      ...
      while (lsa.pageid == log_lsa.pageid)
	{
	  ...
	  LOG_READ_ADD_ALIGN (thread_p, sizeof (*log_rec), &log_lsa,
			      log_pgptr);
	  log_pgptr =
	    log_dump_record (thread_p, stdout, type, &log_lsa, log_pgptr,
			     log_dump_ptr);
	  fflush (stdout);
	  /*
	   * We can fix the lsa.pageid in the case of log_records without forward
  ...
  if (rcv->pgptr != NULL
      ...
      else
	{
	  ...
	  if (rv_err != NO_ERROR)
	    {
	      er_log_debug (ARG_FILE_LINE,
			    "log_rollback_rec: SYSTEM ERROR... Transaction %d, Log record %d|%d, rcvindex = %s, was not undone due to error (%d)\n",
  ...
  do
    {
      ...
      /*
       * Reset the end of a top operation only if the current one is not nested
       * on the previous one. For example:
       *
       * Case 1: xxx Postpone Rec xxx Start SY OP 1 xxx END SY OP 1 xxxx
       *                      xxxxxxx Start Sy OP 2 xxx END SY OP 2 xxxx
       *        NOTE: The above are not nested, so I want to reset the address
       *
       * Case 2: xxx Postpone Rec xxx Start SY OP 1 xxx Start SY OP 2 xxxx
       *                      xxxx END SY OP 2 xxxx END SY OP 1 xxxxx
       *        NOTE: The above is nested, so we do not want to reset.
       */
      ...
      while (!LSA_ISNULL (&next_start_posplsa))
	{
	  ...
	  while (!LSA_ISNULL (&forward_lsa) && !isdone)
	      ...
	      while (forward_lsa.pageid == log_lsa.pageid)
		{
		  ...
		  /*
		   * If the next page is NULL_PAGEID and the current page is an archive
		   * page, this is not the end, this situation happens when an
		   * incomplete log record is archived. Thus, its forward address is
		   * NULL.
		   * Note that we have to set lsa.pageid here since the log_lsa.pageid
		   * value can be changed (e.g., the log record is stored in an
		   * archive page and in an active page. Later, we try to modify it
		  ...
		  if (log_rec->trid == tdes->trid && !skip_head)
			...
			rcv.pgptr = pgbuf_fix_with_retry (thread_p, &rcv_vpid,
							  OLD_PAGE,
							  PGBUF_LATCH_WRITE,
							  10);
			 ...
			 * the postpone action
			 */
			log_append_run_postpone (thread_p, rcvindex, &rvaddr,
						 &rcv_vpid, rcv.length,
						 rcv.data, &ref_lsa);
