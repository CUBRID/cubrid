 ...
 * Note:
 *
 * 	Overview: TRANSACTION OBJECT LOCATOR (AT CLIENT AND SERVER)
 *
 * This file of the transaction object locator provides functions to transfer
 * a set of heap objects from the workspace/client to the heap/server or vice
 * versa. The copy area contains information of the number of objects that are
 * transferred, and then for each object, the operation to be execute (e.g.,
 * fetch, flush-insert, flush-update, flush-delete), if the object has an
 * index, the heap where the object belongs, and so on. See the
 * lc_copyarea_oneobj and lc_copyarea_manyobjs structures.
 *
 ...
 *    ----------------------------------------------------
 *
 * That is, there is an indirect pointer array to every object in the copy
 * area. Every element of the indirect pointer array indicates the oid,
 * length, and offset to the object. In addition there is an operation to be
 * executed on the object. This operation is used during the flushing phase to
 * find out if we want a deletion, insertion, of modification of the object.
 * These structures are shared and built by both the heap manager and the
 * client and server transaction object locator.
 *
 * For a full description of the heap manager see the heap manager file (hf.c)
 *
 *
 * MODULE INTERFACE
 *
 ...
 *
 *      Transaction object locator at client and server:
 *                                   To allocate a fetch/force copy area for
...
locator_free_packed (char *packed_area, int packed_size)
{
  ...
  if (locator_Keep.packed_areas.number < LC_NKEEP_LIMIT)
	...
	length = packed_size;
       ...
       * to free'd areas
       */
      MEM_REGION_SCRAMBLE (locator_Keep.packed_areas.areas[locator_Keep.packed_areas.number]->mem,
                           locator_Keep.packed_areas.areas[locator_Keep.packed_areas.number]->length);
      ++locator_Keep.packed_areas.number;
    }
  else
    {
			...
			int *desc_length)
{
  ...
  *contents_length = 0;
  ...
  mobjs = LC_MANYOBJS_PTR_IN_COPYAREA (copyarea);
  *desc_length = mobjs->num_objs * LC_AREA_ONEOBJ_PACKED_SIZE;
  DB_ALIGN (*desc_length, MAX_ALIGNMENT);
...
locator_initialize_lockset (LC_LOCKSET * lockset, int length, int max_reqobjs,
			    ...
			    int quit_on_errors)
{
  if (lockset == NULL || length < sizeof (*lockset))
    {
      return ER_FAILED;
    }
			...
			bool unpack_objects)
{
  ...
  return unpacked - lockset->packed;
}
...
locator_initialize_lockhint (LC_LOCKHINT * lockhint, int length,
			     int max_classes, int quit_on_errors)
{
  if (lockhint == NULL || length < sizeof (*lockhint))
    {
      return ER_FAILED;
    }
...
locator_unpack_lockhint (LC_LOCKHINT * lockhint, bool unpack_classes)
{
  ...
  return unpacked - lockhint->packed;
}
  ...
  if (class_oidset_p == NULL)
    {
      ...
      class_oidset_p =
	(LC_CLASS_OIDSET *) db_private_alloc (thread_p, sizeof (LC_CLASS_OIDSET));
      if (class_oidset_p == NULL)
	{
	  return NULL;
	}
