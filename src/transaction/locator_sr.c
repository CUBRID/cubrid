...
int 
xlog_supplement_statement(THREAD_ENTRY *thread_p, int statement_type, int num_class, char **classname_list, char * objname, char *stmt_text)
{
...
//  int status; 
  ...
  length = (OR_INT_SIZE
            + OR_INT_SIZE
            + (OR_BIGINT_SIZE * num_class) 
            + or_packed_string_length (stmt_text, &strlen));
  
  if(statement_type == CUBRID_STMT_CREATE_INDEX || statement_type == CUBRID_STMT_ALTER_INDEX || statement_type == CUBRID_STMT_DROP_INDEX)
  {
