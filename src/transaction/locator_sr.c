...
/*
 * locator_allocate_private_by_attr_info () - Transforms attribute information into a disk representation
 *              and allocates a private heap memory big enough to fit the representation
 *
 * return: the allocated memory pointer or NULL
 *
 *   attr_info(in/out): Attribute information
 *                      (Set as a side effect to fill the rest of values)
 *   old_recdes(in): The old representation of the object or NULL if this is a
 *                   new object (to be inserted).
 *   new_recdes(in): The resulting new representation of the object.
 *   length_hint(in): An estimated size for the memory required or -1 if an estimated size is not known.
 *   lob_create_flag(in) :
 *
 * Note: to refactor with locator_allocate_copy_area_by_attr_info 
 */
char *
locator_allocate_private_by_attr_info (THREAD_ENTRY * thread_p, HEAP_CACHE_ATTRINFO * attr_info, RECDES * old_recdes,
				       RECDES * new_recdes, const int length_hint, int lob_create_flag)
{
  char *mem_block = NULL;
  int mem_length = length_hint <= 0 ? DB_PAGESIZE : length_hint;
  SCAN_CODE scan = S_DOESNT_FIT;

  while (scan == S_DOESNT_FIT)
    {
      mem_block = (char *) db_private_alloc (thread_p, mem_length);
      if (mem_block == NULL)
	{
	  break;
	}

      new_recdes->data = mem_block;
      new_recdes->area_size = mem_length;
      ...
      if (scan != S_SUCCESS)
	{
	  ...
	  if (scan == S_DOESNT_FIT)
	    {
	      /*
	       * The object does not fit into copy area, increase the memory size
	       * to estimated size included in length of record descriptor.
	       */
	      if (mem_length < (-new_recdes->length))
		{
		  mem_length = DB_ALIGN (-new_recdes->length, MAX_ALIGNMENT);
		}
	      else
		{
		  /*
 ...
 *   thread_p(in):
 *
 * Note : this functions should be used only on slave replication server
 *
 */
int
			...
			const std::vector < DB_VALUE > &new_values, const RECDES * new_recdes_arg)
{
  ...
  RECDES new_recdes;
  LOG_LSA topop_lsa;
  int dummy_force_count;
  int error_code;
  LC_FIND_CLASSNAME status;
  /* TODO : make these arguments of function */
  ...
  if (new_recdes_arg != NULL)
    {
      assert (LC_IS_FLUSH_INSERT (rbr_operation) || LC_IS_FLUSH_UPDATE (rbr_operation));
...
retry:
  ...
  error_code = xtran_server_start_topop (thread_p, &topop_lsa);
  if (error_code != NO_ERROR)
    {
      goto exit;
    }
  ...
  switch (rbr_operation)
    ...
    case LC_FLUSH_INSERT_PRUNE_VERIFY:
      ...
      error_code =
	locator_insert_force (thread_p, &hfid, &class_oid, &instance_oid, &new_recdes, LC_FLAG_HAS_INDEX,
	...
	locator_update_force (thread_p, &hfid, &class_oid, &instance_oid, NULL, &new_recdes, LC_FLAG_HAS_INDEX,
			      ...
			      REPL_INFO_TYPE_RBR_NORMAL, pruning_type, NULL, NULL, UPDATE_INPLACE_NONE, false);
  ...
  if (error_code != NO_ERROR)
    {
      /* Should not happens. However, we have to handle it. */
      assert (er_errid () != NO_ERROR);
    ...
    {
      (void) xtran_server_end_topop (thread_p, LOG_RESULT_TOPOP_ATTACH_TO_OUTER, &topop_lsa);
    }
...
/*
 * locator_prepare_rbr_apply () - prepare RBR operation
 *
 * return: NO_ERROR if all OK, ER_ status otherwise
 *
 *   thread_p(in):
...
// *INDENT-ON*
{
  ...
  if (LC_IS_FLUSH_INSERT (rbr_operation) == false)
    {
      ...
      if (error_code != NO_ERROR)
	{
      ...
      if (rbr_operation == LC_FLUSH_DELETE)
	{
	  scan_op_type = S_DELETE;
	}
      else if (LC_IS_FLUSH_UPDATE (rbr_operation) == true)
	{
	  scan_op_type = S_UPDATE;
	}
