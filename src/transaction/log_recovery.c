					     ...
					     LOG_PAGE * log_page_p)
{
   ...
   */
  LOG_READ_ADD_ALIGN (thread_p, sizeof (LOG_RECORD_HEADER), log_lsa, log_page_p);
  LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (LOG_REC_TOPOPE_START_POSTPONE), log_lsa, log_page_p);
  ...
  while (!LSA_ISNULL (&lsa))
    {
      ...
      if (logpb_fetch_page (thread_p, log_lsa.pageid, LOGPB_MODE_RW, log_page_p) == NULL)
	{
	  if (is_media_crash == true)
	    {
	      if (stop_at != NULL)
...
log_recovery_resetlog (THREAD_ENTRY * thread_p, LOG_LSA * new_append_lsa, bool is_new_append_page, LOG_LSA * last_lsa)
{
  ...
  if (LSA_ISNULL (new_append_lsa))
	  ...
	  || (log_Gl.hdr.fpageid > new_append_lsa->pageid && new_append_lsa->offset > 0))
	{
	  ...
	  if ((logpb_fetch_page (thread_p, new_append_lsa->pageid, LOGPB_MODE_RW, newappend_pgptr)) == NULL)
	    {
...
log_startof_nxrec (THREAD_ENTRY * thread_p, LOG_LSA * lsa, bool canuse_forwaddr)
{
  ...
  switch (type)
    ...
    case LOG_COMMIT_TOPOPE_WITH_POSTPONE:
      /* Read the DATA HEADER */
      LOG_READ_ADVANCE_WHEN_DOESNT_FIT (thread_p, sizeof (LOG_REC_TOPOPE_START_POSTPONE), &log_lsa, log_pgptr);

      LOG_READ_ADD_ALIGN (thread_p, sizeof (LOG_REC_TOPOPE_START_POSTPONE), &log_lsa, log_pgptr);
      break;
