...
sm_define_view_auth_spec (void)
{
  ...
  int n = snprintf (stmt, sizeof(stmt),
	...
	"WHERE "
          ...
          "AND [a].[object_type] = 0 "
          "AND ( "
	  "{'DBA', 'DB_CATALOG_READER', [c].[owner].[name], [a].[grantee].[name], [a].[grantor].[name]} * ("
	      "SELECT "
		"SET {CURRENT_USER} + COALESCE (SUM (SET {[t].[g].[name]}), SET {}) "
	      "FROM "
		/* AU_USER_CLASS_NAME */
   ...
   "UNION ALL "
	...
	"WHERE "
          ...
          "AND [a].[object_type] = 5 "
          "AND ( "
	  "{'DBA', 'DB_CATALOG_READER', [s].[owner].[name], [a].[grantee].[name], [a].[grantor].[name]} * ("
	      "SELECT "
		"SET {CURRENT_USER} + COALESCE (SUM (SET {[t].[g].[name]}), SET {}) "
	      "FROM "
		/* AU_USER_CLASS_NAME */
...
sm_define_view_authorization_spec (void)
{
  ...
  int n = snprintf (stmt, sizeof(stmt),
	...
	"FROM "
	  ...
	  "[%s] AS [a] "
	"WHERE "
	  "{'DBA', 'DB_CATALOG_READER', [a].[owner].[name]} * ("
	    "SELECT "
	      "SET {CURRENT_USER} + COALESCE (SUM (SET {[t].[g].[name]}), SET {}) "
	    "FROM "
	      /* AU_USER_CLASS_NAME */
