...
sm_define_view_class_spec (void)
{
  ...
  // *INDENT-OFF*
  /* class_flags bitmask; see SM_CLASS_FLAG for bit definitions in class_object.h */
  sprintf (stmt,
	"SELECT "
	  ...
	  "CASE WHEN ([c].[class_flags] & 1) <> 0 THEN 'YES' ELSE 'NO' END AS [is_system_class], "
	  ...
	  "CASE "
	    ...
	    "END AS [partitioned], "
	  "CASE WHEN ([c].[class_flags] & 8) <> 0 THEN 'YES' ELSE 'NO' END AS [is_reuse_oid_class], "
