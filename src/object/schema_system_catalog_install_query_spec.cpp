...
sm_define_view_class_spec (void)
{
  ...
  // *INDENT-OFF*
  /* flags bitmask; see SM_CLASS_FLAG for bit definitions in class_object.h */
  sprintf (stmt,
	"SELECT "
	  ...
	  "CASE WHEN ([c].[flags] & 1) <> 0 THEN 'YES' ELSE 'NO' END AS [is_system_class], "
	  ...
	  "CASE "
	    ...
	    "END AS [partitioned], "
	  "CASE WHEN ([c].[flags] & 8) <> 0 THEN 'YES' ELSE 'NO' END AS [is_reuse_oid_class], "
