 ...
 * TODO: rename this file to virtual_object.c
 *
 */
...
vid_flush_all_instances (MOP class_mop, bool decache)
{
  int rc;
  /*
   * vid_flush_all_instances can be called on views also (eg, see smu.c's
   * install_new_representation call site). vid_flush_all_instances called
   * on a view seems to be flawed (even before Opal) -- it does not flush
   * dirty view instances. The reader can verify this in pre-Opal releases.
   * For now, we preserve the pre-Opal behavior of this function on views.
   */
 ...
 *   This function tries to preserve the behavior of the pre-Opal eager proxy
 *   flushing code in obt_apply_assignments. With Opal's lazy proxy flushing,
 *   obt_check_assignment's check_nested_proxy_insert calls
 *   vid_flush_and_rehash to make sure nested proxy inserts are pre-flushed
 *   and rehashed.
 */
int
...
vid_add_base_instance (MOBJ instance, MOP class_mop)
{
  ...
  /*
   * a new (temporary) instance starts life with a null key. In pre-Opal
   * (v3_5_2 and earlier) code, new OO instances were assigned permanent
   * OIDs here and now resulting in a severe performance penalty. We're
   * now deferring permanent OID assignment until flush-time if possible.
   * But, if other objects need to refer to any new OO proxy instance p,
   * we must assign p a permanent OID before flush-time.
   */
...
vid_build_non_upd_object (MOP mop, DB_VALUE * seq)
{
  ...
  for (attribute_p = class_p->attributes; attribute_p != NULL;
	...
	break;
      ...
      /*
       * Since we're building a non-updatable mop, it's important that all
       * attribute values of this mop have no unswizzled OID or VOBJ values.
       * db_seq_get already swizzles OIDs into mops. We need to convert any
       * remaining VOBJ and nested VOBJ that occur as seq/set elements.
       * db_seq_get gives us clones of the original seq elements. We convert
       * only the clones. We leave the original seq elements untouched. This
       * is important because seq is actually the keys part of the VOBJ from
       * which we're creating this non-updatable mop.
       */
