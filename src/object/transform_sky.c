 ...
 *
 *      tfcl.c: Functions related to the storage of instances and classes.
 *
 * Note:
 *      The first section of the file contains code to perform the
 *      transformation of instances.  This is relatively simple and is
 *      driven from the class definition for the instances.  The second
 *      section performs the transformation of classes which is
 *      signigicantly more complicated.
 *
 *	Since classes are "meta" objects that are represented in memory
 *	with C data structures, they can't be transformed using the
 *	data driven tf_disk_to_mem and tf_mem_to_disk.  Instead, they
 *	have hand written transformation fucntions.  Some day, we might
 *	want to add an easier mechanism for defining and transforming
 *	meta objects but this should be fine for now.
 *
 *	Although the meta objects don't have class definitions in the schema
 *	they do have entries in the catalog manager so that their contents
 *	may be queried.  These catalog manager entries are generated manually
 *	and installed as part of database formatting.
 *
 *	NOTE: It is currently important that the name attribute be the first
 *	variable attribute in both the class object and the rootclass object.
 *	There is a lot of uncomfortable code related to handling the
 *	meta objects, need to give this more thought.
 */
 ...
 * To prevent prim.c from using improperly initialized OR_BUF's which
 * may have a garbage "fixups" field, this flag is used to globally
 * enable or disable the use of the fixups by sites that call
 * tf_add_fixup.
 ...
 * Note:
 *    Originally, temporary OIDs were encountered as the object was
 *    transformed.  This works ok but with the introduction of sorted sets,
 *    its important that we assign the OIDs first, then sort the sets BEFORE
 *    the transformation takes place.  This requires an accurate traversal
 *    of the object which is done here.  We could expose a new PRIM vector
 *    function for this but that's somewhat overkill.  To keep the set
 *    iteration encapsulated, setobj_find_temporary_oids is defined to
 *    do the work.
 *
 *    Initially this will be used at the beginning of each tf_mem_to_disk
 *    call to do the bulk OID assignment once.  Eventually, this can
 *    taken up a level and lc_ can use this to gather temporary OIDs for
 *    all the objects in a mflush buffer its building.  That may not be
 *    possible though as we don't necessarily known how many objects will
 *    fit in the buffer.  We could instead just do this once for ALL the
 *    objects we're trying to flush, that would be even better.
 *
 *    For safety, tf_mem_to_disk will still maintain its own little oidset
 *    but it will only be used in cases where the lc_ module misses something
 *    we may want to send a message to the log in that event.
 *
 *    Note also that CLASS transformation still uses the internal oidset
 *    allocated inside tf_class_to_disk.  Class traversal to find temporary
 *    OIDs is much more complicated than instance traversal.  It could be
 *    done but in practice the only time it would be an issue is if the
 *    class contained default, shared, or class attributes with with sets
 *    or object values.  This can happen, but not on the same scale as
 *    instances.
 *
 *    This will also be called by the locator_assign_all_permanent_oids function as
 *    an optional part of the processing during a transaction commit.
 */
int
...
tf_mem_to_disk (MOP classmop, MOBJ classobj,
		MOBJ volatile obj, RECDES * record, int *index_flag)
{
  OR_BUF orep, *buf;
  ...
  /*
   * Before doing the transformation, make sure that all sets are sorted,
   * this may result in bulk OID assignments.  Eventually, make this a full
   * traversal to do all the OID promotion in one server call, followed
   * by a pass to sort the sets.  After that, the transformation functions
   * won't have to worry about OID promotion.
   *
   * Note that OID's in the sets will be promoted as side effect of
   * sorting the sets. If the OID's have already all been promoted,
   * the set sorting will not repeat the promotion.
   */
  ...
  switch (_setjmp (buf->env))
      ...
      if (class_->fixed_count)
	{
      ...
      /*
       * Don't advance the chn here:  that mustn't happen until the
       * entire object has been successfully transformed.  Otherwise
       * we may advance it without actually transforming the object, and
       * that will break code in lc_internal_flush_instance (in lccl.c).
       */
      chn = WS_CHN (obj) + 1;
      or_put_int (buf, chn);
 ...
 *    the locator.  For now, lccl.c is the only module allowed
 *    to call these and it handles the MOP allocation correctly.
 */
MOBJ
 ...
 *
 * Need to merge the set support in orep.c, tform.c tformcl.c and prim.c
 * into a nicer package.
 *
 ...
 *
 * These really should go in orep.c but that introduces a number of
 * dependencies that aren't very pretty now.
 *
 */
 ...
 *    defined in the catalog.  See tf.h for the definitions of these.
 */
static void
 ...
 *
 * These are for transforming the property lists on attributes, methods, and
 * classes.  Basically, these are just sequences with no domain, allowing
 * them to contain any value.  By convention, the elements are name/value
 * pairs with the name always being a string.
 *
 * There is a bit of added complexity since old versions of the database
 * have reserved space for the property list as an empty sequence of
 * substructures.  When reading these things, make sure to transform it
 * into an empty property list.
 *
 * Attributes present a bit of a problem because the old style hack for
 * storing UNIQUE btid's in the attribute property list is not
 * compatible with the generic property list idea.
 * We must recognize these (by checking for the first element of the sequence
 * being a number rather than a string) and convert it into a new
 * style property list.
 *
 */
...
methsig_to_disk (OR_BUF * buf, SM_METHOD_SIGNATURE * sig)
{
  ...
  or_put_int (buf, offset);
  offset += string_disk_size (sig->sqlx_definition);
  or_put_int (buf, offset);
  offset +=
  ...
  put_string (buf, sig->sqlx_definition);
...
methsig_size (SM_METHOD_SIGNATURE * sig)
{
  ...
  size += string_disk_size (sig->sqlx_definition);
  size +=
    substructure_set_size ((DB_LIST *) sig->value, (LSIZER) metharg_size);
  size +=
    substructure_set_size ((DB_LIST *) sig->args, (LSIZER) metharg_size);
...
disk_to_methsig (OR_BUF * buf)
{
  ...
  if (vars == NULL)
      ...
      else
	{
	  ...
	  sig->sqlx_definition = get_string (buf, vars[1].length);
...
/*
 * This was originally written for something in SQL/M that needed
 * to pack & unpack sets.  At the time, there was some problem with
 * using the or.c functions because of dependencies on the client
 * representation of sets.  This needs to be re-examined now that
 * we have uniform set handling on both sides.
 *
 * NOTE: In the earlier implementation, we were not packing set "values"
 * so the packed representation was not tagged.  This prevents us from
 * using the "writeval" function which will always tag the value.
 * This means that we have to get a handle directly to the set object
 * and pack it.  This probably means that this can only be used reliably
 * on sets that are unattached which I believe will be the case currently.
 *
 * Something similar would be usefull in the property list packer
 * above.
 */
