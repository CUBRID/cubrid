...
sm_rename_class (MOP class_mop, const char *new_name)
{
  ...
  snprintf (buf, sizeof (buf), "%s", sm_ch_name ((MOBJ) class_));
 ...
 *   name(in): user-specified name, with owner when one was specified
 */
MOP
sm_find_synonym_of_owner (MOP synonym_class_obj, const char *name)
{
  ...
  if (synonym_class_obj == NULL || name == NULL)
    {
      return NULL;
    }
  ...
  if (sm_qualifier_name (name, owner_name, DB_MAX_USER_LENGTH) != NULL)
	...
	{
	  db_make_string (&values[0], sm_remove_qualifier_name (name));
...
template_classname (SM_TEMPLATE * template_, char *buf, int buf_size)
{
  ...
  if (template_->op != NULL)
    {
      return sm_get_ch_name (template_->op);
    }
...
candidate_source_name (SM_TEMPLATE * template_, SM_CANDIDATE * candidate, char *buf, int buf_size)
{
  if (candidate->source != NULL)
    {
      return sm_get_ch_name (candidate->source);
    }
		      ...
		      const SM_PREDICATE_INFO * filter_index, const SM_FUNCTION_INFO * func_info)
{
  ...
  return classobj_check_index_exist (class_->constraints, out_shared_cons_name,
				     sm_ch_name ((MOBJ) class_), constraint_type, constraint_name, att_names, asc_desc,
