 ...
 *
 * set_object.c - Set management
 *
 * Note:
 *    This module controls the allocation and access of set objects which
 *    can be used as attribute values in database objects.
 * TODO: rename this file to set_object.c
 * TODO: include set_object_2.c and remove it
 */
...
static long col_is_all_null (COL * col);
	       ...
	       long run1start, long run1stop, long run2start, long run2stop)
{
  ...
  while (run1start <= run1stop || run2start <= run2stop)
      ...
      if (tempindex >= COL_BLOCK_SIZE)
	{
	  ...
	  /* if the current block is already full, move it to the
	   * free block space that must be available in runs by now.
	   *
	   * We know we must have free space by now, because we can
	   * have at most 2 * COL_BLOCK_SIZE - 2 DB_VALUE in need of
	   * temporary space while the block they come from still has
	   * at least 1 value not yet copied. If we reach here, we know
	   * we have copied 2 full temporary blocks, so we must have
	   * at least 1 full block empty. This remains the case until
	   * the loop terminates.
	   */
	  if (secondblockfull)
	    {
  ...
  /* here, we must have freed all the blocks from run1, because
   * it had a whole number of blocks. We may not have freed the
   * last block for run2, if the run2stop was not at the end
   * of a block. This is important to leave till last, because
   * the topmost block of a collection is generally not a full
   * block, It may either be a shorter allocation, or have
   * NULL DB_VALUES represented. Either way, we must preserve its
   * position as the last block in the collection containing non-NULL
   * values. The test below will detect that we have a short
   * block not yet freed, by noticing that the run2start, (which
   * is now 1 greater than run2stop) and run2stop are both on
   * the same page.
   */
  ...
  if (debug_level > 0)
	...
	{
	  printf ("set.c: col_merge_run internal error\n");
	}
    }
}
  ...
  if (col)
    {
      ...
      if (err)
	{
	  db_private_free_and_init (NULL, col);
	  return NULL;
	}
      /* Leave the size at zero, even for sequences.  This is to make things
       * like set_coerce work properly as they assume that they can call
       * setobj_add_element on the new thing to build up the sequence.  Unless
       * the initial size stays zero, we end up with a bunch of NULLs at the start
       * of the set.  Leaving the initial size zero after doing the preallocation
       * isn't that distatestful, in fact its more consistent and probably more
       * logical.  It is unfortunately not the way the old sequence allocator
       * used to work, we may need to make sure that db_seq_create tries to put
       * NULL in the last element so the fill pointer gets advanced.
       */
      col->size = 0;
      ...
      else
	{
	  ...
	  if (col->coltype != DB_TYPE_SEQUENCE && col->sorted)
	      ...
	      else
		{
		  ...
		  if (compare > 0)
		    {
		      /* value falls after current insert index. Look for
		       * it on the right (between index and size-1).
		       *
		       * This should be a VERY frequent case in constructing
		       * one set from another ordered set, or in constructing
		       * sets in ascending order.
		       *
		       * Consequently, we also add the optimization of seeing if
		       * insertindex and rightindex are the same, avoiding
		       * further search.
		       */
		      rightindex =
...
setvobj_compare (COL * set1, COL * set2, int do_coercion, int total_order)
{
  ...
  if ((set1->size == 3) && (set2->size == 3)
    ...
    {
      /* If the key field is an oid, then we need not compare
       * the class field. It would not hurt to do so anyway
       * in principle, but a short cut is taken building vobj structures
       * from oid's in leaving the class feild NULL, since it can
       * be determined from the oid, and since the query processor
       * did not have a convenient oid to class oid function.
       * For consistency, the vobj representations should probably
       * always get their class field filled in. This would be
       * a change from the original design, which made oid's
       * a special case.
       */
      cmp = DB_EQ;
      if (PRIM_TYPE (&set1->array[0][2]) != DB_TYPE_OID)
	{
      ...
      /* non-updatable VOBJ consists of {OID, NULL, {'vclass_name'}}.
       * otherwise, array[0][2] is {instance_OID}
       *
       * if VOBJ is non-updatable:
       *     set->array[0][0] = OID             -- ???
       *     set->array[0][1] = NULL            -- class oid
       *     set->array[0][2] = {'vclass_name'} -- literal string
       *
       * otherwise:
       *     set->array[0][0] = OID             -- ???
       *     set->array[0][1] = NULL            -- class oid
       *     set->array[0][2] = {instance_OID}  -- instance OID
       */
      if (cmp == DB_EQ)
	{
  ...
  else if (debug_level > 0)
    {
      ...
      printf ("Malformed VOBJ(s) sent to setvobj_compare in set.c\n");
    }
...
/* These were moved here from prim.c so they could access fields internal
 * to the SETOBJ structure which is now opaque outside this file.
 * The old functions pr_gc_set and pr_gc_setref just call here, the callers
 * of the pr_ functions should be changed.
 */
 ...
 *    This is not bullet proof enough to be a generally exposed part
 *    of set.c, but it does what is necessary to be used here.
 */
			  ...
			  TP_DOMAIN * domain)
{
  ...
  int added_size = 0;
  int old_vals_size = 0;
  int old_bitmap_size = 0;
  int new_bitmap_size = 0;
  char *new_IDXbuf;
  char *old_IDXbuf;
  char *new_valptr;		/* current value ptr in new midxkey->buf */
  char *old_valptr;		/* current value ptr in old midxkey->buf */
  OR_BUF buf;

  /* phase 1: find old_IDXbuf's start_ptr, valptr, and varptr */
  midxkey = DB_GET_MIDXKEY (keyval);
  old_IDXbuf = midxkey->buf;

  if (midxkey->ncolumns > 0 && midxkey->size > 0)
    {
      old_bitmap_size = OR_BOUND_BIT_BYTES (midxkey->ncolumns);
      old_vals_size = midxkey->size - old_bitmap_size;
    }
  old_valptr = old_IDXbuf + old_bitmap_size;
  new_bitmap_size = OR_BOUND_BIT_BYTES (midxkey->ncolumns + num_dbvals);
  ...
  new_IDXbuf = db_private_alloc (NULL, midxkey->size + added_size);
  if (new_IDXbuf == NULL)
    {
      goto error;
    }
  ...
  if (old_bitmap_size)
    {
      memcpy (new_IDXbuf, old_IDXbuf, old_bitmap_size);
    }
 ...
 *
 *    NOTE: This was at a different time than col_permanent_oids and does
 *    not do anything related to sorting.  It may be possible to combine
 *    these functions in some way to reduce duplication.  This is intended
 *    to be called by the tf_find_temporary_oids function which is in
 *    turn called by the lc_assign_all_temporary_oids function during
...
setobj_check_domain (COL * col, TP_DOMAIN * domain)
{
  ...
  if (col->domain != NULL && col->domain != domain &&
	...
	{
	  status =
	    tp_domain_check (domain->setdomain, INDEX (col, i),
			     TP_EXACT_MATCH);
	}
    }
  return status;
...
setobj_ismember (COL * col, DB_VALUE * proposed_value, int check_null)
{
  ...
  /* If we can be assured that the above coercion is sufficient to
   * get an identically comparable domain, we need not corece for
   * comparisons. However, this is not the case. For example, a
   * set of (int, float) could have eithe an int "4" or a float "4"
   * If this is the case, we must continue to do coercions, even though
   * the value being searched for is "in" the set domain. Otherwise
   * we get anomolies like not finding a value, because we did not
   * correctly guess its unknowable representation.
   * The exception to this rule, is if we know the set has
   * exactly one domain. Then we do not need to keep coercing,
   * because the above can not happen. This is an important
   * optimization, because this is actually the majority case.
   */
...
setobj_compare (COL * set1, COL * set2, int do_coercion)
{
  ...
  /* This routine should ONLY be used for comparing sets and multisets
   * with ordered representations. This is guranteed by sorting in
   * place if necessary.
   *
   * The alogorithm is to step thru each ordered list, noting when an
   * element from one list is NOT found in the other list. A history
   * is also kept of whether each list has yet encountered a non-member.
   *
   * At the end, if both sets had all members - they are equal.
   * If only one set had non-members, the other set is a subset.
   * If both sets had non-members the they are not equal, but neither
   * is a subset.
   * Finally if an "Unknown" was ever encountered, the result is also
   * unknown. Since unknowns are a result of NULL comparisons, and
   * NULL's are always at the end, it is possible to say set A is
   * a subset of set B even if B has NULL's. However, it will never
   * be possible for a set A with a NULL in it to be a subset of another
   * set. The result will be either "not equal" or "unknown" when
   * comparing two sets with NULL's in them.
   */
 ...
 *
 *  Note :
 *              "moves" the value to the given setobj.
 *              The DB_VALUE value is considered a temprary holder.
 *              Its memory allocations are logically "moved" to the
 *              setobj, providing an INTERNAL interface to contruct
 *              setobj's without redundant value clone/clearing.
 *              The value passed in is structure copied to the
 *              setobj, and then its memory pointers are reset, for
 *              safety.
 *
 *              This is a TRUSTED optimized operation, the caller must
 *              insure domain compatibility.
 *
 *              Note this is used in constructin ordered sets from
 *              possibly unordered sets, so the "index" is not trusted
 *              for sets. Rather col_add is used for sets, which is
 *              safer, and has only slightly increased overhead
 *              when the incoming set values are in "set" order.
 *
 *              PR9043: when attempting to insert a duplicate value
 *              in a set will return SET_DUPLICATE_VALUE (which is > 0)
 */
 ...
 *    to instance memory.  See the function mr_setmem_set in prim.c
 ...
 *
 *  Note :
 *    Sweep through the elements in the set and mark any MOPs that
 *    are encountered.  Do this by recursively calling pr_gc_value
 *    for the elements.
 *    This will be called from pr_gc_type when dealing with the attribute
 *    fields of an object.
 *    It can also be called from pr_gc_setref when dealing with a set handle
 *    that may be outside an object.
 *
 *    Unfortunately, this can also be called by ws_gc_mop when it encounters
 *    one of our special GC visibility mops for disconnected sets.
 *
 *    Previously, we marked the set owner here, which is what you want
 *    if this set is in a DB_VALUE.  Unfortunately, setobj_get_reference
 *    creates a reference if one does not already exist, so we were leaking
 *    it.  We could turn around and free the handle, but we may as well
 *    avoid allocating it if it isn't necessary.
 *
 *    Instead, we go inside the SETOBJ structure to get the owner which is
 *    why this function was moved inside the set.c file.
 *
 *    Note that if we could guarentee that any set that is reachable from
 *    a "gc_kludge" mop will be definition not be owned, then we could
 *    avoid this step but for now, lets be safe.
 *
 */
