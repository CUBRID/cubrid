 ...
 * TODO: rename this file to object_print.c
 * TODO: include object_print_2.c and remove it
 */
...
obj_print_help_class (MOP op)
{
  ...
  if (!locator_is_class (op, DB_FETCH_READ) || locator_is_root (op))
	...
	case SM_LDBVCLASS_CT:
	  ...
	  if (obj_id)
	    {
	      is_sqlx = !strcmp (obj_id->name, "");
	      buffer = pt_append_nulstring (parser, buffer,
					    (is_sqlx ? " (intrinsic oid's)"
					     : " (emulated oid's)"));
	    }
	  info->class_type =
	    obj_print_copy_string ((char *) pt_get_varchar_bytes (buffer));
...
help_trigger (DB_OBJECT * trobj)
{
  ...
  if (help != NULL)
      ...
      if (trigger->class_mop != NULL)
	{
	  ...
	  /* format the full event specification so isqlx can display it
	...
	{
	  /* just make a copy of this so isqlx can simply use it without
	     thinking */
 ...
 * aren't allowed.  See the au_info_method() function in au.c
 * for the main use of this function.
 *
