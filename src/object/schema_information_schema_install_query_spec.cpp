...
#define AUTH_CHECK_DBA_OR_SELECT_CATALOG \
  "{'DBA', 'CUB_SELECT_CATALOG'} * (" CURRENT_USER_GROUPS_SUBQUERY ") SETNEQ {}"
...
#define AUTH_CHECK_OBJECT_ANY(owner_name_expr, object_of_expr) \
  "(" \
    AUTH_CHECK_DBA_OR_SELECT_CATALOG " " \
...
#define AUTH_CHECK_PRIVILEGE(grantee_name_expr, grantor_name_expr) \
  "(" \
    "{'DBA', 'CUB_SELECT_CATALOG', " grantee_name_expr ", " grantor_name_expr "} * (" \
...
sm_define_view_routines_spec (void)
{
  ...
  int n = snprintf (stmt, sizeof (stmt),
    "SELECT "
      ...
      /* SP_LANG_PLCSQL */
      "IF ([sp].[lang] = %d "
          "AND (" AUTH_CHECK_DBA_OR_SELECT_CATALOG " OR " AUTH_CHECK_OWNER ("[sp].[owner].[name]") "), "
