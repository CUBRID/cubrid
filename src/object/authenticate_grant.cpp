...
au_revoke_class (MOP user, MOP class_mop, DB_AUTH type)
{
  ...
  AU_DISABLE (save);
  if (ws_is_same_object (user, Au_user) && class_mop->drop_object_statement == 0)
    {
...
au_revoke_procedure (MOP user, MOP obj_mop, DB_AUTH type)
{
  ...
  AU_DISABLE (save);
  if (ws_is_same_object (user, Au_user) && obj_mop->drop_object_statement == 0)
    {
      error = ER_AU_CANT_REVOKE_SELF;
      er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE, error, 0);
      goto fail_end;
...
au_compare_grantor_and_return (MOP *grantor, MOP obj_mop, DB_AUTH type, MOP login_user, MOP class_owner)
{
  ...
  *grantor = NULL;
  ...
  if (obj_mop->drop_object_statement == 1 || au_is_dba_group_member (login_user)
