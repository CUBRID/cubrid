...
/*
 * master_replication_channel_manager.hpp - manages master replication channel entries; it is a singleton
...
namespace cubreplication
{
  ...
  class master_replication_channel_entry
  {
    ...
    public:
      ...
      master_replication_channel_entry (communication_channel &&chn, MASTER_DAEMON_THREAD_ID index, T_DAEMON_TASK_PTR ptr,
					Targs... Fargs) : master_replication_channel_entry (std::forward <communication_channel> (chn), Fargs...)
      {
	...
	m_master_daemon_threads[index] = cubthread::get_manager ()->create_daemon_without_entry (cubthread::looper (
