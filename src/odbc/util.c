...
PUBLIC void add_element_to_setstring(char *setstring, char *element)
{
	...
	if ( setstring[0] == '\0' ) {		// empty set string
								...
								char *keyword)
{
	...
	while (1) {
		pt1 = strstr(pt1, buf);
		if ( pt1 == NULL || pt1 == szConnStrIn  ||  *(pt1 - 1) == ';') break;
		else {
			pt1 += strlen(buf);
		}
	}
...
PUBLIC ERR_CODE ListHeadAdd(ST_LIST* head, void* key, void* val,
...
PUBLIC ERR_CODE ListTailAdd(ST_LIST* head, void* key, void* val,
	 ERR_CODE (*assignFunc)(ST_LIST*, void*, void*))
{
  ST_LIST*	newNode;
  ST_LIST*	temp;
...
/* Create dummy header */
PUBLIC ERR_CODE ListCreate(ST_LIST** head)
{
...
#ifdef DEBUG
void
PUBLIC ListPrint(ST_LIST* head, void (*nodePrint)(ST_LIST*))
{
...
PUBLIC _BOOL_ IsAnyNode(ST_LIST* head)
{
    if ( head->next != NULL ) {
	return _TRUE_;
    } else {
...
PUBLIC ST_LIST* HeadNode(ST_LIST* dummyHead)
{
    if ( dummyHead != NULL ) {
...
PUBLIC ST_LIST* NextNode(ST_LIST* node)
{
    if ( node != NULL ) {
	return node->next;
    } else {
...
PUBLIC ERR_CODE NodeAssign(ST_LIST* node, void* key, void* value)
{
    if ( key != NULL ) {
...
PUBLIC ERR_CODE NodeCompare(void* key, void* search)
{
    if ( key == NULL || key == search ) return -1;
...
PRIVATE int is_korean( unsigned char ch )
{
...
PUBLIC const char* next_element(const char* element_list)
{
	...
	if ( element_list == NULL ) return NULL;

	pt = element_list;

	pt += strlen(element_list) +1	;	// +1 is for '\0'

	if ( *pt == '\0' ) return NULL; // end of list
...
PUBLIC const char*	element_value_by_key(const char *element_list,const char* key)
{
	...
	if ( element_list == NULL || key == NULL) return NULL;
...
PUBLIC short is_oidstr_array(char** array, int size)
{
	...
	for ( i = 0; i < size; ++i ) {
		if ( is_oidstr(array[i]) == _FALSE_ ) return _FALSE_;
	}
					   ...
					   char **oid_param_pos_pt, char **oid_param_val_pt)
{
	...
	for ( pt = sql_text; *pt != '\0'; ++pt ) {
		...
		case '\'' :
			...
			while (	pt_tmp = strchr(pt_tmp + 1, '\'') ) {
				if ( pt_tmp == NULL || *(pt_tmp -1) != '\\' ) break;

			}
			if ( pt_tmp == NULL ) break;
	...
	if ( org_param_pos_pt != NULL ) *org_param_pos_pt = UT_MAKE_STRING(org_param_pos, -1);
	if ( oid_param_pos_pt != NULL ) *oid_param_pos_pt = UT_MAKE_STRING(oid_param_pos, -1);
	if ( oid_param_val_pt != NULL ) *oid_param_val_pt = UT_MAKE_STRING(oid_param_val, -1);
								...
								int* val_len_ptr)
{
	...
	if ( in_value != NULL ) {
		...
		if ( (unsigned int)out_buf_len <= strlen(in_value) ) rc = ODBC_SUCCESS_WITH_INFO;
	}
