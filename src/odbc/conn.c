									 ...
									 long			*string_length_ptr)
{
    ...
    case SQL_ATTR_CURRENT_CATALOG :
		...
		/* CHECK : test */
		rc = str_value_assign(conn->data_source, value_ptr, buffer_length, string_length_ptr);
		...
		rc = str_value_assign(conn->attr_current_catalog, value_ptr, buffer_length, string_length_ptr);
		if ( rc == ODBC_SUCCESS_WITH_INFO ) {
			odbc_set_diag(conn->diag, "01004", 0, NULL);
		}
	...
	case SQL_ATTR_TRACEFILE :
		rc = str_value_assign(conn->attr_tracefile, value_ptr, buffer_length, string_length_ptr);
		if ( rc == ODBC_SUCCESS_WITH_INFO ) {
			odbc_set_diag(conn->diag, "01004", 0, NULL);
		}
		break;
    ...
    case SQL_ATTR_TRANSLATE_LIB :
		...
		rc = str_value_assign(conn->attr_translate_lib, value_ptr, buffer_length, string_length_ptr);
		if ( rc == ODBC_SUCCESS_WITH_INFO ) {
			odbc_set_diag(conn->diag, "01004", 0, NULL);
		}
		break;
							 ...
							 long		*string_length_ptr)
{
	...
	case SQL_AGGREGATE_FUNCTIONS :
		if ( info_value_ptr != NULL )
			*(unsigned long*)info_value_ptr = SQL_AF_ALL | SQL_AF_AVG | SQL_AF_COUNT |
	...
	case SQL_ALTER_TABLE :		/* SQL_AT_DROP_COLUMN  - ODBC 2.0 */
		if ( info_value_ptr != NULL )
			*(unsigned long*)info_value_ptr = SQL_AT_ADD_COLUMN | SQL_AT_ADD_COLUMN_DEFAULT |
	...
	case SQL_CONVERT_BINARY :
		if ( info_value_ptr != NULL )
			*(unsigned long*)info_value_ptr = SQL_CVT_BINARY | SQL_CVT_LONGVARBINARY |
				SQL_CVT_VARBINARY | SQL_CVT_CHAR | SQL_CVT_VARCHAR | SQL_CVT_LONGVARCHAR;
	...
	case SQL_CONVERT_DATE :
		if ( info_value_ptr != NULL )
			*(unsigned long*)info_value_ptr = SQL_CVT_CHAR | SQL_CVT_LONGVARCHAR |
	...
	case SQL_CONVERT_TIME :
		if ( info_value_ptr != NULL )
			*(unsigned long*)info_value_ptr = SQL_CVT_CHAR | SQL_CVT_LONGVARCHAR | SQL_CVT_VARCHAR |
			SQL_CVT_TIME;
	...
	case SQL_CONVERT_TIMESTAMP :
		if ( info_value_ptr != NULL )
			*(unsigned long*)info_value_ptr = SQL_CVT_CHAR | SQL_CVT_LONGVARCHAR | SQL_CVT_VARCHAR |
			SQL_CVT_TIME | SQL_CVT_DATE | SQL_CVT_TIMESTAMP;
	...
	case SQL_DATETIME_LITERALS :
		*(unsigned long*)info_value_ptr = SQL_DL_SQL92_DATE | SQL_DL_SQL92_TIME |
	...
	case SQL_SCROLL_OPTIONS :
		...
		if ( info_value_ptr != NULL )
			*(unsigned long*)info_value_ptr = SQL_SO_FORWARD_ONLY | SQL_SO_KEYSET_DRIVEN |
	...
	case SQL_SQL92_NUMERIC_VALUE_FUNCTIONS :
		if ( info_value_ptr != NULL )
			*(unsigned long*)info_value_ptr = SQL_SNVF_BIT_LENGTH |
			SQL_SNVF_CHAR_LENGTH | SQL_SNVF_OCTET_LENGTH | 
			SQL_SNVF_POSITION;
						 ...
						 int* fetch_size)
{
	...
	if ( fetch_size != NULL ) {
		...
		if ( rcn == 0 ) *fetch_size = 0;
		else *fetch_size = atoi(buf);
	}
