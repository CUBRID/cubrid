...
#define		RESULTSET_STMT_TYPE(stmt_type)	\
		( stmt_type == SQLX_CMD_SELECT || METHOD_STMT_TYPE(stmt_type) || GET_STAT_STMT_TYPE(stmt_type))
#define		GET_STAT_STMT_TYPE(stmt_type)	\
		( stmt_type == SQLX_CMD_GET_STATS )

typedef enum { NULL_RESULT = 0, QUERY, TABLES, STATISTICS, COLUMNS, SPECIALCOLUMNS, TYPE_INFO } RESULT_TYPE;
