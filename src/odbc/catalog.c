...
static ODBC_TYPE_INFO_VALUE type_info[] = {
    {"CHAR", SQL_CHAR, MAX_CUBRID_CHAR_LEN, "'", "'", "length", SQL_NULLABLE,
        ...
        SQL_CHAR, -1, -1, -1},
    {"VARCHAR", SQL_VARCHAR, MAX_CUBRID_CHAR_LEN, "'", "'", "length", SQL_NULLABLE,
        SQL_FALSE, SQL_SEARCHABLE, -1, SQL_FALSE, SQL_FALSE, "VARCHAR", -1, -1,
        SQL_VARCHAR, -1, -1, -1},
    {"BIT", SQL_BINARY, MAX_CUBRID_CHAR_LEN/8, "X'", "'", "length", SQL_NULLABLE,
        SQL_FALSE, SQL_SEARCHABLE, -1, SQL_FALSE, SQL_FALSE, "BIT", -1, -1,
        SQL_BINARY, -1, -1, -1},
    {"BIT VARYING", SQL_VARBINARY, MAX_CUBRID_CHAR_LEN/8, "X'", "'", "length", SQL_NULLABLE,
    ...
    {"NUMERIC", SQL_NUMERIC, 38, NULL, NULL, "precision,scale", SQL_NULLABLE,
        SQL_FALSE, SQL_PRED_BASIC, SQL_FALSE, SQL_FALSE, SQL_FALSE, "NUMERIC", 0, 38,
        SQL_NUMERIC, -1, 10, -1},
    {"DECIMAL", SQL_DECIMAL, 38, NULL, NULL, "precision,scale", SQL_NULLABLE,
        SQL_FALSE, SQL_PRED_BASIC, SQL_FALSE, SQL_FALSE, SQL_FALSE, "DECIMAL", 0, 38,
        SQL_DECIMAL, -1, 10, -1},
    {"INTEGER", SQL_INTEGER, 10, NULL, NULL, NULL, SQL_NULLABLE,
        SQL_FALSE, SQL_PRED_BASIC, SQL_FALSE, SQL_FALSE, SQL_FALSE, "INTEGER", -1, -1,
        SQL_INTEGER, -1, 10, -1},
    {"SMALLINT", SQL_SMALLINT, 5, NULL, NULL, NULL, SQL_NULLABLE,
        SQL_FALSE, SQL_PRED_BASIC, SQL_FALSE, SQL_FALSE, SQL_FALSE, "SMALLINT", -1, -1,
        SQL_SMALLINT, -1, 10, -1},
    ...
    {"FLOAT", SQL_FLOAT, 14, NULL, NULL, "precision", SQL_NULLABLE,
        SQL_FALSE, SQL_PRED_BASIC, SQL_FALSE, SQL_FALSE, SQL_FALSE, "FLOAT", -1, -1,
        SQL_FLOAT, -1, 10, -1},
    ...
    {"TIMESTAMP", SQL_TYPE_TIMESTAMP, 19, "TIMESTAMP '", "'", NULL, SQL_NULLABLE,
        SQL_FALSE, SQL_PRED_BASIC, SQL_FALSE, SQL_FALSE, SQL_FALSE, "TIMESTAMP", -1, -1,
	...
	*/
#ifdef DELPHI
    {"STRING", SQL_LONGVARCHAR, MAX_CUBRID_CHAR_LEN, "'", "'", "length", SQL_NULLABLE,
							...
							char	*table_type)
{
	...
	for ( i = 1; i <= NC_CATALOG_TABLES; ++i ) {
		// set ird field
		odbc_set_ird(stmt, i, table_cinfo[i-1].type, "", (char*)table_cinfo[i-1].name,
						table_cinfo[i-1].precision, (short)0, SQL_NULLABLE_UNKNOWN, SQL_ATTR_READONLY);
	}
	...
	cci_rc = cci_schema_info(stmt->conn->connhd, CCI_SCH_CLASS, 
				table_name, NULL, search_pattern_flag, &cci_err_buf);
							...
							char	*column_name)
{
	...
	for ( i = 1; i <= NC_CATALOG_COLUMNS; ++i ) {
		// set ird field
		odbc_set_ird(stmt, i, column_cinfo[i-1].type, "", (char*)column_cinfo[i-1].name,
						column_cinfo[i-1].precision, (short)0, SQL_NULLABLE_UNKNOWN, SQL_ATTR_READONLY);
	}
	...
	cci_rc = cci_schema_info(stmt->conn->connhd, CCI_SCH_ATTRIBUTE,
					table_name, column_name, search_pattern_flag, &cci_err_buf);
	ERROR_GOTO(cci_rc, cci_error);
...
cci_error :
	odbc_set_diag_by_cci(stmt->diag, cci_rc, NULL);
error :
	if ( handle > 0 ) cci_close_req_handle(handle);
								...
								unsigned short	reversed)
{
	...
	for ( i = 1; i <= NC_CATALOG_STATISTICS; ++i ) {
		// set ird field
		odbc_set_ird(stmt, i, statistics_cinfo[i-1].type, "", (char*)statistics_cinfo[i-1].name,
						statistics_cinfo[i-1].precision, (short)0, SQL_NULLABLE_UNKNOWN, SQL_ATTR_READONLY);
	}
							...
							short		nullable)
{
	...
	for ( i = 1; i <= NC_CATALOG_SPECIALCOLUMNS; ++i ) {
		// set ird field
		odbc_set_ird(stmt, i, special_column_cinfo[i-1].type, "", (char*)special_column_cinfo[i-1].name,
						special_column_cinfo[i-1].precision, (short)0, SQL_NULLABLE_UNKNOWN, SQL_ATTR_READONLY);
	...
	if ( cci_handle > 0 ) cci_close_req_handle(cci_handle);
								  ...
								  short			 data_type)
{
	...
	for ( i = 1; i <= NC_CATALOG_TYPE_INFO; ++i ) {
		// set ird field
		odbc_set_ird(stmt, i, type_cinfo[i-1].type, "", (char*)type_cinfo[i-1].name,
    ...
    if ( data_type == SQL_ALL_TYPES ) {
        for(i = 0; type_info[i].type_name != NULL; ++i) {
			...
			if ( type_info_node == NULL ) continue;
									...
									VALUE_CONTAINER	*c_value)
{
	...
	if ( stmt->catalog_result.current == NULL ) return ODBC_NO_MORE_DATA;
...
PRIVATE void free_table_node(ST_LIST *node)
{
	if ( node != NULL ) {
		...
		if ( node->key != NULL ) UT_FREE(node->key);
	}
}
								...
								char			*column_name_pattern)
{
	...
	if ( handle > 0 ) cci_close_req_handle(handle);
	if ( class_handle > 0 ) cci_close_req_handle(class_handle);
								   ...
								   ODBC_SPC_INFO	*spc_info_ptr)
{
	...
	cci_rc = cci_schema_info(stmt->conn->connhd, CCI_SCH_ATTRIBUTE, 
		table_name, spc_info_ptr->column_name, 0, &cci_err_buf);
	ERROR_GOTO(cci_rc, cci_error);
	...
	// get non-null
	cci_rc = cci_get_data(cci_handle, 6, CCI_A_TYPE_INT,
									&cci_value, &cci_ind);
	if ( cci_rc < 0 ) {
		odbc_set_diag_by_cci(stmt->diag, cci_rc, NULL);
		odbc_set_diag_by_icode(stmt->diag, ODBC_CAS_GET_DATA, NULL);
	...
	// get domain
	cci_rc = cci_get_data(cci_handle, 2, CCI_A_TYPE_INT,
									&cci_value, &cci_ind);
	if ( cci_rc < 0 ) {
		odbc_set_diag_by_cci(stmt->diag, cci_rc, NULL);
		odbc_set_diag_by_icode(stmt->diag, ODBC_CAS_GET_DATA, NULL);
										  ...
										  ODBC_SPC_INFO	*spc_info_ptr)
{
	...
	if ( spc_node == NULL ) return ODBC_MEMORY_ALLOC_ERROR;
