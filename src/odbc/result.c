							 ...
							 long *strlen_indicator)
{
	...
	if ( odbc_type == SQL_DATETIME ) {
		...
		rc = odbc_set_desc_field(ard, column_num, SQL_DESC_DATETIME_INTERVAL_CODE, 
	...
	if ( size > 0 ) {	/* target_type is the fixed type */
								 ...
								 short	*nullable_ptr)
{
	...
	if ( column_name != NULL ) {
		rc = odbc_get_desc_field(ird, column_number, SQL_DESC_NAME, column_name,
	...
	odbc_get_desc_field(ird, column_number, SQL_DESC_SCALE,
						&scale, 0, NULL);
								  ...
								  void*		num_value_ptr)
{
	...
	case SQL_DESC_TABLE_NAME :
		rc = odbc_get_desc_field(ird, column_number,
			field_identifier, str_value_ptr, buffer_length, &int_length);
		ERROR_GOTO(rc, error);
						  ...
						  short		flag_cursor_move)
{
	...
	if ( stmt->result_type == 0 ) return ODBC_NO_DATA;

	else if ( stmt->result_type == QUERY ) {
         rc = move_advanced_cursor(stmt, &stmt->current_tpl_pos, fetch_orientation, 
						fetch_offset);
    } else {
	...
	if ( rc < 0 ) {
		if ( rc == ODBC_NO_MORE_DATA ) return ODBC_NO_DATA;
		else goto error;
	}
	...
	if ( fetched_rows_ptr != NULL ) *fetched_rows_ptr = fetched_rows;
	...
	if ( col_number != stmt->column_data.column_no ) {
		...
		if ( stmt->result_type == QUERY ) {
			...
			if ( target_type == SQL_C_DEFAULT ) {
				odbc_get_desc_field(stmt->ird, col_number, SQL_DESC_CONCISE_TYPE, &sql_type,
							0, NULL);
			...
			if ( cci_ind == -1 ) { // NULL value
				...
				*str_ind_ptr = SQL_NULL_DATA;
				...
				if ( target_type == SQL_C_CHAR || 
					...
					rc = str_value_assign(cci_value.str, bound_ptr, buffer_length, str_ind_ptr);
					...
					rc = bin_value_assign(cci_value.bit.buf, cci_value.bit.size,
				...
				} else {		// for static type
					odbc_get_desc_field(stmt->ard,(short) col_number, SQL_DESC_PRECISION, 
								(void*)&precision, 0 , NULL);
					odbc_get_desc_field(stmt->ard,(short) col_number, SQL_DESC_SCALE,
								(void*)&scale, 0 , NULL);
					length = cci_value_to_odbc(bound_ptr, target_type,
							precision, scale, buffer_length, 
							&cci_value, a_type);
					if ( str_ind_ptr != NULL ) 
						*str_ind_ptr = length;
				}
			}
		} else {	// catalog result set
			...
			if ( c_value.length == 0 ) { // NULL value
				if ( str_ind_ptr == NULL ) {
					odbc_set_diag(stmt->diag, "22002", 0, NULL);
					goto error;
				}
				...
				if ( target_type == SQL_C_CHAR ) {
					...
					if ( rc == ODBC_SUCCESS_WITH_INFO ) {
						...
						stmt->column_data.remain_length = 
							strlen(target_value.value.str) - offset;		// 1 is for '\0'
					...
					rc = bin_value_assign(target_value.value.bin, target_value.length, 
									bound_ptr, buffer_length, str_ind_ptr);
					if ( rc == ODBC_SUCCESS_WITH_INFO ) {
						if ( buffer_length > 0 ) {
							offset = buffer_length;
						...
						stmt->column_data.remain_length = target_value.length - offset;
		...
		if ( target_type == SQL_C_CHAR ) {
			...
			rc = bin_value_assign(stmt->column_data.current_pt, stmt->column_data.remain_length,
						bound_ptr, buffer_length, str_ind_ptr);
			if ( rc == ODBC_SUCCESS_WITH_INFO ) {
				if ( buffer_length >  0 ) {
					offset = buffer_length;
...
//	if ( *bound_ptr == NULL || *strlen_ind_ptr == NULL ) return ;
	if ( *bound_ptr == NULL ) return ;

	odbc_get_desc_field(stmt->ard,(short) col_index, SQL_DESC_CONCISE_TYPE, 
			(void*)type, 0 , NULL);

	// set과 object type은 string으로 match되어 있다.
	if ( *type == SQL_C_CHAR ||	*type == SQL_C_BINARY || *type == SQL_C_UNI_OBJECT ||
			*type == SQL_C_UNI_SET || *type == SQL_C_DEFAULT ) {
		odbc_get_desc_field(stmt->ard, (short)col_index, SQL_DESC_OCTET_LENGTH, 
							...
							long			fetch_offset) 
{
	...
	case SQL_FETCH_NEXT :
		if ( *current_tpl_pos == 0 ) {	// before start
	...
	case SQL_FETCH_LAST :
		if ( row_set_size > result_set_size ) {
			...
			cci_rc = cci_cursor(stmt->stmthd, result_set_size - row_set_size +1, CCI_CURSOR_FIRST, &cci_err_buf);
							...
							ODBC_DIAG		*diag) 
{
	...
	if ( *current_tpl_pos == 0 ) {
			cci_rc = cci_cursor(req_handle, 1, CCI_CURSOR_FIRST, 
														&cci_err_buf);
	} else {
			cci_rc = cci_cursor(req_handle, 1, CCI_CURSOR_CURRENT, 
														&cci_err_buf);
	}
