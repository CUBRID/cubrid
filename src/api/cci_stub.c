...
typedef struct glo_offset_s GLO_OFFSET;
...
static int opool_glo_create (API_OBJECT_RESULTSET_POOL * poo,
			     CI_CONNECTION conn, const char *file_path,
			     const char *init_file_path, CI_OID * glo);
static int opool_glo_get_path_name (API_OBJECT_RESULTSET_POOL * poo,
				    CI_OID * glo, char *buf, size_t bufsz,
				    bool full_path);
static int opool_glo_do_compaction (API_OBJECT_RESULTSET_POOL * poo,
				    CI_OID * glo);
static int opool_glo_insert (API_OBJECT_RESULTSET_POOL * poo,
			     CI_OID * glo, const char *buf, size_t bufsz);
static int opool_glo_delete (API_OBJECT_RESULTSET_POOL * poo,
			     CI_OID * glo, size_t sz, size_t * ndeleted);
static int opool_glo_read (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
			   char *buf, size_t bufsz, size_t * nread);
static int opool_glo_write (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
			    const char *buf, size_t sz);
static int opool_glo_truncate (API_OBJECT_RESULTSET_POOL * poo,
			       CI_OID * glo, size_t * ndeleted);
static int opool_glo_seek (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
			   long offset, int whence);
static int opool_glo_tell (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
			   long *offset);
static int opool_glo_like_search (API_OBJECT_RESULTSET_POOL * poo,
				  CI_OID * glo, const char *str,
				  size_t strsz, bool * found);
static int opool_glo_reg_search (API_OBJECT_RESULTSET_POOL * poo,
				 CI_OID * glo, const char *str,
				 size_t strsz, bool * found);
static int opool_glo_bin_search (API_OBJECT_RESULTSET_POOL * poo,
				 CI_OID * glo, const char *str,
				 size_t strsz, bool * found);
static int opool_glo_copy (API_OBJECT_RESULTSET_POOL * poo, CI_OID * to,
			   CI_OID * from);
...
static int oidstr2xoid (const char *oidstr, CI_CONNECTION conn,
			CI_OID * xoid);
static int opool_lazy_get_glo_offset (CCI_OBJECT_POOL * pool, CI_OID * glo,
				      GLO_OFFSET ** obj);
 ...
 */
static int
opool_glo_create (API_OBJECT_RESULTSET_POOL * poo, CI_CONNECTION conn,
		  ...
		  CI_OID * glo)
{
  ...
  xoid2oidstr (glo, oid_buf);
  res = cci_oid_get_class_name (pool->pconn->conn_handle, oid_buf, class_name,
				512, &pool->pconn->err_buf);
  if (res != 0)
    {
      return err_from_cci (res);
    }

  res = cci_glo_new (pool->pconn->conn_handle, class_name, (char *) file_path,
		     oid_buf, &pool->pconn->err_buf);
  if (res != 0)
    {
      return err_from_cci (res);
    }
  ...
  if (init_file_path != NULL)
    {
      res = cci_glo_save (pool->pconn->conn_handle, oid_buf,
			  (char *) init_file_path, &pool->pconn->err_buf);
      if (res != 0)
	{
	  (void) cci_oid (pool->pconn->conn_handle, CCI_OID_DROP, oid_buf,
			  &err_buf);
	}
    }
 ...
 */
static int
opool_glo_get_path_name (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
...
/*
 * opool_glo_do_compaction -
 *    return: NO_ERROR if successful, error code otherwise
 *    poo():
 *    glo():
 */
static int
opool_glo_do_compaction (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo)
{
  CCI_OBJECT_POOL *pool = (CCI_OBJECT_POOL *) poo;
  int res;
  char oid_buf[32];
  ...
  xoid2oidstr (glo, oid_buf);
  res = cci_glo_compress_data (pool->pconn->conn_handle, oid_buf,
			       &pool->pconn->err_buf);
 ...
 */
static int
opool_glo_insert (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		  const char *buf, size_t bufsz)
{
  ...
  /* return values is number of bytes written */
  res = cci_glo_insert_data (pool->pconn->conn_handle, oid_buf,
			     (int) off->offset, bufsz, (char *) buf,
			     &pool->pconn->err_buf);
  if (res < 0)
    {
      return err_from_cci (res);
 ...
 */
static int
opool_glo_delete (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		  size_t sz, size_t * ndeleted)
{
  ...
  /* return values is number of bytes deleted */
  res = cci_glo_delete_data (pool->pconn->conn_handle, oid_buf,
			     (int) off->offset, (int) sz,
			     &pool->pconn->err_buf);
  if (res < 0)
    {
      return err_from_cci (res);
 ...
 */
static int
opool_glo_read (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		char *buf, size_t bufsz, size_t * nread)
{
  CCI_OBJECT_POOL *pool = (CCI_OBJECT_POOL *) poo;
  GLO_OFFSET *off;
  ...
  res = cci_glo_read_data (pool->pconn->conn_handle, oid_buf,
			   (int) off->offset, (int) bufsz, buf,
			   &pool->pconn->err_buf);
  if (res < 0)
    {
      return err_from_cci (res);
 ...
 */
static int
opool_glo_write (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		 const char *buf, size_t sz)
{
  CCI_OBJECT_POOL *pool = (CCI_OBJECT_POOL *) poo;
  GLO_OFFSET *off;
  ...
  /* return values is number of bytes written */
  res = cci_glo_write_data (pool->pconn->conn_handle, oid_buf,
			    (int) off->offset, (int) sz, (char *) buf,
			    &pool->pconn->err_buf);
  if (res < 0)
    {
      return err_from_cci (res);
 ...
 */
static int
opool_glo_truncate (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		    size_t * ndeleted)
{
  CCI_OBJECT_POOL *pool = (CCI_OBJECT_POOL *) poo;
  GLO_OFFSET *off;
  ...
  /* return values is number of bytes written */
  res = cci_glo_truncate_data (pool->pconn->conn_handle, oid_buf,
			       off->offset, &pool->pconn->err_buf);
  if (res < 0)
    {
      return err_from_cci (res);
    }
 ...
 */
static int
opool_glo_seek (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		long offset, int whence)
{
  CCI_OBJECT_POOL *pool = (CCI_OBJECT_POOL *) poo;
  GLO_OFFSET *off;
  ...
  if (whence == 0)
    ...
    {
      res = cci_glo_data_size (pool->pconn->conn_handle, oid_buf,
			       &pool->pconn->err_buf);
      if (res < 0)
	{
	  return err_from_cci (res);
  ...
  /* CCI do not expose data_seek method call */
  res = cci_glo_read_data (pool->pconn->conn_handle, oid_buf, base + offset,
			   1, &buf, &pool->pconn->err_buf);
  if (res < 0)
    {
      return err_from_cci (res);
    }
...
opool_glo_tell (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo, long *offset)
{
  ...
  *offset = (long) off->offset;

  return NO_ERROR;
}

/*
 * opool_glo_like_search -
 *    return: NO_ERROR if successful, error code otherwise
 *    poo():
 *    glo():
 *    str():
 ...
 */
static int
opool_glo_like_search (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		       const char *str, size_t strsz, bool * found)
{
   ...
   */
  res = cci_glo_like_search (pool->pconn->conn_handle, oid_buf, off->offset,
			     (char *) str, &offset, &curr_pos,
			     &pool->pconn->err_buf);
  if (res != 0)
    {
      return err_from_cci (res);
 ...
 */
static int
opool_glo_reg_search (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		      const char *str, size_t strsz, bool * found)
{
  CCI_OBJECT_POOL *pool = (CCI_OBJECT_POOL *) poo;
  GLO_OFFSET *off;
   ...
   * curr_pos : right after match end
   */
  res = cci_glo_reg_search (pool->pconn->conn_handle, oid_buf, off->offset,
			    (char *) str, &offset, &curr_pos,
			    &pool->pconn->err_buf);
  if (res != 0)
    {
 ...
 */
static int
opool_glo_bin_search (API_OBJECT_RESULTSET_POOL * poo, CI_OID * glo,
		      const char *str, size_t strsz, bool * found)
{
  CCI_OBJECT_POOL *pool = (CCI_OBJECT_POOL *) poo;
  GLO_OFFSET *off;
   ...
   * curr_pos : right after match end
   */
  res = cci_glo_binary_search (pool->pconn->conn_handle, oid_buf, off->offset,
			       (int) strsz, (char *) str, &offset, &curr_pos,
			       &pool->pconn->err_buf);
  if (res != 0)
    {
      return err_from_cci (res);
 ...
 */
static int
opool_lazy_get_glo_offset (CCI_OBJECT_POOL * pool, CI_OID * glo,
			   GLO_OFFSET ** off)
{
  GLO_OFFSET *o;
  int res;
  ...
  res = hash_lookup (pool->ght, glo, (void **) &o);
  if (res != NO_ERROR)
    {
      return res;
    }
  ...
  if (o == NULL)
    {
       ...
       */
      o = API_MALLOC (sizeof (*o));

      o->xoid = *glo;
      o->offset = 0;
      res = hash_insert (pool->ght, o);
      if (res == NO_ERROR)
	{
	  *off = 0;
	}
...
opool_create (CONNECTION_IMPL * pconn, CCI_OBJECT_POOL ** rpool)
{
  ...
  pool->ifs.glo_create = opool_glo_create;
  pool->ifs.glo_get_path_name = opool_glo_get_path_name;
  pool->ifs.glo_do_compaction = opool_glo_do_compaction;
  pool->ifs.glo_insert = opool_glo_insert;
  pool->ifs.glo_delete = opool_glo_delete;
  pool->ifs.glo_read = opool_glo_read;
  pool->ifs.glo_write = opool_glo_write;
  pool->ifs.glo_truncate = opool_glo_truncate;
  pool->ifs.glo_seek = opool_glo_seek;
  pool->ifs.glo_tell = opool_glo_tell;
  pool->ifs.glo_like_search = opool_glo_like_search;
  pool->ifs.glo_reg_search = opool_glo_reg_search;
  pool->ifs.glo_bin_search = opool_glo_bin_search;
  pool->ifs.glo_copy = opool_glo_copy;
