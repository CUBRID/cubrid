...
class page_server
{
  ...
  private: // types
     ...
     *  Because ATS fire and forget the log records, the connecting PS must get the pages from another PS.
     *
     *  Terms:
     *
     ...
     *    Note that the catchup_lsa is different depending on when a PS connects.
     ...
     *         └─────┘   └──────────┘       └──────────┘
     *
     *  1. A page server boots up and become a follower.
     *  2. ATS selects a followee page server and send the node information and the catchup_lsa to the follower.
     ...
     *     - start_lsa: the append_lsa of the follower, where it expects a new log record is sent.
     *     - catchup_lsa: the lsa of the first log record that the ATS will be send.
     *     -> The follower fill the log hole [start_lsa, catchup_lsa) to accept new log records from its ATS.
     ...
     *    if the log pages have been flushed on disk.
     *  - While a follower is catching up with a followee, it can also receive new log records from ATS. They are kept
     *    and going to be appended after the catch-up is completed.
     *
     * TODO
     ...
     *    In this case, the follower who requested should be re-initiate the catch-up with another PS.
     *  - With other communication error with its followee, the follower should re-initiate the catch-up with another PS.
     ...
     *  - The log records from ATS while the catch-up are kept on memory in transient. It can consume all memory and overflow.
     *    This should be addressed. I think we could expand the catch-up range and discard some kept log records.
