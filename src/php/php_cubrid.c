...
PHP_MINIT_FUNCTION(cubrid)
{
	...
	REGISTER_LONG_CONSTANT("CUBRID_SCH_CLASS_ATTRIBUTE", 
			CUBRID_SCH_CLASS_ATTRIBUTE, CONST_CS | CONST_PERSISTENT);
	...
	REGISTER_LONG_CONSTANT("CUBRID_SCH_CLASS_PRIVILEGE", 
			CUBRID_SCH_CLASS_PRIVILEGE, CONST_CS | CONST_PERSISTENT);
	REGISTER_LONG_CONSTANT("CUBRID_SCH_ATTR_PRIVILEGE", 
			CUBRID_SCH_ATTR_PRIVILEGE, CONST_CS | CONST_PERSISTENT);
...
PHP_FUNCTION(cubrid_connect)
{
	...
	res = cci_connect(GET_ZVAL(host)->value.str.val, 
			GET_ZVAL(port)->value.lval, GET_ZVAL(dbname)->value.str.val, 
...
PHP_FUNCTION(cubrid_prepare)
{
	...
	switch(ARG_COUNT(ht)) {
		case 2:
			...
			break;
		case 3:
			if (zend_get_parameters_ex(3, &con_handle, &query, &option) == 
					FAILURE ) {
				RETURN_FALSE;
			}
			if (GET_ZVAL(con_handle)->type == IS_NULL || 
	...
	res = cci_prepare(connect->handle, GET_ZVAL(query)->value.str.val, 
			(char)((real_option & CUBRID_INCLUDE_OID)?CCI_PREPARE_INCLUDE_OID:0), 
			&error);
	if (res < 0) {
		/* cci_prepare에서 에러 발생 */
	...
	{
		request->l_bind =(short *)emalloc( sizeof(short)* (request->bind_num) );
		if( !request->l_bind )
		{
			handle_error( CUBRID_ER_NO_MORE_MEMORY, NULL );
			efree( request );
...
PHP_FUNCTION(cubrid_bind)
{
	...
	{
		case 3:
			if( zend_get_parameters_ex( 3, &req_handle, &nidx, &bind_value) == FAILURE )
			{
				RETURN_FALSE;
			}
			...
			break;
		case 4:
			if( zend_get_parameters_ex( 4, &req_handle, &nidx, &bind_value, &ptype) == FAILURE )
			{
				RETURN_FALSE;
			}
			if( GET_ZVAL( req_handle )->type == IS_NULL ||
			...
			else if( strcasecmp( type, "NUMERIC" ) == 0 || strcasecmp( type, "NUMBER" ) == 0)
	...
	request = (T_CUBRID_REQUEST *)zend_fetch_resource(req_handle TSRMLS_CC, -1, 
			"CUBRID-Request", NULL, 1, le_request);	
	if(!request)
	{
		RETURN_FALSE;
	}
	...
	if( u_type == CCI_U_TYPE_BIT )
	{
		...
		res = cci_bind_param( request->handle, idx, CCI_A_TYPE_BIT, (void *)bit_value, CCI_U_TYPE_BIT,(char)NULL );
		efree( bit_value );
		bit_value = NULL;
	}
	else
	{
		...
		res = cci_bind_param( request->handle, idx, CCI_A_TYPE_STR, value , u_type,(char)NULL );
	}
...
PHP_FUNCTION(cubrid_execute)
{
	...
	switch(ARG_COUNT(ht)) {
		...
		case 2:
			...
			break;
		case 3:
			if (zend_get_parameters_ex(3, &handle, &query, &option) == 
					FAILURE ) {
				RETURN_FALSE;
			}
			if (GET_ZVAL(handle)->type == IS_NULL || 
	...
	if( !connect  )
	{
		...
		request = (T_CUBRID_REQUEST *)zend_fetch_resource(handle TSRMLS_CC, -1, 
				NULL, NULL, 1, le_request);	
		if (!request) {
			/* request handle도 아니다. 완전히 잘못된 변수를 전달 받았다. */
			zend_error(E_WARNING, "%s(): no %s resource supplied", get_active_function_name(TSRMLS_C),"CUBRID-Connect or CUBRID-Request" );
			RETURN_FALSE;
		}
		...
		if( request->bind_num > 0 && l_prepare )
		{
			if ( request->l_bind )
			{
				for( i=0; i<request->bind_num; i++)
				{
					if( !request->l_bind[i] )
					{
						zend_error( E_WARNING, "Execute without value binding : %d\n",i+1 );
						RETURN_FALSE;
					}
				}
			}
	...
	if (!l_prepare)
	{
		...
		/* convert_sql이 (*query)->value.str.val의 내용을 바꾸었을 경우에
		   길이도 수정해야함 */
		GET_ZVAL(query)->value.str.len = strlen(GET_ZVAL(query)->value.str.val);
	...
	res = cci_execute(request_handle, 
			(char)((real_option & CUBRID_ASYNC)?CCI_EXEC_ASYNC:0), 0, &error);
	if (res < 0) {
		/* cci_execute에서 에러 발생 */
...
PHP_FUNCTION(cubrid_current_oid)
{
	...
	ZEND_FETCH_RESOURCE(request, T_CUBRID_REQUEST *, req_handle, -1,
			"CUBRID-Request", le_request);
	if (request->sql_type != SQLX_CMD_SELECT) {
		/* SQL이 select가 아님 */
		handle_error(CUBRID_ER_INVALID_SQL_TYPE, NULL);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(cubrid_column_types)
{
	...
	for (i=0;i<request->col_count;i++) {
		switch(CCI_GET_COLLECTION_DOMAIN(CCI_GET_RESULT_INFO_TYPE(
				request->col_info, i+1))){
		...
		if (CCI_IS_SET_TYPE(CCI_GET_RESULT_INFO_TYPE(
				request->col_info, i+1))) {
			sprintf(full_type_name, "set(%s)", buf);
		} else if (CCI_IS_MULTISET_TYPE(CCI_GET_RESULT_INFO_TYPE(
				request->col_info, i+1))) {
			sprintf(full_type_name, "multiset(%s)", buf);
		} else if (CCI_IS_SEQUENCE_TYPE(CCI_GET_RESULT_INFO_TYPE(
				request->col_info, i+1))) {
...
PHP_FUNCTION(cubrid_column_names)
{
	...
	for (i=0;i<request->col_count;i++) {
		add_index_stringl(return_value, i, 
				...
				strlen(CCI_GET_RESULT_INFO_NAME(request->col_info, i+1)), 1);
	}
	return;
}
...
PHP_FUNCTION(cubrid_get)
{
	...
	if (ARG_COUNT(ht) == 3) { /* attr_name 이 정해진 경우 */
		switch(GET_ZVAL(attr_name)->type){
			case IS_STRING:
				...
				break;
			case IS_ARRAY:
				real_attr_count = zend_hash_num_elements(
						GET_ZVAL(attr_name)->value.ht);
				real_attr_name = (char **)emalloc(sizeof(char *) * 
						(real_attr_count+1));
				if (!real_attr_name) {
					/* 메모리 할당 에러 */
				...
				for (i=0;i<real_attr_count;i++){
					...
					real_attr_name[i] = estrdup(GET_ZVAL(elem_buf)->value.str.val);
				}
				real_attr_name[i] = NULL;
				break;
			default:
...
PHP_FUNCTION(cubrid_put)
{
	...
	switch(ARG_COUNT(ht)){
		case 3:
			if (zend_get_parameters_ex(3, &con_handle, &oid, &attr) 
					== FAILURE) {
				RETURN_FALSE;
			}
			if (GET_ZVAL(con_handle)->type == IS_NULL || 
					GET_ZVAL(oid)->type == IS_NULL ||
					GET_ZVAL(attr)->type == IS_NULL) {
				handle_error(CUBRID_ER_INVALID_PARAM, NULL);
				RETURN_FALSE;
			}
			ZEND_FETCH_RESOURCE(connect, T_CUBRID_CONNECT *, con_handle, -1, 
			...
			for (;;zend_hash_move_forward(GET_ZVAL(attr)->value.ht)){
				res = zend_hash_get_current_key(GET_ZVAL(attr)->value.ht, &key, &index, 1);
				...
				real_attr_value = erealloc(real_attr_value, 
						sizeof(char *) * (real_attr_count+1));
				real_attr_type = erealloc(real_attr_type,
						sizeof(int) * (real_attr_count+1));
				real_attr_name[i] = (char *)emalloc(sizeof(char) * 
						(strlen(key) + 1));
				...
				switch(GET_ZVAL(data)->type) {
					...
					case IS_STRING:
						real_attr_value[i] = (char *)emalloc(sizeof(char) * 
								(GET_ZVAL(data)->value.str.len + 1));
						strcpy(real_attr_value[i], 
								GET_ZVAL(data)->value.str.val);
						real_attr_type[i] = CCI_A_TYPE_STR;

						break;
					case IS_ARRAY:
						res = cubrid_make_set(GET_ZVAL(data)->value.ht, 
								&temp_set);
						if (res < 0) {
							handle_error(res, NULL);
							goto return_error;
		...
		case 4:
			...
			real_attr_name[0] = (char *)emalloc(sizeof(char) * 
					(GET_ZVAL(attr_name)->value.str.len + 1));
			...
			switch(GET_ZVAL(attr_value)->type) {
				...
				case IS_STRING:
					real_attr_value[0] = (char *)emalloc(sizeof(char) * 
							GET_ZVAL(attr_value)->value.str.len + 1);
					strcpy(real_attr_value[0], 
							GET_ZVAL(attr_value)->value.str.val);
					real_attr_type[0] = CCI_A_TYPE_STR;
					break;
				case IS_ARRAY:
					res = cubrid_make_set(GET_ZVAL(attr_value)->value.ht, 
							&temp_set);
					if (res < 0) {
						handle_error(res, NULL);
						goto return_error;
	...
	res = cci_oid_put2(connect->handle, GET_ZVAL(oid)->value.str.val, 
			real_attr_name, real_attr_value, real_attr_type, &error);
	if (res < 0) {
		handle_error(res, &error);
		goto return_error;
	}
...
PHP_FUNCTION(cubrid_get_class_name)
{
	...
	res = cci_oid_get_class_name(connect->handle, GET_ZVAL(oid)->value.str.val, 
			out_buf, 1024, &error);
	if (res < 0) {
		handle_error (res, &error);
		RETURN_FALSE;
...
PHP_FUNCTION(cubrid_schema)
{
	...
	switch(ARG_COUNT(ht)) {
		case 2:
			if (zend_get_parameters_ex(2, &con_handle, &schema_type) == 
					FAILURE){
				RETURN_FALSE;
			}
			if (GET_ZVAL(con_handle)->type == IS_NULL || 
	...
	/* 에러났을 때 리턴값이 -1이어야 하기 때문에 ZEND_FETCH_RESOURCE를 
	   쓰지 않았음 */
	connect = (T_CUBRID_CONNECT *)zend_fetch_resource(con_handle TSRMLS_CC, -1,
			"CUBRID-Connect", NULL, 1, le_connect);
	if (!connect) {
		/* 리소스를 찾지 못 했음(Invalid handle)*/
		RETURN_LONG(-1);
	}
	...
	res = cci_schema_info(connect->handle, GET_ZVAL(schema_type)->value.lval, 
			real_class_name, real_attr_name, (char)real_flag, &error);
...
PHP_FUNCTION(cubrid_col_size)
{
	...
	init_error();
	if (ARG_COUNT(ht) == 3) {
		if (zend_get_parameters_ex(3, &con_handle, &oid, &attr_name) == FAILURE) {
			RETURN_FALSE;
		}
		if (GET_ZVAL(con_handle)->type == IS_NULL || 
				GET_ZVAL(oid)->type == IS_NULL ||
...
PHP_FUNCTION(cubrid_col_get)
{
	...
	res = cci_col_get (connect->handle, GET_ZVAL(oid)->value.str.val, 
			GET_ZVAL(attr_name)->value.str.val, &col_size, &col_type, &error);
	if (res < 0) {
		handle_error (res, &error);
		RETURN_FALSE;
	}
...
PHP_FUNCTION(cubrid_seq_insert)
{
	...
	res = cci_col_seq_insert (connect->handle, GET_ZVAL(oid)->value.str.val,
			GET_ZVAL(attr_name)->value.str.val, GET_ZVAL(index)->value.lval,
			value, &error);
	if (res < 0) {
		handle_error (res, &error);
		RETURN_FALSE;
		...
		if (CCI_IS_SET_TYPE(CCI_GET_RESULT_INFO_TYPE(col_info, i+1)) ||
			...
			if (type & CUBRID_ASSOC) {
				res = cubrid_add_assoc_array(arg, 
						CCI_GET_RESULT_INFO_NAME(col_info, i+1), res_buf);
				if (res < 0) {
					cci_set_free(res_buf);
					error_occured = 1;
					error_code = res;
