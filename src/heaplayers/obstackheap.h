...
namespace HL
{
  ...
  public:
      ...
      if (chkSize > 0)
	{
	  ...
	  currentChunk = makeChunk (NULL, chkSize);
    ...
    {
      chkSize = chkSize_;
      if (chkSize > 0 && currentChunk == NULL)
	{
	  // Get one chunk and set the current position marker.
	  currentChunk = makeChunk (NULL, chkSize);
	  currentBase = nextPos = (char *) (currentChunk + 1);
...
#if 0				/* DO NOT DELETE ME FOR FUTURE USE */
    ...
    inline void clear (void)
    {
      // Free every chunk except the last one
      assert (isValid ());
      ChunkHeader *ch, *pch;
      for (ch = currentChunk, pch = (ch) ? ch->getPrevChunk () : NULL;
	   pch != NULL; ch = pch, pch = (ch) ? ch->getPrevChunk () : NULL)
	{
	  // cout << "Freeing chunk " << ch << endl;
	  SuperHeap::free (ch);
	}
      ...
      if ((int) ((char *) currentChunk->getLimit () - (char *) nextPos) < sz)
	{
      ...
      assert (((int) ((char *) currentChunk->getLimit () - (char *) nextPos)
	       >= sz));
      ...
      if (currentChunk == NULL)
	{
      ...
      // get another one.
      if ((ptrdiff_t) ((char *) currentChunk->getLimit () - (char *) nextPos)
	  < (ptrdiff_t) sz)
	{
      ...
      assert (((ptrdiff_t)
	       ((char *) currentChunk->getLimit () - (char *) nextPos) >=
	       (ptrdiff_t) sz));
      assert ((char *) (sz + nextPos) <= currentChunk->getLimit ());
    ...
    {
      assert (isValid ());
      nextPos = (char *) align ((size_t) nextPos);
      currentBase = nextPos;
      assert (isValid ());
    }
  ...
  private:
    ...
    {
    public:
      inline ChunkHeader (ChunkHeader * prev,
			  size_t sz):_pastEnd ((char *) (this + 1) + sz),
	_prevChunk (prev)
      {
      }

      // Return the end of the current chunk.
      inline char *getLimit (void)
      {
	return _pastEnd;
      }

      // Return the previous chunk.
      inline ChunkHeader *getPrevChunk (void)
      {
	return _prevChunk;
      }
	...
	=
	align ((sz >
		chkSize - sizeof (ChunkHeader)) ? sz : chkSize -
	       sizeof (ChunkHeader));
      // Make a new chunk.
      ...
      if (ptr == NULL)
	{
      ...
      ChunkHeader *newChunk = new (ptr) ChunkHeader (ch, allocSize);
      return newChunk;
    }
