...
namespace HL
{
  ...
  template < int ChunkSize, class SuperHeap > class ObstackHeap:public SuperHeap
  {
  ...
  public:
    ...
    inline void free (void *ptr)
    {
      ...
      while (currentChunk != NULL &&
	     (((char *) currentChunk > (char *) ptr) || ((char *) currentChunk->getLimit () < (char *) ptr)))
	{
  ...
  private:
    ...
    {
    public:
      inline ChunkHeader (ChunkHeader * prev, size_t sz):_pastEnd ((char *) (this + 1) + sz), _prevChunk (prev)
      {
      }

      // Return the end of the current chunk.
      inline char *getLimit ()
      {
	return _pastEnd;
      }

      // Return the previous chunk.
      inline ChunkHeader *getPrevChunk ()
      {
	return _prevChunk;
      }
