...
#define EXEC_INFO_CLOSE_CONNECTION	0x01
#define EXEC_INFO_APPEND_RESULT		0x02
#endif
...
static void add_res_data_bytes (T_NET_BUF * net_buf, char *str, int size,
				char type);
static void add_res_data_string (T_NET_BUF * net_buf, char *str, int size,
				 char type);
...
static void glo_err_msg_set (T_NET_BUF * net_buf, int err_code, char *m_name);
...
#ifdef CAS_AUTOCOMMIT
static int is_autocommit_executable (T_SRV_HANDLE * srv_handle);
static int autocommit_exec_info_set (T_BROKER_VERSION client_version,
				     char auto_commit,
				     T_SRV_HANDLE * srv_handle,
				     T_NET_BUF * net_buf,
				     int exec_info_msg_offset);
#endif
#if 0
...
extern int db_query_seek_tuple (DB_QUERY_RESULT *, int, int);
  ...
  if ((stmt_id = db_compile_statement (session)) < 0)
    {
      ...
      if (stmt_id == ER_PT_SEMANTIC && stmt_type != SQLX_MAX_CMD_TYPE)
	{
	    ...
	    int *clt_cache_reusable)
{
  ...
  if ((!(flag & CCI_EXEC_ASYNC)) &&
      (max_row > 0) &&
      (db_get_statement_type (session, stmt_id) == SQLX_CMD_SELECT) &&
...
#ifndef LIBCAS_FOR_JSP
  if (!(srv_handle->q_result->stmt_type == SQLX_CMD_SELECT ||
		  ...
		  T_REQ_INFO * req_info)
{
  ...
  for (i = 1; i <= argc; i++)
      ...
      do
	{
	  ...
	  if ((stmt_type == SQLX_CMD_INSERT) && (res_count == 1) &&
	      (result != NULL) &&
		  ...
		  T_NET_BUF * net_buf, T_REQ_INFO * req_info)
{
  ...
  err_code =
    make_bind_value (num_bind, argc - 2, argv + 2, &value_list, net_buf,
		     DB_TYPE_NULL);
  if (err_code < 0)
    {
      if (err_code == CAS_ER_DBMS)
...
ux_cursor (int srv_h_id, int offset, char origin, T_NET_BUF * net_buf)
{
  ...
  if (origin == CCI_CURSOR_FIRST)
      ...
      else
	{
	  ...
	  count = 0;
	  err_code =
	    db_query_get_info (result, &done, &count, &error, &err_str);
		  ...
		  void **argv, T_NET_BUF * net_buf)
{
  ...
  if (result == NULL ||
      q_result->col_updatable == FALSE || q_result->col_update_info == NULL)
    {
      return 0;
    }
...
ux_glo_method_call (T_NET_BUF * net_buf, char check_ret, DB_OBJECT * glo_obj,
		    char *method_name, DB_VALUE * ret_val, DB_VALUE ** args)
{
  int err_code;
  int ret;
	      ...
	      char fetch_flag, int result_set_idx, T_NET_BUF * net_buf)
{
  ...
  result = (DB_QUERY_RESULT *) q_result->result;
  if ((result == NULL) ||
      (q_result->stmt_type != SQLX_CMD_SELECT &&
       q_result->stmt_type != SQLX_CMD_GET_STATS &&
       q_result->stmt_type != SQLX_CMD_CALL &&
       q_result->stmt_type != SQLX_CMD_EVALUATE))
    {
      return CAS_ER_NO_MORE_DATA;
    }
		 ...
		 char fetch_flag, int result_set_idx, T_NET_BUF * net_buf)
{
  ...
  result = (DB_QUERY_RESULT *) q_result->result;
  if (result == NULL || q_result->stmt_type != SQLX_CMD_SELECT)
    {
      return CAS_ER_NO_MORE_DATA;
    }
...
static void
add_res_data_string (T_NET_BUF * net_buf, char *str, int size, char type)
{
  if (type)
    {
			      ...
			      T_BROKER_VERSION client_version)
{
  ...
  else if (stmt_type == SQLX_CMD_CALL ||
	   stmt_type == SQLX_CMD_GET_STATS || stmt_type == SQLX_CMD_EVALUATE)
    {
		  ...
		  T_BROKER_VERSION client_version, char exec_flag)
{
  ...
  int retval = 0;
#ifdef CAS_AUTOCOMMIT
  int autocommit_info_offset;
#endif
  CACHE_TIME srv_cache_time;

#ifdef CAS_AUTOCOMMIT
  net_buf_cp_int (net_buf, 0, &autocommit_info_offset);
#endif
  ...
  for (i = 0; i < srv_handle->num_q_result; i++)
    {
      ...
      if (stmt_type == SQLX_CMD_INSERT && tuple_count == 1 &&
	  srv_handle->q_result[i].result != NULL)
	{
	  error =
	    db_query_get_tuple_value ((DB_QUERY_RESULT *) srv_handle->
  ...
  do
    {
      char auto_commit = (exec_flag & CCI_EXEC_AUTOCOMMIT) ? TRUE : FALSE;
      retval = autocommit_exec_info_set (client_version, auto_commit,
					 srv_handle, net_buf,
					 autocommit_info_offset);
    }
  while (0);
#endif
...
static void
glo_err_msg_set (T_NET_BUF * net_buf, int err_code, char *method_nm)
{
  char err_msg[128];
#ifdef CAS_DEBUG
...
#ifdef CAS_AUTOCOMMIT
static int
is_autocommit_executable (T_SRV_HANDLE * srv_handle)
{
  char stmt_type = srv_handle->q_result->stmt_type;
  ...
  if (stmt_type == SQLX_CMD_SELECT ||
    ...
    {
      if (srv_handle->q_result->async_flag == FALSE &&
	  srv_handle->q_result->col_updatable == FALSE &&
	  srv_handle->q_result->tuple_count < 200)
	{
	  return TRUE;
	}
      else
...
static int
autocommit_exec_info_set (T_BROKER_VERSION client_version, char auto_commit,
			  T_SRV_HANDLE * srv_handle, T_NET_BUF * net_buf,
			  int exec_info_msg_offset)
{
  int auto_commit_exec_info = 0;

  if (auto_commit == TRUE &&
#ifndef LIBCAS_FOR_JSP
      shm_appl->as_info[shm_as_index].cur_keep_con == FALSE &&
#endif
      is_autocommit_executable (srv_handle) == TRUE)
    {
      int fetch_res;

      if (srv_handle->prepare_flag & CCI_PREPARE_CALL)
	fetch_res = fetch_call (srv_handle, net_buf);
      else
	fetch_res =
	  fetch_result (srv_handle, 1, srv_handle->q_result->tuple_count,
			0, 1, net_buf);

      if (fetch_res >= 0)
	{
	  auto_commit_exec_info |= EXEC_INFO_CLOSE_CONNECTION;
	  auto_commit_exec_info |= EXEC_INFO_APPEND_RESULT;
	}
      else if (fetch_res == CAS_ER_NO_MORE_DATA)
	{
	  auto_commit_exec_info |= EXEC_INFO_CLOSE_CONNECTION;
	}
      else
  ...
  net_buf_overwrite_int (net_buf, exec_info_msg_offset,
			 auto_commit_exec_info);

  if (auto_commit_exec_info & EXEC_INFO_CLOSE_CONNECTION)
    {
      ux_end_tran (CCI_TRAN_COMMIT, TRUE);
      return EXEC_AUTOCOMMIT_TRUE;
    }

  return EXEC_AUTOCOMMIT_FALSE;
}
#endif /* CAS_AUTOCOMMIT */
...
ux_make_out_rs (int srv_h_id, T_NET_BUF * net_buf, T_REQ_INFO * req_info)
{
  ...
  q_result = (T_QUERY_RESULT *) srv_handle->cur_result;
  if (q_result->stmt_type != SQLX_CMD_SELECT)
    {
      err_code = CAS_ER_SRV_HANDLE;
      goto error1;
    }
...
get_client_result_cache_lifetime (DB_SESSION * session, int stmt_id)
{
  ...
  jdbc_cache_is_hint =
    db_get_jdbccachehint (session, stmt_id, &jdbc_cache_life_time);
