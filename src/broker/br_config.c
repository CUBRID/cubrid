 ...
 *
 * br_config.c - broker configuration utilities
 *
 */
...
#define DEFAULT_ADMIN_LOG_FILE		"log/cubrid_broker.log"
...
#endif
{
  ...
  for (i = 0; i < ini->nsec; i++)
	...
	ini_getint (ini, sec_name, "SESSION_TIMEOUT",
      ...
      strcpy (br_info[num_brs].access_log_file, br_info[num_brs].log_dir);
      strcpy (br_info[num_brs].error_log_file, br_info[num_brs].log_dir);
