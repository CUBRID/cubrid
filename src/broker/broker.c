...
static class client_support __gv_client_support_broker;
#define __gv_cvar_broker (__gv_client_support_broker)
...
connect_to_master_for_server_monitor (const char *db_name, const char *db_host)
{
  ...
  return (__gv_cvar_broker.css_connect_to_master_timeout (db_host, port_id, 5000, &rid));
}
...
get_server_state_from_master (CSS_CONN_ENTRY * conn, const char *db_name)
{
  ...
  error = __gv_cvar_broker.css_send_request (conn, GET_SERVER_STATE, &request_id, db_name, (int) strlen (db_name) + 1);
  if (error != NO_ERRORS)
    {
      return SERVER_STATE_DEAD;
    }
  ...
  /* timeout : 5000 milliseconds */
  error = __gv_cvar_broker.css_receive_data (conn, request_id, (char **) &buffer, &buffer_size, 5000);
  if (error == NO_ERRORS)
    {
...
server_monitor_thr_f (void *arg)
{
  ...
  while (process_flag)
      ...
      for (i = 0; i < check_list_cnt; i++)
	{
	  ...
	  if (conn != NULL)
	    {
	      __gv_cvar_broker.css_free_conn (conn);
	      conn = NULL;
	    }
	}
