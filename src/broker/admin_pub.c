...
admin_start_cmd (T_BROKER_INFO * br_info, int br_num, int master_shm_id)
{
  ...
  chdir ("..");
  mkdir (CUBRID_VAR_DIR, 0777);
  mkdir (CUBRID_TMP_DIR, 0777);
  mkdir (CUBRID_LOG_DIR, 0777);
  mkdir (CUBRID_ASPID_DIR, 0777);
  mkdir (CUBRID_LOG_DIR "/" CUBRID_SQL_LOG_DIR, 0777);
  mkdir (CUBRID_ERR_DIR, 0777);
#ifndef WIN32
  mkdir (CUBRID_LOG_DIR "/" CUBRID_SQL_LOG_DIR "/" SQL_LOG2_DIR, 0777);
  mkdir (CUBRID_SOCK_DIR, 0777);
#endif
  ...
  for (i = 0; i < br_num; i++)
    {
      char buf[PATH_MAX];
      mkdir (br_info[i].log_dir, 0777);
      sprintf (buf, "%s/%s", br_info[i].log_dir, CUBRID_SQL_LOG_DIR);
      mkdir (buf, 0777);
    }
  chdir ("bin");
