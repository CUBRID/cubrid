...
static void prepare_column_info_set (T_NET_BUF * net_buf, char ut,
				     short scale, int prec, char *col_name,
				     char *attr_name, char *class_name,
				     char nullable,
				     T_BROKER_VERSION client_version);
...
prepare_result_set:
  ...
  if (prepare_column_list_info_set
      (session, flag, q_result, net_buf, client_version) < 0)
    {
      goto prepare_error2;
    }
...
int
ux_end_tran (char tran_type, char reset_con_status)
{
  int err_code;
	    ...
	    int *clt_cache_reusable)
{
...
#ifndef LIBCAS_FOR_JSP
      SQL_LOG2_EXEC_APPEND (shm_appl->as_info[shm_as_index].cur_sql_log2,
			    stmt_id, n,
  ...
  if (n < 0)
    {
      if (srv_handle->is_pooled && n == ER_QPROC_INVALID_XASLNODE)
	{
		...
		T_REQ_INFO * req_info)
{
  ...
  if (prepare_column_list_info_set ((DB_SESSION *) srv_handle->session,
				    srv_handle->prepare_flag,
				    cur_result, net_buf, client_version) < 0)
    {
      goto next_result_error2;
    }
	  ...
	  T_REQ_INFO * req_info)
{
  ...
  err_code = (*(fetch_func[fetch_func_index])) (srv_handle,
						cursor_pos,
						fetch_count,
						fetch_flag,
						result_set_index,
...
int
ux_glo_load (int sock_fd, DB_OBJECT * obj, T_NET_BUF * net_buf)
{
  DB_VALUE arg_val, ret_val;
  int err_code;
...
#else
  sprintf (tmp_file, "%s%s_%d.glo", tmp_file, broker_name, shm_as_index + 1);
#endif
...
int
ux_get_class_num_objs (char *class_name, char flag, T_NET_BUF * net_buf)
{
  DB_OBJECT *class_obj;
  int err_code;
  ...
  err_code = db_get_class_num_objs_and_pages (class_obj, (int) flag,
					      &num_objs, &num_pages);
...
prepare_column_info_set (T_NET_BUF * net_buf, char ut, short scale, int prec,
			 char *col_name, char *attr_name, char *class_name,
		 ...
		 T_NET_BUF * net_buf, char desired_type)
{
  ...
  if (type == CCI_U_TYPE_TIMESTAMP)
    {
      if (desired_type == DB_TYPE_DATE)
		   ...
		   char **attr_name)
{
  ...
  for (i = 0; i < attr_num; i++)
      ...
      else
	{
      ...
      net_buf_column_info_set (net_buf,
			       (char) cas_type,
			       scale, precision, (char *) attr_name[i]);
    }
			      ...
			      T_BROKER_VERSION client_version)
{
  ...
  if (stmt_type == CUBRID_STMT_SELECT)
      ...
      for (col = column_info; col != NULL; col = db_query_format_next (col))
	{
	  ...
	  null_type_column =
	    (char *) REALLOC (null_type_column, num_cols + 1);
...
get_domain_str (DB_DOMAIN * domain)
{
  ...
  switch (dtype)
	...
	p = (char *) db_get_class_name (dclass);
      break;
    case DB_TYPE_SET:
      collection_str = "set";
    case DB_TYPE_MULTISET:
  ...
  if (domain_str == NULL)
	...
	{
	  strcat (domain_str, "(");
	  strcat (domain_str, precision_str);
	  if (scale_str[0] != '\0')
	    {
	      strcat (domain_str, ",");
	      strcat (domain_str, scale_str);
	    }
	  strcat (domain_str, ")");
	}
    }
		...
		T_BROKER_VERSION client_version)
{
  ...
  sprintf (sql_stmt,
	   "SELECT class_name, CAST(%s AS short) FROM db_class ", case_stmt);
  if (pattern_flag & CCI_CLASS_NAME_PATTERN_MATCH)
      ...
      if (v_class_flag)
	{
	  if (class_name)
	    ...
	    {
	      sprintf (sql_stmt + strlen (sql_stmt), "WHERE %s",
		       where_vclass);
	    }
	}
      else
	{
	  if (class_name)
	    {
	      sprintf (sql_stmt + strlen (sql_stmt),
		       "WHERE class_name LIKE '%s' ESCAPE '\\' ", class_name);
	    }
	}
    }
  else
	       ...
	       T_SRV_HANDLE * srv_handle)
{
  ...
  strcpy (sql_stmt, "SELECT class_name, attr_name FROM db_attribute WHERE ");

  if (class_attr_flag)
    strcat (sql_stmt, " attr_type = 'CLASS' ");
  else
    strcat (sql_stmt, " attr_type in {'INSTANCE', 'SHARED'} ");
  ...
  if (pattern_flag & CCI_CLASS_NAME_PATTERN_MATCH)
	...
	{
	  sprintf (sql_stmt + strlen (sql_stmt),
		   " AND class_name LIKE '%s' ESCAPE '\\' ", class_name);
	}
    }
  else
    {
  ...
  sprintf (sql_stmt + strlen (sql_stmt), " ORDER BY class_name, def_order");
			...
			int pattern_flag, T_SRV_HANDLE * srv_handle)
{
  ...
  strcpy (sql_stmt, "SELECT class_name, super_class_name \
  ...
  if (pattern_flag & CCI_CLASS_NAME_PATTERN_MATCH)
  ...
  else
    {
      if (class_name == NULL)
	class_name = "";
      sprintf (sql_stmt + strlen (sql_stmt), "WHERE class_name = '%s'",
	       class_name);
    }
...
ux_use_sp_out (int srv_h_id)
{
  ...
  if (srv_handle->session != NULL && q_result->stmt_id >= 0)
    column_info = db_get_query_type_list ((DB_SESSION *) srv_handle->session,
					  q_result->stmt_id);
  else
    column_info = NULL;

  if (q_result->result && column_info)
    {
      new_handle_id =
...
ux_auto_commit (T_NET_BUF * net_buf, T_REQ_INFO * req_info)
{
  ...
  if (req_info->need_auto_commit == TRAN_AUTOCOMMIT)
  ...
  else if (req_info->need_auto_commit == TRAN_AUTOROLLBACK)
    {
      ...
      cas_log_write (0, TRUE, "auto_rollback %d", err_code);
    }
