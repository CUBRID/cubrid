...
static char *tran_type_str[] = { "COMMIT", "ROLLBACK" };
static char *schema_type_str[] = {
  "CLASS",
  "VCLASS",
  "QUERY_SPEC",
  "ATTRIBUTE",
...
CAS_FUNC_PROTOTYPE (fn_auto_commit)
{
  int err_code;
...
CAS_FUNC_PROTOTYPE (fn_execute)
{
  ...
  if (ret_code == EXEC_AUTOCOMMIT_TRUE)
    return -1;
#endif
  return 0;
}
...
CAS_FUNC_PROTOTYPE (fn_close_req_handle)
{
  ...
  srv_handle = hm_find_srv_handle (srv_h_id);
  if (srv_handle &&
      (srv_handle->q_result->stmt_type == SQLX_CMD_SELECT ||
       srv_handle->q_result->stmt_type == SQLX_CMD_CALL ||
       srv_handle->q_result->stmt_type == SQLX_CMD_GET_STATS ||
       srv_handle->q_result->stmt_type == SQLX_CMD_EVALUATE)
      && srv_handle->auto_commit_mode == TRUE)
    need_auto_commit = TRUE;
#else
  srv_handle = NULL;
#endif
...
CAS_FUNC_PROTOTYPE (fn_glo_cmd)
{
  ...
  if (start_pos_index >= 0)
      ...
      if ((err_code = db_make_int (&arg1, start_pos)) < 0)
	{
      ...
      err_code =
	ux_glo_method_call (CAS_FN_ARG_NET_BUF, 1, glo_obj, "data_seek",
			    &ret_val, args);
      if (err_code < 0)
	{
	  goto glo_cmd_end;
  ...
  if (glo_cmd == GLO_CMD_READ_DATA || glo_cmd == GLO_CMD_WRITE_DATA ||
			    ...
			    length)) < 0)
	{
      ...
      err_code =
	ux_glo_method_call (CAS_FN_ARG_NET_BUF, 1, glo_obj,
			    glo_cmd_info[glo_cmd].method_name, &ret_val,
			    args);
      ...
      if (err_code < 0 && glo_cmd == GLO_CMD_READ_DATA)
	{
	  ...
	  if (ux_glo_method_call
	      (NULL, 0, glo_obj, "data_size", &ret_val, args) < 0)
	    goto glo_cmd_end;
	...
	net_buf_cp_str (CAS_FN_ARG_NET_BUF, data_buf, length);
	   ...
	   glo_cmd == GLO_CMD_REG_SEARCH || glo_cmd == GLO_CMD_BINARY_SEARCH)
    {
      char *search_str;
      int length;
      int offset, cur_pos;
      ...
      else
	{
      ...
      err_code =
	ux_glo_method_call (CAS_FN_ARG_NET_BUF, 0, glo_obj,
			    glo_cmd_info[glo_cmd].method_name, &ret_val,
			    args);
      if (err_code < 0)
	goto glo_cmd_end;
      ...
      db_value_clear (&ret_val);
      err_code =
	ux_glo_method_call (CAS_FN_ARG_NET_BUF, 1, glo_obj, "data_pos",
			    &ret_val, args);
      if (err_code < 0)
	goto glo_cmd_end;
