...
const char *broker_keywords[] = {
  ...
  "QUERY_REPLACE_RULE",
  "QUERY_REPLACE_MAX_RULES",
  "QUERY_REPLACE_MAX_QUERY_LEN",
  "QUERY_REPLACE_ALLOW_NON_SELECT",
...
#define INI_GETSTR_CHK(str, ini, sec, key, def, lineno) \
  ...
  for (i = 0; i < ini->nsec; i++)
      ...
      if (make_abs_path (br_info[num_brs].access_log_dir, NULL, ini_string, CONF_LOG_FILE_LEN) < 0)
	{
      ...
      INI_GETSTR_CHK (ini_string, ini, sec_name, "QUERY_REPLACE_RULE", DEFAULT_EMPTY_STRING, &lineno);
      if (make_abs_path (br_info[num_brs].query_replace_rule, NULL, ini_string, BROKER_PATH_MAX) < 0)
	{
      ...
      /* query replace fixed-reservation sizing; an out-of-range value is a config error
       * (PARAM_BAD_RANGE), not silently clamped -- the segment is sized from these. */
      br_info[num_brs].query_replace_max_rules =
	ini_getint (ini, sec_name, "QUERY_REPLACE_MAX_RULES", DEFAULT_QR_MAX_RULES, &lineno);
      if (br_info[num_brs].query_replace_max_rules < MIN_QR_MAX_RULES
	  || br_info[num_brs].query_replace_max_rules > MAX_QR_MAX_RULES)
	{
	  errcode = PARAM_BAD_RANGE;
	  goto conf_error;
	}
      br_info[num_brs].query_replace_max_query_len =
	ini_getint (ini, sec_name, "QUERY_REPLACE_MAX_QUERY_LEN", DEFAULT_QR_MAX_QUERY_LEN, &lineno);
      if (br_info[num_brs].query_replace_max_query_len < MIN_QR_MAX_QUERY_LEN
	  || br_info[num_brs].query_replace_max_query_len > MAX_QR_MAX_QUERY_LEN)
	{
	  errcode = PARAM_BAD_RANGE;
	  goto conf_error;
	}
      ...
      INI_GETSTR_CHK (s, ini, sec_name, "QUERY_REPLACE_ALLOW_NON_SELECT", DEFAULT_QR_ALLOW_NON_SELECT, &lineno);
      {
	...
	br_info[num_brs].query_replace_allow_non_select = (char) on_off;
      }
...
broker_config_dump (FILE * fp, const T_BROKER_INFO * br_info, int num_broker, int br_shm_id, char *admin_log_file)
{
  ...
  for (i = 0; i < num_broker; i++)
      ...
      if (tmp_str)
	{
      ...
      fprintf (fp, "QUERY_REPLACE_RULE\t=%s\n", br_info[i].query_replace_rule);
      fprintf (fp, "QUERY_REPLACE_MAX_RULES\t=%d\n", br_info[i].query_replace_max_rules);
      fprintf (fp, "QUERY_REPLACE_MAX_QUERY_LEN\t=%d\n", br_info[i].query_replace_max_query_len);
      fprintf (fp, "QUERY_REPLACE_ALLOW_NON_SELECT\t=%s\n", br_info[i].query_replace_allow_non_select ? "ON" : "OFF");
