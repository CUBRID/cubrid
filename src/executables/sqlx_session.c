 ...
 */
int
csql_get_help_cmd_no (const char *cmd, DB_HELP_COMMAND * sqlx_cmd_no_ptr)
{
  int i;			/* loop counter */
  size_t input_cmd_length;	/* input command length */
  int num_matches = 0;		/* # of matched commands */
  ...
  if (cmd == NULL)
    {				/* special case */
      *sqlx_cmd_no_ptr = DB_HELP_GENERAL;
      return (CSQL_SUCCESS);
    }
	  ...
	  if (strlen (csql_Help_cmd_table[i].text) == input_cmd_length)
	    {
	      ...
	      *sqlx_cmd_no_ptr = csql_Help_cmd_table[i].cmd_no;
	      return (CSQL_SUCCESS);
	    }
	  num_matches++;
	  matched_index = i;
  ...
  if (num_matches != 1)
    {
      csql_Error_code = (num_matches > 1) ? CSQL_ERR_SQLX_CMD_AMBIGUOUS :
	CSQL_ERR_SQLX_CMD_NOT_FOUND;
      return (CSQL_FAILURE);
    }

  *sqlx_cmd_no_ptr = csql_Help_cmd_table[matched_index].cmd_no;
  return (CSQL_SUCCESS);
}
 ...
 */
void
csql_help_syntax (DB_HELP_COMMAND sqlx_cmd_no)
{
  char **line_ptr;
  COMMAND_HELP *sqlx_syntax = NULL;

  if ((sqlx_syntax = help_command (sqlx_cmd_no)) == NULL)
    {
      csql_Error_code = CSQL_ERR_SQLX_ERROR;
      goto error;
    }
  ...
  APPEND_MORE_LINE (3, sqlx_syntax->name);

  APPEND_HEAD_LINE (msgcat_message (MSGCAT_CATALOG_CSQL,
				    MSGCAT_CSQL_SET_CSQL,
				    CSQL_HELP_SQL_DESCRIPTION_HEAD_TEXT));
  for (line_ptr = sqlx_syntax->description;
  ...
  for (line_ptr = sqlx_syntax->bnf; line_ptr != NULL && *line_ptr != NULL;
       line_ptr++)
    {
      APPEND_MORE_LINE (0, *line_ptr);
    }
  ...
  for (line_ptr = sqlx_syntax->example; line_ptr != NULL && *line_ptr != NULL;
       line_ptr++)
    {
      APPEND_MORE_LINE (0, *line_ptr);
    }
