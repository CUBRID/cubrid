 ...
 *
 * esqlmain.h - Main program header for ESQL
 *
 */
...
/*
 * validate_input_file() - This is a cockup to make sure that lusers that
 *   type 'esqlx' from a command line will get the usual smorgasbord of
 *   option messages rather than a program that silently waits for input.
 *   We do *not* want to see if the named file exists just yet.
 *   that will be done later.
 * return :
 * fname(in): The input file name, or NULL.
 * outfp(in): The output stream to which error messages should go.
 */
static int
...
/*
 * ofile_name() - Make up a filename from the given input filename.  If the
 *    input filename has an extension, that extension is replaced by ".c".
 *    If it has no extension, the output filename is the concatenation of the
 *    input filename and ".c".  In either case, any leading directory
 *    components are ignored, i.e., the output file is in the current
 *    directory.
 * return :
 * fname(in): The name of the current input file.
 */
static char *
 ...
 * msg_num(in) :
 *
 * note : This makes no attempt to copy the messages.  Consequently, each
 * message is valid only until the next call to pp_get_msg() (or
 * util_get_message(), actually, but in this context they should be the
 * same).  This means that trick like
 *
 *      foo(pp_get_msg(...), pp_get_msg(...))
 *
 * will lose.  Figure something else out in that case.
 */
static const char *
