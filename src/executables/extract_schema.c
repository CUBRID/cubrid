 ...
 * 
 *      extract_schema.c: Utility that emits database schema definitions
...
static int emit_schema (DB_OBJLIST * classes, int do_auth,
			DB_OBJLIST ** vclass_list_has_using_index);
static int has_vclass_domains (DB_OBJECT * vclass);
 ...
 * Note:
 *    On conservative flag:
 *    In general, this can't always work since a class can have been altered
 *    to result in circular dependencies.  If that happens, order_classes
 *    will end up calling this function twice, the first time with the
 *    conservative flag on, and the second time with it off.  This allows
 *    us to properly order the domain class references in the "easy" case.
 *
 *    It should be noted that there is one relatively unlikely case that
 *    this will NOT solve which can result in an invalid schema being
 *    emitted.  It is this:
 *
 *    	create class a;
 *      create class b as subclass of a;
 *
 *      create class c (obj a);
 *      create class d as subclass of c (obj b);
 *
 *    The hierarchy for "b" needs to be defined before the hierarchy
 *    of "d", if it isn't then adding superclass "c" to "d" results
 *    in a domain violation because attribute domains "a" and "b" aren't
 *    connected yet and we get an incompatible domain error.
 *    This situation can be avoided by detecting "b" as a dependency
 *    of class "b" due to the fact that "d" is a domain of one of
 *    the attributes.
 *
 *    The problem comes when this happens:
 *
 *	alter class a add attribute uhoh c;
 *      alter class b add attribute uhoh d;
 *
 *    Now, we have a circular dependency, on two different hierarchies.  This
 *    state can only be created in two steps using ALTER statements.
 *
 *    At first blush, one could imagine solving this by emitting the
 *    class hierarchy before any of the attributes were defined.
 *    Unfortunately, when we start adding attributes into a pre-existing
 *    hierarchy, propogation of values can result in unresroved
 *    subclass conflicts that we would have to resolve using the
 *    default rules.  Further, these rules would need to be overridden
 *    later when the attributes for each subclass get defined.
 *    Its not at all clear if this will work, and as usual, its too late
 *    to do the sorts of exhaustive tests necessary to prove that this
 *    will work.
 *
 *    For now, we attempt to catch the common case by making two dependency
 *    passes, one "conservative" and the other "not conservative".
 */
static int
  ...
  while (classes != NULL)
      ...
      if (count == 0)
	{
	  ...
	  if (count == 0)
	    {
	      /*
	       * we made a pass without making any additions, this means
	       * that there are classes with mutual dependencies - shouldn't
	       * happen, indicates cycles in the class hierarchy or
	       * cycles between proxy class domains with attributes that
	       * make up the object id for the proxy.
	       *
	       * If this should happen, dump a message into the output schema
	       * file and forcibly emit the top class on the list.  This
	       * may break the dependency, if not, just keep popping classes
	       * until we're done.
	       * This will at least get the class definitions out to the file
	       * in text form so the user can hand edit it to make it legal.
	       * This indicates a bug in the dependency algorithm that
	       * needs to be fixed.
	       */
...
extractschema (char *exec_name, int do_auth)
{
   ...
   * it doesn't really matter what order they're in.  The dump functions
   * are actually in tr.c.
   */
 ...
 *    This is public so it can be used by exobj.c to tell when to
