...
optimizedb (UTIL_FUNCTION_ARG * arg)
{
  ...
  if (class_name != NULL && class_name[0] != 0)
    ...
    {
      if (sm_update_all_statistics (STATS_WITH_SAMPLING, 0, 0) != NO_ERROR)
	{
	  PRINT_AND_LOG_ERR_MSG ("%s\n", db_error_string (3));
	  db_shutdown ();
	  goto error_exit;
...
#define ARRAY_DEFAULT_SIZE 10
  ...
  do
      ...
      if (error != NO_ERROR)
	{
      ...
      /* attr_name */
      error = db_query_get_tuple_value (query_result, 4, &attr_name_val);
      if (error != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit_on_error;
      ...
      /* attr_data_type */
      error = db_query_get_tuple_value (query_result, 5, &attr_data_type_val);
      if (error != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit_on_error;
      ...
      /* has_partition */
      error = db_query_get_tuple_value (query_result, 6, &value);
      if (error != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit_on_error;
      ...
      fprintf (stdout, "%s | %s %s\n", class_name, attr_name, attr_data_type);
	  ...
	  if (has_partition == 1)
	    {
	      ...
	      error = da_add (partition_array, class_name);
	      if (error != NO_ERROR)
		{
		  ERROR_SET_ERROR (error, ER_OUT_OF_VIRTUAL_MEMORY);
		  goto exit_on_error;
