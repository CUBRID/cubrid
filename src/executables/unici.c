 ...
 *
 * unici.c - CUBRID Call Interface
 *
 * note : The functions in this file are directly embedded in translated
 * ESQL/X C programs. For this reason, there are some different styles of
 * implementation are used as follow:
 *
 *      1. No function returns a meaningful value (just void) because the
 *         emitted code would be very complicated if the sequence of emitted
 *         uci calls are dependent on the previous return status. Rather
 *         than, each uci function set a global variable approrpiately if
 *         it runs into an error and the next uci function checks the global
 *         variable.
 *      2. Since the emitted codes do not introduce any variable, there'd be
 *         performance drawbacks in the case where one uci function locates
 *         a table entry and the next uci function uses the entry. It's the
 *         typical usage of repetitive/dynamic statement table. To avoid such
 *         performance drawback, we introduce a last_searched global
 *         variable in UniCI level. To reduce overhead for searching an entry
 *         which is already found at the previous trial, UniCI keeps the
 *         position lastly searched. And the next searching starts from the
 *         last searched position.
 *      3. Since one ESQL/X statement can have more than one uci functions,
 *         we introduce uci_start() function to catch start of every new
 *         ESQL/X statement (i.e., to manage stuffs whose life is limitted
 ...
 * =======
 *
 * Input Host Variable Binding
 *
 * In UniCI, the host variable binding is done at the run-time using a
 * global `db_Value_table' table. Pre-compiled ESQL/X C programs pass the host 
 * variable addresses to UniCI at run-time, in turn, UniCI builds 
 * db_Value_table and pass it to query processor
 * To avoid overhead of free/allocate of the db_Value_table.db_values table, 
 * UniCI keeps it only increasing. (see uci_set_num_db_values()).
 *
 * Output Host Variable Binding
 *
 * To copy a result column to user's host variable, pre-compiled C program
 * passes the address of the host variable to query processor through
 * UniCI function, uci_get_[db]_value.
 *
 * =======================
 * EXECUTION OF STATEMENTS
 * =======================
 *
 * 1. Execution of Static Statements
 *
 * A static statement can be repetitive or ad hoc. If it is repetitive,
 * UniCI registers the compiled identifier of the statement in `repetitive'
 * table at the first execution. Once it is compiled, the succeeding trials
 * will not re-compile this statement, but just execute with the same
 * compiled statement identifier. This identifier will be used until
 * uci_disconnect() (Note that not until commit/rollback).
 *
 * If it is ad hoc, UniCI simply compile/execute at every execution.
 *
 * 2. Execution of Cursor Statements
 *
 * If a cursor is declared as static select statements, the OPEN statement
 * will compile/execute that statement as ad-hoc. And if it is given with
 * FOR READONLY option, UniCI tells query processor not to have to prepare
 * the result as updatable. However, it the cursor is declared on a dynamic
 * statement (that is, PREPAREd statement - already compiled), the OPEN
 * will get the statement identifer from `dynamic' table and execute it.
 *
 * NOTE: In the current UniCI, FOR READONLY option has no effect if the
 *       cursor declared on a dynamic statements. It is because the current
 *       query processor requires that READONLY_SELECT should be given at
 *       db_compile() instead of db_execute(). It should be fixed for
 *       performance improvement.
 *
 * For update/delete through cursor statements, UniCI treats them as
 * repetitive statement because it is most common that those statements are
 * used in a loop until no more result objects.
 *
 * For opened cursor, UniCI keeps a linked list of cursors, `cursor_list'
 *
 * 3. Execution of Dynamic Statements
 *
 * Every dynamic statement is compiled at PREPARE time and then it can be
 * executed using EXECUTE or cursor statement. To provide users to execute
 * the statement many times once it is prepared, UniCI compiles those
 * statements as repetitive. But, the life time of a prepared stmt is within
 * the current transaction. Since the result of dynamic select statement may
 * be used as update/delete through cursor statement, UniCI does not specify
 * READONLY_SELECT stmt -- it will take overhead in many cases where the
 * select is not for update.
 *
 * 3. Execution of Trivial Dynamic Statements (without statement name)
 *
 * UniCI provides users with the facility to execute a dynamic statement
 * without associating a name. See EXECUTE IMMEDIATE statement.
 * Since there is no possiblilty to be referenced, UniCI compile/execute it
 *
 * ====================
 * LIFE TIME OF RESULTS
 * ====================
 *
 * There are two resources associated with a result of SELECT/CALL/INSERT/
 * FETCH/OBJECT statements. The first one is the result structure which comes
 * from query processor (db_execute). It should be freed using db_query_end()
 * after usage. Another is the space to keep result values. Unlike other
 * systems, CUBRID provides users with variable (unlimited) length of
 * values; string, set, sequence, object. For these values, it is possible
 * for users not to provide a space, but to pass an address of pointer
 * variable.
 * In this case, query processor allocates space to keep the contents and
 * return the pointer to users. These spaces should be freed after usage.
 * Since UniCI cannot know the time when these spaces are not used any more,
 * we introduce the simple life time comcepts of such kind of data.
 * The followings are summary;
 *
 * 1. STATIC/EXECUTE/OBJECT_FETCH statement,
 *
 * The query result structure is effect within the stmt. This is implemented
 * by calling db_query_end() at uci_end().
 * The value contents are valid until the next stmt that belongs to this
 * statement category (or, rollback/commit)
 *
 * 2. FETCH statement
 *
 * The life time of query result structure is the same that of the associated
 * cursor. But, the value contents are valid until next FETCH stmt of the
 * same cursor.
 *
 * 3. Result Column Names of DESRCIBE/OBJECT DESCRIBE stmt
 *
 * For a dynamic statement, the result column names are effective until the
 * statement is invalidated (transaction termination, prepare for other
 * statement with the same dynamic stmt name). For a OBJECT DESCRIBE stmt,
 * they are effective until the next OBJECT DESCRIBE statement or transaction
 * termination.
 *
 * ===================
 * SQLCA - sqlerrd[2]
 * ===================
 *
 * If the statement is INSERT/DELETE/UPDATE/FETCH, this value will be set to
 * the # of objects afftected after execution.
 *
 * ===================
 * SQLCA - NOT FOUND
 * ===================
 *
 * UniCI has a little different policy to set NOT FOUND. It comes from the
 * introduction of INSERT return value and CALL statement.
 * If INTO-clause is given in SELECT/FETCH/CALL/INSERT statement and there
 * is no (more) result, then sqlcode is set to SQL_NOT_FOUND. But, if there
 * is no INTO-clause associated the statement, we are not set that value even
 * though there is no result. Note we are not set NOT FOUND when UPDATE/
 * DELETE statement does not affect any objects.
 *
 * ================================
 * SQLDA FOR CALL/INSERT statement
 * ================================
 *
 * 1. CALL statement
 *
 *      sqlcmd                  = SQLX_CMD_CALL
 *      sqldesc                 = 0 if no return value
 *                              = 1 if unknown yet (e.g., ON ?)
 *      if sqldesc == 1,
 *      sqlvar[0].sqltype       = DB_TYPE_DB_VALUE if unknown,
 *                              = DB_TYPE_..., otherwise.
 *      if sqlvar[0].sqltype != DB_TYPE_DB_VALUE, the other fields are the
 *      same that of normal statement except sqlname will be NULL.
 *
 * 2. INSERT statement
 *
 *      sqlcmd                  = SQLX_CMD_INSERT
 *      sqldesc                 = 1
 *      sqlvar[0].sqltype       = DB_TYPE_OBJECT
 *      the other fields are the same that of normal statement except
 *      sqlname will be NULL.
 *
 *
 * =======================
 * MAKING UniCI RE-ENTRANT
 * =======================
 *
 * Suppose there is a method which is implemented on top of ESQL/X and
 * there is a statement which calls the method. During the execution of
 * the statement, UniCI will set global variables specific to the status
 * (or results) of the current statement. Before completion of the statement,
 * it will invoke another ESQL/X statement because the method is using
 * ESQL/X statements. Then, the same global variables will be changed to
 * reflect the ESQL/X statements. After return of the method execution,
 * the information of the original statement will be lost.
 *
 * To cope with this situation, we make UniCI re-entrant in terms of
 * a statement execution (we don't have to prepare for arbitrary
 * re-entrancy, but only for re-entrant during a statement execution).
 * Every statement pre-compiled into sequences of UniCI functions beginning
 * with uci_start() and end with uci_end(). In order to cope with nested
 * execution of ESQL/X statement, we need to save (push) the current
 * environment just before the execution of a statement and restore (pop) it
 * right after the execution has finished.
 *
 * In UniCI level, however, it may not easy to figure out whether the
 * statement execution will call ESQL/X again or not. To avoid unneccessay
 * stack operation, we put off push operation until UniCI tries to execute
 * the nested ESQL/X statement (it can be detected by successive call of
 * uci_start()). When a statement is executed, UniCI check if it was the
 * execution of nested statement or not (if uci_end() was called, it implies
 * the nested execution finish.
 *
 * Currently, the following variables are build before execution of a
 * statement, have the information specific to the current statement,
 * may be changed by the nested statement and may need to be accessed
 * after the execution (uci_end()) of nested statement.
 *
 *      sqlca (sqlca.sqlwarn)
 *      db_Value_table
 *      curr_file
...
#define	_ESQLX_KERNEL_		/* to inform esqlx.h it is not a ESQL/X user */
...
/* to check if the given type belongs to legal ESQL/X DB_TYPE.
 * Note that some of DB_TYPE are not supported in ESQL/X
 */
...
#define	AFFECT_OBJECTS(stmt_type)	(stmt_type == SQLX_CMD_SELECT || \
					 stmt_type == SQLX_CMD_UPDATE || \
					 stmt_type == SQLX_CMD_DELETE || \
					 stmt_type == SQLX_CMD_INSERT)
...
/* error codes introduced by UniCI module */
...
typedef struct repetitive
{
  ...
  SQLX_CMD_TYPE stmt_type;	/* true if the stmt is SELECT */
  DB_GADGET *gadget;
} REPETITIVE;
...
/* source file identifier which is executing UniCI functions currently.
...
static DYNAMIC *alloc_dynamic (int stmt_no, SQLX_CMD_TYPE stmt_type,
...
/*
 * uci_startup() - tell UniCI the `pgm_name' is the executable file name
 * return : void
 * pgm_name(in) : executable file name of this process
 * note : it will be used for db_restart(), see uci_connect()).
 *    initialize some UniCI global variables.
 */
void
...
/*
 * uci_start() - This function tells UniCI the start of new ESQL/X C statement.
...
uci_static (int stmt_no, const char *stmt, int length, int num_out_vars)
{
  ...
  if (stmt_id != -1)
      ...
      if (!gadget)
	{
	  ...
	  stmt_type =
	    (SQLX_CMD_TYPE) db_get_statement_type (session, stmt_id);
	}
 ...
 *              by users, UniCI keeps all such statements until
...
alloc_repetitive (int no, DB_SESSION * session,
		  STATEMENT_ID stmt_id,
		  SQLX_CMD_TYPE stmt_type, DB_GADGET * gadget)
{
  int new_num;
  REPETITIVE *rt;
 ...
 *    time of a dynamic statement is not given explicitly by users, UniCI
 ...
 */
static DYNAMIC *
alloc_dynamic (int stmt_no, SQLX_CMD_TYPE stmt_type,
	       DB_SESSION * session, STATEMENT_ID stmt_id,
...
/*
 * clean_up() - take care of clean-up UniCI stuffs if abmornal things happen.
 * return : return void.
 *
