 ...
 *
 * es.g - String conversion functions
 */
 ...
 * return : void
 * parser_in(in) : the sqlx parser context
 */
static void
...
sql_or_other:
(sqlx (declare_section | esqlx_statement) << yy_enter (ECHO_mode);
 pp_suppress_echo (false);
 >>SEMI | /* or anything else */ )
  ;

sqlx:SQLX << yy_enter (SQLX_mode);
reset_globals ();
>>;
...
{
  long type;
  type = check_identifier (&sqlx_table);
  ECHO;
  LA (1) = type;
...
>>declare_section_body
  /* note "sqlx" does lexclass mode switch */
  EXEC sqlx END DECLARE SECTION;

/* SQLX statement section. */
esqlx_statement:
...
>>(ALTER
   ...
   | TRIGGER
   | USE)
  sqlx_statement_tail[NO_CURSOR]
  | DELETE
  sqlx_statement_tail[CURSOR_DELETE]
  | UPDATE sqlx_statement_tail[CURSOR_UPDATE];
...
<<
{
  ...
  /*
   * Postpone the switch to recognizing-sqlx-keywords if
 ...
 */
/* '#' is a legal sqlx identifier character, for some odd reason.
...
#token "\-\-~[\n]*\n" <<zzline++; zzendcol = 0; ECHO_SP; zzskip();>>
#token  "~[\n]"       <<ECHO; zzskip();>>
sqlx_statement_tail <[long cursor_kind]:<<PT_NODE ** pt, *cursr;
char *statement, *text;
PT_HOST_VARS *result;
long is_upd_obj;
HOST_LOD *inputs, *outputs;
