...
{
  char class_name[SM_MAX_IDENTIFIER_LENGTH_287];
...
char chksum_result_Table_name[SM_MAX_IDENTIFIER_LENGTH_287];
char chksum_schema_Table_name[SM_MAX_IDENTIFIER_LENGTH_287];
...
#define QUERY_BUF_SIZE		2048
  ...
  if (res >= 0)
      ...
      while (pos == DB_CURSOR_SUCCESS)
	{
	  ...
	  if (db_string_p != NULL)
	    {
	      snprintf (checksum_result->class_name, SM_MAX_IDENTIFIER_LENGTH_287, "%s", db_string_p);
	    }
...
chksum_need_skip_table (const char *table_name, CHKSUM_ARG * chksum_arg)
{
  ...
  char table_in_list[SM_MAX_IDENTIFIER_LENGTH_287];
...
checksumdb (UTIL_FUNCTION_ARG * arg)
{
  ...
  if (sm_check_name (checksum_table) > 0)
    {
      snprintf (chksum_result_Table_name, SM_MAX_IDENTIFIER_LENGTH_287, "%s", checksum_table);
    }
  else
    {
      snprintf (chksum_result_Table_name, SM_MAX_IDENTIFIER_LENGTH_287, "%s", CHKSUM_DEFAULT_TABLE_NAME);
    }

  if (snprintf (chksum_schema_Table_name, SM_MAX_IDENTIFIER_LENGTH_287 - 1, "%s%s", chksum_result_Table_name,
  ...
  if (incl_class_file != NULL)
    {
      chksum_arg.include_list = da_create (CHKSUM_DEFAULT_LIST_SIZE, SM_MAX_IDENTIFIER_LENGTH_287);
  ...
  if (excl_class_file != NULL)
    {
      chksum_arg.exclude_list = da_create (CHKSUM_DEFAULT_LIST_SIZE, SM_MAX_IDENTIFIER_LENGTH_287);
