...
construct_instance (void)
{
  ...
  /*
   * NOTE: We're about to store this in the instance table for later
   * reference.  Unforatunately we now have a temporary OID which can't
   * be stored in disk instance references.  We have to convert this
   * to a permanent one before storing it.  There are a couple
   * of ways to do this, if we were on a client, we would use
   * locator_assign_perm_oid.  Since we're guarenteed to also be a server,
   * there's really no need to defer the flush so we can just
   * call locator_flush_instance as well.  This will also make
   * it behave more like non-constructor instances which are
   * inserted directly into the heap file as we go.
   */
  if (obj != NULL)
    {
...
insert_instance ()
{
  ...
  if (Loader.validation_only)
	...
	{
	  /*
	   * Rather than actually inserting dummy entries into the class tables,
	   * just keep incrementing the counter so we can allocate the real
	   * table once during the insertion pass.  Note that this means any
	   * warnings that might be generated by otable_insert/otable_update
	   * won't be seen until the actual insertion pass.  Since currently
	   * the only errors these two functions can return are memory errors,
	   * this isn't a problem.  If they can detect some fundamental object
	   * reference problem however, we will have to adjust our strategy
	   * here.
	   */
	  if (Loader.inum != -1)
	  ...
	  if (Loader.status_counter >= Loader.status_count)
	    {
	      ...
	      fprintf (stdout, "%d ", Loader.icount);
      ...
      else
	{
	  ...
	  if (sig->num_args && Loader.arg_count >= sig->num_args)
	  ...
	  else
	    {
	       ...
	       */
	      arg = classobj_find_method_arg (&sig->args, Loader.arg_count + 1, 0);
	      index = add_argument ();
	      if (index >= 0)
...
ldr_end_set (void)
{
  if (Loader.valid)
      ...
      if (Loader.set != NULL)
	{
	  ...
	  /*
	   * If we're really supposed to be creating a sequence, the domain
	   * will have a type of DB_TYPE_SEQUENCE even though the set object
	   * claims to be (and *is*) a DB_TYPE_MULTISET.  Version 2.48 of
	   * or.c has a new version of or_put_set() that understands this
	   * possible discrepancy and accounts for it so that we don't
	   * actually need to be building real live sequences down here.
	   *
	   * This relies on the above kludge in ldr_start_set where we stored
	   * the actual domain in the set.  If that changes, we may be out of
	   * luck here.
	   *
	   * There used to be some code here that tried to do the conversion,
	   * but it relied on a function (set_multiset_to_sequence()) that
	   * wasn't actually implemented, and so nothing happened.  
	   */
	  ...
	  if (Loader.set == NULL)
	      ...
	      else
		{
		  ...
		  if (value == NULL)
		    ...
		    {
		      /*
		       * If token type is DB_TYPE_NULL, should check for
		       * existing NULL's. Uses a special set_ function that
		       * appends a set element and returns a pointer directly
		       * to the value.
		       * Make absolutely sure the value gets initialized
		       * properly here with the correct precision scale etc.
		       * If it isn't then the set writer can get confused, this
		       * is especially important for fixed width char types.
		       */
		      if (domain == NULL)
			{
  ...
  if (!(error = check_object_domain (class, &actual_class)))
      ...
      if (Loader.set == NULL)
	{
	  ...
	  if (error == ER_LDR_INTERNAL_REFERENCE)
	    ...
	    {
	      /*
	       * Kludge, if this value is destined for a class attribute, shared
	       * attribute or default value, we will use a standard API function
	       * to set it. During that process, the OID will be tested against
	       * the domain of the target attribute.  If this is a forward
	       * reference, we will have manufactured an OID using heap_assign_address
	       * but the object will have no contents. If we simply make a MOP out
	       * of this object with db_object() we will try to fetch the object in
	       * order to find its class to perform domain verification.  The object
	       * fetch will fail because it has no contents, and because of that,
	       * the class field in the MOP will not be set so the domain validation
	       * fails too.
	       *
	       * To avoid this situation, we pre-create an appropriate MOP in the
	       * workspace with the class pointer set so that domain validation can
	       * be performed. This must happen ONLY for class/shared/default
	       * values, regular attribute values don't do this because they bypass
	       * the API for assignment.
	       *
	       * Note: classes with constructors also require this treatment due to
	       * the API functions used in construct_instance().
	       *
	       */
	      DB_ATTRIBUTE *att;
