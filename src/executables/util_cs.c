...
util_get_class_oids_and_index_btid (dynamic_array * darray, const char *index_name, BTID * index_btid)
{
  ...
  char table[SM_MAX_IDENTIFIER_LENGTH_287];
  char name[SM_MAX_IDENTIFIER_LENGTH_287];
  int i;
  ...
  for (i = 0; i < num_tables; i++)
      ...
      if (table == NULL || table[0] == '\0')
	{
      ...
      sm_downcase_name (table, name, SM_MAX_IDENTIFIER_LENGTH_287);
...
checkdb (UTIL_FUNCTION_ARG * arg)
{
  ...
  darray = da_create (num_tables, SM_MAX_IDENTIFIER_LENGTH_287);
  if (darray == NULL)
    {
      perror ("calloc");
      util_log_write_errid (MSGCAT_UTIL_GENERIC_NO_MEM);
  ...
  if (num_tables > 0)
    {
      char n[SM_MAX_IDENTIFIER_LENGTH_287];
      char *p;
