...
/*
 * INFORMATION FOR MIGRATING FROM R2.0 AND R2.1
 */
/* LOG HEADER FOR R2.0 AND R2.1 */
typedef struct log_header_r20 LOG_HEADER_R20;
struct log_header_r20
{
  char magic[CUBRID_MAGIC_MAX_LENGTH];
  INT32 dummy;
  INT64 db_creation;
...
static int fix_db_partition (void);

/*
 * function prototypes for migrating from R1.x
 */
/* fix volume header */
static int fix_active_log_header (const char *log_path);
static int fix_volume_header (const char *vol_path);
static int fix_volume_and_log_header (const char *db_path, float db_compat);
 ...
 */
static int
fix_active_log_header_from_r20 (const char *log_path)
{
  FILE *fp = NULL;
...
fix_db_serial (void)
{
  ...
  att = db_get_attribute (class_mop, "cached_num");
  if (att != NULL)
    {
      printf ("db_serial class was already fixed\n");
      goto error;
    }
  ...
  snprintf (sql_stmt, LINE_MAX,
	    "INSERT INTO %s(name, owner, current_val, increment_val,"
	    "max_val, min_val, cyclic, started, class_name, att_name, cached_num) "
	    "SELECT name, owner, current_val, increment_val, max_val, min_val,"
	    "cyclic, started, class_name, att_name, 0 FROM old_%s;",
	    CT_SERIAL_NAME, CT_SERIAL_NAME);
...
fix_db_class (void)
{
  ...
  /*
   * change old db_class to new db_class
   */
  class_mop = db_find_class (CTV_CLASS_NAME);
  if (class_mop == NULL)
    {
      error_code = er_errid ();
      goto error;
  ...
  att = db_get_attribute (class_mop, "is_reuse_oid_class");
  if (att != NULL)
    {
      printf ("db_class class was already fixed\n");
      return NO_ERROR;
    }

  ct = dbt_edit_class (class_mop);
  if (ct == NULL)
    {
      error_code = er_errid ();
      goto error;
  ...
  error_code =
    dbt_add_attribute (ct, "is_reuse_oid_class", "varchar(3)", NULL);
  if (error_code != NO_ERROR)
    {
      goto error;
    }
  ...
  error_code = dbt_change_query_spec (ct, stmt, 1);
  if (error_code != NO_ERROR)
    {
      goto error;
    }

  class_mop = dbt_finish_class (ct);
  if (class_mop == NULL)
    {
      error_code = er_errid ();
      goto error;
...
fix_db_partition (void)
{
  ...
  /*
   * change db_partition's query spec
   */
  class_mop = db_find_class (CTV_PARTITION_NAME);
  if (class_mop == NULL)
    {
      error_code = er_errid ();
      goto error;
  ...
  sprintf (stmt,
	   ...
	   "  WHERE u.name = CURRENT_USER) AND"
	   "  au.auth_type = 'SELECT'))",
	   CT_PARTITION_NAME, CT_PARTITION_NAME,
	   au_get_user_class_name (), CT_CLASSAUTH_NAME,
	   au_get_user_class_name ());
...
/*
 * FUNCTIONS FOR MIGRATING FROM R1.x
 */
...
static int
fix_active_log_header (const char *log_path)
{
  FILE *fp = NULL;
  ...
  new.db_logpagesize = old->db_iopagesize;	/* added on R2.2 */
			      ...
			      float *compat_level, char *release_string)
{
  ...
  if (new_header->db_compatibility > 8.0f
      && new_header->db_compatibility <= rel_disk_compatible ())
    {
...
static int
fix_volume_and_log_header (const char *db_path, float db_compat)
{
  char vol_info_path[PATH_MAX], vol_path[PATH_MAX];
...
static char *
make_volume_header_undo_page (const char *vol_path)
{
...
main (int argc, char *argv[])
{
  ...
  if (!is_already_fixed)
    {
      ...
      if (fix_volume_and_log_header (db_full_path, compat_level) != NO_ERROR)
	{
  ...
  if (fix_system_class (compat_level) != NO_ERROR)
    {
      goto rollback;
    }
