 ...
 *
 * parse_tree.c - Parser module for the client and the server
 */
...
parser_free_node (const PARSER_CONTEXT * parser, PT_NODE * node)
{
  ...
  /*
   * Always set the node type to something unrecognizable.  This may
...
/*
 * parser_create_parser () - creates a parser context pointer for use in subsequent
 *      calls to pt_parse routines. The pointer should be passes to top level
