...
#define mq_free_virtual_query_cache_and_init(virtual_query_cache) \
  ...
  extern int pt_check_distinct_spec_names (PARSER_CONTEXT * parser, const PT_NODE * p);
...
#define pt_has_error(parser) ( (parser) && ((parser)->error_msgs || (parser)->flag.has_internal_error))
  ...
  extern const char *pt_name_qualified (const PARSER_CONTEXT * parser, const PT_NODE * name);
