...
pt_to_delete_xasl_remote_subquery (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  /* Source column type of the local subquery. The sink restores the pushed value's declared type at
   * execution time and can only learn that type here, while the select list is still available -- by then
   * only the value itself is left (see remote_src_type in DELETE_PROC).
   *
