...
XASL_NODE *
ptqo_to_hash_build_scan_proc (PARSER_CONTEXT * parser, XASL_NODE * xasl, XASL_NODE * listfile, PT_NODE * namelist,
			      PT_NODE * spec, PT_NODE * pred, PT_NODE * hash_pred, int *poslist)
{
  if (xasl == NULL)
    {
      xasl = regu_xasl_node_alloc (SCAN_PROC);
  ...
  if (xasl && listfile)
    {
      ...
      TABLE_INFO *tbl_info;
      PT_NODE *saved_current_class;
      PT_NODE *build_attrs = NULL, *probe_attrs = NULL;
      int *attr_offsets;

      tbl_info = pt_find_table_info (spec->info.spec.id, parser->symbols->table_info);

      if (pt_split_hash_attrs (parser, tbl_info, hash_pred, &build_attrs, &probe_attrs) != NO_ERROR)
	{
	  return NULL;
	}
      regu_attributes_build =
	pt_to_regu_variable_list (parser, build_attrs, UNBOX_AS_VALUE, tbl_info->value_list, NULL);
      regu_attributes_probe =
	pt_to_regu_variable_list (parser, probe_attrs, UNBOX_AS_VALUE, tbl_info->value_list, NULL);
	...
	pt_make_list_access_spec (listfile, ACCESS_METHOD_SEQUENTIAL, NULL, pred_expr, regu_attributes, NULL,
				  regu_attributes_build, regu_attributes_probe);
