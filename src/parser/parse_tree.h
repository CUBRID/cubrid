...
#define PT_IS_LOB_FAMILY_TYPE(t) \
        ( PT_IS_LOB_TYPE(t)      || \
...
typedef enum
{
  ...
  PT_BIT_TO_BFILE, PT_BFILE_FROM_FILE, PT_BFILE_LENGTH, PT_BFILE_TO_BIT,
  PT_CHAR_TO_BFILE, PT_CHAR_TO_CFILE, PT_CFILE_FROM_FILE, PT_CFILE_LENGTH,
  PT_CFILE_TO_CHAR,
  PT_BLOB_TO_BFILE, PT_CLOB_TO_CFILE,
  ...
  PT_CLOB_TO_CHAR,
  PT_BFILE_TO_BLOB, PT_CFILE_TO_CLOB,
  PT_TYPEOF,
...
struct pt_copy_info
{
  ...
  union				/* format-specific options, selected by `format` */
  {
    struct
    {
      int delimiter;		/* field delimiter char; 0 = default ',' */
      int quote;		/* quote char; 0 = default '"' */
      int header;		/* 1 = skip a leading header line */
    } csv;
  } fmt;
};
