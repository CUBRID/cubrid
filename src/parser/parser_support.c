...
pt_fold_internal_lob_direct_source_values (PARSER_CONTEXT * parser, PT_NODE ** values)
{
  ...
  for (link = values; *link != NULL; link = &(*link)->next)
      ...
      if (value_node == NULL)
	{
      ...
      /* Rewrite first, free the old source only on success: on failure *link must keep pointing at a live
       * node so the caller's statement teardown frees it exactly once, not at a dangling one. */
...
pt_fold_internal_lob_direct_source_assignments (PARSER_CONTEXT * parser, PT_NODE * assignments)
{
  ...
  for (assign = assignments; assign != NULL; assign = assign->next)
      ...
      if (value_node == NULL)
	{
      ...
      /* Rewrite first, free the old rhs only on success: on failure assign->info.expr.arg2 must keep pointing
       * at a live node so the caller's statement teardown frees it exactly once, not at a dangling one. */
      save_next = rhs->next;
      value_node->next = NULL;
      host_var = pt_rewrite_to_auto_param (parser, value_node);
      if (host_var == NULL)
