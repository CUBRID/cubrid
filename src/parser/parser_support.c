			     ...
			     SERVER_NAME_LIST * snl)
{
  ...
  while (tbl_spec)
	...
	{
	  sel = tbl_spec->info.spec.derived_table = parser_new_node (parser, PT_SELECT);  
	  sel->info.query.q.select.list = parser_new_node (parser, PT_VALUE);
	  spec = sel->info.query.q.select.from = parser_new_node (parser, PT_SPEC);
	  spec->info.spec.derived_table = parser_new_node (parser, PT_DBLINK_TABLE);
	  spec->info.spec.derived_table_type = PT_DERIVED_DBLINK_TABLE;
	  spec->info.spec.derived_table->info.dblink_table.conn = tbl_spec->info.spec.remote_server_name;
	  spec->info.spec.derived_table->info.dblink_table.remote_table_name = "t1";
	  qstr = spec->info.spec.derived_table->info.dblink_table.qstr = parser_new_node (parser, PT_NAME);
	  qstr->info.name.original = "t1";
	}
      tbl_spec = tbl_spec->next;
    }
