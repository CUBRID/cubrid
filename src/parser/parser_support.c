  ...
  if (add_as_attr)
    {
      ...
      as_attr->info.name.original = (const char *) pt_append_string (parser, (char*)as_attr->info.name.original, spec_name);
  ...
  while (node != NULL)
    {
      ...
      qstr_trim_trailing (pad, pad_size, node->info.value.data_value.str->bytes, pt_node_to_db_type (node), char_count,
			  node->info.value.data_value.str->length, (INTL_CODESET)dt->info.data_type.units, &trimmed_length,
			  &trimmed_size, true);
