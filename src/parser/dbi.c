 ...
 *
 * dbi.c - a number of auxiliary functions required to convert parse tree
 ...
 */
/* TODO: PT_TYPE_ENUM, which is defined in pt_pub.h, should be changed to
 * PT_TYPE_NUM by adjusting the order of including header files */
DB_TYPE
 ...
 *   return: Returns one of the PT_TYPE_ENUMs defined in pt_parse.h
...
/*
 * pt_node_to_cmd_type() - Convert node to SQLX_CMD_TYPES
 *   return: one of the SQLX_CMD_TYPES defined in dbi.h.
 *   node(in):
 */
int
pt_node_to_cmd_type (PT_NODE * node)
{
  ...
  switch (node->node_type)
	...
	{
	  return SQLX_CMD_GET_ISO_LVL;
	}
      else if (node->info.get_xaction.option == PT_LOCK_TIMEOUT)
	{
	  return SQLX_CMD_GET_TIMEOUT;
	}
