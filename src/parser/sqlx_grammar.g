 ...
 *
 * sq.g - SQL grammar file
 * TODO: rename this file to csql_grammar.g
 */
...
esqlx_query_statement "sqlx query"
	: 	<<PT_NODE *stmt;
                  select_level++;>>
	  sqlx_query
	...
	: 	<< $var = parser_new_node(this_parser, PT_HOST_VAR); >>
	  ...
	  "?" COLON_ UNSIGNED_INTEGER
                << if (parent_parser == NULL) {
                     ...
                     PT_ERRORf(this_parser, $var, "check syntax at %s",
                       parser_print_tree(this_parser, $var));
                   } else if ($var) {
                     $var->info.host_var.var_type = PT_HOST_IN;
		     $var->info.host_var.str   = pt_makename($1.text);
...
*/
	| insert_expression
		...
		/* DELETE THIS WHEN SUPPORTED SQLX VERSIONS USE THIS PARSER.
		 * Probably any time after 1.2
		 */
		   if(ins)
...
/* this is what it should be expression_ */
            search_condition
/* this is subquery part ( see the next 'sqlx_query') subquery */
            { (table_op select_or_subquery
...
subquery "subquery"
	: 	<< PT_NODE *stmt; >>
	  LEFT_PAREN sqlx_query Right_Paren
	  	<< stmt = pt_pop(this_parser);
                    if (within_join_condition) {
