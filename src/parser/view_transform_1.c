 ...
 *
 * vtrans.c - Functions for the translation of virtual queries
 */
...
static PT_NODE *mq_flatten_union (PARSER_CONTEXT * parser,
				  PT_NODE * statement);
static PT_NODE *mq_spec_has_proxy (PARSER_CONTEXT * parser, PT_NODE * node,
				   void *void_arg, int *continue_walk);
...
static PT_NODE *mq_rewrite_agg_names_post (PARSER_CONTEXT * parser,
					   ...
					   int *continue_walk);
static int mq_conditionally_add_objects (PARSER_CONTEXT * parser,
					 PT_NODE * flat,
					 DB_OBJECT *** classes, int *index,
					 int *max);
static int mq_updatable_local (PARSER_CONTEXT * parser, PT_NODE * statement,
...
}				/* mq_substitute_spec_in_method_names */
 ...
 *          to leaf sqlx classes or local db tables/classes
 *   parser(in):
 *   tree(in/out):
 *   spec_list(in):
 *   order_by(in):
  ...
  for (class_spec = spec_list; class_spec != NULL;
      ...
      if (had_some_virtual_classes)
	{
	  ...
	  if (real_part)
	    ...
	    {
	      /* Getting here means there were NO subqueries or real classes.
	       *    This means all classes were virtual, with no query spec's.
	       *    An easy, way to get "there were no results"
	       *    back is STILL to leave the tree. This is not as bad
	       *    as it sounds, as this should be an extraordinarily
	       *    rare thing to do. Selecting from a virtual class
	       *    as if it were a real class will work just fine, since
	       *    there are no instances in it. Putting in some
	       *    fancy code to make this real fast, is fairly likely to
	       *    result in bugs bein introduced that may never get
	       *    found, except by customers. So for ease of maintenance,
	       *    we let this take the slow, safe, well tested route.
	       *    so-don't rewrite the tree.

	       *    However, if this is supposed to modify data, return
	       *    a NULL statement!
	       */
...
/* insert poses some unique translation problems.
   1) It can translate to a list of inserts, but only one should be chosen
      at run time.
   2) It can appear be in an (insert) expression list, and itself be
      translated to a list.
   3) Some implicit composition hierarchy happens with nested inserts.
      When this happens, there may be a list returned for a nested insert.
      Then the list must be matched against the real class of the
      chosen attribute.
*/
	  ...
	  if (flat)
	      ...
	      if (!db_is_class (flat->info.name.db_object))
		{
		  if (spec->info.spec.derived_table_type == PT_IS_SUBQUERY)
		    ...
		    {
		      /* THIS WILL GET THE WRONG ANSWER for views of
		       * sqlx proxies with sub-proxies on the left hand side
		       * All such proxies will be treated as coming from
		       * the super-proxy.
		       * THIS IS AS CLOSE AS WE CAN GET WITH THIS DEFECT.
		       * In fact, if the sqlx proxy objects come back
		       * with their class information, this is correct
		       * and will get the correct answer always.

		       * this is never fruitful to examine for ldb's
		       *   There are two cases.
		       *   1) The ldb has intrinsic ids (sqlx) and
		       *      knows how to evaluate path expressions.
		       *      These will get the correct path expression
		       *      by evaluating it in the ldb, since the
		       *      proxy hierarchy must be compatible with the
		       *      ldb class heirarchy.

		       *      We in fact CANNOT
		       *      get the correct answer by using a derived table
		       *      for this case, since currently we include
		       *      proxy id in equality comparison.
		       *      The only way to get the correct answer with
		       *      the current implementation is to allow
		       *      the ldb to evaluate the path.
		       *      At least, we get the right answer when no
		       *      subcalsses are involved.

		       *   2) The ldb is relational, and cannot have
		       *      a true reference to any but one table.
		       *      This means that only the single level
		       *      mapping of the proxy to the table via
		       *      a foreign key need by considered. The
		       *      subclasses which are allowed for the
		       *      proxy which types left hand of the path
		       *      cannot really be reached from the path.
		       *      They serve only to organize the proxies
		       *      to project a model, but can not give the
		       *      relational db the capability to have a foreign
		       *      key reference multiple tables. Recognizing this
		       *      intrinsic limitation of relational databases
				   ...
				   void *void_arg, int *continue_walk)
{
  ...
  switch (node->node_type)
    {
    case PT_FUNCTION:
      /* check non-updatable vclass oid {'v'}

       * case 1 (join query): v is 'SELECT x.i, y.j FROM x, y'
       *    SELECT v, i, j FROM v
       * -> SELECT {'v'}, x.i, y.j FROM x, y;

       * case 2 (distinct query): v is 'SELECT distinct i FROM x'
       *    SELECT v, i FROM v
       * -> SELECT v, i FROM (SELECT distinct {'v'}, i FROM x) as v(v, i);

       * case 3 (aggregate query): v is 'SELECT sum(i) FROM x'
       *    SELECT v, i FROM v
       * -> SELECT v, i FROM (SELECT {'v'}, sum(i) FROM x) as v(v, i);
       */
