 ...
 *
 * evaluate.c - Helper functions to interprets tree and
			       ...
			       PT_NODE * tree, DB_VALUE * vals, int cnt)
{
  ...
  if (cursor_open (&cursor_id, list_id, false,
	  ...
	  || cursor_get_tuple_value_list (&cursor_id, cnt, vals) != NO_ERROR)
	{
	   ...
	   * ESQL/X context.  Just say that we didn't succeed, which should be
 ...
 *   return:  true (keep going) or allocation error
  ...
  if (pt_Label_table)
    {
      ...
      if ((error = mht_map (pt_Label_table,
			    pt_make_label_list, (void *) list)) != true)
	{
	  nlist_free (*list);
	  *list = NULL;
	}
