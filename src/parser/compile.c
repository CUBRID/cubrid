			 ...
			 const char *name2)
{
  ...
  if (statement)
	...
	{
	  statement->bs.si_datetime |= statement->info.scope.stmt->info.trigger_action.expression->bs.si_datetime;
	}
    }
		      ...
		      DB_VALUE * result)
{
  ...
  if (trigger_stmt->bs.si_datetime)
    {
      server_info_bits |= SI_SYS_DATETIME;
    }

  if (trigger_stmt->bs.si_tran_id)
    {
