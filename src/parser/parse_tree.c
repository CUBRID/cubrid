 ...
 *
 * parse_tree_cl.c - Parser module and parse tree functions for the client
 * TODO: rename this file to parser_tree_cl.c
 * TODO: included parse_tree_2.c and parse_tree_3.c so delete them
 */
 ...
 * ALL 4 function vectors.  (apply, init, print, tree_print -- in pt_parse.c)
 */
		...
		PT_NODE * tree_or_name, void *void_arg, int *continue_walk)
{
  ...
  *continue_walk = PT_CONTINUE_WALK;
  ...
  if (lambda_arg->loc_check == true
	  ...
	  name_node->info.name.location != lambda_name->info.name.location))
    {
      ...
      if (walk->continue_walk)
	{
	  ...
	  if (save_continue == PT_CONTINUE_WALK
	      || save_continue == PT_LEAF_WALK)
	    {
	      ...
	      /* The safe guard to test if the node type falls within
	       * legal values based on the PT_NODE_TYPE enumeration type
	       * range, does not work currently when compiled with gcc.
	       * Specifically for the 'node_type < 0' test, gcc makes the
	       * assumption, (wrongly), that PT_NODE_TYPEs are
	       * unsigned and will gives a compiler warning indicating an
	       * invalid test.  This should be fixed. One solution would be
	       * to introduce a PT_UNDEFINED_NODE_TYPE to set when a node
	       * is unused, e.g., when freed, and perform the check of
	       * node_type against PT_UNDEFINED_NODE_TYPE.
	       */
 ...
 *
 * Note :
 *   helper function for PT_WARNING macro (see pt_parse.h)
 */
 ...
 *
 * Note :
 *   helper function for PT_ERROR macro (see pt_parse.h).
 */
...
/*
 * Note:
 * These parser_print_tree functions are used in at least two ways:
 * 1) to reconstitute a SQL statement (a character string) representation of
 *    a given parse tree
 * 2) to collect together into parser->input_values all the input host vars
 *    that pertain to a given ldb-specific SQL statement.
 * Case (2) is used in the bulk fetching/locking/flushing of proxy instances
 * where (1) is done once per proxy, and (2) is done hundreds of times: once
 * per proxy instance. Quantify showed (1) is expensive because of the mallocs
 * done by pt_append_nulstring and friends. To efficiently do (2), two parser
 * flags: dont_prt and dont_prt_xasl can be asserted to bypass the expensive
 * calls to pt_append_nulstring and friends. This is done in vid.c.
 *
 * If you need to modify or extend these parser_print_tree functions, please make sure
 * that you apply parser_print_tree to any subtree that may hold a host variable. For
 * example, when printing a SELECT's where clause please do it like this
 ...
 *     q=pt_append_varchar(parser, q, r1);
 *   }
 * parser_print_tree must be applied to the q.select.where subtree even when the
 * parser->dont_prt is asserted because the q.select.where subtree may have
 * host variables that need to be collected into parser->input_values. The
 * printed SQL statement from (1) must have exactly the same number of input
 * host var markers ("?") as the parser->input_values collected by case (2).
 * Otherwise the bulk fetching/locking/flushing of proxy instances will fail.
 */
...
pt_print_alter (PARSER_CONTEXT * parser, PT_NODE * p)
{
  ...
  /*
   * Ack! What do we do about the constraint list?  It's already been
   * converted to class constraint form!

   * For right now we can simply blow it off since the only constraints
   * that matter are UNIQUE and NOT NULL and they are already handled
   * in the ATTR_DEF nodes, but eventually we probably need to add a
   * CONSTRAINT class to ALTER statements, and then the constraint list
   * should print in that syntax.
   */
  return q;
}
  ...
  switch (p->info.expr.op)
      ...
      if (!parser->dont_prt)
	{
	  ...
	  q = pt_append_nulstring (parser, q,
				   pt_show_binopcode (p->info.expr.op));
	  /*
	   * Grotesque hack to avoid inadvertently creating a double-dash
	   * token, which tends to be interpreted as a comment start symbol
	   * by parsers ingesting this output.  Right now the only way to
	   * produce such a beast appears to be to print a unary minus sign
	   * immediately following a binary minus sign; we'll catch that
	   * right here as we lay down the text of the second operand.

	   * We don't bother to check the value of r2->length (or q->length)
	   * since in this context it appears that if r2 (or q) is non-null
	   * then r2->length (or q->length) has to be non-zero.
...
pt_print_host_var (PARSER_CONTEXT * parser, PT_NODE * p)
{
  ...
  if (!parser->dont_prt)
      ...
      if (parser->print_db_value)
	{
      ...
      /* In case of internal print, print a host variable with index */
      if (parser->custom_print & PT_INTERNAL_PRINT)
	{
	  char s[PT_MEMB_BUF_SIZE];
      ...
      if (parser->print_db_value)
	{
	  if (p->info.name.meta_class == PT_PARAMETER
	      ...
	      if (parser->custom_print & PT_DYNAMIC_SQL)
		{
		   ...
		   * for example 1:
		   * 'select foo into :p from foo; select :p.i.z from foo;'
		   * because 'i', 'z' as well as 'p' is parameter,
		   * error occurs while printing value

		   * for example 2:
		   * enclosed parameter variable ('select {p} from ...')
		   */
