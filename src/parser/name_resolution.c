...
pt_bind_names (PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
  ...
  *continue_walk = PT_CONTINUE_WALK;
  ...
  switch (node->node_type)
	...
	else if (node->info.dot.arg1->node_type == PT_NAME
		 && node->info.dot.arg2->node_type == PT_FUNCTION
		 && node->info.dot.arg2->info.function.function_type == PT_GENERIC
		 && strchr (node->info.dot.arg2->info.function.generic_name, '.') == NULL)
	  {
