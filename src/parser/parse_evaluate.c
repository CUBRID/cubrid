...
/* DB_IS_DEFAULT_EVAL_ONLY_SERVER check DEFAULT function
 * PT_IS_EXPR_OP_EVAL_ONLY_SERVER check EXPR
 */
#define PT_IS_EXPR_OP_EVAL_ONLY_SERVER(pt_op) \
  (((pt_op == PT_UUID) \
       || (pt_op == PT_SYS_GUID) \
       || (pt_op == PT_INDEX_CARDINALITY)))

typedef struct pt_server_expr_walk_info PT_SERVER_EXPR_WALK_INFO;
struct pt_server_expr_walk_info
{
  bool found;
};

static PT_NODE *
pt_find_expr_eval_on_server_pre (PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
  PT_SERVER_EXPR_WALK_INFO *info = (PT_SERVER_EXPR_WALK_INFO *) arg;
...
bool
pt_has_expr_eval_on_server (PARSER_CONTEXT * parser, PT_NODE * expr)
{
  PT_SERVER_EXPR_WALK_INFO info = { false };
  ...
  (void) parser_walk_tree (parser, expr, pt_find_expr_eval_on_server_pre, &info, pt_continue_walk, NULL);
  return info.found;
}
