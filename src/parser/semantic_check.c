 ...
 *
 * pt_seman.c - semantic checking functions
 */
...
static int pt_vclass_compatible (PARSER_CONTEXT * parser, const PT_NODE * att,
				 const PT_NODE * qcol);
...
static int pt_collection_compatible (PARSER_CONTEXT * parser,
				     ...
				     const PT_NODE * col2,
				     bool view_definition_context);
static int pt_union_compatible (PARSER_CONTEXT * parser, PT_NODE * item1,
				PT_NODE * item2, bool view_definition_context,
				bool * is_object_type,
...
static int pt_check_vclass_attr_qspec_compatible (PARSER_CONTEXT * parser,
						  PT_NODE * attr,
						  PT_NODE * col);
...
pt_check_cast_op (PARSER_CONTEXT * parser, const PT_NODE * node)
{
  ...
  assert (node != NULL && node->node_type == PT_EXPR &&
	  node->info.expr.arg1 != NULL && node->info.expr.cast_type != NULL);
...
/*
 * pt_derive_attribute () - derive a new ATTR_DEF node from a query_spec column
...
pt_derive_attribute (PARSER_CONTEXT * parser, PT_NODE * c)
{
  ...
  if (!c || !(attr = parser_new_node (parser, PT_ATTR_DEF)))
    {
      return NULL;
    }
  ...
  if (c->alias_print)
    ...
    {
      if (c->node_type != PT_NAME)
	{
	  return NULL;
	}
      cname = parser_copy_tree (parser, c);
    }
  ...
  attr->type_enum = c->type_enum;
  if (c->data_type)
    {
      attr->data_type = parser_copy_tree_list (parser, c->data_type);
    }
...
pt_get_attributes (PARSER_CONTEXT * parser, const DB_OBJECT * c)
{
  ...
  while (attributes)
	...
	default:
      ...
      if (list)
	parser_append_node (i_attr, list);
      else
	list = i_attr;
...
{
  int count = 0;
  PT_NODE *parent, *parent_attrs, *inherited_attrs, *attr, *t_attr = NULL,
    *new_name, *resolv_class, *resolv_attr, *r, *i_attr, *name, *next_node;
  ...
  switch (stmt->node_type)
    ...
    case PT_CREATE_ENTITY:
      count = pt_length_of_list
	((*attrs = stmt->info.create_entity.attr_def_list));

      /* exclude class_attributes from count and from the attrs list */
      for (attr = *attrs; attr; attr = attr->next)
	{
      ...
      for (parent = stmt->info.create_entity.supclass_list;
	   parent != NULL; parent = parent->next)
	{
	  /* get this parent's attributes & append them to the list */
	  parent_attrs = pt_get_attributes (parser,
					    parent->info.name.db_object);
	  if (inherited_attrs)
	    parser_append_node (parent_attrs, inherited_attrs);
	  else
	    inherited_attrs = parent_attrs;
	}
       ...
       */
      for (r = stmt->info.create_entity.resolution_list; r; r = r->next)
	{
	  if (!(new_name = r->info.resolution.as_attr_mthd_name))
	    continue;

	  resolv_class = r->info.resolution.of_sup_class_name;
	  resolv_attr = r->info.resolution.attr_mthd_name;

	  for (i_attr = inherited_attrs;
	       i_attr; t_attr = i_attr, i_attr = i_attr->next)
	    {
	      name = i_attr->info.attr_def.attr_name;
	      if (!pt_streq (resolv_attr->info.name.original,
	  ...
	  if (r->info.resolution.as_attr_mthd_name == NULL)
	      ...
	      for (i_attr = inherited_attrs; i_attr; i_attr = i_attr->next)
		{
		  ...
		  if (pt_streq (resolv_attr->info.name.original,
				name->info.name.original))
		    {
		      t_attr = i_attr;	/* i_attr is a keeper.
					   so, advance t_attr. */
		    }
		  else
		    {
			...
			{
			  t_attr = i_attr;	/* i_attr is a keeper.
						   keep the user-specified
						   inherited attribute */
			  continue;
			}

		      /* delete inherited attribute that
		         conflicts with resolv_attr */
	   ...
	   attr; attr = attr->next)
	{
       ...
       */
      for (i_attr = inherited_attrs, next_node =
	   i_attr ? i_attr->next : NULL; i_attr != NULL;
	   i_attr = next_node, next_node = i_attr ? i_attr->next : NULL)
	{
	  for (r = i_attr->next; r != NULL; r = r->next)
		...
		{
		  t_attr = r;	/* r is a keeper. so, advance t_attr. */
		}
	      else
		{
		  if (r == i_attr->next)
      ...
      /* append non-inherited attributes to the inherited attributes */
      if (inherited_attrs)
	{
	  count += pt_length_of_list (inherited_attrs);	/* update attr count */
	  for (i_attr = inherited_attrs;
	       i_attr; t_attr = i_attr, i_attr = i_attr->next)
	    {
	      ;
 ...
 *   return:  1 if att is vclass compatible with qcol
 *   parser(in): the parser context
		      ...
		      const PT_NODE * qcol)
{
  ...
  for (specs = db_get_query_specs (vcls);
	...
	{
	  return 1;		/* att is vclass_compatible with qcol */
	}
    }
  return 0;			/* att is not vclass_compatible with qcol */
}
	  ...
	  if (dt1 && dt2)
	    {
		  ...
		  common_type == PT_TYPE_NCHAR || common_type == PT_TYPE_BIT)
		{
		       ...
		       SEMAN_COMPATIBLE_INFO * cinfo)
{
  ...
  if (pt_is_select (node))
    {
      if (!(attrs = pt_get_select_list (parser, node)))
	return NULL;
      ...
      for (att = attrs, i = 0; att; att = next_att, i++)
	{
	  ...
	  if ((cinfo[i].idx == i) &&
		 ...
		 cinfo[i].prec != att->data_type->info.data_type.precision) ||
 ...
 *   attrds(in): an attribute definition list or NULL
 *
 * Note :
...
pt_check_union_compatibility (PARSER_CONTEXT * parser, PT_NODE * node,
			      PT_NODE * attrds)
{
  PT_NODE *attrs1, *attrs2, *att1, *att2, *result = node, *attrd;
  int cnt1, cnt2, i, c;
  ...
  for (i = 0, att1 = attrs1, att2 = attrs2, attrd = attrds;
      ...
      if (attrd == NULL)
	{
	  ...
	  if (c == PT_UNION_INCOMP)
	      ...
	      if (cinfo == NULL)
		{
		  ...
		  if (!(cinfo = (SEMAN_COMPATIBLE_INFO *)
	      ...
	      if (pt_get_common_type_for_union
		  (parser, att1, att2, &cinfo[i], i, &need_cast) == NULL)
		{
		  result = NULL;
		}
	    }
      ...
      else
	{
	   ...
	   *    do strict union compatibility checks in all other cases.
	   */
	  ...
	  else if ((c = pt_union_compatible (parser, att1, att2, false,
					     &is_object_type,
					     has_nested_union_node)) ==
		   PT_UNION_INCOMP)
	    {
			   ...
			   PT_NODE * stmt)
{
  ...
  for (def = attr_defs;
      ...
      if (pt_is_set_type (def))
	{
	  ...
	  && def->data_type
	  && def->data_type->node_type ==
	  PT_DATA_TYPE
	  && (dtyp =
	      def->data_type->info.data_type.
	      entity) != NULL
...
pt_check_alter (PARSER_CONTEXT * parser, PT_NODE * alter)
{
  ...
  switch (code)
    ...
    case PT_RENAME_ATTR_MTHD:
	  ...
	  alter->info.alter.alter_clause.rename.element_type == PT_ATTRIBUTE)
	{
	  if (!strncmp
	      (alter->info.alter.alter_clause.rename.
	       old_name->info.name.original, keyattr,
...
pt_find_partition_column_count (PT_NODE * expr, PT_NODE ** name_node)
{
  ...
  if (expr->info.expr.arg1 != NULL)
	...
	{
	  ret =
	    pt_find_partition_column_count (expr->info.expr.arg1, name_node);
	  if (ret < 0)
	    return -1;
	  cnt += ret;
	}
  ...
  if (expr->info.expr.arg2 != NULL)
	...
	{			/* except default NULL parameter */
	  if (expr->info.expr.arg2->type_enum ==
	      PT_TYPE_NULL
	      && (expr->info.expr.arg2->line_number !=
		  expr->info.expr.arg3->line_number
		  || expr->info.expr.arg2->
		  column_number != expr->info.expr.arg3->column_number))
	    {
	      return -1;
	    }
	}
	...
	{
	  ret =
	    pt_find_partition_column_count (expr->info.expr.arg2, name_node);
	  if (ret < 0)
	    return -1;
	  cnt += ret;
	}
  ...
  if (expr->info.expr.arg3 != NULL)
	...
	{
	  ret =
	    pt_find_partition_column_count (expr->info.expr.arg3, name_node);
	  if (ret < 0)
	    return -1;
	  cnt += ret;
	}
		  ...
		  for (parts = pinfo->info.partition.parts;
		      ...
		      else
			{
			  for (val = parts->info.parts.values;
				...
				continue;
			      if (pt_coerce_value
				  (parser, val, val, expr_type->type_enum,
				   expr_type->data_type) != NO_ERROR)
				{
				  chkflag = 1;
				  break;
				}
	  ...
	  for (names = name_list, chkflag = 0; names; names = names->next)
		  ...
		  !intl_mbs_casecmp (names->info.name.original, part_name))
		{
		  ...
		  names->info.name.db_object =
		    (cmd ==
		     PT_ANALYZE_PARTITION) ? objs->op : subcls->partition_of;
  ...
  if (part_list)
      ...
      for (parts = part_list; parts; parts = parts->next)
	{
	  ...
	  if (ptype.data.i == PT_PARTITION_RANGE)
		...
		{
		  if (pt_coerce_value
		      (parser, parts->info.parts.values,
      ...
      if (ptype.data.i == PT_PARTITION_RANGE)
	{
	  if (cmd == PT_ADD_PARTITION)
		   ...
		   db_value_compare (maxval, partmin) != DB_LT))
		{
	      ...
	      stmt->info.alter.alter_clause.partition.
		info = pt_dbval_to_value (parser, maxval);
	    }
		      ...
		      && db_value_compare (maxval, partmax) == DB_GT))
		{
	      ...
	      for (min_list = minmax_head,
		   max_list =
		   (min_list) ? min_list->next : NULL;
		   (min_list && max_list);
		   min_list = max_list->next, max_list =
		   (min_list) ? min_list->next : NULL)
		{
		  ...
		  if ((db_value_compare (minval, min_list->val)
		       != DB_GT
		       && db_value_compare (min_list->val,
					    partmax) == DB_LT)
		      ||
		      (db_value_compare (minval, max_list->val)
		       == DB_LT
		       && db_value_compare (max_list->val,
					    partmax) != DB_GT)
		      ||
		      (db_value_compare (min_list->val, minval)
		       != DB_GT
...
static PT_NODE *
pt_check_vclass_query_spec (PARSER_CONTEXT *
			    parser,
			    PT_NODE * qry, PT_NODE * attrs, const char *self)
{
  ...
  info.attrdefs = attrs;
  info.donot_fold = false;
  qry = pt_check_with_info (parser, qry, &info);
  ...
  while (attr && col)
      ...
      if (attr->info.attr_def.attr_type == PT_META_ATTR)
	{
      ...
      /* if attribute has no data type then derive its
       * data type from its matching query_spec column. */
      if (attr->type_enum == PT_TYPE_NONE)
	{
	  /* check minus precision(i.e., TP_FLOATING_PRECISION_VALUE)
	   * for char/bit constants */
	  fixed_precision = 0;
	  ...
	  /* convert char(max)  to varchar(max),
	...
	if (pt_check_vclass_attr_qspec_compatible
	    (parser, attr, col) != PT_UNION_COMP)
	{
	  PT_ERRORmf2 (parser, col,
		       MSGCAT_SET_PARSER_SEMANTIC,
		       MSGCAT_SEMANTIC_ATT_INCOMPATIBLE_COL,
...
/*
 * pt_type_cast_vclass_query_spec_column () - do semantic checks on a vclass
 *                                            query spec
...
static PT_NODE *pt_type_cast_vclass_query_spec
  (PARSER_CONTEXT * parser, PT_NODE * qry, PT_NODE * attrs)
{
  PT_NODE *columns, *col, *attr;
      ...
      if (new_col != col)
	{
	  if (prev_col == NULL)
	      ...
	      if (qry->data_type)
		{
	      ...
	      qry->data_type =
		parser_copy_tree_list (parser, new_col->data_type);
	    }
	  else
	    {
 ...
 * - else if an attribute has no data type then derive it from its
 *   matching query_spec column
 * - else corresponding attribute and query_spec column must match type-wise
...
pt_check_create_view (PARSER_CONTEXT * parser, PT_NODE * stmt)
{
  ...
  if (!stmt
      ...
      || (name = stmt->info.create_entity.entity_name) == NULL)
    return;
  qry_specs = stmt->info.create_entity.as_query_list;
  qry_ptr = &stmt->info.create_entity.as_query_list;
  if (!qry_specs || parser->error_msgs)
    return;
  /* We need to check that no query has an into list or order by
   * clause.  This was being done in pt_check_vclass_query_spec, but
   * we need to check it whether we have an attr list or not.
   * If this is an error, there is no need to go on. */
  for (qry1 = qry_specs; qry1 != NULL; qry1 = qry1->next)
    {
      /* qry must have no into clause or order by clause */
      if (qry1->info.query.into_list)
	{
	  PT_ERRORm (parser, qry1,
		     MSGCAT_SET_PARSER_SEMANTIC,
		     MSGCAT_SEMANTIC_WANT_NO_INTO_CLAUSE);
	  return;
	}

      if (qry1->info.query.order_by)
	{
	  PT_ERRORm (parser, qry1,
		     MSGCAT_SET_PARSER_SEMANTIC,
		     MSGCAT_SEMANTIC_WANT_NO_ORDERBY_CLAUSE);
	  return;
	}

      /* we cannot call pt_semantic_check here because pt_semantic_check needs
       * the view's attr defs which may not yet exist at this point.  we cannot
       * defer calling pt_resolve_names because the immediately following code
       * to derive the view's attr defs from the query_specs assumes any
       * 'SELECT *' in the query_specs have been resolved.
       */
      sc_info.attrdefs = NULL;
      sc_info.top_node = qry1;
      sc_info.donot_fold = false;
      pt_resolve_names (parser, qry1, &sc_info);
      if (parser->error_msgs)
	{
	  if (qry1->node_type == (PT_NODE_TYPE) - 1)
	    {
	      stmt->info.create_entity.as_query_list = NULL;
	    }
	  return;
	}
    }
  ...
  qspec_attr = pt_get_select_list (parser, qry_specs);
  col_count = pt_length_of_select_list (qspec_attr, EXCLUDE_HIDDEN_COLUMNS);
  if (attr_count <= 0)
    {
      /* semantic check any query specs */
      for (qry1 = qry_specs; qry1 != NULL; qry1 = qry1->next)
	{
	  parser_walk_tree (parser, qry1,
			    pt_set_is_view_spec, NULL, NULL, NULL);
	  pt_check_with_info (parser, qry1, NULL);
	}

      for (r = qspec_attr; r; r = r->next)
	{
	  if (r->alias_print)
	    {
	      s_attr = r;
	    ...
	    {
	      /* allow attributes to be derived
	         only from path expressions. */
	      if (r->node_type == PT_NAME)
		{
		  s_attr = r;
		}
	      else
		if (r->node_type == PT_DOT_
		    && r->info.dot.arg2
		    && r->info.dot.arg2->node_type == PT_NAME)
		{
		  s_attr = r->info.dot.arg2;
		}
	      else
		{
		  s_attr = NULL;
	  ...
	  if (!s_attr)
	  ...
	  else
	    {
	      ...
	      if (stmt->info.create_entity.attr_def_list)
		{
		...
		{
		  stmt->info.create_entity.attr_def_list = derived_attr;
		}
	    }
	}
    }
  else
    {
      /* do other checks on query specs
         qry_ptr points to the next qry_spec address,
         when qry1 points to next */
      for (qry1 = qry_specs; qry1 != NULL;
	   qry_ptr = &qry1->next, qry1 = qry1->next)
	{
	  parser_walk_tree (parser, qry1,
			    pt_set_is_view_spec, NULL, NULL, NULL);
	  *qry_ptr =
	    pt_check_vclass_query_spec (parser, qry1,
					all_attrs, name->info.name.original);
	  if (*qry_ptr && !pt_has_error (parser))
	    {
	      pt_type_cast_vclass_query_spec (parser, qry1, all_attrs);
	    }
	}
    }
}
...
pt_check_create_entity (PARSER_CONTEXT * parser, PT_NODE * node)
{
  ...
  switch (entity_type)
	   ...
	   parent != NULL; parent = parent->next)
	{
      ...
      /* an INHERIT_clause in a create_[ldbv|v]class_stmt
...
pt_check_drop (PARSER_CONTEXT * parser, PT_NODE * node)
{
  ...
  if (node->info.drop.entity_type != PT_MISC_DUMMY)
	...
	{
	  if ((name =
	       temp->info.spec.entity_name) != NULL
	      ...
	      if (db_attribute_type (attributes) == DB_TYPE_OBJECT)
		{
		  if (sm_has_text_domain (attributes, 0))
		      ...
		      if (drop_name_list != NULL)
			{
		      ...
		      drop_name_list =
			pt_append_string (parser, drop_name_list,
					  db_get_class_name (domain_class));
		    }
		}
	      attributes = db_attribute_next (attributes);
	    }
  ...
  if (drop_name_list)
    {
      if ((node->info.drop.internal_stmts =
	   pt_append_statements_on_drop_attributes
	   (parser, node->info.drop.internal_stmts, drop_name_list)) == NULL)
	{
  ...
  if (method == NULL)
      ...
      else
	{
	  ...
	  if (((target->node_type != PT_NAME) ||
	       (target->info.name.meta_class !=
		PT_PARAMETER)
	       || !pt_eval_path_expr (parser, target,
				      &val)
	       ||
	       !db_is_instance (DB_GET_OBJECT (&val)))
	      && target->node_type != PT_VALUE
	      && target->data_type->info.data_type.
	      entity->info.name.meta_class != PT_META_CLASS)
	    {
	      PT_ERRORm (parser, node,
			 MSGCAT_SET_PARSER_SEMANTIC,
  ...
  else
    {
	  ...
	  && target->node_type != PT_VALUE
	  && target->data_type->info.data_type.
	  entity->info.name.meta_class != PT_CLASS)
	{
	  PT_ERRORm (parser, node,
		     MSGCAT_SET_PARSER_SEMANTIC,
 ...
 */
static PT_NODE *
pt_check_single_valued_node (PARSER_CONTEXT *
			     parser,
	  ...
	  *continue_walk = PT_LIST_WALK;
	  if (pt_find_spec (parser, info->from, node)
	      && pt_find_attribute (parser, node, info->group_by) < 0)
	    {
		  ...
		  && node->info.name.meta_class !=
		  PT_METHOD
		  && node->info.name.meta_class !=
		  PT_META_ATTR
		  && node->info.name.meta_class !=
		  PT_META_CLASS && node->info.name.meta_class != PT_PARAMETER)
		{
		  PT_ERRORmf (parser, node,
			      MSGCAT_SET_PARSER_SEMANTIC,
...
static PT_NODE *pt_check_single_valued_node_post
  (PARSER_CONTEXT * parser, PT_NODE * node, void *arg, int *continue_walk)
{
  PT_AGG_INFO *info = (PT_AGG_INFO *) arg;
  *continue_walk = PT_CONTINUE_WALK;
...
pt_check_into_clause (PARSER_CONTEXT * parser, PT_NODE * qry)
{
  ...
  col_cnt =
    pt_length_of_select_list (pt_get_select_list
			      (parser, qry), EXCLUDE_HIDDEN_COLUMNS);
  if (tgt_cnt != col_cnt)
    {
 ...
 */
static PT_NODE *
pt_semantic_check_local (PARSER_CONTEXT *
			 parser,
			 PT_NODE * node, void *arg, int *continue_walk)
{
  ...
  switch (node->node_type)
    ...
    case PT_FUNCTION:
	  ...
	  PT_GENERIC
	  && pt_length_of_list (node->info.
				function.arg_list) > NUM_F_GENERIC_ARGS)
	{
	  PT_ERRORmf2 (parser, node,
		       MSGCAT_SET_PARSER_SEMANTIC,
	...
	{
	  if (pt_length_of_select_list
	      (node->info.query.q.select.list, EXCLUDE_HIDDEN_COLUMNS) != 1)
	    {
	...
	if (node->info.spec.derived_table_type ==
	    PT_IS_SUBQUERY && (derived_table = node->info.spec.derived_table))
	  {
		...
		if (col_cnt - attr_cnt > 0)
		  {
		    ...
		    for (i = attr_cnt, j = attr_cnt; i < col_cnt; i++)
		      {
			t_node = pt_name
			  (parser, mq_generate_name (parser, "ha", &j));
		     ...
		     PT_NODE * pred, PT_CHAIN_INFO * chain)
{
  ...
  for (i = 0; i < chain->chain_length - 1; i++)
    {
	  ...
	  (!conj->info.expr.arg1) ||
	  (!conj->info.expr.arg2) ||
	  (conj->info.expr.arg1->node_type !=
	   PT_NAME)
	  || (conj->info.expr.arg2->node_type !=
	      PT_NAME)
      ...
      if (arg1 == NULL)
	{
	   ...
	   */
	  if ((arg1 =
	       parser_copy_tree (parser, conj->info.expr.arg1)) == NULL)
	    goto out_of_mem;
	}
      else
	{
	  ...
	  if ((new_path->info.expr.arg1 =
	       parser_copy_tree (parser, arg1)) == NULL)
	    goto out_of_mem;
	  ...
	  if (arg2->data_type
	      &&
	      ((new_path->data_type =
		parser_copy_tree_list (parser, arg2->data_type)) == NULL))
	    goto out_of_mem;
  ...
  /* Finally link in the original predicate with the new predicate and
   * parenthesize to gather the disjuncts together.  It is necessary to
   * parenthesize since we are adding disjuncts to an arbitrarily complex
   * search condition.
   * Note: this technique relies on the fact that this expression is
   * an operator conjuct tree rather than the linked list conjunct tree.
   * Since this is done before CNF transformation, we are guaranteed this.
   * If this assumption changes, we'll need to worry about pred->next
   * here.
   */
  if ((new_expr = parser_new_node (parser, PT_EXPR)) == NULL)
    goto out_of_mem;
 ...
 */
static PT_NODE *
pt_path_chain (PARSER_CONTEXT * parser,
	       PT_NODE * node, void *arg, int *continue_walk)
{
	      ...
	      if (tmp == chain->chain_ptr[chain->chain_length - 1])
		{
		  ...
		  if (chain->chain_length == chain->chain_size)
		      ...
		      if (chain->chain_ptr == chain->chain)
			{
			  ...
			  if ((chain->chain_ptr = (PT_NODE **)
			       malloc (chain->chain_size *
				       sizeof (PT_NODE *))) == NULL)
			    {
			      goto out_of_mem;
			    }
		      ...
		      else
			{
			  PT_NODE **tmp;
			  if ((tmp =
			       (PT_NODE **) realloc (chain->chain_ptr,
						     chain->
						     chain_size *
						     sizeof (PT_NODE
							     *))) == NULL)
 ...
 */
static PT_NODE *
pt_expand_isnull_preds_helper (PARSER_CONTEXT
			       * parser,
			       PT_NODE * node, void *arg, int *continue_walk)
{
  ...
  if (node->node_type == PT_EXPR &&
    ...
    {
      /* This is the case we're after.  We've got an "IS NULL" path expr.
       * WARNING!!!
       * This is not good enough to capture implicit path expressions which
       * are indicated by path correlation variables.  We could check every
       * PT_NAME node here, but we still don't have enough machinery to tie
       * the correlation spec which the PT_NAME node resolves to to the
       * correct path entity spec.  Resolution of correlation specs will
       * need to change to point to the correct path entity spec and then
       * this conditional will need to be opened up.
       *
       * We need to find the spec where the final segment of the path
       * expression resolves.  This MUST be in a path_entities chain
       * off of some spec.  Once we find it, for each segment of the
       * path entities chain, we must construct an is_null disjunct.
       *
       * We can construct the chain by doing a clever bottom up search for
       * the spec in question and continue walking up the chain squirreling
       * away the node addresses as we climb.
       */
 ...
 */
static PT_NODE *
pt_expand_isnull_preds (PARSER_CONTEXT *
			parser, PT_NODE * node, void *arg, int *continue_walk)
{
  ...
  if (pred)
    {
      *pred =
	parser_walk_tree (parser, *pred, NULL, NULL,
 ...
 */
static PT_NODE *
pt_check_and_replace_hostvar (PARSER_CONTEXT
			      * parser,
			      PT_NODE * node, void *arg, int *continue_walk)
{
  PT_NODE *value;
  ...
  if ((pt_is_input_hostvar (node) &&
       /* only when host var is given before to compile */
		    ...
		    PT_NODE * node, SEMANTIC_CHK_INFO * info)
{
  ...
  switch (node->node_type)
    ...
    case PT_SAVEPOINT:
	  ...
	  (node->info.savepoint.save_name->info.
	   name.meta_class == PT_PARAMETER))
	{
	  node = pt_resolve_names (parser, node, sc_info_ptr);
	}
      break;
    case PT_ROLLBACK_WORK:
	  ...
	  (node->info.rollback_work.save_name->
	   info.name.meta_class == PT_PARAMETER))
	{
	  node = pt_resolve_names (parser, node, sc_info_ptr);
	}
      break;
		      ...
		      PT_NODE * att, PT_NODE_TYPE att_type)
{
  ...
  for (p = att; p; p = p->next)
      ...
      for (q = p->next; q; q = q->next)
	{
	  ...
	  if (!pt_streq
	      (p_nam->info.name.original, q_nam->info.name.original))
	    {
 ...
 */
static int
pt_assignment_compatible (PARSER_CONTEXT *
			  parser, PT_NODE * lhs, PT_NODE * rhs)
{
  int rc;
  ...
  if (lhs->type_enum == PT_TYPE_OBJECT)
    {
	  ...
	  (rhs->type_enum == PT_TYPE_NONE || rhs->type_enum == PT_TYPE_MAYBE))
	{
  ...
  for (a = stmt->info.update.assignment; a; a = a->next)
    {
      if (!
	  (a->node_type == PT_EXPR
	   && a->info.expr.op == PT_ASSIGN && (att_a = a->info.expr.arg1)))
	{
	  goto exit_on_error;
	}
	...
	case PT_PLUS:
	  ...
	  if (pt_find_var (name_expr->info.expr.arg1, 0))
	      ...
	      else
		{
		  ...
		  tmp->info.expr.arg1 = result;
		  tmp->info.expr.arg2 =
		    parser_copy_tree (parser, name_expr->info.expr.arg2);
		  if (tmp->info.expr.arg1->node_type == PT_EXPR)
		    tmp->info.expr.arg1->info.expr.paren_type = 1;
	  ...
	  if (pt_find_var (name_expr->info.expr.arg2, 0))
	      ...
	      else
		{
		  ...
		  tmp->info.expr.arg1 = result;
		  tmp->info.expr.arg2 =
		    parser_copy_tree (parser, name_expr->info.expr.arg1);
		  if (tmp->info.expr.arg1->node_type == PT_EXPR)
		    tmp->info.expr.arg1->info.expr.paren_type = 1;
		  if (tmp->info.expr.arg2->node_type == PT_EXPR)
		    tmp->info.expr.arg2->info.expr.paren_type = 1;
  ...
  if (p->node_type == PT_EXPR)
    {
      return (pt_find_var
	      (p->info.expr.arg1,
	       result) + pt_find_var (p->info.expr.arg2, result));
    }
...
PT_NODE *
pt_find_order_value_in_list (PARSER_CONTEXT *
			     parser,
			     const PT_NODE *
			     sort_value, const PT_NODE * order_list)
{
  PT_NODE *match = NULL;
  match = (PT_NODE *) order_list;
  while (match
	 && sort_value
	 && match->info.sort_spec.expr
	 && match->info.sort_spec.expr->
	 node_type == PT_VALUE
...
pt_check_order_by (PARSER_CONTEXT * parser, PT_NODE * query)
{
  ...
  /* get ORDER BY clause */
  if (!(order_by = query->info.query.order_by))
    {
  ...
  for (order = order_by; order; order = order->next)
	  ...
	  if (!pt_is_query (query))
	    {
	      ...
	      PT_ERRORm (parser, order, MSGCAT_SET_PARSER_SEMANTIC, error);
	    }
      ...
      while (col->node_type == PT_DOT_)
	{
	  ...
	  && (spec =
	      pt_find_spec (parser, from, col))
	  && (entity_name =
	      spec->info.spec.entity_name)
	  && (cls_name = entity_name->info.name.original))
	{
	  /* get class mop */
	  if ((cls_obj = entity_name->info.name.db_object) == NULL)
	    {
	  ...
	  if (cls_obj && attr && db_attribute_is_non_null (attr))
		  ...
		  db_attribute_is_indexed (attr))
		{
		  if ((dp = db_attribute_domain (attr)) &&
		      ...
		      NO_ERROR
		      && (llim =
			  pt_dbval_to_value (parser, &value))
		      && (temp =
			  parser_copy_tree (parser, col))
		      && (expr =
  ...
  for (order = order_by; order; order = order->next)
    {
      while ((match = pt_find_order_value_in_list
	      (parser, order->info.sort_spec.expr, order->next)))
	{
	  ...
	  if (!order_by)
	    {
	      ...
	      PT_ERRORmf (parser, col,
			  MSGCAT_SET_PARSER_SEMANTIC, error, "ORDERBY_NUM()");
	      break;
	    }
	}
      else
...
pt_check_path_eq (PARSER_CONTEXT * parser, PT_NODE * p, PT_NODE * q)
{
  ...
  switch (n)
	...
	return 1;
      if (pt_streq
	  (p->info.dot.arg2->info.name.original,
...
/*
 * pt_coerce_insert_values () - try to coerce insert_values to types
 *  	                        indicated by insert_attributes
 *   return:
 *   parser(in): handle to context used to parse the insert statement
...
pt_coerce_insert_values (PARSER_CONTEXT * parser, PT_NODE * ins)
{
...
#endif /* 0                              */
  a = ins->info.insert.attr_list;
  a_cnt = pt_length_of_list (a);
  if (ins->info.insert.is_value == PT_IS_DEFAULT_VALUE)
    {
      v = NULL;			/* PT_IS_DEFAULT_VALUE */
    }
  else if (ins->info.insert.is_value == PT_IS_SUBQUERY)
    {
      /* this sort of nods at union queries */
      v = pt_get_select_list (parser, ins->info.insert.value_clause);
      v_cnt = pt_length_of_select_list (v, EXCLUDE_HIDDEN_COLUMNS);
      if (a_cnt != v_cnt)
	{
	  PT_ERRORmf2 (parser, ins,
		       MSGCAT_SET_PARSER_SEMANTIC,
    ...
    {
      v = ins->info.insert.value_clause;
  ...
  /* for each value node v in ins' value_clause and for each attrivute
   * a in ins' attribute_list do */
  if (v)
    {
      for (; v != NULL && a != NULL; v = v->next, a = a->next)
	{
...
PT_NODE *
pt_count_input_markers (PARSER_CONTEXT *
			parser, PT_NODE * node, void *arg, int *continue_walk)
{
  int *num_markers;
  num_markers = (int *) arg;
 ...
 */
PT_NODE *
pt_count_output_markers (PARSER_CONTEXT *
			 parser,
			 PT_NODE * node, void *arg, int *continue_walk)
{
  int *num_markers;
 ...
 */
PT_NODE *
pt_has_using_index_clause (PARSER_CONTEXT *
			   parser,
			   PT_NODE * node, void *arg, int *continue_walk)
{
  bool *found = (bool *) arg;
...
/*
 * pt_validate_query_spec () - check if a query_spec is compatible with a given
 * 			       {ldbvclass|vclass} object
 ...
 *   c(in): an ldbvclass or vclass object
 */
...
pt_validate_query_spec (PARSER_CONTEXT * parser, PT_NODE * s, DB_OBJECT * c)
{
  ...
  if (pt_has_error (parser))	/* a syntax error for query_spec */
    {
      pt_report_to_ersys (parser, PT_SYNTAX);	/* report it */
      return er_errid ();
    }
  ...
  if (db_is_vclass (c))
    {
      /* apply semantic checks to query_spec */
      parser_walk_tree (parser, s, pt_set_is_view_spec, NULL, NULL, NULL);
      pt_check_vclass_query_spec (parser, s, attrs, db_get_class_name (c));

      if (!pt_has_error (parser))
	{
	  pt_type_cast_vclass_query_spec (parser, s, attrs);
	}

      if (parser->error_msgs)	/* query_spec s fails semantic checks  */
	{
...
warning:
  PT_WARNINGmf (parser, constraint,
		MSGCAT_SET_PARSER_SEMANTIC,
...
/*
 * pt_find_class_of_index () - ind class name of the given index
 *   return:  the PT_NAME node
 *   parser(in): the current parser context
 *   index(in): a PT_NAME node
 *   type(in): constraint type (DB_CONSTRAINT_UNIQUE or DB_CONSTRAINT_INDEX)
 */

PT_NODE *
pt_find_class_of_index (PARSER_CONTEXT * parser, PT_NODE * index,
			DB_CONSTRAINT_TYPE type)
{
  DB_OBJECT *class_;
  PT_NODE *node = NULL;

  class_ = (DB_OBJECT *) db_find_class_of_index (index->info.name.original,
						 type);
  if (class_)
    {
      node = parser_new_node (parser, PT_NAME);
