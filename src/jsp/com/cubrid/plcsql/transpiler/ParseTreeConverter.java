...
import com.cubrid.plcsql.transpiler.antlrgen.PcsParserBaseVisitor;
...
public class ParseTreeConverter extends PcsParserBaseVisitor<AstNode> {
    ...
    visitCreate_procedure(Create_procedureContext ctx) {
    ...
    @Override public Unit
    visitCreate_function(Create_functionContext ctx) {
    ...
    visitParameter_list(Parameter_listContext ctx) {
    ...
    @Override public DeclParamIn
    visitParameter_in(Parameter_inContext ctx) {
    ...
    @Override public DeclParamOut
    visitParameter_out(Parameter_outContext ctx) {
        String name = ctx.parameter_name().getText().toUpperCase();
        name = Misc.peelId(name);
        TypeSpec typeSpec = (TypeSpec) visit(ctx.type_spec());
    ...
    @Override public TypeSpec
    visitType_spec(Type_specContext ctx) {
    ...
    visitDefault_value_part(Default_value_partContext ctx) {
        if (ctx == null) { return null; }
        return visitExpression(ctx.expression(), null);
    }

    @Override public I_Expr
    visitAnd_exp(And_expContext ctx) {
    ...
    @Override public I_Expr
    visitXor_exp(Xor_expContext ctx) {
        I_Expr l = visitExpression(ctx.expression(0), "Boolean");
        I_Expr r = visitExpression(ctx.expression(1), "Boolean");
    ...
    @Override public I_Expr
    visitIs_null_exp(Is_null_expContext ctx) {
    ...
    @Override public I_Expr
    visitBetween_exp(Between_expContext ctx) {
        I_Expr target = visitExpression(ctx.between_expression(), "Integer");  // TODO
        I_Expr lowerBound = visitExpression(ctx.between_elements().between_expression(0), "Integer");  // TODO
        I_Expr upperBound = visitExpression(ctx.between_elements().between_expression(1), "Integer");  // TODO
    ...
    @Override public NodeList<I_Expr>
    visitIn_elements(In_elementsContext ctx) {
    ...
    @Override public I_Expr
    visitLike_exp(Like_expContext ctx) {
    ...
    @Override public I_Expr
    visitAdd_exp(Add_expContext ctx) {
        String opStr = ctx.PLUS_SIGN() != null ? "Add" :
                       ctx.MINUS_SIGN() != null ? "Subtract" :
    ...
    @Override public I_Expr
    visitPower_exp(Power_expContext ctx) {
    ...
    @Override public I_Expr
    visitNeg_exp(Neg_expContext ctx) {
        I_Expr o = visitExpression(ctx.unary_expression(), "Integer");

        I_Expr ret = ctx.PLUS_SIGN() != null ? o :
    ...
    private static final DateFormat dbgFormat = new SimpleDateFormat("G yyyy-MM-dd HH:mm:ss.SSS XXX", Locale.US);
    static {
        dbgFormat.setTimeZone(TimeZone.getTimeZone("GMT+0"));
    }

    @Override public I_Expr
    visitDate_exp(Date_expContext ctx) {
    ...
    @Override public I_Expr
    visitTime_exp(Time_expContext ctx) {
        String s = ctx.quoted_string().getText();
        s = quotedStrToJavaStr(s);
    ...
    @Override public I_Expr
    visitTimestamp_exp(Timestamp_expContext ctx) {
        String s = ctx.quoted_string().getText();
        s = quotedStrToJavaStr(s);
    ...
    @Override public I_Expr
    visitDatetime_exp(Datetime_expContext ctx) {
        String s = ctx.quoted_string().getText();
        s = quotedStrToJavaStr(s);
    ...
    @Override public I_Expr
    visitTimestamptz_exp(Timestamptz_expContext ctx) {
        String s = ctx.quoted_string().getText();
        s = quotedStrToJavaStr(s);
    ...
    @Override public I_Expr
    visitTimestampltz_exp(Timestampltz_expContext ctx) {
        String s = ctx.quoted_string().getText();
        s = quotedStrToJavaStr(s);
    ...
    @Override public I_Expr
    visitDatetimetz_exp(Datetimetz_expContext ctx) {
        String s = ctx.quoted_string().getText();
        s = quotedStrToJavaStr(s);
    ...
    @Override public I_Expr
    visitDatetimeltz_exp(Datetimeltz_expContext ctx) {
        String s = ctx.quoted_string().getText();
        s = quotedStrToJavaStr(s);
    ...
    @Override public I_Expr
    visitUint_exp(Uint_expContext ctx) {
        try {
    ...
    @Override public I_Expr
    visitFp_num_exp(Fp_num_expContext ctx) {
        try {
    ...
    @Override public I_Expr
    visitStr_exp(Str_expContext ctx) {
        String val = ctx.quoted_string().getText();
        return new ExprStr(quotedStrToJavaStr(val));
    }

    @Override public I_Expr
    visitNull_exp(Null_expContext ctx) {
        return ExprNull.instance();
    }

    @Override public I_Expr
    visitTrue_exp(True_expContext ctx) {
        return ExprTrue.instance();
    }

    @Override public I_Expr
    visitFalse_exp(False_expContext ctx) {
        return ExprFalse.instance();
    }

    @Override public I_Expr
    visitField_exp(Field_expContext ctx) {
        ...
        if (record.decl == null) {
            ...
            assert record.decl instanceof DeclForRecord:
                "field lookup is only allowed for a record, but "  + ctx.record.getText() + " is not a record";
    ...
    @Override public I_Expr
    visitFunction_call(Function_callContext ctx) {
            ...
            if (args != null && args.nodes.size() > 0) {
                ...
                for (I_Expr arg: args.nodes) {
                    ...
                    if (dp instanceof DeclParamOut) {
                        ...
                        assert valid: "argument " + i + " to the function " + name + " must be a variable or " +
    ...
    @Override public I_Expr
    visitSearched_case_expression(Searched_case_expressionContext ctx) {

        NodeList<CondExpr> condParts = new NodeList<>();
        for (Searched_case_expression_when_partContext c: ctx.searched_case_expression_when_part()) {
    ...
    @Override public AstNode
    visitCursor_attr_exp(Cursor_attr_expContext ctx) {
    ...
    @Override public ExprSqlRowCount
    visitSql_rowcount_exp(Sql_rowcount_expContext ctx) {
        return new ExprSqlRowCount();
    }

    @Override public I_Expr
    visitParen_exp(Paren_expContext ctx) {
        return visitExpression(ctx.expression(), null);
    }

    @Override public I_Expr
    visitList_exp(List_expContext ctx) {
        NodeList<I_Expr> elems = visitExpressions(ctx.expressions());
        return new ExprList(elems);
    }

    @Override public NodeList<I_Decl>
    visitSeq_of_declare_specs(Seq_of_declare_specsContext ctx) {
    ...
    @Override public AstNode
    visitPragma_declaration(Pragma_declarationContext ctx) {
    ...
    @Override public AstNode
    visitConstant_declaration(Constant_declarationContext ctx) {
    ...
    @Override public AstNode
    visitException_declaration(Exception_declarationContext ctx) {
    ...
    @Override public AstNode
    visitVariable_declaration(Variable_declarationContext ctx) {
    ...
    @Override public AstNode
    visitCursor_definition(Cursor_definitionContext ctx) {
    ...
    private void
    previsitProcedure_body(Procedure_bodyContext ctx) {
    ...
    @Override public AstNode
    visitProcedure_body(Procedure_bodyContext ctx) {
    ...
    private void
    previsitFunction_body(Function_bodyContext ctx) {
    ...
    @Override public AstNode
    visitFunction_body(Function_bodyContext ctx) {
    ...
    @Override public Body
    visitBody(BodyContext ctx) {
    ...
    @Override public NodeList<I_Stmt>
    visitSeq_of_statements(Seq_of_statementsContext ctx) {
    ...
    @Override public StmtAssign
    visitAssignment_statement(Assignment_statementContext ctx) {
        ...
        assert target.decl instanceof DeclVar || target.decl instanceof DeclParamOut:
            target.decl.typeStr() + " " + target.name + " cannot be used as a target of an assignment";
    ...
    private ExprId
    visitIdentifierInner(IdentifierContext ctx) {
    ...
    @Override public AstNode
    visitContinue_statement(Continue_statementContext ctx) {
    ...
    @Override public AstNode
    visitExit_statement(Exit_statementContext ctx) {
    ...
    @Override public StmtIf
    visitIf_statement(If_statementContext ctx) {
    ...
    @Override public DeclLabel
    visitLabel_declaration(Label_declarationContext ctx) {
    ...
    @Override public I_Expr
    visitLower_bound(Lower_boundContext ctx) {
        return visitExpression(ctx.concatenation(), "Integer");
    }

    @Override public I_Expr
    visitUpper_bound(Upper_boundContext ctx) {
        return visitExpression(ctx.concatenation(), "Integer");
    }

    @Override public I_Expr
    visitStep(StepContext ctx) {
        if (ctx == null) { return null; }
    ...
    @Override public AstNode
    visitStmt_for_cursor_loop(Stmt_for_cursor_loopContext ctx) {
        ...
        assert (cursorDecl.paramList != null && cursorDecl.paramList.nodes.size() > 0) ==
               (args != null && args.nodes.size() > 0) :
            "the number of arguments to cursor " + cursorName +
                " does not match the number of its declared formal parameters";
    ...
    @Override public StmtForSqlLoop
    visitStmt_for_static_sql_loop(Stmt_for_static_sql_loopContext ctx) {
    ...
    @Override public StmtForSqlLoop
    visitStmt_for_dynamic_sql_loop(Stmt_for_dynamic_sql_loopContext ctx) {
        ...
        Restricted_using_clauseContext usingClause = ctx.for_dynamic_sql().restricted_using_clause();
        if (usingClause == null) {
            usedExprList = null;
        } else {
    ...
    @Override public StmtNull
    visitNull_statement(Null_statementContext ctx) {
        return new StmtNull();
    }

    @Override public StmtRaise
    visitRaise_statement(Raise_statementContext ctx) {
        ExName exName = visitException_name(ctx.exception_name());
        return new StmtRaise(exName);
    }

    @Override public ExName
    visitException_name(Exception_nameContext ctx) {
    ...
    @Override public StmtReturn
    visitReturn_statement(Return_statementContext ctx) {
    ...
    @Override public StmtIf
    visitSearched_case_statement(Searched_case_statementContext ctx) {
    ...
    @Override public StmtRaiseAppErr
    visitRaise_application_error_statement(Raise_application_error_statementContext ctx) {
    ...
    @Override public StmtExecImme
    visitData_manipulation_language_statements(Data_manipulation_language_statementsContext ctx) {
    ...
    @Override public AstNode
    visitClose_statement(Close_statementContext ctx) {
    ...
    visitOpen_statement(Open_statementContext ctx) {
        ...
        assert (decl.paramList != null && decl.paramList.nodes.size() > 0) ==
               (args != null && args.nodes.size() > 0) :
            "the number of arguments to cursor " + name +
                " does not match the number of its declared formal parameters";
    ...
    @Override public NodeList<I_Expr>
    visitExpressions(ExpressionsContext ctx) {
    ...
    @Override public AstNode
    visitFetch_statement(Fetch_statementContext ctx) {
    ...
    @Override public AstNode
    visitOpen_for_statement(Open_for_statementContext ctx) {
        ...
        return new StmtOpenFor(new ExprId(name, scope, decl), new ExprStr(sql), stringifier.usedVars);
    }

    @Override public StmtCommit
    visitCommit_statement(Commit_statementContext ctx) {
        return new StmtCommit();
    }

    @Override public StmtRollback
    visitRollback_statement(Rollback_statementContext ctx) {
        return new StmtRollback();
    }

    @Override public AstNode
    visitProcedure_call(Procedure_callContext ctx) {
            ...
            if (args != null && args.nodes.size() > 0) {
                ...
                for (I_Expr arg: args.nodes) {
                    ...
                    if (dp instanceof DeclParamOut) {
                        ...
                        assert valid: "argument " + i + " to the procedure" + name + " must be a variable or " +
                            " out-parameter because it is to an out-parameter";
    ...
    @Override public StmtExecImme
    visitExecute_immediate(Execute_immediateContext ctx) {
    ...
    @Override public NodeList<I_Expr>
    visitRestricted_using_clause(Restricted_using_clauseContext ctx) {
    ...
    @Override public NodeList<I_Expr>
    visitUsing_clause(Using_clauseContext ctx) {
    ...
    @Override public NodeList<ExprId>
    visitInto_clause(Into_clauseContext ctx) {
        ...
        for (IdentifierContext c: ctx.identifier()) {
            ...
            assert id.decl instanceof DeclVar || id.decl instanceof DeclParamOut:
                "variable " + id.name + " cannot be used in the INTO clause because it is not assignable to";
            ret.addNode(id);
        }
    ...
    @Override public NodeList<I_Expr>
    visitFunction_argument(Function_argumentContext ctx) {
    ...
    @Override public ExHandler
    visitException_handler(Exception_handlerContext ctx) {
    ...
    private I_Expr
    visitExpression(ParserRuleContext ctx, String targetType) {
        if (ctx == null) { return null; }
