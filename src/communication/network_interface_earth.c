 ...
 *
 * network_interface_earth.c
 *
 * Note:
 *    Server side network interface functions for client requests.
 *    These functions were originally generated by a program.
 */
...
server_ping_with_handshake (unsigned int rid, char *request, int reqlen)
{
  ...
  reply_size = or_packed_string_length ((char *) (rel_release_string ())) +
    or_packed_string_length ((char *) NET_CLIENT_SERVER_HAND_SHAKE) +
    OR_INT_SIZE;
		     ...
		     int reqlen)
{
  ...
  copy_area = NULL;
  doesexist =
    xlocator_does_exist (thread_p, &oid, chn, lock, &class_oid, class_chn,
				      ...
				      int reqlen)
{
  ...
  success =
    xlocator_fetch_all_reference_lockset (thread_p, &oid, chn, &class_oid,
					  class_chn, lock, quit_on_errors,
					  prune_level, &lockset, &copy_area);
			    ...
			    char *request, int reqlen)
{
  ...
  renamed =
    xlocator_rename_class_name (thread_p, oldname, newname, &class_oid);
			    ...
			    char *request, int reqlen)
{
  ...
  buffer =
    xstats_get_statistics_from_server (thread_p, &classoid, timestamp,
				       &buffer_length);
...
#endif
  tran_index =
    xboot_register_client (thread_p, print_version, db_name, &oid, &hfid,
			   client_prog_name, client_user_name,
			   client_host_name, client_pid, client_lock_wait,
			   client_isolation, &transtate, &current_pagesize);
  if (tran_index == NULL_TRAN_INDEX)
    {
#if defined(DIAG_DEVEL) && defined(SERVER_MODE)
			    ...
			    char *request, int reqlen)
{
  ...
  volid =
    xboot_add_volume_extension (thread_p, ext_path, ext_name, ext_comments,
				(DKNPAGES) ext_npages,
				(DISK_VOLPURPOSE) ext_purpose, ext_overwrite);
		     ...
		     int reqlen)
{
  ...
  if (csserror)
  ...
  else
    {
      ...
      if (xlargeobjmgr_create
	  (thread_p, &loid, datasize, buffer, est_length, &oid) == NULL)
	{
	  /* ER_LK_UNILATERALLY_ABORTED may not occur,
	   * but if it occurs, rollback transaction
	   */
		     ...
		     int reqlen)
{
  ...
  success =
    xbtree_delete_index (thread_p, &btid) == NO_ERROR ? NO_ERROR : ER_FAILED;
  if (success != NO_ERROR)
    {
      /* ER_LK_UNILATERALLY_ABORTED may not occur,
       * but if it occurs, rollback transaction
		    ...
		    int reqlen)
{
  ...
  OID_SET_NULL (&oid);
  success =
    xbtree_find_unique (thread_p, &btid, &key, &class_oid, &oid, false);
				  ...
				  char *request, int reqlen)
{
  ...
  volid =
    xdisk_get_purpose_and_total_free_numpages (thread_p, volid, &vol_purpose,
					       &vol_ntotal_pages,
					       &vol_nfree_pages);
				 ...
				 unsigned int rid, char *request, int reqlen)
{
  ...
  if (var_count && var_datasize)
      ...
      if (csserror)
	{
      ...
      dbvals =
	(DB_VALUE *) db_private_alloc (thrd, sizeof (DB_VALUE) * var_count);
      if (dbvals == NULL)
	{
	  css_send_abort_to_client (rid);
	  goto cleanup;
  ...
  if (listid_length)
      ...
      if (IS_SYNC_EXEC_MODE (flag))
	{
	  if (!VPID_ISNULL (&q_result->first_vpid))
	    page_ptr =
	      qmgr_get_old_page (thrd, &q_result->first_vpid,
				 q_result->tfile_vfid);
	  if (page_ptr)
	    {
		      ...
		      char *request, int reqlen)
{
  ...
  buf =
    (char *) qmgr_get_area_error_async (thread_p, &length, count, query_id);
			...
			char *request, int reqlen)
{
...
#if !defined(PRODUCTION)
  {
	...
	printf
	  ("Server statistics structure does not match expected size !\n");
      }
  }
#endif /* !PRODUCTION */
			 ...
			 char *request, int reqlen)
{
  ...
  success =
    xcatalog_is_acceptable_new_representation (thread_p, &classoid, &hfid,
					       &can_accept);
				   ...
				   char *request, int reqlen)
{
  ...
  malloc_size =
    ((DB_SIZEOF (char *) + DB_SIZEOF (LOCK) + DB_SIZEOF (int) +
  ...
  do
    {
      ...
      copy_area = NULL;
      success =
	xlocator_fetch_lockhint_classes (thread_p, lockhint, &copy_area);
      if (success != NO_ERROR)
	{
	  /* ER_LK_UNILATERALLY_ABORTED may occur,
	   * if it occurs, rollback transaction
				...
				int reprompt_value)
{
  ...
  prompt_length = or_packed_string_length ((char *) prompt);
  prompt_length += or_packed_string_length ((char *) failure_prompt);
  prompt_length += OR_INT_SIZE * 2;
  prompt_length += or_packed_string_length ((char *) secondary_prompt);
				  ...
				  char *request, int reqlen)
{
  ...
  success =
    xheap_get_class_num_objects_pages (thread_p, &hfid, approximation, &nobjs,
				       &npages) ==
    NO_ERROR ? NO_ERROR : ER_FAILED;
		       ...
		       char *request, int reqlen)
{
  ...
  success =
    btree_get_stats (thread_p, &btid, &stat_info,
      ...
      if (info_bits & (1 << i))
	{
	  switch ((1 << i))
	    ...
	    case SI_SYS_TIMESTAMP:
	      success = db_sys_timestamp (&value[i])
		== NO_ERROR ? NO_ERROR : ER_FAILED;
	      break;

	    case SI_LOCAL_TRANSACTION_ID:
	      success =
		xtran_get_local_transaction_id (thread_p, &value[i]) ==
		NO_ERROR ? NO_ERROR : ER_FAILED;
	      break;
	    }
				...
				char *request, int reqlen)
{
  ...
  success =
    xtran_get_local_transaction_id (thread_p,
				    &val) == NO_ERROR ? NO_ERROR : ER_FAILED;
				...
				char *request, int reqlen)
{
  ...
  if (xlocator_build_fk_object_cache
      (thread_p, &class_oid, &hfid, key_type, n_attrs, attr_ids, &pk_cls_oid,
       &pk_btid, cache_attr_id, fk_name) != NO_ERROR)
    {
      if (er_errid () == ER_LK_UNILATERALLY_ABORTED)
	{
	  tran_server_unilaterally_abort_tran (thread_p);
