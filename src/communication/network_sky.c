 ...
 *
 * network_sky.c - client side support functions.
 *
 * Note:
 */
...
/* TODO : remove(from conn.c, connless.c */
#if defined(CS_MODE)
 ...
 *
 * Note:
 *    Compare the sqlx release strings from the server and client to
			...
			if (prompt_id != FILEIO_PROMPT_DISPLAY_ONLY)
			  {
				...
				switch (prompt_id)
				  ...
				  case FILEIO_PROMPT_RANGE_TYPE:
				    ...
				    x =
				      strtol (user_response_ptr, &a_ptr, 10);
				  ...
				  case FILEIO_PROMPT_BOOLEAN_TYPE:
					...
					'y' || (*user_response_ptr == '1')
					||
					intl_mbs_casecmp (user_response_ptr,
							  "yes") == 0)
				      {
					response_needed = true;
					pr_status = NO_ERROR;
	  ...
	  if (*reply_log_area != NULL)
	      ...
	      if (packed_desc_size > 0)
		{
		  css_queue_receive_data_buffer (rc, packed_desc,
						 packed_desc_size);
		  error =
		    css_receive_data_from_server (rc, &packed_desc, &size);
		  if (error != NO_ERROR)
		    {
		      free_and_init (packed_desc);
		      return set_server_error (error);
	      ...
	      if (content_size > 0)
		{
		  css_queue_receive_data_buffer (rc, content_ptr,
						 content_size);
		  error =
		    css_receive_data_from_server (rc, &content_ptr, &size);
		  if (error != NO_ERROR)
		    {
		      return set_server_error (error);
		    }
							    ...
							    reply_copy_area)
{
  ...
  *reply_copy_area = NULL;
  if (packed_desc_size != 0 || content_size != 0)
    {
      if ((error == NO_ERROR) && (*reply_copy_area =
				  locator_recv_allocate_copyarea (num_objs,
								  &packed_desc,
								  packed_desc_size,
								  &content_ptr,
...
net_client_ping_server (void)
{
  ...
  reply_size = or_packed_string_length (client_version) +
    or_packed_string_length (NET_CLIENT_SERVER_HAND_SHAKE) + OR_INT_SIZE;
