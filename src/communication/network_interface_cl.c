...
jsp_restart (void)
{
  ...
  req_error =
    net_client_request (NET_SERVER_JSP_RESTART, NULL, 0, reply, OR_ALIGNED_BUF_SIZE (a_reply), NULL, 0, NULL, 0);
  if (!req_error)
    {
      or_unpack_errcode (reply, &error);
    }
  ...
  int error = NO_ERROR;
  THREAD_ENTRY *thread_p = enter_server ();
  DB_INFO *db = cfg_find_db (boot_db_name ());
  if (db != NULL)
  {
...
jsp_status (FILE * outfp)
{
  ...
  req_error = net_client_request_recv_stream (NET_SERVER_JSP_STATUS, NULL, 0, NULL, 0, NULL, 0, outfp);
#else
  THREAD_ENTRY *thread_p = enter_server ();
