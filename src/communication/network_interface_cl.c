...
/*
 * sysprm_need_filtering_parameter () - handled if parameters require separate handling.
 *
 * return : boolean
 * assignments_ptr (out) : list of assignments.
 *
 * NOTE: If parameters require separate handling, true is returned.
 *   otherwise, false is returned.
 */
static bool
sysprm_need_filtering_parameter (SYSPRM_ASSIGN_VALUE * assignments)
{
  bool filtered = true;
  ...
  switch (assignments->prm_id)
  {
    ...
    case PRM_ID_LOG_ISOLATION_LEVEL:
      (void) tran_reset_isolation ((TRAN_ISOLATION) assignments->value.i, TM_TRAN_ASYNC_WS ());
      break;
...
/*
 * sysprm_filter_change_parameters () - filter out the specific parameters.
 *
 * return : boolean
 * assignments_ptr (out) : list of assignments.
 *
 * NOTE: If the list of assignments is empty after filtering, true is returned.
 *   otherwise, false is returned.
 */
bool
