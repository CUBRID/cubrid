  ...
  *can_end_transaction = false;
  ...
  for (i = 0; i < list_id->type_list.type_cnt; i++)
    {
      ...
      if (TP_IS_CHAR_TYPE (pr_type->id) || pr_type->id == DB_TYPE_CLOB)
	{
  ...
  while (!exceed_a_page)
      ...
      for (i = 0; i < list_id->type_list.type_cnt; i++)
	{
	  ...
	  pr_type = domains[i]->type;
	  if (flag != V_UNBOUND && (TP_IS_CHAR_TYPE (pr_type->id) || pr_type->id == DB_TYPE_CLOB))
	    {
	      buf.ptr = tuple_p;
...
/*
 * internal_lob_check_client_locator_sig () - reject a client-supplied read locator whose per-session signature
 *   does not match this session's secret (CBRD-26780).  A client cannot compute the signature for an OID it did
 *   not legitimately receive, so this stops reads of internal LOB / OOS content the caller is not authorized for.
 */
static int
internal_lob_check_client_locator_sig (THREAD_ENTRY *thread_p, const INTERNAL_LOB_LOCATOR &locator,
				       unsigned long long sig)
{
  ...
  expected = internal_lob_locator_session_sig (locator, locator.adopted, session_key);
  if (sig == 0 || sig != expected)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_INTERNAL_LOB_LOCATOR_NOT_AUTHORIZED, 0);
      return ER_INTERNAL_LOB_LOCATOR_NOT_AUTHORIZED;
    }
...
sinternal_lob_read (THREAD_ENTRY *thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  err = internal_lob_check_client_locator_sig (thread_p, locator, locator_sig);
  if (err != NO_ERROR)
    {
      goto reply;
    }
...
create_stream_session (THREAD_ENTRY *thread_p, int stream_kind, char *config_ptr, int config_len, int *error_code)
{
  switch (stream_kind)
	...
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_STREAM_SESSION_ERROR, 1,
		  "invalid internal LOB load stream configuration");
	  *error_code = ER_STREAM_SESSION_ERROR;
	  return NULL;
	}
	...
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_STREAM_SESSION_ERROR, 1,
		  "invalid internal LOB load stream type");
	  *error_code = ER_STREAM_SESSION_ERROR;
	  return NULL;
	}
	...
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_STREAM_SESSION_ERROR, 1, "no active load session");
	  *error_code = ER_STREAM_SESSION_ERROR;
	  return NULL;
	}
