...
static void event_log_expand_pages (THREAD_ENTRY * thread_p, EXECUTION_INFO * info);
...
sqmgr_execute_query (THREAD_ENTRY * thread_p, unsigned int rid, char *request, int reqlen)
{
  ...
  if (tran_abort == false)
      ...
      if (trace_slow_msec >= 0 || trace_ioreads > 0)
	{
      ...
      if (thread_p->event_stats.expand_pages > 0)
	{
	  event_log_expand_pages (thread_p, &info);
	}
    }
...
/*
 * event_log_expand_pages - log volume expand pages to event log file
 * return:
 *   thread_p(in):
 *   info(in):
 *   num_bind_vals(in):
 ...
 */
static void
event_log_expand_pages (THREAD_ENTRY * thread_p, EXECUTION_INFO * info)
{
  FILE *log_fp;
  int indent = 2;
  LOG_TDES *tdes;
  ...
  tran_index = LOG_FIND_THREAD_TRAN_INDEX (thread_p);
  tdes = LOG_FIND_TDES (tran_index);
  log_fp = event_log_start (thread_p, "ADD/EXPAND_VOLUME_INFO");
  ...
  fprintf (log_fp, "%*ctime: %dms\n", indent, ' ', TO_MSEC (thread_p->event_stats.expand_time));
  fprintf (log_fp, "%*cpages: %d\n\n", indent, ' ', thread_p->event_stats.expand_pages);
