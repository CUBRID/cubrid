 ...
 */
void
sjsp_restart (THREAD_ENTRY * thread_p, unsigned int rid, char *request, int reqlen)
{
  int error = ER_SP_NOT_RUNNING_JVM;
  DB_INFO *db;
  char* pathname;
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);
  ...
  if (prm_get_bool_value (PRM_ID_JAVA_STORED_PROCEDURE))
  {
    ...
    if (db != NULL)
      {
        error = jsp_stop_server ();
        if (error != NO_ERROR)
        {
          (void) return_error_to_client (thread_p, rid);
        }
        
        error = jsp_start_server (boot_db_name (), db->pathname);
        if (error != NO_ERROR)
        {
          (void) return_error_to_client (thread_p, rid);
        }
 ...
 */
void
sjsp_status (THREAD_ENTRY * thread_p, unsigned int rid, char *request, int reqlen)
{
  OR_ALIGNED_BUF (OR_INT_SIZE) a_reply;
  char *reply = OR_ALIGNED_BUF_START (a_reply);

  (void) or_pack_int (reply, jsp_server_port ());
  // (void) or_pack_string_with_length (reply, jsp_jvm_arg_str (), jsp_jvm_arg_str_length ());
