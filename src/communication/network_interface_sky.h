 ...
 *
 * network_interface_sky.h - Definitions for client network support
 *
 * Note :
 *
 *      Do not put any server support functions here.
 *      They belong in network_interface_earth.h.
 *      Any definitions to support both client and server belong in network.h.
 *
 */

#ifndef _NETWORK_INTERFACE_SKY_H_
#define _NETWORK_INTERFACE_SKY_H_
...
extern VOLID disk_get_purpose_and_total_free_numpages (VOLID volid,
						       DISK_VOLPURPOSE *
						       vol_purpose,
						       DKNPAGES *
						       vol_ntotal_pages,
...
extern LOG_COPY *log_client_unknown_state_abort_get_first_undo (LOG_LSA *
								next_lsa);
extern void
log_append_client_undo (LOG_RCVCLIENT_INDEX rcv_index, int length,
			void *data);
...
int boot_initialize_server (int print_version, bool db_overwrite,
...
extern int boot_register_client (int print_restart, const char *db_name,
...
extern int largeobjmgr_read (LOID * loid, int offset, int length,
			     char *buffer);
extern int largeobjmgr_write (LOID * loid, int offset, int length,
			      char *buffer);
extern int largeobjmgr_insert (LOID * loid, int offset, int length,
			       char *buffer);
extern int largeobjmgr_delete (LOID * loid, int offset, int length);
extern int largeobjmgr_append (LOID * loid, int length, char *buffer);
extern int largeobjmgr_truncate (LOID * loid, int offset);
extern int largeobjmgr_compress (LOID * loid);
extern int largeobjmgr_length (LOID * loid);
...
extern QFILE_LIST_ID *qmgr_prepare_and_execute_query (char *xasl_buffer,
						      ...
						      DB_VALUE * dbval_ptr,
						      QUERY_FLAG flag);
extern int qmgr_end_query (int query_id);
extern int
...
extern int
qmgr_get_query_info (DB_QUERY_RESULT * query_result, int *done, int *count,
...
extern void mnt_server_stop_stats (void);
...
extern int catalog_is_acceptable_new_representation (OID * class_id,
						     HFID * hfid,
						     int *can_accept);
extern int css_restart_event_handler (void);
extern int
thread_kill_tran_index (int kill_tran_index, char *kill_user, char *kill_host,
			int kill_pid);
...
extern void histo_start (void);
extern void histo_stop (void);
extern void histo_print (void);
extern void histo_clear (void);
extern void histo_setup_names (void);
...
net_client_request_3_data (int request, char *argbuf, int argsize,
			   ...
			   int datasize2, char *replydata0,
			   int replydatasize0, char *replydata1,
			   int replydatasize1, char *replydata2,
			   int replydatasize2);
extern int
...
extern int
net_client_request3 (int request, char *argbuf, int argsize, char *replybuf,
		     int replysize, char *databuf, int datasize,
		     char **replydata_ptr, int *replydatasize_ptr,
		     char **replydata_ptr2, int *replydatasize_ptr2);
extern int
net_client_request_recv_copyarea (int request, char *argbuf, int argsize,
				  char *replybuf, int replysize,
...
extern int net_client_ping_server (void);
...
#endif /* _NETWORK_INTERFACE_SKY_H_ */
