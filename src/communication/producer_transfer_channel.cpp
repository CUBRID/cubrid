...
namespace cubstream
{
  ...
  {
    public:
      default_producer_transfer_channel_sender_task (cubstream::producer_transfer_channel *producer_channel) : this_producer_channel (
          producer_channel) {}
    ...
    private:
      cubstream::producer_transfer_channel *this_producer_channel;
  };
                                  ...
                                  stream (NULL)
  {
    ...
    if (with_default_daemon)
      {
        m_sender_daemon = cubthread::get_manager ()->create_daemon (std::chrono::milliseconds (10), new default_producer_transfer_channel_sender_task (this));
      }
  }
  ...
  void producer_transfer_channel::set_daemon (cubthread::entry_task *task)
  {
    if (m_sender_daemon != NULL)
      {
        cubthread::get_manager ()->destroy_daemon (m_sender_daemon);
      }

    m_sender_daemon = cubthread::get_manager ()->create_daemon (std::chrono::milliseconds (10), task);
  }
