...
set_server_error (int error)
{
  ...
  switch (error)
    ...
    case REQUEST_REFUSED:
      ...
      break;
    case SERVER_ABORTED:
      /* server error may not be set when SERVER_ABORTED is given. 
       * server may send ABORT_TYPE for some cases and 
       * the server error will not be delivered for the case.
       */
      server_error = er_errid ();
...
net_histo_setup_names (void)
{
  ...
  net_Req_buffer[NET_SERVER_BO_FIND_LAST_PERM].name =
  ...
  net_Req_buffer[NET_SERVER_LC_RESERVE_CLASSNAME_GET_OID].name =
  ...
  net_Req_buffer[NET_SERVER_BTREE_FIND_MULTI_UNIQUES].name =
    "NET_SERVER_FIND_MULTI_UNIQUES";
  net_Req_buffer[NET_SERVER_VACUUM].name = "NET_SERVER_VACUUM";
  net_Req_buffer[NET_SERVER_GET_MVCC_SNAPSHOT].name =
