...
histo_setup_names (void)
{
  ...
  net_Req_buffer[NET_SERVER_PING_WITH_HANDSHAKE].name =
    "NET_SERVER_PING_WITH_HANDSHAKE";
  net_Req_buffer[NET_SERVER_PING].name = "NET_SERVER_PING";
		    ...
		    char *replydata, int replydatasize)
{
   ...
   */
  if (request == NET_SERVER_QM_QUERY_END && save_query_id == -1)
    {
      ...
      do
	{
	  ...
	  switch (server_request)
	    ...
	    case GET_NEXT_LOG_PAGES:
	      {
		error =
		  net_client_get_next_pages (rc, replybuf, replysize, ptr);
		if (error != NO_ERROR)
		  {
		    server_request = END_CALLBACK;
		  }
		else if (logwr_Gl.mode == LOGWR_MODE_ASYNC)
		  {
		    server_request = END_CALLBACK;
		    ptr = or_pack_int (replybuf, (int) END_CALLBACK);
  ...
  if (net_Server_name[0] == '\0')
      ...
      do
	{
	  ...
	  switch (server_request)
		...
		error =
		  net_client_get_next_pages (rc, replybuf, replysize, length);
	      }
	      break;
	    case END_CALLBACK:
...
{
  char *client_rel_string, *server_rel_string = NULL, *ptr;
  int error = NO_ERROR;
  ...
  int eid, reply_size, request_size, server_capabilities;
  ...
  reply_size = REL_MAX_RELEASE_LENGTH + OR_INT_SIZE;
