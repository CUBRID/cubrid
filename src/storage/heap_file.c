...
struct heap_hdr_stats
{
  ...
  struct
  {
    int num_candidates;
    // *INDENT-OFF*
    cubstorage::bestspace_entry candidates[128];
    // *INDENT-ON*
...
static int heap_update_bestspace_chain (THREAD_ENTRY * thread_p, const HFID * hfid, const HEAP_HDR_STATS * heap_hdr);
...
static int heap_take_bestspace (THREAD_ENTRY * thread_p, HFID * hfid, PGBUF_WATCHER * header_watcher,
				...
				cubstorage::bestspace_entry * candidates, int *num_candidates, int *unfill_space);
 ...
 */
static void
heap_bestspace_clear_candidates (HEAP_HDR_STATS * heap_hdr)
{
  int max_candidates;
  int i;
  ...
  heap_hdr->bestspace.num_candidates = 0;
  for (i = 0; i < max_candidates; i++)
    {
      heap_hdr->bestspace.candidates[i].freespace = 0;
      heap_hdr->bestspace.candidates[i].volid = NULL_VOLID;
      heap_hdr->bestspace.candidates[i].pageid = NULL_PAGEID;
    }
}
 ...
 */
static void
heap_bestspace_add_candidate (HEAP_HDR_STATS * heap_hdr, const cubstorage::bestspace_entry * candidate)
{
  int max_candidates;
  int pos;
  ...
  assert (heap_hdr->bestspace.num_candidates >= 0);
  assert (heap_hdr->bestspace.num_candidates <= max_candidates);

  if (heap_hdr->bestspace.num_candidates == max_candidates
      && candidate->freespace <= heap_hdr->bestspace.candidates[max_candidates - 1].freespace)
    {
      return;
    }

  pos = MIN (heap_hdr->bestspace.num_candidates, max_candidates - 1);
  while (pos > 0 && heap_hdr->bestspace.candidates[pos - 1].freespace < candidate->freespace)
    {
      if (pos < max_candidates)
	{
	  heap_hdr->bestspace.candidates[pos] = heap_hdr->bestspace.candidates[pos - 1];
	}
      pos--;
    }

  heap_hdr->bestspace.candidates[pos] = *candidate;
  if (heap_hdr->bestspace.num_candidates < max_candidates)
    {
      heap_hdr->bestspace.num_candidates++;
    }
}
		       ...
		       cubstorage::bestspace_entry * first_entry)
{
  ...
  /* candidates */
  heap_bestspace_clear_candidates (heap_hdr);

  /* shards */
  heap_hdr->bestspace.num_shards = prm_get_integer_value (PRM_ID_HF_MAX_BESTSPACE_ENTRIES) / 64;

  /* calculate the needed pages */
  error = sysprm_get_range (PRM_ID_HF_MAX_BESTSPACE_ENTRIES, (void *) &min_entries, (void *) &max_entries);
  if (error != NO_ERROR)
    {
      assert (false);
      return ER_FAILED;
    }
  max_entries = (max_entries + 63) / 64 * 64;
  ...
  /* bestspace pages */
  max_pages = sizeof (heap_hdr->bestspace.pages) / sizeof (heap_hdr->bestspace.pages[0]);
  ...
  for (i = 0; i < heap_hdr->bestspace.num_pages; i++)
    {
      prev_vpid = (i == 0) ? &header_vpid : &heap_hdr->bestspace.pages[i - 1];
      next_vpid = (i == heap_hdr->bestspace.num_pages - 1) ? &null_vpid : &heap_hdr->bestspace.pages[i + 1];
  ...
  for (i = 0; i < heap_hdr->bestspace.num_pages; i++)
	...
	{
	  entries[j].freespace = 0;
	  entries[j].volid = NULL_VOLID;
	  entries[j].pageid = NULL_PAGEID;
	}
 ...
 */
static int
heap_update_bestspace_chain (THREAD_ENTRY * thread_p, const HFID * hfid, const HEAP_HDR_STATS * heap_hdr)
{
  PGBUF_WATCHER page_watcher;
  PAGE_PTR page_ptr = NULL;
  HEAP_CHAIN chain;
  ...
  for (i = 0; i < heap_hdr->bestspace.num_pages; i++)
    {
      if (VPID_ISNULL (&heap_hdr->bestspace.pages[i]))
	{
	  er_set (ER_FATAL_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
      ...
      error_code = heap_bestspace_fix_page (thread_p, hfid, &heap_hdr->bestspace.pages[i], &page_watcher);
      if (error_code != NO_ERROR)
	{
	  goto error_exit;
	}
      ...
      if (heap_copy_chain (thread_p, page_ptr, &chain) != NO_ERROR)
	{
      ...
      COPY_OID (&chain.class_oid, &heap_hdr->class_oid);
 ...
 */
static int
heap_update_bestspace (THREAD_ENTRY * thread_p, const HFID * hfid, const HEAP_HDR_STATS * heap_hdr,
 ...
 */
static int
heap_take_bestspace (THREAD_ENTRY * thread_p, HFID * hfid, PGBUF_WATCHER * header_watcher,
		     cubstorage::bestspace_entry * entries, int *num_entries, cubstorage::bestspace_entry * candidates,
		     int *num_candidates, int *unfill_space)
{
...
{
  // *INDENT-OFF*
  cubstorage::bestspace_entry *entries, *candidates;
  // *INDENT-ON*
  int min_entries, max_entries, max_candidates;
  ...
  if (error == ER_MHT_NOTFOUND)
      ...
      if (error != NO_ERROR)
	{
      ...
      max_entries = (max_entries + 63) / 64 * 64;
      max_candidates =
	sizeof (((HEAP_HDR_STATS *) 0)->bestspace.candidates) /
	sizeof (((HEAP_HDR_STATS *) 0)->bestspace.candidates[0]);
      entries = malloc (max_entries * sizeof (cubstorage::bestspace_entry));
      if (!entries)
	{
	  pgbuf_ordered_unfix (thread_p, &header_watcher);
	  return ER_FAILED;
	}
      candidates = malloc (max_candidates * sizeof (cubstorage::bestspace_entry));
      if (!candidates)
	{
      ...
      error =
	heap_take_bestspace (thread_p, hfid, &header_watcher, entries, &num_entries, candidates, &num_candidates,
			     &unfill_space);
      if (error != NO_ERROR)
	{
	  pgbuf_ordered_unfix (thread_p, &header_watcher);
	...
	{
	  num_shards = prm_get_integer_value (PRM_ID_HF_MAX_BESTSPACE_ENTRIES) / 64;
	}
      cubstorage::bestspaces.create (class_oid, hfid, entries, num_entries, num_shards, unfill_space);
...
heap_reuse (THREAD_ENTRY * thread_p, const HFID * hfid, const OID * class_oid, const bool reuse_oid)
{
  ...
  HEAP_HDR_STATS heap_hdr_bestspace;
  HEAP_CHAIN *chain;		/* Chain to next and prev page */
  ...
  while (!(VPID_ISNULL (&vpid)))
      ...
      else
	{
	  ...
	  heap_bestspace_add_candidate (heap_hdr, &candidate_entry);
	}
  ...
  heap_hdr_bestspace = *heap_hdr;

  if (heap_update_bestspace_chain (thread_p, hfid, &heap_hdr_bestspace) != NO_ERROR)
    {
      goto error;
    }
  if (heap_update_bestspace (thread_p, hfid, &heap_hdr_bestspace, &first_entry, 1) != NO_ERROR)
    {
      goto error;
    }
  ...
  if (home_hint_p == NULL)
    {
      ...
      if (heap_find_bestspace (thread_p, &context->class_oid, &context->hfid, context->recdes_p->length,
