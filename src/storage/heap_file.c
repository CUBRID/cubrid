...
struct heap_hdr_stats
{
  ...
  HFID oor_hfid;		/* Oout of row file identifier (if any) */
...
static HFID *heap_create_internal (THREAD_ENTRY * thread_p, HFID * hfid, int exp_npgs, const OID * class_oid,
				   const bool reuse_oid, const bool is_secondary_heap);
		      ...
		      const bool is_secondary_heap)
{
  ...
  HFID_SET_NULL (&heap_hdr.oor_hfid);
...
{
  if (heap_create_internal (thread_p, hfid, -1, class_oid, reuse_oid, false) == NULL)
    {
...
heap_assign_address (THREAD_ENTRY * thread_p, const HFID * hfid, OID * class_oid, OID * oid, int expected_length)
{
  ...
  /* create context */
  heap_create_insert_context (&insert_context, (HFID *) hfid, class_oid, &recdes, NULL, NULL, false);
				       ...
				       NON_EXISTENT_HANDLING non_ex_handling_type)
{
  ...
  MVCC_SET_DELETE_INFO (&mvcc_delete_info, MVCCID_NULL, DELETE_RECORD_CAN_DELETE);
  ...
  /* We are here only if a valid record was found: REC_HOME, REC_BIGONE, REC_RELOCATION. Any other case should be 
...
get_heap_record:
  /* We are here because we decided the object from heap(the newest version) is needed */
  if (recdes != NULL)
    {
      /* Get the record data. */
		     ...
		     HEAPATTR_OOR_MODE oor_read_mode)
{
  ...
  if (disk_data == NULL || disk_bound == false)
	...
	case 0:
	  ...
	  if (is_out_of_row)
	    {
	      PR_TYPE *pr_type_oid;		/* Primitive type array function structure */
	      DB_VALUE oid_value;
	      ...
	      pr_type_oid = PR_TYPE_FROM_ID (DB_TYPE_OID);
	      if (pr_type_oid == NULL)
		{
		  ret = ER_FAILED;
		  break;
...
re_check:
  ...
  for (i = 0; i < attr_info->num_values; i++)
      ...
      else
	{
	  ...
	  size += column_size;
	  if (pr_is_overflow_column (&value->dbvalue)
					  ...
					  int lob_create_flag)
{
  ...
  bool check_overflow_column = true;
  ...
  if (attr_info->num_values > 1
    ...
    {
      check_overflow_column = true;
    }
  else
    {
      check_overflow_column = false;
    }
  ...
  switch (_setjmp (buf->env))
      ...
      for (i = 0; i < attr_info->num_values; i++)
	{
	  ...
	  if (value->last_attrepr->is_fixed != 0)
	    ...
	    {
	      bool is_overflow = OVERFLOW_COLUMN_DISABLED;
	      bool copy_oor_oid = false;
	      ...
	      if (value->do_increment != 0)
		{
	      ...
	      if (check_overflow_column)
		{
		  is_overflow = (pr_is_overflow_column (dbvalue)) ? OVERFLOW_COLUMN_ENABLED : OVERFLOW_COLUMN_DISABLED;
		}

	      if (value->state == HEAP_READ_ATTRVALUE_OOR_OID)
		{
		  ...
		  copy_oor_oid = true;
		  is_overflow = OVERFLOW_COLUMN_ENABLED;
		}
		  ...
		  if (is_overflow)
		    {
		      ...
		      int oid_offset_in_home;

		      if (copy_oor_oid)
			{
			   pr_type_oid->data_writeval (buf, dbvalue);
			}
		      else
			{
			  /* TODO[arnia]: */
			  OID dummy_oid = {55, 55, 55};
			  DB_VALUE dummy_oid_value;

			  DB_MAKE_OID (&dummy_oid_value, &dummy_oid);

			  oid_offset_in_home = CAST_BUFLEN (buf->ptr - buf->buffer);
			  pr_type_oid->data_writeval (buf, &dummy_oid_value);
			  ...
			  /* TODO[arnia] : copy common header to oor recdes buffer */
			  memcpy (overflow_col_buf.ptr, common_oor_header, header_size);
			  overflow_col_buf.ptr += header_size;
			  ...
			  out_of_row_recdes->home_recdes_oid_offsets[out_of_row_recdes->recdes_cnt] = oid_offset_in_home;
...
heap_dump_hdr (FILE * fp, HEAP_HDR_STATS * heap_hdr)
{
  ...
  fprintf (fp, "OVF_VFID = %4d|%4d, OOR_HFID = %4d|%4d|%4d, NEXT_VPID = %4d|%4d\n",
	   heap_hdr->ovf_vfid.volid, heap_hdr->ovf_vfid.fileid,
	   heap_hdr->oor_hfid.vfid.volid, heap_hdr->oor_hfid.vfid.fileid, heap_hdr->oor_hfid.hpgid,
...
try_again:
  ...
  if (satisfies_delete_result == DELETE_RECORD_DELETED || satisfies_delete_result == DELETE_RECORD_SELF_DELETED)
      ...
      if (record_locked == true)
	{
      ...
      MVCC_SET_DELETE_INFO (mvcc_delete_info, MVCC_GET_DELID (recdes_header), satisfies_delete_result);
    }
  else if (satisfies_delete_result == DELETE_RECORD_CAN_DELETE)
    {
      /* there is no next row, set satisfies result for current row */
      MVCC_SET_DELETE_INFO (mvcc_delete_info, MVCCID_NULL, satisfies_delete_result);
    }
  else
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_MVCC_ROW_INVALID_FOR_DELETE, 3, oid->volid, oid->pageid,
...
heap_clear_operation_context (HEAP_OPERATION_CONTEXT * context, HFID * hfid_p)
{
  ...
  HFID_SET_NULL (&context->oor_hfid);
  ...
  for (i = 0; i < context->out_of_row_recdes->recdes_cnt; i++)
      ...
      if (context->out_of_row_recdes->oor_recdes[i].length < HEAP_OOR_SLOTTED_THRESHOLD_SIZE)
	{
	  ...
	  heap_create_insert_context (&oor_context, &context->oor_hfid, &context->class_oid,
				      &context->out_of_row_recdes->oor_recdes[i], NULL, NULL, -1);
...
heap_update_bigone (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op)
{
  ...
  if (update_old_home)
      ...
      else
	{
      ...
      if (is_mvcc_op)
	{
	  /* undo logging already done */
	  heap_log_update_redo (thread_p, context->home_page_watcher_p->pgptr, &context->hfid.vfid,
				&context->oid, &new_home_recdes, RVHF_UPDATE);
	}
      else
	{
	  heap_log_update_physical (thread_p, context->home_page_watcher_p->pgptr, &context->hfid.vfid,
				    &context->oid, &context->home_recdes, &new_home_recdes, RVHF_UPDATE);
	}
...
heap_update_relocation (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op)
{
  ...
  if (fits_in_home || (!fits_in_home && !fits_in_forward) || heap_is_big_length (context->recdes_p->length))
    {
      /* old forward record is deleted and the new record is inserted into old home, or into a new location */
      heap_log_delete_physical (thread_p, context->forward_page_watcher_p->pgptr, &context->hfid.vfid, &forward_oid,
				&forward_recdes, true);
      remove_old_forward = true;
      /* old home is updated in any case */
      update_old_home = true;
    }
  else
    {
      /* old forward record is updated in place, home record will not be touched;
  ...
  if (is_mvcc_op)
    {
      /* set the previous version of the new record */
      or_mvcc_set_log_lsa_to_record (context->recdes_p, logtb_find_current_tran_lsa (thread_p));
    }
...
heap_update_home (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op)
{
  ...
  heap_log_update_redo (thread_p, context->home_page_watcher_p->pgptr, &context->hfid.vfid, &context->oid,
			home_page_updated_recdes_p, (is_mvcc_op ? RVHF_UPDATE_NOTIFY_VACUUM : RVHF_UPDATE));
...
heap_create_insert_context (HEAP_OPERATION_CONTEXT * context, HFID * hfid_p, OID * class_oid_p, RECDES * recdes_p,
			    OUT_OF_ROW_RECDES *out_of_row_recdes_p, HEAP_SCANCACHE * scancache_p, bool bigone_max_size)
{
  assert (context != NULL);
  assert (hfid_p != NULL);
  assert (recdes_p != NULL);
...
heap_create_update_context (HEAP_OPERATION_CONTEXT * context, HFID * hfid_p, OID * oid_p, OID * class_oid_p,
			    ...
			    UPDATE_INPLACE_STYLE in_place, bool bigone_max_size)
{
  assert (context != NULL);
  assert (hfid_p != NULL);
  assert (oid_p != NULL);
  ...
  if (scan_cache != NULL && scan_cache->cache_last_fix_page && scan_cache->page_watcher.pgptr != NULL)
    {
      ...
      pgbuf_replace_watcher (thread_p, &scan_cache->page_watcher, &home_pg_watcher);
    }
...
/*
 * heap_oor_find_hfid () - Find overflow file identifier
 *   return: ovf_vfid or NULL
 *   hfid(in): Object heap file identifier
 *   oor_vfid(in/out): Out of row file identifier.
 *   docreate(in): true/false. If true and the overflow file does not
 *                 exist, it is created.
 *
 * Note: Find overflow file identifier. If the overflow file does not
...
heap_oor_find_hfid (THREAD_ENTRY * thread_p, HFID * hfid, int exp_npgs, HFID *oor_hfid, bool docreate)
{
  ...
  heap_hdr = (HEAP_HDR_STATS *) hdr_recdes.data;
  if (HFID_IS_NULL (&heap_hdr->oor_hfid))
    {
      if (docreate == true)
	{
	  FILE_OVF_HEAP_DES hfdes_ovf;
	  ...
	  oor_hfid->vfid.volid = hfid->vfid.volid;
	  /* 
	   * At least three pages since a multipage object will take at least
	   * two pages
	   */
	  ...
	  if (heap_create_internal (thread_p, oor_hfid, 3, NULL, false, true) != NULL)
	    {
	      /* Log undo, then redo */
	      log_append_undo_data (thread_p, RVHF_STATS, &addr_hdr, sizeof (*heap_hdr), heap_hdr);
	      VFID_COPY (&heap_hdr->oor_hfid, oor_hfid);
	      log_append_redo_data (thread_p, RVHF_STATS, &addr_hdr, sizeof (*heap_hdr), heap_hdr);
	      pgbuf_set_dirty (thread_p, addr_hdr.pgptr, DONT_FREE);

	      log_end_system_op (thread_p, LOG_RESULT_TOPOP_COMMIT);
	      (void) file_new_declare_as_old (thread_p, &oor_hfid->vfid);
	    }
	  else
	    {
	      log_end_system_op (thread_p, LOG_RESULT_TOPOP_ABORT);
    ...
    {
      VFID_COPY (oor_hfid, &heap_hdr->oor_hfid);
    }
