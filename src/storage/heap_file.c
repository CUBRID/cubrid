...
#define HEAP_SET_PHYSYCAL_DELETE_UNDO_RECDES(undo_recdes_p, recdes_p, temp_recdes_p, byte_reserved)  \
  do {	\
      if ((recdes_p)->type == REC_ASSIGN_ADDRESS)	\
	{/* special case for REC_ASSIGN */  \
	  assert ((temp_recdes_p) != NULL);   \
	  bytes_reserved = (INT16) ((recdes_p)->length);  \
	  HEAP_SET_RECORD (temp_recdes_p, sizeof (bytes_reserved), sizeof (bytes_reserved), (recdes_p)->type,  \
			   (char *) &bytes_reserved);	\
	  undo_recdes_p = temp_recdes_p;	\
	} \
      else  \
	{ \
...
STATIC_INLINE int heap_create_log_node_for_logical_deletion (THREAD_ENTRY * thread_p, LOG_TDES * tdes,
							     LOG_RCVINDEX rcv_index, VFID * vfid_p, PAGE_PTR page_p,
							     PGLENGTH offset, char *redo_data_p, int redo_data_size,
...
STATIC_INLINE int heap_create_log_node_for_physical_deletion (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p,
							      ...
							      RECDES * old_recdes_p,
							      LOG_PRIOR_NODE ** physical_delete_log_node)
  __attribute__ ((ALWAYS_INLINE));
...
STATIC_INLINE int heap_mvcc_log_home_change_on_delete (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p,
  ...
  __attribute__ ((ALWAYS_INLINE));
STATIC_INLINE void heap_mvcc_log_home_no_change (THREAD_ENTRY * thread_p, PAGE_PTR page_p, VFID * vfid_p, int offset)
  __attribute__ ((ALWAYS_INLINE));
...
STATIC_INLINE int heap_create_insert_log_info_before_page_fixing (THREAD_ENTRY * thread_p, LOG_TDES * tdes,
								  VFID * vfid_p, RECDES * p_recdes,
								  bool is_mvcc_op, bool is_redistribute_op,
								  HEAP_LOG_INFO * insert_log_info)
  __attribute__ ((ALWAYS_INLINE));
STATIC_INLINE int heap_log_insert_physical (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p, VFID * vfid_p,
					    int offset, RECDES * recdes_p, bool is_mvcc_op, bool is_redistribute_op,
					    HEAP_LOG_INFO * physical_insert_log_info) __attribute__ ((ALWAYS_INLINE));
...
static int heap_delete_bigone (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context,
			       bool is_mvcc_op, MVCCID mvccid, HEAP_LOG_INFO * delete_log_info);
STATIC_INLINE int
heap_mvcc_delete_relocation_build_new_record (THREAD_ENTRY * thread_p, LOG_TDES * tdes, MVCCID mvcc_id,
					      HEAP_OPERATION_CONTEXT * context, bool * is_adjusted_size_big,
					      bool * fits_in_home, bool * fits_in_forward)
__attribute__ ((ALWAYS_INLINE));
STATIC_INLINE int
heap_mvcc_delete_relocation_prepare_log (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_page_fixed,
					 bool is_adjusted_size_big, bool fits_in_home, bool fits_in_forward,
					 HEAP_OPERATION_CONTEXT * context, MVCCID mvccid, LOG_RCVINDEX * rcv_index,
					 int *slot_id_with_flags /* TO DO - slotid */ , void **undo_data,
					 void **redo_data,
					 int *redo_data_size, bool * update_old_home, bool * update_old_forward,
					 bool * remove_old_forward) __attribute__ ((ALWAYS_INLINE));
static int heap_delete_relocation (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context,
				   bool is_mvcc_op, MVCCID mvcc_id, HEAP_LOG_INFO * delete_log_info);
STATIC_INLINE int
heap_mvcc_delete_home_build_new_record (THREAD_ENTRY * thread_p, LOG_TDES * tdes, MVCCID mvcc_id,
					HEAP_OPERATION_CONTEXT * context) __attribute__ ((ALWAYS_INLINE));
STATIC_INLINE int heap_mvcc_delete_home_prepare_log (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_page_fixed,
						     HEAP_OPERATION_CONTEXT * context, MVCCID mvccid,
						     LOG_RCVINDEX * rcv_index, int *slot_id_with_flags,
						     void **undo_data, void **redo_data, int *redo_data_size)
  __attribute__ ((ALWAYS_INLINE));
...
static int heap_log_delete_physical (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p, VFID * vfid_p,
				     int offset, RECDES * recdes_p, bool mark_reusable,
...
static int heap_update_bigone (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context,
			       MVCCID mvccid, bool is_mvcc_op, HEAP_LOG_INFO * physical_update_log_info);
STATIC_INLINE int
heap_mvcc_update_relocation_prepare_log (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_page_fixed,
					 HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op, MVCCID mvccid,
					 LOG_RCVINDEX * rcv_index, int *slot_id_with_flags, RECDES ** undo_recdes,
					 RECDES ** redo_recdes, bool * update_old_home_p, bool * update_old_forward_p,
					 bool * remove_old_forward_p) __attribute__ ((ALWAYS_INLINE));
static int heap_update_relocation (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context,
				   bool is_mvcc_op, MVCCID mvccid, HEAP_LOG_INFO * physical_update_log_info);
STATIC_INLINE int heap_mvcc_home_update_prepare_log (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_page_fixed,
						     HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op,
						     MVCCID * mvccid, LOG_RCVINDEX * rcv_index,
						     int *slot_id_with_flags, RECDES ** undo_recdes,
						     RECDES ** redo_recdes);
__attribute__ ((ALWAYS_INLINE));
...
STATIC_INLINE int heap_create_log_node_for_update (THREAD_ENTRY * thread_p, LOG_TDES * tdes,
						   LOG_RCVINDEX rcv_index, VFID * vfid_p, PAGE_PTR page_p,
						   PGLENGTH offset, RECDES * old_recdes_p, RECDES * new_recdes_p,
						   LOG_PRIOR_NODE ** physical_update_log_node)
  __attribute__ ((ALWAYS_INLINE));
STATIC_INLINE int heap_create_update_log_info_before_page_fixing (THREAD_ENTRY * thread_p, LOG_TDES * tdes,
								  HEAP_OPERATION_CONTEXT * context,
								  MVCCID * mvccid, bool is_mvcc_op, char *bcb_area,
								  HEAP_LOG_INFO * update_log_info)
  __attribute__ ((ALWAYS_INLINE));
STATIC_INLINE int heap_log_update_physical (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p,
					    VFID * vfid_p, int offset, RECDES * old_recdes_p,
					    RECDES * new_recdes_p, LOG_RCVINDEX rcv_index,
					    HEAP_LOG_INFO * physical_update_log_info) __attribute__ ((ALWAYS_INLINE));
...
/*
 * heap_create_log_node_for_logical_deletion () - Creates log node for logical deletion
 *   return: error code
 *   thread_p(in): thread entry
 ...
 */
STATIC_INLINE int
heap_create_log_node_for_logical_deletion (THREAD_ENTRY * thread_p, LOG_TDES * tdes, LOG_RCVINDEX rcv_index,
					   VFID * vfid_p, PAGE_PTR page_p, PGLENGTH offset, char *redo_data_p,
...
/*
 * heap_create_log_node_for_update () - Creates log node for update
 *   return: error code
 *   thread_p(in): thread entry
 *   tdes(in): transaction descriptor
 *   rcv_index(in): recovery index
 ...
 *   page_p(in): page pointer
 *   offset(in): offset
 *   old_recdes_p(in): old record descriptor
 *   new_recdes_p(in): new record descriptor
 *   physical_update_log_node(out): physical update log node
 */
STATIC_INLINE int
heap_create_log_node_for_update (THREAD_ENTRY * thread_p, LOG_TDES * tdes, LOG_RCVINDEX rcv_index, VFID * vfid_p,
				 PAGE_PTR page_p, PGLENGTH offset, RECDES * old_recdes_p, RECDES * new_recdes_p,
				 LOG_PRIOR_NODE ** physical_update_log_node)
{
  LOG_DATA_ADDR log_addr;
  assert (tdes != NULL && vfid_p != NULL && physical_update_log_node != NULL);

  /* Populate address field with valid VFID. The offset and page pointer will be generated later */
  LOG_SET_DATA_ADDR (&log_addr, page_p, vfid_p, offset);

  assert (rcv_index == RVHF_UPDATE_NOTIFY_VACUUM || rcv_index == RVHF_UPDATE);
  return log_create_log_node_from_recdes (thread_p, tdes, rcv_index, &log_addr, old_recdes_p, new_recdes_p,
					  physical_update_log_node);
 ...
 *   update_log_info(in/out): update log info
 */
STATIC_INLINE int
heap_create_update_log_info_before_page_fixing (THREAD_ENTRY * thread_p, LOG_TDES * tdes,
						HEAP_OPERATION_CONTEXT * context, MVCCID * mvccid, bool is_mvcc_op,
						char *bcb_area, HEAP_LOG_INFO * update_log_info)
{
  LOG_PRIOR_NODE *update_log_node = NULL;
  RECDES *p_home_recdes_before_fix = NULL;
  bool has_home_record = false, has_forward_record = false;
  PAGE_PTR home_page_copy_before_fix = NULL;
  bool bcb_area_copied = false;
  RECDES *redo_data_p = NULL, *undo_data_p = NULL;
  int copy_retry_count = 1, copy_retry_max = 3;
  int flag_vacuum_status = 0, redo_data_size = 0, undo_data_size = 0;
  LOG_RCVINDEX rcv_index = RV_NOT_DEFINED;
  int rc;
  VPID vpid;
  PGSLOTID slot_id = NULL_SLOTID;
  int slot_id_with_flags;
  bool fits_in_home = false, fits_in_forward = false;
  bool update_old_home = true, update_old_forward = false, remove_old_forward = false;

  assert (thread_p != NULL && tdes != NULL && context != NULL && bcb_area != NULL && update_log_info != NULL);
  VPID_SET (&vpid, context->oid.volid, context->oid.pageid);

  if (heap_is_big_length (context->recdes_p->length))
    {
      /*
       * Currently, do not optimize this case since we don't know the redo data yet. The redo data (overflow OID)
       * is created later, after inserting BIGONE record. Here we can make an improvement later, by inserting BIGONE
...
try_copy_area_again:
  ...
  if (bcb_area_copied)
    {
      ...
      home_page_copy_before_fix = bcb_area;
      perfmon_inc_stat (thread_p, PSTAT_HEAP_NUM_COPY_NOLATCH_SUCCESS);
      pgbuf_copy_log ("pgbuf_copy_page: Successfully copied heap page (%d, %d) \n", vpid.volid, vpid.pageid);
    }
  else
   ...
   */
  if (home_page_copy_before_fix)
    {
      if (has_home_record == false)
	{
	  assert (vpid.pageid == context->oid.pageid && vpid.volid == context->oid.volid);
	  if (spage_get_record (home_page_copy_before_fix, context->oid.slotid, &context->home_recdes_before_page_fix,
				PEEK) != S_SUCCESS)
	    {
	      assert_release (false);
	      return ER_FAILED;
      ...
      else if (has_forward_record == false)
	{
	  /* peek the forward recdes */
	  assert (vpid.pageid == context->forward_oid.pageid && vpid.volid == context->forward_oid.volid);
	  if (spage_get_record (home_page_copy_before_fix, context->forward_oid.slotid,
				&context->home_recdes_before_page_fix, PEEK) != S_SUCCESS)
	    {
	      assert_release (false);
	      return ER_FAILED;
	    }
	  has_forward_record = true;
	}

      switch (context->home_recdes_before_page_fix.type)
	{
	case REC_HOME:
	  {
	    rc = heap_mvcc_home_update_prepare_log (thread_p, tdes, false, context, is_mvcc_op, mvccid, &rcv_index,
						    &slot_id_with_flags, &undo_data_p, &redo_data_p);
	    if (rc != NO_ERROR)
	      {
		return rc;
	      }
	...
	case REC_RELOCATION:
	  rc = heap_mvcc_update_relocation_prepare_log (thread_p, tdes, false, context, is_mvcc_op, *mvccid, &rcv_index,
							&slot_id_with_flags, &undo_data_p, &redo_data_p,
							&update_old_home, &update_old_forward, &remove_old_forward);
	  if (rc != NO_ERROR)
	    {
	      return rc;
	    }

	  /* check update_old_home */
	  if (has_forward_record == false)
	    {
	      assert (has_home_record == true);
	      assert (update_old_forward == false && remove_old_forward == false);
	      if (update_old_home == false)
		{
		  /* can't use home page for update */
		  COPY_OID (&context->forward_oid, (OID *) & context->home_recdes_before_page_fix.data);
		  vpid.volid = context->forward_oid.volid;
		  vpid.pageid = context->forward_oid.pageid;
	    ...
	    {
	      /* check update_old_forward */
	      assert (update_old_home == false && remove_old_forward == false);
	      if (update_old_forward == false)
		{
		  /* can't use forward page for update */
		  return NO_ERROR;
		}
	    }
	...
	case REC_BIGONE:
	  if (spage_is_updatable (thread_p, home_page_copy_before_fix, context->oid.slotid, context->recdes_p->length))
	    {
	      /* set recovery index */
	  ...
	  if (LOG_IS_MVCC_HEAP_OPERATION (rcv_index))
	    {
	      heap_page_update_chain_after_mvcc_op (thread_p, home_page_copy_before_fix, *mvccid, &flag_vacuum_status);
	    }
      ...
      if (rcv_index == RV_NOT_DEFINED)
	{
      ...
      rc = heap_create_log_node_for_update (thread_p, tdes, rcv_index, &context->hfid.vfid, home_page_copy_before_fix,
      ...
      if (rc != NO_ERROR)
	{
      ...
      HEAP_SET_LOG_INFO (update_log_info, update_log_node, rcv_index, &vpid, slot_id_with_flags,
...
#if defined(SERVER_MODE)
/*
 * heap_create_logical_delete_log_node_before_page_fixing () - Creates delete log node before fixing page
 *   return: error code
 *   thread_p(in): thread entry
 *   tdes(in): transaction descriptor
 *   context(in): operation context
 *   mvcc_id(in): MVCC id
 *   bcb_area(in): BCB area where the page is stored
 *   delete_log_info(in): delete log information
 */
STATIC_INLINE int
heap_create_delete_log_info_before_page_fixing (THREAD_ENTRY * thread_p, LOG_TDES * tdes,
						HEAP_OPERATION_CONTEXT * context, MVCCID mvccid, char *bcb_area,
						HEAP_LOG_INFO * delete_log_info)
{
  RECDES build_recdes;
  LOG_PRIOR_NODE *delete_log_node = NULL;
  bool has_home_record = false, has_forward_record = false;
  OID *forward_oid = NULL;
  PAGE_PTR home_page_copy_before_fix = NULL;
  bool bcb_area_copied = false;
  ...
  int flag_vacuum_status = 0, redo_data_size = 0, undo_data_size = 0;
  LOG_RCVINDEX rcv_index = RV_NOT_DEFINED;
  int rc = NO_ERROR, slot_id_with_flags;
  VPID vpid;
  PGSLOTID slot_id = -1;
  /* TO DO - added to structure */
  bool fits_in_home = false, fits_in_forward = false, is_adjusted_size_big = false;
  bool update_old_home = false, update_old_forward = false, remove_old_forward = false;

  assert (thread_p != NULL && tdes != NULL && context != NULL && bcb_area != NULL);
  VPID_SET (&vpid, context->oid.volid, context->oid.pageid);

  if (mvccid == MVCCID_NULL)
    {
      delete_log_info->node = NULL;
      return NO_ERROR;
    }
try_copy_area_again:
  /* Copy the heap page to BCB area. */
  ...
  if (vpid.volid == context->oid.volid && vpid.pageid == context->oid.pageid)
    {
      if (spage_get_record (bcb_area, context->oid.slotid, &context->home_recdes_before_page_fix, PEEK) != S_SUCCESS)
	{
	  return ER_FAILED;
	}
      context->home_page_copy_before_fix = bcb_area;
      if (context->home_recdes_before_page_fix.type == REC_RELOCATION
	  || context->home_recdes_before_page_fix.type == REC_BIGONE)
	{
	  forward_oid = (OID *) context->home_recdes_before_page_fix.data;
	  vpid.volid = forward_oid->volid;
	  vpid.pageid = forward_oid->pageid;
	  goto try_copy_area_again;
	}
    ...
    {
      if (spage_get_record (bcb_area, context->forward_oid.slotid, &context->forward_recdes_before_page_fix,
			    PEEK) != S_SUCCESS)
	{
	  assert_release (false);
	  return ER_FAILED;
	}
      context->forward_page_copy_before_fix = bcb_area;
    }


  HEAP_SET_RECORD (&build_recdes, IO_DEFAULT_PAGE_SIZE + OR_MVCC_MAX_HEADER_SIZE, 0, REC_UNKNOWN,
		   PTR_ALIGN (context->recdes_data_buffer, MAX_ALIGNMENT));
  context->recdes_p = &build_recdes;
  /*
   * Currently we create log node only if copy the page without fix. We can also create partially log node, even without
   * fixing the page. In that case, we will update the unknown log information after fixing the page.
   */
  switch (context->home_recdes_before_page_fix.type)
    {
    case REC_HOME:
      rc = heap_mvcc_delete_home_build_new_record (thread_p, tdes, mvccid, context);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  return rc;
      ...
      rc = heap_mvcc_delete_home_prepare_log (thread_p, tdes, false, context, mvccid, &rcv_index,
					      &slot_id_with_flags, &undo_data_p, &redo_data_p, &redo_data_size);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  return rc;
    ...
    case REC_RELOCATION:
      rc = heap_mvcc_delete_relocation_build_new_record (thread_p, tdes, mvccid, context, &is_adjusted_size_big,
							 &fits_in_home, &fits_in_forward);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  return rc;
      ...
      rc = heap_mvcc_delete_relocation_prepare_log (thread_p, tdes, false, is_adjusted_size_big, fits_in_home,
						    fits_in_forward, context, mvccid, &rcv_index,
						    &slot_id_with_flags, &undo_data_p, &redo_data_p, &redo_data_size,
						    &update_old_home, &update_old_forward, &remove_old_forward);
      if (rc != NO_ERROR)
	{
    ...
    case REC_BIGONE:
      //heap_prepare_log_for_bigone_delete
      ...
      undo_data_p = NULL;
      undo_data_size = 0;
      redo_data_p = PTR_ALIGN (delete_log_info->redo_data_buffer, MAX_ALIGNMENT);
      (void) or_pack_mvccid (redo_data_p, mvccid);
  ...
  if (rcv_index != RV_NOT_DEFINED)
    {
      /* TO DO - when to update chain - bigone, relocation? */
      rc =
	heap_create_log_node_for_logical_deletion (thread_p, tdes, RVHF_MVCC_DELETE_REC_HOME,
						   &context->hfid.vfid, home_page_copy_before_fix,
						   slot_id_with_flags, redo_data_p, redo_data_size, &delete_log_node);
      if (rc != NO_ERROR)
	{
	  return rc;
	}
...
/*
 * heap_create_log_node_for_physical_deletion () - Creates log node for physical deletion
 *   return: error code
 *   thread_p(in): thread entry
 *   tdes(in): transaction descriptor
 *   page_p(in): page pointer
 ...
 *   rcv_index(in): recovery index
 *   old_recdes_p(in): old record descriptor
 *   physical_delete_log_node(out): logical delete log node
 */
STATIC_INLINE int
heap_create_log_node_for_physical_deletion (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p,
					    VFID * vfid_p, PGLENGTH offset, LOG_RCVINDEX rcv_index,
					    RECDES * old_recdes_p, LOG_PRIOR_NODE ** physical_delete_log_node)
{
  LOG_DATA_ADDR log_addr;
  int error_code = NO_ERROR;
  ...
  assert (tdes != NULL && vfid_p != NULL && physical_delete_log_node != NULL);

  /* Populate address field with valid VFID. The offset and page pointer will be generated later */
  LOG_SET_DATA_ADDR (&log_addr, page_p, vfid_p, offset);

  HEAP_SET_PHYSYCAL_DELETE_UNDO_RECDES (undo_recdes_p, old_recdes_p, &temp_recdes, bytes_reserved);
  return log_create_log_node_from_recdes (thread_p, tdes, rcv_index, &log_addr, undo_recdes_p, NULL,
					  physical_delete_log_node);
}

/*
 * heap_mvcc_log_delete () - Log heap logical delete operation (just append delete MVCCID).
 * return		    : error code
 * thread_p (in)	    : Thread entry.
 * tdes(in)		    : Log tdes
 * mvcc_id(in)		    : MVCC id
		      ...
		      HEAP_LOG_INFO * logical_delete_log_info)
{
  ...
  HEAP_LOG_INFO local_logical_delete_log_info;
  int min_length;

  /* build address */
  LOG_SET_DATA_ADDR (&address, page_p, vfid_p, offset);
  if (logical_delete_log_info)
    {
      assert (rcv_index == RVHF_MVCC_DELETE_REC_HOME || rcv_index == RVHF_MVCC_DELETE_REC_NEWHOME
	      || rcv_index == RVHF_MVCC_DELETE_OVERFLOW);
       ...
       * and after fixing page is not different. Maybe an assert would be enough.
       */
      logical_delete_log_node = logical_delete_log_info->node;
      assert (redo_data_size <= OR_MVCCID_SIZE);

      /* Currently, if undo or redo data differs, create a new log node. */
      min_length = MIN (redo_data_size, OR_MVCC_MAX_HEADER_SIZE);
      if ((rcv_index == logical_delete_log_info->rcv_index)
	  && VPID_EQ (&logical_delete_log_info->vpid, pgbuf_get_vpid_ptr (page_p))
	  && (logical_delete_log_info->undo_data_p == NULL)
	  && (logical_delete_log_info->redo_data_p == redo_data_p
	      || (logical_delete_log_info->redo_data_size == redo_data_size
		  && memcmp ((char *) logical_delete_log_info->redo_data_p, redo_data_p, min_length) == 0)))
	{
	  if (logical_delete_log_info->slot_id_with_flags == offset)
	    {
	      /* The log created before page fixing can be safetely appended. */
	      assert (logical_delete_log_info->redo_data_p == NULL
		      || memcmp ((char *) logical_delete_log_info->redo_data_p, redo_data_p, redo_data_size) == 0);
	      goto append_log_node;
	    }

	  if (heap_rv_remove_flags_from_offset (offset)
	      == heap_rv_remove_flags_from_offset (logical_delete_log_info->slot_id_with_flags))
	    {
	      /* Needs to update the offset. */
	      assert (memcmp ((char *) logical_delete_log_info->redo_data_p, redo_data_p, redo_data_size) == 0);
	      error_code = prior_lsa_update_undoredo_data (thread_p, logical_delete_log_node, rcv_index, &address,
							   0, 0, NULL, NULL, true, true);
	      if (error_code == NO_ERROR)
		{
		  /* Successfully updated the offset. Append the log node. */
		  goto append_log_node;
		}
  ...
  /* TO DO - check physical deletion */
  error_code =
    heap_create_log_node_for_logical_deletion (thread_p, tdes, rcv_index, vfid_p, page_p, offset, redo_data_p,
					       redo_data_size, &logical_delete_log_node);
  if (error_code != NO_ERROR)
    {
      return error_code;
    }
  ...
  HEAP_SET_LOG_INFO (logical_delete_log_info, logical_delete_log_node, rcv_index, pgbuf_get_vpid_ptr (page_p),
		     offset, NULL, 0, redo_data_p, redo_data_size, false);
...
append_log_node:
  ...
  logical_delete_log_info->node_appended = true;
  return NO_ERROR;
}
 ...
 */
int
heap_set_autoincrement_value (THREAD_ENTRY * thread_p, HEAP_CACHE_ATTRINFO * attr_info,
			    ...
			    HEAP_CACHE_ATTRINFO * attr_info, OID * oid, const ATTR_ID att_id)
{
      ...
      if (error != NO_ERROR)
	{
	  ...
	  if (set_max_value)
	  ...
	  else
	    {
	      ...
	      error =
		db_value_domain_default (&(value->dbvalue), dest_type, dest_prec, dest_dom->scale,
					 dest_dom->codeset, dest_dom->collation_id, &dest_dom->enumeration);
	      if (error != NO_ERROR)
		{
		  /* this should not happen */
		  goto exit;
  ...
  error =
    locator_attribute_info_force (thread_p, &upd_scancache->node.hfid, oid, attr_info, atts_id,
				  updated_n_attrs_id, LC_FLUSH_UPDATE, SINGLE_ROW_UPDATE, upd_scancache,
				  &force_count, false, REPL_INFO_TYPE_RBR_NORMAL, DB_NOT_PARTITIONED_CLASS, NULL,
				  NULL, NULL, UPDATE_INPLACE_OLD_MVCCID, NULL, false);
  if (error != NO_ERROR)
    {
 ...
 */
static int
heap_eval_function_index (THREAD_ENTRY * thread_p, FUNCTION_INDEX_INFO * func_index_info, int n_atts,
			  int *att_ids, HEAP_CACHE_ATTRINFO * attr_info, RECDES * recdes, int btid_index,
			  DB_VALUE * result, FUNC_PRED_UNPACK_INFO * func_pred_cache, TP_DOMAIN ** fi_domain)
{
  int error = NO_ERROR;
  OR_INDEX *index = NULL;
  ...
  error =
    fetch_peek_dbval (thread_p, func_pred->func_regu, NULL, &cache_attr_info->class_oid,
 ...
 */
int
heap_init_func_pred_unpack_info (THREAD_ENTRY * thread_p, HEAP_CACHE_ATTRINFO * attr_info,
 ...
 */
SCAN_CODE
heap_next_record_info (THREAD_ENTRY * thread_p, const HFID * hfid, OID * class_oid, OID * next_oid,
		       RECDES * recdes, HEAP_SCANCACHE * scan_cache, int ispeeking, DB_VALUE ** cache_recordinfo)
{
 ...
 */
SCAN_CODE
heap_prev_record_info (THREAD_ENTRY * thread_p, const HFID * hfid, OID * class_oid, OID * next_oid,
		       RECDES * recdes, HEAP_SCANCACHE * scan_cache, int ispeeking, DB_VALUE ** cache_recordinfo)
{
...
/*
 * heap_mvcc_log_home_change_on_delete () - Log the change of record in home page when MVCC delete changes not only
 *					    MVCCID information in record.
 * returns	   : error code
 * thread_p (in)   : Thread entry.
 * tdes (in)	   : Transaction descriptor
 ...
 * new_recdes (in) : Record including delete info
 * logical_delete_log_info(in/out): Logical delete log info, obtained before page fixing
 */
STATIC_INLINE int
heap_mvcc_log_home_change_on_delete (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p, VFID * vfid_p,
				     int offset, RECDES * old_recdes, RECDES * new_recdes,
				     HEAP_LOG_INFO * logical_delete_log_info)
{
  LOG_DATA_ADDR address;
  int error_code = NO_ERROR;
  LOG_PRIOR_NODE *logical_delete_log_node = NULL;
  int min_undo_length, min_redo_length;
  HEAP_LOG_INFO local_logical_delete_log_info;
  ...
  if (logical_delete_log_info)
    {
      ...
      min_undo_length = MIN (old_recdes->length, OR_MVCC_MAX_HEADER_SIZE);
      min_redo_length = MIN (new_recdes->length, new_recdes->length);
      if ((logical_delete_log_info->rcv_index == RVHF_MVCC_DELETE_MODIFY_HOME)
	  && VPID_EQ (&logical_delete_log_info->vpid, pgbuf_get_vpid_ptr (page_p))
	  && (logical_delete_log_info->undo_data_p == old_recdes->data
	      || (logical_delete_log_info->undo_data_size == old_recdes->length
		  && memcmp ((char *) logical_delete_log_info->undo_data_p, old_recdes->data, min_undo_length) == 0))
	  && (logical_delete_log_info->redo_data_p == new_recdes->data
	      || (logical_delete_log_info->redo_data_size == new_recdes->length
		  && memcmp ((char *) logical_delete_log_info->redo_data_p, new_recdes->data, min_redo_length) == 0)))
	{
	  if (logical_delete_log_info->slot_id_with_flags == offset)
	    {
	      /* The log created before page fixing can be safetly appended. */
	      assert (logical_delete_log_info->undo_data_p == NULL
		      || memcmp ((char *) logical_delete_log_info->undo_data_p, old_recdes->data,
				 old_recdes->length) == 0);
	      assert (logical_delete_log_info->redo_data_p == NULL
		      || memcmp ((char *) logical_delete_log_info->redo_data_p, new_recdes->data,
				 min_redo_length) == 0);
	      goto append_log_node;
	    }
	  ...
	  if (heap_rv_remove_flags_from_offset (offset)
	      == heap_rv_remove_flags_from_offset (logical_delete_log_info->slot_id_with_flags))
	    {
	      assert (memcmp ((char *) logical_delete_log_info->undo_data_p, old_recdes->data, old_recdes->length) ==
		      0);
	      assert (memcmp ((char *) logical_delete_log_info->redo_data_p, new_recdes->data, min_redo_length) == 0);
	      error_code = prior_lsa_update_undoredo_data_from_recdes (thread_p, logical_delete_log_node,
								       RVHF_MVCC_DELETE_MODIFY_HOME, &address,
								       NULL, NULL, true, true);
	      if (error_code == NO_ERROR)
		{
  ...
  error_code = log_create_log_node_from_recdes (thread_p, tdes, RVHF_MVCC_DELETE_MODIFY_HOME, &address,
						old_recdes, new_recdes, &logical_delete_log_node);
  if (error_code != NO_ERROR)
    {
      return error_code;
    }
  ...
  HEAP_SET_LOG_INFO (logical_delete_log_info, logical_delete_log_node, RVHF_MVCC_DELETE_MODIFY_HOME,
		     pgbuf_get_vpid_ptr (page_p), offset, logical_delete_log_info->undo_data_p,
		     logical_delete_log_info->undo_data_size, logical_delete_log_info->redo_data_p,
		     logical_delete_log_info->redo_data_size, false);

append_log_node:
  assert (logical_delete_log_node != NULL && logical_delete_log_info != NULL);
  log_append_prior_node (thread_p, tdes, RVHF_MVCC_DELETE_MODIFY_HOME, page_p, logical_delete_log_node);
  logical_delete_log_info->node_appended = true;
...
/*
 * heap_mvcc_log_home_no_change () - Notify vacuum even when home page is not changed.
 ...
 */
STATIC_INLINE void
heap_mvcc_log_home_no_change (THREAD_ENTRY * thread_p, PAGE_PTR page_p, VFID * vfid_p, int offset)
{
  LOG_DATA_ADDR address;

  /* build address */
  LOG_SET_DATA_ADDR (&address, page_p, vfid_p, offset);
  log_append_undoredo_data (thread_p, RVHF_MVCC_NO_MODIFY_HOME, &address, 0, 0, NULL, NULL);
}
			    ...
			    HEAP_SCANCACHE * scan_cache, int *again_count_p, int again_max)
{
  ...
  *hdr_pgptr_p = heap_scan_pb_lock_and_fetch (thread_p, hdr_vpid_p, OLD_PAGE, X_LOCK, scan_cache, NULL);
  if (*hdr_pgptr_p == NULL)
	...
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_HEAP_UNKNOWN_OBJECT, 3, hdr_vpid_p->volid,
		  hdr_vpid_p->pageid, 0);
	  ...
	  if ((*again_count_p)++ >= again_max)
	    {
	      ...
	      if (error_code == ER_PB_BAD_PAGEID)
		{
		  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_HEAP_UNKNOWN_OBJECT, 3, oid_p->volid,
			  oid_p->pageid, oid_p->slotid);
		  error_code = ER_HEAP_UNKNOWN_OBJECT;
		}
	      else if (error_code == NO_ERROR)
		{
 ...
 */
STATIC_INLINE int
heap_try_fetch_header_with_forward_page (THREAD_ENTRY * thread_p, PAGE_PTR * home_pgptr_p,
					 const VPID * home_vpid_p, const OID * oid_p, PAGE_PTR * hdr_pgptr_p,
					 const VPID * hdr_vpid_p, PAGE_PTR * fwd_pgptr_p,
					 const VPID * fwd_vpid_p, const OID * fwd_oid_p,
					 HEAP_SCANCACHE * scan_cache, int *again_count_p, int again_max)
{
  int error_code = NO_ERROR;
...
heap_link_watchers (HEAP_OPERATION_CONTEXT * child, HEAP_OPERATION_CONTEXT * parent)
{
  ...
  child->new_forward_page_watcher_p = &parent->new_forward_page_watcher;
}
...
heap_unfix_watchers (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context)
{
  ...
  if (context->new_forward_page_watcher.pgptr != NULL)
    {
      pgbuf_ordered_unfix (thread_p, &context->new_forward_page_watcher);
    }
}
...
heap_clear_operation_context (HEAP_OPERATION_CONTEXT * context, HFID * hfid_p)
{
  ...
  PGBUF_INIT_WATCHER (&context->new_forward_page_watcher, PGBUF_ORDERED_HEAP_HDR, hfid_p);
  ...
  context->heap_log_info_p = NULL;
  context->home_recdes_before_page_fix.data = NULL;
  context->home_recdes_before_page_fix.length = 0;
  context->home_recdes_before_page_fix.area_size = 0;
  context->home_recdes_before_page_fix.type = REC_UNKNOWN;
...
heap_find_location_and_insert_rec_newhome (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context)
{
  ...
  if (heap_stats_find_best_page
      (thread_p, &context->hfid, context->recdes_p->length, false, context->recdes_p->length,
       context->scan_cache_p, context->home_page_watcher_p) == NULL)
    {
      ASSERT_ERROR_AND_SET (error_code);
      return error_code;
		     ...
		     OID * out_oid_p, PGBUF_WATCHER * newhome_pg_watcher)
{
  ...
  HEAP_LOG_INFO physical_insert_log_info, *p_physical_insert_log_info = NULL;
  int error_code = NO_ERROR;
  LOG_TDES *tdes = NULL;
  LOG_RCVINDEX rcv_index = 0;

  /* check input */
  assert (recdes_p != NULL && recdes_p->type == REC_NEWHOME);
  ...
  error_code = heap_create_insert_log_info_before_page_fixing (thread_p, tdes, &(ins_context.hfid.vfid),
							       ins_context.recdes_p, false, false,
							       &physical_insert_log_info);
  if (error_code != NO_ERROR)
    {
      goto exit;
    }
  ...
  if (physical_insert_log_info.node != NULL)
    {
   ...
   * Since is not MVCC operation, no need to update VACUUM status.
   * Operation logging.
   */
  error_code =
    heap_log_insert_physical (thread_p, tdes, ins_context.home_page_watcher_p->pgptr, &(ins_context.hfid.vfid),
			      ins_context.res_oid.slotid, ins_context.recdes_p, false, false,
			      p_physical_insert_log_info);
  if (error_code != NO_ERROR)
    {
      ASSERT_ERROR ();
      goto exit;
...
exit:
  ...
  if (p_physical_insert_log_info != NULL && p_physical_insert_log_info->node_appended == false)
    {
      assert (p_physical_insert_log_info->node != NULL);
      prior_lsa_free_node (p_physical_insert_log_info->node);
    }
...
/*
 /*
 * heap_create_log_node_for_physical_insertion () - Creates log node for physical insertion
 *   return: error code
 *   thread_p(in): thread entry
 *   tdes(in): transaction descriptor
 *   page_p(in): page pointer
 *   vfid_p(in): virtual file identifier
 *   offset(in): offset
 *   p_recdes(in): the record that will be inserted
 ...
 *   rcv_index(out): recovery index 
 *   physical_insert_log_node(out): logical delete log node
 */
int
heap_create_log_node_for_physical_insertion (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p,
					     VFID * vfid_p, PGLENGTH offset, RECDES * p_recdes, bool is_mvcc_op,
					     bool is_redistribute_op, LOG_RCVINDEX * rcv_index,
					     LOG_PRIOR_NODE ** physical_insert_log_node)
{
  LOG_DATA_ADDR log_addr;
  INT16 bytes_reserved;
  RECDES temp_recdes;
...
#define HEAP_LOG_MVCC_REDISTRIBUTE_MAX_REDO_CRUMBS	    4
	  ...
	  *rcv_index = RVHF_MVCC_REDISTRIBUTE;
	  ...
	  rc = log_create_log_node_from_undoredo_crumbs (thread_p, tdes, RVHF_MVCC_REDISTRIBUTE, &log_addr, 0,
							 n_redo_crumbs, NULL, redo_crumbs, physical_insert_log_node);
	  if (rc != NO_ERROR)
	    {
	      return rc;
	    }
...
#define HEAP_LOG_MVCC_INSERT_MAX_REDO_CRUMBS	    4
	  ...
	  *rcv_index = RVHF_MVCC_INSERT;
	  ...
	  rc = log_create_log_node_from_undoredo_crumbs (thread_p, tdes, RVHF_MVCC_INSERT, &log_addr, 0,
							 n_redo_crumbs, NULL, redo_crumbs, physical_insert_log_node);
	  if (rc != NO_ERROR)
	    {
	      return rc;
	...
	{
	  /* set recovery index */
	  *rcv_index = RVHF_INSERT;

	  /* special case for REC_ASSIGN */
	  bytes_reserved = (INT16) p_recdes->length;
	  HEAP_SET_RECORD (&temp_recdes, sizeof (bytes_reserved), sizeof (bytes_reserved), p_recdes->type,
			   (char *) &bytes_reserved);

	  rc = log_create_log_node_from_recdes (thread_p, tdes, RVHF_INSERT, &log_addr, NULL, &temp_recdes,
						physical_insert_log_node);
	  if (rc != NO_ERROR)
	    {
	      return NO_ERROR;
	    }
	...
	{
	  /* set recover index */
	  *rcv_index = RVHF_INSERT_NEWHOME;
	  /* replication for REC_NEWHOME is performed by following the link (OID) from REC_RELOCATION */
	  rc = log_create_log_node_from_recdes (thread_p, tdes, RVHF_INSERT_NEWHOME, &log_addr, NULL, p_recdes,
						physical_insert_log_node);
	  if (rc != NO_ERROR)
	    {
	      return NO_ERROR;
	...
	{
	  *rcv_index = RVHF_INSERT;
	  rc = log_create_log_node_from_recdes (thread_p, tdes, RVHF_INSERT, &log_addr, NULL, p_recdes,
						physical_insert_log_node);
	  if (rc != NO_ERROR)
	    {
	      return NO_ERROR;
 ...
 *       be provided later, after insertion in heap page, when need to append log node.
 */
STATIC_INLINE int
heap_create_insert_log_info_before_page_fixing (THREAD_ENTRY * thread_p, LOG_TDES * tdes, VFID * vfid_p,
						RECDES * p_recdes, bool is_mvcc_op, bool is_redistribute_op,
						HEAP_LOG_INFO * insert_log_info)
{
  int rc;
  RECDES *undo_data_p = NULL, *redo_data_p = NULL;
  int undo_data_size = 0, redo_data_size = 0;
  ...
  /* we don't know yet the page id, will be set later, after finding page for insert */
  VPID_SET_NULL (&vpid);

  /* we don't know yet the slot, will be set after page fixing */
  slotid = NULL_SLOTID;
  ...
  undo_data_p = NULL;
  undo_data_size = 0;
  redo_data_p = (void *) p_recdes;
  redo_data_size = sizeof (p_recdes);

  /* create log node and get recovery index */
  rc = heap_create_log_node_for_physical_insertion (thread_p, tdes, page, vfid_p, slotid, redo_data_p, is_mvcc_op,
						    is_redistribute_op, &rcv_index, &insert_log_node);
  if (rc != NO_ERROR)
    {
      return rc;
    }
  ...
  /* set  log info */
  HEAP_SET_LOG_INFO (insert_log_info, insert_log_node, rcv_index, &vpid, slotid, undo_data_p, undo_data_size,
		     redo_data_p, redo_data_size, false);
 ...
 *   is_mvcc_op(in): specifies type of operation (MVCC/non-MVCC)
 *   is_redistribute_op(in): whether the insertion is due to partition redistribute operation and has a valid delid

 *   physical_insert_log_info(in/out): physical insert log info, obtained before page fixing
 */
STATIC_INLINE int
heap_log_insert_physical (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p, VFID * vfid_p, int offset,
			  RECDES * recdes_p, bool is_mvcc_op, bool is_redistribute_op,
			  HEAP_LOG_INFO * physical_insert_log_info)
{
  int error_code;
  LOG_PRIOR_NODE *physical_insert_log_node = NULL;
  HEAP_LOG_INFO local_physical_insert_log_info;
  int min_length;
  RECDES *p_update_log_redo_recdes = NULL;
  LOG_RCVINDEX rcv_index;

  assert (page_p != NULL && recdes_p != NULL);
  if (physical_insert_log_info != NULL)
    {
      LOG_DATA_ADDR address;
      LOG_SET_DATA_ADDR (&address, page_p, vfid_p, offset);

      /* Log node was previously created, updates its log header data. */
      assert (physical_insert_log_info->node != NULL && physical_insert_log_info->node_appended == false);
      rcv_index = physical_insert_log_info->rcv_index;
      assert (rcv_index == RVHF_MVCC_INSERT || rcv_index == RVHF_INSERT || rcv_index == RVHF_INSERT_NEWHOME
	      || rcv_index == RVHF_MVCC_REDISTRIBUTE);

      /* Currently, if undo or redo data differs, create a new log node. */
      physical_insert_log_node = physical_insert_log_info->node;
      p_update_log_redo_recdes = (RECDES *) physical_insert_log_info->redo_data_p;
      min_length = MIN (recdes_p->length, OR_MVCC_DELETE_ID_SIZE);
      if ((physical_insert_log_info->undo_data_p == NULL)
	  && (p_update_log_redo_recdes->data == recdes_p->data
	      || (p_update_log_redo_recdes->length == recdes_p->length
		  && memcmp (p_update_log_redo_recdes->data, recdes_p->data, min_length) == 0)))
	{
	  if (physical_insert_log_info->slot_id_with_flags == offset
	      && VPID_EQ (&physical_insert_log_info->vpid, pgbuf_get_vpid_ptr (page_p)))
	    {
	      assert (p_update_log_redo_recdes->data == NULL
		      || memcmp (p_update_log_redo_recdes->data, recdes_p->data, recdes_p->length) == 0);
	      goto append_log_node;
	    }
	  ...
	  assert (p_update_log_redo_recdes->data == NULL
		  || memcmp (p_update_log_redo_recdes->data, recdes_p->data, recdes_p->length) == 0);
	  error_code =
	    prior_lsa_update_undoredo_data (thread_p, physical_insert_log_node, rcv_index, &address, 0, 0,
					    NULL, NULL, true, true);
	  if (error_code == NO_ERROR)
	    {
	      /* Successfully updated undo/redo data. Append the log node. */
	  ...
	  if (er_errid () == ER_INCOMPATIBLE_LOG_TYPE)
	    {			/* TO DO - check incompatible type */
	      /* Need to add a new log node. The caller has to clean the old log node. */
	      er_clear ();
	      error_code = NO_ERROR;
	      physical_insert_log_node = NULL;
	    }
	  else
	    {
	      return error_code;
  ...
  error_code =
    heap_create_log_node_for_physical_insertion (thread_p, tdes, page_p, vfid_p, offset, recdes_p, is_mvcc_op,
						 is_redistribute_op, &rcv_index, &physical_insert_log_node);
  if (error_code != NO_ERROR)
    {
      return error_code;
    }
  ...
  HEAP_SET_LOG_INFO (physical_insert_log_info, physical_insert_log_node, rcv_index, pgbuf_get_vpid_ptr (page_p),
		     offset, NULL, 0, recdes_p->data, recdes_p->length, false);

append_log_node:
  assert (physical_insert_log_node != NULL && physical_insert_log_info != NULL);
  log_append_prior_node (thread_p, tdes, rcv_index, page_p, physical_insert_log_node);
  physical_insert_log_info->node_appended = true;
 ...
 *   context(in): operation context
 *   is_mvcc_op(in): specifies type of operation (MVCC/non-MVCC)
 *   mvcc_id(in): current transaction MVCCID
 *   delete_log_info(in/out): delete log info
 */
static int
heap_delete_bigone (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op,
		    MVCCID mvcc_id, HEAP_LOG_INFO * delete_log_info)
{
  ...
  if (is_mvcc_op)
	  ...
	  != NO_ERROR)
	{
      ...
      /* log operation. */
      rc =
	heap_mvcc_log_delete (thread_p, tdes, mvcc_id, context->overflow_page_watcher_p->pgptr,
			      &context->hfid.vfid, overflow_oid.slotid | flag_vacuum_status,
			      RVHF_MVCC_DELETE_OVERFLOW, redo_data_p, OR_MVCCID_SIZE, delete_log_info);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  return rc;
      ...
      if (rc != NO_ERROR)
	{
      ...
      heap_mvcc_log_home_no_change (thread_p, context->home_page_watcher_p->pgptr, &context->hfid.vfid,
      ...
      if (context->home_page_watcher_p->page_was_unfixed)
	{
      ...
      /* log operation */
      rc = heap_log_delete_physical (thread_p, tdes, context->home_page_watcher_p->pgptr, &context->hfid.vfid,
				     context->oid.slotid, &context->home_recdes, is_reusable, delete_log_info, NULL);
      if (rc != NO_ERROR)
	{
	  return rc;
	}
      ...
      rc = heap_delete_physical (thread_p, &context->hfid, context->home_page_watcher_p->pgptr, context->oid.slotid);
      if (rc != NO_ERROR)
	{
	  return rc;
	}
...
/*
 * heap_mvcc_build_delete_new_record () - build mvcc delete new record
 *   returns: error code
 *   thread_p(in): thread entry
 *   tdes(in): transaction descriptor
 *   mvcc_id(in): current transaction MVCCID
 *   context(in/out): operation context
 *   is_adjusted_size_big(out):  true, if adjusted size is big
 *   fits_in_home(out): true, if fits in home
 *   fits_in_forward(out): true, if fits in forward
 *
 *    Note: This function builds context->recdes_p, that is used at mvcc delete to update the old recdes.
 *	Thus, the context->recdes_p is built by copying context->home_recdes and adding DELID. Before calling this
 *	function, context->recdes_p->area must be allocated.
 */
STATIC_INLINE int
heap_mvcc_delete_relocation_build_new_record (THREAD_ENTRY * thread_p, LOG_TDES * tdes, MVCCID mvcc_id,
					      HEAP_OPERATION_CONTEXT * context, bool * is_adjusted_size_big,
					      bool * fits_in_home, bool * fits_in_forward)
{
  ...
  assert (MVCCID_IS_VALID (mvcc_id) && is_adjusted_size_big != NULL && fits_in_home != NULL && fits_in_forward != NULL);

  repid_and_flag_bits = OR_GET_MVCC_REPID_AND_FLAG (context->forward_recdes.data);
  mvcc_flags = (repid_and_flag_bits >> OR_MVCC_FLAG_SHIFT_BITS) & OR_MVCC_FLAG_MASK;
  adjusted_size = context->forward_recdes.length;
  ...
  if (context->home_page_watcher_p && context->home_page_watcher_p->pgptr != NULL)
    {
      home_page_ptr = context->home_page_watcher_p->pgptr;
    }
  else
    {
      home_page_ptr = context->home_page_copy_before_fix;
    }

  if (context->forward_page_watcher_p && context->forward_page_watcher_p->pgptr != NULL)
    {
  ...
  *fits_in_home = spage_is_updatable (thread_p, home_page_ptr, context->oid.slotid, adjusted_size);
  *fits_in_forward = spage_is_updatable (thread_p, forward_page_ptr, context->forward_oid.slotid, adjusted_size);
  if ((*is_adjusted_size_big) || (!(*fits_in_forward) && !(*fits_in_home)))
	  ...
	  && context->forward_page_watcher_p->page_was_unfixed)
	{
	  ...
	  forward_page_ptr = context->forward_page_watcher_p->pgptr;
	  if (spage_get_record
	      (forward_page_ptr, context->forward_oid.slotid, &context->forward_recdes, PEEK) != S_SUCCESS)
	    {
	      return ER_FAILED;
	    }
	  ...
	  if (mvcc_flags != ((repid_and_flag_bits >> OR_MVCC_FLAG_SHIFT_BITS) & OR_MVCC_FLAG_MASK))
	      ...
	      if (*is_adjusted_size_big)
		{
		  /* not exactly necessary, but we'll be able to compare sizes */
		  adjusted_size =
		    context->forward_recdes.length - mvcc_header_size_lookup[mvcc_flags] + OR_MVCC_MAX_HEADER_SIZE;
		}
#endif
	    }
	}
  ...
  if (use_optimization)
    {
      ...
      build_recdes_data = start_p = context->recdes_p->data;

      /* Copy up to MVCC DELID first. */
      memcpy (build_recdes_data, context->forward_recdes.data, delid_offset);
      ...
      else
	{
      ...
      memcpy (build_recdes_data, context->forward_recdes.data + delid_offset,
	      context->forward_recdes.length - delid_offset);
      context->recdes_p->length = adjusted_size;
    }
  else
    {
       ...
       */
      if (or_mvcc_get_header (&context->forward_recdes, &forward_rec_header) != NO_ERROR)
	{
	  return ER_FAILED;
	}
      ...
      memcpy (context->recdes_p->data + context->recdes_p->length,
	      context->forward_recdes.data + forward_rec_header_size,
	      context->forward_recdes.length - forward_rec_header_size);
      context->recdes_p->length += context->forward_recdes.length - forward_rec_header_size;
      assert (context->recdes_p->length == adjusted_size);
    }
...
/*
 * heap_mvcc_delete_home_prepare_log () - prepare log for MVCC home record deletion
 *   thread_p(in): thread entry
 *   tdes(in): transaction descriptor
 *   is_page_fixed(in): true, if the page is fixed
 *   context(in): operation context
 *   is_mvcc_op(in): type of operation (MVCC/non-MVCC)
 *   mvccid(in/out): current transaction MVCCID
 *   rcv_index(out): recovery index
 *   slot_id_with_flags(out): slot id with flags
 *   undo_recdes(out): undo recdes
 *   redo_recdes(out): redo recdes
 *   physical_update_log_info(in/out): physical update log info, obtained before page fixing
 *
 *  Note: This function set flag vacuum status into page as a side effect.
 */
STATIC_INLINE int
heap_mvcc_delete_relocation_prepare_log (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_page_fixed,
					 bool is_adjusted_size_big, bool fits_in_home, bool fits_in_forward,
					 HEAP_OPERATION_CONTEXT * context, MVCCID mvccid, LOG_RCVINDEX * rcv_index,
					 int *slot_id_with_flags /* TO DO - slotid */ , void **undo_data,
					 void **redo_data, int *redo_data_size, bool * update_old_home,
					 bool * update_old_forward, bool * remove_old_forward)
{
...
/* check whether relocation is necessary */
  *redo_data_size = 0;
  *update_old_home = *update_old_forward = *remove_old_forward = false;
  if (is_page_fixed && is_adjusted_size_big)
    {
      ...
      if (heap_ovf_insert (thread_p, &context->hfid, &context->new_forward_oid, context->recdes_p) == NULL)
	{
      ...
      heap_build_forwarding_recdes (&context->new_home_recdes, REC_BIGONE, &context->new_forward_oid);
      ...
      if (context->home_page_watcher_p->page_was_unfixed)
	{
      ...
      /*
       * Delete old forward record. Re-peek forward record descriptor; forward page may have been unfixed
       * by previous pgbuf_ordered_fix() call.
       */
      if (context->forward_page_watcher_p->page_was_unfixed)
	{
	  if (spage_get_record (context->forward_page_watcher_p->pgptr, context->forward_oid.slotid,
      ...
      *rcv_index = RVHF_MVCC_DELETE_MODIFY_HOME;
      *slot_id_with_flags = context->oid.slotid;
      *undo_data = &context->home_recdes;
      *redo_data = &context->new_home_recdes;
      return NO_ERROR;
    }
  ...
  if (fits_in_home)
  ...
  else if (fits_in_forward)
    {
      ...
      *rcv_index = RVHF_MVCC_DELETE_REC_NEWHOME;
      *slot_id_with_flags = context->new_forward_oid.slotid;
      *undo_data = &context->home_recdes;
      *redo_data = redo_data_p;
      *redo_data_size = OR_MVCCID_SIZE;

      perfmon_inc_stat (thread_p, PSTAT_HEAP_REL_MVCC_DELETES);
    }
  else if (is_page_fixed)
    {
      /* doesn't fit in either home or forward page */
      /* insert a new forward record */
      context->recdes_p->type = REC_NEWHOME;
      if (heap_insert_newhome (thread_p, context, context->recdes_p, &context->new_forward_oid, NULL) != NO_ERROR)
	{
	...
	{
	  /* 
	   * Need to get the record again, since record may have changed by other transactions (INSID removed
	   * by VACUUM, page compact). The object was already locked, so the record size may be the same or
	   * smaller (INSID removed by VACUUM). 
	   */
	  int is_peeking = (context->home_recdes.area_size >= context->home_recdes.length) ? COPY : PEEK;
	  if (spage_get_record
 ...
 */
static int
heap_delete_relocation (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context,
			bool is_mvcc_op, MVCCID mvcc_id, HEAP_LOG_INFO * delete_log_info)
{
  RECDES build_recdes;
  bool fits_in_home, fits_in_forward;
  bool update_old_home = false, update_old_forward = false, remove_old_forward = false, is_adjusted_size_big = false;
  int rc, flag_vacuum_status = 0, slot_id_with_flags = 0;
  ...
  /* get forward oid */
  context->forward_oid = *((OID *) context->home_recdes.data);

  /* fix forward page */
  if (heap_fix_forward_page (thread_p, context, &context->forward_oid) != NO_ERROR)
    {
      return ER_FAILED;
    }

  /* get forward record */
  if (spage_get_record (context->forward_page_watcher_p->pgptr, context->forward_oid.slotid, &context->forward_recdes,
			PEEK) != S_SUCCESS)
    {
      return ER_FAILED;
    }
  ...
  if (is_mvcc_op)
    {
      HEAP_SET_RECORD (&build_recdes, IO_DEFAULT_PAGE_SIZE + OR_MVCC_MAX_HEADER_SIZE, 0, REC_UNKNOWN,
		       PTR_ALIGN (context->recdes_data_buffer, MAX_ALIGNMENT));
      context->recdes_p = &build_recdes;
      rc = heap_mvcc_delete_relocation_build_new_record (thread_p, tdes, mvcc_id, context, &is_adjusted_size_big,
							 &fits_in_home, &fits_in_forward);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
      ...
      /* TO DO - add new members in context */
      rc = heap_mvcc_delete_relocation_prepare_log (thread_p, tdes, true, is_adjusted_size_big, fits_in_home,
						    fits_in_forward, context, mvcc_id, &rcv_index,
						    &slot_id_with_flags, &old_data, &new_data, &new_data_size,
						    &update_old_home, &update_old_forward, &remove_old_forward);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	...
	{
	  /* log operation */
	  heap_page_update_chain_after_mvcc_op (thread_p, context->home_page_watcher_p->pgptr, mvcc_id,
						&flag_vacuum_status);
	  rc = heap_mvcc_log_home_change_on_delete (thread_p, tdes, context->home_page_watcher_p->pgptr,
						    ...
						    (RECDES *) old_data, (RECDES *) new_data, NULL);
	  if (rc != NO_ERROR)
	    {
	      ASSERT_ERROR ();
	      return rc;
      ...
      if (update_old_forward)
	{
	  ...
	  rc = heap_mvcc_log_delete (thread_p, tdes, mvcc_id, context->forward_page_watcher_p->pgptr,
				     &context->hfid.vfid, context->forward_oid.slotid, rcv_index,
				     new_data, new_data_size, delete_log_info);
	  if (rc != NO_ERROR)
	    {
	      ASSERT_ERROR ();
	      return rc;
	  ...
	  rc =
	    heap_update_physical (thread_p, context->forward_page_watcher_p->pgptr, context->forward_oid.slotid,
				  context->recdes_p);
	  if (rc != NO_ERROR)
	    {
	      return rc;
	...
	{
	  /* Remove old forward. Probably will be better to be done by VACUUM. */
	  /* operation logging */
	  LOG_DATA_ADDR forward_addr;
	  LOG_SET_DATA_ADDR (&forward_addr, context->forward_page_watcher_p->pgptr, &context->hfid.vfid,
			     context->forward_oid.slotid);
	  log_append_undoredo_recdes (thread_p, RVHF_DELETE, &forward_addr, &context->forward_recdes, NULL);
	  ...
	  rc = heap_delete_physical (thread_p, &context->hfid, context->forward_page_watcher_p->pgptr,
				     context->forward_oid.slotid);
	  if (rc != NO_ERROR)
	    {
	      return rc;
      ...
      if (context->forward_page_watcher_p->page_was_unfixed)
	{
       ...
       */
      rc = heap_log_delete_physical (thread_p, tdes, context->forward_page_watcher_p->pgptr, &context->hfid.vfid,
					...
					HEAP_OPERATION_CONTEXT * context)
{
  ...
  char *start_p = NULL, *build_recdes_data = NULL;
  PAGE_PTR page_ptr = NULL;
  int adjusted_size, delid_offset, repid_and_flag_bits, mvcc_flags;
  bool is_adjusted_size_big = false;
  bool use_optimization;
  int error_code = NO_ERROR;

  assert (context->recdes_p != NULL && context->recdes_p->data != NULL && context->recdes_p->area_size > 0);
  ...
  if (use_optimization)
      ...
      else
	{
      ...
      memcpy (build_recdes_data, context->home_recdes.data + delid_offset, context->home_recdes.length - delid_offset);
      context->recdes_p->length = adjusted_size;
    }
  else
    {
      ...
      if (error_code != NO_ERROR)
	{
      ...
      memcpy (context->recdes_p->data + context->recdes_p->length, context->home_recdes.data + header_size,
	      context->home_recdes.length - header_size);
      context->recdes_p->length += (context->home_recdes.length - header_size);
      assert (context->recdes_p->length == adjusted_size);
    }
  ...
  if (context->home_page_watcher_p && context->home_page_watcher_p->pgptr != NULL)
    ...
    {
      page_ptr = context->home_page_copy_before_fix;
    }
  ...
  else if (!spage_is_updatable (thread_p, page_ptr, context->oid.slotid, context->recdes_p->length))
    {
      context->recdes_p->type = REC_NEWHOME;
    }
  else
 ...
 */
STATIC_INLINE int
heap_mvcc_delete_home_prepare_log (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_page_fixed,
				   HEAP_OPERATION_CONTEXT * context, MVCCID mvccid, LOG_RCVINDEX * rcv_index,
				   int *slot_id_with_flags, void **undo_data, void **redo_data, int *redo_data_size)
{
  INT16 type;
  ...
  *rcv_index = RV_NOT_DEFINED;
/* check whether relocation is necessary */

  if (is_page_fixed && (context->recdes_p->type == REC_BIGONE || context->recdes_p->type == REC_NEWHOME))
    {
      /* 
       * Relocation necessary
       */
      /* insertion of built record */
      if (context->recdes_p->type == REC_BIGONE)
	{
	  /* new record is overflow record - REC_BIGONE case */
	  type = REC_BIGONE;
	  if (heap_ovf_insert (thread_p, &context->hfid, &context->forward_oid, context->recdes_p) == NULL)
	    {
	      ASSERT_ERROR_AND_SET (error_code);
	      return error_code;
	    }
      ...
      else
	{
	  ...
	  error_code = heap_insert_newhome (thread_p, context, context->recdes_p, &context->forward_oid, NULL);
	  if (error_code != NO_ERROR)
	    {
	      ASSERT_ERROR ();
	      return error_code;
      ...
      heap_build_forwarding_recdes (&context->new_home_recdes, type, &context->forward_oid);
      ...
      if (context->home_page_watcher_p->page_was_unfixed)
	{
      ...
      *slot_id_with_flags = context->oid.slotid | flag_vacuum_status;

      *undo_data = &context->home_recdes;
      *redo_data = &context->new_home_recdes;
      *redo_data_size = sizeof (RECDES *);
    }
  else
    {
      char *page_ptr;
 ...
 *   is_mvcc_op(in): specifies type of operation (MVCC/non-MVCC)
 *   mvcc_id(in): current transaction MVCCID
 *   delete_log_info(in/out): delete log info, obtained before page fixing
 */
static int
heap_delete_home (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op,
		  MVCCID mvcc_id, HEAP_LOG_INFO * delete_log_info)
{
  void *undo_recdes = NULL, *redo_recdes = NULL;
  RECDES *home_page_updated_recdes, build_recdes;
  char data_buffer[IO_DEFAULT_PAGE_SIZE + OR_MVCC_MAX_HEADER_SIZE + MAX_ALIGNMENT];
  LOG_RCVINDEX rcv_index = RV_NOT_DEFINED;
  bool is_adjusted_size_big = false, use_optimization = false;
  int slot_id_with_flags = 0, redo_data_size = 0;
  int error_code = NO_ERROR, flag_vacuum_status = 0;
  ...
  if (context->home_page_watcher_p->page_was_unfixed)
    {
      ...
      int is_peeking = (context->home_recdes.area_size >= context->home_recdes.length) ? COPY : PEEK;
      if (spage_get_record
	  (context->home_page_watcher_p->pgptr, context->oid.slotid, &context->home_recdes, is_peeking) != S_SUCCESS)
	{
	  assert (false);
	  return ER_FAILED;
	}
  ...
  if (is_mvcc_op)
    {
      assert (MVCCID_IS_VALID (mvcc_id));
      /* Create the new record - context->recdes_p */
      HEAP_SET_RECORD (&build_recdes, IO_DEFAULT_PAGE_SIZE + OR_MVCC_MAX_HEADER_SIZE, 0, REC_UNKNOWN,
		       PTR_ALIGN (data_buffer, MAX_ALIGNMENT));
      context->recdes_p = &build_recdes;
      error_code = heap_mvcc_delete_home_build_new_record (thread_p, tdes, mvcc_id, context);
      if (error_code != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  return error_code;
      ...
      /* Create log data. */
      error_code = heap_mvcc_delete_home_prepare_log (thread_p, tdes, true, context, mvcc_id, &rcv_index,
						      &slot_id_with_flags, &undo_recdes, &redo_recdes, &redo_data_size);
      if (error_code != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  return error_code;
	...
	case RVHF_MVCC_DELETE_REC_HOME:
	  ...
	  error_code = heap_mvcc_log_delete (thread_p, tdes, mvcc_id, context->home_page_watcher_p->pgptr,
					     &context->hfid.vfid, slot_id_with_flags, RVHF_MVCC_DELETE_REC_HOME,
					     (char *) redo_recdes, redo_data_size, delete_log_info);
	  if (error_code != NO_ERROR)
	    {
	      ASSERT_ERROR ();
	      return error_code;
	  ...
	  home_page_updated_recdes = context->recdes_p;
	  break;
	...
	case RVHF_MVCC_DELETE_MODIFY_HOME:
	  error_code =
	    heap_mvcc_log_home_change_on_delete (thread_p, tdes, context->home_page_watcher_p->pgptr,
						 &context->hfid.vfid, context->oid.slotid | flag_vacuum_status,
						 (RECDES *) undo_recdes, (RECDES *) redo_recdes, delete_log_info);
	  if (error_code != NO_ERROR)
	    {
	      ASSERT_ERROR ();
	      return error_code;
	  ...
	  home_page_updated_recdes = redo_recdes;
	  break;
      ...
      if (error_code != NO_ERROR)
	{
      ...
      /* physical deletion */
      error_code = heap_delete_physical (thread_p, &context->hfid, context->home_page_watcher_p->pgptr,
					 context->oid.slotid);
 ...
 */
static int
heap_delete_physical (THREAD_ENTRY * thread_p, HFID * hfid_p, PAGE_PTR page_p, PGSLOTID slotid)
{
  int free_space;
 ...
 */
static int
heap_log_delete_physical (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p, VFID * vfid_p, int offset,
			  RECDES * recdes_p, bool mark_reusable, HEAP_LOG_INFO * physical_delete_log_info,
			  LOG_LSA * undo_lsa)
{
  LOG_DATA_ADDR address;
  ...
  if (physical_delete_log_info)
    {
      ...
      assert (physical_delete_log_info->rcv_index == RVHF_DELETE);
      assert (physical_delete_log_node->rlength == 0);
      ...
      if (VPID_EQ (&physical_delete_log_info->vpid, pgbuf_get_vpid_ptr (page_p))
	  && (physical_delete_log_info->undo_data_p == recdes_p->data
	      || (physical_delete_log_info->undo_data_size == recdes_p->length
		  && memcmp (physical_delete_log_info->undo_data_p, recdes_p->data, min_length) == 0)))
	{
	  if (physical_delete_log_info->slot_id_with_flags == offset)
	    {
	      assert (physical_delete_log_info->undo_data_p == NULL
		      || memcmp (physical_delete_log_info->undo_data_p, recdes_p->data, recdes_p->length) == 0);
	      goto append_log_node;
	    }

	  if (heap_rv_remove_flags_from_offset (offset)
	      == heap_rv_remove_flags_from_offset (physical_delete_log_info->slot_id_with_flags))
	    {
	      assert (memcmp (physical_delete_log_info->undo_data_p, recdes_p->data, recdes_p->length) == 0);
	      error_code =
  ...
  HEAP_SET_LOG_INFO (physical_delete_log_info, physical_delete_log_node, RVHF_DELETE, pgbuf_get_vpid_ptr (page_p),
		     offset, NULL, 0, recdes_p->data, recdes_p->length, false);
 ...
 *   tdes(in): transaction descriptor
 *   context(in): operation context
 *   mvccid(in): current transaction MVCCID
 *   is_mvcc_op(in): type of operation (MVCC/non-MVCC)
 *   physical_update_log_info(in/out): physical update log info
 */
static int
heap_update_bigone (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context, MVCCID mvccid,
		    bool is_mvcc_op, HEAP_LOG_INFO * physical_update_log_info)
{
  int error_code = NO_ERROR;
  ...
  if (is_old_home_updated)
      ...
      if (LOG_IS_MVCC_HEAP_OPERATION (rcv_index))
	{
      ...
      heap_log_update_physical (thread_p, tdes, context->home_page_watcher_p->pgptr, &context->hfid.vfid,
				context->oid.slotid | flag_vacuum_status, &context->home_recdes, &new_home_recdes,
				rcv_index, physical_update_log_info);
      HEAP_PERF_TRACK_LOGGING (thread_p, context);
...
/*
 * heap_mvcc_update_relocation_prepare_log () - prepare log for relocation record update
 *   thread_p(in): thread entry
 *   tdes(in): transaction descriptor
 *   is_page_fixed(in): true, if the page is fixed
 *   context(in): operation context
 ...
 *   undo_recdes(out): undo recdes
 *   redo_recdes(out): redo recdes
 *   update_old_home_p(out): true, if need to update old home
 *   update_old_forward(out): true, if need to update old forward
 *   remove_old_forward(out): true, if need to remove old forward
 *
 *  Note: This function set flag vacuum status into page as a side effect.
 */
STATIC_INLINE int
heap_mvcc_update_relocation_prepare_log (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_page_fixed,
					 HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op, MVCCID mvccid,
					 LOG_RCVINDEX * rcv_index, int *slot_id_with_flags, RECDES ** undo_recdes,
					 RECDES ** redo_recdes, bool * update_old_home_p,
					 bool * update_old_forward_p, bool * remove_old_forward_p)
{
  RECDES *local_redo_recdes = NULL, *local_undo_recdes = NULL;
  int rc;
  bool fits_in_home, fits_in_forward;
  PGBUF_WATCHER *new_forward_page_watcher_p = NULL;
  LOG_RCVINDEX local_rcv_index = 0;
  int flag_vacuum_status = 0;

  assert (context != NULL);
  assert (context->recdes_p != NULL);
  assert (context->type == HEAP_OPERATION_UPDATE);
  assert (update_old_home_p != NULL && update_old_forward_p != NULL && remove_old_forward_p != NULL);

  *update_old_home_p = *update_old_forward_p = *remove_old_forward_p = false;
  local_rcv_index = RV_NOT_DEFINED;
  /* set local recovery index */
#if defined (SERVER_MODE)
  if (is_mvcc_op)
    {
      assert (MVCCID_IS_VALID (mvccid));
      local_rcv_index = RVHF_UPDATE_NOTIFY_VACUUM;
    }
  else
#endif /* SERVER_MODE */
    {
      local_rcv_index = RVHF_UPDATE;
    }

  local_undo_recdes = &context->home_recdes;
  local_redo_recdes = context->recdes_p;
  ...
  if (is_page_fixed)
    {
      ...
      /* fix forward page */
      rc = heap_fix_forward_page (thread_p, context, &context->forward_oid);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit;
	...
	spage_is_updatable (thread_p, context->home_page_watcher_p->pgptr, context->oid.slotid,
      ...
      fits_in_forward =
	spage_is_updatable (thread_p, context->forward_page_watcher_p->pgptr, context->forward_oid.slotid,
			    context->recdes_p->length);
      if (heap_is_big_length (context->recdes_p->length) || (!fits_in_forward && !fits_in_home))
	{
      ...
      /* get forward record */
      context->forward_recdes.area_size = DB_PAGESIZE;
      context->forward_recdes.data = PTR_ALIGN (context->forward_recdes_buffer, MAX_ALIGNMENT);
      if (spage_get_record (context->forward_page_watcher_p->pgptr, context->forward_oid.slotid,
			    &context->forward_recdes, COPY) != S_SUCCESS)
	{
	  assert (false);
	  ASSERT_ERROR_AND_SET (rc);
	  goto exit;
      ...
      if (heap_is_big_length (context->recdes_p->length))
	{
	  ...
	  /* TO DO - check
	   * The home record descriptor will be an overflow OID and will be placed in original home page. Redo logging
	   * changed also, so we have to update physical_update_log_info.
	   */
	  heap_build_forwarding_recdes (&context->new_home_recdes, REC_BIGONE, &context->new_forward_oid);
	  local_redo_recdes = &context->new_home_recdes;

	  /* remove old forward record */
	  *remove_old_forward_p = true;
	  *update_old_home_p = true;
	  ...
	  if (context->forward_page_watcher_p->page_was_unfixed)
	    {
	      ...
	      /* TO DO - context -> forward recdes-> forward - recdes */
	      if (spage_get_record (context->forward_page_watcher_p->pgptr, context->forward_oid.slotid,
				    &context->forward_recdes, COPY) != S_SUCCESS)
		{
		  assert (false);
	  ...
	  if (LOG_IS_MVCC_HEAP_OPERATION (local_rcv_index))
	    {
	      heap_page_update_chain_after_mvcc_op (thread_p, context->home_page_watcher_p->pgptr,
						    mvccid, &flag_vacuum_status);
	    }
	  ...
	  if (is_mvcc_op)
	    {
	      ...
	      new_forward_page_watcher_p = context->new_forward_page_watcher_p;
	      assert (new_forward_page_watcher_p != NULL);
	    }
	...
	{
	  /* Updated forward record fits in old forward page. Redo logging need recreation since type modified. */
	  context->recdes_p->type = REC_NEWHOME;

	  /* home record will not be touched */
	  *update_old_forward_p = true;

	  local_rcv_index = RVHF_UPDATE;
	  local_undo_recdes = &context->forward_recdes;
    ...
    {
      char *home_page_copy_before_fix = context->home_page_copy_before_fix;
      bool has_forward_page = false;
      PGSLOTID slotid;

      if (context->home_recdes_before_page_fix.type == REC_RELOCATION)
	{
	  /* prepare for updating old home */
	  fits_in_home =
	    spage_is_updatable (thread_p, home_page_copy_before_fix, context->oid.slotid, context->recdes_p->length);
	  if (!fits_in_home)
	    {
	      *update_old_home_p = false;
	      return NO_ERROR;
	    }

	  /* recovery index already computed */

	  /* set slot id */
	  slotid = context->oid.slotid;
	  ...
	  *update_old_home_p = true;
      ...
      else if (context->home_recdes_before_page_fix.type == REC_NEWHOME)
	{
	  fits_in_forward =
	    spage_is_updatable (thread_p, home_page_copy_before_fix, context->forward_oid.slotid,
				context->recdes_p->length);
	  if (!fits_in_forward)
	    {
	      *update_old_forward_p = false;
	      return NO_ERROR;
	    }
	  ...
	  /* set slot id */
	  slotid = context->forward_oid.slotid;

	  /* TO DO - is it dangerous ? - probably a copy is better */
	  context->recdes_p->type = REC_NEWHOME;
	  *update_old_forward_p = true;
	}

      local_undo_recdes = &context->home_recdes_before_page_fix;
      local_redo_recdes = context->recdes_p;
      assert (local_undo_recdes != NULL && local_redo_recdes != NULL);
    }

  /* set recovery index */
  *rcv_index = local_rcv_index;

  /* set slot_id with flags */
  *slot_id_with_flags = context->oid.slotid | flag_vacuum_status;

  /* set undo/redo recdes */
  *undo_recdes = local_undo_recdes;
  *redo_recdes = local_redo_recdes;
  assert (*undo_recdes != NULL && *redo_recdes != NULL);

  return NO_ERROR;

exit:
  if (context->new_forward_page_watcher_p->pgptr != NULL)
    {
      pgbuf_ordered_unfix (thread_p, context->new_forward_page_watcher_p);
    }
 ...
 *   is_mvcc_op(in): type of operation (MVCC/non-MVCC)
 *   mvccid(in): current transaction MVCCID
 *   physical_update_log_info(in/out): physical update log into, obtained before page fixing
 */
static int
heap_update_relocation (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context,
			bool is_mvcc_op, MVCCID mvccid, HEAP_LOG_INFO * physical_update_log_info)
{
  int rc;
  ...
  LOG_RCVINDEX rcv_index = RV_NOT_DEFINED;
  int flag_vacuum_status = 0, slot_id_with_flags = 0;
  ...
  /* TO DO - non MVCC? */
  /* create log data */
  rc = heap_mvcc_update_relocation_prepare_log (thread_p, tdes, true, context, is_mvcc_op, mvccid, &rcv_index,
						&slot_id_with_flags, &old_recdes, &new_recdes, &update_old_home,
						&update_old_forward, &remove_old_forward);
  if (rc != NO_ERROR)
    {
      ASSERT_ERROR ();
      goto exit;
  ...
  if (update_old_home)
    {
      /* log operation */
      rc = heap_log_update_physical (thread_p, tdes, context->home_page_watcher_p->pgptr, &context->hfid.vfid,
				     slot_id_with_flags, old_recdes, new_recdes, rcv_index, physical_update_log_info);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit;
      ...
      /* update home record */
      rc = heap_update_physical (thread_p, context->home_page_watcher_p->pgptr, context->oid.slotid, new_recdes);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit;
  ...
  if (update_old_forward)
    {
      /* log operation. */
      rc = heap_log_update_physical (thread_p, tdes, context->forward_page_watcher_p->pgptr, &context->hfid.vfid,
				     context->forward_oid.slotid, old_recdes, new_recdes, rcv_index,
				     physical_update_log_info);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit;
      ...
      if (is_mvcc_op)
	{
      ...
      /* physical update of forward record */
      rc = heap_update_physical (thread_p, context->forward_page_watcher_p->pgptr, context->forward_oid.slotid,
				 context->recdes_p);
      if (rc != NO_ERROR)
	{
	  ASSERT_ERROR ();
  ...
  if (is_mvcc_op)
    {
      if (context->new_forward_page_watcher_p->pgptr != NULL)
	{
	  forward_page_watcher_p = context->new_forward_page_watcher_p;
	}
      else
	{
	  forward_page_watcher_p = context->forward_page_watcher_p;
	}
      ...
      rc = heap_update_set_prev_version (thread_p, &context->oid, context->home_page_watcher_p,
					 forward_page_watcher_p, &prev_version_lsa);
...
/*
 * heap_mvcc_home_update_prepare_log () - prepare log for home record update
 *   thread_p(in): thread entry
 *   tdes(in): transaction descriptor
 *   is_page_fixed(in): true, if the page is fixed
 *   context(in): operation context
 ...
 */
STATIC_INLINE int
heap_mvcc_home_update_prepare_log (THREAD_ENTRY * thread_p, LOG_TDES * tdes, bool is_page_fixed,
				   HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op, MVCCID * mvccid,
				   LOG_RCVINDEX * rcv_index, int *slot_id_with_flags, RECDES ** undo_recdes,
				   RECDES ** redo_recdes)
{
  ...
  PGBUF_WATCHER *new_forward_page_watcher_p = NULL;
  int flag_vacuum_status = 0;
  LOG_RCVINDEX local_rcv_index;
  RECDES *local_redo_recdes = NULL;

  assert (context != NULL && context->recdes_p != NULL && context->type == HEAP_OPERATION_UPDATE && mvccid != NULL
	  && rcv_index != NULL && slot_id_with_flags != NULL && undo_recdes != NULL && redo_recdes != NULL);
  ...
  *rcv_index = RV_NOT_DEFINED;
  ...
  if (is_mvcc_op)
    ...
    {
      /* Quick fix: Assign address is update in-place. Vacuum must be notified. Needs MVCCID. */
      local_rcv_index = RVHF_UPDATE_NOTIFY_VACUUM;
  ...
  if (is_page_fixed)
      ...
      if (heap_is_big_length (context->recdes_p->length))
	{
	  ...
	  heap_build_forwarding_recdes (&context->new_home_recdes, REC_BIGONE, &context->forward_oid);

	  /* TO DO - check
	   * We'll be updating home with forwarding record. Redo logging changed also, so we have
	   * to update physical_update_log_info.
	   */
	  local_redo_recdes = &context->new_home_recdes;
				    ...
				    context->recdes_p->length))
	{
	  ...
	  HEAP_PERF_TRACK_PREPARE (thread_p, context);
	  context->recdes_p->type = REC_NEWHOME;
	  error_code = heap_insert_newhome (thread_p, context, context->recdes_p, &context->forward_oid,
					    new_forward_page_watcher_p);
	  if (error_code != NO_ERROR)
	    {
	      ASSERT_ERROR ();
	      return error_code;
      ...
      else
	{
	  ...
	  /* Updated record fits in home page. Redo logging unchanged. */
	  local_redo_recdes = context->recdes_p;
    ...
    {
      char *home_page_copy_before_fix = context->home_page_copy_before_fix;
      RECDES *p_home_recdes_before_fix = &context->home_recdes_before_page_fix;

      if (home_page_copy_before_fix == NULL)
	{
	  /* can't create the log since don't have the page the copy of the page */
	  return NO_ERROR;
	}
      assert (p_home_recdes_before_fix->type == REC_HOME);

      if (!spage_is_updatable (thread_p, home_page_copy_before_fix, context->oid.slotid, context->recdes_p->length))
	{
	  /* do not uses the optimization for now */
	  return NO_ERROR;
	}

      /* TO DO - new buffer probably */
      context->recdes_p->type = REC_HOME;
  ...
  *slot_id_with_flags = context->oid.slotid | flag_vacuum_status;
  ...
  /* set undo/redo recdes */
  *undo_recdes = &context->home_recdes;
  *redo_recdes = local_redo_recdes;
  assert (*undo_recdes != NULL && *redo_recdes != NULL);
 ...
 */
static int
heap_update_home (THREAD_ENTRY * thread_p, LOG_TDES * tdes, HEAP_OPERATION_CONTEXT * context, bool is_mvcc_op,
		  MVCCID mvccid, HEAP_LOG_INFO * physical_update_log_info)
{
  int error_code = NO_ERROR;

  RECDES *home_recdes_p = NULL, *home_page_updated_recdes_p = NULL;
  LOG_RCVINDEX rcv_index;
  LOG_LSA prev_version_lsa;
  int slot_id_with_flags = 0;
  ...
  error_code =
    heap_mvcc_home_update_prepare_log (thread_p, tdes, true, context, is_mvcc_op, &mvccid, &rcv_index,
				       &slot_id_with_flags, &home_recdes_p, &home_page_updated_recdes_p);
  if (error_code != NO_ERROR)
    {
      ASSERT_ERROR ();
      goto exit;
  ...
  error_code =
    heap_log_update_physical (thread_p, tdes, context->home_page_watcher_p->pgptr, &context->hfid.vfid,
			      slot_id_with_flags, home_recdes_p, home_page_updated_recdes_p, rcv_index,
			      physical_update_log_info);
  if (error_code != NO_ERROR)
    {
      assert (false);
  ...
  if (is_mvcc_op)
    {
      ...
      error_code = heap_update_set_prev_version (thread_p, &context->oid, context->home_page_watcher_p,
						 context->new_forward_page_watcher_p, &prev_version_lsa);
      if (error_code != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit;
...
exit:
  ...
  if (context->new_forward_page_watcher_p != NULL && context->new_forward_page_watcher_p->pgptr != NULL)
    {
      /* newhome_pg_watcher is used only locally; must be unfixed */
      pgbuf_ordered_unfix (thread_p, context->new_forward_page_watcher_p);
    }
 ...
 */
STATIC_INLINE int
heap_log_update_physical (THREAD_ENTRY * thread_p, LOG_TDES * tdes, PAGE_PTR page_p, VFID * vfid_p, int offset,
			  RECDES * old_recdes_p, RECDES * new_recdes_p, LOG_RCVINDEX rcv_index,
			  HEAP_LOG_INFO * physical_update_log_info)
{
  LOG_DATA_ADDR address;
  LOG_PRIOR_NODE *physical_update_log_node = NULL;
  int min_undo_length, min_redo_length;
  int error_code = NO_ERROR;
  HEAP_LOG_INFO local_physical_update_log_info;
  RECDES *p_update_log_undo_recdes = NULL, *p_update_log_redo_recdes = NULL;

  assert (page_p != NULL && old_recdes_p && new_recdes_p != NULL);
  /* build address */
  if (physical_update_log_info != NULL)
    {
      /* Check the log recovery index consistency. */
      assert (physical_update_log_info->node != NULL && physical_update_log_info->node_appended == false);

      physical_update_log_node = physical_update_log_info->node;

      min_undo_length = MIN (old_recdes_p->length, OR_MVCC_MAX_HEADER_SIZE);
      min_redo_length = MIN (new_recdes_p->length, OR_MVCC_MAX_HEADER_SIZE);
      p_update_log_undo_recdes = (RECDES *) physical_update_log_info->undo_data_p;
      p_update_log_redo_recdes = (RECDES *) physical_update_log_info->redo_data_p;
      if (physical_update_log_info->rcv_index == rcv_index
	  && VPID_EQ (&physical_update_log_info->vpid, pgbuf_get_vpid_ptr (page_p))
	  && (p_update_log_undo_recdes->data == old_recdes_p->data
	      || (p_update_log_undo_recdes->length == old_recdes_p->length
		  && memcmp (p_update_log_undo_recdes->data, old_recdes_p->data, min_undo_length) == 0))
	  && (p_update_log_redo_recdes->data == new_recdes_p->data
	      || (p_update_log_redo_recdes->length == new_recdes_p->length
		  && memcmp (p_update_log_redo_recdes->data, new_recdes_p->data, min_redo_length) == 0)))
	{
	  assert (p_update_log_undo_recdes->type == old_recdes_p->type
		  && p_update_log_redo_recdes->type == new_recdes_p->type);
	  if (physical_update_log_info->slot_id_with_flags == offset)
	    {
	      /* The log created before page fixing can be safetely appended */
	      assert (p_update_log_undo_recdes->data == NULL
		      || memcmp (p_update_log_undo_recdes->data, old_recdes_p->data, old_recdes_p->length) == 0);
	      assert (p_update_log_redo_recdes->data == NULL
		      || memcmp (p_update_log_redo_recdes->data, new_recdes_p->data, new_recdes_p->length) == 0);
	      goto append_log_node;
	    }

	  if (heap_rv_remove_flags_from_offset (offset)
	      == heap_rv_remove_flags_from_offset (physical_update_log_info->slot_id_with_flags))
	    {
	      assert (p_update_log_undo_recdes->data == NULL
		      || memcmp (p_update_log_undo_recdes->data, old_recdes_p->data, old_recdes_p->length) == 0);
	      assert (p_update_log_redo_recdes->data == NULL
	      ...
	      LOG_SET_DATA_ADDR (&address, page_p, vfid_p, offset);
	      error_code =
		prior_lsa_update_undoredo_data_from_recdes (thread_p, physical_update_log_node, rcv_index,
							    &address, NULL, NULL, true, true);
	      if (error_code == NO_ERROR)
		{
		  /* Successfully updated undo/redo data. Append the log node. */
		  goto append_log_node;
  ...
  /* Create the log node. */
  assert (new_recdes_p != NULL);
  error_code = heap_create_log_node_for_update (thread_p, tdes, rcv_index, vfid_p, page_p, offset, old_recdes_p,
						new_recdes_p, &physical_update_log_node);
  if (error_code != NO_ERROR)
    {
      return error_code;
    }
  ...
  HEAP_SET_LOG_INFO (physical_update_log_info, physical_update_log_node, rcv_index, pgbuf_get_vpid_ptr (page_p),
		     offset, old_recdes_p, sizeof (old_recdes_p), new_recdes_p, sizeof (new_recdes_p), false);
append_log_node:
  /* Append the log node. */
  assert (physical_update_log_node != NULL && physical_update_log_info != NULL);
  log_append_prior_node (thread_p, tdes, rcv_index, page_p, physical_update_log_node);
  physical_update_log_info->node_appended = true;
 ...
 */
void
heap_create_insert_context (HEAP_OPERATION_CONTEXT * context, HFID * hfid_p, OID * class_oid_p,
...
heap_insert_logical (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context)
{
  ...
  if (!heap_is_big_length (context->recdes_p->length))
    {
       ...
       */
      rc = heap_create_insert_log_info_before_page_fixing (thread_p, tdes, &context->hfid.vfid,
   ...
   * Operation logging
   */
  rc = heap_log_insert_physical (thread_p, tdes, context->home_page_watcher_p->pgptr, &(context->hfid.vfid),
				 context->res_oid.slotid | flag_vacuum_status, context->recdes_p, is_mvcc_op,
				 context->is_redistribute_insert_with_delid, p_physical_insert_log_info);
  if (rc != NO_ERROR)
    {
      goto error;
    }
...
heap_delete_logical (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context)
{
  ...
  PERF_UTIME_TRACKER time_track;
  LOG_TDES *tdes = NULL;
  HEAP_LOG_INFO *p_delete_before_fix_log_info = NULL;
  MVCCID mvccid = MVCCID_NULL;
#if defined(SERVER_MODE)
  HEAP_LOG_INFO delete_log_info;
  char *bcb_area = NULL;
...
#if defined(SERVER_MODE)
  /*
   * Generate log record before fixing the page. The purpose is to release the latch faster. The generated log data
   * can be updated later after fixing the page. The update is faster than generating the log record.
   * In case of MVCC heap deletion, the logging is done mainly with the following recovery indexes:
   *    RVHF_MVCC_DELETE_REC_HOME, RVHF_MVCC_DELETE_REC_NEWHOME, RVHF_MVCC_DELETE_OVERFLOW:
   *      - log only delete MVCCID in redo data (adding MVCCID does fit in page)
   *    RVHF_MVCC_DELETE_MODIFY_HOME:
   *      - log not only delete MVCCID, in undo redo data (adding MVCCID does not fit in page)
   *  Currently, before the page fixing, we don't know the following informations:
   *      - the record type
   *      - whether the MVCCID does fit in page
   *      - the recovery index
   *
   * After page fixing, the log record data will be updated. However, we do not update if the undo/redo information
   * before and after the fix differs. We want to benefit by log compression that will be skipped if the undo is
   * generated before page fixing and redo after page fixing. In future, we may consider using update even if the
   * undo/redo information differs.
   * In case of non-MVCC heap deletion, the logging is done with RVHF_DELETE recovery index.
   */

  /* Acquire transaction BCB area, where the page will be copied */
  ...
  if (bcb_area)
    {
      ...
      HEAP_INIT_LOG_INFO (&delete_log_info);
      rc = heap_create_delete_log_info_before_page_fixing (thread_p, tdes, context, mvccid, bcb_area, &delete_log_info);
      if (rc != NO_ERROR)
	{
	  goto error;
	}

      if (delete_log_info.node != NULL)
	{
  ...
  switch (context->record_type)
    {
    case REC_BIGONE:
      rc = heap_delete_bigone (thread_p, tdes, context, is_mvcc_op, mvccid, p_delete_before_fix_log_info);
      break;

    case REC_RELOCATION:
      rc = heap_delete_relocation (thread_p, tdes, context, is_mvcc_op, mvccid, p_delete_before_fix_log_info);
      break;

    case REC_HOME:
    case REC_ASSIGN_ADDRESS:
      rc = heap_delete_home (thread_p, tdes, context, is_mvcc_op, mvccid, p_delete_before_fix_log_info);
      break;
...
heap_update_logical (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context)
{
  ...
  bool is_mvcc_class;
  MVCCID mvccid = MVCCID_NULL;
  HEAP_LOG_INFO update_log_info, *p_update_log_info = NULL;
  ...
  /*
   * Generate log record before fixing the page. The purpose is to release the latch faster. The generated log data
   * can be updated later after fixing the page.
   * In case of MVCC heap updating, the logging is done with RVHF_UPDATE_NOTIFY_VACUUM (log undo/redo record).
   * In case of non-MVCC heap deletion, the logging is done with RVHF_UPDATE (log undo/redo record).
   * Currently, before the page fixing, we don't know the record type. Uses uninitialized home_recdes_buffer now, to
   * allocate log undo space. Later, after obtaining the home recdes, we will update the undo log data. If we implement
   * page copy without latch, then we will have undo log data from beginning, in most of the cases. Also, with page copy
   * without latch, we can insert NEWHOME record without fixing the home page.
   * The recovery index can't be changed from non-MVCC style to MVCC style or reverse way. That's because the MVCC
   * log header differs by non-MVCC log header. However, in some cases, the recovery index style generated before page
   * fixing, may be different than recovery index style needed after fixing the page. In such cases, the generated log
   * can't be used after fixing the page, so we have to re-create the entire log record.
   */
  ...
  if (bcb_area)
    {
      ...
      pgbuf_copy_log ("pgbuf_copy_page: Successfully acquired BCB area to copy heap page (%d, %d)\n",
		      context->oid.volid, context->oid.pageid);
      context->home_page_copy_before_fix = bcb_area;
      HEAP_INIT_LOG_INFO (&update_log_info);
      rc = heap_create_update_log_info_before_page_fixing (thread_p, tdes, context, &mvccid, is_mvcc_op, bcb_area,
							   &update_log_info);
      if (rc != NO_ERROR)
	{
	  assert (false);
	...
	{
	  p_update_log_info = &update_log_info;
	}
    }
  else
    {
  ...
  switch (context->record_type)
    {
    case REC_RELOCATION:
      rc = heap_update_relocation (thread_p, tdes, context, is_mvcc_op, mvccid, p_update_log_info);
      break;

    case REC_BIGONE:
      rc = heap_update_bigone (thread_p, tdes, context, mvccid, is_mvcc_op, p_update_log_info);
      break;
    ...
    case REC_ASSIGN_ADDRESS:
      ...
      /* fall trough */
    case REC_HOME:
      rc = heap_update_home (thread_p, tdes, context, is_mvcc_op, mvccid, p_update_log_info);
      break;
...
exit:
  ...
  if (p_update_log_info != NULL && p_update_log_info->node_appended == false)
    {
      assert (p_update_log_info->node != NULL);
      prior_lsa_free_node (p_update_log_info->node);
    }
...
/*
 * heap_page_update_chain_after_mvcc_op () - Update max MVCCID and vacuum status in heap page chain after
				      ...
				      int *flag_vacuum_status)
{
  ...
  assert (heap_page != NULL && flag_vacuum_status != NULL);
  assert (MVCCID_IS_NORMAL (mvccid));
  ...
  *flag_vacuum_status = 0;
  ...
  switch (vacuum_status)
    ...
    case HEAP_PAGE_VACUUM_NONE:
      ...
      *flag_vacuum_status = HEAP_RV_FLAG_VACUUM_STATUS_CHANGE;
      vacuum_er_log (VACUUM_ER_LOG_HEAP,
...
heap_page_set_vacuum_status_none (THREAD_ENTRY * thread_p, PAGE_PTR heap_page)
{
  ...
  vacuum_er_log (VACUUM_ER_LOG_HEAP,
		 "VACUUM: Changed vacuum status for page %d|%d from vacuum once to no vacuum.\n",
		 pgbuf_get_volume_id (heap_page), pgbuf_get_page_id (heap_page));
}
...
heap_rv_mvcc_redo_redistribute (THREAD_ENTRY * thread_p, LOG_RCV * rcv)
{
  ...
  if (record_type == REC_BIGONE)
    {
      /* no data header */
      HEAP_SET_RECORD (&recdes, rcv->length - sizeof (record_type), rcv->length - sizeof (record_type),
		       REC_BIGONE, rcv->data + sizeof (record_type));
    }
  else
    {
      char data_buffer[IO_DEFAULT_PAGE_SIZE + OR_MVCC_MAX_HEADER_SIZE + MAX_ALIGNMENT];
      ...
      if (snapshot_res == TOO_NEW_FOR_SNAPSHOT)
	{
	  ...
	  scan =
	    heap_get_visible_version_from_log (thread_p, context->recdes_p,
					       &MVCC_GET_PREV_VERSION_LSA (&mvcc_header), context->scan_cache,
					       context->old_chn);
	  goto exit;
	}
      else if (snapshot_res == TOO_OLD_FOR_SNAPSHOT)
