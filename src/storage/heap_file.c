...
heap_delete_logical (THREAD_ENTRY * thread_p, HEAP_OPERATION_CONTEXT * context)
{
  ...
  /* An MVCC delete's row may be seen DELETE_IN_PROGRESS by the lockless referential integrity checks, which wait
   * on the deleter's MVCCID rather than a row lock, so take the same self-lock the insert path takes. */
  if (is_mvcc_op)
    {
