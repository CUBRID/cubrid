 ...
 *   no_referrer (in): there is no one referreing to a heap record with OID even if it is DONT_REUSE_OID
 *
 * Note: Destroy the heap file associated with the given heap
 * identifier if it is a newly created heap file.
 */
int
xheap_destroy_newly_created (THREAD_ENTRY * thread_p, const HFID * hfid, const OID * class_oid, const bool no_referrer)
{
  VFID vfid;
  FILE_TYPE file_type;
  int ret;
  ...
  if (file_type == FILE_HEAP_REUSE_SLOTS || no_referrer)
    {
