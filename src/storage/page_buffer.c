...
#if defined (SERVER_MODE)
/* test-only: BCB fcnt without exposing struct PGBUF_BCB to the test TU */
int
pgbuf_test_get_fcnt (PAGE_PTR pgptr)
{
  PGBUF_BCB *bufptr;
  if (pgptr == NULL)
    {
...
/* test-only: fcnt for a VPID without taking a latch ourselves (read-only probe) */
int
pgbuf_test_get_fcnt_by_vpid (THREAD_ENTRY * thread_p, const VPID * vpid_p)
{
  PGBUF_BUFFER_HASH *hash_anchor;
  PGBUF_BCB *bufptr;
  int fcnt;
  ...
  /* pgbuf_search_hash_chain returns with bufptr->mutex held */
  fcnt = get_fcnt (&bufptr->atomic_latch);
  PGBUF_BCB_UNLOCK (bufptr);
  return fcnt;
}

/* test-only: drive caller into pgbuf_block_bcb's PGBUF_LATCH_FLUSH branch */
int
pgbuf_test_request_flush_block (THREAD_ENTRY * thread_p, const VPID * vpid_p)
{
  PGBUF_BUFFER_HASH *hash_anchor;
  PGBUF_BCB *bufptr;
  int rv;
...
/* test-only: wake FLUSH waiter(s) for vpid without booting xboot_shutdown_server */
void
pgbuf_test_wake_flush_waiters_for_test (THREAD_ENTRY * thread_p, const VPID * vpid_p)
{
  PGBUF_BUFFER_HASH *hash_anchor;
  PGBUF_BCB *bufptr;
  THREAD_ENTRY *crt, *prev, *next;
  ...
  /* pgbuf_search_hash_chain returns with bufptr->mutex held */
  prev = NULL;
  for (crt = bufptr->next_wait_thrd; crt != NULL; crt = next)
    {
      next = crt->next_wait_thrd;
      if (crt->request_latch_mode != PGBUF_LATCH_FLUSH)
	{
	  prev = crt;
	  continue;
	}
      if (prev == NULL)
	{
	  bufptr->next_wait_thrd = next;
	}
      else
	{
	  prev->next_wait_thrd = next;
	}
      crt->next_wait_thrd = NULL;
      thread_wakeup (crt, THREAD_RESUME_DUE_TO_INTERRUPT);
    }
  if (bufptr->next_wait_thrd == NULL)
    {
      set_waiter_exists (&bufptr->atomic_latch, false);
