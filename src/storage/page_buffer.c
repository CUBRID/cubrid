			       ...
			       bool * stop)
{
  ...
  bool detailed_perf = perfmon_is_perf_tracking_and_active (thread_p, PERFMON_ACTIVATION_FLAG_PB_VICTIMIZATION);
		  ...
		  if (!pgbuf_bcb_avoid_victim (bufptr) && pgbuf_assign_direct_victim (thread_p, bufptr))
		    {
		      ...
		      if (perfmon_is_perf_tracking_and_active (thread_p, PERFMON_ACTIVATION_FLAG_PB_VICTIMIZATION))
			{
...
#define MAX_DEPTH 1000
  ...
  bool perf_tracking = perfmon_is_perf_tracking_and_active (thread_p, PERFMON_ACTIVATION_FLAG_PB_VICTIMIZATION);
	  ...
	  if (lru_list->count_vict_cand > 0 && PGBUF_LRU_LIST_IS_OVER_QUOTA (lru_list))
	    {
		  ...
		  && perfmon_is_perf_tracking_and_active (thread_p, PERFMON_ACTIVATION_FLAG_PB_VICTIMIZATION))
		{
