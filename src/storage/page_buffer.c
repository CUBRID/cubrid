...
/*
 * Each thread has its own free BCB holder list. The list has PGBUF_DEFAULT_FIX_COUNT entries by default.
 */
#define PGBUF_DEFAULT_FIX_COUNT    7

/*
 * Each BCB holder array, that is allocated from OS, has PGBUF_NUM_ALLOC_HOLDER elements(BCB holder entries).
 */
...
typedef enum {
	...
	/*
	 * LRU zones explained: 1. This is hottest zone and this is where most fixed/unfixed bcb's are found. We'd like
	 * to keep the page unfix complexity to a minimum, therefore no boost to top are done here. This zone's bcb's
	 * cannot be victimized. 2. This is a buffer between the hot lru 1 zone and the victimization lru 3 zone. The
	 * buffer zone gives bcb's that fall from first zone a chance to be boosted back to top (if they are still
	 * hot). Victimization is still not allowed. 3. Third zone is the victimization zone. BCB's can still be
	 * boosted if fixed/unfixed, but in aggressive victimizing systems, non-dirty bcb's rarely survive here.
	 */
	...
	PGBUF_VOID_ZONE = 2 << (PGBUF_LRU_NBITS + 2),	/* void zone: temporary zone after reading bcb from disk until
							 * and until adding to a lru list, or after removing from lru
							 * list and until victimizing. */
...
/*
 * is flushing: set to true when someone intends to flush the bcb to disk. dirty flag is usually set to false, but bcb
 * cannot be yet victimized. flush must succeed first.
 */
 ...
 * providers of such bcb's. there is a small window of opportunity for active workers to fix this bcb. when fixing a
 * direct victim, we need to replace the flag with PGBUF_BCB_INVALIDATE_DIRECT_VICTIM_FLAG. there is not point of
 * victimizing this bcb to fix it again. The thread waiting for the bcb will know it was fixed again and will request
 * another bcb.
 */
...
/*
 * 1. dirty bcb's cannot be victimized. 2. bcb's that are in the process of being flushed cannot be victimized. flush
 * must succeed! 3. bcb's that are already assigned as victims are not valid victim candidates.
 */
...
/*
 * Activity on each LRU is probed and cumulated; to avoid long history cumulation effect, the activity indicator is
 * limited (PGBUF_TRAN_MAX_ACTIVITY); Inactivity threshold is defined: private LRU dropping beneath this threshold are
 * destroyed and its BCBs will be victimized.
 */
...
/*
 * Maximum overboost flush multiplier: controls the maximum factor to apply to configured flush ratio, when the miss
 * rate (victim_request/fix_request) increases.
 */
...
struct pgbuf_bcb {
	...
	int		tick_lru_list;	/* age of lru list when this BCB was inserted into. used to decide when bcb has
					 * aged enough to boost to top. */
	int		tick_lru3;	/* position in lru zone 3. small numbers are at the bottom. used to update LRU
					 * victim hint. */
	volatile int	count_fix_and_avoid_dealloc;	/* two-purpose field: 1. count fixes up to a threshold (to
							 * detect hot pages). 2. avoid deallocation count. we don't use
							 * two separate shorts because avoid deallocation needs to be
							 * changed atomically... 2-byte sized atomic operations are not
							 * common. */
 ...
 * 
 * buffer lock table is the array of buffer lock records # of buffer lock records is fixed as the total # of threads.
 */
...
struct pgbuf_lru_list {
	...
	PGBUF_BCB      *volatile victim_hint;	/* hint to start searching for victims in lru list. everything below
						 * the hint should be dirty, but the hint is not always the first bcb
						 * that can be victimized. */
	/*
	 * TODO: I have noticed while investigating core files from TPCC that hint is sometimes before first bcb that
	 * can be victimized. this means there is a logic error somewhere. I don't know where, but there must be.
	 */
...
/*
 * The page replacement algorithm is LRU + Aout of 2Q. This algorithm uses two linked lists as follows: - LRU list:
 * this is a list of BCBs managed as a Least Recently Used queue - Aout list: this is a list on VPIDs managed as a FIFO
 * queue The LRU list manages the "hot" pages, Aout list holds a short term history of pages which have been
 * victimized.
 */
/* Aout list node */
...
struct pgbuf_aout_list {
	...
	PGBUF_AOUT_BUF *bufarray;	/* Array holding all the nodes in the list. Since Aout has a predefined fixed
					 * size, it makes more sense to preallocate all the nodes */
...
/*
 * Generic structure to manage sequential flush with flush rate control: Flush rate control is achieved by breaking
 * each 1 second into intervals, and attempt to flush an equal number of pages in each interval. Compensation is
 * applied across all intervals in one second to achieve overall flush rate. In each interval, the pages are flushed
 * either in burst mode or equally time spread during the entire interval.
 */
...
struct pgbuf_page_monitor {
	...
	bool		victim_rich;	/* true if page buffer pool has many victims. pgbuf_adjust_quotas will update
					 * this value. */
};
...
struct pgbuf_page_quota {
	...
	int            *private_lru_session_cnt;	/* Number of active session for each private LRU:  Contains
							 * only private lists ! */
	...
	int		avoid_shared_lru_idx;	/* index of shared LRU to avoid when relocating to shared; this is
						 * usually the index of shared LRU with maximum number of BCBs;
						 * transaction will avoid this list when relocating to shared LRU (like
						 * when moving from a garbage LRU); such LRU list returns to normal
						 * size through victimization */
...
struct pgbuf_buffer_pool {
	...
	PGBUF_LRU_LIST *buf_LRU_list;	/* LRU lists. When Page quota is enabled, first 'num_LRU_list' store shared
					 * pages; the next 'num_garbage_LRU_list' lists store shared garbage pages; the
					 * last 'num_private_LRU_list' are private lists. When page quota is disabled
					 * only shared lists are used */
	...
	/*
	 * the structures for maintaining information on BCB holders. 'thrd_holder_info' has entries as many as the #
	 * of threads and each entry maintains free BCB holder list and used BCB holder list of the corresponding
	 * thread. 'thrd_reserved_holder' has memory space for all BCB holder entries.
	 */
	PGBUF_HOLDER_ANCHOR *thrd_holder_info;
	PGBUF_HOLDER   *thrd_reserved_holder;

	/*
	 * free BCB holder list shared by all the threads. When a thread needs more free BCB holder entries, the thread
	 * allocates them one by one from this list. However, the thread never return the entries into this list. The
	 * structure is a list of the arrays of BCB holder entries. 'free_holder_set' points to the first array that
	 * has free entries and 'free_index' indicates the first free entry in the array.
	 */
#if defined(SERVER_MODE)
	...
	/*
	 * 'check_for_interrupt' is set true when interrupts must be checked. Log manager set and clears this value
	 * while holding TR_TABLE_CS.
	 */
...
/*
 * One daemon thread performs flush task for victim candidates. The daemon finds and saves victim candidates using
 * following list. And then, based on the list, the daemon performs actual flush task.
 */
 ...
 * different code than the one that caused the crash. my workaround is to save the line of code in this global variable
 * pgbuf_Abort_release_line.
 * 
...
/*
 * pgbuf_hash_func_mirror () - Hash VPID into hash anchor return: hash value key_vpid(in): VPID to hash
 */
...
/*
 * pgbuf_hash_vpid () - Hash a volume_page identifier return: hash value key_vpid(in): VPID to hash htsize(in): Size of
 * hash table
 */
unsigned int
...
/*
 * pgbuf_compare_vpid () - Compare two vpids keys for hashing return: int (key_vpid1 == key_vpid2 ?) key_vpid1(in):
 * First key key_vpid2(in): Second key
 */
int
...
/*
 * pgbuf_initialize () - Initialize the page buffer pool return: NO_ERROR, or ER_code
 * 
 * Note: Function invalidates any resident page, creates a hash table for easy lookup of pages in the page buffer pool,
 * and resets the clock tick for the  page replacement algorithm.
 */
int
pgbuf_initialize(void)
{
	...
	pgbuf_Pool.ratio_lru2 = MIN(pgbuf_Pool.ratio_lru2, 1.0 f - PGBUF_LRU_ZONE_MIN_RATIO - pgbuf_Pool.ratio_lru1);
	assert(pgbuf_Pool.ratio_lru2 >= PGBUF_LRU_ZONE_MIN_RATIO && pgbuf_Pool.ratio_lru2 <= PGBUF_LRU_ZONE_MAX_RATIO);
	assert((pgbuf_Pool.ratio_lru1 + pgbuf_Pool.ratio_lru2) >= 0.099 f
	       && (pgbuf_Pool.ratio_lru1 + pgbuf_Pool.ratio_lru2) <= 0.951 f);
	...
	{
		int		cnt;
		cnt = (int)(0.25 f * pgbuf_Pool.num_buffers);
		cnt = MIN(cnt, 65536);
...
/*
 * pgbuf_finalize () - Terminate the page buffer manager return: void
 * 
 * Note: Function invalidates any resident page, destroys the hash table used for lookup of pages in the page buffer pool.
 */
void
pgbuf_finalize(void)
{
...
/*
 * pgbuf_fix_with_retry () - return: Pointer to the page or NULL vpid(in): Complete Page identifier fetch_mode(in):
 * Page fetch mode request_mode(in): Lock request_mode retry(in): Retry count
 */
PAGE_PTR
...
/*
 * below two functions are dummies for Windows build (which defined at cubridsa.def)
 */
#if defined(WINDOWS)
#if !defined(NDEBUG)
PAGE_PTR
...
/*
 * pgbuf_fix () - return: Pointer to the page or NULL vpid(in): Complete Page identifier fetch_mode(in): Page fetch
 * mode. request_mode(in): Page latch mode. condition(in): Page latch condition.
 */
#if !defined(NDEBUG)
PAGE_PTR
pgbuf_fix_debug(THREAD_ENTRY * thread_p, const VPID * vpid, PAGE_FETCH_MODE fetch_mode, PGBUF_LATCH_MODE request_mode,
...
#endif				/* NDEBUG */
{
	...
	if (condition == PGBUF_UNCONDITIONAL_LATCH) {
		/*
		 * Check the wait_msecs of current transaction. If the wait_msecs is zero wait that means no wait,
...
try_again:
	...
	if (bufptr != NULL) {
		...
		if (fetch_mode == NEW_PAGE) {
			/*
			 * Fix a page as NEW_PAGE, when oldest_unflush_lsa of the page is not NULL_LSA, it should be
			 * dirty.
			 */
			assert(LSA_ISNULL(&bufptr->oldest_unflush_lsa) || pgbuf_bcb_is_dirty(bufptr));

			/*
			 * The page may be invalidated and has been remained in the buffer and it is going to be used
			 * again as a new page.
			 */
		}
	...
	if (pgbuf_check_bcb_page_vpid(bufptr, maybe_deallocated) != true) {
		if (buf_lock_acquired) {
			...
			/*
			 * Now, caller is not holding any mutex. the last argument of pgbuf_unlock_page () is true that
	...
	if (buf_lock_acquired) {
		...
		/*
		 * the caller is holding hash_anchor->hash_mutex. Therefore, the third argument of pgbuf_unlock_page ()
		 * is false that means hash mutex does not need to be held.
		 */
	...
	if (bufptr->iopage_buffer->iopage.prv.ptype == PAGE_UNKNOWN) {
		...
		case OLD_PAGE_MAYBE_DEALLOCATED:
			/*
			 * OLD_PAGE_MAYBE_DEALLOCATED is called when deallocated page may be fixed. The caller wants
			 * the page only if it is not deallocated. However, if it is deallocated, no error is required.
			 */
		...
		/*
		 * note: maybe we could check this in an earlier stage, but would have been a lot more complicated. the
		 * only interesting case here is OLD_PAGE_MAYBE_DEALLOCATED. However, even this is used in cases where
		 * the vast majority of pages will not be deallocated! So in terms of performance, the loss is
		 * insignificant. However, it is safer and easier to treat the case here, where we have latch to
		 * prevent concurrent deallocations.
		 */
	} else {
		/*
		 * this cannot be a new page or a deallocated page. note: temporary pages are not strictly handled in
		 * regard with their deallocation status.
		 */
...
/*
 * pgbuf_promote_read_latch () - Promote read latch to write latch return: error code or NO_ERROR pgptr(in/out): page
 * pointer condition(in): promotion condition (single reader holder/shared reader holder)
 */
#if !defined (NDEBUG)
int
...
#endif				/* NDEBUG */
{
	...
	if (holder->fix_count == bufptr->fcnt) {
		...
		if ((condition == PGBUF_PROMOTE_ONLY_READER)
			...
			/*
			 * CASE #1: first waiter is from a latch promotion - we can't guarantee both will see the same
			 * page they initially fixed so we'll abort the current promotion CASE #2:
			 * PGBUF_PROMOTE_ONLY_READER condition, we're only allowed to promote if we're the only reader;
...
/*
 * pgbuf_unfix () - Free the buffer where the page associated with pgptr resides return: void pgptr(in): Pointer to
 * page
 * 
...
#endif				/* NDEBUG */
{
...
#if defined(CUBRID_DEBUG)
	/*
	 * If the buffer is dirty and the log sequence address of the buffer has not changed since the database
	 * restart, a warning is given about lack of logging
	 */
	if (pgbuf_bcb_is_dirty(bufptr) && !pgbuf_is_temp_lsa(bufptr->iopage_buffer->iopage.prv.lsa)
		...
		/*
		 * Do not give warnings on this page any longer. Set the LSA of the buffer for this purposes
		 */
...
#if defined(CUBRID_DEBUG)
	/*
	 * CONSISTENCIES AND SCRAMBLES You may want to tailor the following debugging block since its operations and
	 * their implications are very expensive. Too much I/O
	 */
	if (pgbuf_get_check_page_validation_level(PGBUF_DEBUG_PAGE_VALIDATION_ALL)) {
		/*
		 * Check if the content of the page is consistent and then scramble the page to detect illegal access
		...
		if (bufptr->fcnt == 0) {
			...
			if (!VPID_ISNULL(&bufptr->vpid) && pgbuf_is_consistent(bufptr, 0) == PGBUF_CONTENT_BAD) {
				...
				if (pgbuf_bcb_is_dirty(bufptr)) {
					...
					/*
					 * Since above function releases bufptr->mutex, the caller must hold
					 * bufptr->mutex again.
					 */
					PGBUF_BCB_LOCK(bufptr);
				}
				/*
				 * If the buffer is associated with a page (i.e., if the buffer is not used as a
				 * working area --malloc--), invalidate the page on this buffer. Detach the buffer area
				 * or scramble tha area.
				 */
				if (!VPID_ISNULL(&bufptr->vpid)) {
					/* invalidate the page with PGBUF_LATCH_INVALID mode */
					(void)pgbuf_invalidate_bcb(thread_p, bufptr);
					/*
					 * Since above function releases mutex after flushing, the caller must hold
					 * bufptr->mutex again.
					 */
					PGBUF_BCB_LOCK(bufptr);
				}
				...
				/*
				 * Note that the buffer is not declared for immediate replacement. wait for a while to
				 * see if an invalid access is found.
				 */
			}
		}
		PGBUF_BCB_UNLOCK(bufptr);
...
/*
 * pgbuf_unfix_all () - Unfixes all the buffers that have been fixed by current thread at the time of request
 * termination return: void
 * 
 * Note: At the time of request termination, there must be no buffers that were fixed by the thread. In current CUBRID
 * system, however, above situation has occurred. In some later time, our system must be corrected to prevent above
...
pgbuf_unfix_all(THREAD_ENTRY * thread_p)
{
	...
	if (thrd_holder_info->num_hold_cnt > 0) {
		...
		while (holder != NULL) {
			...
			/*
			 * Within the execution of pgbuf_unfix(), the BCB holder entry is moved from the holder list of
			 * BCB to the free holder list of thread, and the BCB holder entry is removed from the holder
			 * list of the thread.
			 */
			holder = thrd_holder_info->thrd_hold_list;
#else				/* NDEBUG */
...
/*
 * pgbuf_invalidate () - Invalidate page in buffer return: NO_ERROR, or ER_code pgptr(in): Pointer to page
 * 
 * Note: Invalidate the buffer corresponding to page associated with pgptr when the page has been fixed only once,
 * otherwise, the page is only unfixed. If the page is invalidated, the page will not be associated with the buffer any
 * longer and the buffer can be used for the buffer allocation immediately.
 * 
 * The page invalidation task is executed only for performance enhancement. This task is irrespective of correctness. That
 * is, If this task is not performed, there is no problem in the correctness of the system. When page invalidation task
 * is used, however, following things must be kept to prevent incorrectness incurred by using page invalidation task.
 * 
 * 1. For temporary pages, page invalidation can be performed at any time. 2. For regular pages(used to save persistent
 * data such as meta data and user data), page invalidation must be performed as postpone operation that is executed
 * after the commit decision of transaction has been made. The reason will be explained in the document[TM-2001-04].
 */
#if !defined(NDEBUG)
int
...
#endif				/* NDEBUG */
{
	...
	/*
	 * This function is called by the caller while it is fixing the page with PGBUF_LATCH_WRITE mode in CUBRID
	 * environment. Therefore, the caller must unfix the page and then invalidate the page.
	 */
	if (bufptr->fcnt > 1) {
...
/*
 * pgbuf_invalidate_all () - Invalidate all unfixed buffers corresponding to the given volume return: NO_ERROR, or
 * ER_code volid(in): Permanent Volume Identifier or NULL_VOLID
 * 
 * Note: The pages in these buffers are disassociated from the buffers. If a page was dirty, it is flushed before the
...
#endif				/* NDEBUG */
{
	...
	/*
	 * While searching all the buffer pages or corresponding buffer pages, the caller flushes each buffer page if
	 * it is dirty and invalidates the buffer page if it is not fixed on the buffer.
	 */
...
/*
 * pgbuf_flush () - Flush a page out to disk return: pgptr on success, NULL on failure pgptr(in): Page pointer
 * free_page(in): Free the page too ?
 * 
 * Note: The page associated with pgptr is written out to disk (ONLY when the page is dirty) and optionally is freed (See
 * pb_free). The interface requires the pgptr instead of vpid to avoid hashing.
 * 
 * The page flush task is also executed only for performance enhancement like page invalidation task. And, this task can
 * be performed at any time unlike page invalidation task.
 */
void
pgbuf_flush(THREAD_ENTRY * thread_p, PAGE_PTR pgptr, bool free_page)
{
	/*
	 * caller flushes page but does not really care if page really makes it to disk. or doesn't know what to do in
	 * that case... I recommend against using it.
	 */
...
/*
 * pgbuf_flush_with_wal () - Flush a page out to disk after following the wal rule return: pgptr on success, NULL on
 * failure pgptr(in): Page pointer
 * 
 * Note: The page associated with pgptr is written out to disk (ONLY when the page is dirty) Before the page is flushed,
 * the WAL rule of the log manager is called.
 */
PAGE_PTR
 ...
 * 
 * return        : void thread_p (in) : thread entry page (in)     : page
 */
void
...
/*
 * pgbuf_flush_all () - Flush all dirty pages out to disk return: NO_ERROR, or ER_code volid(in): Permanent Volume
 * Identifier or NULL_VOLID
 * 
 * Note: Every dirty page of the specified volume is written out to disk. If volid is equal to NULL_VOLID, all dirty pages
 * of all volumes are written out to disk. Its use is recommended by only the log and recovery manager.
 */
int
...
/*
 * pgbuf_flush_all_unfixed () - Flush all unfixed dirty pages out to disk return: NO_ERROR, or ER_code volid(in):
 * Permanent Volume Identifier or NULL_VOLID
 * 
 * Note: Every dirty page of the specified volume which is unfixed is written out to disk. If volid is equal to
 * NULL_VOLID, all dirty pages of all volumes that are unfixed are written out to disk. Its use is recommended by only
 * the log and recovery manager.
 */
int
 ...
 * return: NO_ERROR, or ER_code volid(in): Permanent Volume Identifier or NULL_VOLID
 * 
 * Note: Every dirty page of the specified volume which is unfixed is written out after its lsa is initialized to a null
 * lsa. If volid is equal to NULL_VOLID, all dirty pages of all volumes that are unfixed are flushed to disk after its
 * lsa is initialized to null. Its use is recommended by only the log and recovery manager.
 */
int
...
/*
 * pgbuf_compare_victim_list () - Compare the vpid of victim candidate list return: p1 - p2 p1(in): victim candidate
 * list 1 p2(in): victim candidate list 2
 */
static int
...
/*
 * pgbuf_get_victim_candidates_from_lru () - get victim candidates from LRU list return                  : number of
 * victims found thread_p (in)           : thread entry check_count (in)        : number of items to verify before
 * abandoning search flush_ratio (in)        : flush ratio assigned_directly (out) : output true if a bcb was assigned
 * directly.
 */
static int
...
#if defined (SERVER_MODE)
	/*
	 * as part of handling a rare case when there are rare direct victim waiters although there are plenty victims,
	 * flush thread assigns one bcb per iteration directly. this will add only a little overhead in general cases.
	 */
 ...
 * 
 * return                : error code thread_p (in)         : thread entry flush_ratio (in)      : desired flush ratio
 * perf_tracker (in/out) : time tracker for performance statistics stop (out)            : output to stop looping
 */
int
			      ...
			      bool * stop)
{
	...
	int		error = NO_ERROR;
	float		lru_miss_rate;
	float		lru_dynamic_flush_adj = 1.0 f;
	...
	*stop = false;
...
#if defined (SERVER_MODE)
	/*
	 * do not apply flush boost during checkpoint; since checkpoint is already flushing pages we expect some of the
	 * victim candidates are already flushed by checkpoint
	 */
	if (pgbuf_Pool.is_checkpoint == false) {
		lru_dynamic_flush_adj = MAX(1.0 f, 1 + (PGBUF_FLUSH_VICTIM_BOOST_MULT - 1) * lru_miss_rate);
		lru_dynamic_flush_adj = MIN(PGBUF_FLUSH_VICTIM_BOOST_MULT, lru_dynamic_flush_adj);
	} else
#endif
	{
		lru_dynamic_flush_adj = 1.0 f;
	}
	...
	if (victim_count == 0) {
		...
		/*
		 * if pgbuf_get_victim_candidates_from_lru failed to provide candidates, it means we already flushed
		 * enough. give threads looking for victims a chance to find them before looping again. output hint to
		 * stop looping.
		 */
...
repeat:
	...
	for (i = 0; i < victim_count; i++) {
		...
		if (logpb_need_wal(&bufptr->iopage_buffer->iopage.prv.lsa)) {
			/*
			 * we cannot flush a page unless log has been flushed up until page LSA. otherwise we might
			 * have recovery issues.
			 */
			count_need_wal++;
	...
	if (pgbuf_is_any_thread_waiting_for_direct_victim() && victim_count != 0 && count_need_wal == victim_count) {
		...
		if (repeated) {
			/*
			 * already waited and failed again? all bcb's must have changed again (confirm by comparing
			 * save_lsa_need_wal and lsa_need_wal.
			 */
 ...
 * return:error code or NO_ERROR flush_upto_lsa(in): prev_chkpt_redo_lsa(in): Redo_LSA of previous checkpoint
 * smallest_lsa(out): Smallest LSA of a dirty buffer in buffer pool flushed_page_cnt(out): The number of flushed pages
 * 
 * Note: The function flushes and dirty unfixed page whose LSA is smaller that the last_chkpt_lsa, it returns the
 * smallest_lsa from the remaining dirty buffers which were not flushed. This function is used by the log and recovery
 * manager when a checkpoint is issued.
 */
int
...
/*
 * pgbuf_flush_chkpt_seq_list () - flush a sequence of pages during checkpoint return:error code or NO_ERROR
 * thread_p(in): seq_flusher(in): container for list of pages chkpt_smallest_lsa(out): smallest LSA found in a page
 * 
 */
static int
...
#define WAIT_FLUSH_VICTIMS_MAX_MSEC	1500.0f
	...
	if (sleep_msecs > 0) {
		chkpt_flush_rate = 1000.0 f / (float)sleep_msecs;
	} else {
		chkpt_flush_rate = 1000.0 f;
	}

	flush_interval = (int)(1000.0 f * PGBUF_CHKPT_BURST_PAGES / chkpt_flush_rate);
...
/*
 * pgbuf_flush_seq_list () - flushes a sequence of pages return:error code or NO_ERROR thread_p(in): seq_flusher(in):
 * container for list of pages limit_time(in): absolute time limit allowed for this call chkpt_smallest_lsa(out):
 * smallest LSA found in a page time_rem(in): time remaining until limit time expires
 * 
 * Note : burst_mode from seq_flusher container controls how the flush is performed: - if enabled, an amount of pages is
 * flushed as soon as possible, according to desired flush rate and time limit - if disabled, the same amount of pages
 * is flushed, but with a pause between each flushed page. Since data flush is concurrent with other IO, burst mode
 * increases the chance that data and other IO sequences do not mix at IO scheduler level and break each-other's
 * sequentiality.
 */
static int
...
#define detailed_er_log(...) if (detailed_logging) _er_log_debug (ARG_FILE_LINE, __VA_ARGS__)
	...
	*time_rem = 0;
	if (limit_time != NULL) {
		...
		control_total_cnt_intervals = (int)(1000. f / (float)seq_flusher->interval_msec + 0.5 f);
	...
	flush_per_interval =
		(int)MAX(flush_per_interval, (PGBUF_CHKPT_MIN_FLUSH_RATE * seq_flusher->interval_msec) / 1000.0 f);
#else
	flush_per_interval = seq_flusher->flush_cnt;
#endif				/* SERVER_MODE */
	...
	for (; seq_flusher->flush_idx < seq_flusher->flush_cnt && seq_flusher->flushed_pages < flush_per_interval;
		...
		/*
		 * prefer sequentiality to an unnecessary flush; skip already flushed page if is the last in list or if
		 * there is already a gap due to missing next page
		 */
		...
		if (pgbuf_bcb_safe_flush_force_lock(thread_p, bufptr, true) == NO_ERROR) {
			if (!LSA_ISNULL(&bufptr->oldest_unflush_lsa)
				...
				/*
				 * I am not sure if this is really possible. But let's assume that bcb was already
				 * flushing before checkpoint reached it. And that it was modified again. And that the
				 * new oldest_unflush_lsa is less than flush_upto_lsa. It may seem that many planets
				 * should align, but let's be conservative and flush again.
				 */
		...
		if (seq_flusher->burst_mode == false && seq_flusher->flush_rate > 0
			...
			if (limit_time != NULL) {
				...
				sleep_msecs = 1000.0 f / (double)(seq_flusher->flush_rate);
			}

			if (sleep_msecs > (1000.0 f / PGBUF_CHKPT_MAX_FLUSH_RATE)) {
...
/*
 * pgbuf_copy_to_area () - Copy a portion of a page to the given area return: area or NULL vpid(in): Complete Page
 * identifier start_offset(in): Start offset of interested content in page length(in): Length of the content of page to
 * copy area(in): Area where to copy the needed content of the page do_fetch(in): Do we want to cache the page in the
 * buffer pool when it is not already cached?
 * 
 * Note: If the page is not in the page buffer pool, it is only buffered when the value of "do_fetch" is false.
 * 
 * WARNING: The user should be very careful on deciding wheater or not to allow buffering of pages. If the page is going
 * to be used in the short future, it is better to allow buffering the page to avoid extra I/O. It is better to avoid
 * I/Os than to avoid memcpys.
 */
void           *
pgbuf_copy_to_area(THREAD_ENTRY * thread_p, const VPID * vpid, int start_offset, int length, void *area, bool do_fetch)
{
	...
	if (bufptr == NULL) {
		...
		if (do_fetch == true) {
			...
			/*
			 * Do not cache the page in the page buffer pool. Read the needed portion of the page directly
			 * from disk
			 */
			if (pgbuf_get_check_page_validation_level(PGBUF_DEBUG_PAGE_VALIDATION_ALL)) {
...
/*
 * pgbuf_copy_from_area () - Copy area to a portion of given page return: area or NULL vpid(in): Complete Page
 * identifier start_offset(in): Start offset of interested content in page length(in): Length of the content of page to
 * copy area(in): Area where to copy the needed content of the page do_fetch(in): Do we want to cache the page in the
 * buffer pool when it is not already cached?
 * 
 * Note: Copy the content of the given area to the page starting at the given offset. If the page is not in the page
 * buffer pool, it is only buffered when the value of "do_fetch" is not false.
 * 
 * WARNING: The user should be very careful on deciding wheater or not to allow buffering of pages. If the page is going
 * to be used in the short future, it is better to allow buffering the page to avoid extra I/O. If you do not buffer
 * the page, not header recovery information is copied along with the write of the page. In this case, the page may not
 * be able to be recovered. DO NOT USE THIS FEATURE IF YOU LOGGED ANYTHING RELATED TO THIS PAGE.
 */
void           *
		     ...
		     bool do_fetch, TDE_ALGORITHM tde_algo)
{
	...
	if (bufptr == NULL) {
		...
		if (do_fetch == false) {
			/*
			 * Do not cache the page in the page buffer pool. Write the desired portion of the page
			 * directly to disk
			 */
			if (pgbuf_get_check_page_validation_level(PGBUF_DEBUG_PAGE_VALIDATION_ALL)) {
				if (pgbuf_is_valid_page(thread_p, vpid, false NULL, NULL) != DISK_VALID) {
...
/*
 * pgbuf_set_dirty () - Mark as modified the buffer associated with pgptr and optionally free the page return: void
 * pgptr(in): Pointer to page free_page(in): Free the page too ?
 */
void
...
/*
 * pgbuf_get_lsa () - Find the log sequence address of the given page return: page lsa pgptr(in): Pointer to page
 */
LOG_LSA        *
...
/*
 * pgbuf_page_has_changed () - check if page has change based on current LSA and a previous reference LSA return: page
 * lsa pgptr(in): Pointer to page
 */
int
...
/*
 * pgbuf_set_lsa () - Set the log sequence address of the page to the given lsa return: page lsa or NULL pgptr(in):
 * Pointer to page lsa_ptr(in): Log Sequence address
 * 
...
pgbuf_set_lsa(THREAD_ENTRY * thread_p, PAGE_PTR pgptr, const LOG_LSA * lsa_ptr)
{
	...
	/*
	 * Don't change LSA of temporary volumes or auxiliary volumes. (e.g., those of copydb, backupdb).
	 */
	...
	/*
	 * Always set the lsa of temporary volumes to the special temp lsa, if it was somehow changed.
	 */
	...
	/*
	 * If this is the first time the page is set dirty, record the new LSA of the page as the oldest_unflush_lsa
	 * for the page. We could have placed these feature when the page is set dirty, unfortunately, some pages are
	 * set dirty before an LSA is set.
	 */
	if (LSA_ISNULL(&bufptr->oldest_unflush_lsa)) {
...
#if defined (NDEBUG)
	/*
	 * We expect the page was or will be set as dirty before unfix. However, there might be a missing case to set
	 * dirty. It is correct to set dirty here. Note that we have set lsa of the page and it should be also flushed.
...
/*
 * pgbuf_reset_temp_lsa () -  Reset LSA of temp volume to special temp LSA (-2,-2) return: void pgptr(in): Pointer to
 * page
 */
void
...
/*
 * pgbuf_set_tde_algorithm () - set tde encryption algorithm to the page return: void thread_p (in)  : Thread entry
 * pgptr(in): Page pointer tde_algo (in) : encryption algorithm - NONE, AES, ARIA
 */
void
pgbuf_set_tde_algorithm(THREAD_ENTRY * thread_p, PAGE_PTR pgptr, TDE_ALGORITHM tde_algo, bool skip_logging)
{
	...
	default:
		...
		/*
		 * pgbuf_rv_set_tde_algorithm () - recovery setting tde encryption algorithm to the page return
		 * : NO_ERROR, or ER_code thread_p (in)  : Thread entry pgptr(in): Page pointer tde_algo (in) :
		 * encryption algorithm - NONE, AES, ARIA
		 */
		int
		...
		/*
		 * pgbuf_get_tde_algorithm () - get tde encryption algorithm of the page return: TDE_ALGORITHM
		 * pgptr(in): Page pointer tde_algo (out) : encryption algorithm - NONE, AES, ARIA
		 */
				TDE_ALGORITHM
		...
		/*
		 * pgbuf_get_vpid () - Find the volume and page identifier associated with the passed buffer return:
		 * void pgptr(in): Page pointer vpid(out): Volume and page identifier
		 */
		void
		 ...
		 * return: pointer to vpid pgptr(in): Page pointer
		 * 
		 * Note: Once the buffer is freed, the content of the vpid pointer may be updated by the page buffer
		 * manager, thus a lot of care should be taken. The values of the vpid pointer must not be altered by
		 * the caller. Once the page is freed, the vpid pointer should not be used any longer.
		 */
		VPID           *
		...
		/*
		 * pgbuf_get_latch_mode () - Find the latch mode associated with the passed buffer return: latch mode
		...
		/*
		 * pgbuf_get_page_id () - Find the page identifier associated with the passed buffer return: PAGEID
		 * pgptr(in): Page pointer
		 */
		PAGEID
		...
		/*
		 * pgbuf_get_page_ptype () - return: pgptr(in): Pointer to page
		 */
		PAGE_TYPE
		...
		/*
		 * pgbuf_get_volume_id () - Find the volume associated with the passed buffer return: VOLID pgptr(in):
		 * Page pointer
		 */
		VOLID
		...
		/*
		 * pgbuf_get_volume_label () - Find the name of the volume associated with the passed buffer return:
		 * Volume label pgptr(in): Page pointer
		 */
		const char     *
		...
		/*
		 * pgbuf_force_to_check_for_interrupts () - Force the page buffer manager to check for possible
		 * interrupts when pages are fetched return: void void(in):
		 */
		void
		...
		/*
		 * pgbuf_is_log_check_for_interrupts () - Force the page buffer manager to check for possible
		 * interrupts when pages are fetched return: if there is interrupt, return true, otherwise return false
		 * void(in):
		 */
				bool
		 ...
		 * return: void pgptr(in): Pointer to page
		 * 
		 * Note: Set the log sequence address of the page to the non recoverable LSA address. In this case the
		 * page is declared a non recoverable page (temporary page). Logging must not be done in a temporary
		 * page, however it is not enforced. A warning message is issued if someone logs something. This
		 * warning will indicate a potential bug.
		 * 
		...
		/*
		 * pgbuf_set_bcb_page_vpid () - return: void bufptr(in): pointer to buffer page
		 * 
		 */
		STATIC_INLINE void
		...
		/*
		 * pgbuf_set_page_ptype () - return: void pgptr(in): Pointer to page ptype(in): page type
		 * 
		...
		/*
		 * pgbuf_is_lsa_temporary () - Find if the page is a temporary one return: true/false pgptr(in):
		 * Pointer to page
		 */
		bool
		...
		/*
		 * pgbuf_is_temporary_volume () - Find if the given permanent volume has been declared for temporary
		 * storage purposes return: true/false volid(in): Volume identifier of last allocated permanent volume
		 */
		STATIC_INLINE	bool
				pgbuf_is_temporary_volume(VOLID volid) {
			/*
			 * TODO: I don't know why page buffer should care about temporary files and what this does, but
			 * it is really annoying. until database is loaded and restarted, I will return false always.
			 */
			if (!LOG_ISRESTARTED()) {
				return false;
			}
		...
		/*
		 * pgbuf_init_BCB_table () - Initializes page buffer BCB table return: NO_ERROR, or ER_code
		 */
		static int
		...
		/*
		 * pgbuf_initialize_hash_table () - Initializes page buffer hash table return: NO_ERROR, or ER_code
		 */
		static int
		...
		/*
		 * pgbuf_initialize_lock_table () - Initializes page buffer lock table return: NO_ERROR, or ER_code
		 */
		static int
		...
		/*
		 * pgbuf_initialize_lru_list () - Initializes the page buffer LRU list return: NO_ERROR, or ER_code
		 */
		static int
		...
		/*
		 * pgbuf_initialize_aout_list () - initialize the Aout list return : error code or NO_ERROR
		 */
		static int
...
#undef PGBUF_LIMIT_AOUT_BUFFERS
		...
		/*
		 * pgbuf_initialize_invalid_list () - Initializes the page buffer invalid list return: NO_ERROR
		 */
		static int
		...
		/*
		 * pgbuf_initialize_thrd_holder () - return: NO_ERROR, or ER_code
		 */
		static int
			...
			/*
			 * Each thread has both free holder list and used(held) holder list. The free holder list of
			 * each thread is initialized to have PGBUF_DEFAULT_FIX_COUNT entries and the used holder list
			 * of each thread is initialized to have no entry.
			 */
			for (i = 0; i < thrd_num_total; i++) {
		...
		/*
		 * pgbuf_allocate_thrd_holder_entry () - Allocates one buffer holder entry from the free holder list of
		 * given thread return: pointer to holder entry or NULL
		 * 
		 * Note: If the free holder list is empty, allocate it from the list of free holder arrays that is shared.
		 */
		STATIC_INLINE PGBUF_HOLDER *
		...
		/*
		 * pgbuf_find_thrd_holder () - Find the holder entry of current thread on the BCB holder list of given
		 * BCB return: pointer to holder entry or NULL bufptr(in):
		 */
		STATIC_INLINE PGBUF_HOLDER *
				pgbuf_find_thrd_holder(THREAD_ENTRY * thread_p, PGBUF_BCB * bufptr) {
			int		thrd_index;
		...
		/*
		 * pgbuf_unlatch_thrd_holder () - decrements fix_count by one to the holder entry of current thread on
		 * the BCB holder list of given BCB return: pointer to holder entry or NULL bufptr(in):
		 */
		STATIC_INLINE int
	...
	exit_on_error:
		...
		/*
		 * pgbuf_remove_thrd_holder () - Remove holder entry from given BCB return: NO_ERROR, or ER_code
		 * holder(in): pointer to holder entry to be removed
		 * 
		 * Note: This function removes the given holder entry from the holder list of given BCB, and then connect
		 * it to the free holder list of the corresponding thread.
		 */
		STATIC_INLINE int
			...
			/*
			 * holder->fix_count is always set to some meaningful value when the holder entry is allocated
			 * for use. So, at this time, we do not need to initialize it. connect the BCB holder entry
			 * into free BCB holder list of given thread.
			 */
	...
	exit_on_error:
		...
		/*
		 * pgbuf_latch_bcb_upon_fix () - return: NO_ERROR, or ER_code bufptr(in): request_mode(in):
		 * buf_lock_acquired(in): condition(in):
		 * 
		 * Note: This function latches BCB with latch mode LatchMode as far as LatchMode is compatible with
		 * bcb->LatchMode and there is not any blocked reader or writer. If it cannot latch the BCB right away,
		 * (1) in case of conditional request, release mutex and return eERROR. (2) in case of unconditional
		 * request, add thread on the BCB queue and release mutex and block the thread. In any case, if
		 * LeafLatchMode is not NO_LATCH and the PageType of the page that BCB points is P_BPLEAF, latch BCB
		 * with latch mode LeafLatchMode.
		 */
		STATIC_INLINE int
			...
			if              (buf_lock_acquired || bufptr->latch_mode == PGBUF_NO_LATCH) {
				...
				if (holder == NULL) {
					/*
					 * It means bufptr->latch_mode was leaked by the previous holder, since there
					 * should be no user except me in SA_MODE.
					 */
					assert(0);
					is_page_idle = true;
				}
			...
			if (bufptr->latch_mode == PGBUF_LATCH_WRITE) {	/* only the holder */
	...
	do_block:
		...
		/*
		 * pgbuf_unlatch_bcb_upon_unfix () - Unlatches BCB return: NO_ERROR, or ER_code bufptr(in):
		 * 
		 * Note: It decrements FixCount by one. If FixCount becomes 0, (1) if LatchMode != FLUSH and LatchMode !=
		 * VICTIM, set LatchMode = NO_LATCH. (2) if BCB waiting queue is empty and Wait is false, replace the
		 * BCB to the top of LRU list. If Flush_Request == TRUE, set LatchMode = FLUSH, flush the buffer by WAL
		 * protocol and wake up threads on the BCB waiting queue. If Flush_Request == FALSE if LatchMode ==
		 * NO_LATCH, then, wake up the threads on the BCB waiting queue. Before return, it releases BCB mutex.
		 */
		STATIC_INLINE int
			...
			if (bufptr->fcnt == 0) {
				...
				/*
				 * When the page buffer in LRU_1_Zone, do not move the page buffer into the top of LRU.
				...
				if (pgbuf_bcb_should_be_moved_to_bottom_lru(bufptr)) {
					...
					case PGBUF_VOID_ZONE:
						/*
						 * bcb was recently allocated. the case may vary from never being used
						 * (or almost never), to up to few percent (when hit ratio is very
						 * low). in any case, this is not needed to be very optimized here, so
						 * the code was moved outside unlatch... do not inline it
						 */
					...
					case PGBUF_LRU_2_ZONE:
						/*
						 * this is the buffer zone between hot and victimized. is less hot than
						 * zone one and we allow boosting (if bcb's are old enough).
						 */
						if (PGBUF_THREAD_SHOULD_IGNORE_UNFIX(thread_p)) {
							/* do nothing */
							/* ... except collecting statistics */
		 ...
		 * 
		 * return                        : void thread_p (in)                 : thread entry bcb (in)
		 * : void zone bcb to unlatch thread_private_lru_index (in) : thread's private lru index. -1 if thread
		 * does not have any private list.
		 * 
		...
		/*
		 * pgbuf_should_move_private_to_shared () - return true if bcb belongs to private lru list and if
		 * should be moved to a shared lru list.
		 * 
		 * return                        : true if move from private to shared is needed. thread_p (in)
		 * : thread entry bcb (in)                      : bcb thread_private_lru_index (in) : thread's private
		 * lru index. -1 if thread does not have any private list.
		 */
		STATIC_INLINE	bool
			...
			/*
			 * two conditions to move from private to shared: 1. bcb is fixed by more than one transaction.
		...
		/*
		 * pgbuf_block_bcb () - Adds it on the BCB waiting queue and block thread return: NO_ERROR, or ER_code
		 * bufptr(in): request_mode(in): request_fcnt(in): as_promote(in): if true, will wait as first promoter
		 * 
		...
		static int
			...
			if (request_mode == PGBUF_LATCH_FLUSH) {
				...
				/*
				 * We do not guarantee that there is no deadlock between page latches. So, we made a
				 * decision that when read/write buffer fix request is not granted immediately, block
				 * the request with timed sleep method. That is, unless the request is not waken up by
				 * other threads within some time interval, the request will be waken up by timeout.
...
#if !defined (NDEBUG)
				/*
				 * To hold mutex is not required because I hold the latch. This means at least my fix
				 * count is kept.
				 */
				assert(0 < bufptr->fcnt);
#endif
...
#if defined(SERVER_MODE)
		/*
		 * pgbuf_timed_sleep_error_handling () - return: bufptr(in): thrd_entry(in):
		 */
		static int
			...
			if              (bufptr->next_wait_thrd == NULL) {
				/*
				 * The thread entry has been already removed from the BCB waiting queue by another
				 * thread.
				 */
				return NO_ERROR;
			}
			...
			while (bufptr->next_wait_thrd != NULL) {
				...
				if (bufptr->latch_mode == PGBUF_LATCH_READ && curr_thrd_entry->request_latch_mode == PGBUF_LATCH_READ) {
					...
					if (curr_thrd_entry->request_latch_mode == PGBUF_LATCH_READ) {
						...
						/*
						 * do not handle BCB holder entry, at here. refer
		...
		/*
		 * pgbuf_timed_sleep () - return: NO_ERROR, or ER_code bufptr(in): thrd_entry(in):
		 */
		static int
	...
	try_again:
			...
			if (r == 0) {
				...
				/*
				 * request_latch_mode == PGBUF_NO_LATCH means that the thread has waken up by timeout.
	...
	er_set_return:
			...
			if (old_wait_msecs == LK_INFINITE_WAIT) {
				...
				if (logtb_is_current_active(thread_p) == true) {
					...
					/*
					 * We are already aborting, fall through. Don't do double aborts that could
		...
		/*
		 * pgbuf_wakeup_reader_writer () - Wakes up blocked threads on the BCB queue with read or write latch
		 * mode
		 * 
		 * return        : error code thread_p (in) : thread entry bufptr (in)   : bcb
		 */
		STATIC_INLINE void
			 ...
			 * how it works:
			 * 
			 * we can have here multiple types of waiters: 1. PGBUF_NO_LATCH - thread gave up waiting for bcb
			 * (interrupted or timed out). just remove it from list. 2. PGBUF_LATCH_FLUSH - thread is
			 * waiting for bcb to be flushed. this is not actually a latch and thread is not awaken here.
			 * bcb must be either marked to be flushed asynchronously or is currently in process of being
			 * flushed. 3. PGBUF_LATCH_READ - multiple threads can be waked at once (all readers at the
			 * head of the list). 4. PGBUF_LATCH_WRITE - only first waiter is waked.
			 */
			...
			for             (thrd_entry = bufptr->next_wait_thrd; thrd_entry != NULL; thrd_entry = next_thrd_entry) {
				...
				/*
				 * if thrd_entry->request_latch_mode is PGBUF_NO_LATCH, it means the corresponding
				 * thread has been waken up by timeout.
				 */
		 ...
		 * return: if success, BCB pointer, otherwise NULL hash_anchor(in): vpid(in):
		 */
	...
	try_again:
			...
			mutex_lock2:
					...
					if (rv == 0) {
						...
						/*
						 * An unconditional request is given for acquiring mutex after
						 * releasing hash_mutex.
						 */
						pthread_mutex_unlock(&hash_anchor->hash_mutex);
						PGBUF_BCB_LOCK(bufptr);
					}
			...
			/*
			 * at this point, if (bufptr != NULL) caller holds bufptr->mutex but not
			 * hash_anchor->hash_mutex if (bufptr == NULL) caller holds hash_anchor->hash_mutex.
			 */
			return bufptr;
		}

		/*
		 * pgbuf_insert_into_hash_chain () - Inserts BCB into the hash chain return: NO_ERROR hash_anchor(in):
		 * hash anchor bufptr(in): pointer to buffer page (BCB)
		 * 
		 * Note: Before insertion, it must hold the mutex of the hash anchor. It doesn't release the mutex of the
		 * hash anchor. The mutex of the hash anchor will be released in the next call of pgbuf_unlock_page ().
		 */
		STATIC_INLINE int
			...
			/*
			 * hash_anchor->hash_mutex is not released at this place. The current BCB is the newly
			 * allocated BCB by the caller and it is connected into the corresponding buffer hash chain,
			 * now. hash_anchor->hahs_mutex will be released in pgbuf_unlock_page () after releasing the
			 * acquired buffer lock on the BCB.
			 */
			return NO_ERROR;
		}

		/*
		 * pgbuf_delete_from_hash_chain () - Deletes BCB from the hash chain return: NO_ERROR, or ER_code
		...
		STATIC_INLINE int
			...
			if (pgbuf_bcb_is_flushing(bufptr)) {
				...
				/*
				 * Someone tries to fix the current buffer page. So, give up selecting current buffer
				 * page as a victim.
				 */
				pthread_mutex_unlock(&hash_anchor->hash_mutex);
				bufptr->latch_mode = PGBUF_NO_LATCH;
		...
		/*
		 * pgbuf_lock_page () - Puts a buffer lock on the buffer lock chain return: If success,
		 * PGBUF_LOCK_HOLDER, otherwise PGBUF_LOCK_WAITER hash_anchor(in): vpid(in):
		 * 
		 * Note: This function is invoked only when the page is not in the buffer hash chain. The caller is
		 * holding hash_anchor->hash_mutex. Before return, the thread releases hash_anchor->hash_mutex.
		 */
		static int
			...
			/*
			 * buf_lock_table is implemented to have one entry for each thread. At first design, it had one
			 * entry for each thread. cur_thrd_entry->index : thread entry index cur_thrd_entry->tran_index
		...
		/*
		 * pgbuf_unlock_page () - Deletes a buffer lock from the buffer lock chain return: NO_ERROR
		 * hash_anchor(in): vpid(in): need_hash_mutex(in):
		 * 
		 * Note: This function is invoked only after the page is read into buffer and the BCB is connected into
		 * its corresponding buffer hash chain. Before return, the thread releases the hash mutex on the hash
		 * anchor and wakes up all the threads blocked on the queue of the buffer lock record.
		 */
		static int
		...
		/*
		 * pgbuf_allocate_bcb () - Allocates a BCB return:  If success, a newly allocated BCB, otherwise NULL
		 * src_vpid(in):
		 * 
		 * Note: This function allocates a BCB from the buffer invalid list or the LRU list. It is invoked only
		 * when a page is not in buffer.
		 */
		static PGBUF_BCB *
			...
			/*
			 * how it works: we need to free a bcb for new VPID. 1. first source should be invalid list.
			 * initially, all bcb's will be in this list. sometimes, bcb's can be added to this list during
			 * runtime. in any case, these bcb's are not used by anyone, do not need any flush or other
			 * actions and are the best option for allocating a bcb. 2. search the bcb in lru lists by
			 * calling pgbuf_get_victim. 3. if search failed then: SERVER_MODE: thread is added to one of
			 * two queues: high priority waiting threads queue or low priority waiting threads queue. high
			 * priority is usually populated by vacuum threads or by threads holding latch on very hot
			 * pages (b-tree roots, heap headers, volume header or file headers). thread will then be
			 * assigned a victim directly (there are multiple ways this can happen) and woken up. TODO: we
			 * have one big vulnerability with waiting threads. what if, for any reason, no one feeds the
			 ...
			 * consider a backup plan to generate victims for a forgotten waiter. SA_MODE: pages are
			 * flushed and victim is searched again (and we expect this time to find a victim).
			 * 
			 * note: SA_MODE approach also applies to server-mode recovery (or in any circumstance which has
			 * page flush thread unavailable).
			 */
		...
		retry:
				...
				if (high_priority) {
					...
					if (!pgbuf_Pool.direct_victims.waiter_threads_low_priority->produce(thread_p)) {
						/*
						 * ok, we have this very weird case when a consumer can be preempted
						 * for a very long time (which prevents producers from being able to
						 * push to queue). I don't know how is this even possible, I just know
						 * I found a case. I cannot tell exactly how long the consumer is
						 * preempted, but I know the time difference between the producer still
						 * waiting to be waken by that consumer and the producer failing to add
						 * was 93 milliseconds. Which is huge if you ask me. I doubled the size
						 * of the queue, but theoretically, this is still possible. I also
						 * removed the ABORT_RELEASE, but we may have to think of a way to
						 * handle this preempted consumer case.
						 */

						/*
						 * we do a hack for this case. we add the thread to high-priority
						 * instead, which is usually less used and the same case is (almost)
				...
				//before migration of the page_flush_daemon it was a try_wakeup, check if still
					needed
		...
		/*
		 * pgbuf_claim_bcb_for_fix () - function used for page fix to claim a bcb when page is not found in
		 * buffer
		 * 
		 * return               : claimed BCB thread_p (in)        : thread entry vpid (in)            : page
		 * identifier fetch_mode (in)      : fetch mode hash_anchor (in/out) : hash anchor perf (in/out)
		 * : page fix performance monitoring helper try_again (out)      : output true to trying getting bcb
		 * again
		 */
		static PGBUF_BCB *
			...
			/*
			 * In this case, the caller is holding only hash_anchor->hash_mutex. The
			 * hash_anchor->hash_mutex is to be released in pgbuf_lock_page ().
			 */
			...
			if (fetch_mode != NEW_PAGE) {
				...
				if (dwb_read_page(thread_p, vpid, &bufptr->iopage_buffer->iopage, &success) != NO_ERROR) {
					...
					/*
					 * There was an error in reading the page. Clean the buffer... since it may
					 * have been corrupted
					 */
					ASSERT_ERROR();
					...
					/*
					 * Now, caller is not holding any mutex. the last argument of pgbuf_unlock_page
					 * () is true that means hash_mutex must be held before unlocking page.
					 */
					(void)pgbuf_unlock_page(thread_p, hash_anchor, vpid, true);
		...
		/*
		 * pgbuf_victimize_bcb () - Victimize given buffer page return: NO_ERROR, or ER_code bufptr(in):
		...
		static int
			...
			/*
			 * If above function returns success, the caller is still holding bufptr->mutex. Otherwise, the
			 * caller does not hold bufptr->mutex.
			 */
		...
		/*
		 * pgbuf_invalidate_bcb () - Invalidates BCB return: NO_ERROR, or ER_code bufptr(in): pointer to buffer
		 * page
		 */
		static int
			...
			if (pgbuf_bcb_is_direct_victim(bufptr)) {
				/*
				 * bcb is already assigned as direct victim, should be victimized soon, so there is no
				 * point in invalidating it here
				 */
				PGBUF_BCB_UNLOCK(bufptr);
				return NO_ERROR;
			}
			...
			if (bufptr->latch_mode == PGBUF_NO_LATCH) {
				...
				/*
				 * If above function returns failure, the caller does not hold bufptr->mutex.
				 * Otherwise, the caller is holding bufptr->mutex.
				 */
		 ...
		 * 
		 * return           : error code thread_p (in)    : thread entry bufptr (in)      : bcb to flush
		 * synchronous (in) : true if caller wants to wait for bcb to be flushed (if it cannot flush
		 * immediately it gets blocked). if false, the caller will only request flush and continue.
		 */
		static int
		...
		/*
		 * pgbuf_bcb_safe_flush_internal () - safe-flush bcb. function will do all the necessary checks. flush
		 * is executed only bcb is dirty. function is safe in regard with concurrent latches and flushes.
		 * 
		 * return           : error code thread_p (in)    : thread entry bufptr (in)      : bcb to flush
		 * synchronous (in) : true if caller wants to wait for bcb to be flushed (if it cannot flush
		 * immediately it gets blocked). if false, the caller will only request flush and continue. locked
		 * (out)     : output if bcb is locked.
		 */
		static int
			...
			/*
			 * there are two cases when we cannot flush immediately: 1. page is write latched. we cannot
			 * know when the latcher makes modifications, so it is not safe to flush the page. 2. another
			 * thread is already flushing. allowing multiple concurrent flushes is not safe (we cannot
			 * guarantee the order of disk writing, therefore it is theoretically possible to write an old
			 * version over a newer version of the page).
			 * 
			 * for the first case, we use the PGBUF_BCB_ASYNC_FLUSH_REQ flag to request a flush from the
			 * thread holding latch. for the second case, we know the bcb is already being flushed. if we
			 * need to be sure page is flushed, we'll put ourselves in bcb's waiting list (and a thread
			 * doing flush should wake us).
			 */
		 ...
		 * 
		 * return: If success, a newly allocated BCB, otherwise NULL thread_p (in)     : thread entry
		 * 
		 * Note: This function disconnects a BCB on the top of the buffer invalid list and returns it. Before
		 * disconnection, the thread must hold the invalid list mutex and after disconnection, release the
		 * mutex.
		 */
		static PGBUF_BCB *
		...
		/*
		 * pgbuf_put_bcb_into_invalid_list () - Put BCB into buffer invalid list return: NO_ERROR bufptr(in):
		 * 
		 * Note: This function connects BCB to the top of the buffer invalid list and makes its zone
		 * PB_INVALIDZone. Before connection, must hold the invalid list mutex and after connection, release
		 * the mutex.
		 */
		static int
		...
		/*
		 * pgbuf_get_shared_lru_index_for_add () - get a shared index to add a new bcb. we'll use a round-robin
		 * way to choose next list, but we'll avoid biggest list (just to keep things balanced).
		 * 
  ...
  MAX (2 * pgbuf_Pool.num_buffers / PGBUF_SHARED_LRU_COUNT, 10000)
			...
			if              (refresh_stat_cnt == 0) {
				...
				if (shared_lru_bcb_sum > pgbuf_Pool.num_buffers / 10
				    && (max_bcb > (int)(1.3 f * shared_lru_bcb_sum) / PGBUF_SHARED_LRU_COUNT || max_bcb > 2 * min_bcb)) {
...
#undef PAGE_ADD_REFRESH_STAT
		 ...
		 * 
		 * return        : victim candidate or NULL if no candidate was found thread_p (in) : thread entry
		 * 
		 * Note: If a victim BCB is found, this function will already lock it. This means that the caller will
		 * have exclusive access to the returned BCB.
		 */
		static PGBUF_BCB *
...
#define PERF(id) if (detailed_perf) perfmon_inc_stat (thread_p, id)
			...
			/*
			 * how this works: we need to find a victim in one of all lru lists. we have two lru list
			 * types: private and shared. private are pages fixed by a single transaction, while shared are
			 * pages fix by multiple transactions. we usually prioritize the private lists. the order we
			 * look for victimize is this: 1. first search in own private list if it is not under quota. 2.
			 * look in another private list. 3. look in a shared list.
			 * 
			 * normally, if the system does not lack victims, one of the three searches should provide a
			 * victim candidate. however, we can be unlucky and not find a candidate with the three steps.
			 * this is especially possible when we have only one active transaction, with long
			 * transactions, and many vacuum workers trying to catch up. all candidates are found in a
			 * single private list, which means that many vacuum workers may not find the lists in lru
			 * queue. for this case, we loop the three searches, as long as pgbuf_Pool.monitor.victim_rich
			 * is true.
			 * 
			 * note: if quota is disabled (although this is not recommended), only shared lists are searched.
			 * 
			 * note: if all above failed to produce a victim, we'll try to victimize from own private even if
			 * it is under quota. we found a strange particular case when all private lists were on par
			 * with their quota's (but just below), shared lists had no lru 3 zone and nothing could be
			 * victimized or flushed.
			 */
			...
			if              (PGBUF_THREAD_HAS_PRIVATE_LRU(thread_p)) {
				...
				if (PGBUF_LRU_LIST_IS_ONE_TWO_OVER_QUOTA(lru_list)
					...
					/*
					 * if over quota, we are not allowed to search in other lru lists. we'll wait
					 * for victim. note: except vacuum threads who ignore unfixes and have no
					 * quota.
					 */
					if (!PGBUF_THREAD_SHOULD_IGNORE_UNFIX(thread_p)) {
						/*
						 * still, offer a chance to those that are just slightly over quota.
						 * this actually targets new transactions that do not have a quota
						 * yet... let them get a few bcb's first until their activity becomes
						 * relevant.
						 */
			 ...
			 * loop:
			 * 
			 * DOESN'T HAVE FLUSH THREAD: one iteration could fail, because the shared list's last victims
			 * have been set dirty. however, if there are other lists having victims, we should find them.
			 * it is possible to not have any victims, in which case the shared list queue should become
			 * empty. we'll have to do a flush and search again. we'd like to avoid looping infinitely (if
			 * there's a bug), so we use the nloops safe-guard. Each shared list should be removed after a
			 * failed search, so the maximum accepted number of loops is pgbuf_Pool.num_LRU_list.
			 */
			...
			/*
			 * todo: maybe we can find a less complicated condition of looping. Probably no need to use
			 * nloops <= pgbuf_Pool.num_LRU_list.
			 */
			if (detailed_perf) {
				PERF_UTIME_TRACKER_TIME(thread_p, &perf_tracker, PSTAT_PB_VICTIM_SEARCH_SHARED_LISTS);
...
#undef PERF
		 ...
		 * 
		 * return               : NO_ERROR PGBUF_BCB * bcb (in) : bcb has_mutex_lock (in)  : true if current
		 * thread has lock on bcb
		 * 
		 * note: if has_mutex_lock is true, even if bcb->latch_mode is not PGBUF_NO_LATCH, we consider this to be
		 * temporary. this must be during pgbuf_unfix and latch_mode will be set to PGBUF_NO_LATCH before bcb
		 * mutex is released.
		 */
		STATIC_INLINE	bool
			...
			/*
			 * note: sometimes, the next wait thread could only be threads waiting for flush. however,
			 * these are exceptional cases. we'd rather miss a few good bcb's from time to time, rather
			 * than processing the waiting list for every bcb.
			 */
		 ...
		 * 
		 * return              : true if bcb can be victimized, false otherwise bcb (in)            : bcb
		...
		/*
		 * pgbuf_get_victim_from_lru_list () - Get victim BCB from the bottom of LRU list return: If success,
		 * BCB, otherwise NULL lru_idx (in)     : index of LRU list
		 * 
		 * Note: This function disconnects BCB from the bottom of the LRU list and returns it if its fcnt == 0. If
		 * its fcnt != 0, makes bufptr->PrevBCB bottom and retry. While this processing, the caller must be the
...
#define MAX_DEPTH 1000
			...
			for (bufptr = bufptr_start; bufptr != NULL && PGBUF_IS_BCB_IN_LRU_VICTIM_ZONE(bufptr) && search_cnt < MAX_DEPTH;
				...
				if (pgbuf_is_bcb_fixed_by_any(bufptr, false)) {
					/*
					 * this bcb cannot be used now, but it is a valid victim candidate. maybe we
					 * should update victim hint
					 */
					if (bufptr_victimizable == NULL) {
						bufptr_victimizable = bufptr;

						/*
						 * update hint if this is not bufptr_start and hint has not changed in
						 * the meantime.
						 */
				...
				/*
				 * a victim candidate. we need to lock its BCB, but since we have LRU mutex, we can
				 * only do it conditionally. chances are we'll get the mutex.
				 */
				if (PGBUF_BCB_TRYLOCK(bufptr) == 0) {
					...
					/*
					 * save the avoid victim bufptr. maybe it will be reset until we finish the
					 * search
					 */
					if (bufptr_victimizable == NULL) {
						bufptr_victimizable = bufptr;
			...
			pgbuf_wakeup_page_flush_daemon(thread_p);
			/*
			 * failed finding victim in single-threaded, although the number of victim candidates is
			 * positive? impossible! note: not really impossible. the thread may have the victimizable
			 * fixed. but bufptr_victimizable must not be NULL.
			 */
...
#undef MAX_DEPTH
		 ...
		 * 
		 * return         : number of assigned victims. thread_p (in)  : thread entry lru_list (in)  : lru list
...
#undef MAX_DEPTH
		...
		/*
		 * pgbuf_direct_victims_maintenance () - assign direct victims via searching. the purpose of function
		 * is to make sure a victim is assigned even when system has low to no activity, which prevents bcb's
		 * from being assigned to a waiting thread. basically, this is the backup plan.
		 * 
		 * return        : void thread_p (in) : thread entry
		 */
		void
...
#define DEFAULT_ASSIGNS_PER_ITERATION 5
			...
			/*
			 * note this is designed for single-threaded use only. the static values are used for pick
			 * lists with a round-robin system
			 */
...
#undef DEFAULT_ASSIGNS_PER_ITERATION
		 ...
		 * 
		 * return                 : void thread_p (in)          : thread entry lru_idx (in)           : lru index
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : bcb added to top lru_list (in) : lru
		 * list
		 */
		STATIC_INLINE void
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : bcb added to middle lru_list (in) :
		 * lru list
		 */
		STATIC_INLINE void
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : bcb added to bottom lru_list (in) :
		 * lru list
		 */
		STATIC_INLINE void
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry lru_list (in) : lru list min_one (in)  : true to stop
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry lru_list (in) : lru list min_one (in)  : true to keep
		 * at least one entry in 1&2 zones.
		 */
		STATIC_INLINE void
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : bcb in lru list lru_list (in) : lru
		 * list
		 */
		STATIC_INLINE void
			...
			if (pgbuf_is_bcb_victimizable(bcb, false) && pgbuf_is_any_thread_waiting_for_direct_victim()) {
				if (pgbuf_bcb_is_to_vacuum(bcb)) {
					...
					/*
					 * we first need mutex on bcb. however, we'd normally first get mutex on bcb
					 * and then on list. since we don't want to over complicate things, just try a
					 * conditional lock on mutex. if it fails, we'll just give up assigning the bcb
					 * directly as victim
					 */
					if (PGBUF_BCB_TRYLOCK(bcb) == 0) {
						...
						if (pgbuf_is_bcb_victimizable(bcb, true) && pgbuf_assign_direct_victim(thread_p, bcb)) {
							...
							/*
							 * since bcb is going to be removed from list and I have both
							 * lru and bcb mutex, why not do it now.
							 */
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : bcb to move to top
		 */
		static void
			...
			/*
			 * rules to boosting bcb's in lru lists (also see code in pgbuf_unlatch_bcb_upon_unfix): 1.
			 * never boost bcb's in zone 1. this is usually the hottest part of the lists and should have a
			 * big hit ratio. we'd like to avoid locking list mutex and making changes, these bcb's are in
			 * no danger of being victimized, so we just don't move them. 2. avoid boosting new and cold
			 * bcb's. a bcb can be fixed/unfixed several times and still be cold. many operations will fix
			 * a page at least twice (once to read and once to write), and we'd like to avoid boosting the
			 * bcb on second unfix. we do have a trick to detect such cases. we keep the list tick whenever
			 * new bcb's are inserted to zones 1 and 2. if a page is quickly fixed several times, its "age"
			 * is really small (age being the difference between the bcb's saved tick and current list
			 * tick), and we don't boost it. it should be unfixed again after aging a little before being
			 * boosted to top. 3. always boost from third zone, since these are decently old.
			 * 
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : new bcb lru_idx (in)  : lru list
		 * index
		 */
		STATIC_INLINE void
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : bcb
		 */
		STATIC_INLINE void
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : private list bcb
		 */
		static void
			...
			/*
			 * note: from statistics analysis, moves from private to shared are very rare, so we don't
			 * inline the function
			 */
		...
		/*
		 * pgbuf_remove_from_lru_list () - Remove a BCB from the LRU list return : void bufptr (in) : BCB
		...
		STATIC_INLINE void
			 ...
			 * pgbuf_lru_remove_victim_candidate will not which is the previous BCB. we cannot change the
			 * hint before disconnecting the bcb from list, we need to be sure no one else sets the hint to
			 * this bcb.
			 */
		...
		/*
		 * pgbuf_move_bcb_to_bottom_lru () - move a bcb to the bottom of its lru (or other lru if it is in the
		 * void zone).
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : bcb
		 */
		...
		/*
		 * pgbuf_add_vpid_to_aout_list () - add VPID to Aout list return : void thread_p (in) : vpid (in) :
		...
		/*
		 * pgbuf_remove_vpid_from_aout_list () - Search for VPID in Aout and remove it from the queue return :
		 * identifier of list from which was removed: 0 and positive: LRU list PGBUF_AOUT_NOT_FOUND: not found
		 * in Aout list thread_p (in) : vpid (in) :
		 */
		static int
			...
			/*
			 * We can assume that aout_buf is what we're looking for if it still has the same VPID as
			 * before acquiring the mutex. The reason for this is that nobody can change it while we're
			 * holding the mutex. Any changes must be visible before we acquire this mutex
			 */
		...
		/*
		 * pgbuf_remove_private_from_aout_list () - Search for VPID in Aout and removes all VPIDs having a
		 * specific LRU idx
		 * 
		 * return : number of VPIDs removed lru_idx (in) :
		 */
		static int
		 ...
		 * 
		 * return                    : error code thread_p (in)             : thread entry bufptr (in)
		 * : bcb is_page_flush_thread (in) : true if caller is page flush thread. false otherwise.
		...
		STATIC_INLINE int
			 ...
			 * how this works:
			 * 
			 * caller should already have bcb locked. we don't do checks of opportunity or correctness here
			 * (that's up to the caller).
			 * 
			 * we copy the page and save oldest_unflush_lsa and then we try to write the page to disk. if
			 * writing fails, we "revert" changes (restore dirty flag and oldest_unflush_lsa).
			 * 
			 * if successful, we choose one of the paths: 1. send the page to post-flush to process it and
			 * assign it directly (if this is page flush thread and victimization system is stressed). 2.
			 * lock bcb again, clear is flushing status, wake up of threads waiting for flush and return.
			 */
	...
	copy_unflushed_lsa:
			...
			*is_bcb_locked = false;
			...
			/*
			 * Activating/deactivating DWB while the server is alive, needs additional work. For now, we
			 * don't care about this case, we can use it to test performance differences.
			 */
			if (uses_dwb) {
		 ...
		 * 
		 * return        : void thread_p (in) : thread entry bcb (in)      : flushed bcb
		 */
		static void
		 ...
		 * return: if found, true, otherwise, false bufptr(in): pointer to buffer page
		 */
		STATIC_INLINE	bool
		...
		/*
		 * pgbuf_get_check_page_validation_level - return:
		 * 
		 */
		STATIC_INLINE	bool
		...
		/*
		 * pgbuf_is_valid_page () - Verify if given page is a valid one return: either: DISK_INVALID,
		 * DISK_VALID, DISK_ERROR vpid(in): Complete Page identifier fun(in): A second function to call to
		 * verify if the above page is valid The function is called on vpid, and arguments args(in): Additional
		 * argument for fun
		 * 
		 * Note: Verify that the given page is valid according to functions: 1) disk_isvalid_page 2) given fun2 is
		 * any The function is a NOOP if we are not running with full debugging capabilities.
		 */
				DISK_ISVALID
		...
		/*
		 * pgbuf_is_valid_page_ptr () - Validate an in-memory page pointer return: true/false pgptr(in):
		 * Pointer to page
		 * 
		 * Note: Verify if the given page pointer points to the beginning of a in-memory page pointer. This
		 ...
		 * 
		 * return	 : True if the page type is as expected. thread_p (in) : Thread entry. pgptr (in)	 :
		 * Pointer to buffer page. ptype (in)	 : Expected page type.
		 */
		bool
		...
		/*
		 * pgbuf_check_page_type_no_error () - Return if the page type is the expected type given as argument.
		 * No assert is hit if not.
		 * 
		 * return	 : True if the page type is as expected. thread_p (in) : Thread entry. pgptr (in)	 :
		 * Pointer to buffer page. ptype (in)	 : Expected page type.
		 */
		...
		/*
		 * pgbuf_check_page_ptype_internal () - return: true/false bufptr(in): pointer to buffer page
		 * ptype(in): page type
		 * 
		 * Note: Verify if the given page's ptype is valid. This function is used for debugging purposes.
		 */
		STATIC_INLINE	bool
		...
		/*
		 * pgbuf_check_bcb_page_vpid () - Validate an FILEIO_PAGE prv return: true/false bufptr(in): pointer to
		 * buffer page maybe_deallocated(in) : true, if page may be deallocated
		 * 
		 * Note: Verify if the given page's prv is valid. This function is used for debugging purposes.
		 */
		STATIC_INLINE	bool
...
#if defined(CUBRID_DEBUG)
		/*
		 * pgbuf_scramble () - Scramble the content of the buffer return: void iopage(in): Pointer to page
		 * portion
		 * 
		 * Note: This is done for debugging reasons to make sure that a user of a buffer does not assume that
		 * buffers are initialized to zero. For safty reasons, the buffers are initialized to zero, instead of
		 * scrambled, when running in production mode.
		 */
		static void
		...
		/*
		 * pgbuf_dump_if_any_fixed () - Dump buffer pool if any page buffer is fixed return: void
		 * 
		 * Note: This is a debugging function that can be used to verify if buffers were freed after a set of
		 * operations (e.g., a request or a API function). This function will not give you good results when
		 * there are multiple users in the system (multiprocessing)
		 */
		void
		...
		/*
		 * pgbuf_dump () - Dump the system area of each buffer return: void
		 * 
		...
		/*
		 * pgbuf_is_consistent () - Check if a page is consistent return: bufptr(in): Pointer to buffer
		 * likely_bad_after_fixcnt(in): Don't tell me that he page is bad if fixcnt is greater that this
		 * 
		 * Note: Consistency rule: If memory page is dirty, the content of page should be different to the content
		 * of the page on disk, otherwise, page is considered inconsistent. That is, someone set a page dirty
		 * without updating the page. This rule may fail since a page can be updated with the same content that
		 * the page on disk, however, this is a remote case.
		 * 
		 * If memory page is not dirty, the content of page should be identical to the content of the page on
		 * disk, otherwise, page is considered inconsistent. This is the case that someone updates the page
		 * without setting it dirty.
		 */
		static int
			...
			if              (!VPID_ISNULL(&bufptr->vpid)) {
				...
				if (fileio_read(NULL, fileio_get_volume_descriptor(bufptr->vpid.volid), malloc_io_pgptr, bufptr->vpid.pageid,
					...
					if (!LSA_EQ(&malloc_io_pgptr->prv.lsa, &bufptr->iopage_buffer->iopage.prv.lsa)
						...
						/*
						 * If fix count is greater than likely_bad_after_fixcnt, the function
						 * cannot state that the page is bad
						 */
		...
		/*
		 * pgbuf_wakeup_page_flush_daemon () - Wakeup the flushing daemon thread to flush some of the dirty
		 * pages in buffer pool to disk return : void thread_p (in) :
		 */
		static void
		 ...
		 * 
		 * return	       : The number of pages fixed by the thread. thread_p (in)       : Thread entry.
		 * 
		 */
				bool
		 ...
		 * 
		 * return	       : true if the threads has any fixed pages and the other is waiting on any of them or it
		 * has an important hot page such as volume header, file header, index root and heap header. thread_p
		 * (in)       : Thread entry.
		 */
		static		bool
		...
		/*
		 * pgbuf_flush_page_and_neighbors_fb () - Flush page pointed to by the supplied BCB and also flush
		 * neighbor pages
		 * 
		 * return : error code or NO_ERROR thread_p (in) : thread entry bufptr (in)	 : BCB to flush
...
#undef PGBUF_PAGES_COUNT_THRESHOLD
		...
		/*
		 * pgbuf_add_bufptr_to_batch () - Add a page to the flush helper return : void bufptr (in) : BCB of
		 * page to add
		 */
		STATIC_INLINE void
		...
		/*
		 * pgbuf_flush_neighbor_safe () - Flush collected page for neighbor flush if it's safe: 1. VPID of
		 * bufptr has not changed. 2. Page has no latch or is only latched for read.
		 * 
		 * return	      : Error code. thread_p (in)      : Thread entry. bufptr (in)	      : Buffered page
		 * collected for neighbor flush. expected_vpid (in) : Expected VPID for bufptr. flushed (out)      :
		 * Output true if page was flushed.
		 */
		STATIC_INLINE int
		...
		/*
		 * pgbuf_compare_hold_vpid_for_sort () - Compare the vpid for sort return: p1 - p2 p1(in): victim
		 * candidate list 1 p2(in): victim candidate list 2
		 */
		static int
		...
		/*
		 * pgbuf_ordered_fix () - Fix page in VPID order; other previously fixed pages may be unfixed and
		 * re-fixed again. return: error code thread_p(in): req_vpid(in): fetch_mode(in): old or new page
		 * request_mode(in): latch mode req_watcher(in/out): page watcher object, also holds output page
		 * pointer
		 * 
		 * Note: If fails to re-fix previously fixed pages (unfixed with this request), the requested page is
		 * unfixed (if fixed) and error is returned. In such case, older some pages may be re-fixed, other not
		 * : the caller should check page pointer of watchers before using them in case of error.
		 * 
		 * Note2: If any page re-fix occurs for previously fixed pages, their 'unfix' flag in their watcher is
		 * set. (caller is responsible to check this flag)
		 * 
		 */
#if !defined(NDEBUG)
		int
...
#endif				/* NDEBUG */
		{
			...
			if (ret_pgptr != NULL) {
				...
				if (wait_msecs == LK_ZERO_WAIT || wait_msecs == LK_FORCE_ZERO_WAIT) {
					/*
					 * attempts to unfix-refix old page may fail since CONDITIONAL latch will be
					 * enforced; just return page cannot be fixed
					 */
					if (er_status == NO_ERROR) {
						/*
						 * LK_FORCE_ZERO_WAIT is used in some page scan functions (e.g.
						 * heap_stats_find_page_in_bestspace) to skip busy pages; here we
						 * return an error code (which means the page was not fixed), however
						 * no error is set : this allows scan of pages to continue
						 */
			...
			while (holder != NULL) {
				...
				if (holder->watch_count <= 0) {
					/*
					 * cannot perform unfix-ordered fix without watcher; we assume that this
					 * holder's page will not trigger a latch deadlock and ignore it
					 */
					holder = next_holder;
					continue;
				}
				...
				if (saved_pages_cnt >= PGBUF_MAX_PAGE_FIXED_BY_TRAN) {
					...
					if (holder_fix_cnt != holder->watch_count) {
						/*
						 * this page was fixed without watcher, without being unfixed before
						 * another page fix ; we do not allow this
						 */
						assert_release(false);
					...
					if (diff < 0) {
						...
						assert(diff > 0);
						/*
						 * this page is correctly fixed before new requested page, the
						 * accumulated watchers are just ignored
						 */
#if defined(PGBUF_ORDERED_DEBUG)
						_er_log_debug(__FILE__, __LINE__,
			...
			/*
			 * the following code assumes that if the class OID is deleted, after the requested page is
			 * unlatched, the HFID page is not reassigned to an ordinary page; in such case, a page
			 * deadlock may occur in worst case. Example of scenario when such situation may occur : We
			 * assume an existing latch on VPID1 (0, 90) 1. Fix requested page VPID2 (0, 100), get
			 * class_oid from page 2. Unfix requested page 3. Get HFID from schema : < between 2 and 3,
			 * other threads drop the class, and HFID page is reused, along with current page which may be
			 * allocated to the HFID of another class > 4. Still assuming that HFID is valid, this thread
			 * starts latching pages: In order VPID1, VPID2 At the same time, another thread, starts
			 * latching pages VPID1 and VPID2, but since this later thread knows that VPID2 is a HFID, will
			 * use the order VPID2, VPID1.
			 */
			if (req_page_has_group == false) {
#if !defined(NDEBUG)
			...
			for (i = 0; i < saved_pages_cnt; i++) {
				...
				/*
				 * get holder of last fix: last fixed pages is in top of holder list, we use parse code
				 * just for safety
				 */
				for (holder = pgbuf_Pool.thrd_holder_info[thrd_idx].thrd_hold_list; holder != NULL; holder = holder->thrd_link) {
				...
				if (VPID_EQ(req_vpid, &(ordered_holders_info[i].vpid))) {
					...
					/*
					 * restore number of fixes for previously fixed page: just use pgbuf_fix since
					 * it is safer
					 */
	...
	exit:
		...
		/*
		 * pgbuf_get_groupid_and_unfix () - retrieves group identifier of page and performs unlatch if
		 * requested. return: error code req_vpid(in): id of page for which the group is needed (for debug)
		 * pgptr(in): page (already latched); only heap page allowed groupid(out): group identifier (VPID of
		 * HFID) do_unfix(in): if true, it unfixes the page.
		 * 
		...
		/*
		 * pgbuf_ordered_unfix () - Unfix a page which was previously fixed with ordered_fix (has a page
		 * watcher) return: void thread_p(in): watcher_object(in/out): page watcher
		 * 
		 */
#if !defined (NDEBUG)
		void
		...
		/*
		 * pgbuf_add_watch_instance_internal () - Adds a page watcher for a fixed page holder(in): holder
		 * object pgptr(in): holder object watcher(in/out): page watcher latch_mode(in): latch mode used for
		 * fixing the page clear_unfix_flag(in): True to reset page_was_unfixed flag, false otherwise.
		 * 
		 */
#if !defined(NDEBUG)
		STATIC_INLINE void
		 ...
		 * 
		 * return	   : Void. thread_p (in)   : Thread entry. pgptr (in)	   : Fixed page pointer. latch_mode
		 * (in) : Latch mode. hfid (in)	   : Heap file identifier. watcher (out)   : Page water.
		 */
#if !defined (NDEBUG)
		void
		...
		/*
		 * pgbuf_get_holder () - Searches holder of fixed page Return : holder object or NULL if not found
		 * thread_p(in): pgptr(in): pgptr
		 */
		static PGBUF_HOLDER *
		...
		/*
		 * pgbuf_remove_watcher () - Removes a page watcher holder(in): holder object watcher_object(in):
		 * watcher object
		 */
		static void
		...
		/*
		 * pgbuf_replace_watcher () - Replaces a page watcher with another page watcher thread_p(in):
		 * old_watcher(in/out): current page watcher to replace new_watcher(in/out): new page watcher to use
		 * 
		 */
#if !defined(NDEBUG)
		void
		 ...
		 * (previously fixed with ordered fix) return: void thread_p(in): pg_watcher(in): page watcher holding
		 * the page to dirty and unfix
		 */
		void
		...
		/*
		 * pgbuf_get_condition_for_ordered_fix () - returns the condition which should be used to latch
		 * (vpid_new_page) knowing that we already have a latch on (vpid_fixed_page)
		 * 
		 * return: latch condition (PGBUF_LATCH_CONDITION) vpid_new_page(in): vpid_fixed_page(in): hfid(in): HFID
		 * of both pages
		 * 
		 * Note: This is intended only for HEAP/HEAP_OVERFLOW pages. The user should make sure both pages belong
		 * to the same heap. To be used when pgbuf_ordered_fix is not possible: In vacuum context, unfixing a
		 * older page to prevent deadlatch, requires flushing of the old page first - this is not possible with
		 * pgbuf_ordered_fix.
		 */
		int
...
#if !defined(NDEBUG)
		/*
		 * pgbuf_watcher_init_debug () - return: void watcher(in/out): add(in): if add or reset the "init"
		 * field
		 */
		void
		...
		/*
		 * pgbuf_is_page_fixed_by_thread () - return: true if page is already fixed, false otherwise
		 * thread_p(in): thread entry vpid_p(in): virtual page id
		 */
		bool
		...
		/*
		 * pgbuf_initialize_page_quota () - Initializes page quota return: NO_ERROR, or ER_code
		 */
		static int
			...
			if (PGBUF_PAGE_QUOTA_IS_ENABLED) {
				quota->private_pages_ratio = 1.0 f;
			} else {
	...
	exit:
		...
		/*
		 * pgbuf_initialize_page_monitor () - Initializes page monitor return: NO_ERROR, or ER_code
		 */
		static int
	...
	exit:
		 ...
		 * 
		 * lru_sum_flush_priority(out) : sum of all flush priorities of all LRUs return : void
		 */
		static void
			...
			PGBUF_LRU_LIST *lru_list;
				 ...
				 &&		pgbuf_Pool.	monitor.lru_shared_pgs_cnt <= pgbuf_Pool.num_buffers);
					...
					prv_real_ratio = 1.0 f - ((float)pgbuf_Pool.monitor.lru_shared_pgs_cnt / pgbuf_Pool.num_buffers);
					diff =	prv_quota - prv_real_ratio;

					prv_flush_ratio = prv_real_ratio * (1.0 f - diff);
					prv_flush_ratio = MIN(1.0 f, prv_flush_ratio);

			for             (i = PGBUF_LRU_INDEX_FROM_PRIVATE(0); i < PGBUF_TOTAL_LRU_COUNT; i++) {
				lru_list = PGBUF_GET_LRU_LIST(i);

				/*
				 * note: we target especially over quota private lists or close to quota. we cannot
				 * target only over quota lists (I tried), because you may find yourself in the
				 * peculiar case where quota's are on par with list size, while shared are right below
				 * minimum desired size... and flush will not find anything.
				 */
			...
			if              (total_prv_target == 0) {
				...
				if (pgbuf_Pool.monitor.lru_shared_pgs_cnt
					...
					/*
					 * we won't be able to victimize from shared. this is a backup hack, I don't
					 * like to rely on it. let's find smarter ways to avoid the case.
					 */
					/*
					 * right now, considering we target all bcb's beyond 90% of quota, but
					 * total_prv_target is 0, that means all private bcb's must be less than 90% of
					 * buffer. that means shared bcb's have to be 10% or more of buffer.
					 * PGBUF_MIN_SHARED_LIST_ADJUST_SIZE is currently set to 50, which is 5% to
					 * targeted 1k shared list size. we shouldn't be here unless I messed up the
					 * calculus.
					 */
					if (pgbuf_Pool.buf_invalid_list.invalid_cnt > 0) {
						/*
						 * This is not really an interesting case. Probably both shared and
						 * private are small and most of buffers in invalid list. We don't
						 * really need flush for the case, since BCB could be allocated from
						 * invalid list.
						 */
						return;
					}
					assert(false);
					use_prv_size = true;
					prv_flush_ratio = 1.0 f;
					/*
					 * we can compute the zone 3 total size (for privates, zones 1 & 2 are both set
					 * to minimum ratio).
					 */
					total_prv_target =
						(int)((pgbuf_Pool.num_buffers - pgbuf_Pool.monitor.lru_shared_pgs_cnt)
						      * (1.0 f - 2 * PGBUF_LRU_ZONE_MIN_RATIO));
				}
			}
			shared_flush_ratio = 1.0 f - prv_flush_ratio;
			...
			for (i = 0; i < PGBUF_TOTAL_LRU_COUNT; i++) {
				...
				if (PGBUF_IS_SHARED_LRU_INDEX(i)) {
					...
					if (prv_flush_ratio == 0.0 f) {
						pgbuf_Pool.quota.lru_victim_flush_priority_per_lru[i] = 0.0 f;
					} else {
						if (use_prv_size) {
							/*
							 * back plan: use zone 3 size instead of computed target based
							 * on quota.
							 */
		...
		/*
		 * pgbuf_adjust_quotas () - Adjusts the quotas for private LRU's. The quota's are decided based on
		 * thread activities on private and shared lists. Activity is counted as number of accessed pages.
...
#define MIN_PRIVATE_RATIO 0.01f
			...
			/*
			 * quota adjust if : - or more than 500 msec since last adjustment and activity is more than
			 * threshold - or more than 5 min since last adjustment and activity is more 1% of threshold
			...
			/*
			 * process hits since last adjust: 1. collect lru_private_hits and lru_shared_hits. 2. update
			 * each private list activity. 3. collect total activity.
			 */
			for (i = 0; i < PGBUF_TOTAL_LRU_COUNT; i++) {
				...
				if (PGBUF_IS_PRIVATE_LRU_INDEX(i)) {
					/*
					 * adjust private lru activity. for convenience reasons, we consider that
					 * previous lru_activity value was same for 10 seconds minus the time since
					 * last adjustment. if previous adjustment is more than 10 seconds old then we
					 * set new activity.
					 */
			...
			/*
			 * is pool victim rich? we consider this true if the victim count is more than 10% of page
			 * buffer. I think we could lower the bar a little bit
			 */
		...
		/*
		 * pgbuf_assign_private_lru_id () - return: NO_ERROR
		 */
		int
			...
			/*
			 * Priority for choosing a private list : 1. the list with zero sessions having the least
			 * number of pages 2. the list having least activity
			 */
	...
	retry:
		...
		/*
		 * pgbuf_release_private_lru () - return: NO_ERROR bufptr(in): pointer to buffer page
		 * 
		 ...
		 * 
		 * return: error code seq_flusher(in/out): f_list(in/out): flush list to use or NULL if needs to be
		 * allocated cnt(in/out): size of flush list
		 */
		static int
		 ...
		 * 
		 * return     : True if page has any waiters, false otherwise. pgptr (in) : Page pointer.
		 */
		bool
		 ...
		 * 
		 * return     : True if page has waiters, false otherwise. pgptr (in) : Page pointer.
		 */
		bool
		...
		int
			...
			/*
			 * If the dirty ratio is now above the desired level, try to suggest a more aggressive flush to
			 * bring it back.
			 */
			...
			/*
			 * Now consider dirty growth rate. Even if page buffer dirty ratio is not yet reached, try to
			 * avoid a sharp growth. Flush may be not be aggressive enough and may require time to get
			 * there. In the meantime, the dirty ratio could go well beyond the desired ratio.
			 */
		...
		/*
		 * pgbuf_rv_flush_page () - Flush page during recovery. Some changes must be flushed immediately to
		 * provide consistency, in case server crashes again during recovery.
		 * 
		 * return	 : Error code. thread_p (in) : Thread entry. rcv (in)	 : Recovery data (VPID of page to
		 * flush).
		 */
		int
		 ...
		 * 
		 * return      : Void. fp (in)     : Output target. length (in) : Length of recovery data. data (in)   :
		 * Recovery data (VPID of page to flush).
		 */
		void
		 ...
		 * 
		 * return          : const char * latch_mode (in) :
		 */
		static const char *
		 ...
		 * 
		 * return          : const char * consistent (in) :
		 */
		static const char *
		 ...
		 * 
		 * return     : Fix count. pgptr (in) : Page pointer.
		 */
		int
		 ...
		 * 
		 * return        : Hold count thread_p (in) : Thread entry
		 */
		int
		 ...
		 * 
		 * return        : page type pgptr (in)    : pointer to a page
		 */
				PERF_PAGE_TYPE
		 ...
		 * 
		 * return         : error code thread_p (in)  : thread entry page_new (in)  : new page data_size (in) :
		 * size of page data ptype_new (in) : new page type
		 */
		void
		 ...
		 * 
		 * return	 : NO_ERROR. thread_p (in) : Thread entry. rcv (in)	 : Recovery data.
		 */
		int
		 ...
		 * 
		 * return        : error code thread_p (in) : thread entry page (in)     : page to deallocate
		 */
		void
			...
			char		undo_data [8];
			              //pageid(4) + volid(2) + pyte(1) + pflag(1)
			int		holder_status;

			/*
			 * how it works: page is "deallocated" by resetting its type to PAGE_UNKNOWN. also prepare bcb
			 * for victimization.
			 * 
			 * note: the bcb used to be invalidated. but that means flushing page to disk and waiting for IO
			 * write. that may be too slow. if we add the bcb to the bottom of a lru list, it will be
			 * eventually flushed by flush thread and victimized.
			 */
		...
		/*
		 * pgbuf_rv_dealloc_undo () - undo page deallocation. the page is validated by setting its page type
		 * back.
		 * 
		 * return        : error code thread_p (in) : thread entry rcv (in)      : recovery data
		 * 
		 * note: we had to make this function logical, because if a page is deallocated, it cannot be fixed,
		 * unless we use fetch type OLD_PAGE_DEALLOCATED.
		 */
		int
		...
		/*
		 * pgbuf_rv_dealloc_undo_compensate () - compensation for undo page deallocation. the page is validated
		 * by setting its page type back.
		 * 
		 * return        : error code thread_p (in) : thread entry rcv (in)      : recovery data
		 * 
		 */
		...
		/*
		 * pgbuf_fix_if_not_deallocated () - fix a page if it is not deallocated. the difference compared to
		 * regulat page fix finding deallocated pages is expected. if the page is indeed deallocated, it will
		 * not fix it
		 * 
		 * return               : error code thread_p (in)        : thread entry vpid (in)            : page
		 * identifier latch_mode (in)      : latch mode latch_condition (in) : latch condition page (out)
		 * : output fixed page if not deallocated. output NULL if deallocated. caller_file (in)     : caller
		 * file name caller_line (in)     : caller line
		 */
		int
			...
			/*
			 * First, checks whether the file was destroyed. Such check may create performance issues. This
			 * function must be adapted. Thus, if the transaction has a lock on table, we can skip the code
			 * that checks whether the file was destroyed.
			 */
		...
		/*
		 * pgbuf_assign_direct_victim () - try to assign bcb directly to a thread waiting for victim. bcb must
		 * be a valid victim candidate
		 * 
		 * return        : true if bcb was assigned directly as victim, false otherwise thread_p (in) : thread
		 * entry bcb (in)      : bcb to assign as victim
		 */
		STATIC_INLINE	bool
			...
			/*
			 * must hold bcb mutex and victimization should be possible. the only victim-candidate
			 * invalidating flag allowed here is PGBUF_BCB_FLUSHING_TO_DISK_FLAG (because flush also calls
			 * this).
			 */
			...
			/*
			 * is flushing is expected, since this is called from flush too. caller should make sure no
			 * other case should get here with is flushing true.
			 */
			/*
			 * if marked as victim candidate, we are sorry for the one that marked it. we'll override the
			 * flag.
			 */
		...
		/*
		 * pgbuf_assign_flushed_pages () - assign flushed pages directly. or just mark them as flushed if it
		 * cannot be assigned.
		 * 
		 * return        : void thread_p (in) : thread entry
		 */
		bool
			...
			/*
			 * invalidation flag for direct victim assignment: any flag invalidating victim candidates,
			 * except is flushing flag
			 */
		 ...
		 * 
		 * return                   : true if got thread, false otherwise waiting_thread_out (out) : output thread
		 * waiting for victim
		 */
		STATIC_INLINE	bool
		 ...
		 * 
		 * return        : pointer to victim bcb thread_p (in) : thread entry
		 */
		STATIC_INLINE PGBUF_BCB *
		 ...
		 * 
		 * return        : void lru_list (in) : lru list
		 */
		STATIC_INLINE void
			...
			/*
			 * We don't own the LRU mutex here, so after we read the victim_hint, another thread may change
			 * that BCB, or the victim_hint pointer itself. All changes of lru_list->victim_hint, must be
			 * precedeed by changing the new hint BCB to LRU3 zone, the checks must be repetead here in the
			 * same sequence: 1. read lru_list->victim_hint 2. stop if old_victim_hint is still in LRU3 and
			 * is older than proposed to be hint 3. atomically change the hint (old_victim_hint may suffer
			 * other changes including relocating to another LRU, this is protected by the atomic op)
			 */
			do {
				/*
				 * replace current victim hint only if this candidate is better. that is if its age in
				 * zone 3 is greater that of current hint's
				 */
				...
				/*
				 * compare & swap. if it fails, the hint must have been updated by someone else (it is
				 * possible even if we hold lru and bcb mutexes, see pgbuf_set_dirty). we try until we
				 * succeed changing the hint or until the current hint is better.
				 */
			}
			...
			/*
			 * add to lock-free circular queue so victimizers can find it... if this is not a private list
			 * under quota.
			 */
		...
		STATIC_INLINE void
			...
			if (ATOMIC_INC_32(&lru_list->count_vict_cand, -1) == 0) {
				/*
				 * we cannot remove an entry from lock-free circular queue easily. we just hope that
				 * this does not happen too often. do nothing here.
				 */
			}
		}

		/*
		 * pgbuf_lru_advance_victim_hint () - invalidate bcb_prev_hint as victim hint and advance to
		 * bcb_new_hint (if possible). in the case we'd reset hint to NULL, but we know victim candidates still
		 * exist, hint is set to list bottom.
		 * 
		 * return                      : void thread_p (in)               : thread entry lru_list (in)
		 * : LRU list bcb_prev_hint (in)          : bcb being invalidated as hint bcb_new_hint (in)           :
		 * new desired hint (can be adjusted to NULL or bottom) was_vict_count_updated (in) : was victim count
		 * updated? (false if bcb_prev_hint is still counted as victim candidate)
		 */
		STATIC_INLINE void
		 ...
		 * 
		 * return           : void bcb (in)         : bcb set_flags (in)   : flags to set clear_flags (in) : flags
		 * to clear
		 * 
		 * note: this makes sure the bcb flags field (which is actually flags + zone + lru index) is modified
		 * atomically. it also handles changes of victim candidates.
		 */
		STATIC_INLINE void
			...
			if (PGBUF_GET_ZONE(old_flags) == PGBUF_LRU_3_ZONE) {
				/*
				 * bcb is in lru zone that can be victimized. some flags invalidate the victimization
				 * candidacy of a bcb; therefore we need to check if the bcb status regarding
				 * victimization is changed.
				 */
		...
		/*
		 * pgbuf_bcb_change_zone () - change the zone and lru index of bcb, but keep the bcb flags. also
		 * handles the zone counters, victim counter and victim hint for lru lists.
		 * 
		 * return       : void bcb (in)     : bcb lru_idx (in) : lru index (0 if not in any lru zone) zone (in)
		 * : zone
		 * 
		 * this is called whenever the bcb is moved from a logical zone to another. possible transitions:
		 * 
		 * FIXME: correct the following description 1. get from invalid list                 invalid      => void
		 * (bcb is locked) 2. get victim                            lru          => void  (list & bcb are
		 * locked) 3. unfix                                 void/lru     => lru   (list & bcb are locked) 4.
		 * lru adjust zones                      lru          => lru   (list is locked)
		 * 
		 * note: two simultaneous change zones on the same bcb should not be possible. the only case when bcb is
		 * not locked is case 4, however list is locked. other possible cases that can call change zone on same
		 * bcb must have lock on lru mutex.
		 * 
		 * note: bcb->flags is changed here and simultaneous calls of pgbuf_bcb_update_flags is possible. in some
		 * cases, the flags may change even with no mutex (pgbuf_set_dirty). since we have to handle victim
		 * counter and hint for lru lists, we must do atomic operations to modify the zone, and keep previous
		 * and new flag values. based on these flags, we then update lru zone counters, lru victim counter and
		 * lru victim hint. lru zone counters can only be modified by other calls pgbuf_bcb_change_zone in same
		 * lru and are protected by lru mutex, so they can be modified without atomic operations.
		 */
		STATIC_INLINE void
			...
			/*
			 * update bcb->flags. make sure we are only changing the values for zone and lru index, but we
			 * preserve the flags.
			 */
			do {
			...
			/*
			 * was bcb a valid victim candidate (we only consider flags, not fix counters or zone)? note
			 * that this is still true after the change of zone.
			 */
			...
			if (old_flags & PGBUF_LRU_ZONE_MASK) {
				...
				/*
				 * hint should have been changed already if the BCB was in LRU3; otherwise (if
				 * downgraded, we may expect that victim hint is changed by other thread
				 * (checkpoint->pgbuf_bcb_update_flags)
				 */
				...
				case PGBUF_LRU_3_ZONE:
					...
					if (is_valid_victim_candidate) {
						/*
						 * bcb was a valid victim and in the zone that could be victimized.
						 * update victim counter & hint
						 */
						pgbuf_lru_remove_victim_candidate(thread_p, lru_list, bcb);
					}
					break;
		 ...
		 * 
		 * return   : void bcb (in) : bcb
		 */
		STATIC_INLINE void
				pgbuf_bcb_set_dirty(THREAD_ENTRY * thread_p, PGBUF_BCB * bcb) {
			/* set dirty flag and clear none */
			/*
			 * note: we usually use pgbuf_bcb_update_flags function. we do an exception for
			 * pgbuf_bcb_set_dirty to since it is the most used case and the code should be as optimal as
			 * possible.
			 */
			int		old_flags;
		...
		/*
		 * pgbuf_bcb_mark_is_flushing () - mark page is being flushed. dirty flag is also cleared because while
		 * the page is flushed to disk, another thread may fix the page and modify it. the new change must be
		 * tracked.
		 * 
		 * return   : void bcb (in) : bcb
		 */
		 ...
		 * 
		 * return   : void bcb (in) : bcb mark_dirty(in): true if BCB needs to be marked dirty
		 */
		STATIC_INLINE void
		...
		STATIC_INLINE void
			...
			do {
				...
				if ((count_crt & PGBUF_BCB_AVOID_DEALLOC_MASK) > 0) {
					 ...
					 * 
					 * well, pgbuf_ordered_fix may be forced to unfix all pages currently held by
					 * transaction to fix a new page. all pages that are "less" than new page are
					 * marked to avoid deallocation and unfixed. then transaction is blocked on
					 * latching new page, which may take a while, pages previously unfixed can be
					 * victimized. when pgbuf_ordered_fix tries to fix back these pages, it will
					 * load them from disk and tadaa, the avoid deallocation count is 0. so we
					 * expect the case.
					 * 
					 * note: avoid deallocation count is supposed to prevent vacuum workers from
					 * deallocating these pages. so, victimizing a bcb marked to avoid deallocation
					 * is not perfectly safe. however, the likelihood of page really getting
					 * deallocated is ... almost zero. the alternative of avoiding victimization
					 * when bcb's are marked for deallocation is much more complicated and poses
					 * serious risks (what if we leak the counter and prevent bcb from being
					 * victimized indefinitely?). so, we prefer the existing risks.
					 */
					er_log_debug(ARG_FILE_LINE,
		...
		/*
		 * pgbuf_bcb_check_and_reset_fix_and_avoid_dealloc () - check avoid deallocation is 0 and reset the
		 * whole bcb field.
		 * 
		 * return    : void bcb (in)  : bcb file (in) : caller file line (in) : caller line
		 * 
		 * note: avoid deallocation is allowed to be non-zero due to pgbuf_ordered_fix and the possibility of
		 * victimizing its bcb. avoid crashing the server and just issue a warning.
		 */
		STATIC_INLINE void
		...
		STATIC_INLINE void
			...
			/*
			 * note: we only register to detect hot pages. once we hit the threshold, we are no longer
			 * required to fix it.
			 */
		...
		/*
		 * pgbuf_lfcq_add_lru_with_victims () - add lru list to queue of lists that can be victimized. this
		 * queue was designed so victimizers can find a list with victims quickly without iterating through
		 * many lists that are full.
		 * 
		 * return        : true if list was added, false if it was already added by someone else. lru_list (in) :
		 * lru list
		 */
		STATIC_INLINE	bool
		 ...
		 * 
		 * return               : victim or NULL thread_p (in)        : thread entry restricted (in)      : true
		 * if victimizing is restricted to big private lists
		 */
		static PGBUF_BCB *
...
#define PERF(id) if (detailed_perf) perfmon_inc_stat (thread_p, id)
			...
			/*
			 * we're not adding the list back to the queue... so we need to reflect that in the list flags.
			 * next time when a new candidate is added, lru list should also be added to the queue.
			 * 
			 * note: we can have a race here. candidates are 0 now and incremented before we manage to change
			 * the victim counter. we should not worry that much, the list will be added by
			 * pgbuf_adjust_quotas eventually.
			 */
			assert((lru_list->flags & PGBUF_LRU_VICTIM_LFCQ_FLAG) != 0);
			/*
			 * note: we are not using an atomic operation here, because this is the only flag and we are
			 * certain no one else changes it from set to cleared. however, if more flags are added, or
			 * more cases that should clear the flag, then consider replacing with some atomic operation.
			 */
			lru_list->flags &= ~PGBUF_LRU_VICTIM_LFCQ_FLAG;
...
#undef PERF
		 ...
		 * 
		 * return              : victim or NULL thread_p (in)       : thread entry multi_threaded (in) : true if
...
#define PERF(id) if (detailed_perf) perfmon_inc_stat (thread_p, id)
			...
			if ((multi_threaded || victim != NULL) && lru_list->count_vict_cand > 0) {
				...
				if (pgbuf_Pool.shared_lrus_with_victims->produce(lru_idx)) {
					...
					/*
					 * we couldn't add to queue. it usually does not happen, but a consumer can be
					 * preempted for a long time, temporarily creating the impression that queue is
					 * full. it will be added later, when a new victim candidate shows up or when
					 * adjust quota checks it.
					 */
					/* fall through */
				}
			}
...
#undef PGBUF_MIN_VICTIM_REQ
		 ...
		 * 
		 * return           : void bcb (in)         : BCB to lock caller_line (in) : caller line
		 */
		static void
		 ...
		 * 
		 * return           : try lock result bcb (in)         : BCB to lock caller_line (in) : caller line
		 */
		static int
		 ...
		 * 
		 * return   : void bcb (in) : BCB to unlock
		 */
		static void
		...
		/*
		 * pgbuf_bcbmon_check_mutex_leaks () - check for mutex leaks. must be called on exit points where no
		 * BCB should be locked.
		 * 
		...
		static void
			...
			/*
			 * sanity check: make sure the masks for bcb flags, zone and lru index do not overlap. this
			 * should be immediately caught, so abort the server whenever happens.
			 */
...
//TODO:	find a better place for this
		...
		void
			...
			if              (page_flush_interval_msecs > 0) {
				//if page_flush_interval_msecs > 0(zero)
					then loop for	fixed interval
							is_timed_wait = true;
	...
	statsp += cubthread: : daemon: :get_stats_value_count();
		...
		/*
		 * pgbuf_is_page_flush_daemon_available () - check if page flush daemon is available return: true if
		 * page flush daemon is available, false otherwise
		 */
			static		bool
				pgbuf_is_page_flush_daemon_available() {
#if defined (SERVER_MODE)
		...
		static void
			...
			for             (bufid = 0; bufid < pgbuf_Pool.num_buffers; bufid++) {
				...
				if (pgbuf_is_temporary_volume(vpid.volid) == true) {
					...
					assert((page_type == PAGE_UNKNOWN) ||	/* dealloc pages, we don't know page
										 * type */
					       (page_type == PAGE_AREA) || (page_type == PAGE_QRESULT) ||	/* temporary page type */
					       (page_type == PAGE_EHASH) || (page_type == PAGE_VOLHEADER)	/* It can be temporary
														 * or permanent pages */
					       ||(page_type == PAGE_VOLBITMAP) || (page_type == PAGE_FTAB));	/* It can be temporary
														 * or permanent pages */
				} else {
					switch (page_type) {
					case PAGE_BTREE:
		...
		/*
		 * pgbuf_start_scan () - start scan function for show page buffer status return: NO_ERROR, or ER_code
		 * 
		 * thread_p(in): type (in): arg_values(in): arg_cnt(in): ptr(in/out):
		 */
		int
			...
			time_delta = difftime(cur_time, status_old->print_out_time) + 0.0001;
			//avoid dividing by 0
