...
spage_header_next_scan (THREAD_ENTRY * thread_p, int cursor, DB_VALUE ** out_values, int out_cnt, void *ptr)
{
  ...
  db_make_string_copy (out_values[idx], spage_anchor_flag_string (header->anchor_type));
  idx++;

  db_make_string_copy (out_values[idx], spage_alignment_string (header->alignment));
  idx++;
...
spage_slots_next_scan (THREAD_ENTRY * thread_p, int cursor, DB_VALUE ** out_values, int out_cnt, void *ptr)
{
  ...
  db_make_string_copy (out_values[idx], spage_record_type_string (ctx->slot->record_type));
  idx++;
