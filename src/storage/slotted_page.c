...
static int spage_update_record_after_compact (PAGE_PTR page_p, SPAGE_HEADER * page_header_p, SPAGE_SLOT * slot_p,
					      const RECDES * record_descriptor_p, int space, int old_waste,
					      int new_waste);
...
spage_check_space (THREAD_ENTRY * thread_p, PAGE_PTR page_p, SPAGE_HEADER * page_header_p, int space)
{
  ...
  else if (spage_has_enough_contiguous_space (page_p, page_header_p, space) == false)
    {
      return SP_ERROR;
    }
  ...
  *out_slot_id_p = NULL_SLOTID;
  ...
  if (slot_id == page_header_p->num_slots)
  ...
  else
    {
      ...
      if (spage_has_enough_contiguous_space (page_p, page_header_p, space) == false)
	{
	  return SP_ERROR;
	}
    }
			    ...
			    int mvcc_insert_record_length)
{
  ...
  if (slot_id == page_header_p->num_slots)
      ...
      if (spage_has_enough_total_space (thread_p, page_p, page_header_p, space) == false)
	{
      ...
      else if (spage_has_enough_contiguous_space (page_p, page_header_p, insert_record_space) == false)
	{
	  return SP_ERROR;
	}
    }
    ...
    {
      /* We already know that there is total space available since the slot is reused and the space was checked above */
      if (spage_has_enough_contiguous_space (page_p, page_header_p, insert_record_space) == false)
	{
	  return SP_ERROR;
	}
    }
 ...
 */
static int
spage_update_record_after_compact (PAGE_PTR page_p, SPAGE_HEADER * page_header_p, SPAGE_SLOT * slot_p,
...
spage_update (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID slot_id, const RECDES * record_descriptor_p)
{
  ...
  if (record_descriptor_p->length <= (int) slot_p->record_length)
    ...
    {
      status =
	spage_update_record_after_compact (page_p, page_header_p, slot_p, record_descriptor_p, space, old_waste,
					   new_waste);
    }
...
spage_take_out (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID slot_id, int takeout_offset, int takeout_length)
{
  ...
  if (mayshift_left == 0 && (takeout_offset < ((int) slot_p->record_length - takeout_offset - takeout_length)))
	...
	{
	  if (slot_p->offset_to_record + takeout_length + takeout_offset > DB_PAGESIZE)
	    {
	      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	      assert_release (false);
	      return SP_ERROR;
      ...
      if (((int) slot_p->record_length - takeout_offset - takeout_length) > 0)
	{
	  ...
	  if (slot_p->offset_to_record + takeout_offset + (slot_p->record_length - takeout_offset - takeout_length) >
	      DB_PAGESIZE)
	    {
	      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	      assert_release (false);
		  ...
		  const RECDES * record_descriptor_p, bool is_append)
{
  ...
  if (spage_is_record_located_at_end (page_header_p, slot_p) && space <= page_header_p->cont_free)
      ...
      if (!is_append)
	{
	  ...
	  if ((slot_p->offset_to_record + offset + record_descriptor_p->length + (slot_p->record_length - offset)) >
	      DB_PAGESIZE)
	    {
	      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	      assert_release (false);
  ...
  else if ((int) record_descriptor_p->length + (int) slot_p->record_length <= page_header_p->cont_free)
    {
      ...
      if (is_append)
	{
	  if (page_header_p->offset_to_free_area + slot_p->record_length > DB_PAGESIZE)
	    {
	      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	      assert_release (false);
	      return SP_ERROR;
  ...
  if (is_append)
    ...
    {
      if ((slot_p->offset_to_record + offset + record_descriptor_p->length) > DB_PAGESIZE)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	  assert_release (false);
	  return SP_ERROR;
		 ...
		 const RECDES * record_descriptor_p)
{
  ...
  if ((slot_p->offset_to_record + overwrite_offset + record_descriptor_p->length) > DB_PAGESIZE)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
      assert_release (false);
      return SP_ERROR;
...
spage_merge (THREAD_ENTRY * thread_p, PAGE_PTR page_p, PGSLOTID first_slot_id, PGSLOTID second_slot_id)
{
  ...
  if (first_slot_p == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_SP_UNKNOWN_SLOTID, 3, first_slot_id, pgbuf_get_page_id (page_p),
	      pgbuf_get_volume_label (page_p));
      return SP_ERROR;
    }
  ...
  if (second_slot_p == NULL)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_SP_UNKNOWN_SLOTID, 3, second_slot_id, pgbuf_get_page_id (page_p),
	      pgbuf_get_volume_label (page_p));
      return SP_ERROR;
    }
  ...
  if (spage_is_record_located_at_end (page_header_p, first_slot_p)
  ...
  else if ((int) first_slot_p->record_length + (int) second_slot_p->record_length <= page_header_p->cont_free)
    {
       ...
       * place. */
      if ((page_header_p->offset_to_free_area + first_slot_p->record_length) > DB_PAGESIZE)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	  assert_release (false);
	  return SP_ERROR;
      ...
      if (copyarea == NULL)
	{
      ...
      if (first_slot_p->offset_to_record + first_slot_p->record_length > DB_PAGESIZE)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	  assert_release (false);
	  return SP_ERROR;
      ...
      if (second_slot_p->offset_to_record + second_slot_p->record_length > DB_PAGESIZE)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	  assert_release (false);
	  return SP_ERROR;
      ...
      if (spage_compact (page_p) != NO_ERROR)
	{
      ...
      /* Move the old data to the end */
      if ((page_header_p->offset_to_free_area + first_slot_p->record_length + second_slot_p->record_length) >
	  DB_PAGESIZE)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	  assert_release (false);
  ...
  if (second_slot_p->record_length != 0)
    {
      if (page_header_p->offset_to_free_area + second_slot_p->record_length > DB_PAGESIZE)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
	  assert_release (false);
	  return SP_ERROR;
 ...
 */
SCAN_CODE
spage_get_record (PAGE_PTR page_p, PGSLOTID slot_id, RECDES * record_descriptor_p, int is_peeking)
{
  SPAGE_HEADER *page_header_p;
  SPAGE_SLOT *sptr;
 ...
 */
static bool
spage_has_enough_contiguous_space (PAGE_PTR page_p, SPAGE_HEADER * page_header_p, int space)
{
  assert (page_p != NULL);
  SPAGE_VERIFY_HEADER (page_header_p);
