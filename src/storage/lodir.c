 ...
 *
 * lodir.c - LOM Directory Manager 
 * 
 * This module is responsible for managing the directory for Large Object 
 * file. It provides read, insert, delete, update functionality for the
 * directory entries. It is called by the Large Object Manager.
 */
...
/*
 * largeobjmgr_initdir_newpage () - Initialize a newly directory page
 *   return: 
			   ...
			   VPID * vpid)
{
  ...
  if (file_new_isvalid (thread_p, &ds->curdir.hdr->loid.vfid) == DISK_INVALID)
    {
			 ...
			 int n)
{
  ...
  if (ds->curdir.idx < split_ind)
	...
	{
	  pgbuf_unfix (thread_p, ds->curdir.pgptr);
	  ds->curdir.pgptr = NULL;
	}
  ...
  if (ds->index_level > 0)
		  ...
		  && VPID_ISNULL (&next_vpid))))
	{
	  ...
	  mv_ent_cnt =
	    (ds->firstdir.hdr->pg_act_idxcnt - 1) - ds->firstdir.idx;
			  ...
			  VPID * p_vpid)
{
  ...
  if (ds->index_level > 0 && VPID_EQ (&ds->firstdir.idxptr->vpid, &cur_vpid))
      ...
      if (ds->firstdir.idxptr->length == 0)
	{
	  ...
	  log_append_undo_data (thread_p, RVLOM_DIR_PG_REGION, &addr,
				(mv_ent_cnt +
				 1) * sizeof (LARGEOBJMGR_DIRMAP_ENTRY),
				(char *) ds->firstdir.idxptr);
...
largeobjmgr_dir_pgdump (PAGE_PTR curdir_pgptr)
{
  ...
  if (dir_hdr->index_level > 0)
    {
      /* Map index directory */
      for (k = 0, firstdir_idxptr =
	   largeobjmgr_first_dirmap_entry (curdir_pgptr);
			...
			LARGEOBJMGR_DIRENTRY * X)
{
  ...
  if (X != NULL)
  ...
  else
    {
	  ...
	  (pg_lastact_idx = ds->curdir.hdr->pg_lastact_idx) == ds->curdir.idx)
	{
	  last_act_ent_ptr =
		      ...
		      int opr_mode, LARGEOBJMGR_DIRSTATE * ds)
{
  ...
  if (offset >= ds->tot_length)
      ...
      if (LARGEOBJMGR_SCAN_MODE (ds->opr_mode))
	{
	  ...
	  pgbuf_unfix (thread_p, ds->firstdir.pgptr);
	  ds->firstdir.pgptr = NULL;
	  ds->firstdir.hdr = NULL;
	  ds->pos = S_AFTER;
  ...
  if (ds->index_level > 0)
      ...
      if (ds->curdir.pgptr == NULL)
	{
      ...
      page_ptr =
	largeobjmgr_dir_search (thread_p, ds->curdir.pgptr,
				offset - ds->lo_offset, &ds->curdir.idx,
				&dlo_offset);
      if (page_ptr == NULL)
	{
	  pgbuf_unfix (thread_p, ds->curdir.pgptr);
	...
	{
	  page_ptr =
	    largeobjmgr_dir_search (thread_p, ds->firstdir.pgptr, offset,
				    &ds->curdir.idx, &ds->lo_offset);
	  if (page_ptr == NULL)
	    {
	      goto exit_on_error;
	    }
...
/*
 * largeobjmgr_rv_dir_rcv_state_undoredo () - Put directory page back to the state indicated by
 *                           the recovery structure for UNDO or REDO purposes
 *   return: 
