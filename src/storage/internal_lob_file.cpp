...
static int internal_lob_make_locator_db_value_internal (THREAD_ENTRY *thread_p, DB_VALUE *value, DB_TYPE lob_type,
...
/* Per-session signature over the (public) token, keyed by the session secret (CBRD-26780).  A client cannot
 * compute this for an OID it did not legitimately receive, so it cannot forge a client-facing read locator.
 * session_key == 0 means "no signature" (server-internal / loaddb adopted locators, verified by token only). */
unsigned long long
internal_lob_locator_session_sig (const INTERNAL_LOB_LOCATOR &locator, bool adopted, INT64 session_key)
{
  unsigned long long token = internal_lob_locator_token (locator, adopted);
  unsigned long long sig = internal_lob_marker_mix_u64 (token ^ (unsigned long long) session_key);
				    ...
				    INT64 session_key)
{
  ...
  if (session_key != 0)
    {
      unsigned long long sig = internal_lob_locator_session_sig (locator, adopted, session_key);

      return snprintf (buf, buf_size, INTERNAL_LOB_LOCATOR_PREFIX "%s%d|%d|%d:%lld:%016llx:%016llx", adopt_marker,
		       (int) locator.oid.volid, (int) locator.oid.pageid, (int) locator.oid.slotid,
		       (long long) locator.length, token, sig);
    }
				   ...
				   unsigned long long *sig_out)
{
    ...
    }
  {
    ...
    int after_off = prefix_len + marker_len + consumed + 1 + token_consumed;

    if (after_token[0] == ':')
      {
	/* optional per-session signature (CBRD-26780); the caller verifies it with the session key */
	unsigned long long parsed_sig = 0;
	int sig_consumed = 0;

	if (sscanf (after_token + 1, "%llx%n", &parsed_sig, &sig_consumed) != 1 || sig_consumed <= 0
	    || after_token[1 + sig_consumed] != '\0' || after_off + 1 + sig_consumed != size)
	  {
	    return false;
	  }
	if (sig_out != NULL)
    ...
    else if (after_token[0] != '\0' || after_off != size)
      {
	return false;
      }
  }
				  ...
				  unsigned long long *sig_out)
{
  ...
  return internal_lob_parse_locator_string (data, size, locator, sig_out);
}
...
/*
 * internal_lob_db_value_is_load_slot () - Is this value the load-slot envelope a loaddb worker builds for a row
 *                                         whose Internal LOB payload it already wrote?
 *   The envelope text is INTERNAL_LOB_LOAD_SLOT_PREFIX followed by the 1-based slot number.
 */
bool
internal_lob_db_value_is_load_slot (const DB_VALUE *value, INTERNAL_LOB_LOAD_SLOT *load_slot)
{
  DB_TYPE type;
  const char *data = NULL;
  int size = 0;
  int prefix_len = (int) strlen (INTERNAL_LOB_LOAD_SLOT_PREFIX);
  char marker_buf[80];
  long long slot;
  int consumed = 0;

  if (value == NULL || DB_IS_NULL (value)
      || !db_value_has_internal_lob_marker (value, DB_VALUE_INTERNAL_LOB_MARKER_LOAD_SLOT))
    {
      return false;
    }
  ...
  if (data == NULL || size <= prefix_len || size >= (int) sizeof (marker_buf)
      || memcmp (data, INTERNAL_LOB_LOAD_SLOT_PREFIX, prefix_len) != 0)
    {
      return false;
    }
  ...
  memcpy (marker_buf, data, size);
  marker_buf[size] = '\0';
  if (sscanf (marker_buf + prefix_len, "%lld%n", &slot, &consumed) != 1
      || marker_buf[prefix_len + consumed] != '\0' || slot <= 0)
    {
      return false;
    }
  ...
  if (load_slot != NULL)
    {
      load_slot->slot = (INT64) slot;
    }
  return true;
}

/*
 * internal_lob_make_load_slot_db_value () - Build the load-slot envelope for a loaddb row value.
 *   The heap sink resolves it through session_internal_lob_load_consume () while the row is inserted, so the
 *   chunk chain's replication bookkeeping lands on the row that owns it.
 */
int
internal_lob_make_load_slot_db_value (DB_VALUE *value, DB_TYPE lob_type, INT64 slot)
{
  char marker_text[64];
  char *marker = NULL;
  int marker_len;
  ...
  if (value == NULL || (lob_type != DB_TYPE_BLOB && lob_type != DB_TYPE_CLOB) || slot <= 0)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_GENERIC_ERROR, 0);
      return ER_GENERIC_ERROR;
    }

  marker_len = snprintf (marker_text, sizeof (marker_text), INTERNAL_LOB_LOAD_SLOT_PREFIX "%lld", (long long) slot);
  ...
  if (lob_type == DB_TYPE_CLOB)
    {
      err = db_make_clob (value, DB_MAX_LOB_PRECISION, marker, marker_len);
    }
  else
    {
      err = db_make_blob (value, DB_MAX_LOB_PRECISION, (DB_CONST_C_BIT) marker, marker_len * 8);
    }
  if (err != NO_ERROR)
    {
  ...
  db_value_mark_internal_lob (value, DB_VALUE_INTERNAL_LOB_MARKER_LOAD_SLOT);
  return NO_ERROR;
}
...
int
internal_lob_make_locator_db_value (THREAD_ENTRY *thread_p, DB_VALUE *value, DB_TYPE lob_type,
				    const INTERNAL_LOB_LOCATOR &locator)
{
  return internal_lob_make_locator_db_value_internal (thread_p, value, lob_type, locator, false);
}

int
internal_lob_make_adopt_locator_db_value (DB_VALUE *value, DB_TYPE lob_type, const INTERNAL_LOB_LOCATOR &locator)
{
  /* adopted locators (standalone loaddb) are server-internal and carry no per-session signature */
  return internal_lob_make_locator_db_value_internal (NULL, value, lob_type, locator, true);
}

static int
internal_lob_make_locator_db_value_internal (THREAD_ENTRY *thread_p, DB_VALUE *value, DB_TYPE lob_type,
    const INTERNAL_LOB_LOCATOR &locator, bool adopted)
{
  char stack_buf[128];
  ...
  if (!adopted)
    {
      /* Sign the client-facing locator with the session secret (CBRD-26780); with no session (a
       * server-internal caller) the key stays 0 and the locator is emitted unsigned. */
      (void) session_get_internal_lob_locator_key (thread_p, &session_key);
    }

  locator_len = internal_lob_format_locator_string (locator, stack_buf, sizeof (stack_buf), adopted, session_key);
