...
/*
 * Replication bookkeeping of one Internal LOB value written ahead of the row that will own it.
 *
 * The chunk writer publishes every chunk OID in thread_p->oos_oids and every RVOOS_INSERT LSA in the
 * transaction's oos_insert_lsa_queue, and the heap insert of a row resets both before it pairs them with
 * the row's attributes.  A value written before its row (a direct DML slot, a loaddb batch payload) must
 * therefore take its entries out right after writing (mark before, capture after) and put them back while
 * the row is inserted (restore), or the slave would never see the chunks.
 */
struct internal_lob_repl_tracking
{
  std::vector<OID> oids;
  std::vector<LOG_LSA> lsas;
  std::size_t oid_start = 0;
  std::size_t lsa_start = 0;
};

extern int internal_lob_repl_tracking_mark (THREAD_ENTRY *thread_p, internal_lob_repl_tracking &tracking);
extern int internal_lob_repl_tracking_capture (THREAD_ENTRY *thread_p, internal_lob_repl_tracking &tracking);
extern int internal_lob_repl_tracking_restore (THREAD_ENTRY *thread_p, internal_lob_repl_tracking &tracking);
...
class internal_lob_dml_staging_context : public internal_lob_dml_context
{
  ...
  private:
    struct slot_state
    {
      ...
      internal_lob_repl_tracking direct_tracking;
