		     ...
		     INT32 hint_pageid, VPID * ftb_vpids, INT32 num_ftb_pages, FILE_TYPE file_type)
{
  ...
  do
      ...
      if (hint_volid != NULL_VOLID)
	{
	  ...
	  pageid = disk_alloc_page (thread_p, hint_volid, DISK_SECTOR_WITH_ALL_PAGES, num_ftb_pages, hint_pageid,
				    alloc_page_type);
		       ...
		       VPID * ftb_vpids, int num_ftb_pages, int *current_ftb_page_index)
{
  ...
  if (num_ftb_pages <= idx)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_FILE_TABLE_OVERFLOW, 5, num_ftb_pages, (idx + 1),
	      ftb_vpids[0].volid, ftb_vpids[0].pageid, fileio_get_volume_label (ftb_vpids[0].volid, PEEK));
...
/*
 * file_find_last_page () - Find the page identifier of the last allocated page of the given file
 *   return: pageid or NULL_PAGEID
 *   vfid(in): Complete file identifier
 *   last_vpid(in):
 */
...
/*
 * file_allocset_alloc_sector () - Allocate a new sector for the given allocation set
 ...
 *   allocset_offset(in): Location in allocset_pgptr where allocatio set is located
 *   exp_npages(in): Expected pages that sector will have
 *
 * Note: The sector identifier is stored onto the sector table of the given allocation set.
 *
 *       Allocation of pages and sector is only done for the last allocation set. 
 *       This is a consequence of the ordering of pages
 */
static INT32
...
#ifdef FILE_DEBUG
/*
 * file_debug_maybe_newset () - Create a new allocation set every multiple set of pages
 *   return: void
 *   fhdr_pgptr(in): Page pointer to file header table
 *   fhdr(in): File header
...
/*
 * file_allocset_new_set () - Create a new allocation for allocation of future pages
 *   return: NO_ERROR
					...
					INT32 npages, void *args), void *args)
{
  ...
  allocstate = file_allocset_alloc_pages (thread_p, fhdr_pgptr, &fhdr->last_allocset_vpid,
...
file_allocset_find_page (THREAD_ENTRY * thread_p, PAGE_PTR fhdr_pgptr, PAGE_PTR allocset_pgptr, INT16 allocset_offset,
			 INT32 pageid, int (*fun) (THREAD_ENTRY * thread_p, FILE_HEADER * fhdr,
						   ...
						   PAGE_PTR allocset_pgptr, INT16 allocset_offset,
						   PAGE_PTR pgptr, INT32 * aid_ptr, INT32 pageid, void *args),
			 void *args)
{
  FILE_HEADER *fhdr;
  FILE_ALLOCSET *allocset;
 ...
 *   return: NO_ERROR
 *   fhdr_pgptr(in): Page pointer to file header
 *   prev_allocset_vpid(in): Page address where the previous allocation set is located
 *   prev_allocset_offset(in): Offset in page address where the previous allocation set is located
 *   allocset_vpid(in): Page address where the current allocation set is located
 *   allocset_offset(in): Location in allocset page where allocation set is located
 *
 * Note: If this is the only allocation set in the file, the deletion is ignored
 */
static int
 ...
 *   prev_allocset_vpid(in): Page address where the previous allocation set is located
 *   prev_allocset_offset(in): Offset in page address where the previous allocation set is located
 *   allocset_vpid(in/out): Page address where the current allocation set is located
 *   allocset_offset(in/out): Location in allocset page where allocation set is located
 *   ftb_vpid(in/out): The file table page where the allocation set is moved
 *   ftb_offset(in/out): Offset in the file table page where the allocation set is moved
 */
static int
...
file_check_deleted (THREAD_ENTRY * thread_p, const VFID * vfid)
{
  ...
  if (fhdr->num_user_pages_mrkdelete != total_marked_deleted)
    {
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_FILE_INCONSISTENT_EXPECTED_MARKED_DEL, 5, vfid->fileid,
	      vfid->volid, fileio_get_volume_label (vfid->volid, PEEK),
...
/*
 * file_rv_dump_ftab_chain () - Dump redo information to double link file table pages
 *   return: void
 *   length_ignore(in): Length of Recovery Data
 *   data(in): The data being logged
 */
...
/*
 * file_rv_allocset_undoredo_page () - UNDO/REDO sector expansion information on allocation set
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_allocset_dump_page () - Dump redo sector expansion information on allocation set
 *   return: void
 *   length_ignore(in): Length of Recovery Data
 *   data(in): The data being logged
 */
...
/*
 * file_rv_fhdr_dump_expansion () -  Dump redo file table expansion information at header
 *   return: void
 *   length_ignore(in): Length of Recovery Data
 *   data(in): The data being logged
 */
...
/*
 * file_rv_fhdr_last_allocset_helper () - UNDO/REDO address of last allocation set on file header
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
static int
...
/*
 * file_rv_fhdr_add_last_allocset () - REDO address of last allocation set on file header
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_fhdr_remove_last_allocset () - REDO address of last allocation set on file header
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_fhdr_change_last_allocset () - REDO address of last allocation set on file header
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_allocset_undoredo_add_pages () -  Recover allocset information related to allocation of pages
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_allocset_dump_add_pages () -  Dump allocset information related to allocation of pages
 *   return: void
 *   length_ignore(in): Length of Recovery Data
 *   data(in): The data being logged
 */
...
/*
 * file_rv_fhdr_undoredo_mark_deleted_pages () - Undo/Redo mark deleted page info at file header
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_fhdr_undoredo_update_num_user_pages () - Undo/redo update num user pages.
 *
 * return	 : NO_ERROR.
 * thread_p (in) : Thread entry.
...
/*
 * file_rv_allocset_dump_delete_pages () - Dump Redo information of the deallocation of pages
 *   return: void
 *   length_ignore(in): Length of Recovery Data
 *   data(in): The data being logged
 */
...
/*
 * file_rv_allocset_undoredo_sectortab () - REDO sector address information on allocation set
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_allocset_dump_sectortab () - Dump sector address information on allocation set
 *   return: void
 *   length_ignore(in): Length of Recovery Data
 *   data(in): The data being logged
 */
...
/*
 * file_rv_descriptor_undoredo_firstrest_nextvpid () - UNDO/REDO next vpid filed for file descriptor of header page
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_descriptor_dump_firstrest_nextvpid () - Dump undo/redo first rest description VPID page identifier
 *   return: void
 *   length_ignore(in): Length of Recovery Data
 *   data(in): The data being logged
 */
...
/*
 * file_rv_descriptor_undoredo_nrest_nextvpid () - UNDO/REDO next vpid filed for file descriptor of a rest page
 *   return: NO_ERROR
 *   rcv(in): Recovery structure
 */
int
...
/*
 * file_rv_descriptor_dump_nrest_nextvpid () - Dump undo/redo first rest description VPID page identifier
 *   return: void
 *   length_ignore(in): Length of Recovery Data
 *   data(in): The data being logged
 */
...
static VFID flre_Tracker_vfid = VFID_INITIALIZER;
static VPID flre_Tracker_vpid = VPID_INITIALIZER;

typedef struct flre_track_heap_metadata FLRE_TRACK_HEAP_METADATA;
struct flre_track_heap_metadata
{
  bool is_marked_deleted;
  bool dummy[7];		/* dummy fields to 8 bytes */
};

typedef union flre_track_metadata FLRE_TRACK_METADATA;
union flre_track_metadata
{
  FLRE_TRACK_HEAP_METADATA heap;

  INT64 metadata_size_tracker;
};

typedef struct flre_track_item FLRE_TRACK_ITEM;
struct flre_track_item
{
  INT32 fileid;			/* 4 bytes */
  INT16 volid;			/* 2 bytes */
  INT16 type;			/* 2 bytes */
  FLRE_TRACK_METADATA metadata;	/* 8 bytes */
...
static int file_extdata_func_for_search_ordered (THREAD_ENTRY * thread_p, const FILE_EXTENSIBLE_DATA * extdata,
						 bool * stop, void *args);
static int file_extdata_search_item (THREAD_ENTRY * thread_p, FILE_EXTENSIBLE_DATA ** extdata, const void *item_to_find,
...
STATIC_INLINE int flre_create_temp_internal (THREAD_ENTRY * thread_p, int npages, FILE_TYPE ftype,
...
static int flre_tracker_register (THREAD_ENTRY * thread_p, const VFID * vfid, FILE_TYPE ftype,
				  FLRE_TRACK_METADATA * metadata);
static int flre_tracker_unregister (THREAD_ENTRY * thread_p, const VFID * vfid);
static int flre_tracker_apply_to_file (THREAD_ENTRY * thread_p, const VFID * vfid, PGBUF_LATCH_MODE mode,
				       FILE_TRACK_ITEM_FUNC func, void *args);
static int flre_tracker_map (THREAD_ENTRY * thread_p, PGBUF_LATCH_MODE latch_mode, FILE_TRACK_ITEM_FUNC func,
			     void *args);
static int flre_tracker_item_reuse_heap (THREAD_ENTRY * thread_p, PAGE_PTR page_of_item, FILE_EXTENSIBLE_DATA * extdata,
					 int index_item, bool * stop, void *args);
...
/*
 * file_header_sanity_check () - Debug function used to check the sanity of file header before and after certain file
 *				 operations.
 *
 * return     : Void.
...
{
  FLRE_TRACK_ITEM *first_item = (FLRE_TRACK_ITEM *) first;
  FLRE_TRACK_ITEM *second_item = (FLRE_TRACK_ITEM *) second;
	     ...
	     bool is_temp, bool is_numerable, VFID * vfid)
{
   ...
   * file tables depend on the properties of permanent/temporary and numerable.
...
flre_destroy (THREAD_ENTRY * thread_p, const VFID * vfid)
{
  ...
  error_code = disk_unreserve_ordered_sectors (thread_p,
					       (FILE_IS_TEMPORARY (fhead)
						? DB_TEMPORARY_DATA_PURPOSE : DB_PERMANENT_DATA_PURPOSE),
...
file_perm_expand (THREAD_ENTRY * thread_p, PAGE_PTR page_fhead)
{
  ...
  expand_max_size_in_sectors = MIN (fhead->tablespace.expand_max_size / DB_SECTORSIZE,
				    file_extdata_remaining_capacity (extdata_part_ftab));
...
#define LOG_DATA_SIZE (sizeof (VFID) + sizeof (VPID))
  ...
  file_log ("file_dealloc", "file %d|%d marked vpid %d|%d as deleted", VFID_AS_ARGS (vfid), VPID_AS_ARGS (vpid));
...
file_perm_dealloc (THREAD_ENTRY * thread_p, PAGE_PTR page_fhead, const VPID * vpid_dealloc, FILE_ALLOC_TYPE alloc_type)
{
  ...
  if (found)
      ...
      if (file_extdata_size (extdata_full_ftab) * 2 < file_extdata_max_size (extdata_full_ftab))
	{
	  VPID vpid_merged;
	  error_code =
	    file_extdata_try_merge (thread_p, addr.pgptr, extdata_full_ftab, file_compare_vsids,
...
file_rv_dealloc_internal (THREAD_ENTRY * thread_p, LOG_RCV * rcv, bool compensate_or_run_postpone)
{
  ...
  if (FILE_IS_NUMERABLE (fhead))
      ...
      if (file_extdata_size (extdata_user_page_ftab) * 2 < file_extdata_max_size (extdata_user_page_ftab))
	{
	  VPID vpid_merged;
	  error_code =
	    file_extdata_try_merge (thread_p, addr.pgptr, extdata_user_page_ftab, NULL, RVFL_EXTDATA_MERGE,
				    &vpid_merged);
	  if (error_code != NO_ERROR)
	    {
	      ASSERT_ERROR ();
...
flre_update_descriptor (THREAD_ENTRY * thread_p, const VFID * vfid, void *des_new)
{
  ...
  log_append_undoredo_data2 (thread_p, RVFL_FILEDESC_UPD, NULL, page_fhead, (char *) &fhead->descriptor - page_fhead,
			     sizeof (fhead->descriptor), sizeof (fhead->descriptor), &fhead->descriptor, des_new);
...
/*
 * flre_tracker_create () - create file tracker.
 *
 * return                 : error code
 * thread_p (in)          : thread entry
 ...
 */
int
flre_tracker_create (THREAD_ENTRY * thread_p, VFID * vfid_tracker_out)
{
  ...
  error_code = flre_create_with_npages (thread_p, FILE_TRACKER, 1, NULL, vfid_tracker_out);
  if (error_code != NO_ERROR)
    {
      ASSERT_ERROR ();
      goto exit;
  ...
  error_code = flre_alloc_sticky_first_page (thread_p, vfid_tracker_out, &flre_Tracker_vpid);
  if (error_code != NO_ERROR)
    {
      ASSERT_ERROR ();
      goto exit;
  ...
  page_tracker = pgbuf_fix (thread_p, &flre_Tracker_vpid, NEW_PAGE, PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH);
  ...
  file_extdata_init (sizeof (FLRE_TRACK_ITEM), DB_PAGESIZE, extdata);
  ...
  if (error_code != NO_ERROR)
    {
      ...
      VPID_SET_NULL (&flre_Tracker_vpid);
    }
  else
    {
      log_sysop_commit (thread_p);

      VFID_COPY (&flre_Tracker_vfid, vfid_tracker_out);
    }
  return error_code;
}

/*
 * flre_tracker_register () - register new file in file tracker
 *
 * return        : error code
 * thread_p (in) : thread entry
 * vfid (in)     : file identifier
  ...
  if (!found)
    {
      ...
      error_code = flre_alloc (thread_p, &flre_Tracker_vfid, &vpid_new_page);
      if (error_code != NO_ERROR)
	{
	  ASSERT_ERROR ();
	  goto exit;
...
/*
 * flre_tracker_unregister () - unregister file from tracker
 *
 * return        : error code
 * thread_p (in) : thread entry
 * vfid (in)     : file identifier
 */
static int
flre_tracker_unregister (THREAD_ENTRY * thread_p, const VFID * vfid)
{
  ...
  error_code =
    file_extdata_search_item (thread_p, &extdata, &item_search, file_compare_track_items, true, true, &found, &pos,
			      &page_track_other);
  if (error_code != NO_ERROR)
    {
      ASSERT_ERROR ();
...
/*
 * flre_tracker_apply_to_file () - search for file tracker item and apply function
 *
 * return        : error code
 * thread_p (in) : thread entry
 * vfid (in)     : file identifier
 ...
 */
static int
flre_tracker_apply_to_file (THREAD_ENTRY * thread_p, const VFID * vfid, PGBUF_LATCH_MODE mode,
			    FILE_TRACK_ITEM_FUNC func, void *args)
{
  PAGE_PTR page_track_head = NULL;
  ...
  bool found = false;
  int pos = -1;
  FLRE_TRACK_ITEM *item_in_page = NULL;
  ...
  page_track_head = pgbuf_fix (thread_p, &flre_Tracker_vpid, OLD_PAGE, mode, PGBUF_UNCONDITIONAL_LATCH);
  if (page_track_head == NULL)
    {
      ASSERT_ERROR_AND_SET (error_code);
      return error_code;
  ...
  item_search.fileid = vfid->fileid;
  error_code =
    file_extdata_search_item (thread_p, &extdata, &item_search, file_compare_track_items, true, for_write, &found,
			      &pos, &page_track_other);
  if (error_code != NO_ERROR)
    {
      ASSERT_ERROR ();
      goto exit;
...
/*
 * flre_tracker_map () - map item function to all tracked files
 *
 * return          : error code
 * thread_p (in)   : thread entry
 ...
 */
static int
flre_tracker_map (THREAD_ENTRY * thread_p, PGBUF_LATCH_MODE latch_mode, FILE_TRACK_ITEM_FUNC func, void *args)
{
  PAGE_PTR page_track_head = NULL;
  PAGE_PTR page_track_other = NULL;
  PAGE_PTR page_extdata = NULL;
  ...
  page_track_head = pgbuf_fix (thread_p, &flre_Tracker_vpid, OLD_PAGE, latch_mode, PGBUF_UNCONDITIONAL_LATCH);
  if (page_track_head == NULL)
    {
      ASSERT_ERROR_AND_SET (error_code);
      return error_code;
...
/*
 * flre_tracker_item_reuse_heap () - reuse heap file if marked as deleted
 *
 * return            : error code
 * thread_p (in)     : thread entry
 ...
 */
static int
flre_tracker_item_reuse_heap (THREAD_ENTRY * thread_p, PAGE_PTR page_of_item, FILE_EXTENSIBLE_DATA * extdata,
			      int index_item, bool * stop, void *args)
{
  FLRE_TRACK_ITEM *item = (FLRE_TRACK_ITEM *) file_extdata_at (extdata, index_item);
...
/*
 * flre_tracker_reuse_heap () - search for heap file marked as deleted and reuse.
 *
 * return        : error code
 * thread_p (in) : thread entry
...
/*
 * flre_tracker_item_mark_heap_deleted () - FILE_TRACK_ITEM_FUNC to mark heap entry as deleted
 *
 * return            : error code
 * thread_p (in)     : thread entry
 * page_of_item (in) : page of item
 ...
 */
static int
flre_tracker_item_mark_heap_deleted (THREAD_ENTRY * thread_p, PAGE_PTR page_of_item, FILE_EXTENSIBLE_DATA * extdata,
...
/*
 * flre_tracker_mark_heap_deleted () - search for heap file and mark it for delete
 *
 * return        : error code
 * thread_p (in) : thread entry
 * vfid (in)     : file identifier
 */
int
flre_tracker_mark_heap_deleted (THREAD_ENTRY * thread_p, const VFID * vfid)
{
