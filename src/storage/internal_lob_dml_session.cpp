...
/*
 * internal_lob_repl_tracking_mark () - Remember where the publications of the value about to be written begin.
 */
int
internal_lob_repl_tracking_mark (THREAD_ENTRY *thread_p, internal_lob_repl_tracking &tracking)
{
  LOG_TDES *tdes;

  if (thread_p == NULL)
    {
      return internal_lob_dml_session_set_error ("missing server thread for internal LOB replication tracking");
    }
  tdes = LOG_FIND_TDES (LOG_FIND_THREAD_TRAN_INDEX (thread_p));
  if (tdes == NULL)
    {
      return internal_lob_dml_session_set_error ("missing transaction for internal LOB replication tracking");
    }

  tracking.oid_start = thread_p->oos_oids.size ();
  tracking.lsa_start = tdes->oos_insert_lsa_queue.m_queue.size ();
  return NO_ERROR;
}

/*
 * internal_lob_repl_tracking_capture () - Move the publications made since the mark out of the thread and
 *                                         transaction state, so the next heap insert does not see them.
 */
int
internal_lob_repl_tracking_capture (THREAD_ENTRY *thread_p, internal_lob_repl_tracking &tracking)
{
  LOG_TDES *tdes;
  ...
  tdes = LOG_FIND_TDES (LOG_FIND_THREAD_TRAN_INDEX (thread_p));
  if (tdes == NULL || tracking.oid_start > thread_p->oos_oids.size ()
      || tracking.lsa_start > tdes->oos_insert_lsa_queue.m_queue.size ())
    {
      return internal_lob_dml_session_set_error ("invalid internal LOB replication tracking state");
    }

  tracking.oids.insert (tracking.oids.end (), thread_p->oos_oids.begin () + tracking.oid_start,
			thread_p->oos_oids.end ());
  tracking.lsas.insert (tracking.lsas.end (), tdes->oos_insert_lsa_queue.m_queue.begin () + tracking.lsa_start,
			tdes->oos_insert_lsa_queue.m_queue.end ());

  thread_p->oos_oids.resize (tracking.oid_start);
  tdes->oos_insert_lsa_queue.m_queue.resize (tracking.lsa_start);
  return NO_ERROR;
}

/*
 * internal_lob_repl_tracking_restore () - Publish the captured entries again, on behalf of the row being inserted.
 */
int
internal_lob_repl_tracking_restore (THREAD_ENTRY *thread_p, internal_lob_repl_tracking &tracking)
{
  LOG_TDES *tdes;
  ...
  if (tdes == NULL)
    {
      return internal_lob_dml_session_set_error ("missing transaction for internal LOB adoption");
    }

  for (const LOG_LSA &lsa : tracking.lsas)
    {
      tdes->oos_insert_lsa_queue.push (lsa);
    }
  thread_p->oos_oids.insert (thread_p->oos_oids.end (), tracking.oids.begin (), tracking.oids.end ());
  tracking.lsas.clear ();
  tracking.oids.clear ();
  return NO_ERROR;
}
...
internal_lob_dml_staging_context::finalize_direct_slot (THREAD_ENTRY *thread_p, slot_state &slot)
{
  ...
  error = internal_lob_repl_tracking_mark (thread_p, slot.direct_tracking);
  if (error != NO_ERROR)
    {
      return error;
    }
  error = internal_lob_reverse_insert_end (thread_p, slot.direct_writer, slot.direct_writer.locator);
  tracking_error = internal_lob_repl_tracking_capture (thread_p, slot.direct_tracking);
...
internal_lob_dml_staging_context::delete_unadopted_direct_slots (THREAD_ENTRY *thread_p)
{
  for (slot_state &slot : m_slots)
      ...
      if (error != NO_ERROR)
	{
      ...
      slot.direct_tracking.oids.clear ();
      slot.direct_tracking.lsas.clear ();
    }
  return NO_ERROR;
}
    ...
    DB_TYPE expected_type, INTERNAL_LOB_LOCATOR &locator)
{
  ...
  if (slot->direct_enabled)
	...
	{
	  error = internal_lob_repl_tracking_restore (thread_p, slot->direct_tracking);
	  if (error != NO_ERROR)
	    {
	      return error;
	    }
