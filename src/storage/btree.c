...
static DISK_ISVALID btree_repair_prev_link_by_class_oid (THREAD_ENTRY *
							 ...
							 BTID * idx_btid,
							 bool repair);
static bool btree_check_compressed (THREAD_ENTRY * thread_p, BTID_INT * btid,
				    PAGE_PTR page_ptr);
...
static bool
btree_check_compressed (THREAD_ENTRY * thread_p, BTID_INT * btid,
			PAGE_PTR page_ptr)
{
  int key_cnt;
  RECDES peek_rec;
			  ...
			  PAGE_PTR page_ptr)
{
  ...
  if (btree_check_compressed (thread_p, btid, page_ptr) == false)
    {
      return 0;
    }
