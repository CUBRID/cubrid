...
extern PAGE_PTR pgbuf_flush_debug (THREAD_ENTRY * thread_p, PAGE_PTR pgptr, int free_page, const char *caller_file,
				   int caller_line);
#define pgbuf_copy(thread_p, vpid, page_ptr, size) \
	pgbuf_copy_debug(thread_p, vpid, page_ptr, size, __FILE__, __LINE__)
extern int pgbuf_copy_debug (THREAD_ENTRY * thread_p, const VPID * vpid, PAGE_PTR page_ptr, int size,
...
#define pgbuf_copy(thread_p, vpid, page_ptr, size) \
      pgbuf_copy_release(thread_p, vpid, page_ptr, size)
extern int pgbuf_copy_release (THREAD_ENTRY * thread_p, const VPID * vpid, PAGE_PTR * page_ptr, int size);
#define pgbuf_fix(thread_p, vpid, fetch_mode, requestmode, condition) \
