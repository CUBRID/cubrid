...
catcls_get_or_value_from_class (THREAD_ENTRY * thread_p, OR_BUF * buf_p, OR_VALUE * value_p)
{
  ...
  /* supplemental_log */
  tp_Integer.data_readval (buf_p, &attrs[11].value, NULL, -1, true, NULL, 0);
  ...
  /* (class_of) */
  if (catcls_find_class_oid_by_class_name (thread_p, db_get_string (&attrs[12].value), &class_oid) != NO_ERROR)
    {
      assert (er_errid () != NO_ERROR);
      error = er_errid ();
      goto error;
