...
#define fault_inject_random_crash()
  ...
  error_code = log_get_db_start_parameters (&vhdr->db_creation_time, &vhdr->chkpt_lsa);
		   ...
		   const LOG_LSA * new_chkptlsa, bool logchange, DISK_FLUSH_TYPE flush)
{
  ...
  if (logchange != false)
      ...
      if (redo_recv == NULL)
	{
      ...
      /* Undo stuff */
      memcpy (&undo_recv->db_creation_time, &vhdr->db_creation_time, sizeof (vhdr->db_creation_time));
      ...
      /* Redo stuff */
      memcpy (&redo_recv->db_creation_time, new_dbcreation, sizeof (*new_dbcreation));
...
disk_rv_undoredo_set_creation_time (THREAD_ENTRY * thread_p, LOG_RCV * rcv)
{
  ...
  memcpy (&vhdr->db_creation_time, &change->db_creation_time, sizeof (change->db_creation_time));
...
disk_rv_dump_set_creation_time (FILE * fp, int length_ignore, void *data)
{
  ...
  fprintf (fp, "Label = %s, Db_creation_time = %lld, chkpt = %lld|%lld\n", change->vol_fullname,
	   (long long) change->db_creation_time, (long long) change->chkpt_lsa.pageid,
	   (long long) change->chkpt_lsa.offset);
}
...
disk_volume_header_next_scan (THREAD_ENTRY * thread_p, int cursor, DB_VALUE ** out_values, int out_cnt, void *ptr)
{
  ...
  db_localdatetime ((time_t *) (&vhdr->vol_creation_time), &vol_creation_time);
  db_make_datetime (out_values[idx], &vol_creation_time);
  idx++;
...
disk_vhdr_dump (FILE * fp, const DISK_VOLUME_HEADER * vhdr)
{
  ...
  tmp_time = (time_t) vhdr->db_creation_time;
 ...
 *   return: void
 *   volid(in): Permanent volume identifier
 *   db_creation_time(out): Database creation time according to the volume
 */
int
disk_get_creation_time (THREAD_ENTRY * thread_p, INT16 volid, INT64 * db_creation_time)
{
  DISK_VOLUME_HEADER *vhdr;
  PAGE_PTR hdr_pgptr = NULL;
  int error_code = NO_ERROR;
  ...
  memcpy (db_creation_time, &vhdr->db_creation_time, sizeof (*db_creation_time));
...
disk_can_overwrite_data_volume (THREAD_ENTRY * thread_p, const char *vol_label_p, bool * can_overwrite)
{
  ...
  *can_overwrite = false;
  ...
  read_size = offsetof (DISK_VOLUME_HEADER, db_creation_time) + sizeof (((DISK_VOLUME_HEADER *) 0)->db_creation_time);
  ...
  if (strncmp (vhdr->magic, CUBRID_MAGIC_DATABASE_VOLUME, CUBRID_MAGIC_MAX_LENGTH) == 0
      && log_Gl.hdr.db_creation_time > vhdr->db_creation_time)
    {
