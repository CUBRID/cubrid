...
cubrid deletedb demodb && $CUBRID/demo/make_cubrid_demo.sh
...
select /*+ recompile no_parallel_heap_scan */ id, vec <-> '[0, 1, 0]' from random_xs_20_angular_train order by vec <-> '[0,1,0]' limit 5;