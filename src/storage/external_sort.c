...
sort_check_parallelism (THREAD_ENTRY * thread_p, SORT_PARAM * sort_param)
{
  ...
  if (sort_param->px_type == SORT_ORDER_BY)
	...
	{
	  /* clamp to the number of workers actually reserved: try_reserve_workers () may grant a partial
	   * reservation under worker-pool contention. Pushing parallel_num tasks when fewer were reserved
	   * over-subscribes the shared pool. Same pattern as parallel scan / hash join. */
      ...
      else
	{
	   ...
	   * same as the SORT_ORDER_BY branch above. */
	  return sort_param->px_worker_manager->get_reserved_workers ();
	}
    }
  else
