...
{				/* Volume header */
  ...
  INT32 max_npages;		/* max page count of this volume this is not equal to the total_pages, if this volume
				 * is auto extended */
  INT32 used_data_npages;	/* allocated pages for data purpose *//* TODO: Replace with sectors */
  INT32 used_index_npages;	/* allocated pages for index purpose *//* TODO: Replace with sectors */
...
extern VOLID disk_add_auto_volume_extension (THREAD_ENTRY * thread_p, DKNPAGES min_npages,
...
#endif
extern VOLID disk_find_goodvol (THREAD_ENTRY * thread_p, INT16 hint_volid, INT16 undesirable_volid, INT32 exp_numpages,
				DISK_SETPAGE_TYPE setpage_type, DISK_VOLPURPOSE vol_purpose);
extern bool disk_goodvol_refresh_with_new (THREAD_ENTRY * thread_p, INT16 volid);

extern INT16 disk_format (THREAD_ENTRY * thread_p, const char *dbname, INT16 volid, DBDEF_VOL_EXT_INFO * ext_info);
extern int disk_unformat (THREAD_ENTRY * thread_p, const char *vol_fullname);
extern INT32 disk_expand_tmp (THREAD_ENTRY * thread_p, INT16 volid, INT32 min_pages, INT32 max_pages);
extern int disk_expand_perm (THREAD_ENTRY * thread_p, INT16 volid, INT32 npages);
extern VOLID disk_cache_get_auto_extend_volid (THREAD_ENTRY * thread_p);
extern int disk_cache_set_auto_extend_volid (THREAD_ENTRY * thread_p, VOLID volid);
...
extern DISK_ISVALID disk_isvalid_page (THREAD_ENTRY * thread_p, INT16 volid, INT32 pageid);
...
extern int disk_set_alloctables (DISK_VOLPURPOSE vol_purpose, INT32 total_sects, INT32 total_pages,
				 INT32 * sect_alloctb_npages, INT32 * page_alloctb_npages, INT32 * sect_alloctb_page1,
				 INT32 * page_alloctb_page1, INT32 * sys_lastpage);

extern INT16 disk_get_all_total_free_numpages (THREAD_ENTRY * thread_p, DISK_VOLPURPOSE vol_purpose, INT16 * nvols,
					       int *total_pages, int *free_pages);
#if defined(ENABLE_UNUSED_FUNCTION)
extern INT16 disk_get_first_total_free_numpages (THREAD_ENTRY * thread_p, DISK_VOLPURPOSE purpose, INT32 * ntotal_pages,
						 INT32 * nfree_pgs);
...
#if defined(ENABLE_UNUSED_FUNCTION)
extern INT32 disk_get_overhead_numpages (THREAD_ENTRY * thread_p, INT16 volid);
#endif /* ENABLE_UNUSED_FUNCTION */
extern INT32 disk_get_maxcontiguous_numpages (THREAD_ENTRY * thread_p, INT16 volid, INT32 max_npages);
...
extern INT32 disk_get_max_numpages (THREAD_ENTRY * thread_p, DISK_VOLPURPOSE vol_purpose);
extern DISK_ISVALID disk_check (THREAD_ENTRY * thread_p, INT16 volid, bool repair);
...
extern int disk_vhdr_rv_undoredo_free_sectors (THREAD_ENTRY * thread_p, LOG_RCV * rcv);
extern void disk_vhdr_rv_dump_free_sectors (FILE * fp, int length_ignore, void *data);
...
extern void disk_rv_dump_alloctable (FILE * fp, int length_ignore, void *data);
...
extern void disk_deallocate_volheader (THREAD_ENTRY * thread_p, LOG_RCV * rcv, LOG_LSA * ref_lsa);
extern int disk_rv_alloctable_with_volheader (THREAD_ENTRY * thread_p, LOG_RCV * rcv, LOG_LSA * ref_lsa);
extern void disk_rv_dump_alloctable_with_vhdr (FILE * fp, int length_ignore, void *data);
extern int disk_rv_set_alloctable_bitmap_only (THREAD_ENTRY * thread_p, LOG_RCV * rcv);
extern int disk_rv_clear_alloctable_bitmap_only (THREAD_ENTRY * thread_p, LOG_RCV * rcv);

extern int disk_rv_set_alloctable_vhdr_only (THREAD_ENTRY * thread_p, LOG_RCV * rcv);
extern int disk_rv_clear_alloctable_vhdr_only (THREAD_ENTRY * thread_p, LOG_RCV * rcv);
