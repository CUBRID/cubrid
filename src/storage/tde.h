...
{
  unsigned char
    perm_key[TDE_DATA_KEY_LENGTH];
  unsigned char
    temp_key[TDE_DATA_KEY_LENGTH];
  unsigned char
    log_key[TDE_DATA_KEY_LENGTH];
} TDE_DATA_KEY_SET;

typedef struct tde_mk_file_item
{
  time_t
    created_time;		/* If it is -1, it is invalid and avaliable for a new key */
  unsigned char
    master_key[TDE_MASTER_KEY_LENGTH];
} TDE_MK_FILE_ITEM;
...
typedef struct tde_cipher
{
  bool
    is_loaded;
  TDE_DATA_KEY_SET
    data_keys;			/* data keys decrypted from tde keyinfo heap, which is constant */
  int64_t
    temp_write_counter;		/* used as nonce for temp file page, it has to be dealt atomically */
} TDE_CIPHER;

extern TDE_CIPHER
  tde_Cipher;			/* global var for TDE Module */
...
typedef struct tde_keyinfo
{
  ...
  time_t
    set_time;
  unsigned char
    mk_hash[TDE_MASTER_KEY_LENGTH];
  unsigned char
    dk_perm[TDE_DATA_KEY_LENGTH];
  unsigned char
    dk_temp[TDE_DATA_KEY_LENGTH];
  unsigned char
    dk_log[TDE_DATA_KEY_LENGTH];
} TDE_KEYINFO;
...
extern int
tde_copy_keys_file (THREAD_ENTRY * thread_p, const char *dest_fullname, const char *src_fullname,
...
extern int
tde_delete_mk (int vdes, const int mk_index);
extern void
