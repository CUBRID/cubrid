 ...
 *
 */ 
  BTID * xbtree_load_index (THREAD_ENTRY * thread_p, BTID * btid, const char *bt_name, TP_DOMAIN * key_type,
			     OID * class_oids, int n_classes, int n_attrs, int *attr_ids, int *attrs_prefix_length,
			     HFID * hfids, int unique_pk, int not_null_flag, OID * fk_refcls_oid,
			     BTID * fk_refcls_pk_btid, const char *fk_name, char *pred_stream, int pred_stream_size,
			     ...
			     int func_attr_index_start) 
{
  ...
  int track_id;
    ...
    
    {
	   ...
	   (thread_p, (FUNC_PRED **) (&func_index_info.expr), func_pred_stream, func_pred_stream_size,
	    &func_unpack_info))
    ...
    has_fk = (fk_refcls_oid != NULL && !OID_ISNULL (fk_refcls_oid));
       ...
       (thread_p, &sort_args->hfscan_cache, &sort_args->hfids[cur_class], &sort_args->class_ids[cur_class], !has_fk,
	false, NULL) != NO_ERROR)
	...
	(thread_p, &sort_args->class_ids[cur_class], sort_args->n_attrs, &sort_args->attr_ids[attr_offset],
  ...
  /** Initialize the fields of loading argument structures **/ 
    ...
    *load_args->btid->sys_btid = *btid;
	...
	log_sysop_abort (thread_p);
	   ...
	   (thread_p, btid, key_type, &class_oids[0], attr_ids[0], unique_pk, sort_args->n_oids, sort_args->n_nulls,
		    ...
		    int node_level) 
{
	...
	(thread_p, load_args->btid, &nleaf_rec, key, BTREE_NON_LEAF_NODE, key_type, key_len, true, NULL, NULL, NULL,
	...
	if (btree_load_new_page
	    (thread_p, load_args->btid->sys_btid, &load_args->nleaf.hdr, node_level, &load_args->nleaf.vpid,
	     &load_args->nleaf.pgptr) != NO_ERROR)
...
btree_build_nleafs (THREAD_ENTRY * thread_p, LOAD_ARGS * load_args, int n_nulls, int n_oids, int n_keys) 
{
   ...
   ****************************************************/ 
	...
	{			/* while pop_list is not empty */
	    ...
	    cur_nleafpgptr =
	    pgbuf_fix (thread_p, &cur_nleafpgid, OLD_PAGE, PGBUF_LATCH_WRITE, PGBUF_UNCONDITIONAL_LATCH);
...
btree_construct_leafs (THREAD_ENTRY * thread_p, const RECDES * in_recdes, void *arg) 
{
	...
	(*(load_args->btid->key_type->type->data_readval)) (&buf, &this_key, load_args->btid->key_type, key_size, copy,
							    NULL, 0);
	...
	{			/* This is not the first call to this function */
		...
		
		{
			...
			
			{
			    ...
			    btree_mvcc_info_from_heap_mvcc_header (&mvcc_header, &mvcc_info);
							   ...
							   &this_oid, &this_class_oid, &mvcc_info, &offset, NULL,
							   NULL);
		...
		{		/* There is no space for the new oid */
			...
			btree_leaf_record_change_overflow_link (thread_p, load_args->btid,
	...
	
	{
	  ...
	  char *pgptr;
	    ...
	    
	    {
			      ...
			      original_oid.pageid, original_oid.slotid, original_class_oid.volid,
...
btree_dump_sort_output (const RECDES * recdes, LOAD_ARGS * load_args) 
{
	...
	(&buf, &this_key, load_args->btid->key_type, key_size, copy, NULL, 0) != NO_ERROR)
...
btree_get_perf_btree_page_type (THREAD_ENTRY * thread_p, PAGE_PTR page_ptr) 
{
	...
	|| spage_get_record (thread_p, page_ptr, HEADER, &header_record, PEEK) != S_SUCCESS)
...
btree_load_check_fk (THREAD_ENTRY * thread_p, const LOAD_ARGS * load_args, const SORT_ARGS * sort_args) 
{
  ...
  bool has_null = false, clear_pcontext = false;
  PRUNING_CONTEXT pcontext;
  BTID pk_btid;
  OID pk_oid;
  HFID pk_hfid;
  ret = db_value_clone (&pk_key, &load_args->current_key);
    ...
    /*
       if (sort_args->n_attrs > 1)
       {
       has_null = btree_multicol_key_has_null (&pk_key);
       }
       else
       {
       has_null = DB_IS_NULL (&pk_key);
       }
       
       if (!has_null)
       { */ 
    /* Get class representaion to find partition information */ 
    classrepr = heap_classrepr_get (thread_p, sort_args->fk_refcls_oid, NULL, NULL_REPRID, &classrepr_cacheindex);
  if (classrepr == NULL)
    
    {
      val_print = pr_valstring (&pk_key);
	...
	{
	  ret = partition_prune_unique_btid (&pcontext, &pk_key, &pk_oid, &pk_hfid, &pk_btid);
	...
	
	{
		...
		btree_advance_to_next_slot_and_fix_page (thread_p, &pk_bt_scan.btid_int, &pk_bt_scan.C_vpid,
							 ...
							 &pk_node_key_cnt, &pk_node_header, &mvcc_snapshot_dirty);
