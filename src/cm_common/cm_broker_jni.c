 ...
 */
static jobjectArray
new_hash_map_array (JNIEnv * env, int len, int create)
{
  jclass map_class;
  jobjectArray res;
  int i;
  ...
  FAIL_RET_X (env, NULL, map_class = (*env)->FindClass (env, "java/util/HashMap"));
  FAIL_RET_X (env, NULL, map_constructor_id = (*env)->GetMethodID (env, map_class, "<init>", "()V"));
  FAIL_RET_X (env, NULL, res = (*env)->NewObjectArray (env, len, map_class, NULL));
  ...
  if (create)
    {
      ...
      for (i = 0; i < len; i++)
	{
	  jobject map = (*env)->NewObject (env, map_class, map_constructor_id);
...
static jobjectArray
reduce_array (JNIEnv * env, jobjectArray array)
{
...
static int
throw_runtime_exception (JNIEnv * env, const char *msg)
{
  jclass exception_clazz = (*env)->FindClass (env, "java/lang/RuntimeException");
  FAIL_RET_X (env, -1, exception_clazz);
  return (*env)->ThrowNew (env, exception_clazz, msg);
}

static jobject
put_object (JNIEnv * env, jobject map, const char *key, jobject val)
{
  jclass clazz;
  ...
  FAIL_RET_X (env, NULL, put_method_id =
	      (*env)->GetMethodID (env, clazz, "put", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;"));
  FAIL_RET_X (env, NULL, key0 = (*env)->NewStringUTF (env, key));
  return (*env)->CallObjectMethod (env, map, put_method_id, key0, val);
}

static jobject
put_string_utf8 (JNIEnv * env, jobject map, const char *key, const char *val)
{
  jstring val0;
...
static jobject
put_char_as_string (JNIEnv * env, jobject map, const char *key, char val)
{
  char str[4] = { 0 };
  str[0] = val;
...
static jobject
put_int (JNIEnv * env, jobject map, const char *key, int val)
{
  jclass clazz;
  jmethodID constructor_id;
  jobject val0;

  FAIL_RET_X (env, NULL, clazz = (*env)->FindClass (env, "java/lang/Integer"));
  FAIL_RET_X (env, NULL, constructor_id = (*env)->GetMethodID (env, clazz, "<init>", "(I)V"));
  FAIL_RET_X (env, NULL, val0 = (*env)->NewObject (env, clazz, constructor_id, val));
  return put_object (env, map, key, val0);
}

static jobject
put_long (JNIEnv * env, jobject map, const char *key, int64_t val)
{
  jclass clazz;
  jmethodID constructor_id;
  jobject val0;
  ...
  FAIL_RET_X (env, NULL, constructor_id = (*env)->GetMethodID (env, clazz, "<init>", "(J)V"));
  FAIL_RET_X (env, NULL, val0 = (*env)->NewObject (env, clazz, constructor_id, val));
  return put_object (env, map, key, val0);
}

static jobject
put_float (JNIEnv * env, jobject map, const char *key, float val)
{
  jclass clazz;
  jmethodID constructor_id;
  jobject val0;

  FAIL_RET_X (env, NULL, clazz = (*env)->FindClass (env, "java/lang/Float"));
  FAIL_RET_X (env, NULL, constructor_id = (*env)->GetMethodID (env, clazz, "<init>", "(F)V"));
  FAIL_RET_X (env, NULL, val0 = (*env)->NewObject (env, clazz, constructor_id, val));
  return put_object (env, map, key, val0);
}
  ...
  for (i = 0; i < count; i++)
    {
      ...
      FAIL_GOTO (env, fail, obj = (*env)->GetObjectArrayElement (env, res, i));
      ...
      EXCEP_GOTO (env, fail);
      put_int (env, obj, "long_transaction_time", br_info->long_transaction_time);
      EXCEP_GOTO (env, fail);
      put_string_utf8 (env, obj, "session_timeout", br_info->session_timeout);
      EXCEP_GOTO (env, fail);
      put_int (env, obj, "as_max_size", br_info->as_max_size);
      EXCEP_GOTO (env, fail);
      put_string_utf8 (env, obj, "keep_connection", br_info->keep_connection);
      EXCEP_GOTO (env, fail);
      put_string_utf8 (env, obj, "sql_log_mode", br_info->sql_log_mode);
      EXCEP_GOTO (env, fail);
      put_char_as_string (env, obj, "log_backup", br_info->log_backup);
      EXCEP_GOTO (env, fail);
      put_char_as_string (env, obj, "source_env_flag", br_info->source_env_flag);
      EXCEP_GOTO (env, fail);
      put_char_as_string (env, obj, "access_list_flag", br_info->access_list_flag);
      EXCEP_GOTO (env, fail);
...
Java_com_cubrid_jni_BrokerJni_getAllCasInfo0 (JNIEnv * env, jclass clazz, jstring name, jboolean only_active)
{
  ...
  for (i = 0; i < count; i++)
      ...
      if (only_active && strcasecmp ("ON", cas_info->service_flag) != 0)
	{
      ...
      EXCEP_GOTO (env, fail);
      put_long (env, obj, "last_access_time", cas_info->last_access_time * 1000LL);
      EXCEP_GOTO (env, fail);
      ...
      EXCEP_GOTO (env, fail);
      put_long (env, obj, "last_connect_time", cas_info->last_connect_time * 1000LL);
      EXCEP_GOTO (env, fail);
      put_long (env, obj, "num_requests_received", cas_info->num_requests_received);
      EXCEP_GOTO (env, fail);
      put_long (env, obj, "num_queries_processed", cas_info->num_queries_processed);
      EXCEP_GOTO (env, fail);
      put_long (env, obj, "num_transactions_processed", cas_info->num_transactions_processed);
      EXCEP_GOTO (env, fail);
      put_long (env, obj, "num_long_queries", cas_info->num_long_queries);
      EXCEP_GOTO (env, fail);
      put_long (env, obj, "num_long_transactions", cas_info->num_long_transactions);
      EXCEP_GOTO (env, fail);
...
Java_com_cubrid_jni_BrokerJni_getAllJobInfo0 (JNIEnv * env, jclass clazz, jstring name, jboolean only_active)
{
  ...
  FAIL_GOTO (env, fail, res = new_hash_map_array (env, job_info_all.num_info, 1));
