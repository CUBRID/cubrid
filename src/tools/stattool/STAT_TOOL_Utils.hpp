...
class Utils
{
  public:
    ...
    static void perfmeta_custom_dump_stats_in_table_form (const std::vector<StatToolSnapshot *> &snapshots, FILE *stream,
        int show_complex,
        int show_zero);
  private:
    static void perfmon_print_timer_to_file_in_table_form (FILE *stream, int stat_index,
        const std::vector<StatToolSnapshot *> &snapshots,
        int show_zero, int show_header);
    static void perfmon_stat_dump_in_file_in_table_form (FILE *stream, PSTAT_METADATA *stat,
        const std::vector<StatToolSnapshot *> &snapshots,
        int show_zeroes);
};
