 ...
 */
 /* 
  *                                                                           
  * The main purpose of the query interface module is is to provide a uniform,
  * coherent interface mechanism between the query processor and the           
  * applications, such as ISQL/X, ESQL/X applications which deal with  
  * queries. In addition to providing routines to verify and execute queries,  
  * the module manages the query results through its inherent cursor mechanism.
  *                                                                           
  * The query interface module forms a bridge between applications and the
  * query processor, which can be diagramatically shown as below:
  *
  *     ----------             ---------             ---------
  *     | ISQL/X |             |ESQL/X |             | Other |
  *     ----------             ---------             ---------
  ...
  *                          ------------
  *
  * The set of interface routines fall into three categories:
  *
  *  i - pre-processing routines: query strings are checked for both (1)
  * validity and (2) to pre-determine the format (type list) without actually
  * executing the query.
  *
  * ii - processing routines: the query strings are executed, LIST FILEs
  * which are used to store query results are generated by query processor,
  * tuple level and tuple value level cursor accessor functions are used
  * to access each tuple/value of the LIST FILEs.
  *
  * iii - post-processing routines: client side query pages and structures are
  * freed and messages are sent to the server side to remove the LIST FILEs
  * from the disk.
  *
  * There are four main data structures used the Query Interface:
  *  (i) query format/type structure
  * (ii) query result structure
  *(iii) global query table structure
  * (iv) query result structure resource pool
  *
  * Query format/type structure is a linked list of nodes, each node
  * containing information related to the corresponding query result column,
  * such as the data type of the column, the name of the column and domain
  * information for the column.
  *
  * Query result structure is a handle that is given to applications and
  * represents the query result. It includes a cursor identifier which
  * includes the query result list file identifer in the case of regular SQLX
  * select queries and other pertinent information in the case of "object
  * fetch", "object insert" and "call method" cases. The query result structure
  * handle forms the basic communication unit between applications and query
  * interface routines.
  *
  * The global query table structure is the main structure where all the query
  * result structures are maintained. In addition to some global information
  * such as number of active query entries, it basically includes an array of
  * pointers to query result structures.
  *
  * The query result structure resource pool is a pool of query result
  * structures which can be used as a newly allocated query result structure    
  * and returned back to the pool. The main use of query result structure       
  * resource pool is to avoid expensive malloc/free operations.                 
  *                                                                             
  *  i - pre-processing routines:                                               
  *                                                                             
  * The query interface routines pass the query strings given by applications   
  * to the query processor to verify the query and to get the type information  
  * for the query. The query type structure thus formed is returned back to     
  * applications. This is an independent phase between applications and the     
  * query processor and the query is no longer known by the query processor.    
  * A set of accessor functions are provided to the applications to access      
  * the query type information.                                                 
  *                                                                             
  * ii - processing routines:                                                   
  *                                                                             
  * The query strings given by applications are passed to the query processor   
  * for execution. The results of the queries are stored in what is called a    
  * LIST FILE and the list file identifier is returned to the query interface   
  * module. LIST FILEs are created, stored and managed on the server. The data  
  * items in a LIST FILE reflect the result of a query with respect to the      
  * database content at the time query is issued and are no longer associated   
  * with the database (with some exceptions like the use of the oid).           
  *                                                                             
  ...
  *  +----------+                                                               
  *                                                                             
  * Once the list file identifier is returned to the query interface routines,  
  * the query interface routines forms a query result structure to be returned  
  * to applications by opening a cursor on the list file and encapsulating the  
  * cursor identifier in the query result structure. The job of the cursor is   
  * to bring the LIST file pages from server to client as needed, manage        
  * storage of LIST FILE pages on the client side and allow access to each LIST 
  * FILE tuple in both forward and backward directions.                         
  *                                                                             
  ...
  *                                                                             
  * The query result structure handle forms the basic commuication parameter    
  * to scan through the cursor to access list file tuples. A set of accessor    
  * functions are provided to access each tuple value. Tuple values can be      
  * referenced by column name as well as index, ie. position number of the      
  * value within the tuple.                                                     
  *                                                                             
  * iii - post-processing routines:                                             
  *                                                                             
  * When applications indicate that they are done with a query, the query       
  * interface routines invalidates the corresponding query result structure     
  * and informs the query processor to destroy the query result list file       
  * stored on the server.                                                       
  *                                                                             
  * When a transaction commits/aborts on the client, or when the server goes    
  * down, the transaction manager contacts the query interface module and       
  * query interface module closes existing query result structures.             
  *                                                                             
  * The callers of the module include ISQLX, ESQLX, SQLX component      
  * modules and the transaction manager.                                        
  *                                                                             
  * The callees of the module include Query Processor.
  *       
  */
...
static int query_compile_local (const char *SQLX_query,
				DB_QUERY_ERROR * query_error,
				int include_oid,
				DB_SESSION ** session, int *stmt_no);
static int query_execute_local (const char *SQLX_query, void *result,
				DB_QUERY_ERROR * query_error,
...
db_init_query_result (DB_QUERY_RESULT * r, DB_RESULT_TYPE r_type)
{
  ...
  switch (r->type)
	...
	r->res.s.query_id = -1;
	r->res.s.stmt_id = -1;
	r->res.s.stmt_type = (SQLX_CMD_TYPE) 0;
 ...
 *              query_execute_local.
 * return : error code.
 * SQLX_query (in) : query string to be executed
 ...
 */
static int
query_compile_local (const char *SQLX_query, DB_QUERY_ERROR * query_error,
		     int include_oid, DB_SESSION ** session, int *stmt_no)
{
  ...
  CHECK_CONNECT_ERROR ();
  *session = db_open_buffer_local (SQLX_query);
  if (!(*session))
    {
      return (er_errid ());
    }
 ...
 */
int
db_execute_with_values (const char *SQLX_query, DB_QUERY_RESULT ** result,
			...
			DB_VALUE * vals)
{
  ...
  error = query_compile_local (SQLX_query, query_error, DB_NO_OIDS,
			       &session, &stmt_no);
  if (error < 0)
    {
      return error;
 ...
 */
static int
query_execute_local (const char *SQLX_query, void *result,
		     DB_QUERY_ERROR * query_error,
		     int include_oid, int execute, QUERY_EXEC_MODE exec_mode)
{
  ...
  /* compile the query */
  error = query_compile_local (SQLX_query, query_error, include_oid,
			       &session, &stmt_no);
  if (error < 0)
    {
      return error;
 ...
 */
int
db_get_query_format (const char *SQLX_query, DB_QUERY_TYPE ** result,
		     DB_QUERY_ERROR * query_error)
{
  int error;

  error = query_execute_local (SQLX_query, result, query_error,
 ...
 * query_type(in): Pointer to the current type list node
 *									      
 * note : The source (physical) domain of an expression is defined as:  
 *              1. If the expression is a path expression, the source domain  
 *                 of the expression is the scalar-subdomain of the result    
 *                 (physical) domain of the last non-leaf node in the path    
 *                 expression.                                                
 *                 A host variable is not a path expression, but a correlation
 *                 variable (including selector variable) is a trivial path
 *                 expression.                        
 *              2. If the expression is not a path expression, the source     
 *                 domain is a domain with null type (DB_TYPE_NULL).
 *
 *                 E.g., assume class person has a subclass employee;         
 *                 address_1 has a subclass address_2; address_2 in turn has  
 *                 a subclass address_3; residence is an attribute of person  
 *                 that has type address_1; city is an attribute of address_1 
 *                 that has type string.                                      
 *                 Consider the following query:                              
 *                 select 3, x, x.residence, x.residence[y],                  
 *                        x.residence[y].city                                 
 *                 from person x, (address_1, address_3) y;                   
 *                 The source domain for the four columns are (NULL),         
 *                 (person), (person), (person), and (address_1, address_3),  
 *                 respectively.                                              
 */
SM_DOMAIN *
 ...
 * return : error code.
 * SQLX_query(in): SQL/X query string to be executed
 * result(out): Pointer to the query result structure
 * query_error(out): Set to the error information, if any.
 * 
 ...
 */
int
db_query_execute (const char *SQLX_query, DB_QUERY_RESULT ** result,
		  DB_QUERY_ERROR * query_error)
{
  int error;
 ...
 */
int
db_execute_async (const char *SQLX_text, DB_QUERY_RESULT ** result,
		  DB_QUERY_ERROR * query_error)
{
  int retval;

  retval = query_execute_local (SQLX_text, result, query_error,
				DB_NO_OIDS, 1, ASYNC_EXEC);
  return (retval);
}

/*
 * db_execute() - This function is used to evaluate the SQLX_text statement(s)
 ...
 * SQLX_text(in): query string to be executed
 * result(out): Pointer to the query result structure
 * query_error(out): Set to the error information, if any.
 *									      
 ...
 */
int
db_execute (const char *SQLX_text, DB_QUERY_RESULT ** result,
	    DB_QUERY_ERROR * query_error)
{
  int retval;
 ...
 */
int
db_execute_oid (const char *SQLX_text, DB_QUERY_RESULT ** result,
		DB_QUERY_ERROR * query_error)
{
  int retval;
 ...
 */
int
db_query_execute_immediate (const char *SQLX_query, DB_QUERY_RESULT ** result,
			    DB_QUERY_ERROR * query_error)
{
  int r;
  ...
  r = query_execute_local (SQLX_query, result, query_error,
			   DB_NO_OIDS, 1, SYNC_EXEC);
  return r;
}
