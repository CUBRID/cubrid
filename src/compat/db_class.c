...
/*
 * db_add_filter_and_function_index_to_catalog_classes() -
 *  return: error code
 *  added(out): 1 if filter and function index are added to "_db_index", 
 *		0 otherwise
 */
int
db_add_filter_and_function_index_to_catalog_classes (int *added)
{
  MOP class_mop;
  SM_TEMPLATE *def;
  int error_code;
  ...
  *added = 0;
  ...
  AU_DISABLE (save);
  class_mop = db_find_class ("_db_index");
  if (class_mop == NULL)
    {
      return er_errid ();
    }

  id = sm_att_id (class_mop, "filter_expression");
  if (id != -1)
    {
      return NO_ERROR;
    }

  id = sm_att_id (class_mop, "function_expr");
  if (id != -1)
    {
      return NO_ERROR;
    }
  ...
  error_code =
    smt_add_attribute (def, "filter_expression", "varchar(255)", NULL);
  if (error_code != NO_ERROR)
    {
      return error_code;
    }
