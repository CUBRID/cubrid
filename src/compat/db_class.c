  ...
  if (def == NULL)
      ...
      else
	{
	  if (default_value != NULL)
		...
		{
		  error = smt_set_attribute_default (def, name, 1, default_value, NULL, NULL);
		}
	      else
		{
		  error = smt_set_attribute_default (def, name, 0, default_value, NULL, NULL);
		}
	    }
	  if (error)
	    {
...
db_change_default (MOP class_, const char *name, DB_VALUE * value)
{
  ...
  if (def == NULL)
    ...
    {
      error = smt_set_attribute_default (def, name, false, value, NULL, NULL);
      if (error)
	{
	  smt_quit (def);
	}
