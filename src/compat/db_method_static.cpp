 ...
 *       created.
 *
 * Note: restricted to DBA group members (KVE-2026-1859) -- this stores an
 *       already-encoded password verbatim, so it must not be reachable by a
 *       non-DBA user, even for their own account.
 */
static void
au_set_password_encoded_method (MOP user, DB_VALUE *returnval, DB_VALUE *password)
{
  ...
  if (Au_dba_user != NULL && !au_is_dba_group_member (Au_user))
    {
      error = ER_AU_DBA_ONLY;
      er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE, error, 1, "set_password_encoded");
      db_make_error (returnval, error);
      return;
    }
...
au_set_password_encoded_sha1_method (MOP user, DB_VALUE *returnval, DB_VALUE *password)
{
  ...
  if (Au_dba_user != NULL && !au_is_dba_group_member (Au_user))
    {
      error = ER_AU_DBA_ONLY;
      er_set (ER_WARNING_SEVERITY, ARG_FILE_LINE, error, 1, "set_password_encoded_sha1");
      db_make_error (returnval, error);
      return;
    }
