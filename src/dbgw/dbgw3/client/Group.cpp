...
namespace dbgw
{
  ...
  public:
    Impl(_Group *pSelf, _Service *pService, const std::string &fileName,
      ...
      m_dbType(dbType), m_pConnPoolStatItem(new _StatisticsItem("CS")),
      m_pStmtPoolStatItem(new _StatisticsItem("SS"))
    {
      m_logger.setGroupName(m_name);

      trait<_StatisticsMonitor>::sp pMonitor = m_pSelf->getMonitor();

      m_pConnPoolStatItem->addColumn(
      ...
      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_PADDING) = "*";
      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_NAMESPACE) =
          getNameSpace().c_str();
      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_GROUPNAME) = m_name.c_str();

      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_ACTIVE_CNT).setRightAlign();
      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_IDLE_CNT).setRightAlign();
      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_SUCC_CNT).setRightAlign();
      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_FAIL_CNT).setRightAlign();
      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_EVICT_CNT).setRightAlign();
      ...
      m_pStmtPoolStatItem->getColumn(DBGW_STMT_POOL_STAT_COL_PADDING) = "*";
      m_pStmtPoolStatItem->getColumn(DBGW_STMT_POOL_STAT_COL_GROUPNAME) = m_name.c_str();

      m_pStmtPoolStatItem->getColumn(DBGW_STMT_POOL_STAT_COL_TOTAL_CNT).setRightAlign();
      m_pStmtPoolStatItem->getColumn(DBGW_STMT_POOL_STAT_COL_EVICT_CNT).setRightAlign();
      m_pStmtPoolStatItem->getColumn(DBGW_STMT_POOL_STAT_COL_HIT_RATIO).setRightAlign();

      m_pStmtPoolStatItem->getColumn(DBGW_STMT_POOL_STAT_COL_HIT_RATIO).setPrecision(3);
    ...
    void initPool(const _ExecutorPoolContext &context)
    {
      ...
      try
          ...
          for (size_t i = 0; i < context.initialSize; i++)
            {
              ...
              m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_IDLE_CNT)++;
        ...
        {
          m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_FAIL_CNT)++;
          ...
          if ((*it)->isEvictable(m_execPoolContext.minEvictableIdleTimeMillis))
            {
              ...
              m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_IDLE_CNT)--;
              m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_EVICT_CNT)++;
            }
          else
            {
              ++it;
    ...
    trait<_Executor>::sp getExecutor()
    {
      ...
      m_pConnPoolStatItem->getColumn(DBGW_CONN_POOL_STAT_COL_ACTIVE_CNT)++;
  ...
  private:
    ...
    trait<_StatisticsItem>::sp m_pConnPoolStatItem;
    trait<_StatisticsItem>::sp m_pStmtPoolStatItem;
  };
  ...
  trait<_StatisticsItem>::sp _Group::getStatementStatItem()
  {
