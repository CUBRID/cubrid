...
#define LOCK_ITEMS_SIZE    (VACUUM_MAX_WORKER_COUNT)
  ...
  public:
    ...
    void
    Lock (int lock_index, int owner_id)
    {
    ...
    void
    UnLock (int lock_index, int owner_id)
    {
      assert (lock_index >= 0 && lock_index < LOCK_ITEMS_SIZE);
      m_ovfp_monitor_mutex.lock();
...
class COvfpThreshold
{
  private:
    INDEX_OVFP_INFO    *m_free_head;
    COvfpMonitorLock   *m_lock_ptr;
  ...
  private:
    ...
    void
    free_info_mem (INDEX_OVFP_INFO *pt)
    {
      assert (pt != NULL);
    ...
    INDEX_OVFP_INFO *
    add (BTID *btid, OID *class_oid, time_t time_now, int npages)
    {
      INDEX_OVFP_INFO  *pt;
  ...
  protected:
      ...
      else
	{
      ...
      //memset(pt, 0x00, sizeof(INDEX_OVFP_INFO));
      return pt;
    }

    INDEX_OVFP_INFO *
    find (BTID *btid, OID *class_oid)
    {
      int cmp;
  ...
  public:
    ...
    void
    set_worker_idx (int idx, COvfpMonitorLock *clockmgr)
    {
    ...
    void
    add_info (BTID *btid, OID *class_oid, int npages)
    {
      time_t     time_now;
    ...
    void
    chcek_over_threshold_times (time_t *over_tm)
    {
...
class COvfpPrinter: public COvfpThreshold
{
  public:
    void
    sort()
    ...
    void
    add_info (INDEX_OVFP_INFO *new_info)
    {
      INDEX_OVFP_INFO *pt;
...
class COvfpThresholdMgr
{
  private:
    COvfpMonitorLock     m_cOvfpLock;
    COvfpThreshold       m_cOvfpThreshold[VACUUM_MAX_WORKER_COUNT];
  ...
  private:
    bool
    get_classoid (THREAD_ENTRY *thread_p, BTID *btid, OID *class_oid)
    {
    ...
    void
    get_class_name_index_name (THREAD_ENTRY *thread_p, BTID *btid, OID *class_oid, char **class_name, char **index_name)
    {
      int ret = NO_ERROR;
      char tmp[128];
    ...
    char *
    time_to_string (time_t er_time, char *buf, int size)
    {
      struct tm er_tm;
    ...
    void
    print (THREAD_ENTRY *thread_p, FILE *outfp, const INDEX_OVFP_INFO *head_ptr)
    {
      INDEX_OVFP_INFO *pt;
...
#define NUM_ATTRS_CNT (5)
  ...
  public:
	...
	{
	  m_cOvfpThreshold[worker_idx].set_worker_idx (worker_idx, &m_cOvfpLock);
	}
    }
    ...
    void
    add_read_pages_count (THREAD_ENTRY *thread_p, int worker_idx, BTID *btid, int npages)
    {
      OID class_oid;
	...
	{
	  m_cOvfpThreshold[worker_idx].add_info (btid, &class_oid, npages);
	}
    }

    void
    dump (THREAD_ENTRY *thread_p, FILE *outfp)
    {
      INDEX_OVFP_INFO *pt;
      COvfpPrinter cPrinter;
	...
	{
	  m_cOvfpLock.Lock (i, LOCK_ALL_OWNER_ID);

	  m_cOvfpThreshold[i].chcek_over_threshold_times (&over_tm);

	  pt = m_cOvfpThreshold[i].get_head();
	  while (pt)
	    {
	      cPrinter.add_info (pt);
	      pt = pt->next;
	    }

	  m_cOvfpLock.UnLock (i, LOCK_ALL_OWNER_ID);
	}

      // sort by recent time
      cPrinter.sort();
      print (thread_p, outfp, cPrinter.get_head());
    }
};
