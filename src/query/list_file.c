...
{
  if (list_id_p && list_id_p->last_pgptr)
    {
...
qfile_allocate_new_page (THREAD_ENTRY * thread_p, QFILE_LIST_ID * list_id_p, PAGE_PTR page_p, bool is_ovf_page)
{
  ...
  if (page_p)
    {
      qmgr_set_dirty_page (thread_p, page_p, FREE);
    }
  else
    {
			     ...
			     int tuple_length, int offset, int *tuple_page_size_p)
{
  ...
  *tuple_page_size_p = MIN (tuple_length - offset, qfile_Max_tuple_page_size);
  ...
  if (prev_page_p != page_p)
    {
      qmgr_set_dirty_page (thread_p, prev_page_p, FREE);
    }
...
qfile_add_tuple_to_list (THREAD_ENTRY * thread_p, QFILE_LIST_ID * list_id_p, QFILE_TUPLE tuple)
{
  ...
  qmgr_set_dirty_page (thread_p, cur_page_p, DONT_FREE);
				  ...
				  QFILE_LIST_ID * input_list_id_p)
{
  ...
  for (offset = tuple_page_size; offset < tuple_length; offset += tuple_page_size)
      ...
      if (ovf_page_p == NULL)
	{
	  if (new_page_p)
	    {
	      qmgr_set_dirty_page (thread_p, new_page_p, FREE);
	    }
	  return ER_FAILED;
	}
...
qfile_append_list (THREAD_ENTRY * thread_p, QFILE_LIST_ID * base_list_id, QFILE_LIST_ID * append_list_id)
{
  ...
  while (!VPID_ISNULL (&old_vpid))
      ...
      if (prev_page != NULL)
	{
	  ...
	  qmgr_set_dirty_page (thread_p, prev_page, FREE);
	}
      else
	{
      ...
      while (!VPID_ISNULL (&old_overflow_vpid))
	{
	  ...
	  if (prev_page != prev_overflow_page)
	    {
	      qmgr_set_dirty_page (thread_p, prev_overflow_page, FREE);
	    }
	  ...
	  QMGR_FREE_OLD_PAGE_AND_INIT (thread_p, old_overflow_page);
	}
      ...
      if (new_overflow_page != NULL)
	{
	  ...
	  qmgr_set_dirty_page (thread_p, new_overflow_page, FREE);
	  new_overflow_page = NULL;
	}

      /* next page */
      QFILE_GET_NEXT_VPID (&old_vpid, old_page);
      QMGR_FREE_OLD_PAGE_AND_INIT (thread_p, old_page);
    }

  if (new_page != NULL)
    {
      qmgr_set_dirty_page (thread_p, new_page, FREE);
    }
  ...
  if (prev_page != NULL)
    {
      ...
      QMGR_FREE_OLD_PAGE_AND_INIT (thread_p, prev_page);
    }

  if (new_page != NULL)
    {
      QMGR_FREE_OLD_PAGE_AND_INIT (thread_p, new_page);
    }

  if (prev_overflow_page != NULL)
    {
      new_overflow_page = (prev_overflow_page != new_overflow_page) ? new_overflow_page : NULL;
      QMGR_FREE_OLD_PAGE_AND_INIT (thread_p, prev_overflow_page);
    }

  if (new_overflow_page != NULL)
    {
      QMGR_FREE_OLD_PAGE_AND_INIT (thread_p, new_overflow_page);
    }
...
qfile_connect_list (THREAD_ENTRY * thread_p, QFILE_LIST_ID * base_list_id, QFILE_LIST_ID * append_list_id)
{
  ...
  qmgr_set_dirty_page (thread_p, base_last_page, FREE);

  QFILE_PUT_PREV_VPID (append_first_page, &base_list_id->last_vpid);
  qmgr_set_dirty_page (thread_p, append_first_page, FREE);
...
error_exit:
  if (base_last_page != NULL)
    {
      QMGR_FREE_OLD_PAGE_AND_INIT (thread_p, base_last_page);
    }

  if (append_first_page != NULL)
    {
      QMGR_FREE_OLD_PAGE_AND_INIT (thread_p, append_first_page);
    }
...
#if 1				/* not SortCache */
	  QMGR_FREE_OLD_PAGE_RO_AND_INIT (thread_p, page_p);
#endif /* not SortCache */
	}
      else
	{
  ...
  *new_last_vpid_p = *new_first_vpid_p;
  ...
  while (true)
      ...
      while (!VPID_ISNULL (&old_ovfl_vpid))
	{
      ...
      if (new_ovfl_page_p)
	{
	  qmgr_set_dirty_page (thread_p, new_ovfl_page_p, FREE);
	}
			      ...
			      QFILE_TUPLE tuple_p, int col_num, DB_VALUE * value_p, TP_DOMAIN * domain_p)
{
  ...
  if (QFILE_GET_OVERFLOW_PAGE_ID (page_p) == NULL_PAGEID)
      ...
      else
	{
      ...
      qmgr_set_dirty_page (thread_p, page_p, DONT_FREE);
    }
  else
    {
