...
struct qfile_list_cache_candidate
{
  ...
  int selcnt;
  QFILE_LIST_CACHE_ENTRY **time_candidates;	/* candiates who are old aged */
  QFILE_LIST_CACHE_ENTRY **ref_candidates;	/* candidates who are recently used */
  QFILE_LIST_CACHE_ENTRY **victims;	/* vitims; cache entries to be deleted */
  int c_idx;
  int v_idx;
  bool include_in_use;
};
...
#if defined(SERVER_MODE)
static MUTEX_T qfile_Free_sort_list_mutex = MUTEX_INITIALIZER;
#endif /* SERVER_MODE */
  ...
  if (qfile_Free_sort_list_count < PRM_MAX_THREADS)
    {
      ...
      MUTEX_UNLOCK (qfile_Free_sort_list_mutex);
    }
  else
    {
  ...
  if (length1 == 0)
  ...
  else
    {
      or_init (&buf, (char *) tuple1 + QFILE_TUPLE_VALUE_HEADER_SIZE,
	       length1);
      rc =
	(*(pr_type_p->readval)) (&buf, &dbval1, domain_p, -1, is_copy, NULL,
				 0);
      if (rc != NO_ERROR)
	{
	  return ER_FAILED;
  ...
  if (length2 == 0)
  ...
  else
    {
      or_init (&buf, (char *) tuple2 + QFILE_TUPLE_VALUE_HEADER_SIZE,
	       length2);
      rc =
	(*(pr_type_p->readval)) (&buf, &dbval2, domain_p, -1, is_copy, NULL,
				 0);
      if (rc != NO_ERROR)
	{
	  return ER_FAILED;
 ...
 *   tpl(in): tuple
 * Note: Each tuple start is aligned with MAX_ALIGN
		 ...
		 SORT_LIST * sort_list_p, int query_id, int flag)
{
  ...
  if (QFILE_IS_FLAG_SET (flag, QFILE_FLAG_RESULT_FILE)
    ...
    {
      list_id_p->tfile_vfid = qmgr_create_new_temp_file (thread_p, query_id);
    }
...
qfile_close_list (THREAD_ENTRY * thread_p, QFILE_LIST_ID * list_id_p)
{
  ...
  if (list_id_p->last_pgptr)
    {
      qmgr_free_old_page (thread_p, list_id_p->last_pgptr,
				  ...
				  QFILE_LIST_ID * input_list_id_p)
{
  ...
  for (offset = tuple_page_size; offset < tuple_length;
      ...
      if (new_page_p == NULL)
	{
      ...
      qmgr_free_old_page (thread_p, ovf_page_p, input_list_id_p->tfile_vfid);
      prev_page_p = new_page_p;
    }
				  ...
				  QMGR_WAIT_ARGS * arg_p)
{
  ...
  qmgr_free_old_page (thread_p, page_p, arg_p->tfile_vfidp);
  ...
  if (tuple_length < QFILE_MAX_TUPLE_SIZE_IN_PAGE)
    {
	  ...
	  (thread_p, list_id_p, T_SINGLE_BOUND_ITEM) != NO_ERROR)
	{
	  return ER_FAILED;
	}
    }
	       ...
	       QFILE_LIST_ID * side_file_p, int *have_side_p)
{
  ...
  scan_result =
    (*advance_func) (thread_p, scan_p, side_p, last_scan_p, last_side_p,
		     &side_file_p->type_list);
  switch (scan_result)
    {
    case S_SUCCESS:
      *have_side_p = 1;
			...
			QFILE_LIST_ID * rhs_file_p, int flag)
{
  ...
  if (rhs_file_p)
    {
      rhs_file_p =
	qfile_sort_list (thread_p, rhs_file_p, NULL, distinct_or_all);
      if (rhs_file_p == NULL)
	{
	  goto error;
	}
  ...
  while (1)
	...
	{
	  if (qfile_advance
	      (thread_p, advance_func, &lhs, &last_lhs, lhs_scan_p,
	...
	{
	  if (qfile_advance
	      (thread_p, advance_func, &rhs, &last_rhs, rhs_scan_p,
  ...
  for (key_p = (SORT_REC *) recdes_p->data; key_p && error == NO_ERROR;
      ...
      if (sort_info_p->key_info.use_original)
	{
	  ...
	  if (VPID_EQ (&(sort_info_p->fixed_vpid), &vpid))
		...
		{
		  qmgr_free_old_page (thread_p, sort_info_p->fixed_page,
				      list_id_p->tfile_vfid);
				...
				QFILE_TUPLE_VALUE_TYPE_LIST * types)
{
  ...
  if (list_p)
      ...
      for (i = 0, p = list_p; p; i++, p = p->next)
	{
	  ...
	  subkey->sort_f = p->pos_descr.dom->type->cmpdisk;
      ...
      for (i = 0; i < n; i++)
	{
	  ...
	  subkey->sort_f = types->domp[i]->type->cmpdisk;
	  subkey->is_desc = 0;
	}
    }
		       ...
		       QMGR_TEMP_FILE * new_tfile_vfid_p)
{
  ...
  if (new_page_p == NULL)
    {
      qmgr_free_old_page (thread_p, old_page_p, old_tfile_vfid_p);
      return ER_FAILED;
    }
  ...
  while (true)
      ...
      while (!VPID_ISNULL (&old_ovfl_vpid))
	{
	  ...
	  if (new_ovfl_page_p == NULL)
	    {
	      qmgr_free_old_page (thread_p, old_ovfl_page_p,
				  old_tfile_vfid_p);
	      qmgr_free_old_page (thread_p, old_page_p, old_tfile_vfid_p);
	      qfile_set_dirty_page (thread_p, new_page_p, FREE,
				    new_tfile_vfid_p);
	  ...
	  qmgr_free_old_page (thread_p, old_ovfl_page_p, old_tfile_vfid_p);
      ...
      if (scan_id_p->position == S_ON)
	{
	  if (scan_id_p->curr_vpid.pageid != tuple_position_p->vpid.pageid
	      ...
	      if (page_p == NULL)
		{
	      ...
	      qmgr_free_old_page (thread_p, scan_id_p->curr_pgptr,
				  scan_id_p->list_id.tfile_vfid);
...
qfile_initialize_list_cache (THREAD_ENTRY * thread_p)
{
  ...
  qfile_List_cache_entry_pool.pool =
    (QFILE_POOLED_LIST_CACHE_ENTRY *) calloc (qfile_List_cache_entry_pool.
					      n_entries,
					      sizeof
