...
xvacuum (THREAD_ENTRY * thread_p)
{
  ...
  while (cursor.is_valid ())
      ...
      if (!cursor.get_current_entry ().is_available ())
	{
	  ...
	  || !cursor.is_valid ()	// cursor is at the end; force update vacuum data because it
	  // might generate another block
	)
	{
 ...
 */
  ...
  void vacuum_stop (THREAD_ENTRY * thread_p)
  {
    if (!vacuum_Is_booted)
      {
	// not booted
	...
	if (ptype != PAGE_HEAP)
	  {
	    ...
	    vacuum_er_log_warning (VACUUM_ER_LOG_HEAP,
				   "Heap page %d|%d was deallocated during previous run and reused as"
	...
	if (VFID_ISNULL (&helper->overflow_vfid))
	  {
		...
		if (heap_ovf_find_vfid
		    (thread_p, &helper->hfid, &helper->overflow_vfid, false, PGBUF_UNCONDITIONAL_LATCH) == NULL
 ...
 */
  static int vacuum_heap_record_insid_and_prev_version (THREAD_ENTRY * thread_p, VACUUM_HEAP_HELPER * helper)
  {
	...
	repid_and_flag_bits &=
	  ~((OR_MVCC_FLAG_VALID_INSID | OR_MVCC_FLAG_VALID_PREV_VERSION) << OR_MVCC_FLAG_SHIFT_BITS);
	...
	vacuum_log_vacuum_heap_page (thread_p, helper->forward_page, 1, &helper->forward_oid.slotid,
				     &helper->can_vacuum, helper->reusable, false);
	...
	if (error_code != NO_ERROR)
	  {
	    ...
	    vacuum_er_log_error (VACUUM_ER_LOG_HEAP,
				 ...
				 (int) MVCC_GET_REPID (&helper->mvcc_header), MVCC_GET_CHN (&helper->mvcc_header),
				 MVCC_GET_INSID (&helper->mvcc_header), MVCC_GET_DELID (&helper->mvcc_header),
 ...
 */
  void
    vacuum_produce_log_block_data (THREAD_ENTRY * thread_p, LOG_LSA * start_lsa, MVCCID oldest_mvccid,
				   MVCCID newest_mvccid)
  {
    ...
    vacuum_er_log (VACUUM_ER_LOG_LOGGING | VACUUM_ER_LOG_VACUUM_DATA,
		   ...
		   (unsigned long long int) block_data.oldest_mvccid,
	...
	vacuum_er_log_error (VACUUM_ER_LOG_ERROR, "%s",
 ...
 */
  static int vacuum_process_log_block (THREAD_ENTRY * thread_p, VACUUM_DATA_ENTRY * data, bool sa_mode_partial_block)
  {
    ...
    assert (!LSA_ISNULL (&data->start_lsa)
	    && (data->get_blockid () == vacuum_get_log_blockid (data->start_lsa.pageid)));
	...
	if (LOG_IS_MVCC_HEAP_OPERATION (log_record_data.rcvindex))
	...
	else if (LOG_IS_MVCC_BTREE_OPERATION (log_record_data.rcvindex))
	  {
		...
		COPY_OID (&class_oid, &old_version.class_oid);
	      ...
	      {
		btree_rv_read_keybuf_nocopy (thread_p, undo_data, undo_data_size, &btid_int, &class_oid, &oid,
					     &mvcc_info, &key_buf);
	      }
		...
		vacuum_er_log (VACUUM_ER_LOG_BTREE | VACUUM_ER_LOG_WORKER,
			       ...
			       (unsigned long long int) mvccid, LSA_AS_ARGS (&rcv_lsa));
		error_code =
		  btree_vacuum_insert_mvccid (thread_p, btid_int.sys_btid, &key_buf, &oid, &class_oid, mvccid);
	      }
	    else
	      {
		/* Unexpected. */
 ...
 */
  static void vacuum_finished_block_vacuum (THREAD_ENTRY * thread_p, VACUUM_DATA_ENTRY * data, bool is_vacuum_complete)
  {
    VACUUM_LOG_BLOCKID blockid;
 ...
 */
  static void
    vacuum_read_advance_when_doesnt_fit (THREAD_ENTRY * thread_entry, size_t size, LOG_LSA * log_lsa,
					 LOG_PAGE * log_page)
  {
 ...
 */
  static int
    vacuum_process_log_record (THREAD_ENTRY * thread_p, VACUUM_WORKER * worker, LOG_LSA * log_lsa_p,
			       LOG_PAGE * log_page_p, LOG_DATA * log_record_data, MVCCID * mvccid, char **undo_data_ptr,
			       int *undo_data_size, LOG_VACUUM_INFO * vacuum_info, bool * is_file_dropped,
	...
	for (next_blockid = vacuum_Data.get_last_blockid () + 1; next_blockid <= consumed_data.blockid; next_blockid++)
	  {
		...
		error_code =
		  file_alloc (thread_p, &vacuum_Data.vacuum_data_file, file_init_page_type, &ptype, &next_vpid,
			      (PAGE_PTR *) (&data_page));
		if (error_code != NO_ERROR)
		  {
 ...
 */
  static void vacuum_update_keep_from_log_pageid (THREAD_ENTRY * thread_p)
  {
	...
	if (last_mvcc_lsa.is_null ()
	    || vacuum_get_log_blockid (last_mvcc_lsa.pageid) <= vacuum_Data.get_last_blockid ())
	  {
 ...
 */
  static int vacuum_add_dropped_file (THREAD_ENTRY * thread_p, VFID * vfid, MVCCID mvccid)
  {
	...
	/* binary search */
	position =
	  util_bsearch (vfid, page->dropped_files, page_count, sizeof (VACUUM_DROPPED_FILE),
			vacuum_compare_dropped_files, &found);
	...
	if (found)
	  {
	    ...
	    vacuum_er_log (VACUUM_ER_LOG_DROPPED_FILES,
			   ...
			   "Page count=%d, global count=%d", VFID_AS_ARGS (&page->dropped_files[position].vfid),
			   position, (unsigned long long int) save_mvccid,
 ...
 */
  void
    vacuum_log_add_dropped_file (THREAD_ENTRY * thread_p, const VFID * vfid, const OID * class_oid,
 ...
 */
  int vacuum_rv_set_next_page_dropped_files (THREAD_ENTRY * thread_p, LOG_RCV * rcv)
  {
    ...
    vacuum_er_log (VACUUM_ER_LOG_RECOVERY, "Set link for dropped files from page %d|%d to page %d|%d.",
		   pgbuf_get_vpid_ptr (rcv->pgptr)->pageid, pgbuf_get_vpid_ptr (rcv->pgptr)->volid,
	...
	for (i = data_page->index_unvacuumed; i < data_page->index_free; i++)
	  {
	    ...
	    assert (last_unvacuumed == NULL
		    || !MVCC_ID_PRECEDES (entry->oldest_mvccid, last_unvacuumed->oldest_mvccid));
 ...
 *
 */
  static int vacuum_fetch_log_page (THREAD_ENTRY * thread_p, LOG_PAGEID log_pageid, LOG_PAGE * log_page_p)
  {
    int error = NO_ERROR;
 ...
 */
  DISK_ISVALID
    vacuum_check_not_vacuumed_rec_header (THREAD_ENTRY * thread_p, OID * oid, OID * class_oid,
 ...
 */
  MVCCID vacuum_get_global_oldest_active_mvccid (void)
  {
...
//
					     ...
					     VACUUM_LOG_BLOCKID blockid)
  {
    ...
    log_append_redo_data2 (thread_p, RVVAC_DATA_INIT_NEW_PAGE, NULL, (PAGE_PTR) data_page, 0, sizeof (blockid),
			   &blockid);
    vacuum_set_dirty_data_page (thread_p, data_page, DONT_FREE);
  }
