...
vacuum_heap_record_insid_and_prev_version (THREAD_ENTRY * thread_p, VACUUM_HEAP_HELPER * helper)
{
  ...
  switch (helper->record_type)
    ...
    case REC_RELOCATION:
      ...
      existing_data_p = start_p + mvcc_header_size_lookup[mvcc_flags & 0x7];
...
vacuum_rv_redo_vacuum_heap_page (THREAD_ENTRY * thread_p, LOG_RCV * rcv)
{
  ...
  for (i = 0; i < n_slots; i++)
      ...
      else
	{
	  ...
	  old_header_size = mvcc_header_size_lookup[MVCC_GET_FLAG (&rec_header) & 0x7];
	  /* Clear insert MVCCID. */
	  ...
	  new_header_size = mvcc_header_size_lookup[MVCC_GET_FLAG (&rec_header) & 0x7];
