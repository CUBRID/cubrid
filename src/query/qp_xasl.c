 ...
 *
 * qp_xasl.c - Query evaluator module
 *
...
/* This module is responsible for the evaluation of queries represented by
 * XASL trees and generation of query results in the form of LIST FILEs. The
 * module basically consists of an XASL tree interpreter which makes use of
 * other group of routines consisting of scanning routines, predicate
 * evaluation routines, value fetch routines and various utility routines
 * to interpret XASL trees and generate query result list files.
 *
 * The query evaluator evaluates the queries submitted by the query manager
 * in the form of XASL trees basically by interacting with the query file
 * manager and returns the query result file identifiers back to the query
 * manager. The interaction of query evaluator with other modules can be
 ...
 *           ---------------------    ------------------
 *
 * The main data structure used by the XASL interpreter is the XASL tree
 * structure itself which is used to represent SQL/X queries. XASL trees are
 * created on the client side by query processor module. An XASL tree
 * consists of a group of connected XASL tree nodes/blocks. Each XASL tree
 * block represents a set of actions to be taken by the interpreter while
 * interpreting that node. Each XASL tree block consists of a head node and a
 * body node. The head node defines the type of the block and also contains
 * pointers to information that represents the result of interpretation of the
 * body node which is usually a list file identifier. The body node content
 * may include a linked list of access specification nodes which defines where
 * (heap files, list files) and how (sequentially, using indices) to get
 * information to be processed, a set of pointers to other XASL tree blocks
 * which defines related queries such as corrrelated subqueries, uncorrelated
 * subqueries, join queries and a set of nodes which define the post
 * processing operations on the result of the interpretation of the body node.
 *
 * Scan, Predicate Evaluation and Value Fetch routines basically use the nodes
 * inside the XASL trees.
 *
 * There are three main data structures used by the utility routines:
 *
 *   (i) Catalog Directory: Catalog directory is a linked list structure,
 * each node of which contains relevant class information from catalog.
 * Catalog Directory is used for efficient access to Catalog. During query
 * evalution, necessary catalog information for each class is fetched from the
 * catalog only once and stored in the catalog directory, thus eliminating
 * further unnecessary accesses to the Catalog for the same class information.
 * The nodes of the catalog directory are directly pointed by the XASL tree
 * pointers. Catalog Directory contains information only for the most recent
 * representation of the class. Thus, when required class information is not
 * found in the Catalog Directory, the Catalog itself is consulted.
 *
 *  (ii) Memory Buffer: Memory Buffer structure is a linked list of memory
 * buffers of a fixed size which is used for main memory allocation to avoid
 * expensive Malloc calls and memory fragmentation. The main extension of the
 * memory buffer, relative to a regular memory buffer, is that it is possible
 * iteratively, in a nested manner, to define and use regions of memory buffer
 * This conforms well to the iterative interpretion of XASL nodes.
 *
 * (iii) Type Function Vector Structures: For each data type used during query
 * processing, a functional vector is defined to include a set of functions
 * that can be used in the context of that function.
 *
 * The XASL tree interpreter consists of a set of routines which processes
 * each different XASL tree block. XASL tree blocks are processed in an
 * defined order. Since each XASL tree block may point to other XASL tree
 * blocks, interpreting a specific XASL tree block may involve other XASL
 * tree block interpretations. An XASL tree block interpretation may generate
 * a specific result for the block, such as a list file, a constant value
 * and/or fill in some other nodes of the XASL tree with information that will
 * be processed later on. Thus, the order of XASL tree interpretation
 * quarantees that the necessary information is there in an XASL tree node by
 * the time it is needed.
 *
 * The following pseudo code shows the basic steps of interpreting an XASL
 * tree procedure block:
 *
 * start main block interpretation
 * evaluate uncorrelated subquery blocks
 * open a scan to cover main block access items
 * for each qualifying main block scan item do {
 *     fetch values from scan item
 *     evaluate path expression value fetch blocks
 *     if (path expression fetch inqualifies)
 *         continue
 *     if (no scan(join) procedure block) {
 *         evaluate correlated subquery blocks
 *         evaluate overall (if) predicate
 *         end one scan iteration
 *     } else {
 *         initialize scan(join) on the scan block
 *         for each qualifying scan block item do {
 *             evaluate correlated subquery blocks
 *             evaluate overall (if) predicate
 *             end one scan iteration
 *         }
 *    }
 * }
 * end main block interpretation
 *
 * For "selection" and "projection" SQL queries [DATE81], the interpretation
 * started by opening a LIST FILE to put the query results and initializing
 * a scan mechanism to access each element in a set of elements represented
 * by the scan, typically each object instance in a heap file or each tuple
 * in a LIST FILE. Accessing scan items can be done in a sequential manner
 * or indices may be used to access scan items randomly. The selection
 * predicate, if any, is applied to each scan item, i.e. heap file object
 * instance or LIST FILE tuple, to determine whether the item is qualified or
 * not. The projection values are then extracted from qualifying scan items.
 * After each iteration, a new tuple is formed and added to the query result
 * LIST FILE. The main block interpretation end operation for these type of
 * queries involve closing the query result LIST FILE. If indicated by the
 * represented SQL query, the result LIST FILE may be sorted on certain
 * columns, and/or partitioned into groups of rows and/or duplicate tuples
 * may be eliminated from the LIST FILE. Duplication elimination from the
 * query result FILE is done by first sorting the LIST FILE on all columns and
 * eliminating duplicate tuples on the fly with a second pass. If query
 * involves correlated or uncorrelated subqueries, these are evaluated in the
 * order shown in the above pseudo code and their results are used during that
 * XASL tree block interpretation. Path expressions [KIM91] form another
 * mechanism to fetch values at every scan iteration, by directly accessing a
 * set of objects as indicated by the path and fetching values from these
 * objects. A path expression may be dead ended, that is, may not result in
 * any final reference to an object. In this case, the current scan item is
 * determined to be qualified and the value list is set to NULLs.
 *
 * For "aggregation" SQL queries [DATE81], the main block interpretation is
 * started by initializing the aggregation function results stored in
 * specific XASL nodes and initializing a scan mechanism to access each
 * element in a set of elements represented by the scan, a heap file or a
 * LIST FILE. Accessing scan items are done in a sequential manner. At each
 * iteration, after nesessary values are fetched from qualifying scan items
 * as determined by the "selection" predicate if any, aggregation functions
 * are recomputed to include the contributions of that iteration. During main
 * block interpretation end operation, a final evaluation is made on the
 * aggregation function results.
 *
 * For "join" queries [DATE81], as seen in the above pseudo code, the basic
 * join algorithm utilized by the query evaluator is nested-loop join
 * algorithm [SELI79] and it also takes advantage of indices when available.
 *
 * For "union" queries [DATE81], first the two union queries are executed
 * and the query result LIST FILEs from these two queries are merged to
 * form the resultant LIST FILE for the query. By default, any redundant
 * duplicate tuples from the result LIST FILE are eliminated[ISO89]. If
 * explicitly indicated in the query, the duplicate tuples are kept in the
 * result. By default, there is no ordering imposed to the resultant LIST
 * FILE. However, if explicitly indicated in the query, the union query result
 * LIST FILE is further processed to generate a sorted result file.
 *
 ...
 *
 * Scan management routines use vale fetch routines to fetch values either
 ...
 *
 * Predicate expressions are represented in the form of a predicate
 * expression tree and evaluated against a heap file object instance, a
 * list file tuple or a constant predicate expression tree. The module
 * uses a 3-valued logic with true, false and UNKNOWN values. The
 * non_leaf nodes of the predicate expression tree are evaluated applying
 * a 3-valued logic to the results coming from the subtrees. The
 * leaf_nodes of the predicate expression tree are evaluated considering
 * the type of the predicate involved and the value content of the
 * nodes. The leaf node predicate expression types can be categorized to
 * three groups:
 *
 ...
 * NULL predicate is a unary predicate whic return true for a bound value
 ...
 * Utility Routines:
 *
 * The utility routines provided are used by the XASL tree interpreter during
 * XASL tree interpretation. Utility routines include the following category
 * of routines:
 *
 * Catalog access routines: used to contact Catalog Manager and receive
 * information which is used to interpret heap file object instance disk
 * representation to fetch values from object instances. The information
 * stored in XASL tree nodes, basically attribute and class identifiers are
 * used to fetch this information from the catalog.
 *
 * Memory buffer routines: used to create memory buffers and allocate space
 * from these buffers for effective main memory utilization.
 *
 * Type management routines: used to provide function vectors for each data
 * type which includes a set of functions which take specific actions
 * depending on the data type in the query processor contex. Each function
 * vector for each data type currently includes functions to get disk
 * representation interpretation information for the type, to make comparison
 * of disk values of the type, to sort disk values of the type, to print disk
 * values of the type and to get some disk representation size information for
 * the type.
 *
 * Value Transformation Routines: used to transform values from one
 * representation to another or upgrade a value with respect to a new
 * compatible data type.
 *
 * Arithmetic Expression Evalution Routines: used to evaluate arithmetic
 * expressions represented in XASL tree nodes.
 *
 * Aggregate Function Evalution Routines: used to evaluate aggregate function
 * expressions represented in XASL tree nodes.
 *
 * The callers of the module include Query Manager.
 *
 * The callees of the module include Query File Manager, B+tree Index Manager,
 * Catalog Manager, Heap File Manager, Sorting Module and Scheduler.
 *
 * References:
 *
 * [DATE81] Date, C.J., An Introduction to Database Systems (3rd ed),
 * Addison-Wesley, Reading, Mass., 1981
 *
 * [ISO89]  Information processing systems - Database Language SQL with
 * integrity enhancements (2nd ed), ISO/IEC 9075,
 *
 * [LORI79] Lorie, A.R., and Nilsson, J.F., "An Access Specification Language
 * for a Relational Data Base System", IBM J. Res. Develop., Vol.23, No. 3,
 * May 1979
 *
 * [SELI79] Selinger P.G., et al., "Access Path Selection in a Relational
 * Database Management System", Proc. Inter. Conf. Management of Data (ACM),
 * Boston, May 1979.
 */
...
#define GOTO_EXIT_ON_ERROR do { qexec_failure_line(__LINE__, xasl_state); goto exit_on_error; } while (0)
...
#define QPROC_MAX_GROUPED_SCAN_CNT       4	/* Keep the scans grouped only for
						   that many classes scans for join
						   queries.
						 */

#define QPROC_SINGLE_CLASS_GROUPED_SCAN  0	/* if 1, single class query scans
						   are done in a grouped manner.
						 */

#define QPROC_MULTI_CLASS_GROUPED_SCAN   1	/* if 1, multi class query scans
						   are done in a grouped manner.
						 */

#define QEXEC_CLEAR_AGG_LIST_VALUE(agg_list) do { \
 ...
 * text in order to improve the readibilty.
 */
...
static int qexec_open_scan (THREAD_ENTRY * thread_p,
			    ...
			    int grouped, int keep_iscan_order,
			    SCAN_ID * s_id);
  ...
  if (xasl->instnum_pred)
    {
      /* evaluate predicate */
      ev_res =
	eval_pred (thread_p, xasl->instnum_pred, &xasl_state->vd, NULL);
      switch (ev_res)
	{
	case V_FALSE:
			...
			QUERY_OPTIONS option, XASL_STATE * xasl_state)
{
  ...
  if (order_list != NULL || option == Q_DISTINCT)
	...
	{
	  ls_flag =
	    (option == Q_DISTINCT) ? QFILE_FLAG_DISTINCT : QFILE_FLAG_ALL;
	  ...
	  list_id =
	    qfile_sort_list_with_func (thread_p, list_id, orderby_list,
				       option, ls_flag, NULL, put_fn, NULL,
				       &ordby_info);
	  if (list_id == NULL)
	    {
  ...
  if (gbstate->grbynum_pred)
    {
      /* evaluate predicate */
      ev_res =
	eval_pred (thread_p, gbstate->grbynum_pred, &gbstate->xasl_state->vd,
		   NULL);
      switch (ev_res)
	{
	case V_FALSE:
	       ...
	       XASL_STATE * xasl_state)
{
   ...
   */
  {
	...
	(buildlist->g_outptr_list, &output_type_list) != NO_ERROR)
      {
	GOTO_EXIT_ON_ERROR;
      }
  ...
  gbstate.cmp_fn
    = (gbstate.key_info.use_original == 1
			...
			XASL_STATE * xasl_state, int ls_flag)
{
  ...
  /* Notice:
     If 'sid->s.llsid.tplrecp' is not NULL pointer,
     'scan_next_scan(scan_idp)' will set the pointer to tuple record fetched.
     When called 'scan_next_scan(sid)' with setting
     'sid->qualification' to QPROC_QUALIFIED_OR_NOT,
     'sid->qualification' will be one of
     followings.
     QPROC_QUALIFIED if there's no access spec or it is a qualified tuple
     QPROC_NOT_QUALIFIED if it is a not-qualified tuple (with access spec)
     QPROC_QUALIFIED_OR_NOT if it is NULL value (with access spec) */
...
qexec_open_scan (THREAD_ENTRY * thread_p, ACCESS_SPEC_TYPE * curr_spec,
		 ...
		 int fixed, int grouped, int keep_iscan_order, SCAN_ID * s_id)
{
  SCAN_TYPE scan_type;
  INDX_INFO *indx_info;
 ...
 *   xasl(in)   : XASL Tree pointer
 *
 * Note: This function is used to generate next combination of scan
 * blocks for a list of scans on a list of XASL tree scan blocks.
 * It uses a nested loop algorithm among the scan groups to get
 * iteratively all the scan blocks involved. Thus, for example,
 * if we have following scan groups for two XASL tree blocks:
 *
 *                          A   1
 *                          B   2
 *                              3
 *
 * the routine will return successively the following scan block combinations:
 *
 *                          A   1
 *                          A   2
 *                          A   3
 ...
 *                          B   3
 *
 * Each scan block, such as A and B for the first scan, is
 * determined by the scan management routines depending on the
 * type of scans opened. Once a combination of scan blocks is
 * determined by this routine, the scan items for the scans
 * involved are accesses only within the boundaries of the scan blocks.
 *
 * If all the scan block combinations have been generated, the
 * routine returns S_END.
 */
static SCAN_CODE
		  ...
		  XASL_SCAN_FNC_PTR next_scan_fnc)
{
	  ...
	  for (agg_ptr = buildvalue->agg_list; agg_ptr;
	      ...
	      if (agg_ptr->flag_agg_optimize)
		{
		  ...
		  if (qdata_evaluate_aggregate_optimize
		      (thread_p, agg_ptr,
		       &xasl->spec_list->s.cls_node.hfid) != NO_ERROR)
		    {
		      agg_ptr->flag_agg_optimize = false;
		      flag_scan_needed = true;
		    }
  ...
  while ((xb_scan =
	      ...
	      scan_next_scan (thread_p, &xasl->curr_spec->s_id)) == S_SUCCESS)
	{
	  ...
	  for (xptr = xasl->bptr_list; qualified &&
	       xptr != NULL; xptr = xptr->next)
	    {
	      if (qexec_execute_obj_set_fetch (thread_p, xptr, xasl_state) !=
		  NO_ERROR)
		{
		  ...
		  if (qualified)
			...
			{	/* no scan procedure block */
			  ...
			  if (xasl->instnum_val)
				...
				{
				  /* at here, do not clear list files.
				   * for method scan, dptr_list and scan_proc list
				   * refer the same list file. so cause FMR/FMW.
				   *
				   * list files are cleared in qexec_execute_mainblock()
				   * when all the scans are closed.
				   *
				   * the following shows XASL tree structure
				   * which has method.
				   *
				   * buildlist_proc         scan_proc
				   * +------------+         +------------------+
				   * | scan_ptr --+-------->|                  |
				   * |            |         | access_spec_list |
				   * |            |         |                | |
			...
			{	/* handle the scan procedure */
			  ...
			  while ((xs_scan =
				  (*next_scan_fnc) (thread_p, xasl->scan_ptr,
						    xasl_state, tplrec,
						    next_scan_fnc + 1)) ==
				 S_SUCCESS)
			    {
  ...
  while ((ls_scan1 = qexec_next_scan_block (thread_p, xasl)) == S_SUCCESS)
      ...
      while ((ls_scan1 = scan_next_scan (thread_p, s_id1)) == S_SUCCESS)
	{
	  ...
	  if (qualified)
		...
		{
		  ev_res =
		    eval_pred (thread_p, xasl->if_pred, &xasl_state->vd,
			       NULL);
		  if (ev_res == V_ERROR)
		    {
		      GOTO_EXIT_ON_ERROR;
	...
	}			/* while ((ls_scan1 = scan_next_scan(s_id1)) == S_SUCCESS) */
      if (ls_scan1 == S_ERROR)
	{
	  GOTO_EXIT_ON_ERROR;
	}
 ...
 *   xasl_state(in)     :
 *
 * Note: This routine executes an update by executing the aptr
 * to find qualified instances and to retrieve the instance OID
 * and attribute expression values.  It then calls lc_put for
 * the OID with the attribute ID's, attribute expressions values
 * and constant values.
 */
static int
		      ...
		      XASL_STATE * xasl_state)
{
  ...
  for (specp = aptr->spec_list; specp; specp = specp->next)
	...
	{
	  if (lock_object
	      (thread_p, &(specp->s.cls_node.cls_oid), oid_Root_class_oid,
  ...
  while ((xb_scan =
      ...
      while ((ls_scan = scan_next_scan (thread_p, s_id)) == S_SUCCESS)
	{
	  ...
	  if (update->cons_pred != NULL)
		...
		eval_pred (thread_p, update->cons_pred, &xasl_state->vd,
			   NULL);
	  ...
	  if (!OID_EQ (&prev_class_oid, class_oid))
	      ...
	      if (!OID_ISNULL (&prev_class_oid) && attr_info_inited)
		{
		  ...
		  (thread_p, class_oid, -1, NULL, &attr_info) != NO_ERROR)
		{
		  GOTO_EXIT_ON_ERROR;
		}
	      attr_info_inited = 1;
	  ...
	  if (!HFID_IS_NULL (hfid))
		...
		{		/* partition adjust */
		  if (update->partition[cl_index]->key_attr == -1)
		    ...
		    {
		      retidx =
			qexec_partition_chg_select (thread_p, xasl_state,
						    &attr_info,
						    update->
						    partition[cl_index],
						    &rvsave);
 ...
 *   xasl_state(in)     :
 *
 * Note: This routine executes a delete by executing the aptr
 * to find qualified instances and to retrieve the instance OIDs.
 * It then calls lc_delete for the OID.
 */
static int
 ...
 *   xasl_state(in)     :
 *
 * Note: This routine executes an insert by executing the aptr
 * to build instance values.  It then calls lc_insert for the values.
 */
static int
      ...
      if (!dead_end)
	{
	  ...
	  if (heap_get_with_class_oid (thread_p, dbvaloid, &oRec, &scan_cache,
	      ...
	      if (er_errid () == ER_HEAP_UNKNOWN_OBJECT)
		{
	      ...
	      /* I added this clause to catch the condition where
	       * a set can contain instances of multiple classes but
	       * the scan was limited with the [classname] syntax.
	       * This will cause only the intsances of the specified
	       * class to be flushed.  There may however be other
	       * instances that were not flushed although they were
	       * assigned a permanent OID when the referencing object
	       * was flushed.  For these, the following error may be
	       * returned.  We can assume that in this case the data
	       * is not of interest and can be ignored.
	       */
	  ...
	  if (!dead_end)
	      ...
	      else
		{
		  ...
		  if (xasl->val_list != NULL)
		    {
		      status =
			fetch_val_list (thread_p, specp->s.cls_node.
					cls_regu_list_rest, &xasl_state->vd,
					NULL, dbvaloid, NULL, COPY);
		      if (status != NO_ERROR)
			{
			  goto wrapup;
			}
			   ...
			   XASL_STATE * xasl_state)
{
  ...
  for (selupd = list; selupd; selupd = selupd->next)
      ...
      for (outptr = selupd->select_list; outptr; outptr = outptr->next)
	{
	  ...
	  if (fetch_peek_dbval
	      (thread_p, varptr->value.arithptr->thirdptr, NULL, NULL, NULL,
	       NULL, &thirdvalp) != NO_ERROR)
	    {
	      goto exit_on_error;
	    }
	  ...
	  lock_ret =
	    lock_object (thread_p, oid, class_oid, X_LOCK, LK_UNCOND_LOCK);
	  switch (lock_ret)
	    {
	    case LK_GRANTED:
	      /* normal case */
	    ...
	    case LK_NOTGRANTED_DUE_TIMEOUT:
	      ...
	      er_log_debug (ARG_FILE_LINE,
			    "qexec_execute_selupd_list: lock(X_LOCK) timed out for OID { %d %d %d } class OID { %d %d %d }\n",
	    ...
	    default:
	      ...
	      er_log_debug (ARG_FILE_LINE,
			    "qexec_execute_selupd_list: skip for OID { %d %d %d } class OID { %d %d %d } lock_ret %d\n",
			    oid->pageid, oid->slotid, oid->volid,
			    class_oid->pageid, class_oid->slotid,
	...
	}			/* for (outptr = node->select_list; outptr; outptr = outptr->next) */
				  ...
				  XASL_STATE * xasl_state)
{
  ...
  switch (xasl->type)
	...
	/*
	 * open the result list file if not already open
	 * If this is a streaming query, the list file has already
	 * been completely setup AND returned to the client app so
	 * we can't create a whole new list. qexec_start_mainblock_iterations
	 * is called from xqmgr_execute_query and xqmgr_prepare_and_execute_query
	 * for streaming queries
	 * --------------------------------------------------
	 * xasl->outptr_list is the first select out list,
	 * if aggregation is applied we should use buildlist->g_outptr_list
	 * --------------------------------------------------
	 */
	if (xasl->list_id->type_list.type_cnt == 0)
	  {
	    ...
	    /* If BUILDLIST_PROC does not have
	       'group by'(buildlist->groupby_list)
	       and 'order by'(xasl->orderby_list), then the list file to be open
	       at here will be the last one.
	       Otherwise, the last list file will be open at qexec_groupby()
	       or qexec_orderby_distinct().
	       (Note that only one that can have 'group by' is BUILDLIST_PROC
	       type.)
	       And, the top most XASL is the other condition for the list file
	       to be the last result file. */
	    QFILE_SET_FLAG (ls_flag, QFILE_FLAG_ALL);
	    ...
	    t_list_id =
	      qfile_open_list (thread_p, &type_list, xasl->after_iscan_list,
			       xasl_state->query_id, ls_flag);
	    if (t_list_id == NULL)
	      {
		if (type_list.domp)
				      ...
				      XASL_STATE * xasl_state)
{
  ...
  if (buildvalue->having_pred != NULL)
    {
      ev_res =
	eval_pred (thread_p, buildvalue->having_pred, &xasl_state->vd, NULL);
      if (ev_res == V_ERROR)
	{
	  GOTO_EXIT_ON_ERROR;
	}
				...
				XASL_STATE * xasl_state)
{
  ...
  switch (xasl->type)
	   ...
	   XASL_IS_FLAGED (xasl, XASL_SKIP_ORDERBY_LIST)))
	{
      ...
      t_list_id =
	qfile_combine_two_list (thread_p, xasl->proc.union_.left->list_id,
				xasl->proc.union_.right->list_id, ls_flag);
      ...
      if (!t_list_id)
	{
      ...
      /*
       * Do NOT destroy the listfiles pointed at by
       * xasl->proc.union_.{left,right}->list_id.  Those are pointers to the
       * list_id fields of other XASL blocks, and those blocks may be
       * correlated to outer scopes.  The XASL blocks themselves are in
       * charge of destroying the listfiles at the appropriate time.
       * That is NOT the responsibility of this operator, and doing so
       * can in fact lead to wrong answers if a listfile is destroyed
 ...
 *
 * Note: This routines interpretes a main procedure node in the XASL
 * tree which can appear at the root level of an XASL tree to
 * represent a query and which can result in generation of a
 * LIST FILE or a single value. The main procedure block nodes
 * of this type are BUILDLIST_PROC, BUILDVALUE, UNION_PROC,
 * DIFFERENCE_PROC and INTERSECTION_PROC. The resultant list file
 * identifier is stored in the XASL node itself. The xasl_state
 * information contains a value descriptor which is used to store
 * embedded SQL/X positional values, if any, and a memory buffer
 * used for allocation of memory during the tree interpretation,
 * and a pointer list used to remember catalog information
 * related memory allocation areas. The main algorithm followed
 * by the routine is as follows:
 *
 *              start main block interpretation
 *              evaluate uncorrelated subquery blocks
 *              open a scan to cover main block access items
 ...
 *              end main block interpretation
 *
 * The routine first does a quick analysis of each XASL tree
 * block and determines a specific interpretation of that block.
 * It then calls necessary specific routines to perform the
 * specific interpretation of that block.
 */
int
			 ...
			 XASL_STATE * xasl_state)
{
  ...
  switch (xasl->type)
...
#endif
			  if (qexec_open_scan
			      (thread_p, specp, xptr->merge_val_list,
			       &xasl_state->vd, readonly_scan,
			       specp->fixed_scan, specp->grouped_scan,
			       iscan_order, &specp->s_id) != NO_ERROR)
			    {
			      GOTO_EXIT_ON_ERROR;
			    }
			}
      ...
      if (xasl->list_id->tuple_cnt == 0)
	{
      ...
      /*
       * An optimization that we should eventually undertake is to push
       * the orderby stuff into any UNION/DIFFERENCE/INTERSECT
       * processing that might take place (which now happens in
       * qexec_end_mainblock_iterations).  That stuff now uses sorted
       * files to accomplish the work, and it might as well sort on the
       * orderby criteria (augmented as necessary if the orderby list
       * doesn't cover the full rank of the result) so that the answer
       * is emitted in the right order.  That would save us an extra
       * sorting pass.  Groupby complicates things somewhat, but we can
       * certainly do this if there is no groupby clause.
       *
       * MRS 4/12/95
       */
      ...
      else
	{
	  ...
	  if ((xasl->orderby_list &&	/* it has ORDER BY clause */
	    ...
	    {
	      if ((qexec_orderby_distinct
		   (thread_p, xasl, xasl->option, xasl_state) != NO_ERROR))
		{
		  GOTO_EXIT_ON_ERROR;
		}
	    }
 ...
 * values (coming from ESQL/X programs positional values) which
      ...
      if (stat != NO_ERROR)
	{
	  ...
	  if (er_errid () == ER_QPROC_NOMORE_QFILE_PAGES &&
		  ...
		  QMGR_TRAN_RESUME_DUE_DEADLOCK)
		{
		  ...
		  /*
		   * This variable is used to detect external deadlocks involved
		   * among transactions in the TG and other no-TG transactions.
		   * We need to avoid the re-introducing the same deadlock when
		   ...
		   *    1 -> 2
		   *
		   * If 5 is selected as a victim to solve the deadlock, it is
		   * possible that we can reintroduce the same deadlock many times.
		   */
		}
...
#define ENT_V_RATIO 0.02	/* victim ratio such as 2% */
  ...
  return (xasl_ent_cache.qstr_ht && xasl_ent_cache.xid_ht
	  ...
	  && xasl_ent_cv->c_time && xasl_ent_cv->c_ref && xasl_ent_cv->victim)
    ? NO_ERROR : ER_FAILED;
}
			     ...
			     const OID * user_oid)
{
  ...
  if (ent)
      ...
      if (ent && !OID_EQ (&ent->creator_oid, user_oid))
	{
	  ...
	  difftime (time (NULL),
		    ent->time_created.tv_sec) > PRM_XASL_PLAN_CACHE_TIMEOUT)
	{
      ...
      if (ent)
	{
	  for (i = 0, oidp = ent->class_oid_list, rep_idp = ent->repr_id_list;
	    ...
	    {
	      if (catalog_get_last_representation_id
		  (thread_p, (OID *) oidp, &id) != NO_ERROR || id != *rep_idp)
		{
...
#endif
	  if (qexec_delete_xasl_cache_ent (thread_p, ent, NULL))
	    {
