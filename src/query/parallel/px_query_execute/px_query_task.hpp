...
namespace parallel_query_execute
{
  ...
  class task : public cubthread::entry_task
  {
    ...
    private:
      ...
      std::map<XASL_NODE *, int> *m_xasl_thread_executed_map_p;
  };
  ...
  class task_queue
  {
    public:
      ...
      task_tuple *add_task (THREAD_ENTRY *orig_thread_p, XASL_NODE *xasl, xasl_state *xasl_state, pthread_mutex_t *mutex_p,
			    std::vector<err_desc_t> *error_messages_p, std::map<XASL_NODE *, int> *xasl_thread_executed_map_p);
  ...
  class task_queue_global
  {
    public:
      ...
      void join();
      std::map<XASL_NODE *, int> m_xasl_thread_executed_map;
    private:
      std::vector<task_tuple *> m_tasks;
  };
}
