...
namespace parallel_query
{
  ...
  int worker_manager_with_dedicated_pool::try_reserve_workers (int num_workers, int task_max_count)
  {
    assert (num_workers > 0);
    assert (task_max_count > 0);
    ...
    m_worker_pool = cubthread::get_manager()->create_worker_pool (reserved /* pool_size */, task_max_count,
  ...
  void worker_manager_with_dedicated_pool::wait_workers ()
  {
    assert (m_reserved_workers > 0);
    while (m_active_tasks.load (std::memory_order_acquire) > 0)
      {
  ...
  int worker_manager_reserver::try_reserve_workers (int num_workers)
  {
    assert (num_workers > 0);
