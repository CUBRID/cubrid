...
qexec_orderby_distinct (THREAD_ENTRY * thread_p, XASL_NODE * xasl, QUERY_OPTIONS option, XASL_STATE * xasl_state)
{
  ...
  if (thread_is_on_trace (thread_p))
	...
	{
	  old_sort_pages = mnt_get_from_statistic (thread_p, PSTAT_SORT_NUM_DATA_PAGES);
	  old_sort_ioreads = mnt_get_from_statistic (thread_p, PSTAT_SORT_NUM_IO_PAGES);
	}
    }
  ...
  if (thread_is_on_trace (thread_p))
    {
      ...
      if (xasl->orderby_stats.orderby_filesort)
	{
	  xasl->orderby_stats.orderby_pages = mnt_get_from_statistic (thread_p, PSTAT_SORT_NUM_DATA_PAGES) - old_sort_pages;
	  xasl->orderby_stats.orderby_ioreads = mnt_get_from_statistic (thread_p, PSTAT_SORT_NUM_IO_PAGES) - old_sort_ioreads;
	}
    }
...
wrapup:
  {
	...
	if (xasl->groupby_stats.groupby_sort == true)
	  {
	    xasl->groupby_stats.groupby_pages = mnt_get_from_statistic (thread_p, PSTAT_SORT_NUM_DATA_PAGES) - old_sort_pages;
	    xasl->groupby_stats.groupby_ioreads =
	      mnt_get_from_statistic (thread_p, PSTAT_SORT_NUM_IO_PAGES) - old_sort_ioreads;
	  }
      }
			...
			bool has_delete)
{
  ...
  if (qexec_create_mvcc_reev_assignments (thread_p, aptr, has_delete, internal_classes, no_classes, no_assigns, assigns,
					  mvcc_reev_assigns) != NO_ERROR)
    {
      GOTO_EXIT_ON_ERROR;
    }
...
qexec_create_delete_lob_info (THREAD_ENTRY * thread_p, XASL_STATE * xasl_state, UPDDEL_CLASS_INFO_INTERNAL * class_info)
{
  ...
  if (heap_attrinfo_start (thread_p, class_info->class_oid, class_info->no_lob_attrs, class_info->lob_attr_ids,
  ...
  if (on_trace)
    {
      ...
      xasl->xasl_stats.fetches += mnt_get_from_statistic (thread_p, PSTAT_PB_NUM_FETCHES) - old_fetches;
      xasl->xasl_stats.ioreads += mnt_get_from_statistic (thread_p, PSTAT_PB_NUM_IOREADS) - old_ioreads;
    }
      ...
      if (xasl->spec_list)
	{
	  ...
	  if (XASL_IS_FLAGED (xasl, XASL_HAS_CONNECT_BY))
	      ...
	      if (on_trace)
		{
		  ...
		  xasl_stats->fetches = mnt_get_from_statistic (thread_p, PSTAT_PB_NUM_FETCHES) - old_fetches;
		  xasl_stats->ioreads = mnt_get_from_statistic (thread_p, PSTAT_PB_NUM_IOREADS) - old_ioreads;
		}
	    }
	}
...
qexec_initialize_xasl_cache (THREAD_ENTRY * thread_p)
{
  ...
  perfmon_set_stat (thread_p, 0, PSTAT_PC_NUM_QUERY_STRING_HASH_ENTRIES);
  ...
  perfmon_set_stat (thread_p, 0, PSTAT_PC_NUM_XASL_ID_HASH_ENTRIES);
  ...
  perfmon_set_stat (thread_p, 0, PSTAT_PC_NUM_CLASS_OID_HASH_ENTRIES);
			     ...
			     int dbval_cnt, bool is_pinned_reference)
{
  ...
  ent =
    qexec_alloc_xasl_cache_ent (XASL_CACHE_ENTRY_ALLOC_SIZE (sql_hash_text_len + sql_plan_text_len + sql_user_text_len,
							     n_oids));
  if (ent == NULL)
    {
      goto end;
  ...
  perfmon_set_stat (thread_p, mht_count (xasl_ent_cache.xid_ht), PSTAT_PC_NUM_XASL_ID_HASH_ENTRIES);
  ...
  perfmon_set_stat (thread_p, mht_count (xasl_ent_cache.oid_ht), PSTAT_PC_NUM_CLASS_OID_HASH_ENTRIES);
...
qexec_delete_xasl_cache_ent (THREAD_ENTRY * thread_p, void *data, void *args)
{
  ...
  if (ent->num_fixed_tran == 0 || force_delete)
      ...
      if (mht_rem2 (xasl_ent_cache.qstr_ht, &ent->qstr_ht_key, ent, NULL, NULL) != NO_ERROR)
	{
      ...
      perfmon_set_stat (thread_p, mht_count (xasl_ent_cache.qstr_ht), PSTAT_PC_NUM_QUERY_STRING_HASH_ENTRIES);
  ...
  if (orderby_list != NULL)
      ...
      else
	{
	  ...
	  if (thread_is_on_trace (thread_p))
		...
		mnt_get_from_statistic (thread_p, PSTAT_SORT_NUM_DATA_PAGES) - old_sort_pages;
