	      ...
	      if (ev_res == V_TRUE)
		{
		  ...
		  if (!parent_tuple_added)
		      ...
		      if (qfile_add_tuple_get_pos_in_list (thread_p, listfile0, tuple_rec.tpl, &parent_pos) != NO_ERROR)
			{
		      ...
		      db_make_bit (parent_pos_valp, DB_DEFAULT_PRECISION, (DB_CONST_C_BIT) (&parent_pos),
				   sizeof (parent_pos) * 8);
	  ...
	  if (level > 1)
	    {
	      ...
	      db_make_bit (&parent_pos_dbval, DB_DEFAULT_PRECISION, (DB_CONST_C_BIT) & pos_info_p->tpl_pos,
