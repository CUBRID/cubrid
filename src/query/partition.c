			    ...
			    BTID * btid, DB_VALUE * partition_key) 
{
	...
	
	{
      ...
      partition_set_cache_info_for_expr (pcontext->partition_pred->func_regu, pcontext->attr_id,
					   &pcontext->attr_info);
...
btree_get_prunning_partition_index (PRUNING_CONTEXT * pcontext, DB_VALUE * key, OID * class_oid, BTID * btid,
				    int *position) 
{
