...
#define QMGR_FREE_OLD_PAGE_RO_AND_INIT(thread_p, page_p) \
  do \
    { \
      qmgr_free_old_page_read_only ((thread_p), (page_p)); \
      (page_p) = NULL; \
    } \
  while (0)
...
extern void qmgr_free_old_page_read_only (THREAD_ENTRY * thread_p, PAGE_PTR page_ptr);
