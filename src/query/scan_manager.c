...
/*
 * scan_free_hash_list_scan () - free the hash list scan table idempotently.
 *   return:
 *   thread_p(in): Thread entry.
 *   hlsid_p(in/out): hash list scan holding the hash table to free;
 *                    its table pointer is nulled out
 */
void
scan_free_hash_list_scan (THREAD_ENTRY * thread_p, HASH_LIST_SCAN * hlsid_p)
{
  switch (hlsid_p->hash_list_scan_type)
	...
	{
	  mht_destroy_hls (hlsid_p->memory.hash_table);
	  hlsid_p->memory.hash_table = NULL;
	}
      break;

    case HASH_METH_HASH_FILE:
      if (hlsid_p->file.hash_table != NULL)
	{
	  fhs_destroy (thread_p, hlsid_p->file.hash_table);
	  db_private_free_and_init (thread_p, hlsid_p->file.hash_table);
	}
      break;
 ...
 */
void
scan_print_stats_json (SCAN_ID * scan_id, cub_json_t * scan_stats)
{
  cub_json_t *scan, *lookup;
  ...
  scan = cub_json_pack ("{s:i, s:I, s:I}", "time", TO_MSEC (scan_id->scan_stats.elapsed_scan), "fetch",
  ...
  switch (scan_id->type)
    ...
    case S_PARALLEL_HEAP_SCAN:
    case S_PARALLEL_LIST_SCAN:
      cub_json_object_set_new (scan, "readrows", cub_json_integer (scan_id->scan_stats.read_rows));
      cub_json_object_set_new (scan, "rows", cub_json_integer (scan_id->scan_stats.qualified_rows));
      ...
      if (scan_id->type == S_HEAP_SCAN || scan_id->type == S_PARALLEL_HEAP_SCAN)
	{
	  if (scan_id->scan_stats.agl)
	      ...
	      for (agl = scan_id->scan_stats.agl; agl; agl = agl->next)
		{
	      ...
	      cub_json_object_set_new (scan, "agl", cub_json_string (agl_index));
	      free (agl_index);
	    }

	  if (scan_id->scan_stats.noscan)
	    {
	      cub_json_object_set_new (scan_stats, "noscan", scan);
	    }
	  else
	    {
	      cub_json_object_set_new (scan_stats, "heap", scan);
	    }
	}
      else
	{
	  cub_json_object_set_new (scan_stats, "temp", scan);
	}
      break;

    case S_INDX_SCAN:
    case S_PARALLEL_INDEX_SCAN:
      cub_json_object_set_new (scan, "readkeys", cub_json_integer (scan_id->scan_stats.read_keys));
      cub_json_object_set_new (scan, "filteredkeys", cub_json_integer (scan_id->scan_stats.qualified_keys));
      cub_json_object_set_new (scan, "rows", cub_json_integer (scan_id->scan_stats.key_qualified_rows));
      cub_json_object_set_new (scan_stats, "btree", scan);

      if (scan_id->scan_stats.covered_index == true)
	{
	  cub_json_object_set_new (scan_stats, "covered", cub_json_true ());
	}
      else
	{
	  lookup = cub_json_pack ("{s:i, s:i}", "time", TO_MSEC (scan_id->scan_stats.elapsed_lookup), "rows",
				  scan_id->scan_stats.data_qualified_rows);

	  cub_json_object_set_new (scan_stats, "lookup", lookup);
	}

      if (scan_id->scan_stats.multi_range_opt == true)
	{
	  cub_json_object_set_new (scan_stats, "mro", cub_json_true ());
	}

      if (scan_id->scan_stats.index_skip_scan == true)
	{
	  cub_json_object_set_new (scan_stats, "iss", cub_json_true ());
	}

      if (scan_id->scan_stats.loose_index_scan == true)
	{
	  cub_json_object_set_new (scan_stats, "loose", cub_json_true ());
	}
      break;

    case S_SHOWSTMT_SCAN:
      cub_json_object_set_new (scan_stats, "show", scan);
      break;

    case S_SET_SCAN:
      cub_json_object_set_new (scan_stats, "set", scan);
      break;

    case S_METHOD_SCAN:
      cub_json_object_set_new (scan_stats, "method", scan);
      break;

    case S_DBLINK_SCAN:
      cub_json_object_set_new (scan_stats, "dblink", scan);
      break;

    case S_CLASS_ATTR_SCAN:
      cub_json_object_set_new (scan_stats, "class_attr", scan);
      break;
...
scan_build_hash_list_scan (THREAD_ENTRY * thread_p, SCAN_ID * scan_id)
{
  ...
  while ((qp_scan = qfile_scan_list_next (thread_p, &llsidp->lsid, &tplrec, PEEK)) == S_SUCCESS)
	...
	case HASH_METH_IN_MEM:
	  {
	    ...
	    entry = qdata_alloc_hscan_value (thread_p, llsidp->hlsid.memory.hash_table->heap_id, tplrec.tpl);
	    if (entry == NULL)
	      {
		return S_ERROR;
	      }
	    /* add to hash table */
	    if (mht_put_hls (llsidp->hlsid.memory.hash_table, (void *) &hash_key, entry) == NULL)
	      {
		return S_ERROR;
	      }
	  }
	...
	case HASH_METH_HYBRID:
	  {
	    /* store the tuple position in the table obstack */
	    entry = qdata_alloc_hscan_value_OID (thread_p, llsidp->hlsid.memory.hash_table->heap_id, &llsidp->lsid);
	    if (entry == NULL)
	      {
		return S_ERROR;
	      }
...
scan_hash_probe_next (THREAD_ENTRY * thread_p, SCAN_ID * scan_id, QFILE_TUPLE * tuple)
{
  ...
  if (scan_id_p->position == S_BEFORE)
	...
	case HASH_METH_HYBRID:
	  ...
	  entry =
	    mht_get_hls (llsidp->hlsid.memory.hash_table, (void *) &hash_key, &llsidp->hlsid.memory.curr_hash_entry);
	  if (entry == NULL)
	    {
	      return S_END;
	    }
	...
	case HASH_METH_HYBRID:
	  entry =
	    mht_get_next_hls (llsidp->hlsid.memory.hash_table,
			      (void *) &llsidp->hlsid.curr_hash_key, &llsidp->hlsid.memory.curr_hash_entry);
	  if (entry == NULL)
	    {
...
check_hash_list_scan (LLIST_SCAN_ID * llsidp, int *val_cnt, int hash_list_scan_yn)
{
  ...
  INT64 tuple_cnt;
  UINT64 slot_array_size, entries_size, payload_size, in_mem_size, hybrid_size;
  ...
  tuple_cnt = llsidp->list_id->tuple_cnt;
  ...
  *val_cnt = build_cnt;
  ...
  /* The IN_MEM/HYBRID table is int-indexed, capped at INT_MAX rows;
   * larger builds use HASH_FILE. */
  if (tuple_cnt > INT_MAX)
    {
      return HASH_METH_HASH_FILE;
    }

  slot_array_size = (UINT64) mht_hls_slot_count ((int) tuple_cnt) * sizeof (MHT_HLS_SLOT);
  entries_size = (UINT64) tuple_cnt *sizeof (MHT_HLS_ENTRY);

  /* IN_MEM: slot array + one entry (header) per row + the tuples themselves */
  payload_size = (UINT64) llsidp->list_id->page_cnt * DB_PAGESIZE;
  ...
  payload_size = (UINT64) tuple_cnt *sizeof (QFILE_TUPLE_SIMPLE_POS);
  hybrid_size = slot_array_size + entries_size + payload_size;

  if (in_mem_size <= mem_limit)
    {
      return HASH_METH_IN_MEM;
    }
  else if (hybrid_size <= mem_limit)
    {
