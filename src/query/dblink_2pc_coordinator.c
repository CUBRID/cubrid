...
static int dblink_2pc_add_to_retry_queue (int gtrid, int req_type, 
...
dblink_2pc_enqueue_request (GTRAN_2PC_REQUEST * req, bool wait_ack)
{
  ...
  while (dblink_2pc_Coordinator_daemon.queue.count >= GTRAN_2PC_QUEUE_SIZE)
    {
      pthread_cond_wait (&dblink_2pc_Coordinator_daemon.queue.not_full,
                         &dblink_2pc_Coordinator_daemon.queue.mutex);
    }
...
dblink_2pc_add_to_retry_queue (int gtrid, int req_type, const PARTICIPANT_INFO * participant)
{
  ...
  er_log_debug (ARG_FILE_LINE, "Added gtrid=%d bqual=%d to retry queue (type=%d)", 
                gtrid, participant->bqual, req_type);
...
dblink_2pc_remove_from_retry_queue (int index)
{
  ...
  for (i = index; i < dblink_2pc_Coordinator_daemon.retry_queue.count - 1; i++)
    {
      dblink_2pc_Coordinator_daemon.retry_queue.entries[i] = 
        dblink_2pc_Coordinator_daemon.retry_queue.entries[i + 1];
    }
...
dblink_2pc_coordinator_daemon_thread (void *arg)
{
  ...
  er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE, ER_NOTIFICATION_SEVERITY, 0,
          "2PC Coordinator daemon started");
  ...
  er_set (ER_NOTIFICATION_SEVERITY, ARG_FILE_LINE, ER_NOTIFICATION_SEVERITY, 0,
          "2PC Coordinator daemon stopped");
 ...
 */
GTRAN_2PC_REQUEST *
dblink_2pc_create_request (int req_type, int gtrid, int trid, char state,
                    const PARTICIPANT_INFO * participant)
{
  GTRAN_2PC_REQUEST *req;
