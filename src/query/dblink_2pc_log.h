...
extern int dblink_2pc_log_insert (THREAD_ENTRY * thread_p, int gtrid, 
                                    const PARTICIPANT_INFO * participant, char state);

/* Update state of existing log */
extern int dblink_2pc_log_update_state (THREAD_ENTRY * thread_p, int gtrid, 
                                      int bqual, char new_state);

/* Read a log entry by GTRID and BQUAL */
extern int dblink_2pc_log_read (THREAD_ENTRY * thread_p, int gtrid, int bqual, 
                                  GTRAN_2PC_LOG_ENTRY * log_entry);
