...
xserial_get_current_value_internal (THREAD_ENTRY * thread_p, DB_VALUE * result_num, const OID * serial_oidp)
{
  ...
  scan =
    heap_get_visible_version (thread_p, serial_oidp, &serial_class_oid, &recdesc, false, &scan_cache, PEEK, NULL_CHN);
  if (scan != S_SUCCESS)
    {
...
serial_update_cur_val_of_serial (THREAD_ENTRY * thread_p, SERIAL_CACHE_ENTRY * entry)
{
  ...
  scan =
    heap_get_visible_version (thread_p, &entry->oid, &serial_class_oid, &recdesc, false, &scan_cache, PEEK, NULL_CHN);
  if (scan != S_SUCCESS)
    {
      if (er_errid () == ER_PB_BAD_PAGEID)
	{
