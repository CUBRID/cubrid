 ...
 *
 * qp_num.c - Basig manipulations of DB_NUMERIC type data
 *
...
numeric_compare (DB_C_NUMERIC arg1, DB_C_NUMERIC arg2)
{
  ...
  arg1_sign = numeric_is_negative (arg1);
  arg2_sign = numeric_is_negative (arg2);
...
{
  /*
   * This routine is guaranteed to receive a double that satisfies this
   * property:
   *
   *  adouble * (10**dst_scale) < DB_INT32_MAX
   *
   * That means that we can multiply it by the scale factor and then
   * use a native cast-to-long-int and not lose any digits.
   *
   * Maybe we should explicitly use long int here (rather than
   * int) so that we can exploit the extended precision of 64-bit
   * architectures like Alpha?  That would mean a little additional
   * parameterization for the static arrays and the qualification test
   * in numeric_internal_double_to_num ().
   */

  /*
   * This code rounds halfway cases toward the integral value of larger
   * magnitude, i.e.,
   *
   * anint(1.49)  == 1.0
   * anint(1.50)  == 2.0
   ...
   * anint(-1.50) == -2.0
   *
   * If that's not what we want, this code needs to change.
   */
