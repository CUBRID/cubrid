...
namespace cubscan
{
  namespace json_table
  {
	...
	case json_table_expand_type::JSON_TABLE_NO_EXPAND:
	  if (m_node->m_path != NULL)
	    {
	      std::string tmp (m_node->m_path);

	      // if we don't find wildcard we should ignore expansion:
	      // we do not iterate over the values generated by json_extract
	  ...
	  if (db_json_get_type (m_input_doc) == DB_JSON_ARRAY)
	    {
	      m_node->m_expand_type = JSON_TABLE_ARRAY_EXPAND;

	      m_is_node_consumed = false;
	      m_node->m_iterator = db_json_create_iterator (DB_JSON_TYPE::DB_JSON_ARRAY);
	      db_json_set_iterator (m_node->m_iterator, *m_input_doc);
	    }

	  if (db_json_get_type (m_input_doc) == DB_JSON_OBJECT)
	    {
	      m_node->m_expand_type = JSON_TABLE_OBJECT_EXPAND;

	      m_is_node_consumed = false;
	      m_node->m_iterator = db_json_create_iterator (DB_JSON_TYPE::DB_JSON_OBJECT);
	      db_json_set_iterator (m_node->m_iterator, *m_input_doc);
	    }
	  break;

	// these 2 are for safety (remove them)
	case JSON_TABLE_ARRAY_EXPAND:
      ...
      if (m_process_doc == NULL)
	{
      ...
      char *temp = NULL;
      db_json_unquote (*m_process_doc, temp);
      std::string s (temp);
