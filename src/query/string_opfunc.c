...
db_char_to_blob (const DB_VALUE * src_value, DB_VALUE * result_value)
{
  ...
  if (QSTR_IS_ANY_CHAR (src_type))
      ...
      if ((length >= 0) && (char_data != NULL))
	{
      ...
      /* db_make_blob's last argument is a BIT length (int), so convert the byte
       * count to bits; passing the byte count stored only ceil(length/8)
       * bytes and silently dropped the rest of the string.  Guard the * 8 so a
       * source larger than INT_MAX / 8 bytes cannot overflow the int bit length. */
      if (length > INT_MAX / 8)
	{
...
internal_lob_make_scalar_stream_marker (const INTERNAL_LOB_LOCATOR & locator, char lob_type, DB_VALUE * result_value)
{
  ...
  {
    /* Sign the client-facing scalar-stream locator with the session secret (CBRD-26780). */
    THREAD_ENTRY *thread_p = thread_get_thread_entry_info ();
    INT64 session_key = 0;

    (void) session_get_internal_lob_locator_key (thread_p, &session_key);
    locator_len = internal_lob_format_locator_string (locator, locator_buf, sizeof (locator_buf), false, session_key);
  }
...
db_blob_to_bit (const DB_VALUE * src_value, const DB_VALUE * length_value, DB_VALUE * result_value)
{
  ...
  void *buf = NULL;
  DB_TYPE src_type = DB_TYPE_NULL;
  DB_TYPE length_type = DB_TYPE_NULL;
  int length = 0;
  int max_length = 0;
  int error_status = NO_ERROR;
...
{
  DB_TYPE src_type = DB_TYPE_NULL;
  DB_CONST_C_CHAR clob_data = NULL;
  void *buf = NULL;
  int codeset = LANG_SYS_CODESET;
  int length = 0;
  ...
  if (codeset_value != NULL)
    {
      ...
      codeset = db_get_int (codeset_value);
      if (codeset != INTL_CODESET_UTF8 && codeset != INTL_CODESET_ISO88591 && codeset != INTL_CODESET_KSC5601_EUC)
	{
	  error_status = ER_OBJ_INVALID_ARGUMENTS;
	  goto error;
  ...
  if (src_type == DB_TYPE_CLOB)
      ...
      if ((length >= 0) && (clob_data != NULL))
	{
	  ...
	  memcpy (buf, clob_data, length);
	}

      error_status =
	db_make_varchar (result_value, DB_MAX_VARCHAR_PRECISION, (DB_CONST_C_CHAR) buf, length, codeset, collation);
      if (error_status != NO_ERROR)
	{
	  goto error;
	}
