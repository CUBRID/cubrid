...
db_string_chr (DB_VALUE * res, DB_VALUE * dbval1, DB_VALUE * dbval2)
{
  ...
  switch (arg_type)
	...
	int scale = db_get_numeric_scale (dbval1, &is_float_numeric);
	numeric_coerce_num_to_double (db_locate_numeric (dbval1), scale, &dtmp);
      }
      break;
...
db_round_dbvalue_to_int (const DB_VALUE * src, int *result)
{
  ...
  switch (src_type)
	...
	*result = (int) ((x) > 0 ? ((x) + .5) : ((x) - .5));
	...
	numeric_coerce_num_to_double (db_locate_numeric ((DB_VALUE *) src), scale, &x);
	*result = (int) ((x) > 0 ? ((x) + .5) : ((x) - .5));
	return NO_ERROR;
      }
...
db_timestamp (const DB_VALUE * src_datetime1, const DB_VALUE * src_time2, DB_VALUE * result_datetime)
{
  ...
  switch (type)
    ...
    case DB_TYPE_NUMERIC:
      {
	bool is_float_numeric = false;
	int scale = db_get_numeric_scale (src_time2, &is_float_numeric);
	numeric_coerce_num_to_double ((DB_C_NUMERIC) db_locate_numeric (src_time2), scale, &amount_d);
      }
      break;
			       ...
			       int is_add)
{
  ...
  switch (expr_type)
    ...
    case DB_TYPE_NUMERIC:
      {
	bool is_float_numeric = false;
	int scale = db_get_numeric_scale (expr, &is_float_numeric);
	numeric_coerce_num_to_double ((DB_C_NUMERIC) db_locate_numeric (expr), scale, &dbl);
