 ...
 *
 * qp_str.c - Routines that manipulate arbitrary strings
 *
 ...
 *
 * Note:
 *   Compares string1 with string2 and returns an integer greater than,
 *   equal to, or less than 0, according as string1 is lexicographically
 *   greater than, equal to, or less  than  string2.  This function
 *   is intended to behave analogously to strcmp(3).
 *
 *   Unlike strcmp(3), these strings are not strictly ASCII, may contain
 *   '\0' characters and are not necessarily NULL terminated.  Depending
 *   on the character code set, a character may occupy one or two bytes.
 *   The comparison is, however, performed on a byte-by-byte basis and
 *   is therefore more of a binary comparison rather than a character
 *   comparison.
 *
 ...
 *     -1 if string1 < string2
 *
 *   The character code sets and string lengths are read from the domain
 *   information stored in DB_VALUE.  Strings can only be compared IFF
 *   they belong to the same character code set or if they are both
 *   bit strings.
 *
 *   A NULL valued string string will be ordered less than any non-NULL
 *   valued string.  Two NULL valued strings are considered equivalent
 *   (e.i. '=') since they posses the same ordering.
 *
 *   Note: For NULL valued strings, equality here merely means they
 *         are ordered equivalently.
 *
 * Assert:
 *
		   ...
		   DB_VALUE * result)
{
  ...
  if (DB_IS_NULL (string1) && !DB_IS_NULL (string2))
	...
	case QSTR_CHAR:
	  cmp_result =
	    qstr_compare ((unsigned char *) DB_GET_STRING (string1),
			  (int) DB_GET_STRING_SIZE (string1),
			  (unsigned char *) DB_GET_STRING (string2),
			  (int) DB_GET_STRING_SIZE (string2));
	  break;
	case QSTR_NATIONAL_CHAR:
	  cmp_result =
	    varnchar_compare ((unsigned char *) DB_GET_STRING (string1),
			      (int) DB_GET_STRING_SIZE (string1),
			      (unsigned char *) DB_GET_STRING (string2),
			      (int) DB_GET_STRING_SIZE (string2),
	...
	case QSTR_BIT:
	  cmp_result =
	    varbit_compare ((unsigned char *) DB_GET_STRING (string1),
			    (int) DB_GET_STRING_SIZE (string1),
			    (unsigned char *) DB_GET_STRING (string2),
			    (int) DB_GET_STRING_SIZE (string2));
	  break;
			 ...
			 int is_reverse)
{
  ...
  /* Find the first byte where the 2 strings differ.  Set the result
    ...
    {
      int size1, size2, result_size, pad_size, n;
	   ...
	   result_size++)
	{
      ...
      /*
       * If we assume that this routine is never called with identical
       * keys, exactly one of the following conditions should hold at
       * this point:
       *
       *   1. (result_size == string1_size)
       *      We're at the end of the first string so it is a proper
       *      prefix of the second.
       *
       *      Prefix is entire first string.
       *
       *   2. (result_size == string2_size but != string1_size)
       *      We're at the end of the second string so it is a proper
       *      prefix of the first;
       *
       *      This should never happen.
       *
       *
       *   3. (string1[result_size] != string2[result_size])
       *      We're at the first byte that differs between the keys
       *
       *      Prefix is either the prefix and the next character of
       *      string2 (if that isn't the last significant character
       *      in the key), or the entire first key.
       */
 ...
 *      data_status: DB_DATA_STATUS which indicates if truncation occured.
 *
 * Returns: int
 *
 * Errors:
 ...
 *
 * Note:
 *   Behaves analogously to the strcat() function.  This
 *   function returns the concatenation of the two strings
 *   <string1> and <string2> in the result.
 *
 *   This function make use of the domain information that
 *   resides in the DB_VALUE structure regarding the character
 *   set and lengths of the character strings.  Strings can
 *   only be concatenated IFF they belong to the same character
 *   code set, or if they are both bit strings.
 *
 *   If the NULL flag is set for either <string1> or
 *   <string2>, then the NULL flag for the result is set.
 *
 *   If the length of <string1> + the length of <string2>
 *   exceeds the maximum storable length, the result is
 *   truncated and <data_status> is set to DATA_STATUS_TRUNCATED.
 *
 * Assert:
 *
 *   1. string1 != (DB_VALUE *) NULL
  ...
  *data_status = DATA_STATUS_OK;
  ...
  if (!QSTR_IS_ANY_CHAR_OR_BIT (string_type1)
      ...
      if (string1_code_set == QSTR_BIT)
	{
	  ...
	  error_status = qstr_bit_concatenate ((unsigned char *)
					       DB_GET_STRING (string1),
					       (int)
					       DB_GET_STRING_LENGTH (string1),
					       (int)
					       QSTR_VALUE_PRECISION (string1),
					       DB_VALUE_DOMAIN_TYPE (string1),
					       (unsigned char *)
					       DB_GET_STRING (string2),
					       (int)
					       DB_GET_STRING_LENGTH (string2),
					       (int)
					       QSTR_VALUE_PRECISION (string2),
      ...
      else
	{
	  ...
	  error_status = qstr_concatenate ((unsigned char *)
					   DB_GET_STRING (string1),
					   (int)
					   DB_GET_STRING_LENGTH (string1),
					   (int)
 ...
 * Note:
 *
 *   This function searches for the beginning of the first
 *   occurrence of <sub_string> within <src_string>. If
 *   <sub_string> is found within <src_string>, the result
 *   holds the character or bit position of the beginning of
 *   <sub_string> within <src_string> (where the first character/bit
 *   is '1').
 *
 *   The following conventions are used for special cases:
 *      If <sub_string> has length 0, the result is 1.
 *      If <sub_string> is not found, the result is 0.
 *
 *   This function make use of the domain information that
 *   resides in the DB_VALUE structure regarding the character
 *   set and lengths of the character strings.  Strings can
 *   only be used in this function IFF they belong to the same
 *   character code set or if they are both bit strings.
 *
 *   If the NULL flag is set for either <sub_string> or
 *   <src_string>, then the NULL flag for the result is set.
 *
 * Assert:
 *
 *   1. sub_string != (DB_VALUE *) NULL
 *   2. src_string != (DB_VALUE *) NULL
 *   3. result     != (DB_VALUE *) NULL
 *
 */
		 ...
		 const DB_VALUE * start_pos, DB_VALUE * result)
{
  ...
  if (DB_IS_NULL (src_string) || DB_IS_NULL (sub_string) ||
    ...
    {
      if (!(str1_type == DB_TYPE_STRING ||
	    str1_type == DB_TYPE_CHAR || str1_type == DB_TYPE_VARCHAR ||
	    str1_type == DB_TYPE_NCHAR || str1_type == DB_TYPE_VARNCHAR) ||
	  !(str2_type == DB_TYPE_STRING ||
	    str2_type == DB_TYPE_CHAR || str2_type == DB_TYPE_VARCHAR ||
      ...
      else
	{
	  ...
	  if (offset > 0)
		...
		{
		  search_from = DB_GET_STRING (src_string) + offset;

		  intl_char_count ((unsigned char *) search_from,
				   strlen (search_from), codeset,
				   &src_str_len);

		  error_status = qstr_position (DB_GET_STRING (sub_string), sub_str_len, search_from, src_str_len, codeset, true,	/* forward search */
						&position);
		...
		{
		  search_from =
		    DB_GET_STRING (src_string) + src_str_len + offset -
		    (sub_str_len - 1);

		  error_status = qstr_position (DB_GET_STRING (sub_string), sub_str_len, search_from, src_str_len + offset + 1, codeset, false,	/* backward search */
						&position);
		  if (position != 0)
		    {
		      position = src_str_len - (-offset - 1) - (position - 1)
			- (sub_str_len - 1);
		    }
		}
	    }
		    ...
		    const DB_VALUE * src_string, DB_VALUE * result)
{
  ...
  if (DB_IS_NULL (sub_string) || DB_IS_NULL (src_string))
	...
	{
	  error_status = qstr_position (DB_GET_STRING (sub_string),
					DB_GET_STRING_LENGTH (sub_string),
					DB_GET_STRING (src_string),
	...
	{
	  error_status = qstr_bit_position ((unsigned char *)
					    DB_GET_STRING (sub_string),
					    DB_GET_STRING_LENGTH (sub_string),
					    (unsigned char *)
					    DB_GET_STRING (src_string),
					    DB_GET_STRING_LENGTH (src_string),
 ...
 * Note:
 *
 *   This function returns a <sub_string> that was extracted from
 *   a <src_string>.
 *
 *   The <start_pos> holds the integer position (first
 *   character = 1) of the character within the <src_string>
 *   to begin the extraction.  If <start_pos> is < 1, the
 *   extraction begins at 1.
 *
 *   The <extraction_length> is an optional parameter that
 *   holds the number of characters to extract.  If it is NULL
 *   or < 1, the extraction will go to the end of the <src_string>.
 *
 *   If the NULL flag is set for either <src_string> or
 *   <start_pos> then the NULL flag for <sub_string> is set.
 *
 * Assert:
 *
 *   1. src_string        != (DB_VALUE *) NULL
 *   2. start_pos         != (DB_VALUE *) NULL
 *   3. sub_string        != (DB_VALUE *) NULL
 *
 */
		     ...
		     DB_VALUE * sub_string)
{
  ...
  /*
   *  Verify
   *    that the input parameters are all character strings.
   *    that the trim character is valid.  A trim character of more
   *       than 1 byte is not valid.  A NULL trim character is,
   *       however.  Go figure.
   *    that the input strings belong to compatible categories.
   *
   *  If the input parameters have been properly validated, then
   *  we are ready to perform the operation.
   */
  if (DB_IS_NULL (src_string) || DB_IS_NULL (start_position))
      ...
      else
	{
	  ...
	  if (QSTR_IS_CHAR (src_type) || QSTR_IS_NATIONAL_CHAR (src_type))
		...
		(unsigned char *) DB_GET_STRING (src_string);
	      ...
	      if (substr_operand == SUBSTR)
		{
	      ...
	      error_status = qstr_substring (string,
					     string_len,
					     start_offset,
					     extract_nchars,
	    ...
	    {
	      error_status = qstr_bit_substring ((unsigned char *)
						 DB_GET_STRING (src_string),
						 (int)
...
db_string_lower (const DB_VALUE * string, DB_VALUE * lower_string)
{
   ...
   */
  else
    {
      ...
      intl_lower_string ((unsigned char *) DB_GET_STRING (lower_string),
...
db_string_upper (const DB_VALUE * string, DB_VALUE * upper_string)
{
   ...
   */
  else
    {
      ...
      intl_upper_string ((unsigned char *) DB_GET_STRING (upper_string),
 ...
 * Note:
 *
 *   This function removes a specified characters from the
 *   beginning or end of src_string and returns the result.
 *
 *   The enumeration MISC_OPERAND specifies whether the
 *   character to be trimmed is removed from the beginning of
 *   the string (case LEADING), ending of the string (case
 *   TRAILING), or both (case BOTH).
 *
 *   <trim_char> holds a character string of length 1 which
 *   represents the character to remove. Conventions for
 *   special cases are:
 *      <trim_charset> has NULL flag set, the space character is used
 *                  as the <trim_char>.
 *      <trim_charset> has length 0, the space character is used
 *                  as the <trim_char>.
 *
 *   If the NULL flag is set for <src_string>, then the NULL
 *   flag for the <trimmed_string> is set.
 *
 * Assert:
 *
 *   1. trim_charset   != (DB_VALUE *) NULL
 *   2. src_string     != (DB_VALUE *) NULL
 *   3. trimmed_string != (DB_VALUE *) NULL
 *
 */
		...
		const DB_VALUE * src_string, DB_VALUE * trimmed_string)
{
  ...
  unsigned char *result;
  int result_length, result_size, result_domain_length;
  DB_TYPE result_type;
	   ...
	   DB_TYPE * result_type, int *result_length, int *result_size)
{
  ...
  int lead_trimmed_size, trail_trimmed_size, pad_char_size;
  int error_status = NO_ERROR;
...
{
  int error_status = NO_ERROR;
  int pad_charset_is_null = false;
  int total_length;

  unsigned char *result;
  int result_length, result_size;
  DB_TYPE result_type;
  ...
  src_type = DB_VALUE_DOMAIN_TYPE (src_string);
  if (!QSTR_IS_ANY_CHAR (src_type)
      || (!pad_charset_is_null && !is_char_string (pad_charset)))
    {
      error_status = ER_QSTR_INVALID_DATA_TYPE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_QSTR_INVALID_DATA_TYPE, 0);
      return error_status;
  ...
  error_status = qstr_pad (pad_operand,
			   total_length,
			   pad_charset_ptr,
			   pad_charset_length,
			   pad_charset_size,
	  ...
	  DB_TYPE * result_type, int *result_length, int *result_size)
{
  ...
  int cur_char_size;
  int def_pad_char_size;	/* default padding char */
 ...
 *
 * Note:
 *   This function returns a Boolean result (encoded in an
 *   integer) based on whether the <src_string> is equivalent
 *   to the specified <pattern>.  If the value for <result> is
 *   0, then the <src_string> did not match the <pattern>;  if
 *   it is 1, then it did match.
 *
 *   <pattern> may contain special characters '%'
 *   to mean an arbitrary number of characters, and/or '_' to
 *   specify a single character.  If <esc_char> is not NULL,
 *   then it must be a string of length of 1 to represent an
 *   escape character in order to escape the meaning of '%' or '_'.
 *
 *   If an error occurred, the NULL bit is set for the <result>.
 *
 *   If <src_string> or <pattern> are NULL then <result> will be NULL.
 *
 *   If the length of <src_string> and <pattern> are 0, then result
 *   will be TRUE.
 *
 * Assert:
 *
 *   1. src_string != (DB_VALUE *) NULL
 *   2. pattern    != (DB_VALUE *) NULL
 *   3. result     != (DB_VALUE *) NULL
 *
 */
		...
		const unsigned char esc_char, int *result)
{
  ...
  unsigned char *src_char_string_p, *pattern_char_string_p;
  ...
  /*
   *  Categorize the two input parameters and check for errors.
   *  Note the special handling of the escape character <esc_char>.
   *    Verify that the source and pattern parameters are both
   *    character strings and that the result is an integer.
   *
   *    Verify that the input strings belong to compatible
   *    categories.
   *
   *    If either input string is NULL, then the result will be
   *    NULL.
   */
  ...
  if (src_type == DB_TYPE_CHAR || src_type == DB_TYPE_NCHAR)
    {
      ...
      src_char_string_p = (unsigned char *) DB_GET_CHAR (src_string,
							 &src_len);
  ...
  else
    {
      ...
      src_char_string_p = (unsigned char *) DB_GET_STRING (src_string);
    }
  ...
  if (pattern_type == DB_TYPE_CHAR || pattern_type == DB_TYPE_NCHAR)
    {
      ...
      pattern_char_string_p = (unsigned char *) DB_GET_CHAR (pattern,
							     &pattern_len);
    ...
    {
      /* varchar, varnchar has a terminating NULL */
      pattern_char_string_p = (unsigned char *) DB_GET_STRING (pattern);
    }
...
{
  int error_status = NO_ERROR;
  int srch_string_is_null = false;
  int repl_string_is_null = false;

  unsigned char *result_ptr;
  int result_length, result_size;
  DB_TYPE result_type;

  unsigned char *srch_string_ptr = NULL;
  int srch_string_length = 0, srch_string_size = 0;
  unsigned char *repl_string_ptr = NULL;
  int repl_string_length = 0, repl_string_size = 0;
  ...
  if (srch_string == NULL)
    {
      srch_string_is_null = true;
    }

  if (repl_string == NULL)
    {
      repl_string_is_null = true;
    }

  if ((!srch_string_is_null && !is_char_string (srch_string))
      || (!repl_string_is_null && !is_char_string (repl_string))
  ...
  if (!srch_string_is_null)
    {
      srch_string_ptr = (unsigned char *) DB_GET_STRING (srch_string);
      srch_string_length = DB_GET_STRING_LENGTH (srch_string);
      srch_string_size = DB_GET_STRING_SIZE (srch_string);
    }

  if (!repl_string_is_null)
    {
      repl_string_ptr = (unsigned char *) DB_GET_STRING (repl_string);
      repl_string_length = DB_GET_STRING_LENGTH (repl_string);
      repl_string_size = DB_GET_STRING_SIZE (repl_string);
    }

  error_status = qstr_replace ((unsigned char *) DB_GET_STRING (src_string),
			       DB_VALUE_DOMAIN_TYPE (src_string),
			       DB_GET_STRING_LENGTH (src_string),
			       DB_GET_STRING_SIZE (src_string),
			       (INTL_CODESET)
			       DB_GET_STRING_CODESET (src_string),
			       srch_string_ptr,
			       srch_string_size, repl_string_ptr,
			       repl_string_size, &result_ptr, &result_type,
		     ...
		     DB_VALUE * transed_string)
{
  ...
  unsigned char *from_str_ptr = NULL;
  unsigned char *to_str_ptr = NULL;
  int from_str_size = 0, to_str_size = 0;
  ...
  if (from_string == NULL)
    {
      from_string_is_null = true;
    }

  if (to_string == NULL)
    {
      to_string_is_null = true;
    }

  if ((!from_string_is_null && !is_char_string (from_string))
      || (!to_string_is_null && !is_char_string (to_string))
      || !is_char_string (src_string))
    {
      error_status = ER_QSTR_INVALID_DATA_TYPE;
  ...
  if ((!from_string_is_null
       && (qstr_get_category (src_string)
	   != qstr_get_category (from_string)))
      || (!to_string_is_null
	  && (qstr_get_category (src_string)
	      != qstr_get_category (to_string)))
      || (!from_string_is_null && !to_string_is_null
	  && (qstr_get_category (from_string)
	      != qstr_get_category (to_string))))
    {
      error_status = ER_QSTR_INCOMPATIBLE_CODE_SETS;
  ...
  if (!from_string_is_null)
    {
      from_str_ptr = (unsigned char *) DB_GET_STRING (from_string);
      from_str_size = DB_GET_STRING_SIZE (from_string);
    }

  if (!to_string_is_null)
    {
      to_str_ptr = (unsigned char *) DB_GET_STRING (to_string);
      to_str_size = DB_GET_STRING_SIZE (to_string);
    }

  error_status = qstr_translate ((unsigned char *) DB_GET_STRING (src_string),
				 DB_VALUE_DOMAIN_TYPE (src_string),
				 DB_GET_STRING_SIZE (src_string),
				 (INTL_CODESET)
				 DB_GET_STRING_CODESET (src_string),
				 from_str_ptr, from_str_size,
				 to_str_ptr, to_str_size, &result_ptr,
				 &result_type, &result_length, &result_size);
		...
		DB_TYPE * result_type, int *result_len, int *result_size)
{
  ...
  unsigned char *srcp, *fromp, *target;
  ...
  for (srcp = src_ptr; srcp < src_ptr + src_size;)
      ...
      if (!PRM_SINGLE_BYTE_COMPARE && IS_KOREAN (*srcp))
	{
      ...
      for (fromp = from_str_ptr;
	   fromp < from_str_ptr + from_str_size; from_char_loc++)
	{
  ...
  *data_status = DATA_STATUS_OK;
  ...
  if (!QSTR_IS_BIT (src_type) || !QSTR_IS_BIT (dest_type))
      ...
      else
	{
      ...
      error_status = qstr_bit_coerce ((unsigned char *)
				      DB_GET_STRING (src_string),
				      DB_GET_STRING_LENGTH (src_string),
				      QSTR_VALUE_PRECISION (src_string),
				      src_type, &dest,
				      &dest_length, dest_prec, dest_type,
				      data_status);
  ...
  *data_status = DATA_STATUS_OK;
  ...
  if (!is_char_string (src_string) || !is_char_string (dest_string))
      ...
      else
	{
      ...
      error_status =
	qstr_coerce ((unsigned char *) DB_GET_STRING (src_string),
		     DB_GET_STRING_LENGTH (src_string),
		     QSTR_VALUE_PRECISION (src_string),
		     DB_VALUE_DOMAIN_TYPE (src_string),
		     (INTL_CODESET) DB_GET_STRING_CODESET (src_string),
 ...
 *              truncation occured and will equal the size of the dst buffer
 ...
 *
 * Note:
 *   Concatenates character string1 + string2.  The concatenated string is
 *   returned in result.
 *
 *
 *   Excerpt from SQL2...
 ...
 *         catenation> is fixed-length character string with length
 *         M.
 *
 *   Variable length results will follow the rules:
 *
 *       Let S be the string consisting of S1 followed by S2 and
 ...
 *
 *   Fixed length result will follow the rule:
 *
 *    ii) If the data types of both S1 and S2 are fixed-length char-
 *        acter string, then the result of the <concatenation> is
 *        S.
 *
 *   Notes: Although not stated in SQL2, if M exceeds FL for fixed length
 *          results, an exception condition is reaised: data exception-string
 *          data, right truncation.
 *
 *          This function does not support strings of type DB_TYPE_STRING.
 *
 */
  ...
  if (QSTR_IS_FIXED_LENGTH (s1_type) && QSTR_IS_FIXED_LENGTH (s2_type))
      ...
      if (*result_length > QSTR_MAX_PRECISION (s1_type))
	{
	  ...
	  *data_status = DATA_STATUS_TRUNCATED;
      ...
      /*
       *  With national characters, it is impossible to know
       *  appriori, how many bytes are needed per character
       *  since it varies with the characters.  But we need
       *  to allocate room for a "result_length" character
       *  string.  So what do we do?
       *
       *  Currently, a character occupies one or two characters
       *  so we can assume worse case by doubling the length of
       *  the result field.
       */
 ...
 *
 * Note:
 *   Concatenates bit string1 + string2.  The concatenated string is
 *   returned in result.
 *
 *
 *   Excerpt from SQL2...
 ...
 *
 *     WARNING!!  This is duplicated from ch_set.c since ch_set.c can not
 *                be built on the server.  When that restriction is removed
 *                this should be replaced with a call to
 *                lang_charset_space_char()
 *
 */
...
{
  unsigned char pad[2];
  int pad_size, trim_length, trim_size;
  int s_size;
...
bit_ncat (unsigned char *r, int offset, const unsigned char *s, int n)
{
  ...
  /*
   *  Now we need to copy the second string after the first.
   *  First we check if the first string ended on a byte
   *  boundary.  If it did, we can gain a lot of performand
   *  by byte copying the second string.  This is the case if
   *  the remainder of the modulus is 0.  This also handles the
   *  case where the offset is 0.
   *
   *  Otherwise, we need to bit shift the second string after
   *  the first.
   */
  ...
  if (remainder == 0)
  ...
  else
    {
      ...
      /*
       *  Calculate "shift_amount" which is the number of bits
       *  shifts needed to fill in the empty bits left from
       *  the last byte of the first string (<r>).  A mask is
       *  created to zero out the unsed bits.
       *
       *  (ex) If the last byte is binary 10__ ____
       *       shift_amount is decimal 6
       *       mask is binary 1100 0000
       */
  ...
  if (*dest == NULL)
  ...
  else
    {
      ...
      end_of_string = (char *) qstr_pad_string ((unsigned char *)
						...
						codeset);
      *dest_size = end_of_string - (char *) (*dest);
    }
	       ...
	       INTL_CODESET codeset, bool is_forward_search, int *position)
{
  ...
  *position = 0;
  ...
  /*
   *  If the length of the sub-string is 0, then it implicitly
   *  matches the first character of the source string.
   *
   *  If the length of the sub-string is greater than the length
   *  of the source string, a match will not occur.
   *
   *  Now that the two trivial cases are removed, we need to actually
   *  search the source string for occurences of the sub-string.
   */
		   ...
		   int src_length, int *position)
{
  ...
  *position = 0;
   ...
   *
   *  If the length of the sub-string is greater than the length
   *  of the source string, a match does will not occur.
   *
   *  Now that the two trivial cases are removed, we need to actually
   *  search the source string for occurences of the sub-string.
   */
  if (sub_length == 0)
  ...
  else
    {
      ...
      /*
       *  Since the entire sub-string must be found in order
       *  for a match to occur, a reduced number of compares
       *  are needed.  This is computed as <num_searches>.
       *
       *  We will be performing a binary comparison, so the
       *  length of the comparison (in bytes) is computed
       *  as <sub_size>.  Since we will be performing byte
       *  comparisons, we will need to pad the strings.
       *
       *  For padding, we need to compute a mask value that will
       *  be used to zero out irrelevant bits from the source
       *  string before comparison.  The sub-string is already
       *  padded with 0's.
       */
      num_searches = src_length - sub_length + 1;
	    ...
	    const DB_VALUE * date_lang, DB_VALUE * result_date)
{
  ...
  cs = (unsigned char *) DB_GET_STRING (src_str);
  ...
  /* Else                            */
      ...
      if (DB_GET_STRING_SIZE (format_str) == 0)
	{
      ...
      cur_format_str_ptr = (unsigned char *) DB_GET_STRING (format_str);
      ...
      while (cs < last_src)
	{
	  ...
	  switch (cur_format)
	    ...
	    case DT_PUNCTUATION:
	      if (strncasecmp
		  ((const char *) cur_format_str_ptr, (const char *) cs,
		   cur_format_size) != 0)
		{
		  error_status = ER_QSTR_INVALID_FORMAT;
		  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
  ...
  /* Else                            */
      ...
      while (cs < last_src)
	{
	  ...
	  switch (cur_format)
	    ...
	    case DT_TEXT:
	      if (strncasecmp
		  ((const char *) cur_format_str_ptr + 1, (const char *) cs,
		   cur_format_size - 2) != 0)
		{
		  error_status = ER_TIME_CONVERSION;
		  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
		 ...
		 const DB_VALUE * date_lang, DB_VALUE * result_timestamp)
{
  ...
  /* Else                            */
      ...
      while (cs < last_src)
	{
	  ...
	  cur_format =
	    get_next_format (cur_format_str_ptr, DB_TYPE_TIMESTAMP,
			     &cur_format_size, &next_format_str_ptr);
	  switch (cur_format)
	    {
	      /* Case statement for DATE begin{    */
	    case DT_YYYY:
	      if (yearcount != 0)
		{
		  error_status = ER_QSTR_FORMAT_DUPLICATION;
	      ...
	      if (day < 0 || day > 31)
		{
	      ...
	      break;
	      /*    }end Case statement for DATE    */
	      /*    Case statement for TIME    begin{    */
	    case DT_AM:
	    case DT_A_M:
	    case DT_PM:
	    case DT_P_M:
	    ...
	    case DT_TEXT:
	      if (strncasecmp
		  ((const char *) (void *) cur_format_str_ptr + 1,
		   (const char *) cs, cur_format_size - 2) != 0)
		{
		  error_status = ER_QSTR_INVALID_FORMAT;
		  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
	    ...
	    case DT_PUNCTUATION:
	      if (strncasecmp
		  ((const char *) (void *) cur_format_str_ptr,
...
{
  char tmp_data[DB_MAX_NUMERIC_PRECISION + 1];
  int scale_counter = 0;
  int i = 0;
...
{
  /* default precisin and scale is (38, 0) */
  ...
  if (format_str == NULL)
      ...
      if (is_char_string (format_str))
	{
	  ...
	  format_str_ptr = DB_GET_STRING (format_str);
	  last_format = &format_str_ptr[DB_GET_STRING_SIZE (format_str)];
	}
      else
	{
	  error_status = ER_UCI_INVALID_DATA_TYPE;
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
	  return error_status;
	}
  ...
  while (cs < last_cs)
	...
	case N_FORMAT:
	  ...
	  error_status =
	    make_number (cs, last_cs, format_str_ptr, &token_length,
			 result_num, precision, scale);
	  if (error_status == NO_ERROR)
	    {
	      count_format++;
	      cs += token_length;
	      ...
	      const DB_VALUE * date_lang, DB_VALUE * result_str)
{
  ...
  if (src_type != DB_TYPE_DATE &&
      src_type != DB_TYPE_TIME && src_type != DB_TYPE_TIMESTAMP)
    {
      error_status = ER_QSTR_INVALID_DATA_TYPE;
      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
      return error_status;
  ...
  if (no_user_format)
      ...
      if (DB_IS_NULL (format_str))
	{
      ...
      result_len =
	DB_GET_STRING_LENGTH (format_str) * QSTR_EXTRA_LENGTH_RATIO;
...
#define FLOAT_MAX_SCALE 6
  ...
  switch (DB_VALUE_TYPE (src_value))
    ...
    case DB_TYPE_FLOAT:
      ...
      if (NO_ERROR != scientific_to_decimal_string (tmp_str, &cs))
	{
	  error_status = ER_OUT_OF_VIRTUAL_MEMORY;
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
	  return error_status;
  ...
  if (no_user_format)
    {
      ...
      DB_MAKE_VARCHAR (result_str, strlen (cs), cs, strlen (cs));
      result_str->need_clear = true;
      return error_status;
    }
  /* Else                            */
      ...
      while (format_str_ptr != last_format)
	{
	  ...
	  switch (cur_format)
	    ...
	    case N_FORMAT:
	      if (NO_ERROR !=
		  make_number_to_char (cs, format_str_ptr, &token_length,
				       &res_ptr))
		{
		  error_status = ER_QSTR_INVALID_FORMAT;
		  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, error_status, 0);
  ...
  DB_MAKE_VARCHAR (result_str, strlen (res_string), res_string,
		   strlen (res_string));
  result_str->need_clear = true;
  db_private_free_and_init (NULL, cs);
  return error_status;
		     ...
		     char *format_str, int *length, char **result_str)
{
  ...
  if (char_tolower (*format_str) == 'c')
	...
	case DB_CURRENCY_YEN:
	  *res_str = moneysymbols[0][0];
	  res_str++;
	  *res_str = moneysymbols[0][1];
	  break;
  ...
  *(res_str + (format - format_str)) = *format;
  ...
  if (*format == '.' && *num == '.')
      ...
      while (*format != '\0')
	{
	  ...
	  else			/* if((*format=='9' || *format=='0') && num=='\0' ) */
	    {
	      while (*format != '\0')
		{
		  if (*format == '9' || *format == '0')
      ...
      if (*num != '\0')
	{
	  ...
	  if (*num - '0' > 4)
	    {
	      if (NO_ERROR !=
		  roundoff (*result_str, 0, (int *) NULL, format_str))
		{
		  return !NO_ERROR;
		}
	    }
  ...
  else if (*format == '\0' && *num == '.')
    {
      if (*(num + 1) - '0' > 4)
	{
	  if (NO_ERROR != roundoff (*result_str, 0, (int *) NULL, format_str))
	    {
	      return !NO_ERROR;
	    }
	}
  ...
  *token_length = 0;
  ...
  if (strlen (token) >= 4 && strncasecmp (token, "eeee", 4) == 0 &&
      ...
      while (src < last_src && char_isdigit (*src))
	{
      ...
      if (NO_ERROR != scientific_to_decimal_string (result_str, &res_ptr))
	{
	  return ER_QSTR_MISMATCHING_ARGUMENTS;
		 ...
		 int *format_length, unsigned char **next_pos)
{
  ...
  *format_length = 0;
  ...
  switch (char_tolower (*sp))
    {
      /*      Case statements for DATE format begin{  */
    case 'y':
      ...
      else
	{
      ...
      /*      }end    Case statements for DATE format         */
      /*      Case statements for TIME || DATE format begin{          */
    case 'm':
      if ((str_type == DB_TYPE_DATE || str_type == DB_TYPE_TIMESTAMP) &&
	  qstr_length (sp) >= 2
      ...
      else
	{
      ...
      /*      }end Case statements for TIME || DATE format            */
      /*      Case statements for TIME format begin{          */
    case 'a':
      ...
      else
	{
      ...
      /*      }end    Case statements for TIME format         */
    case '"':
      *format_length += 1;
