...
do_alter_one_clause_with_template (PARSER_CONTEXT * parser, PT_NODE * alter)
{
  ...
  char *user_query;		/* to save the original user's view query */
  ...
  switch (alter_code)
      ...
      else
	{
      ...
      user_query = alter->info.alter.alter_clause.query.query->sql_view_text;
...
add_query_to_virtual_class (PARSER_CONTEXT * parser, DB_CTMPL * ctemplate, const PT_NODE * queries)
{
  ...
  parser->custom_print = save_custom;
  error = dbt_add_query_spec (ctemplate, query, queries->sql_view_text);
