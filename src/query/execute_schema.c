			  ...
			  const char *classname)
{
  ...
  if (has_self_ref)
	...
	{
	  PT_NODE *temp;
	  DB_VALUE *db_src_val, db_dest_val;
	  ...
	  original_type = def_val->type_enum;
	  data_type = attribute->data_type;

	  if (PT_HAS_COLLATION (original_type) && PT_HAS_COLLATION (desired_type)
	      && def_val->data_type != NULL && data_type != NULL
	      && def_val->data_type->info.data_type.collation_id != data_type->info.data_type.collation_id)
	    {
