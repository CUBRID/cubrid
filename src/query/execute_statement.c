...
do_check_delete_trigger (PARSER_CONTEXT * parser, PT_NODE * statement, PT_DO_FUNC * do_func)
{
  ...
  if (statement->bs.use_auto_commit)
    {
...
init_compile_context (PARSER_CONTEXT * parser)
{
  ...
  parser->context.is_xasl_pinned_reference = (bool) parser->bs.is_xasl_pinned_reference;
  parser->context.recompile_xasl_pinned = (bool) parser->bs.recompile_xasl_pinned;
}
  ...
  if (list_id)
      ...
      if (count > 0)
	{
	  ...
	  while (spec)
		...
		{
		  if (statement->bs.use_auto_commit && tran_was_latest_query_committed ())
		    {
...
do_prepare_update (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  for (err = NO_ERROR; statement && (err >= NO_ERROR); statement = statement->next)
      ...
      if (server_update)
	{
	  ...
	  /* make query string */
	  parser->bs.dont_prt_long_string = 1;
	  parser->bs.long_string_skipped = 0;
	  parser->bs.print_type_ambiguity = 0;
	  ...
	  parser->bs.dont_prt_long_string = 0;
	  if (parser->bs.long_string_skipped || parser->bs.print_type_ambiguity)
	    {
      ...
      if (server_update)
	{
	  ...
	  contextp->recompile_xasl = statement->bs.recompile;
	  if (statement->bs.recompile == 0)
	    {
...
do_execute_update (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  for (err = NO_ERROR, result = 0; statement && (err >= NO_ERROR); statement = statement->next)
      ...
      if (statement->info.update.server_update || !statement->info.update.do_class_attrs)
	{
	  ...
	  if (parser->bs.is_xasl_pinned_reference)
	    {
	  ...
	  if (spec->info.spec.flag & PT_SPEC_FLAG_UPDATE)
	    {
	      ...
	      if (class_obj && (statement->bs.use_auto_commit == false || !tran_was_latest_query_committed ()))
		{
...
do_prepare_delete (PARSER_CONTEXT * parser, PT_NODE * statement, PT_NODE * parent)
{
  ...
  if (err != NO_ERROR || (statement != NULL && statement->bs.cannot_prepare == 1))
    {
		    ...
		    int *row_count_ptr)
{
  ...
  *otemplate = NULL;
	  ...
	  *otemplate = dbt_create_object_internal (class_->info.name.db_object);
	  ...
	  if (*otemplate != NULL)
	      ...
	      else
		{
		  ...
		  include_new_obj = (parser->bs.return_generated_keys && (*otemplate)->is_autoincrement_set);
  ...
  if (*otemplate != NULL && parser->bs.return_generated_keys)
    {
    ...
    {
      if (!parser->bs.return_generated_keys && (*otemplate == NULL || value_clauses->next != NULL))
	{
			 ...
			 const char **savepoint_name)
{
		  ...
		  if (otemplate != NULL)
		    {
		      ...
		      include_new_obj = (obj && parser->bs.return_generated_keys && otemplate->is_autoincrement_set);
  ...
  if (parser->bs.return_generated_keys && seq != NULL)
    {
      value = db_value_create ();
      if (value == NULL)
	{
...
do_select_internal (PARSER_CONTEXT * parser, PT_NODE * statement, bool for_ins_upd)
{
  ...
  /* click counter check */
  if (statement->bs.is_click_counter)
    {
      CHECK_MODIFICATION_ERROR ();
    }
  ...
  if (parser->bs.dont_collect_exec_stats)
    {
...
do_execute_session_statement (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  if (statement->bs.clt_cache_check)
    {
      clt_cache_time = statement->cache_time;
      statement->bs.clt_cache_check = 0;
    }
  CACHE_TIME_RESET (&statement->cache_time);
  statement->bs.clt_cache_reusable = 0;
  ...
  if (CACHE_TIME_EQ (&clt_cache_time, &statement->cache_time))
    {
      statement->bs.clt_cache_reusable = 1;
    }
...
do_execute_select (PARSER_CONTEXT * parser, PT_NODE * statement)
{
  ...
  if (statement->bs.si_datetime == 1 || statement->bs.si_tran_id == 1)
    {
      statement->info.query.bs.reexecute = 1;
      statement->info.query.bs.do_not_cache = 1;
    }

  if (statement->info.query.bs.reexecute == 1)
    {
      query_flag |= NOT_FROM_RESULT_CACHE;
    }

  if (statement->info.query.bs.do_cache == 1)
    {
      query_flag |= RESULT_CACHE_REQUIRED;
    }

  if (statement->info.query.bs.do_not_cache == 1 || statement->info.query.oids_included)
    {
      query_flag |= RESULT_CACHE_INHIBITED;
    }
  if (parser->bs.is_holdable)
