 ...
 *
 * qp_xdata.c - The manipulation of data stored in the XASL nodes
 *
 ...
 *   do_tran_closure(in)        :
 * 
 * Note: Get the offspring of an object.
 *              Do transitive closure if asked for.  This is the work function 
 *              for get_children() and get_descendants().
 * 
 * args is an array of 6 DB_VALUES that have the following semantics:
 * 
 * 	The first DB_VALUE is the instance object that the function is
 * 	called on.  This object is the instance whose children or 
 * 	descendants we want.
 * 		
 * 	The second and third DB_VALUESs are sequences that associate 
 * 	class objects and attribute numbers.  Thus, the first object in the 
 * 	second DB_VALUE is used with the first attribute number in the 
 * 	third DB_VALUE.  Semantically, these class object/attribute number 
 * 	pairs indicate to the function which attributes of which classes 
 * 	contain the children we are interested in.  Each sequence can
 * 	have arbitrary length, but both must have the same cardinality.
 * 	Call these sequences the parent sequences.
 * 
 * 	The fourth and fifth DB_VALUEs are again sequences that 
 * 	associate class objects and attribute numbers.  Semantically, 
 * 	these class object/attribute number pairs determine how we
 * 	traverse the composition hierarchy.  Each sequence must have
 * 	a cardinality of four.  Call these sequences the child sequences.
 * 	
 * 	The sixth DB_VALUE is a sequence of class objects.  Semantically,
 * 	this sequence controls which child objects are placed into
 * 	the result sequence.  For an object to be entered into the 
 * 	result sequence, its class must be present in the sequence.
 * 	This sequence can have arbitrary cardinality.  Call this sequence 
 * 	the result class sequence.
 * 
 */
static int
