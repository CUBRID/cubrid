...
/*
 *  new XASL stream format :=
 *      [size of header data][header data][size of body data][body data]
 *       (int)           (char ...)   (int)         (char ...)
 *  header data :=
 *      [creator_oid][n_oid_list][class_oid_list][repr_id_list][dbval_cnt]
 *       (OID)        (int)  (OID ...)       (int ...)(int)
 *  body data :=
 *      original XASL stream
*/
...
/* vals_cnt is an Opal addition to facilitate the computation of the number
 * of fetchees that can fit in a network page. It is not an incompatible
 * addition because an Opal server never reads nor restores vals_cnt.
 */
...
struct xasl_node
{
  ...
  int iscan_order;		/* keep index scan order? */
