...
unsigned int
mht_get_hash_number_with_tuple (unsigned int ht_size, char *value, int value_size, DB_TYPE type, int precision)
{
  OR_BUF buf;
  unsigned int hash = 0;
  int error = NO_ERROR;

  or_init (&buf, value, value_size);

  if (ht_size <= 1 || value == NULL || value_size <= 0)
    {
      hash = 0;
    }
  ...
  switch (type)
    ...
    case DB_TYPE_SMALLINT:
      {
	unsigned int x;
	x = or_get_short (&buf, &error);
	hash = mht_get_shiftmult32 (x, ht_size);
	break;
      }
	...
	DB_BIGINT bigint;
	unsigned int x, y;
	bigint = or_get_bigint (&buf, &error);
	x = bigint >> 32;
	y = (unsigned int) bigint;
	hash = mht_get_shiftmult32 (x ^ y, ht_size);
	break;
      }
	...
	float f;
	unsigned int *x, y;
	f = or_get_float (&buf, &error);
	x = (unsigned int *) &f;
	y = (*x) & 0xFFFFFFF0;
	hash = mht_get_shiftmult32 (y, ht_size);
	break;
      }
	...
	double d;
	unsigned int *x, y, z;
	d = or_get_double (&buf, &error);
	x = (unsigned int *) &d;
	y = (x[0]) & 0xFFFFFFF0;
	z = (x[1]) & 0xFFFFFFF0;
	hash = mht_get_shiftmult32 (y ^ z, ht_size);
	break;
      }
    ...
    case DB_TYPE_NUMERIC:
      {
	unsigned int *x;
	assert ((buf.ptr + DB_NUMERIC_BUF_SIZE) <= buf.endptr);
	x = (unsigned int *) buf.ptr;
	hash = mht_get_shiftmult32 (x[0] ^ x[1] ^ x[2] ^ x[3], ht_size);
      }
      break;
    ...
    case DB_TYPE_BIT:
      hash = MHT2STR_COLL (LANG_COLL_ISO_BINARY, (unsigned char *) buf.ptr, BITS_TO_BYTES (precision));
      hash %= ht_size;
      break;
	...
	hash = MHT2STR_COLL (LANG_COLL_ISO_BINARY, (unsigned char *) buf.ptr, BITS_TO_BYTES (size));
	hash %= ht_size;
	break;
      }
