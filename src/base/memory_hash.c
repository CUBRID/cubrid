...
unsigned int
mht_get_hash_number_with_tuple (const unsigned int ht_size, const char *tuple, const int tuple_size, const DB_TYPE type)
{
  unsigned int hashcode = 0;
  int part = 0;
  int i;
  ...
  if (ht_size <= 1 || tuple == NULL || tuple_size <= 0)
    {
      hashcode = 0;
    }
  else
	...
	{
	case DB_TYPE_SMALLINT:
	  hashcode = mht_get_shiftmult32 (OR_GET_SHORT (tuple), ht_size);
	  break;

	case DB_TYPE_INTEGER:
	  hashcode = mht_get_shiftmult32 (OR_GET_INT (tuple), ht_size);
	  break;
	...
	case DB_TYPE_NUMERIC:
	  {
	    unsigned int *buf = (unsigned int *) tuple;
	    hashcode = mht_get_shiftmult32 (buf[0] ^ buf[1] ^ buf[2] ^ buf[3], ht_size);
	  }
	  break;
	...
	case DB_TYPE_DATETIMETZ:
	  {
	    DB_DATETIME datetime;
	    OR_GET_DATETIME (tuple, &datetime);
	    hashcode = mht_get_shiftmult32 (datetime.date ^ datetime.time, ht_size);
	    break;
	  }
	...
	case DB_TYPE_OID:
	  {
	    OID oid;
	    OR_GET_OID (tuple, &oid);
	    unsigned int x = (oid.volid << 16) | (oid.slotid);
	    unsigned int y = oid.pageid;
	...
	case DB_TYPE_ENUMERATION:
...
#if 0
	  {
	    ...
	    int rc;
	    int size_prefix;
	    int compressed_size, expected_decompressed_size;
	    or_init (&buf, tuple, tuple_size);
	    size_prefix = or_get_string_size_byte (buf, &rc);
	    if (size_prefix == OR_MINIMUM_STRING_LENGTH_FOR_COMPRESSION)
	      {
		compressed_size = or_get_int (buf, &rc);
		expected_decompressed_size = or_get_int (buf, &rc);
		hashcode = MHT2STR_COLL (domain->char_info.collation_id, (unsigned char *) buf.ptr, i);
	      }
	    else
	      {
		hashcode = MHT2STR_COLL (domain->char_info.collation_id, (unsigned char *) buf.ptr,);
	      }
	  ...
	  {
	    bit_length = BITS_TO_BYTES (OR_GET_INT (tuple, &rc));
	    hashcode = MHT2STR_COLL (0, (unsigned char *) tuple + OR_INT_SIZE, i);
	    hashcode %= ht_size;
	  }
#endif
	...
	default:
	  hashcode = mht_2str_pseudo_key (tuple, tuple_size) % ht_size;
	  break;
	}
    }
