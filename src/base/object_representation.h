...
typedef struct or_midxkey OR_MIDXKEY;
struct or_midxkey
{
private:
  OR_BUF * m_or_buf;
...
public:
  ...
  void init (OR_BUF * or_buf, const int ncolumns);

  void set_header_size (const int ncolumns);
  void set_buffer (OR_BUF * or_buf);
  void set_nullmap (const int index);
  void set_offset (const int index);

  void clear_header ();
  void clear_nullmap (const int index);

  void seek (int offset);

  int get_header_size ();
  OR_BUF *get_buffer ();
  int get_offset (const int index);
  int get_current_offset ();

  bool has_null ();
  bool is_null ();
  bool is_null_with_index (const int index);
};
...
inline void
OR_MIDXKEY::init (OR_BUF * or_buf, const int ncolumns)
{
  set_header_size (ncolumns);
  set_buffer (or_buf);
}

inline void
OR_MIDXKEY::set_header_size (const int ncolumns)
{
  assert (ncolumns > 0);
  /* OR_MULTI_BOUND_BIT_BYTES */
  m_nullmap_size = ((ncolumns + 7) >> 3);
  assert (m_nullmap_size > 0);
  m_offset_size = ncolumns + 1;
  m_header_size = m_nullmap_size + m_offset_size;
}

inline void
OR_MIDXKEY::set_buffer (OR_BUF * or_buf)
{
  assert (m_nullmap_size > 0);
  assert (m_offset_size > 0);
  assert (m_header_size > 0);
  assert (or_buf != NULL);
  assert (or_buf->endptr == static_cast < char *>(OR_INFINITE_POINTER)
	  || (or_buf->endptr - or_buf->buffer) >= m_header_size);
  m_or_buf = or_buf;
  m_nullmap_ptr = m_or_buf->buffer;
  m_offset_ptr = m_nullmap_ptr + m_nullmap_size;
  m_or_buf->ptr += m_header_size;
}

inline void
OR_MIDXKEY::set_nullmap (const int index)
{
  assert (m_nullmap_ptr != NULL);
  assert (index >= 0);
  /* OR_ENABLE_BOUND_BIT */
...
inline void
OR_MIDXKEY::set_offset (const int index)
{
  assert (m_or_buf != NULL);
  assert (m_offset_ptr != NULL);
  assert (index >= 0);
  int offset = CAST_BUFLEN (m_or_buf->ptr - m_or_buf->buffer);
  assert (offset > 0);
  if (offset < OR_MIDXKEY_MAX_OFFSET_SIZE)
    ...
    {
      OR_PUT_BYTE (m_offset_ptr + index, OR_MIDXKEY_MAX_OFFSET_SIZE);
    }
}
...
OR_MIDXKEY::clear_header ()
{
  assert (m_nullmap_ptr != NULL);
  assert (m_header_size > 0);
  /* MIDXKEY_BOUNDBITS_INIT */
  memset (m_nullmap_ptr, 0x00, m_header_size);
}

inline void
OR_MIDXKEY::clear_nullmap (const int index)
{
  assert (m_nullmap_ptr != NULL);
  assert (index >= 0);
  /* OR_CLEAR_BOUND_BIT */
...
inline void
OR_MIDXKEY::seek (int offset)
{
  assert (m_or_buf != NULL);
  assert ((m_or_buf->buffer + offset) <= m_or_buf->endptr);
  assert (offset >= 0);
  m_or_buf->ptr = m_or_buf->buffer + offset;
}

inline int
OR_MIDXKEY::get_header_size ()
{
  assert (m_header_size > 0);
...
inline int
OR_MIDXKEY::get_offset (const int index)
{
  assert (m_offset_ptr != NULL);
  assert (index >= 0);
  int offset = OR_GET_BYTE (m_offset_ptr + index);
  assert (offset > 0);
  if (offset < OR_MIDXKEY_MAX_OFFSET_SIZE)
    {
      return offset;
...
OR_MIDXKEY::has_null ()
{
  assert (m_nullmap_ptr != NULL);
  assert (m_nullmap_size > 0);
  for (int i = 0; i < m_nullmap_size; i++)
    {
      if ((~(*(m_nullmap_ptr + i))) & OR_MIDXKEY_NULLMAP_MASK)
	{
	  return true;
	}
    }
...
inline bool
OR_MIDXKEY::is_null_with_index (const int index)
{
  assert (m_nullmap_ptr != NULL);
  assert (m_nullmap_size > 0);
  assert (index >= 0);
