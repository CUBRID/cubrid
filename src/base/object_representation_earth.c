 ...
 *
 * orsr.c - Class representation parsing for the server only
 * 
			 ...
			 int *num_classes)
{
  ...
  *hfids = NULL;
  ...
  attr_name
    = diskatt + OR_VAR_TABLE_ELEMENT_OFFSET (diskatt, ORC_ATT_NAME_INDEX);

  if (!found ||
      (OR_VAR_TABLE_ELEMENT_LENGTH (diskatt, ORC_ATT_NAME_INDEX) == 0) ||
      (or_get_hierarchy_helper (thread_p, &source_class, &source_class, btid,
 ...
 * 
 * Note:  Kludge, we need to be able to extract fully specified domains from
 *        the disk representation of classes but we're not in a position right    
 *        now to share code with the transformer (tfcl).                          
 *                                                                            
 *        This makes further use of lots of magic ORC_ constants that must        
 *        be kept in sync with the META_CLASS definitions in tf.c which is        
 *        annoying.  It might be nice if the META_CLASS definitions could move    
 *        to the server so we wouldn't have to rely on so many hard coded         
 *        constants.                                                              
 *                                                                            
 *        Unlike the client transformer, we don't bother checking for buffer      
 *        underflow here.  Underflow can only happen if the disk representation   
 *        is corrupted which is very rare and if present, it would have been      
 *        caught by the client by now.                                            
 *                                                                            
 *        Since orsr_ doesn't have any dependencies, we could move this           
 *        to the client and use it in tfcl.c but not right now. 
 */
static TP_DOMAIN *
 ...
 * 
 * Note: This was written for orsr.c but could be used in other cases if 
 *       you're careful. 
