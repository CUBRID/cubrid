...
/*
 * qo_plan_estimated_leaf_pages () - Estimate the number of B+tree leaf pages
 *   that an index scan is expected to read, using the same selectivity model
 *   as qo_iscan_cost. Used by the parallel-scan cost gate to decide whether
 *   the index is large enough to benefit from worker parallelism.
 *   return: ceil (sel * cum_statsp->leafs), or -1 when the estimate cannot
 *           be computed (non-iscan plan, or missing leaf statistics).
 *   plan(in): index-scan QO_PLAN
 */
int
qo_plan_estimated_leaf_pages (QO_PLAN * plan)
{
  QO_NODE *nodep;
  QO_NODE_INDEX_ENTRY *ni_entryp;
  QO_ATTR_CUM_STATS *cum_statsp;
  ...
  nodep = plan->plan_un.scan.node;
  ni_entryp = plan->plan_un.scan.index;
  if (ni_entryp == NULL || ni_entryp->head == NULL)
    {
      return -1;
    }
  index_entryp = ni_entryp->head;
  ...
  if (cum_statsp->leafs <= 0)
    {
      /* statistics not collected - bypass the gate */
      return -1;
    }

  n = index_entryp->col_num;

  /* selectivity of the index terms (mirror of qo_iscan_cost) */
  sel = 1.0;
  pkeys_num = MIN (n, cum_statsp->pkeys_size);
  ...
  return (int) ceil (sel * (double) cum_statsp->leafs);
}
