...
public boolean getMoreResults() throws SQLException
{
  try {
	...
	if (result_type) {
	  ...
	  if (type == ResultSet.TYPE_SCROLL_SENSITIVE && u_stmt.isOIDIncluded() == false)
		  rs_type = ResultSet.TYPE_SCROLL_INSENSITIVE;
	  if (concurrency == ResultSet.CONCUR_UPDATABLE && u_stmt.isOIDIncluded() == false)
		  rs_concurrency = ResultSet.CONCUR_READ_ONLY;
	  current_result_set = new CUBRIDResultSet(con, this, rs_type, rs_concurrency);
	}
	else {
 ...
 * the object inserted by the given query.
 *
...
public byte getStatementType()
{
  if (u_stmt != null)
    ...
    return CUBRIDCommandType.SQLX_CMD_UNKNOWN;
}
...
protected void executeCoreInternal(boolean all, UStatementCacheData cache_data) throws SQLException
{
  ...
  u_stmt.execute(false, max_rows, max_field_size, all, is_sensitive, is_scrollable, query_info_flag, only_query_plan, cache_data);
...
{
  byte prepareFlag;
  prepareFlag = (is_updatable || is_sensitive) ?
		UConnection.PREPARE_UPDATABLE : (byte) 0;
