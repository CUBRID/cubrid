...
/*
  ...
  bSize[0] = (byte) ((length >>> 24) & 0xFF);
  bSize[1] = (byte) ((length >>> 16) & 0xFF);
  bSize[2] = (byte) ((length >>> 8) & 0xFF);
  bSize[3] = (byte) ((length >>> 0) & 0xFF);
  outStream.write(bSize);
  outStream.flush();
*/
...
private void writeCollection(CUBRIDArray data)
{
  ...
  switch (data.getBaseType()) {
    ...
    case UUType.U_TYPE_VARBIT :
	{
	  ...
	  if (objectValues != null && objectValues instanceof byte[][])
		...
		byteValues[i][0] = (((Boolean[]) objectValues)[i].booleanValue() == true) ? (byte) 1: (byte) 0;
	      }
	      else
		byteValues[i] = null;
	    }
  ...
  overwriteInt(collection_size,
	       collection_size_msg_buffer_cursor,
	       collection_size_msg_buffer_position);
}

private void overwriteInt(int data, int msg_buffer_cursor, int msg_buffer_position)
{
