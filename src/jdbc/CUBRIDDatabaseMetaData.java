...
public synchronized String getDriverVersion() throws SQLException
{
  checkIsOpen();
  String s;
  s = "" + CUBRIDDriver.major_version + "." +
      CUBRIDDriver.minor_version + "." +
...
{
  checkIsOpen();
  if (level == Connection.TRANSACTION_READ_COMMITTED) return true;
  if (level == Connection.TRANSACTION_READ_UNCOMMITTED) return true;
  if (level == Connection.TRANSACTION_REPEATABLE_READ) return true;
  if (level == Connection.TRANSACTION_SERIALIZABLE) return true;
  if (level == CUBRIDConnection.TRAN_REP_CLASS_COMMIT_INSTANCE) return true;
  if (level == CUBRIDConnection.TRAN_REP_CLASS_UNCOMMIT_INSTANCE) return true;
  return false;
}
...
public synchronized ResultSet getProcedureColumns(String catalog, String schemaPattern, String procedureNamePattern, String columnNamePattern) throws SQLException
{
  ...
  int[] types = {UUType.U_TYPE_VARCHAR, UUType.U_TYPE_VARCHAR,
		 ...
		 UUType.U_TYPE_VARCHAR, UUType.U_TYPE_INT,
		 UUType.U_TYPE_INT, UUType.U_TYPE_SHORT,
		 UUType.U_TYPE_SHORT, UUType.U_TYPE_SHORT,
...
public synchronized ResultSet getTables(String catalog, String schemaPattern, String tableNamePattern, String[] types) throws SQLException
{
  ...
  CUBRIDResultSetWithoutQuery rs = new CUBRIDResultSetWithoutQuery(5, type, names, nullable);

  // schema를 가져온다.
  UStatement us = null;
  synchronized (u_con) {
    us = u_con.getSchemaInfo(USchType.SCH_CLASS, tableNamePattern, null, (byte)3);
    error = u_con.getRecentError();
    switch (error.getErrorCode()) {
    case UErrorCode.ER_NO_ERROR :
	break;
...
public synchronized ResultSet getTableTypes() throws SQLException
{
  ...
  CUBRIDResultSetWithoutQuery rs = new CUBRIDResultSetWithoutQuery(1, types, names, nullable);
...
public synchronized ResultSet getColumns(String catalog, String schemaPattern, String tableNamePattern, String columnNamePattern) throws SQLException
{
  ...
  int[] types = {UUType.U_TYPE_VARCHAR, UUType.U_TYPE_VARCHAR,
		 ...
		 UUType.U_TYPE_INT, UUType.U_TYPE_NULL,
		 UUType.U_TYPE_INT, UUType.U_TYPE_INT,
  ...
  CUBRIDResultSetWithoutQuery rs = new CUBRIDResultSetWithoutQuery(18, types, names, nullable);

  // schema를 가져온다.
  UStatement us = null;
  synchronized (u_con) {
    us = u_con.getSchemaInfo(USchType.SCH_ATTRIBUTE, tableNamePattern, columnNamePattern, (byte)3);
    error = u_con.getRecentError();
    switch (error.getErrorCode()) {
    case UErrorCode.ER_NO_ERROR :
	break;
...
public synchronized ResultSet getColumnPrivileges(String catalog, String schema, String table, String columnNamePattern) throws SQLException
{
  ...
  CUBRIDResultSetWithoutQuery rs = new CUBRIDResultSetWithoutQuery(8, types, names, nullable);

  UStatement us = null;
  synchronized (u_con) {
    us = u_con.getSchemaInfo(USchType.SCH_ATTR_PRIVILEGE, table, columnNamePattern, (byte)2);
    error = u_con.getRecentError();
    switch (error.getErrorCode()) {
    case UErrorCode.ER_NO_ERROR :
	break;
...
public synchronized ResultSet getTablePrivileges(String catalog, String schemaPattern, String tableNamePattern) throws SQLException
{
  ...
  CUBRIDResultSetWithoutQuery rs = new CUBRIDResultSetWithoutQuery(7, types, names, nullable);

  UStatement us = null;
  synchronized (u_con) {
    us = u_con.getSchemaInfo(USchType.SCH_CLASS_PRIVILEGE, tableNamePattern, null, (byte)3);
    error = u_con.getRecentError();
    switch (error.getErrorCode()) {
    case UErrorCode.ER_NO_ERROR :
	break;
...
public synchronized ResultSet getBestRowIdentifier(String catalog, String schema, String table, int scope, boolean nullable) throws SQLException
{
  ...
  String[] names = {"SCOPE", "COLUMN_NAME", "DATA_TYPE",
...
public synchronized ResultSet getTypeInfo() throws SQLException
{
  ...
  int[] types = {UUType.U_TYPE_VARCHAR, UUType.U_TYPE_SHORT,
		 UUType.U_TYPE_INT, UUType.U_TYPE_VARCHAR,
		 UUType.U_TYPE_VARCHAR, UUType.U_TYPE_VARCHAR,
		 UUType.U_TYPE_SHORT, UUType.U_TYPE_BIT,
		 UUType.U_TYPE_SHORT, UUType.U_TYPE_BIT,
		 UUType.U_TYPE_BIT, UUType.U_TYPE_BIT,
		 UUType.U_TYPE_VARCHAR, UUType.U_TYPE_SHORT,
		 UUType.U_TYPE_SHORT, UUType.U_TYPE_INT,
		 UUType.U_TYPE_INT, UUType.U_TYPE_INT};
  boolean[] nullable = {false, false, false, true, true, true,
  			false, false, false, false, false, false,
  ...
  Object[] column1 = {"BIT", "NUMERIC", "NUMERIC", "BIT VARYING",
		      "BIT VARYING", "BIT", "VARCHAR", "CHAR",
		      "NUMERIC", "INTEGER", "SMALLINT", "DOUBLE",
		      "FLOAT", "DOUBLE", "VARCHAR", "DATE",
		      "TIME", "TIMESTAMP"};
  ...
  Object[] column3 = {new Integer(8), new Integer(3),
		      new Integer(38), new Integer(1073741823),
		      new Integer(1073741823), new Integer(1073741823),
		      new Integer(1073741823), new Integer(1073741823),
		      new Integer(38), new Integer(10),
		      new Integer(5), new Integer(38),
		      new Integer(38), new Integer(38),
		      new Integer(1073741823), new Integer(10),
		      new Integer(11), new Integer(22)};
  Object[] column4 = {"B'", null, null, "X'", "X'", "X'",
		      "'", "'", null, null, null, null,
		      null, null, "'", "DATE'", "TIME'", "TIMESTAMP'"};
  Object[] column5 = {"'", null, null, "'", "'", "'", "'", "'", null,
		      null, null, null, null, null, "'", "'", "'", "'"};
  Object[] column6 = {"(8)", "(3)", null, null, null, null, null, null, null,
...
public synchronized ResultSet getIndexInfo(String catalog, String schema, String table, boolean unique, boolean approximate) throws SQLException
{
  ...
  CUBRIDResultSetWithoutQuery rs = new CUBRIDResultSetWithoutQuery(13, types, names, nullable);
...
{
  checkIsOpen();
  if (type == ResultSet.TYPE_FORWARD_ONLY && concurrency == ResultSet.CONCUR_READ_ONLY) return true;
  if (type == ResultSet.TYPE_FORWARD_ONLY && concurrency == ResultSet.CONCUR_UPDATABLE) return true;
  if (type == ResultSet.TYPE_SCROLL_INSENSITIVE && concurrency == ResultSet.CONCUR_READ_ONLY) return true;
  if (type == ResultSet.TYPE_SCROLL_INSENSITIVE && concurrency == ResultSet.CONCUR_UPDATABLE) return true;
  if (type == ResultSet.TYPE_SCROLL_SENSITIVE && concurrency == ResultSet.CONCUR_READ_ONLY) return true;
  if (type == ResultSet.TYPE_SCROLL_SENSITIVE && concurrency == ResultSet.CONCUR_UPDATABLE) return true;
  return false;
}
...
public synchronized ResultSet getAttributes(String catalog, String schemaPattern, String typeNamePattern, String attributeNamePattern) throws SQLException
{
  ...
  String[] names = {"TYPE_CAT", "TYPE_SCHEM", "TYPE_NAME",
		    "ATTR_NAME", "DATA_TYPE", "ATTR_TYPE_NAME",
		    "ATTR_SIZE", "DECIMAL_DIGITS", "NUM_PREC_RADIX",
		    "NULLABLE", "REMARKS", "ATTR_DEF",
  ...
  int[] types = {UUType.U_TYPE_VARCHAR, UUType.U_TYPE_VARCHAR,
		 ...
		 UUType.U_TYPE_VARCHAR, UUType.U_TYPE_VARCHAR,
		 UUType.U_TYPE_SHORT};
  boolean[] nullable = {true, true, false, false, false, false, false, false,
...
{
  checkIsOpen();
  if (holdability == ResultSet.CLOSE_CURSORS_AT_COMMIT) return true;
...
public synchronized  ResultSet getSuperTables(String catalog, String schemaPattern, String tableNamePattern) throws  SQLException
{
  ...
  CUBRIDResultSetWithoutQuery rs = new CUBRIDResultSetWithoutQuery(4, types, names, nullable);

  UStatement us = null;
  synchronized (u_con) {
    us = u_con.getSchemaInfo(USchType.SCH_DIRECT_SUPER_CLASS, tableNamePattern, null, (byte)3);
    error = u_con.getRecentError();
    switch (error.getErrorCode()) {
    case UErrorCode.ER_NO_ERROR :
	break;
...
} // end of class CUBRIDDatabaseMetaData
