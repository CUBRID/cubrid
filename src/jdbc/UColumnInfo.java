...
private final static byte GLO_INSTANCE_FALSE = 0,
			  GLO_INSTANCE_TRUE = 1,
...
/*
 * column value가 Driver를 통해 end-user에게 전해지는 value의 FQDN
 * (Fully Qulified Domain Name)을 return한다.
 */
...
/*
 * collection type의 경우 server쪽에서 넘어오는 type정보가 collection type정보와
 * collection base type정보가 logical or로 조합되어 넘어온다.
 * 이 method는 server쪽으로부터 넘어온 type정보를 읽고 collection type정보와
 * collection base type정보를 따로 추출하여 array로 넘겨준다.
 * (index 0 : collection type, index 1 : collection base type)
 */
