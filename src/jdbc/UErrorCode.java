...
/* Cas 구현되지 않았음.(현재상태 : set data tye에 대해 클라이언트에서 서버로 데이터 전송하는 경우) */
...
private static void setMessageHash()
{
  ...
  messageString.put(new Integer(ER_UNKNOWN),
  		    "Error");
  messageString.put(new Integer(ER_NO_ERROR),
  		    "No Error");
  messageString.put(new Integer(ER_DBMS),
  		    "Server error");
  ...
  messageString.put(new Integer(ER_FILE),
  		    "File access failed");
  ...
  messageString.put(new Integer(ER_NO_MORE_RESULT),
  		    "No More Result");
...
private static void setCASMessageHash()
{
  ...
  CASMessageString.put(new Integer(CAS_ER_COMMUNICATION),
		       "Communication error");
  CASMessageString.put(new Integer(CAS_ER_ARGS),
		       "Invalid argument");
  ...
  CASMessageString.put(new Integer(CAS_ER_NO_MORE_DATA),
		       "No more data");
  CASMessageString.put(new Integer(CAS_ER_OBJECT),
		       "Object is not valid");
  CASMessageString.put(new Integer(CAS_ER_OPEN_FILE),
		       "File open error");
  CASMessageString.put(new Integer(CAS_ER_SCHEMA_TYPE),
		       "Invalid schema type");
  CASMessageString.put(new Integer(CAS_ER_VERSION),
		       "Version mismatch");
  ...
  CASMessageString.put(new Integer(CAS_ER_IS),
		       "Authentication failure");
}

} // end of class UErrorCode
