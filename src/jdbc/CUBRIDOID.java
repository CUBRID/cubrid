...
/*=======================================================================
 ...
 =======================================================================*/
	...
	/*
	 * Just for Driver's uses.
	 * DO NOT create an object with this constructor!
	 */
...
/**
 * <code>this</code> object가 가리키고 있는 database의 object로부터
 * 주어진 attribute들의 값들을 가져온다. <code>attrNames</code>가
 * <code>null</code> 이면 모든 attribute들의 값을 가져온다.
 *
 * @param attrNames 값을 가져오고자 하는 attribute들의 이름을 담은 array
 * @return 1개의 row에 <code>attrNames.length</code>개 또는 모든 column을 담은
 * <code>CUBRIDResultSet</code> object가 return된다.
 * column 의 순서는 attrNames에 저장된 column 이름들의 순서와 같다.
...
/**
 * <code>this</code> object가 가리키고 있는 database의 object의 값을
 * 주어진 값으로 수정한다.
 *
 * @param attrNames 값을 수정하고자 하는 attribute들의 이름을 담은 array
 * @param values attribute들의 이름에 해당하는 값을 담은 array
 * @exception IllegalArgumentException if <code>attrNames</code> is
 * <code>null</code>, <code>values</code> is <code>null</code> or
 * <code>attrNames.length</code> is not same with <code>values.length</code>
 * @exception SQLException if type conversion fails
 * @exception SQLException if a database access error occurs
 */
...
/**
 * <code>this</code> object가 가리키고 있는 database의 object의
 * read lock을 set한다. 다른 transaction에 의해 write lock이 set되어 있을때에는
 * reset될 때까지 block된다.
 *
 * @exception SQLException if a database access error occurs
 */
...
/**
 * <code>this</code> object가 가리키고 있는 database의 object의
 * write lock을 set한다. 다른 transaction에 의해 read lock이나 write lock이
 * set되어 있을때에는 reset될 때까지 block된다.
 *
 * @exception SQLException if a database access error occurs
 */
...
/**
 * <code>this</code> object가 가리키고 있는 database의 GLO object로부터
 * 값을 읽어와서 stream으로 보낸다.
 *
 * @param stream 값을 내보낼 <code>OutputStream</code> object
 ...
 *
 * @param stream 값을 읽어들일 <code>InputStream</code> object
 * @exception IllegalArgumentException if <code>stream</code> is <code>null</code>
 * @exception SQLException if a database access error occurs
 */
...
/**
 * <code>this</code> object가 가리키고 있는 object의 set column에
 * 값을 추가한다.
 *
 * @param attrName 값을 추가하고자 하는 set column의 이름
 * @param value 추가하고자 하는 값
 * @exception IllegalArgumentException if <code>attrName</code> is <code>null</code>
 * @exception SQLException if a database access error occurs
 */
...
/**
 * <code>this</code> object가 가리키고 있는 object의 set column에서
 * 값을 삭제한다.
 *
 * @param attrName 값을 삭제하고자 하는 set column의 이름
 * @param value 삭제하고자 하는 값
 * @exception IllegalArgumentException if <code>attrName</code> is <code>null</code>
 * @exception SQLException if a database access error occurs
 */
...
/**
 * <code>this</code> object가 가리키고 있는 object의 sequence column에
 * 값을 추가한다.
 *
 * @param attrName 값을 추가하고자 하는 sequence column의 이름
 * @param index 값이 추가될 위치(0-based)
 * @param value 추가하고자 하는 값
 * @exception IllegalArgumentException if <code>attrName</code> is <code>null</code>
 * @exception SQLException if a database access error occurs
 */
 ...
 * 값을 수정한다.
 *
 * @param attrName 값을 수정하고자 하는 sequence column의 이름
 * @param index 값이 수정될 위치(0-based)
 * @param value 수정하고자 하는 값
 * @exception IllegalArgumentException if <code>attrName</code> is <code>null</code>
 * @exception SQLException if a database access error occurs
 */
 ...
 * 값을 삭제한다.
 *
 * @param attrName 값을 삭제하고자 하는 sequence column의 이름
 * @param index 값이 삭제될 위치(0-based)
 * @exception IllegalArgumentException if <code>attrName</code> is <code>null</code>
 * @exception SQLException if a database access error occurs
 */
...
/**
 * <code>this</code> object가 가지고 있는 oid string값을 return한다.
 * oid string의 format은 "@page_id|slot_id|volumn_id"이다.
 *
 ...
 * 생성한다.
 *
 * @param con 사용하고자 하는 <code>CUBRIDConnection</code> object
 * @param oidStr oid string값
 * @return 생성된 <code>CUBRIDOID</code> object
 * @exception IllegalArgumentException if either <code>con</code> or
 * <code>oidStr</code> is <code>null</code>
 * @exception SQLException if a database access error occurs
 */
...
public synchronized int gloRead(long pos, int length, byte[] dataBuffer, int buf_offset) throws SQLException
{
  checkIsOpen();
  ...
  while (length > 0) {
    ...
    real_read_len = gloCmd(UConnection.GLO_CMD_READ_DATA, pos, read_len, dataBuffer, buf_offset);
...
public synchronized long gloBinarySearch(long start, byte[] pattern, int offset, int len) throws SQLException
{
  checkIsOpen();

  int position;
  len = Math.min(GLO_MAX_SEARCH_LEN, len);
  position = gloCmd(UConnection.GLO_CMD_BINARY_SEARCH, start-1, len, pattern, offset);
