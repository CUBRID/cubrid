...
* manage하는 class이다. 모든 statement는 UStatement로 표현되며 input parameter를
* 가지고 있어 같은 format의 statement를 reuse할 수 있다.
*
* Execute하기 전에 모든 input parameter는 bind되어야 한다.
...
* 때에는 Error를 Set한다.
* prepare할 때 parameter의 type 정보를 알 수 없으므로 bind value를 확인하고
* 그 type에 해당하는 CUBRID Type정보를 match하여 parameter정보를 set한다. 따라서
* user는 parameter를 set할 때 parameter의 type에 대한 정보를 알고 있어야 한다.
*
* since 1.0
*/
...
/* statement를 execute할 때 사용되는 flag이다.
 *   NORMAL_EXECUTE는 synchronous execute를
 *   ASYNC_EXECUTE는 asynchronous execute를 의미한다. */

private final static byte NORMAL_EXECUTE=0, ASYNC_EXECUTE=1;
private final static byte NORMAL=0, GET_BY_OID=1, GET_SCHEMA_INFO=2, GET_AUTOINCREMENT_KEYS=3;
...
/* 3.0
private int resultset_index;
private int resultset_index_flag;
private UParameterInfo parameterInfo[];
private int pramNumber;
*/
...
/* normal statement를 위한 UStatement constructor
*   ResultSet의 column정보를 Fetch한다. */

UStatement(UConnection relatedC, UInputBuffer inBuffer, boolean assign_only, String sql, byte _prepare_flag) throws IOException, UJciException
{
...
private void init(UConnection relatedC, UInputBuffer inBuffer, String sql, byte _prepare_flag, boolean clear_bind_info) throws IOException, UJciException
{
  sql_stmt = sql;
  prepare_flag = _prepare_flag;
  ...
  if (commandTypeIs == CUBRIDCommandType.SQLX_CMD_CALL_SP)
    columnNumber = parameterNumber + 1;

/* 3.0
  resultset_index = 0;
  resultset_index_flag = java.sql.Statement.CLOSE_CURRENT_RESULT;
*/
}
...
//  oidString = oString;
  ...
  className = inBuffer.readString(inBuffer.readInt(), relatedConnection.conCharsetName);
  columnNumber = inBuffer.readInt();
  firstColumnInfo = readColumnInfo(inBuffer);
  columnInfo = firstColumnInfo;
  fetchSize = 1;
...
UStatement(UConnection relatedC, String cName, String attributePattern,int type, UInputBuffer inBuffer)
	throws IOException, UJciException
{
  outBuffer = relatedC.outBuffer;
...
/*  index번째 paramenter에 boolean type value를 set한다.
 *  CUBRID은 boolean type을 지원하지 않으므로 bit(1) type으로 match한다.
 *  다만 현재 bit(1) type manage에 문제가 있어 bit(8) type으로 간주한다.
 */
...
/* index번째 paramenter에 byte type value를 set한다.
 * byte type value는 Database의 tinyint type에 match되어야 하나 CUBRID에서는
 * tinyint를 지원하지 않으므로 smallint type으로 match한다.
 */
...
/* index번째 paramenter에 byte[] type value를 set한다.
 *   CUBRID의 Bit Varying이나 Bit(n)과 match되지만 parameter의 type 정보를 알 수
 *   없으므로 Bit Varying type으로 match시킨다.
 */
 ...
 * 만들어진 UConnection object는 cancel이 끝나면 close된다.
 * Error Code : ER_IS_CLOSED, ER_ILLEGAL_DATA_SIZE, ER_CONNECTION
 */
 ...
 * clearBatch가 불린 후 parameter array cursor는 1번째 element에 놓이며
 * 어떠한 parameter도 binding되어 있지 않게 된다.
 * Error Code : ER_IS_CLOSED
 */
 ...
 * CAS handler또한 close시킨다.
 * Memory garbage를 정리하기 위해 garbage collector를 call한다.
 * Error Code : ER_IS_CLOSED, ER_ILLEGAL_DATA_SIZE, ER_COMMUNICATION
 */
 ...
 * 경우 ER_OID_IS_NOT_INCLUDED error가 set된다.
 * Error Code :
...
synchronized public boolean cursorIsInstance(int cursor)
{
  ...
  if ((currentFirstCursor < 0) || (cursor < 0) ||
      (cursor >= 0 && ((cursor < currentFirstCursor) || (cursor > currentFirstCursor+fetchedTupleNumber-1))))
  {
    errorHandler.setErrorCode(UErrorCode.ER_INVALID_ARGUMENT);
    return false;
  }
  ...
  synchronized (relatedConnection) {
    instance_obj = relatedConnection.oidCmd(tuples[cursorPosition-currentFirstCursor].getOid(), UConnection.IS_INSTANCE);
  }
 ...
 * ER_OID_IS_NOT_INCLUDED error가 set된다.
 * Error Code :
 *   ER_IS_CLOSED, ER_INVALID_ARGUMENT, ER_OID_IS_NOT_INCLUDED,
...
synchronized public void deleteCursor(int cursor)
{
  ...
  if ((currentFirstCursor < 0) || (cursor < 0) || (cursor >= 0 &&
    ((cursor < currentFirstCursor) || (cursor > currentFirstCursor+fetchedTupleNumber-1)))) {
    errorHandler.setErrorCode(UErrorCode.ER_INVALID_ARGUMENT);
    return;
  }
  if (tuples[cursorPosition-currentFirstCursor].oidIsIncluded() == false) {
  ...
  synchronized (relatedConnection) {
    relatedConnection.oidCmd(tuples[cursorPosition-currentFirstCursor].getOid(), UConnection.DROP_BY_OID);
  }
  errorHandler.copyValue(relatedConnection.getRecentError());
}

/* Bind된 UStatement를 실행한다.
 * Prepare한 statement가 'q1;q2;…;qn'와 같을 경우 allExecute가 true이면
 * q1, q2, …, qn의 모든 statement가 execute되며 false일 경우 q1만 실행된다.
 * 두 경우 모두 모든 parameter가 binding되어야 한다. 그리고 allExecute가
 * true일 경우 isAsync는 언제나 false로 간주된다.
 * isSensitive가 true일 경우 sensitive하게 ResultSet을 fetch한다.
 * Error Code :
                                 ...
                                 UStatementCacheData cache_data)
{
  ...
  if (isClosed == true) {
    if (relatedConnection.brokerInfoStatementPooling() == true) {
      retry_flag = false;
      UStatement tmp_ustmt = relatedConnection.prepare(sql_stmt, prepare_flag, true);
      ...
      try {
        init(relatedConnection, tmp_ustmt.tmp_inbuffer, sql_stmt, prepare_flag, false);
      }
      catch (UJciException e) {
        e.toUError(errorHandler);
        return;
  ...
  try {
	...
	outBuffer.addNull();
      ...
      /* fetch flag */
      if (commandTypeIs == CUBRIDCommandType.SQLX_CMD_SELECT)
  ...
  if (errorHandler.getErrorCode() != UErrorCode.ER_NO_ERROR) {
	...
	return;
      ...
      errorHandler.clear();
      execute(isAsync, maxRow, maxField,
                allExecute, is_sensitive, is_scrollable, query_plan_flag, only_query_plan,
                cache_data);
      return;
    }
    else {
  ...
  totalTupleNumber = executeResult =
    ((maxFetchSize>0) && (executeResult>maxFetchSize)) ? maxFetchSize : executeResult;
  batchParameter = null;
  isSensitive = is_sensitive;


  try {
    if (commandTypeIs == CUBRIDCommandType.SQLX_CMD_SELECT && totalTupleNumber > 0) {
  ...
  for (int i=0 ; i < resultInfo.length ; i++) {
    if (resultInfo[i].statementType != CUBRIDCommandType.SQLX_CMD_SELECT) {
...
synchronized public CUBRIDOID executeInsert(boolean isAsync)
{
  ...
  if (commandTypeIs != CUBRIDCommandType.SQLX_CMD_INSERT) {
...
/* PreparedStatement의 batch execute를 위한 interface이다. Binding된
 * parameter list를 batch execute하며 각 statement의 결과는 integer array로
 * return되어 진다. result value는 각 statement가 success한 경우
 * result count(0이상)를 나타내고 fail인 경우 -3값을 가진다.
 * Prepare된 Statement가 "q1;…;qn"와 같을 때 단지 q1 statement만이 execute된다.
 * Error Code
 *   ER_IS_CLOSED, ER_NOT_BIND, ER_ILLEGAL_DATA_SIZE, ER_TYPE_CONVERSION
 *   ER_COMMUNICATION
 */
...
synchronized public UBatchResult executeBatch()
{
  ...
  try {
    synchronized (relatedConnection) {
      outBuffer.newRequest(UFunctionCode.EXECUTE_BATCH_PREPAREDSTATEMENT);
      outBuffer.addInt(serverHandler);
    ...
    for (int i=0 ; i < batchResult.getResultNumber() ; i++) {
      ...
      if (result < 0)
	batchResult.setResultError(i, result, inBuffer.readString(inBuffer.readInt(), UJCIManager.sysCharsetName));
      else {
...
/* UStatement를 execute하여 얻어진 ResultSet을 CAS로부터 fetch한다.
 * getByOid에 의해 만들어진 UStatement는 UStatement의 constructor에서
 * 이미 ResultSet을 fetch하므로 바로 return한다.
 * Error Code
 *   ER_IS_CLOSED, ER_NO_MORE_DATA, ER_ILLEGAL_DATA_SIZE, ER_INVALID_ARGUMENT,
 *   ER_COMMUNICATION
 */
...
/* ResultSet의 현재 Cursor가 위치한 Tuple의 index번째 data를 BigDecimal Type으로
 * return한다. Type conversion이 불가한 Type match인 경우 error를 set한다.
 * Error Code
 *   ER_IS_CLOSED, ER_COLUMN_INDEX, ER_NO_MORE_DATA, ER_WAS_NULL, ER_TYPE_CONVERSION
 */
 ...
 * return한다. Type conversion이 불가한 Type match인 경우 error를 set한다.
 * Error Code : ER_IS_CLOSED, ER_COLUMN_INDEX, ER_NO_MORE_DATA,
...
/* ResultSet의 현재 Cursor가 위치한 Tuple의 index번째 data를 CUBRIDOID Type으로
 * return한다. column value가 object instance인 경우 oid값만 server쪽에서
 * 넘어오는데 이 oid값을 얻을 때 사용되어진다. 그 외의 경우 type conversion
 * error를 set한다.
 * Error Code :
 *   ER_IS_CLOSED, ER_COLUMN_INDEX, ER_NO_MORE_DATA, ER_WAS_NULL, ER_NOT_OBJECT
 *   ER_TYPE_CONVERSION
 */
...
synchronized public CUBRIDOID getGloOID(int index)
{
  ...
  if (obj instanceof CUBRIDOID) {
	...
	return ((CUBRIDOID) obj);
      ...
      synchronized (relatedConnection) {
	obj = relatedConnection.oidCmd((CUBRIDOID) obj, UConnection.IS_GLO_INSTANCE);
      }
      if (obj != null)
	return ((CUBRIDOID) obj);
    }
...
/* Execute한 statement의 result count를 return한다.
 * Error Code : ER_IS_CLOSED
 */
...
/* ResultSet의 fetch direction을 알려준다.
 * Error Code : ER_IS_CLOSED
 */
 ...
 * type으로 간주하고 Boolean object로 바꾸어준다.
 * Error Code : ER_IS_CLOSED, ER_COLUMN_INDEX, ER_NO_MORE_DATA, ER_WAS_NULL
 */
 ...
 * array가 return된다.
 * 반드시 UStatement.execute()가 선행된 후 call되어야 하며 그렇지 않은 경우
 * null을 return한다.
 * Error Code : ER_IS_CLOSED
 */
...
/* UStatement가 query문이면 true를, 그렇지 않으면 false를 return한다.
 * Error Code : ER_IS_CLOSED
 */
...
public boolean getSqlType()
{
  ...
  if ((commandTypeIs == CUBRIDCommandType.SQLX_CMD_SELECT) ||
      (commandTypeIs == CUBRIDCommandType.SQLX_CMD_CALL) ||
      (commandTypeIs == CUBRIDCommandType.SQLX_CMD_GET_STATS) ||
      (commandTypeIs == CUBRIDCommandType.SQLX_CMD_EVALUATE))
  {
    errorHandler = localError;
    return true;
  }
...
/* ResultSet의 현재 Cursor가 위치한 Tuple의 index번째 data를 Timestamp Type으로
 * return한다. Type conversion이 불가한 Type match인 경우 error를 set한다.
 * Error Code : ER_IS_CLOSED, ER_COLUMN_INDEX, ER_NO_MORE_DATA, ER_WAS_NULL,
 		ER_TYPE_CONVERSION
 */
...
/* 현재 statement가 updatable인지 판단하기 위한 interface이다.
 * prepare시 isUpdatable이 true, statement type이 select이고 result set에
 * OID가 include되어져 있을 경우 true로 return되며 그렇지 않은 경우 false로
 * return된다.
 * Error Code : ER_IS_CLOSED
 */
...
/* cursor의 위치를 옮긴다.
 * synchronous query의 경우 JCI내부에서만 cursor의 위치를 움직이며 asynchronous
 * query의 경우 cursor가 현재까지 fetch된 tuple내로 움직이고자 하면 JCI내부에서
 * move하지만 fetch된 tuple의 범위를 벗어난 곳으로 move하고자 하면 CAS의
 * CURSOR function을 call하여 cursor의 move가능을 check한다.
 * Error Code : ER_IS_CLOSED, ER_NO_MORE_DATA, ER_ILLEGAL_DATA_SIZE,
		ER_COMMUNICATION
 */
...
synchronized public void moveCursor(int offset, int origin)
{
  ...
  if (origin == CURSOR_SET || origin == CURSOR_CUR) {
	...
	return;
      ...
      if (cursorPosition < totalTupleNumber)
	return;
      else{
	errorHandler.setErrorCode(UErrorCode.ER_NO_MORE_DATA);
 ...
 * ER_NO_MORE_RESULT error를 set한다.
 * Error Code : ER_IS_CLOSED, ER_NO_MORE_RESULT, ER_ILLEGAL_DATA_SIZE,
 		ER_COMMUNICATION
 */
...
/* 3.0
synchronized public boolean nextResult (int rs_mode)
{
  UInputBuffer inBuffer;
 ...
 * sensitivity는 execute시 넘겨준 flag에 의해 결정된다.
 * Error Code : ER_IS_CLOSED, ER_ILLEGAL_DATA_SIZE, ER_INVALID_ARGUMENT,
 		ER_COMMUNICATION
 */
 ...
 * static variable이어야 하며 그렇지 않은 경우 ER_INVALID_ARGUMENT error를 set한다.
 * Error Code : ER_IS_CLOSED, ER_INVALID_ARGUMENT
 */
...
/* ResultSet의 특정 attribute value를 update하기 위한 interface이다.
 * 이때 각 value들은 해당 index의 columnInfo에 저장되어 있는 type으로 간주되어
 * conversion을 시도하며 type conversion이 불가할 경우 Type Conversion Error를
 * Set한다.
 * JCI에서는 해당 column이 updatable인지 판단하지 않는다. 따라서 updatable
 * 하지 않는 column을 update하고자 한다면 server error가 발생할 것이다.
 * Error Code : ER_IS_CLOSED, ER_INVALID_ARGUMENT, ER_ILLEGAL_DATA_SIZE,
 		ER_TYPE_CONVERSION, ER_COMMUNICATION
 */
...
/* 3.0
synchronized public UParameterInfo[] getParameterInfo()
{
  UInputBuffer inBuffer;
  ...
  try {
    ...
    pramNumber = inBuffer.getResCode();
    return readParameterInfo();
  }
  catch (UJciException e) {
    e.toUError(errorHandler);
  }
...
synchronized public String getQueryplan()
{
  ...
  try {
    ...
    plan = inBuffer.readString(inBuffer.remainedCapacity(), relatedConnection.conCharsetName);
  }
  catch (UJciException e) {
    closeInternal();
    e.toUError(errorHandler);
...
private Object beforeGetXXX(int index)
{
  ...
  if ((tuples == null) ||
      ...
      ((obj = tuples[cursorPosition-currentFirstCursor].getAttribute(index)) == null))
  {
...
{
  if ((currentFirstCursor < 0) || (cursorPosition >= 0 && ((cursorPosition < currentFirstCursor) || (cursorPosition > currentFirstCursor+fetchedTupleNumber-1))))
  {
    fetch();
    if (errorHandler.getErrorCode() != UErrorCode.ER_NO_ERROR)
      return false;
...
{
  if ((commandTypeIs == CUBRIDCommandType.SQLX_CMD_CALL) ||
      (commandTypeIs == CUBRIDCommandType.SQLX_CMD_EVALUATE) ||
      (commandTypeIs == CUBRIDCommandType.SQLX_CMD_CALL_SP) ||
 ...
 * 이 method는 getSchemaInfo에 의해 얻어진 ResultSet의 Collection type
 * 정보를 나타내는 value를 check하여 collection type 정보만을 추출하여
 * 그 값을 Result set의 value로 바꾸어준다.
 * 이때 collection base type 정보는 무시된다.
 */
...
private void read_fetch_data(UInputBuffer inBuffer)
	throws IOException, UJciException
{
	...
	throws IOException, UJciException
{
  ...
  for (int i=0; i < columnNumber ; i++) {
    ...
    name = inBuffer.readString(inBuffer.readInt(), relatedConnection.conCharsetName);
    localColumnInfo[i] = new UColumnInfo(type, scale, precision, name);
    if (statementType == NORMAL) {
      String attributeName =inBuffer.readString(inBuffer.readInt(), relatedConnection.conCharsetName);
      String className =inBuffer.readString(inBuffer.readInt(), relatedConnection.conCharsetName);
      byte byteData = inBuffer.readByte();
      localColumnInfo[i].setRemainedData(attributeName, className, ((byteData == (byte)0) ? true : false));
    }
    /*
    if ((commandTypeIs==CUBRIDCommandType.SQLX_CMD_EVALUATE) ||
	(commandTypeIs==CUBRIDCommandType.SQLX_CMD_CALL))
    {
      localColumnInfo[i] = new UColumnInfo(UUType.U_TYPE_VARCHAR, (short) 0, 1073741823 , "method_result");
      localColumnInfo[i].setRemainedData("", "", false);
    }
    */
  }
...
private void readResultInfo(UInputBuffer inBuffer)
	throws UJciException
{
...
/* 3.0
private UParameterInfo[] readParameterInfo() throws UJciException
{
  byte mode;
  byte type;
  short scale;
  ...
  UParameterInfo localParameterInfo[] = new UParameterInfo[pramNumber];
    for (int i=0; i < pramNumber ; i++) {
    mode = inBuffer.readByte();
    type = inBuffer.readByte();
    scale = inBuffer.readShort();
    precision = inBuffer.readInt();
    localParameterInfo[i] = new UParameterInfo(mode, type, scale, precision);
  }
...
/*
public UStatementCacheData makeCacheData()
{
  if (fetchedTupleNumber < totalTupleNumber)
    return null;
