...
protected CUBRIDPreparedStatement(CUBRIDConnection c, UStatement us, int t, int concur, int autoGeneratedKeys)
{
  super(c, t, concur);
  u_stmt = us;
...
public ResultSet executeQuery() throws SQLException
{
  try {
	...
	if ((!first_result_type) &&
	    (u_stmt.getCommandType() != CUBRIDCommandType.SQLX_CMD_CALL_SP))
	{
...
public int executeUpdate() throws SQLException
{
  try {
	...
	executeCore(false);
	getMoreResults();
        if ( autoGeneratedKeys == Statement.RETURN_GENERATED_KEYS && u_stmt.getCommandType() == CUBRIDCommandType.SQLX_CMD_INSERT )
           MakeAutoGeneratedKeysResultSet();

	if (u_stmt.getCommandType() != CUBRIDCommandType.SQLX_CMD_CALL_SP) {
	  complete();
	}
	return update_count;
      }
...
public synchronized void setCharacterStream(int parameterIndex, Reader reader, int length) throws SQLException
{
  ...
  synchronized (u_stmt) {
    /*
    try {
      u_stmt.bindCharacterStream(parameterIndex-1, new String(value, 0, len).getBytes("ISO-8859-1"));
    }
...
public synchronized ParameterMetaData getParameterMetaData() throws SQLException
{
  ...
  UParameterInfo[] pram_info = null;
  synchronized (u_stmt) {
    pram_info = u_stmt.getParameterInfo();
    error = u_stmt.getRecentError();
  }
...
/**
 * Sets the designated parameter to a Java <code>Object[]</code> value.
 * The driver converts this to an <code>CUBRID COLLECTION</code> value
 * when it sends it to the database.
 *
 * @param parameterIndex the first parameter is 1, the second is 2, ...
 * @param x the parameter
 * @exception SQLException if <code>this</code> object is closed.
 * @exception IllegalArgumentException if <code>x</code> is not
 * <code>null</code> and all attempts to cast <code>x</code> to
 * one of the following types fail.<BR>
 ...
 * <code>CUBRIDOID</code> object that represents the OID of
 * the object inserted by the statement.
 *
 * @return a <code>CUBRIDOID</code> object that represents the OID of
 * the object inserted by the statement.
 * @exception SQLException if <code>this</code> object is closed.
 * @exception SQLException if the statement in <code>this PreparedStatement</code>
...
}  // end of class CUBRIDPreparedStatement
