...
enum STREAM_KIND
{
  ...
  /*
   * loaddb (client/server) Internal LOB payload of one batch.  The payload goes straight to the batch's
   * load worker, which writes it into the class' Internal LOB file inside the batch transaction; END
   * returns the slot number the batch text refers to.  Frames arrive tail-first, see
   * INTERNAL_LOB_LOAD_STREAM_FRAME_HEADER_SIZE.
   */
