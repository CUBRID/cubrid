		    ...
		    int num_cols, int format, int delimiter, int quote, int header, int bulk)
{
   ...
   * need to sort by def_order to get the correct column-to-attr mapping. */
  {
	...
	for (int i = 0; i < num_cols; i++)
      ...
      {
	/* No explicit column list: the incoming columns must not outnumber the table's attributes, otherwise
	 * the extra m_attr_ids entries would stay 0 and silently misplace values onto the first attribute. */
	if (num_cols > n_attrs)
	  {
	    heap_attrinfo_end (thread_p, &attrinfo);
	    er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_STREAM_SESSION_ERROR, 1,
		    "COPY column count exceeds the number of table columns");
	    return ER_STREAM_SESSION_ERROR;
	  }
	for (int i = 0; i < num_cols; i++)
	  {
...
copy_session::receive_chunk (THREAD_ENTRY *thread_p, const char *data, int data_len)
{
  ...
  while (pos < buf_len)
	...
	if (heap_attrinfo_transform_to_disk_except_lobfile (thread_p, &attrinfo, old_recdes, &new_recdes) != S_SUCCESS)
	  {
	    error = er_errid ();
	    if (error == NO_ERROR)
	      {
