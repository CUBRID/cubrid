  ...
  *consumed = sizeof (int32_t);
  ...
  switch (type)
      ...
      if (field_len < OR_INT_SIZE)
	{
       ...
       * negative-size allocation downstream. The upper bound keeps
       * OR_INT_SIZE + dim * OR_FLOAT_SIZE within INT_MAX. */
      const int32_t max_dim = (INT_MAX - OR_INT_SIZE) / OR_FLOAT_SIZE;
