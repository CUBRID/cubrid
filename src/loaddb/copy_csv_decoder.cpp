...
csv_coerce_field (const std::string &s, DB_TYPE type, DB_VALUE *val)
{
  ...
  switch (type)
    ...
    case DB_TYPE_INTEGER:
    {
      ...
      if (endp == s.c_str () || *endp != '\0' || errno == ERANGE || v < INT_MIN || v > INT_MAX)
	{
	  goto bad_value;
	}
    ...
    case DB_TYPE_BIGINT:
    {
      ...
      if (endp == s.c_str () || *endp != '\0' || errno == ERANGE)
	{
	  goto bad_value;
	}
    ...
    case DB_TYPE_VARCHAR:
      db_make_varchar (val, (int) s.size (), s.data (), (int) s.size (), LANG_SYS_CODESET, LANG_SYS_COLLATION);
      break;
    case DB_TYPE_CHAR:
      db_make_char (val, (int) s.size (), s.data (), (int) s.size (), LANG_SYS_CODESET, LANG_SYS_COLLATION);
      break;
    case DB_TYPE_DATE:
    {
      DB_DATE d;
