...
namespace cubload
{
  ...
  class session
  {
    public:
      ...
      int internal_lob_payload_begin (class_id clsid, char type, DB_BIGINT data_length, DB_BIGINT bit_length,
				      INT64 &token);
      int internal_lob_payload_append (INT64 token, const char *data, int data_size);
      int internal_lob_payload_end (INT64 token);
      int internal_lob_payload_abort (INT64 token);
    ...
    private:
      ...
      struct internal_lob_payload
      {
	class_id clsid = NULL_CLASS_ID;
	char type = '\0';
	...
	FILE *file = NULL;
      ...
      INT64 m_internal_lob_next_token;
      std::unordered_map<INT64, internal_lob_payload> m_internal_lob_payloads;
  };
