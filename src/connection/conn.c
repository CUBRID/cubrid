 ...
 *
 * conn.c - Client/Server connection list management
 *
 * When a server starts up, it tries to connect the master process which
 * manages all the server informations in the local machine. All clients
 * contact to master to connect server. Then the master process passes the
 * client connection socket descriptor to the server via inter master-server
 * connection channel. Finally the server recoginze the connected client by
 * received socket destriptor which is also used for direct client-server
 * communication channel. A css_conn_entry which makes up the css_conn_list
 * is used for one connection that is possible between server and client or
 * server and master.
 */
...
{
  fprintf (stderr,
	   "CONN_ENTRY(idx(%d),fd(%d),request_id(%d),transaction_id(%d),client_id(%d)\n",
	   conn->idx, conn->fd, conn->request_id, conn->transaction_id,
	   conn->client_id);
}
  ...
  if (css_Free_conn_anchor != NULL)
    {
      ...
      for (conn = css_Free_conn_anchor; conn != NULL; conn = next)
	{
	  next = conn->next;
	  css_print_conn_entry_info (conn);
	}
 ...
 */
void
css_register_handler_routines (int (*connect_handler)
			       (CSS_CONN_ENTRY * conn),
