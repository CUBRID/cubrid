  ...
  if (remote_ip == INADDR_NONE)
    {
      ...
      if (dest_host == NULL)
	{
	  er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_CSS_WINSOCK_HOSTNAME, 2, WSAGetLastError (), HOSTS_FILE);
      ...
      else
	{
	  ...
	  if (hp != NULL)
	  ...
	  else
	    {
	      ...
	      er_set (ER_ERROR_SEVERITY, ARG_FILE_LINE, ER_CSS_WINSOCK_HOSTID, 2, WSAGetLastError (), HOSTS_FILE);
	    }
	}
    }
