...
namespace cubhb
{
  ...
  cluster::cluster (transport *transport_)
    : lock ()
    , state (node_state ::UNKNOWN)
    , nodes ()
    , myself (NULL)
    ...
    , m_hb_service (new heartbeat_service (*transport_, *this))
    , m_group_id ()
    , m_hostname ()
  {
  ...
  cluster::init ()
  {
    ...
    if (m_hostname.as_str ().empty ())
      {
	error_code = m_hostname.fetch ();
