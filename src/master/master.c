...
static bool css_send_new_request_to_server (SOCKET server_fd,
                                            /* *INDENT-OFF**/ const std::vector<SOCKET> &client_fds /* *INDENT-ON**/ ,
 ...
 */
static bool
css_send_new_request_to_server (SOCKET server_fd, /* *INDENT-OFF**/ const std::vector<SOCKET> &client_fds /* *INDENT-ON**/ , unsigned short rid,
				CSS_SERVER_REQUEST request)
{
		  ...
		  if (ack_chn != -1)
		    {
		      ...
		      client_fds = {conn->fd, ack_chn};
                      ...
                      // *INDENT-ON*
					    ...
					    {
					    css_reject_client_request (conn, rid, SERVER_HANG);
					    free_and_init (server_name); css_free_conn (conn); return;}
#endif
					    ...
					    CSS_CONN_ENTRY * css_create_new_connection (SOCKET fd, SOCKET ack)
					    {
					    CSS_CONN_ENTRY * conn;
					    int buffer_size = sizeof (NET_HEADER);
					    css_Total_request_count++; conn = css_make_conn (fd); return conn;}
 ...
 */
					    ...
					    return;}
					    unsigned short rid;
					    int function_code; int buffer_size; if (css_check_magic (conn) != NO_ERRORS)
					    {
					    css_free_conn (conn); return;}
...
css_add_request_to_socket_queue (conn, true, NULL, conn->fd, READ_WRITE, 0, &css_Master_socket_anchor); break; case DATA_REQUEST:	/* request from a remote client */
css_send_to_existing_server (conn, -1, rid, SERVER_START_NEW_CLIENT); break; case SERVER_REQUEST:	/* request from a new server */
css_register_new_server (conn, rid); break; case SERVER_REQUEST_NEW:	/* request from a new server */
					    /* here the server wants to manage its own connection port */
css_register_new_server2 (conn, rid); break; case SERVER_REQUEST_CONNECT_NEW_SLAVE:
					    {
 ...
 */
					    ...
					    SOCKET_QUEUE_ENTRY * temp;
					    SOCKET max_fd = 0;
					    FD_ZERO (fd_var); for (temp = anchor_p; temp; temp = temp->next)
					    {
					    if (!IS_INVALID_SOCKET (temp->fd) && temp->fd_type != WRITE_ONLY)
					    {
					    FD_SET (temp->fd, fd_var); if (temp->fd > max_fd)
					    {
					    max_fd = temp->fd;}
					    }
					    }
 ...
 */
					    static int
					    css_enroll_exception_sockets (SOCKET_QUEUE_ENTRY * anchor_p,
									  fd_set * fd_var)
					    {
					    SOCKET_QUEUE_ENTRY * temp;
					    SOCKET max_fd = 0;
 ...
 */
					    ...
					    {
#if !defined(WINDOWS)
int rv; SOCKET_QUEUE_ENTRY * temp; again:
					    rv = pthread_mutex_lock (&css_Master_socket_anchor_lock);
					    for (temp = css_Master_socket_anchor; temp; temp = temp->next)
					    {
					    ...
					    {
					    css_remove_entry_by_conn (temp->conn_ptr, &css_Master_socket_anchor);}
					    pthread_mutex_unlock (&css_Master_socket_anchor_lock); goto again;}
					    }
					    pthread_mutex_unlock (&css_Master_socket_anchor_lock);
#endif
					    }
 ...
 */
					    ...
					    int rv;
#endif
					    SOCKET_QUEUE_ENTRY * temp, *next; SOCKET new_fd;
#if !defined(WINDOWS)
					    rv = pthread_mutex_lock (&css_Master_socket_anchor_lock);
#endif
					    ...
					    {
					    new_fd = css_master_accept (temp->fd); if (!IS_INVALID_SOCKET (new_fd))
					    {
 ...
 */
					    ...
					    int rc, run_code; run_code = 1; while (run_code)
					    {
					    ...
					    {
case 0:
run_code = css_master_timeout (); break; case -1:
					    /* switch error */
css_master_select_error (); break; default:
					    css_check_master_socket_input (&rc, &read_fd);
					    css_check_master_socket_output ();
					    run_code = css_check_master_socket_exception (&exception_fd); break;}
					    }
					    }
 ...
 */
					    ...
					    const char *msg_format; if (utility_initialize () != NO_ERROR)
					    {
					    return EXIT_FAILURE;}

#if defined(WINDOWS)
					    FreeConsole (); if (css_windows_startup () < 0)
					    {
					    ...
					    strcat (hostname, suffix); errlog = hostname;}

					    if (er_init (errlog, ER_NEVER_EXIT) != NO_ERROR)
					    {
					    PRINT_AND_LOG_ERR_MSG ("Failed to initialize error manager.\n");
					    status = EXIT_FAILURE; goto cleanup;}
					    if (master_util_config_startup ((argc > 1) ? argv[1] : NULL, &port_id) ==
						false)
					    {
					    msg_format =
					    msgcat_message (MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_MASTER,
							    MASTER_MSG_NO_PARAMETERS); css_master_error (msg_format);
					    util_log_write_errstr (msg_format); status = EXIT_FAILURE; goto cleanup;}
					    ...
					    TPRINTF (msgcat_message
						     (MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_MASTER,
						      MASTER_MSG_STARTING), 0);
					    /* close the message catalog and let the master daemon reopen. */
					    (void) msgcat_final (); er_final (ER_ALL_FINAL);
#if !defined(WINDOWS)
					    ...
					    css_master_error (msgcat_message
							      (MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_MASTER,
							       MASTER_MSG_PROCESS_ERROR)); status = EXIT_FAILURE;
					    goto cleanup;}
...
conn = css_make_conn (css_Master_socket_fd[0]); css_add_request_to_socket_queue (conn, false, NULL, css_Master_socket_fd[0], READ_WRITE, 0, &css_Master_socket_anchor); conn = css_make_conn (css_Master_socket_fd[1]); css_add_request_to_socket_queue (conn, false, NULL, css_Master_socket_fd[1], READ_WRITE, 0, &css_Master_socket_anchor); css_master_loop (); css_master_cleanup (SIGINT); css_master_error (msgcat_message (MSGCAT_CATALOG_UTILS, MSGCAT_UTIL_SET_MASTER, MASTER_MSG_EXITING)); cleanup:
					    ...
					    ();
#endif /* WINDOWS */
					    msgcat_final (); er_final (ER_ALL_FINAL); return status;}
 ...
 */
					    ...
					    {
					    SOCKET_QUEUE_ENTRY * p, *q; if (conn_p == NULL)
					    {
					    return;}
 ...
 */
					    ...
					    p->conn_ptr = conn_p; p->fd = fd; p->ha_mode = FALSE; if (name_p)
					    {
					    p->name = (char *) malloc (strlen (name_p) + 1); if (p->name)
					    {
					    ...
					    p->pid = pid;
					    p->db_error = 0;
					    p->next = *anchor_p; p->port_id = -1; *anchor_p = p; return p;}
 ...
 */
					    SOCKET_QUEUE_ENTRY *
					    css_return_entry_of_server (char *name_p, const size_t name_len,
									SOCKET_QUEUE_ENTRY * anchor_p)
					    {
					    SOCKET_QUEUE_ENTRY * p; assert (name_len >= 0); if (name_p == NULL)
					    {
					    return NULL;}
 ...
 */
					    ...
					    css_return_entry_by_conn (CSS_CONN_ENTRY * conn_p,
								      SOCKET_QUEUE_ENTRY ** anchor_p)
					    {
					    SOCKET_QUEUE_ENTRY * p; if (conn_p == NULL)
					    {
					    return NULL;}
...
setsid (); out:
   ...
   */
					    ...
					    fd_max = css_get_max_socket_fds (); for (fd = 3; fd < fd_max; fd++)
					    {
					    close (fd);}
