...
cibadmin_exec()
{
	$CIBADMIN -V -o constraints -C -x ./constraints-reply-add-s2.xml.TEMP
	sleep 3 

	$CIBADMIN -V -o resources -C -x ./resources-reply-add-s2.xml.TEMP
	sleep 3 

	$CIBADMIN -V -o resources -U -x ./resources-reply-inc-max-clone-s2.xml.TEMP
	sleep 5

	$CIBADMIN -V -o resources -U -x ./resources-reply-start-s2.xml.TEMP
}
...
do
  sleep 10
  ONLINE_NODES=`crm_mon | grep online | awk '{print $2}'`
