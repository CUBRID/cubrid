...
AS_HELP_STRING([--with-cubrid],[Include CUBRID support]))

PHP_ARG_WITH(cubrid-includedir, for cubrid include path,
AS_HELP_STRING([--with-cubrid-includedir=PATH], [Include CUBRID library path]),no,no)

PHP_ARG_WITH(cubrid-libdir, for cubrid library path,
AS_HELP_STRING([--with-cubrid-libdir=PATH],[Include CUBRID include path]),no,no)

PHP_ARG_ENABLE(64bit, for cubrid 64 bit code,
AS_HELP_STRING([--enable-64bit],[build 64 bit module @<:@default=no@:>@]),no,no)
