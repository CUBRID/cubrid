...
namespace test_lockfree
{
  namespace advp
  {
    ...
    run ()
    {
      ...
      // through offsets; the rewrite wraps the entry in a freelist node that adds a vtable pointer, the retire
      // link, the retire id, the owning freelist and the entry descriptor. every bucket chain walk therefore
      // touches more bytes per node in the rewrite than in legacy.
      say ("node footprint: entry=%zu bytes; legacy chain node=%zu bytes;"
	   " rewrite chain node >= %zu bytes (reclaimable_node=%zu + freelist* + entry + edesc*)",
	   sizeof (adv_entry), sizeof (adv_entry),
	   sizeof (lockfree::tran::reclaimable_node) + sizeof (void *) + sizeof (adv_entry) + sizeof (void *),
