...
namespace test_lockfree
{
  ...
  //
  // Calibrated: with hashmap's entry_uninit policy made to skip f_uninit, this
  ...
  test_adversarial_functional ()
  {
    ...
    if (ran == 0)
      {
	// a stale LFADV_ONLY matches nothing, and a suite that tested nothing must not report success
	test_common::sync_cout ("LFADV_ONLY matched no case - nothing ran\n");
	err = 1;
      }
  ...
  // Calibrated: hashmap::init () made to ignore edesc.max_alloc_cnt. Detected without amplification - this
  // one is a plain functional property, not a race.
  static int
  case_max_alloc_cap ()
  {
    const int CAP = 128;
  ...
  // Calibration: NOT detected by the amplified by-value read that case_insert_given_promote (),
  // case_index_churn (), case_shared_transys () and case_alloc_cap_under_load () all catch. That is the
  // answer this case was written to get, not a missing power: the mutex path re-checks the delete mark after
  // locking and restarts, so it really is immune to that defect, and find_or_insert () and erase_locked ()
  // are covered here for the first time. Treat a green run as coverage, not as a regression guard - nothing
  // is known to make this case fail.
  static int
  ...
  // Calibrated: same amplified by-value read, with the cap engaged. Reports "a held entry changed identity
  // with the allocation cap engaged" - which is the combination that turns an epoch hole into a real
  // use-after-free, because over the cap the node is freed rather than recycled.
  static int
  ...
  // Calibrated: same amplified by-value read. Reports "four maps on one transaction system did not behave as
  // one map does".
  static int
  case_shared_transys ()
  {
  ...
  // Calibrated: same amplified by-value read. Reports "an entry held under this load was not the entry that
  // was asked for". Note what this says about the unamplified suite - the defect is there either way, and a
  // green run without the spin means the race did not happen, not that it cannot.
  static int
  case_index_churn ()
  {
  ...
  // Calibrated: iterate () made to advance the bucket index by two. Reports "an iterator pass did not return
  // the resident set exactly once". Neither case_iterator_edges () nor case_iterate_under_churn () sees that
  // fault, which is the gap this case was written for.
  static int
  ...
  // Calibrated: list_find () made to take the bucket head by value again, with a 20000-iteration spin between
  // that read and start_tran () - the shape PR 7733 measured at 96 records amplified against 0 in 30 runs
  // unamplified. Reports "a held entry changed identity on one of the two insert paths".
  static int
