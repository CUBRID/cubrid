...
namespace Al{
    ...
    public:
        ...
        //--------------------------------------------------------------------------------
        ~StackAllocator(){//can be called explicitly to reinitialize: a.~StackAllocator();
            _0 = _ptr;
        }
        ...
        void deallocate(Blk blk){
            //assert(owns(blk));
            if((char*)blk.ptr+blk.dim == _0){//last allocated block
                _0 = blk.ptr;
            }
        }

        //--------------------------------------------------------------------------------
        bool owns(Blk blk){// test if blk in [_0, _1)
        ...
        //--------------------------------------------------------------------------------
        Blk realloc(Blk blk, size_t size){//fit additional size bytes; extend block if possible
            if(blk.ptr+blk.dim==_0 && blk.ptr+blk.dim+size<=_1){//last allocated block & enough space to extend
                blk.dim += size;
                _0 += size;
                return blk;
            }
