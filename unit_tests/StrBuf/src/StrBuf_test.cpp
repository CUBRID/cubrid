...
public:
    ...
    void operator()(size_t size){//prepare for a test with a buffer of <size> bytes
        if(_dim < size){//adjust internal buffer is necessary
            do{
                _dim *= 2; 
            }while(_dim < size);
    ...
    template<size_t Size, typename...Args> void operator()(const char* file, int line, const char (&format)[Size], Args&&...args){
    ...
    void operator()(const char* file, int line, char ch){
        if(_len+1 < _blk.dim){//include also '\0'
            _ref[_len] = ch;
            _ref[_len+1] = '\0';
        }
        ++_len;
...
int main(int argc, char** argv){
    enum Flags{flDEBUG=(1<<0), flTIME=(1<<1)};
    unsigned flags = 0;
