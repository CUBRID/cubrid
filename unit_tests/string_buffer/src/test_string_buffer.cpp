...
public:
    test_string_buffer ():m_buf (), m_dim (1024), m_len (0),
    m_ref ((char *) calloc (m_dim, 1)), m_stack_allocator (m_buf,
							   sizeof (m_buf)),
    m_affix_allocator (m_stack_allocator), m_block (), m_sb ()
  {
  }
  ...
  template < size_t Size, typename...Args >
    void operator  () (const char *file, int line, const char (&format)[Size],
		       Args && ... args)
  {
    int len =
      snprintf (m_ref + m_len, m_len < m_block.dim ? m_block.dim - m_len : 0,
		format, args...);
    if (len < 0)
      {
	ERR ("[%s(%d)] StrBuf([%zu]) snprintf()=%d", file, line, m_block.dim,
	     len);
	return;
      }
    else
    ...
    if (m_sb.len () != m_len)
      {
	ERR ("[%s(%d)] StrBuf([%zu]) len()=%zu expect %zu", file, line,
	     m_block.dim, m_sb.len (), m_len);
	return;
      }
    if (strcmp (m_block.ptr, m_ref))
      {
	ERR ("[%s(%d)] StrBuf([%zu]) {\"%s\"} expect{\"%s\"}", file, line,
	     m_block.dim, m_block.ptr, m_ref);
	return;
      }
    if (m_affix_allocator.check (m_block))
      {
	ERR ("[%s(%d)] StrBuf(buf[%zu]) memory corruption", file, line,
	     m_block.dim);
	return;
      }
  }
  ...
  void operator  () (const char *file, int line, char ch)
  {
    ...
    if (m_affix_allocator.check (m_block))
      {
	ERR ("[%s(%d)] StrBuf([%zu]) memory corruption", file, line,
	     m_block.dim);
      }
  }
};
