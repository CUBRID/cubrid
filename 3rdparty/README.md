...
- libexpat v2.8.2 (Linux; built from source)
- Editline (libedit) for CUBRID version, https://github.com/CUBRID/libedit (Linux Only)
- OpenSSL v3.5.7 (Linux; built from source. Windows links the prebuilt OpenSSL
  1.1.1f under `win/3rdparty/openssl`, which is not upgraded by this build)
- unixODBC v2.3.14 (Linux only)
- LZ4 v1.10.0 (Linux; built from source. Windows links the prebuilt LZ4 1.9.2
  under `win/3rdparty/lz4`)
