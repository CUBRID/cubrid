...
public class QueryPlan extends Dialog {
	...
	private String class_modify(String str, String word) {
		...
		if (split[1].length() == 1)
			r_str = str.substring(0, str.length() - 2) + split[1];
		else {
			String temp = str
					.substring(0, str.length() - split[2].length() - 1);
			r_str = temp + split[1] + " " + split[2];
		}
	...
	private String node_modify(String str, String word) {
		...
		if (split[1].length() == 1)
			r_str = str.substring(0, str.length() - 2) + split[1] + " (card="
					+ split[2] + ",page=" + split[3] + ")";
		else {
			String temp = str
					.substring(0, str.length() - split[2].length() - 1);
			r_str = temp + split[1] + " " + split[2] + " (card=" + split[3]
					+ ",page=" + split[4] + ")";
		}
