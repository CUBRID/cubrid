...
#7990 commit `2c792b423ccac027c41317c0ba28e26f5336870d`.
...
image fails with `ER_OOS_HISTORY_ACTIVATION_REQUIRED` (-1387). This includes
...
whose original storage still exists, with `ER_CDC_LEGACY_OOS_IMAGE` (-1389)
without reading the referenced storage. Malformed expanded images return
`ER_CDC_INVALID_HISTORY_IMAGE` (-1390). CDC reports an extraction failure and
