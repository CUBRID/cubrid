...
/* Convert the big integer A into the external representation
   ...
   has been passed as NULL. */
     gcry_error_t gcry_mpi_print (enum gcry_mpi_format format,
				  unsigned char *buffer, size_t buflen, size_t * nwritten, const gcry_mpi_t a);
...
/* Convert the big integer A into the external representation described
   ...
   external representation. */
     gcry_error_t gcry_mpi_aprint (enum gcry_mpi_format format,
				   unsigned char **buffer, size_t * nwritten, const gcry_mpi_t a);
...
/* W = B ^ E mod M. */
     void gcry_mpi_powm (gcry_mpi_t w, const gcry_mpi_t b, const gcry_mpi_t e, const gcry_mpi_t m);
...
/* Decode and store VALUE into RESULT.  */
     gpg_error_t gcry_mpi_ec_decode_point (gcry_mpi_point_t result, gcry_mpi_t value, gcry_ctx_t ctx);
...
/* Store NBITS of the value P points to in A and mark A as an opaque
   ...
   WARNING: Never use an opaque MPI for anything thing else than
   gcry_mpi_release, gcry_mpi_get_opaque. */
     gcry_mpi_t gcry_mpi_set_opaque_copy (gcry_mpi_t a, const void *p, unsigned int nbits);
...
/* Check authentication tag for AEAD modes/ciphers.  */
     gcry_error_t gcry_cipher_checktag (gcry_cipher_hd_t hd, const void *intag, size_t taglen);
...
/* Set counter for CTR mode.  (CTR,CTRLEN) must denote a buffer of
   block size length, or (NULL,0) to set the CTR to the all-zero block. */
     gpg_error_t gcry_cipher_setctr (gcry_cipher_hd_t hd, const void *ctr, size_t ctrlen);
...
/* Return the name of the curve matching KEY.  */
     const char *gcry_pk_get_curve (gcry_sexp_t key, int iterator, unsigned int *r_nbits);
...
/* Create a MAC handle for algorithm ALGO.  FLAGS may be given as an bitwise OR
   ...
   associated with HANDLE.  */
     gcry_error_t gcry_mac_open (gcry_mac_hd_t * handle, int algo, unsigned int flags, gcry_ctx_t ctx);
...
/* Pass LENGTH bytes of data in BUFFER to the MAC object HD so that
   it can update the MAC values.  */
     gcry_error_t gcry_mac_write (gcry_mac_hd_t hd, const void *buffer, size_t length);
...
/* Verify the final authentication code from the MAC object HD with BUFFER. */
     gcry_error_t gcry_mac_verify (gcry_mac_hd_t hd, const void *buffer, size_t buflen);
...
/* Return NBYTES of allocated random using a random numbers of quality
   LEVEL. */
     void *gcry_random_bytes (size_t nbytes, enum gcry_random_level level) _GCRY_GCC_ATTR_MALLOC;

/* Return NBYTES of allocated random using a random numbers of quality
   LEVEL.  The random numbers are created returned in "secure"
   memory. */
     void *gcry_random_bytes_secure (size_t nbytes, enum gcry_random_level level) _GCRY_GCC_ATTR_MALLOC;
...
/* Generate a new prime number of PRIME_BITS bits and store it in
   ...
   the prime number generation process.  */
     gcry_error_t gcry_prime_generate (gcry_mpi_t * prime,
				       ...
				       void *cb_arg, gcry_random_level_t random_level, unsigned int flags);
...
/* Log data using Libgcrypt's own log interface.  */
     ...
     void gcry_log_debugpnt (const char *text, gcry_mpi_point_t point, gcry_ctx_t ctx);
...
/* Register a custom memory allocation functions. */
     void gcry_set_allocation_handler (gcry_handler_alloc_t func_alloc,
				       ...
				       gcry_handler_realloc_t func_realloc, gcry_handler_free_t func_free);
