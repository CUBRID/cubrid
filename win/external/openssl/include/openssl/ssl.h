...
#define SSL_CTX_sess_cache_full(ctx) \
  ...
  void SSL_CTX_sess_set_get_cb (SSL_CTX * ctx,
				SSL_SESSION * (*get_session_cb) (struct ssl_st
								 * ssl, const unsigned char *data, int len, int *copy));
  ...
  void SSL_CTX_set_cookie_verify_cb (SSL_CTX * ctx,
				     int (*app_verify_cookie_cb) (SSL * ssl,
								  const unsigned
								  char *cookie, unsigned int cookie_len));
...
#define PSK_MAX_PSK_LEN 256
  ...
  typedef int (*SSL_psk_find_session_cb_func) (SSL * ssl,
					       const unsigned char *identity, size_t identity_len, SSL_SESSION ** sess);
 ...
 */
  DEFINE_STACK_OF_CONST (SSL_CIPHER) DEFINE_STACK_OF (SSL_COMP)
/* compatibility */
...
#define SSL_COMP_free_compression_methods() while(0) continue
  ...
  int SSL_bytes_to_cipher_list (SSL * s, const unsigned char *bytes, size_t len,
				int isv2format, STACK_OF (SSL_CIPHER) ** sk, STACK_OF (SSL_CIPHER) ** scsvs);
